...
| Non-loopback bind only with `-botapiBind <addr>` **and** `-botapiToken <secret>` | Remote bots (e.g. a GPU box) are possible but never accidental |
...
- Frame = `uint32 length (LE) | uint8 kind | payload`.
...
```
C→S  hello {protocol: "zh-bot/1", client: "<name/version>", want: {mode, encodings, timing}}
