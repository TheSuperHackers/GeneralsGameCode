...
{
	// sanity
	DEBUG_ASSERTCRASH(m_fileFP != NULL, ("XferLoad::skip - file pointer for '%s' is NULL", m_identifier.str()));

	// sanity
	DEBUG_ASSERTCRASH(dataSize >= 0, ("XferLoad::skip - dataSize '%d' must be greater than 0", dataSize));
...
{
	// sanity
	DEBUG_ASSERTCRASH(m_fileFP != NULL, ("XferLoad - file pointer for '%s' is NULL", m_identifier.str()));
