    ...
    const char *strToCat)
{
    ...
    if (m_data && m_data->m_refCount == 1 && m_data->m_numCharsAllocated >= numCharsNeeded)
    {
        ...
        if (strToCopy)
            // TheSuperHackers @fix Mauller 04/04/2025 Replace strcpy with safer memmove as memory regions can overlap when
...
{
    // DEBUG_ASSERTCRASH(isMemoryManagerOfficiallyInited(), ("Initializing AsciiStrings prior to main (ie, as static vars)
    // can cause memory leak reporting problems. Are you sure you want to do this?"));
