    ...
    Bool overwrite)
{
    ...
    while (it != filenameList.end())
    {
        ...
        if (dirInfo->m_files.find(token) == dirInfo->m_files.end() || overwrite)
        {
            ...
            path2 = debugpath;
            path2.concat(token);
            //			DEBUG_LOG(("ArchiveFileSystem::loadIntoDirectoryTree - adding file %s, archived in %s", path2.str(),
            //archiveFilename.str()));
...
void ArchiveFileSystem::loadMods()
{
    if (TheGlobalData->m_modBIG.isNotEmpty())
    {
        ...
        if (archiveFile != NULL)
        {
            ...
            DEBUG_LOG(
                ("ArchiveFileSystem::loadMods - %s inserted into the archive file map.", TheGlobalData->m_modBIG.str()));
        }
        else
        {
    ...
    Bool searchSubdirectories) const
{
    ...
    {
        it->second
            ->getFileListInDirectory(currentDirectory, originalDirectory, searchName, filenameList, searchSubdirectories);
        it++;
    }
}
