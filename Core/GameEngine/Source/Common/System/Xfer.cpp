...
void Xfer::xferVersion(XferVersion *versionData, XferVersion currentVersion)
{
	...
	{
		DEBUG_CRASH(("XferVersion - Unknown version '%d' should be no higher than '%d'", *versionData, currentVersion));
...
void Xfer::xferScienceVec(ScienceVec *scienceVec)
{
	...
	else if (getXferMode() == XFER_LOAD)
	{
		...
		if (scienceVec->empty() == FALSE)
		{
			...
			// Homework for today.  Write 2000 words reconciling "Your code must never crash" with "Intentionally putting crashes
			// in the code".  Fucktard.
...
void Xfer::xferUpgradeMask(UpgradeMaskType *upgradeMaskData)
{
	...
	if (getXferMode() == XFER_SAVE)
	{
		...
		for (upgradeTemplate = TheUpgradeCenter->firstUpgradeTemplate(); upgradeTemplate;
