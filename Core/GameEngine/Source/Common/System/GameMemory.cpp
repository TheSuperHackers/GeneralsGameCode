...
private:
    ...
    MemoryPoolSingleBlock
        *m_nextBlock; ///< if m_owningBlob is nonnull, this points to next free (unallocated) block in the blob; if
                      ///< m_owningBlob is null, this points to the next used (allocated) raw block in the pool.
#ifdef MPSB_DLINK
    MemoryPoolSingleBlock
        *m_prevBlock; ///< if m_owningBlob is nonnull, this points to prev free (unallocated) block in the blob; if
                      ///< m_owningBlob is null, this points to the prev used (allocated) raw block in the pool.
#endif
#ifdef MEMORYPOOL_CHECKPOINTING
...
Int MemoryPoolSingleBlock::debugSingleBlockReportLeak(const char *owner)
{
    ...
    {
        DEBUG_LOG(
            ("Leaked a block of size %d, tagstring %s, from pool/dma %s", m_logicalSize, m_debugLiteralTagString, owner));
    }
...
/*static*/ void MemoryPool::debugPoolInfoReport(MemoryPool *pool, FILE *fp)
{
    ...
    {
        DEBUG_LOG(
            ("%s,%32s,%6s,%6s,%6s,%6s,%6s,%6s", PREPEND, "POOLNAME", "BLKSZ", "INIT", "OVRFL", "USED", "TOTAL", "PEAK"));
        if (fp)
            fprintf(
                fp,
...
void initMemoryManager()
{
    if (TheMemoryPoolFactory == NULL)
    {
        ...
        TheMemoryPoolFactory =
            new (::sysAllocateDoNotZero(sizeof(MemoryPoolFactory))) MemoryPoolFactory; // will throw on failure
        TheMemoryPoolFactory->init(); // will throw on failure
        TheDynamicMemoryAllocator =
            TheMemoryPoolFactory->createDynamicMemoryAllocator(numSubPools, pParms); // will throw on failure
