...
static const BlockParse theTypeTable[] = {
    ...
    {"MultiplayerStartingMoneyChoice",
     INI::parseMultiplayerStartingMoneyChoiceDefinition},
...
//-------------------------------------------------------------------------------------------------
UnsignedInt INI::loadDirectory(AsciiString dirName, INILoadType loadType,
                               Xfer *pXfer, Bool subdirs) {
  UnsignedInt filesRead = 0;
  ...
  try {
    ...
    dirName.concat('\\');
    TheFileSystem->getFileListInDirectory(dirName, "*.ini", filenameList,
                                          subdirs);
  ...
  try {
    ...
    while (m_endOfFile == FALSE) {
      ...
      if (token) {
        ...
        if (parse) {
#ifdef DEBUG_CRASHING
          static_assert(ARRAY_SIZE(m_curBlockStart) >= ARRAY_SIZE(m_buffer),
          ...
          try {
            ...
            snprintf(buff, ARRAY_SIZE(buff),
                     "Error parsing INI file '%s' (Line: '%s')\n",
 ...
 *
 * TheSuperHackers @performance xezon 18/01/2026 The file contents are now read
 * directly from a full File Ram buffer into the INI Line Buffer without a third
 * buffer in between.
 */
//-------------------------------------------------------------------------------------------------
void INI::readLine(void) {
  ...
  if (s_xfer) {
    ...
    // DEBUG_LOG(("Xfer val is now 0x%8.8X in %s, line %s", ((XferCRC
...
void INI::parseAsciiStringVector(INI *ini, void * /*instance*/, void *store,
  ...
  asv->clear();
  for (const char *token = ini->getNextTokenOrNull(); token != nullptr;
  ...
  if (token != nullptr) {
    if (token[0] != '\"') {
      ...
      if (token) {
        ...
        Int len = strlen(buff);
        if (len &&
            buff[len - 1] == '"') { // strip off trailing quote jba. [2/12/2003]
          buff[len - 1] = 0;
        }
        result.set(buff);
      }
...
void INI::parseBitString32(INI *ini, void * /*instance*/, void *store,
  ...
  if (flagList == nullptr || flagList[0] == nullptr) {
    DEBUG_ASSERTCRASH(
        flagList, ("INTERNAL ERROR! parseBitString32: No flag list provided!"));
...
void INI::parseWeaponTemplate(INI *ini, void * /*instance*/, void *store,
  ...
  DEBUG_ASSERTCRASH(tt || stricmp(token, "None") == 0,
                    ("WeaponTemplate %s not found!", token));
  // assign it, even if null!
...
void INI::parseParticleSystemTemplate(INI *ini, void * /*instance*/,
  ...
  DEBUG_ASSERTCRASH(pSystemT || stricmp(token, "None") == 0,
                    ("ParticleSystem %s not found!", token));
...
void INI::parseObjectCreationList(INI *ini, void * /*instance*/, void *store,
  ...
  DEBUG_ASSERTCRASH(ocl || stricmp(token, "None") == 0,
                    ("ObjectCreationList %s not found!", token));
  // assign it, even if null!
...
void INI::initFromINIMulti(void *what,
  ...
  while (!done) {
    ...
    if (done == FALSE && INI::isEOF() == TRUE) {
      ...
      DEBUG_ASSERTCRASH(
          0, ("Error parsing block '%s', in INI file '%s'.  Missing '%s' token",
...
void INI::parseDamageTypeFlags(INI *ini, void * /*instance*/, void *store,
  ...
  for (const char *token = ini->getNextToken(); token;
    ...
    if (token[0] == '+') {
      DamageType dt =
          (DamageType)DamageTypeFlags::getSingleBitFromName(token + 1);
