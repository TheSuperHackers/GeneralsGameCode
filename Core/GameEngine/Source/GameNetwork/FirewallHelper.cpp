  ...
  while (tries--) {
    ...
    */
    ...
    if (result) {
      ...
      DEBUG_LOG(("FirewallHelperClass::getNextTemporarySourcePort - failed to "
                 "open socket on port %d",
                 return_port));
    }
  }
...
Bool FirewallHelperClass::sendToManglerFromPort(UnsignedInt address,
  ...
  */
  ...
  /*
          ...
          for (Int i = 0; i < sizeof(ManglerData); ++i) {
                  DEBUG_LOG_RAW(("%02x", *(((unsigned char *)(&(packet.data))) +
     i)));
          }
          DEBUG_LOG_RAW(("\n"));
  */
  ...
  */
  ...
  DEBUG_LOG(("FirewallHelperClass::sendToManglerFromPort - Sending from port "
             "%d to %d.%d.%d.%d:%d",
             (UnsignedInt)port, PRINTF_IP_AS_4_INTS(address), MANGLER_PORT));
  /*
  ...
  */
  ...
  if (spareSocket == nullptr) {
    DEBUG_ASSERTCRASH(spareSocket != nullptr,
                      ("Could not find spare socket for send."));
    DEBUG_LOG(("FirewallHelperClass::sendToManglerFromPort - failed to find "
               "the spare socket for port %d",
               port));
    return FALSE;
  }
...
SpareSocketStruct *
FirewallHelperClass::findSpareSocketByPort(UnsignedShort port) {
  DEBUG_LOG(("FirewallHelperClass::findSpareSocketByPort - trying to find "
             "spare socket with port %d",
             port));
  for (Int i = 0; i < MAX_SPARE_SOCKETS; ++i) {
      ...
      if (retval > 0) {
        ...
        if (crc.get() != htonl(message->data.CRC)) {
          DEBUG_LOG(("FirewallHelperClass::getManglerResponse - Saw message, "
                     "CRC mismatch.  Expected CRC %u, computed CRC %u",
                     message->data.CRC, crc.get()));
          continue;
        }
        byteAdjust(&(message->data));
        message->length = retval;
        DEBUG_LOG(("FirewallHelperClass::getManglerResponse - Saw message of "
                   "%d bytes from mangler %d on port %u",
                   retval, i, m_spareSockets[i].port));
        DEBUG_LOG(("FirewallHelperClass::getManglerResponse - Message has "
                   "packet ID %d 0x%08X, looking for packet id %d 0x%08X",
                   message->data.PacketID, message->data.PacketID, packetID,
                   packetID));
        if (message->data.PacketID == packetID) {
          DEBUG_LOG(("FirewallHelperClass::getManglerResponse - packet ID's "
                     "match, returning message"));
          msg = message;
          message->length = 0;
        }
        if (ntohs(message->data.PacketID) == packetID) {
          DEBUG_LOG(("FirewallHelperClass::getManglerResponse - NETWORK BYTE "
                     "ORDER packet ID's match, returning message"));
          msg = message;
          message->length = 0;
        }
      }
...
Bool FirewallHelperClass::detectionBeginUpdate() {
  ...
  */
  if (TheGlobalData->m_firewallSendDelay) {
    ...
    addbehavior |= (UnsignedInt)m_behavior;
    m_behavior = (FirewallBehaviorType)addbehavior;
    DEBUG_LOG(("FirewallHelperClass::detectionBeginUpdate - Netgear bug "
               "specified by command line or SendDelay flag"));
  } else {
    DEBUG_LOG(("FirewallHelperClass::detectionBeginUpdate - Netgear bug not "
               "specified"));
  }
  ...
  */
  if (m_mangledPorts[0] == 0 || m_mangledPorts[0] == m_sparePorts[0]) {
    if (m_mangledPorts[0] == m_sparePorts[0]) {
      m_sourcePortAllocationDelta = 0;
      DEBUG_LOG(("FirewallHelperClass::detectionTest1Update - Non-mangled "
                 "response from mangler, quitting test."));
    }
    if ((m_mangledPorts[0] == 0) &&
        ((timeGetTime() - m_timeoutStart) < m_timeoutLength)) {
      // we are still waiting for a response and haven't timed out yet.
      DEBUG_LOG(("FirewallHelperClass::detectionTest1Update - waiting for "
    ...
    if ((m_mangledPorts[0] == 0) &&
      ...
      DEBUG_LOG(("FirewallHelperClass::detectionTest1Update - timed out "
                 "waiting for response from mangler."));
    }
...
Bool FirewallHelperClass::detectionTest2Update() {
  ...
  if (m_mangledPorts[1] == 0) {
    ...
    DEBUG_LOG(("FirewallHelperClass::detectionTest2Update - timed out waiting "
               "for mangler response"));
    m_currentState = DETECTIONSTATE_DONE;
    return TRUE;
  }
  ...
  */
  if (m_mangledPorts[1] == 0 || m_mangledPorts[1] == m_sparePorts[0]) {
    ...
    if (m_mangledPorts[1] == 0) {
      DEBUG_LOG(("FirewallHelperClass::detectionTest2Update - got no response "
                 "from mangler"));
    } else {
      DEBUG_LOG(("FirewallHelperClass::detectionTest2Update - got a mangler "
                 "response, no port mangling"));
    }
    DEBUG_LOG(("FirewallHelperClass::detectionTest2Update - Setting behavior "
               "to SIMPLE, done testing"));
    return TRUE;
  }

  if (m_mangledPorts[0] == m_mangledPorts[1]) {
    DEBUG_LOG(("FirewallHelperClass::detectionTest2Update - port mangling "
               "doesn't depend on destination IP, setting to DUMB_MANGLING"));
    UnsignedInt addBehavior = (UnsignedInt)FIREWALL_TYPE_DUMB_MANGLING;
    addBehavior |= (UnsignedInt)m_behavior;
    m_behavior = (FirewallBehaviorType)addBehavior;
  } else {
    DEBUG_LOG(("FirewallHelperClass::detectionTest2Update - port mangling "
               "depends on destination IP, setting to SMART_MANGLING"));
  ...
  */
  if (m_currentTry < 3) {
    ...
    for (; i < NUM_TEST_PORTS; i++) {
      ...
      if (!openSpareSocket(m_sparePorts[i])) {
        ...
        */
        ...
        DEBUG_LOG(("FirewallHelperClass::detectionTest3Update - Failed to open "
                   "all the spare sockets, bailing test"));
        m_currentState = DETECTIONSTATE_DONE;
        return TRUE;
      }
    }
    ...
    */
    ...
    DEBUG_LOG(("FirewallHelperClass::detectionTest3Update - Waiting for "
               "mangler responses"));
    return FALSE;
  }

  DEBUG_LOG(("FirewallHelperClass::detectionTest3Update - Failed to complete "
             "test, quitting"));
  m_currentState = DETECTIONSTATE_DONE;
  return TRUE;
}
...
Bool FirewallHelperClass::detectionTest3WaitForResponsesUpdate() {
  ...
  if (m_numResponses < NUM_TEST_PORTS) {
    if ((timeGetTime() - m_timeoutStart) > m_timeoutLength) {
      ...
      */
      ...
      DEBUG_LOG(("FirewallHelperClass::detectionTest3WaitForResponsesUpdate - "
                 "timed out waiting, bailing test"));
      m_currentState = DETECTIONSTATE_DONE;
      return TRUE;
    }
    return FALSE;
  ...
  */
  if (m_numResponses < 4) {
    ...
    DEBUG_LOG(("FirewallHelperClass::detectionTest3WaitForResponsesUpdate - "
               "didn't get enough responses, using %d as the source port "
               "allocation delta, finished test",
  ...
  if (delta) {
    ...
    */
    ...
    DEBUG_LOG(("FirewallHelperClass::detectionTest3WaitForResponsesUpdate - "
               "setting source port delta to %d",
               delta));
  } else {
    DEBUG_LOG(("FirewallHelperClass::detectionTest3WaitForResponsesUpdate - "
               "didn't get a delta value"));
    if (m_lastSourcePortAllocationDelta != 0 &&
        (Int)m_lastBehavior > (Int)FIREWALL_TYPE_SIMPLE) {
      /*
      ** If the delta we got last time we played looks good then use that.
  ...
  */
  if ((m_behavior & FIREWALL_TYPE_SMART_MANGLING) != 0) {
    ...
    if ((m_behavior & FIREWALL_TYPE_SIMPLE_PORT_ALLOCATION) != 0) {
      ...
      DEBUG_LOG(("FirewallHelperClass::detectionTest3WaitForResponsesUpdate - "
                 "simple port allocation, Testing to see if the NAT mangles "
                 "differently per destination port at the same IP"));
      ...
      */
      ...
      DEBUG_LOG(("FirewallHelperClass::detectionTest3WaitForResponsesUpdate - "
                 "adding DESTINATION PORT DELTA to behavior"));
      addbehavior |= (UnsignedInt)m_behavior;
      m_behavior = (FirewallBehaviorType)addbehavior;
    }
  } else {
    DEBUG_LOG(("FirewallHelperClass::detectionTest3WaitForResponsesUpdate - We "
               "don't have smart mangling, skipping test 4, entering test 5"));
  }
...
Bool FirewallHelperClass::detectionTest4Stage1Update() {
  ...
  if (m_mangledPorts[0] == 0) {
    if ((timeGetTime() - m_timeoutStart) > m_timeoutLength) {
      ...
      m_currentState = DETECTIONSTATE_DONE;
      DEBUG_LOG(("FirewallHelperClass::detectionTest4Stage1Update - timed out "
...
Bool FirewallHelperClass::detectionTest4Stage2Update() {
  ...
  if (m_mangledPorts[1] == 0) {
    if ((timeGetTime() - m_timeoutStart) > m_timeoutLength) {
      ...
      closeSpareSocket(m_sparePorts[1]);
      m_currentState = DETECTIONSTATE_DONE;
      DEBUG_LOG(("FirewallHelperClass::detectionTest4Stage2Update - timed out "
  ...
  if (m_mangledPorts[1] != m_mangledPorts[0] + m_sourcePortAllocationDelta) {
    DEBUG_LOG(("FirewallHelperClass::detectionTest4Stage2Update - NAT uses "
               "different source ports for different destination ports"));
    ...
    DEBUG_ASSERTCRASH(m_mangledPorts[1] ==
                          m_mangledPorts[0] + m_sourcePortAllocationDelta,
                      ("Problem getting the source port deltas."));
    if (m_mangledPorts[1] == m_mangledPorts[0] + m_sourcePortAllocationDelta) {
      DEBUG_LOG(("FirewallHelperClass::detectionTest4Stage2Update - NAT uses "
                 "the same source port for different destination ports"));
    } else {
      DEBUG_LOG(("FirewallHelperClass::detectionTest4Stage2Update - Unable to "
...
Bool FirewallHelperClass::detectionTest5Update() {
  ...
  */
  if (TheGlobalData->m_firewallSendDelay) {
    ...
    addbehavior |= (UnsignedInt)m_behavior;
    m_behavior = (FirewallBehaviorType)addbehavior;
    DEBUG_LOG(("FirewallHelperClass::detectionTest5Update - Netgear bug "
               "specified by command line or SendDelay flag"));
  } else {
    DEBUG_LOG(("FirewallHelperClass::detectionTest5Update - Netgear bug not "
               "specified"));
  }
#endif // #if (0)
...
Int FirewallHelperClass::getNATPortAllocationScheme(
  ...
  */
  if (diff1 == diff2 && diff2 == diff3) {
    DEBUG_LOG(
        ("Looks good for absolute allocation sequence delta of %d", diff1));
    relativeDelta = FALSE;
    looksGood = TRUE;
    return (diff1);
  }
  ...
  */
  if (diff1 == diff2 || diff1 == diff3) {
    DEBUG_LOG(
        ("Detected probable broken relative port range delta of %d", diff1));
    relativeDelta = TRUE;
    looksGood = FALSE;
    return (diff1);
  }
  ...
  if (diff2 == diff3) {
    DEBUG_LOG(
        ("Detected probable broken relative port range delta of %d", diff2));
    relativeDelta = TRUE;
    looksGood = FALSE;
    return (diff2);
  }
...
Bool FirewallHelperClass::openSpareSocket(UnsignedShort port) {
  ...
  if (m_spareSockets[i].udp == nullptr) {
    DEBUG_LOG(
        ("FirewallHelperClass::openSpareSocket - failed to create UDP object"));
    return FALSE;
  }
