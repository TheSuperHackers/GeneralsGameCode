...
// FRAME_DATA_LENGTH needs to be MAX_FRAMES_AHEAD+1 because a player on a
// different computer can send commands for a frame that is one beyond twice the
// max runahead.
...
enum ConnectionNumbers CPP_11( : Int){
    MAX_PLAYER =
        7, // The index of the highest possible player number.  This is 0 based,
           // so the most players allowed in a game is MAX_PLAYER+1.
    NUM_CONNECTIONS};

static constexpr const Int MAX_SLOTS = MAX_PLAYER + 1;

// TheSuperHackers @info As we are not detecting for network fragmentation and
// dynamically adjusting payload sizes, we set an 1100 bytes UDP payload as a
// safe upper limit for various networks We chose 1100 bytes as when taking
// mobile networks into account, maximum transmission unit sizes can vary from
// 1340 - 1500 bytes and when the packet headers for PPPOE, IPV6 and other
// virtual network encapsulation are considered, we need a lower safe UDP
// payload to prevent fragmentation.
static constexpr const Int MAX_UDP_PAYLOAD_SIZE = 1100;
// UDP (8 bytes) + IP header (28 bytes) = 36 bytes total.  We want a total
// packet size of 512, so 512 - 36 = 476
static constexpr const Int RETAIL_GAME_PACKET_SIZE = 476;

// TheSuperHackers @info The legacy lanapi cannot use a larger packet size
// without breaking the gameinfo command
#ifdef __APPLE__
// macOS wchar_t is 4 bytes vs 2 on Windows, so LANMessage WideChar arrays are
// 2x larger
static constexpr const Int MAX_LANAPI_PACKET_SIZE = RETAIL_GAME_PACKET_SIZE * 2;
#else
static constexpr const Int MAX_LANAPI_PACKET_SIZE = RETAIL_GAME_PACKET_SIZE;
#endif

// TheSuperHackers @bugfix Mauller 08/02/2026 Allow larger ethernet UDP payload
// to be used for game messages, this fixes connection issues and eliminates
// disconnection bugs
...
#else
static constexpr const Int MAX_PACKET_SIZE =
    MAX_UDP_PAYLOAD_SIZE - sizeof(TransportMessageHeader);
static constexpr const Int MAX_NETWORK_MESSAGE_LEN = MAX_UDP_PAYLOAD_SIZE;
#endif

// TheSuperHackers @bugfix Mauller 08/02/2026 Double send and receive buffer
// sizes to alleviate the occurance of disconnection issues in retail and non
// retail code.
 ...
 */
static const Int numCommandsPerCommandPacket =
    (MAX_NETWORK_MESSAGE_LEN - sizeof(UnsignedInt) - sizeof(UnsignedShort)) /
    sizeof(GameMessage);
#pragma pack(push, 1)
...
#if RETAIL_COMPATIBLE_NETWORKING
  // TheSuperHackers @info This value is not the correct one that should be used
  // here, it should have been max packet size The non retail max network
  // message len takes the extra bytes of the network message header into
  // account when handling UDP payload data In retail this only works since no
  // data larger than the retail game packet size is put into a network message
 ...
 * Message types
 */
enum NetMessageFlag CPP_11( : Int){MSG_ACK = 1, MSG_NEEDACK = 2,
                                   MSG_SEQUENCED = 4, MSG_SUPERCEDING = 8};
...
enum NetCommandType CPP_11( : Int){
    ...
    NETCOMMANDTYPE_WRAPPER, // A wrapper command that holds a command that's too
                            // big to fit in a single packet.
    NETCOMMANDTYPE_FILE,
...
enum NetLocalStatus CPP_11( : Int){
    NETLOCALSTATUS_PREGAME = 0, NETLOCALSTATUS_INGAME, NETLOCALSTATUS_LEAVING,
    NETLOCALSTATUS_LEFT, NETLOCALSTATUS_POSTGAME};

enum PlayerLeaveCode CPP_11( : Int){
    PLAYERLEAVECODE_CLIENT = 0, PLAYERLEAVECODE_LOCAL,
    PLAYERLEAVECODE_PACKETROUTER, PLAYERLEAVECODE_UNKNOWN};
...
// The amount of slack in the run ahead value.  This is the percentage of the
// calculated run ahead that is added.
// static const Int NETWORK_RUN_AHEAD_SLACK = 20;

// The number of seconds between when the connections to each player send a
// keep-alive packet. This should be less than 30 just to keep firewall ports
// open.
// static const Int NETWORK_KEEPALIVE_DELAY = 20;

// The number of milliseconds between when the game gets stuck on a frame for a
// network stall and and when the disconnect dialog comes up.
// static const Int NETWORK_DISCONNECT_TIME = 5000;

// The number of miliseconds between when a player's last disconnect keep alive
// command was received and when they are considered disconnected from the game.
// static const Int NETWORK_PLAYER_TIMEOUT_TIME = 60000;

// The base port number used for the transport socket.  A players slot number is
// added to this value to get their actual port number.
