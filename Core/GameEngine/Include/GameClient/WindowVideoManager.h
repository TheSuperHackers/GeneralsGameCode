...
enum WindowVideoStates CPP_11( : Int){ WINDOW_VIDEO_STATE_START = 0, WINDOW_VIDEO_STATE_STOP,		WINDOW_VIDEO_STATE_PAUSE,
																			 WINDOW_VIDEO_STATE_PLAY,			 WINDOW_VIDEO_STATE_HIDDEN,
...
public:
	...
	void hideMovie(GameWindow *win); ///< If the window becomes hidden while we're playing, stop the movie but test to see if
																	 ///< we should resume
...
private:
	...
	typedef std::hash_map<ConstGameWindowPtr, WindowVideo *, hashConstGameWindowPtr, std::equal_to<ConstGameWindowPtr> >
			WindowVideoMap;
...
inline VideoBuffer *WindowVideo::getVideoBuffer(void)
{
	return m_videoBuffer;
};
inline GameWindow *WindowVideo::getWin(void)
{
	return m_win;
};
inline AsciiString WindowVideo::getMovieName(void)
{
	return m_movieName;
};
...
inline WindowVideoStates WindowVideo::getState(void)
{
	return m_state;
};
