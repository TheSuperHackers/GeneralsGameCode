...
enum WindowVideoPlayType CPP_11( : Int){WINDOW_PLAY_MOVIE_ONCE = 0,
...
enum WindowVideoStates CPP_11( : Int){
    WINDOW_VIDEO_STATE_START = 0, WINDOW_VIDEO_STATE_STOP,
    WINDOW_VIDEO_STATE_PAUSE,     WINDOW_VIDEO_STATE_PLAY,
...
public:
  ...
  void playMovie(GameWindow *win, AsciiString movieName,
                 WindowVideoPlayType playType);
  void hideMovie(
      GameWindow *win); ///< If the window becomes hidden while we're playing,
                        ///< stop the movie but test to see if we should resume
  void
  pauseMovie(GameWindow *win); ///< Pause a movie and display it's current frame
  void
  resumeMovie(GameWindow *win);    ///< If a movie has been stopped, resume it.
  void stopMovie(GameWindow *win); ///< Stop a movie
  void stopAndRemoveMovie(
      GameWindow *win);       ///< Stop a movie, and remove it from the manager
...
private:
  ...
  // use special class for hashing, since std::hash won't compile for arbitrary
  // ptrs
  ...
  typedef std::hash_map<ConstGameWindowPtr, WindowVideo *,
                        hashConstGameWindowPtr,
                        std::equal_to<ConstGameWindowPtr>>
      WindowVideoMap;

  WindowVideoMap m_playingVideos; ///< List of currently playin Videos
  // WindowVideoMap m_pausedVideos;
  // ///< List of currently paused Videos
...
// INLINING ///////////////////////////////////////////////////////////////////
//-----------------------------------------------------------------------------
inline VideoStreamInterface *WindowVideo::getVideoStream(void) {
...
inline WindowVideoPlayType WindowVideo::getPlayType(void) {
  return m_playType;
};
inline WindowVideoStates WindowVideo::getState(void) { return m_state; };

inline void WindowVideo::setPlayType(WindowVideoPlayType playType) {
  m_playType = playType;
};
