...
enum OwnerType CPP_11( : Int){ OT_Positional, OT_Drawable, OT_Object, OT_Dead, OT_INVALID };

enum PortionToPlay CPP_11( : Int){ PP_Attack, PP_Sound, PP_Decay, PP_Done };
...
protected:
	...
	{
		ObjectID m_objectID; ///< ObjectID of the object that this sound is tied to. Position can be automatically updated from
												 ///< this.
	...
	Int m_loopCount; ///< The current loop count value. Only valid if this is a looping type event or the override has been
									 ///< set.
	Int m_playingAudioIndex; ///< The sound index we are currently playing. In the case of non-random, we increment this to
													 ///< move to the next sound
	Int m_allCount; ///< If this sound is an ALL type, then this is how many sounds we have played so far.

	Int m_playerIndex; ///< The index of the player who owns this sound. Used for sounds that should have an owner, but don't
										 ///< have an object, etc.
