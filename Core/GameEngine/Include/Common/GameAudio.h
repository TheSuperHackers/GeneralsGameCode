...
class AudioManager : public SubsystemInterface
{
  ...
  virtual AudioHandle addAudioEvent(
      const AudioEventRTS *eventToAdd); ///< Add an audio event (event must be declared in an INI file)
  ...
  virtual void getInfoForAudioEvent(const AudioEventRTS *eventToFindAndFill)
      const; // Note: m_eventInfo is Mutable, and so this function will overwrite it if found
