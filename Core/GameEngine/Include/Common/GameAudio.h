...
public:
	...
	virtual AudioHandle addAudioEvent(const AudioEventRTS *eventToAdd); ///< Add an audio event (event must be declared in an
																																			///< INI file)
	...
	virtual void getInfoForAudioEvent(const AudioEventRTS *eventToFindAndFill) const; // Note: m_eventInfo is Mutable, and so
																																										// this function will overwrite it if
																																										// found
	...
	// Device Dependent enumerate providers functions. It is okay for there to be only 1 provider (Miles provides a maximum
	// of 64.
