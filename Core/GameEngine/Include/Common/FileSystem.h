...
public:
	...
	File *openFile(const Char *filename, Int access = File::NONE, size_t bufferSize = File::BUFFERSIZE); ///< opens a File
																																																			 ///< interface to the
																																																			 ///< specified file
	...
	Bool getFileInfo(const AsciiString &filename, FileInfo *fileInfo) const; ///< fills in the FileInfo struct for the file
																																					 ///< given. returns TRUE if successful.
	...
	AsciiString normalizePath(const AsciiString &path) const; ///< normalizes a file path. The path can refer to a directory.
																														///< File path must be absolute, but does not need to exist.
																														///< Returns an empty string on failure.
	static Bool isPathInDirectory(const AsciiString &testPath, const AsciiString &basePath); ///< determines if a file path is
																																													 ///< within a base path. Both
																																													 ///< paths must be absolute, but
