...
// -----------------------------------------------------
/**
        AsciiString is the fundamental single-byte string type used in the
   Generals code base, and should be preferred over all other string
   constructions (e.g., array of char, STL string<>, WWVegas StringClass, etc.)
   ...
   appropriate!
        ...
        AsciiStrings are suitable for use as automatic, member, or static
   variables.
*/
...
protected:
  ...
  void ensureUniqueBufferOfSize(int numCharsNeeded, Bool preserveData,
                                const char *strToCpy, const char *strToCat);
...
// -----------------------------------------------------
inline AsciiString::AsciiString() : m_data(0) { validate(); }
