...
const char *CompressionManager::getCompressionNameByType(CompressionType compType)
{
	static const char *s_compressionNames[COMPRESSION_MAX + 1] = {
		"No compression",		"RefPack", "LZHL",	 "ZLib 1 (fast)", "ZLib 2",				 "ZLib 3", "ZLib 4",
		"ZLib 5 (default)", "ZLib 6",	 "ZLib 7", "ZLib 8",				"ZLib 9 (slow)", "BTree",	 "Huff",
	};
...
const char *CompressionManager::getDecompressionNameByType(CompressionType compType)
{
	static const char *s_decompressionNames[COMPRESSION_MAX + 1] = {
		"d_None",	 "d_RefPack", "d_NoxLZW", "d_ZLib1", "d_ZLib2", "d_ZLib3", "d_ZLib4",
		"d_ZLib5", "d_ZLib6",		"d_ZLib7",	"d_ZLib8", "d_ZLib9", "d_BTree", "d_Huff",
	};
		...
		else
		{
			DEBUG_LOG(
					("ZLib decompression error (src is level %d, %d bytes long) %d",
					 compType - COMPRESSION_ZLIB1 + 1 /* 1-9 */,
					 srcLen,
					 err));
			return 0;
		}
...
void DoCompressTest(void)
{
		...
		if (f)
		{
			...
			for (i = COMPRESSION_MIN; i <= COMPRESSION_MAX; ++i)
			{
				...
				for (Int j = 0; j < NUM_TIMES; ++j)
				{
					...
					compressedLen =
							CompressionManager::compressData((CompressionType)i, buf, origSize, compressedBuf, maxCompressedSize);
			...
			s_sizes[it->first] = d;
			DEBUG_LOG(
					("s_sizes[%s] = %d -> %d", it->first.str(), s_sizes[it->first].origSize, s_sizes[it->first].compressedSize[i]));
	...
	for (i = COMPRESSION_MIN; i <= COMPRESSION_MAX; ++i)
	{
		...
		DEBUG_LOG(
				("%d bytes compressed to %d (%g%%)",
				 totalUncompressedBytes,
				 totalCompressedBytes,
				 totalCompressedBytes / (Real)totalUncompressedBytes * 100.0f));
		DEBUG_LOG(("Min ratio: %g%%, Max ratio: %g%%", minCompression * 100.0f, maxCompression * 100.0f));
		DEBUG_LOG((""));
	}
