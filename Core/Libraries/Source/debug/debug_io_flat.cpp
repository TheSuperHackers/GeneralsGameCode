...
DebugIOFlat::OutputStream::OutputStream(const char *filename, unsigned maxSize) : m_bufferUsed(0), m_nextChar(0)
{
	...
	if (!m_limitedFileSize)
		m_fileHandle =
				CreateFile(m_fileName, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_WRITE_THROUGH, NULL);
}
...
DebugIOFlat::DebugIOFlat(void) :
		m_firstStream(NULL), m_firstSplit(NULL), m_lastStreamPtr(&m_firstStream), m_lastSplitPtr(&m_firstSplit)
{
	*m_copyDir = 0;
}
...
void DebugIOFlat::Execute(class Debug &dbg, const char *cmd, bool structuredCmd, unsigned argn, const char *const *argv)
{
