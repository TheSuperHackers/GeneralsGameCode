...
void DebugExceptionhandler::LogRegisters(Debug &dbg, struct _EXCEPTION_POINTERS *exptr)
{
	...
	dbg << Debug::FillChar('0') << Debug::Hex() << "EAX:" << Debug::Width(8) << ctx.Eax << " EBX:" << Debug::Width(8)
			<< ctx.Ebx << " ECX:" << Debug::Width(8) << ctx.Ecx << "\n"
			<< "EDX:" << Debug::Width(8) << ctx.Edx << " ESI:" << Debug::Width(8) << ctx.Esi << " EDI:" << Debug::Width(8)
			<< ctx.Edi << "\n"
			<< "EIP:" << Debug::Width(8) << ctx.Eip << " ESP:" << Debug::Width(8) << ctx.Esp << " EBP:" << Debug::Width(8)
			<< ctx.Ebp << "\n"
			<< "Flags:" << Debug::Bin() << Debug::Width(32) << ctx.EFlags << Debug::Hex() << "\n"
			<< "CS:" << Debug::Width(4) << ctx.SegCs << " DS:" << Debug::Width(4) << ctx.SegDs << " SS:" << Debug::Width(4)
			<< ctx.SegSs << "\nES:" << Debug::Width(4) << ctx.SegEs << " FS:" << Debug::Width(4) << ctx.SegFs
...
void DebugExceptionhandler::LogFPURegisters(Debug &dbg, struct _EXCEPTION_POINTERS *exptr)
{
	...
	dbg << Debug::Bin() << Debug::FillChar('0') << "CW:" << Debug::Width(16) << (flt.ControlWord & 0xffff) << "\n"
			<< "SW:" << Debug::Width(16) << (flt.StatusWord & 0xffff) << "\n"
			<< "TW:" << Debug::Width(16) << (flt.TagWord & 0xffff) << "\n"
			<< Debug::Hex() << "ErrOfs:      " << Debug::Width(8) << flt.ErrorOffset << " ErrSel:  " << Debug::Width(8)
			<< flt.ErrorSelector << "\n"
