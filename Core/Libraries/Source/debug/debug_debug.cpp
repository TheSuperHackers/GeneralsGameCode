...
void Debug::PostStaticInit(void)
{
	...
	if (q)
		strcpy(q, ".dbgcmd");
	HANDLE h = CreateFile(ioBuffer, GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	if (h == INVALID_HANDLE_VALUE)
		h = CreateFile("default.dbgcmd", GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	if (h != INVALID_HANDLE_VALUE)
	{
		char cmdBuffer[512];
...
Debug &Debug::AssertBegin(const char *file, int line, const char *expr)
{
	...
	{
		Instance.StartOutput(
				DebugIOInterface::StringType::Assert,
				"%s(%i)",
...
bool Debug::AssertDone(void)
{
	...
	if (curType == DebugIOInterface::StringType::Assert)
	{
		...
		if (IsWindowed())
		{
			...
			int result =
					MessageBox(NULL, help, "Assertion failed", MB_ABORTRETRYIGNORE | MB_ICONSTOP | MB_TASKMODAL | MB_SETFOREGROUND);
			switch (result)
			{
				case IDABORT:
					curFrameEntry = NULL;
				...
				case IDIGNORE:
				{
					...
					wsprintf(help, "%s(%i)", curFrameEntry->fileOrGroup, curFrameEntry->line);
...
Debug &Debug::CheckBegin(const char *file, int line, const char *expr)
{
	...
	if (Instance.curFrameEntry->status == NoSkip)
	{
		++Instance.curFrameEntry->hits;
		Instance.StartOutput(
				DebugIOInterface::StringType::Check,
				"%s(%i)",
				Instance.curFrameEntry->fileOrGroup,
				Instance.curFrameEntry->line);
...
Debug &Debug::LogBegin(const char *fileOrGroup)
{
	...
	// anything to flush first?
	if (Instance.curType != DebugIOInterface::StringType::MAX && Instance.curType != DebugIOInterface::StringType::Log)
		Instance.FlushOutput();
	...
	if (Instance.curFrameEntry->status == NoSkip)
	{
		...
		if (Instance.curType != DebugIOInterface::StringType::Log)
			Instance.StartOutput(DebugIOInterface::StringType::Log, "%s", Instance.curFrameEntry->fileOrGroup);
	}
...
Debug &Debug::CrashBegin(const char *file, int line)
{
	...
	if (Instance.curFrameEntry->status == NoSkip)
	{
		...
		if (file)
		{
			...
			Instance << "Crash in " << p << ", line " << line << ", reason:\n";
		}
	}
	...
	if (curType == DebugIOInterface::StringType::Crash)
	{
		...
		if (!die)
		{
			if (IsWindowed())
			{
				...
				// now display message, wait for user input
				int result =
						MessageBox(NULL, help, "Crash hit", MB_ABORTRETRYIGNORE | MB_ICONSTOP | MB_TASKMODAL | MB_SETFOREGROUND);
				switch (result)
				{
					case IDABORT:
						curFrameEntry = NULL;
...
#endif
		{
			MessageBox(NULL, help, "Game crash", MB_OK | MB_ICONSTOP | MB_TASKMODAL | MB_SETFOREGROUND);
			curFrameEntry = NULL;
			_exit(1);
		}
	}
...
Debug &Debug::operator<<(const MemDump &dump)
{
	...
	// then).
	unsigned itemPerLine = (dump.m_withChars ? 64 : 65) / (1 + 2 * dump.m_bytePerItem + (dump.m_withChars ? 1 : 0));
	if (!itemPerLine)
		itemPerLine = 1;
...
void Debug::AddHResultTranslator(unsigned prio, HResultTranslator func, void *user)
{
	...
	Instance.hrTranslators =
			(HResultTranslatorEntry *)DebugReAllocMemory(Instance.hrTranslators, (Instance.numHrTranslators + 1) * sizeof(void *));
...
void Debug::RemoveHResultTranslator(HResultTranslator func, void *user)
{
	...
	for (unsigned k = 0; k < Instance.numHrTranslators; ++k)
		if (Instance.hrTranslators[k].func == func && Instance.hrTranslators[k].user == user)
		{
			// remove it
			memmove(
					Instance.hrTranslators + k,
					Instance.hrTranslators + k + 1,
					(Instance.numHrTranslators - k - 1) * sizeof(void *));
			--Instance.hrTranslators;
			Instance.hrTranslators =
					(HResultTranslatorEntry *)DebugReAllocMemory(Instance.hrTranslators, Instance.numHrTranslators * sizeof(void *));
		}
}
...
bool Debug::AddIOFactory(const char *io_id, const char *descr, DebugIOInterface *(*func)(void))
{
	...
	IOFactoryListEntry *entry = (IOFactoryListEntry *)DebugAllocMemory(sizeof(IOFactoryListEntry));
...
bool Debug::AddCommands(const char *cmdgroup, DebugCmdInterface *cmdif)
{
	...
	// allocate & init new list entry
	CmdInterfaceListEntry *entry = (CmdInterfaceListEntry *)DebugAllocMemory(sizeof(CmdInterfaceListEntry));
	entry->next = NULL;
...
Debug::FrameHashEntry *Debug::AddFrameEntry(unsigned addr, unsigned type, const char *fileOrGroup, int line)
{
	...
	if (!numAvailableFrameHash)
	{
		...
		nextUnusedFrameHash = (FrameHashEntry *)DebugAllocMemory(numAvailableFrameHash * sizeof(FrameHashEntry));
	}
...
void Debug::UpdateFrameStatus(FrameHashEntry &entry)
{
	...
	char help[512];
	if (entry.frameType == FrameTypeAssert || entry.frameType == FrameTypeCheck)
	...
	while (remainingLen)
	{
		...
		if (timeStamp)
		{
			...
			if (ioBuffer[curType].lastWasCR)
			{
				...
				char ts[40];
				wsprintf(ts, "[%02i:%02i.%02i.%03i] ", systime.wHour, systime.wMinute, systime.wSecond, systime.wMilliseconds);
		...
		if (ioBuffer[curType].used + len + 64 >= ioBuffer[curType].alloc)
		{
			...
			ioBuffer[curType].buffer = (char *)DebugReAllocMemory(ioBuffer[curType].buffer, ioBuffer[curType].alloc);
		}
...
void Debug::FlushOutput(bool defaultLog)
{
	...
	if (!hadWrite && curType != DebugIOInterface::StringType::StructuredCmdReply)
	{
		...
		HANDLE h = CreateFile("default.log", GENERIC_WRITE, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
void Debug::AddPatternEntry(unsigned types, bool isActive, const char *pattern)
{
	...
	PatternListEntry *cur = (PatternListEntry *)DebugAllocMemory(sizeof(PatternListEntry));
