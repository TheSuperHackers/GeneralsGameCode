...
WWProfileHierachyNodeClass::WWProfileHierachyNodeClass(const char *name, WWProfileHierachyNodeClass *parent) :
    Name(name), TotalCalls(0), TotalTime(0), StartTime(0), RecursionCounter(0), Parent(parent), Child(NULL), Sibling(NULL)
{
    Reset();
        ...
        if (file != NULL)
        {
            ...
            for (i = 0; i < ProfileCollectVector.Count(); ++i)
            {
                ...
                str.Format(
                    ...
                    i,
                    frame_time * 1000.0f,
                    frame_time > avg_frame_time * 2.0f ? "SPIKE" : "OK");
...
static unsigned Read_Frame(
    char *memory,
    unsigned pos,
    unsigned maxpos,
    WWProfileHierachyInfoClass *&root,
...
WWProfileHierachyInfoClass::WWProfileHierachyInfoClass(const char *name, WWProfileHierachyInfoClass *parent) :
    Name(name), TotalCalls(0), TotalTime(0), Parent(parent), Child(NULL), Sibling(NULL)
{
}
