            ...
            while (merged)
            {
                ...
                for (edge = FIRST_EDGE; edge <= MAX_EDGE; edge = (SegmentEdge)((int)edge + 1))
                {
                         ...
                         intersectionIndex_r++, intersectionIndex_w++)
                    {
                        ...
                        // Note: intersectionIndex_r is used for anything at or after the current position,
                        // intersectionIndex_w is used for anything before the current position (previous positions have
        ...
        */
        ...
        // Copy in the data to the  VB
        {
            ...
            for (i = 0; i < vnum; i++)
            {
                ...
                DEBUG_ASSERTCRASH(
                    (!_isnan(vertexArray[i].x) && _finite(vertexArray[i].x) && !_isnan(vertexArray[i].y)
                     && _finite(vertexArray[i].y) && !_isnan(vertexArray[i].z) && _finite(vertexArray[i].z)),
                    ("Bad vertexArray[%d]", i));
                ...
                *reinterpret_cast<unsigned int *>(vb + diffuseOffset) = DX8Wrapper::Convert_Color_Clamp(
                    colors[MIN((i / 2), point_cnt)]); // TODO: Does not work correctly when subdivision are not 0
