...
AggregateDefClass::Create (void)
{
	...
	if (pmodel != NULL) {
		...
		WWDEBUG_SAY (("Unable to load aggregate %s.\r", m_Info.BaseModelName));
	}
		...
		if (psubobj_info != NULL) {
			...
			if (prender_obj != NULL) {
				...
				if (base_model.Add_Sub_Object_To_Bone (prender_obj, psubobj_info->BoneName) == false) {
					WWDEBUG_SAY (("Unable to attach %s to %s.\r", psubobj_info->SubobjectName, psubobj_info->BoneName));
				}

				// Release our hold on this pointer
				prender_obj->Release_Ref ();
			} else {
				WWDEBUG_SAY (("Unable to load aggregate subobject %s.\r", psubobj_info->SubobjectName));
			}
		}
	}
			...
			case W3D_CHUNK_TEXTURE_REPLACER_INFO:
				if (chunk_load.Read (&header, sizeof (header)) == sizeof (header)) {
					if (header.ReplacedTexturesCount > 0) {
						WWDEBUG_SAY(("Obsolete texture replacement chunk encountered in aggregate: %s\r",m_pName));
					}
				} 
				break;
