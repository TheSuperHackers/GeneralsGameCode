...
public:
	...
	// this is called only for the nearest intersection. If the request passes a Interpolated_Normal pointer then it will be
	// calculated. otherwise the results are copied into the request structure. This does not copy the matrix or location
	// members; it is intended to be used during poly testing where these values are identical between results, or as a
	// completion function for Copy_Results()
	...
	{
		Set(source->RayLocation,
				source->RayDirection,
				source->IntersectionNormal,
				source->InterpolateNormal,
				source->MaxDistance,
				source->ConvexTest);
	...
	// this will only set the result's range if intersection occurs; it is intended to be used as a first pass intersection
	// test before intersecting the mesh polygons itself. Note: Does NOT do Max_Distance testing
	...
	inline bool Intersect_Polygon_Z(
			IntersectionResultClass *Result,
			Vector3 &PolygonNormal,
			Vector3 &v1,
			Vector3 &v2,
	...
	*/
	...
	bool Intersect_Screen_Point_RenderObject(
			float screen_x,
			float screen_y,
	...
	bool Intersect_Screen_Point_Layer_Range(
			float ScreenX,
			float ScreenY,
	...
	*/
	RenderObjClass *Intersect_Sub_Object(
			float screenx,
			float screeny,
			LayerClass &layer,
			RenderObjClass *robj,
	...
	*/
	...
	static inline bool _Point_In_Polygon(
			IntersectionResultClass *FinalResult,
			Vector3 &Normal,
			Vector3 &loc1,
			Vector3 &loc2,
...
protected:
	...
	*/
	// static inline void _Intersect_Lines_Z(Vector3 &A, Vector3 &B, Vector2 &UVStart, Vector2 &UVEnd, Vector3 &C, Vector3 &D,
	// Vector3 ClippedPoints[6], Vector2 ClippedUV[6], int &DestIndex);
