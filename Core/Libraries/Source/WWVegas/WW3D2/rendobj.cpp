...
PersistClass *	RenderObjPersistFactoryClass::Load(ChunkLoadClass & cload) const
{
	...
	while (cload.Open_Chunk()) {
		switch (cload.Cur_Chunk_ID()) {
			...
			default:
				WWDEBUG_SAY(("Unhandled Chunk: 0x%X File: %s Line: %d\r",__FILE__,__LINE__));
				break;
		};
		cload.Close_Chunk();
	}
	...
	if (strlen(name) == 0) {
		static int count = 0;
		if ( ++count < 10 ) {
			WWDEBUG_SAY(("RenderObjPersistFactory attempted to load an un-named render object!\r"));
			WWDEBUG_SAY(("Replacing it with a NULL render object!\r"));
		}
		strcpy(name,"NULL");
	}
	...
	if (new_obj == NULL) {
		static int count = 0;
		if ( ++count < 10 ) {
			WWDEBUG_SAY(("RenderObjPersistFactory failed to create object: %s!!\r",name));
			WWDEBUG_SAY(("Either the asset for this object is gone or you tried to save a procedural object.\r"));
			WWDEBUG_SAY(("Replacing it with a NULL render object!\r"));
		}
		strcpy(name,"NULL");
