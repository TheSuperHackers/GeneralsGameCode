...
static int Build_List_From_String(const char *buffer, const char *delimiter, StringClass **string_list)
{
	...
	if ((buffer != NULL) && (delimiter != NULL) && (string_list != NULL))
	{
		...
		for (; (entry != NULL) && (entry[1] != 0); entry = ::strstr(entry, delimiter))
		{
			//
		...
		if (count > 0)
		{
			...
			//
			count = 0;
			for (entry = buffer; (entry != NULL) && (entry[1] != 0); entry = ::strstr(entry, delimiter))
			{
				//
				// Move past the current delimiter (if necessary)
				//
...
static bool Is_In_Param_List(StringClass *param_list, int param_count, const char *param_to_check)
{
	//
...
void AnimatedSoundMgrClass::Initialize(const char *ini_filename)
{
	...
	if (ini_file != NULL)
	{
		...
		for (INISection *section = section_list.First(); section != NULL && section->Is_Valid(); section = section->Next_Valid())
		{
			//
