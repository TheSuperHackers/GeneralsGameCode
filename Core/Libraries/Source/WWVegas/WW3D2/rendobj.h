...
#define GET_CONTAINER_INLINE
    ...
    {
        ObjectScale = scale;
    } // set's a scale factor that's factored into transform matrix.									{ScaleFactor=scale;
      // };
    float Get_ObjectScale(void) const { return ObjectScale; };
    void Set_ObjectColor(unsigned int color)
    {
        ObjectColor = color;
    } // the color that was used to modify the asset for player team color (for Generals). -MW
    ...
    //	virtual int						Is_VisibleWithCheatSpy(void) const								{ return
    //((Bits&=~0x80) & (IS_VISIBLE); }
    //	virtual void					Set_VisibleWithCheatSpy(int onoff)								{ if (onoff) { Bits
    //|= IS_VISIBLE|0x80; } else { Bits &= ~IS_VISIBLE; } }
