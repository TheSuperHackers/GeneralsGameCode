...
{
	// TheSuperHackers @todo Omar Aglan 10/09/2026 Use supplementary-aware bidi classification in a follow-up PR.
	// GetStringTypeW can classify surrogate pairs as LTR even when passed together.
...
struct ComplexTextLayout
{
	ComplexTextLayout (HDC source_dc) :
		...
		Runs(nullptr), VisualToLogical(nullptr), RunCount(0), Width(0), Height(0), Ascent(0), Descent(0)
	{
	}
	...
	HFONT primary_font, HFONT alternate_font, ComplexTextLayout *layout)
{
	...
	Usp10Loader::ScriptItem *items = W3DNEWARRAY Usp10Loader::ScriptItem[text_length + 2];
	int item_count = 0;
