...
struct ComplexTextRun
{
	...
	SCRIPT_STRING_ANALYSIS Analysis;
	HFONT Font;
	...
	~ComplexTextLayout ()
	{
		for (size_t index = 0; index < Runs.size(); ++index) {
			if (Runs[index].Analysis != nullptr) {
				::ScriptStringFree(&Runs[index].Analysis);
			}
		}
	...
	HFONT primary_font, HFONT alternate_font, ComplexTextLayout *layout)
{
	...
	SCRIPT_STATE initial_state = { 0 };
	initial_state.uBidiLevel = UnicodeBidi::Get_Paragraph_Level(text, text_length);

	std::vector<SCRIPT_ITEM> items(text_length + 2);
	int item_count = 0;
	if (::ScriptItemize(text, text_length, text_length + 1, &control, &initial_state, &items[0], &item_count) != S_OK ||
		item_count <= 0)
	{
		return false;
	}

	std::vector<SCRIPT_LOGATTR> attributes(text_length);
	for (int break_item_index = 0; break_item_index < item_count; ++break_item_index) {
		const int item_start = items[break_item_index].iCharPos;
		const int item_length = items[break_item_index + 1].iCharPos - item_start;
		if (::ScriptBreak(text + item_start, item_length, &items[break_item_index].a,
	...
	for (int run_item_index = 0; run_item_index < item_count; ++run_item_index) {
		const int item_end = items[run_item_index + 1].iCharPos;
		int run_start = items[run_item_index].iCharPos;
		...
		for (int run_end = run_start + 1; run_end <= item_end; ++run_end) {
			...
			if (run_ends) {
				...
				run.State = items[run_item_index].a.s;
				run_start = run_end;
				if (run_end < item_end) {
					uses_alternate_font = Uses_Alternate_Unicode_Font(text, run_end, alternate_font);
				}
	...
	for (int index = 0; index < run_count; ++index) {
		...
		bidi_levels[index] = (BYTE)run.State.uBidiLevel;
		const int glyph_count = run.CharacterCount + run.CharacterCount / 2 + 16;
		DWORD flags = SSA_GLYPHS | SSA_FALLBACK;
		if ((run.State.uBidiLevel & 1) != 0) {
			flags |= SSA_RTL;
		}

		// TheSuperHackers @bugfix Omar Aglan 13/09/2026 Keep directional overrides inside font runs.
		const HRESULT analysis_result = ::ScriptStringAnalyse(dc,
		...
		const SIZE *run_size = analysis_result == S_OK ? ::ScriptString_pSize(run.Analysis) : nullptr;
	...
	if (success) {
		success = ::ScriptLayout(run_count, &bidi_levels[0], &layout->VisualToLogical[0], nullptr) == S_OK &&
...
static bool Draw_Complex_Text_Layout (ComplexTextLayout &layout)
{
	...
	for (size_t visual_index = 0; visual_index < layout.Runs.size(); ++visual_index) {
		...
		if (::SelectObject(layout.DC, run.Font) == nullptr ||
			::ScriptStringOut(run.Analysis, x, layout.Ascent - run.Ascent,
