...
void SegLineRendererClass::Set_Texture_Tile_Factor(float factor)
{
	...
	{
		WWDEBUG_SAY(
				("Texture (%s) Tile Factor (%.2f) too large in SegLineRendererClass!",
				 Get_Texture()->Get_Texture_Name().str(),
				 TextureTileFactor));
		...
		for (iidx = 2; iidx < num_intersections[TOP_EDGE]; iidx++)
		{
			...
			if (fabs(vdp) < parallel_factor)
			{
				...
				Vector3::Cross_Product(
						segment[iidx - 1].EdgePlane[TOP_EDGE],
			...
			if (fabs(vdp) < parallel_factor)
			{
				...
				// pointing towards the midpoint)
				Vector3::Cross_Product(
						segment[iidx - 1].EdgePlane[BOTTOM_EDGE],
...
void SegLineRendererClass::subdivision_util(
		...
		float *subdiv_tex_v,
		Vector4 *base_diffuse,
		Vector4 *subdiv_diffuse)
{
