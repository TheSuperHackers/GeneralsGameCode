...
void VertexClass::Intersect_Plane(const VertexClass &p0, const VertexClass &p1, const PlaneClass &plane, VertexClass *res)
{
    float alpha = 0.0f;
    ...
    */
    ...
    for (ipass = 0; ipass < model->Get_Pass_Count(); ipass++)
    {
        ...
        for (istage = 0; istage < MeshMatDescClass::MAX_TEX_STAGES; istage++)
        {
            ...
            {
                WWDEBUG_SAY(
                    ("Failed to shatter model: %s.  Texture array in pass: %d stage: %d", model->Get_Name(), ipass, istage));
                REF_PTR_RELEASE(model);
                return;
            }
        }
    ...
    for (ipoly = 0; ipoly < model->Get_Polygon_Count(); ipoly++)
    {
        ...
        */
        ...
        for (ivert = 0; ivert < 3; ivert++)
        {
            ...
            SHATTER_DEBUG_SAY(
                ("normal: %f %f %f", src_vnorms[vert_index].X, src_vnorms[vert_index].Y, src_vnorms[vert_index].Z));
...
void ShatterSystem::Process_Clip_Pools(const Matrix3D &Mshatter_to_mesh, MeshClass *mesh, MeshMtlParamsClass &mtl_params)
{
    /*
                ...
                for (ivert = 0; ivert < poly.Get_Vertex_Count(); ivert++)
                {
                    ...
                    for (ipass = 0; ipass < mtl_params.PassCount; ipass++)
                    {
                        ...
                        */
                        ...
                        {
                            SHATTER_DEBUG_SAY(
                                ("DCG: pass:%d: %f %f %f", ipass, vert.DCG[ipass].X, vert.DCG[ipass].Y, vert.DCG[ipass].Z));
                            /* OLD CODE
                        ...
                        {
                            SHATTER_DEBUG_SAY(
                                ("DIG: pass:%d: %f %f %f", ipass, vert.DIG[ipass].X, vert.DIG[ipass].Y, vert.DIG[ipass].Z));
