	...
	*/
	...
	for (int pidx = first_piv; pidx < NumPivots; pidx++)
	{
		...
		Pivot[pidx].BaseTransform = Pivot[pidx].BaseTransform
				* Build_Matrix3D(Quaternion(piv.Rotation.Q[0], piv.Rotation.Q[1], piv.Rotation.Q[2], piv.Rotation.Q[3]), mtx);
#else
...
HTreeClass *HTreeClass::Alter_Avatar_HTree(const HTreeClass *tree, Vector3 &scale)
{
	...
	// scaling factor.
	const char *flip_list[] = { " RIGHTFOREARM",
															" RIGHTHAND",
															" LEFTFOREARM",
															" LEFTHAND",
															"RIGHTINDEX",
		...
		float lerp_b)
{
	...
	{
		Vector3::Lerp(
				tree_a0_b0->Pivot[pi].BaseTransform.Get_Translation(),
				tree_a0_b1->Pivot[pi].BaseTransform.Get_Translation(),
				lerp_b,
				&pos_a0);
		Vector3::Lerp(
				tree_a1_b0->Pivot[pi].BaseTransform.Get_Translation(),
