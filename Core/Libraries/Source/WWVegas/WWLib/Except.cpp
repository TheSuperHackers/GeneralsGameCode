...
typedef BOOL(WINAPI *SymLoadModuleType)(
    HANDLE hProcess,
    HANDLE hFile,
    LPSTR ImageName,
    LPSTR ModuleName,
...
typedef BOOL(WINAPI *StackWalkType)(
    DWORD MachineType,
    HANDLE hProcess,
    HANDLE hThread,
...
void Dump_Exception_Info(EXCEPTION_POINTERS *e_info)
{
    ...
    */
    static char const *_code_txt[] = {
        ...
        "Error code: EXCEPTION_ARRAY_BOUNDS_EXCEEDED\r\r\nDescription: The thread tried to access an array element that is "
        "out of bounds and the underlying hardware supports bounds checking.",
        ...
        "Error code: EXCEPTION_IN_PAGE_ERROR\r\r\nDescription:	The thread tried to access a page that was not present, and "
        "the system was unable to load the page. For example, this exception might occur if a network connection is lost "
        ...
        "Error code: EXCEPTION_INT_OVERFLOW\r\r\nDescription: The result of an integer operation caused a carry out of the "
        "most significant bit of the result.",
        "Error code: EXCEPTION_INVALID_DISPOSITION\r\r\nDescription: An exception handler returned an invalid disposition "
        "to the exception dispatcher. Programmers using a high-level language such as C should never encounter this "
        "exception.",
    ...
    */
    ...
    else
    {
        ...
        if (!symload)
        {
            ...
            DebugString(
                "Exception Handler: SymLoad failed for module %s with code %d - %s\n",
                module_name,
                GetLastError(),
                Last_Error_Text());
        }
    ...
    */
    ...
    if (!IsBadCodePtr((FARPROC)context->Eip))
    {
        ...
        else
        {
            ...
            {
                DebugString(
                    "Exception Handler: SymGetSymFromAddr failed with code %d - %s\n",
                    GetLastError(),
                    Last_Error_Text());
            }
...
int Exception_Handler(int exception_code, EXCEPTION_POINTERS *e_info)
{
    ...
    if (!WindowedMode)
    {
        ...
        MessageBox(
            MainWindow,
            "This demo has timed out. Thank you for playing Red Alert 2.",
            "Byeee!",
