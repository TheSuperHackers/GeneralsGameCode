...
#else
	// TheSuperHackers @info Mauller 30/3/2020 All modern CPUs have the CPUID instruction, VS22 code will not run on a cpu that
	// doesn't.
...
bool CPUDetectClass::CPUID(unsigned &u_eax_, unsigned &u_ebx_, unsigned &u_ecx_, unsigned &u_edx_, unsigned cpuid_type)
{
	if (!Has_CPUID_Instruction())
...
void CPUDetectClass::Init_Processor_Log()
{
	...
	{
		SYSLOG(
				("L1 Instruction Trace Cache: %d way set associative, %dk micro-OPs\r\n",
				 CPUDetectClass::Get_L1_Instruction_Cache_Set_Associative(),
				 CPUDetectClass::Get_L1_Instruction_Cache_Size() / 1024));
	}
	else
