...
class FastAllocatorGeneral; // Allocates and deletes items of any size. Can use
                            // as a fast replacement for global new/delete.
...
//      T           -- The type of object being allocated. Examples include
//      char,
...
//                     of stack space. So make sure nStackCount*sizeof(T) <
//                     2048.
...
//       char* pArray = stackAllocator.New(nSize);    //Allocate memory, it will
//       auto-freed. memset(pArray, 0, nSize*sizeof(char));       //Do something
//       with the memory.
//    }
//
//    void Example(int nSize){
//       StackAllocator<int*, 512, 0> stackAllocator; //Create an instance. We
//       use the 'construct' hint feature here. int** pArray =
//       stackAllocator.New(nSize);    //Allocate memory. memset(pArray, 0,
//       nSize*sizeof(int*));       //Do something with the memory.
//       stackAllocator.Delete(pArray);               //In this example, we
//       explicitly free the memory.
//    }
//
//    void Example(int nSize){
//       StackAllocator<cRZRect, 200> stackAllocator;       //Create an instance
//       cRZRect* pRectArray = stackAllocator.New(nSize);   //Allocate memory,
//       it will auto-freed. pRectArray[0].SetRect(0,0,1,1); //Do something with
//       the memory.
//    }
//
//    void Example(int nSize){
...
//       char* pArray2 = stackAllocator.New(nSize);   //Allocate some additional
//       memory. memset(pArray, 0, nSize*sizeof(char));       //Do something
//       with the memory. stackAllocator.Delete(pArray);               //Delete
//       the memory pArray = stackAllocator.New(nSize*2);        //Allocate some
//       new memory. We'll let the allocator free it. memset(pArray, 0,
//       nSize*2*sizeof(char));     //Do something with the new memory.
//       stackAllocator.Delete(pArray2);              //Delete the additional
//       memory.
//    }
//
...
public:
  ...
  ~StackAllocator() {
    if (mnAllocCount != -1) { // If there is anything to do...
      if (mpTHeap)
        ...
        if (bConstruct) { // Since this constant, the comparison gets optimized
                          // away.
          T *pTArray = (T *)mTArray;
  ...
  T *New(unsigned nCount) {
    if (mnAllocCount == -1) {
      mnAllocCount = nCount;
      if (nCount < nStackCount) { // If the request is small enough to come from
                                  // the stack...
        ...
        if (bConstruct) { // Since this constant, the comparison gets optimized
          ...
          while (pTArray < pTArrayEnd) {
            ...
            // a '()' after the T this is because () causes trivial types like
            // int and class* to be assigned zero/null. We don't want that.
            new (pTArray) T;
            ++pTArray;
          }
        }
      ...
      return (
          mpTHeap =
              new T[nCount]); // The compiler will call the constructors here.
    }
...
protected:
  int mnAllocCount; // Count of objects allocated. -1 means that nothing is
                    // allocated. We don't use zero because zero is a legal
                    // allocation count in C++.
  T *mpTHeap; // This is normally null, but gets used of the allocation request
              // is too high.
...
public:
  ...
  ~FastFixedAllocator();
  void Init(unsigned int n); // Useful for setting allocation size *after*
                             // construction, but before first use.
  void *Alloc();
...
WWINLINE FastFixedAllocator::FastFixedAllocator(unsigned int n)
    : esize(1), TotalHeapSize(0), TotalAllocatedSize(0),
...
WWINLINE void *FastAllocatorGeneral::Alloc(unsigned int n) {
  ...
  *((unsigned int *)pMemory) =
      n; // Write modified (augmented by 4) count into first four bytes.
  return ((unsigned int *)pMemory) +
         1; // return ptr to bytes after it back to user.
}
...
WWINLINE void FastAllocatorGeneral::Free(void *pAlloc) {
  if (pAlloc) {
    unsigned int *n = ((unsigned int *)pAlloc) -
                      1; // Subtract four bytes and the count is stored there.
...
//   (2) realloc(pblock, 0) is equivalent to free(pblock) (except that nullptr
//   is returned). (3) if the realloc() fails, the object pointed to by pblock
//   is left unchanged.
//
WWINLINE void *FastAllocatorGeneral::Realloc(void *pAlloc, unsigned int n) {
  if (n) {
    void *const pNewAlloc =
        Alloc(n); // Allocate the new memory. This never fails.
    if (pAlloc) {
      n = *(((unsigned int *)pAlloc) -
            1); // Subtract four bytes and the count is stored there.
      ::memcpy(pNewAlloc, pAlloc, n); // Copy the old memory into the new
                                      // memory.
...
template <class T> struct FastSTLAllocator {
  ...
  const T *address(const T &t) const {
    return (&t);
  } // required functions. Just do it.
  static T *allocate(size_t n, const void * = nullptr) {
    return (T *)FastAllocatorGeneral::Get_Allocator()->Alloc(n * sizeof(T));
  }
  ...
  char *_Charalloc(size_t n) {
    return (char *)FastAllocatorGeneral::Get_Allocator()->Alloc(n *
                                                                sizeof(char));
  }
};
#else
...
public:
  ...
  T *allocate(size_type n, const void * = nullptr) {
    return n != 0
               ? static_cast<T *>(FastAllocatorGeneral::Get_Allocator()->Alloc(
                     n * sizeof(T)))
               : nullptr;
  }
  void deallocate(pointer p, size_type n) {
    FastAllocatorGeneral::Get_Allocator()->Free(p);
  }
...
template <class T>
WWINLINE bool operator==(const FastSTLAllocator<T> &,
                         const FastSTLAllocator<T> &) {
  return true;
}
template <class T>
WWINLINE bool operator!=(const FastSTLAllocator<T> &,
...
///////////////////////////////////////////////////////////////////////////////
// Here's how you declare a custom allocator for every regular STL container
class:
//
typedef vector<int, FastSTLAllocator<int> >                            IntArray;
typedef list<int, FastSTLAllocator<int> >                              IntList;
typedef deque<int, FastSTLAllocator<int> > IntDequeue; typedef queue<int,
deque<int, FastSTLAllocator<int> > >                IntQueue; typedef
priority_queue<int, vector<int, FastSTLAllocator<int> > >      IntPriorityQueue;
...
typedef hash_map<int, int, hash<int>, equal_to<int>, FastSTLAllocator<int> >
IntHashMap; typedef hash_multimap<int, int, hash<int>, equal_to<int>,
FastSTLAllocator<int> >   IntHashMultiMap; typedef hash_set<int, hash<int>,
equal_to<int>, FastSTLAllocator<int> >             IntHashSet; typedef
hash_multiset<int, hash<int>, equal_to<int>, FastSTLAllocator<int> >
IntHashMultiSet;

typedef basic_string<char, char_traits<char>, FastSTLAllocator<char> >
CharString;
