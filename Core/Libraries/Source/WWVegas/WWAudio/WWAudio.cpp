...
WWAudioClass::Open_2D_Device (LPWAVEFORMAT format)
{
	...
	if ((success == AIL_NO_ERROR) &&
		...
		success = 2;
		WWDEBUG_SAY (("WWAudio: Detected 2D DirectSound emulation, switching to WaveOut.\r"));
   }
	...
	if (success == AIL_NO_ERROR) {
		...
		Close_2D_Device ();
		WWDEBUG_SAY (("WWAudio: Error initializing 2D device.\r"));
	}
...
WWAudioClass::Get_Sound_Buffer (const char *filename, bool is_3d)
{
	...
	if (buffer == NULL) {
		...
		if (file != NULL && file->Is_Available ()) {
			...
			static int count = 0;
			if ( ++count < 10 ) {
				WWDEBUG_SAY(( "Sound \"%s\" not found\r", filename ));
			}
		}
		Return_File (file);
	}
	...
	if (Is_Disabled () == false) {
		...
		if (filename != NULL) {
					...
					Validate_3D_Sound_Buffer (buffer) == false)
			{
				...
				static int count = 0;
				if ( ++count < 10 ) {
					WWDEBUG_SAY(( "Sound File not Found \"%s\"\r", filename ));
				}
			}
...
)
{
	...
	if (sound != NULL) {
		...
		if (sound->Get_Loop_Count () != INFINITE_LOOPS) {
			WWDEBUG_SAY (("Audio Error:  Creating a continuous sound with a finite loop count!\r"));
		}
	}
...
)
{
	...
	if (sound != NULL) {
		...
		if (sound->Get_Loop_Count () == INFINITE_LOOPS) {
			WWDEBUG_SAY (("Audio Error:  Creating an instant sound %s with an infinite loop count!\r",sound->Get_Definition()->Get_Name()));
		}
...
WWAudioClass::Build_3D_Driver_List (void)
{
	...
	while (::AIL_enumerate_3D_providers (&next, &provider, &name) > 0) {
		...
		if (::AIL_open_3D_provider (provider) == M3D_NOERR) {
			...
			WWDEBUG_SAY (("WWAudio: Unable to open %s.\r", name));
			WWDEBUG_SAY (("WWAudio: Reason %s.\r", error_info));
		}
	}
...
WWAudioClass::Select_3D_Device (int index)
{
	...
	if ((index >= 0) && (index < m_Driver3DList.Count ())) {
		...
		WWDEBUG_SAY (("WWAudio: Selecting 3D sound device: %s.\r", m_Driver3DList[index]->name));
		retval = true;
	}
	...
	if (_firsttime) {
		...
		if (registry.Is_Valid ()) {
			if (registry.Get_Int ("Disabled", 0) == 1) {
				_disabled = true;
				WWDEBUG_SAY (("WWAudio: Audio system disabled in registry.\r"));
			}
		}
	}
