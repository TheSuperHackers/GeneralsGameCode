...
static const char *FULLDAYS[] = { "Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday" };
...
bit8 Wtime::FormatTime(char *out, const char *format)
{
	...
	while (*ptr != 0)
	{
		...
		{
			sprintf(out + strlen(out), "%s %02ld, %ld", FULLMONTHS[GetMonth() - 1], GetMDay(), GetYear());
			ptr += 6;
		}
...
{
	sprintf(out, "%s, %lu %s %lu", DAYS[GetWDay() - 1], GetMDay(), MONTHS[GetMonth() - 1], GetYear());
}
