	...
	while (1)
	{
		...
		if (retval > 0)
		{
			...
			else
			{
				...
				packet->MyMangledPortNumber = addr.sin_port; // not changing to host order, cause its in network byte order now, and
																										 // the game will expect it to stay that way.
				...
				INFMSG(
						"Saw " << (int)theAddr[0] << "." << (int)theAddr[1] << "." << (int)theAddr[2] << "." << (int)theAddr[3] << ":"
