...
void __cdecl DebugPrint(const char *string, ...)
{
	...
	if (string != NULL)
	{
		...
		if (strlen(_filename) == 0)
		{
			...
			file = CreateFile(_filename, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
		}
		else
		{
			file = CreateFile(_filename, GENERIC_WRITE, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
		}
