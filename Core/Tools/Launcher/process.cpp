...
bit8 Create_Process(Process &process)
{
	...
	DBGMSG(
			"(" << retval << ") New process:  HANDLE " << (void *)piProcess.hProcess << "   ID " << (DWORD)piProcess.dwProcessId);
	DBGMSG("(" << retval << ") New thread:  HANDLE " << (void *)piProcess.hThread << "   ID " << (DWORD)piProcess.dwThreadId);
	if (retval == 0)
	{
