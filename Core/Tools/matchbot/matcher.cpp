...
static void NickErrorCallback(
		...
		const const char **suggestedNicks,
		void *param)
{
...
void callbackEach(CHAT chat, CHATBool success, int index, const char *channel, const char *topic, int numUsers, void *param)
{
	...
	DEBUG_LOG(("Channel[%d]: %s (%s), %d users", index, channel, topic, numUsers));
}
		...
		void *param)
{
	...
	{
		DEBUG_LOG(("Channel[%d]: %s (%s), %d users", i, channels[i], topics[i], numUsers[i]));
	}
}
