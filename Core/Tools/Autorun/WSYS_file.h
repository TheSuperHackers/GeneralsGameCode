...
public:
	...
	virtual Int write(void *buffer, Int bytes) = NULL; /**< Write the specified number of bytes from the
																											*	 memory pointed at by buffer to the file. Returns the number of bytes
																											*written. Returns -1 if an error occured.
																											*/
	virtual Int seek(Int bytes, seekMode mode = CURRENT) = NULL; /**< Sets the file position of the next read/write operation.
																																* Returns the new file position as the number of bytes from
																																* the start of the file. Returns -1 if an error occured.
																																*
																																*  seekMode determines how the seek is done:
																																*
																																*  START : means seek to the specified number of bytes from
																																* the start of the file CURRENT: means seek the specified the
																																* number of bytes from the current file position END: means
																																* seek the specified number of bytes back from the end of the
																																* file
																																*/
...
inline Char *File::getName(void)
{
	return m_name;
};
inline Int File::getAccess(void)
{
	return m_access;
};
