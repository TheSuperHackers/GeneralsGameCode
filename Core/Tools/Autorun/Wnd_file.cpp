...
bool StandardFileClass::Open(const char *no_path_file_name, int open_mode)
{
	...
	ASSERT(
			open_mode == MODE_READ_ONLY || open_mode == MODE_WRITE_ONLY || open_mode == MODE_READ_AND_WRITE
			|| open_mode == MODE_WRITE_TRUNCATE || open_mode == MODE_WRITE_APPEND);
...
bool StandardFileClass::Seek(int distance, int seek_file_position)
{
	...
	//
	ASSERT(Currently_Open == TRUE);
	ASSERT(seek_file_position == SEEK_SET || seek_file_position == SEEK_CUR || seek_file_position == SEEK_END);
...
HANDLE Open_File(char const *file_name, int mode)
{
	...
	ASSERT(mode == MODE_READ_ONLY || mode == MODE_WRITE_ONLY || mode == MODE_READ_AND_WRITE);
	...
	//
	windows_file_handle =
			CreateFile(file_name, access, share, NULL, creation, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_SEQUENTIAL_SCAN, NULL);
	//
	// error?
	//
...
int Read_File(HANDLE handle, void *buffer, unsigned long int bytes_to_read)
{
	...
	// &&& use real HANDLE
	//
	success = ReadFile((HANDLE)handle, (void *)buffer, (DWORD)bytes_to_read, (DWORD *)&bytes_actually_read, NULL);
...
int Write_File(HANDLE handle, void const *buffer, unsigned long int bytes_to_write)
{
	...
	//
	success = WriteFile((HANDLE)handle, buffer, (DWORD)bytes_to_write, (DWORD *)&bytes_actually_written, NULL);
...
bool Seek_File(HANDLE handle, int distance, int seek_file_location)
{
	...
	ASSERT(handle != INVALID_FILE_HANDLE);
	// ASSERT( distance >= 0 );
	ASSERT(seek_file_location == SEEK_SET || seek_file_location == SEEK_CUR || seek_file_location == SEEK_END);
	// ASSERT( Windows_File_Handles[ handle ] != NULL );
