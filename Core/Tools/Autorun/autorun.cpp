...
BOOL CALLBACK Dialog_Box_Proc(HWND window_handle, UINT message, WPARAM w_param, LPARAM l_param)
{
    ...
    switch (message)
    {
        case WM_INITDIALOG:
        {
            ...
            else if (b800X600)
            {
                ...
                else
                {
                    ...
                    dlg_rect.bottom =
                        480; // bm.bmHeight + GetSystemMetrics( SM_CYCAPTION ) + 6;		// desktop larger than image
                }
            }
            else
            {
            ...
            Msg(__LINE__,
                TEXT(__FILE__),
                TEXT("DeskTopWindowRect	= [%d,%d,%d,%d]"),
                rect.left,
                rect.top,
                rect.right,
                rect.bottom);
        ...
        case WM_PAINT:
            ...
            {
                //				Msg( __LINE__, TEXT(__FILE__), TEXT("---------------------- WM_PAINT ---------------------"),
                //i ); 				Msg( __LINE__, TEXT(__FILE__), TEXT("Rectangle to update  = [%d,%d,%d,%d]"), paint_rect.left,
                ...
                if (hBitmap)
                {
                    ...
                    for (i = 0; i < NUM_BUTTONS; i++)
                    {
                        if (ButtonList[i])
                        {
                                ...
                                && paint_rect.bottom == rect.bottom)
                            {
                                ...
                                //									Msg( __LINE__, TEXT(__FILE__), TEXT("Rectangle matches a
                                //button to update = [%d,%d,%d,%d]"), rect.left, rect.top, rect.right, rect.bottom );
                                break;
                            }
                        }
                    }
                    ...
                    if (PaintBackground)
                    {
                        ...
                        for (int i = 0; i < (NUM_BUTTONS * 3) + 3; i++)
                        {
                                ...
                                || BackgroundRect[i].bottom != 0)
                            {
                                ...
                                if (result != 0)
                                {
                                    //										Msg( __LINE__, TEXT(__FILE__), TEXT("Paint this
                                    //rect = [%d,%d,%d,%d]"), BackgroundRect[i].left, BackgroundRect[i].top,
                    ...
                    if (Flicker && hFlicker[0])
                    {
                        ...
                        //							Msg( __LINE__, TEXT(__FILE__), TEXT("		Drawing Flicker
                        //[%d,%d,%d,%d]."), flicker_rect.left, flicker_rect.top, flicker_rect.right, flicker_rect.bottom );
                    }
            ...
            switch (idCtl)
            {
                ...
                case IDD_PREVIEWS:
                {
                    ...
                    /*
                       ...
                       _MAX_PATH, NULL, NULL ); length = wideBuffer2.length(); WideCharToMultiByte( CodePage, 0,
                       wideBuffer2.c_str(), length+1, szBuffer2, _MAX_PATH, NULL, NULL ); MessageBox( NULL, szBuffer,
                       szBuffer2, MB_APPLMODAL | MB_OK );
                                            }
                    */
                }
                break;
