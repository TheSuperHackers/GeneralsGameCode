...
BOOL CALLBACK Dialog_Box_Proc(HWND window_handle, UINT message, WPARAM w_param, LPARAM l_param)
{
  ...
  switch (message)
  {
    case WM_INITDIALOG:
    {
      ...
      //				Msg( __LINE__, TEXT(__FILE__), TEXT("buttons_rect = [%d,%d,%d,%d]"), buttons_rect.left,
      ...
      if (GetUpdateRect(window_handle, &paint_rect, FALSE))
      {
        ...
        if (hBitmap)
        {
          ...
          if (PaintBackground)
          {
            ...
            for (int i = 0; i < (NUM_BUTTONS * 3) + 3; i++)
            {
                  ...
                  || BackgroundRect[i].bottom != 0)
              {
                ...
                if (result != 0)
                {
                  //										Msg( __LINE__, TEXT(__FILE__), TEXT("Paint this rect =
                  //[%d,%d,%d,%d]"), BackgroundRect[i].left, BackgroundRect[i].top,
                  // BackgroundRect[i].right, BackgroundRect[i].bottom );
                }
          ...
          if (Flicker && hFlicker[0])
          {
            ...
            //							Msg( __LINE__, TEXT(__FILE__), TEXT("		Drawing Flicker [%d,%d,%d,%d]."),
            // flicker_rect.left, flicker_rect.top, flicker_rect.right, flicker_rect.bottom );
          }
        ...
        case IDD_PREVIEWS:
        {
          ...
          /*
                      if (success == 0) {
                        ...
                        WideCharToMultiByte( CodePage, 0, wideBuffer.c_str(), length+1, szBuffer, _MAX_PATH, NULL,
             NULL ); length = wideBuffer2.length(); WideCharToMultiByte( CodePage, 0, wideBuffer2.c_str(),
             length+1, szBuffer2, _MAX_PATH, NULL, NULL ); MessageBox( NULL, szBuffer, szBuffer2, MB_APPLMODAL |
             MB_OK );
                      }
          */
        }
          ...
          if (CurrentButton != LastButton)
          {
            ...
            for (j = 0; j < NUM_BUTTONS; j++)
            {
              if (ButtonList[j])
              {
                ...
                //									Msg( __LINE__, TEXT(__FILE__), TEXT("WM_MOUSEMOVE --
