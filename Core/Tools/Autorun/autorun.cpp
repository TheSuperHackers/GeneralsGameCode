	...
	/*
		if ( DisplayRegisterButton ) {
			...
			if ( RegOpenKeyEx( HKEY_LOCAL_MACHINE, key, 0, KEY_QUERY_VALUE, &phKey ) == ERROR_SUCCESS ) {
				...
				//-----------------------------------------------------------------------
				Size = _MAX_PATH;
				if ( RegQueryValueEx( phKey, INSTALLPATH_SUBKEY, NULL, &Type, (unsigned char *)szRegisterPath, &Size ) ==
		ERROR_SUCCESS ) {
...
void MainWindow::Create_Buttons(HWND hWnd, RECT *dlg_rect)
{
	...
	else
	{
		...
		/*
				...
				//---------------------------------------------------------------------
				Msg( __LINE__, TEXT(__FILE__), TEXT("ButtonList[%d]: Id=%d, ButtonSizes=%d, String=%s."), i, IDD_OK2, j,
		"WorldBuilder" );
		//		ButtonList[i--] = new DrawButton(
				ButtonList[i++] = new DrawButton(
					IDD_OK3,
		...
		*/
		...
		Msg(__LINE__,
				TEXT(__FILE__),
				TEXT("ButtonList[%d]: Id=%d, ButtonSizes=%d, String=%s."),
				i,
				IDD_OK2,
				j,
				"Check For Updates");
	...
	for (i = 0; i < NUM_BUTTONS; i++)
	{
		if (ButtonList[i])
		{
			Msg(__LINE__,
					TEXT(__FILE__),
					TEXT("ButtonList[%d]: Id=%d, ButtonSizes=[%d,%d,%d,%d]."),
					i,
...
BOOL CALLBACK Dialog_Box_Proc(HWND window_handle, UINT message, WPARAM w_param, LPARAM l_param)
{
	...
	switch (message)
	{
		case WM_INITDIALOG:
		{
			...
			//				Msg( __LINE__, TEXT(__FILE__), TEXT("buttons_rect = [%d,%d,%d,%d]"), buttons_rect.left, buttons_rect.top,
			// buttons_rect.right, buttons_rect.bottom );
			...
			for (; index < NUM_BUTTONS; index++)
			{
				...
				if (ButtonList[index] != NULL)
				{
					...
					BackgroundRect[j].right =
							BackgroundRect[3].left - (ButtonList[index]->Return_X_Pos() + ButtonList[index]->Return_Width());
					j++;
				}
			}
			...
			Msg(__LINE__,
					TEXT(__FILE__),
					TEXT("DeskTopWindowRect(w/o tray) = [%d,%d,%d,%d]"),
					tray_rect.left,
					tray_rect.top,
					tray_rect.right,
					tray_rect.bottom);
			...
			for (index = 0; index < (NUM_BUTTONS * 3) + 3; index++)
			{
				Msg(__LINE__,
						TEXT(__FILE__),
						TEXT("BackgroundRect[%d]	= [%d,%d,%d,%d]"),
						index,
						BackgroundRect[index].top,
						BackgroundRect[index].bottom,
						BackgroundRect[index].left,
						BackgroundRect[index].right);
			}

			Msg(__LINE__,
					TEXT(__FILE__),
					TEXT("BitmapRect	   	= [%d,%d,%d,%d]"),
					bitmap_rect.left,
					bitmap_rect.top,
					bitmap_rect.right,
					bitmap_rect.bottom);
		...
		case WM_TIMER:
		{
			if (w_param == 1000)
			{
				...
				if (Flicker && hFlicker[0])
				{
					...
					Msg(__LINE__,
							TEXT(__FILE__),
							TEXT("WM_TIMER: FlickerRect = [%d,%d,%d,%d]"),
							flicker_rect.left,
							flicker_rect.right,
							flicker_rect.top,
							flicker_rect.bottom);
		...
		case WM_PAINT:
			...
			if (GetUpdateRect(window_handle, &paint_rect, FALSE))
			{
				...
				//				Msg( __LINE__, TEXT(__FILE__), TEXT("Rectangle to update  = [%d,%d,%d,%d]"), paint_rect.left,
				...
				if (hBitmap)
				{
					...
					for (i = 0; i < NUM_BUTTONS; i++)
					{
						if (ButtonList[i])
						{
							...
							if (paint_rect.left == rect.left && paint_rect.top == rect.top && paint_rect.right == rect.right
									&& paint_rect.bottom == rect.bottom)
							{
								PaintBackground = FALSE;
								//									Msg( __LINE__, TEXT(__FILE__), TEXT("Rectangle matches a button to update =
					...
					if (PaintBackground)
					{
						...
						{
							if (BackgroundRect[i].left != 0 || BackgroundRect[i].top != 0 || BackgroundRect[i].right != 0
									|| BackgroundRect[i].bottom != 0)
							{
								...
								if (result != 0)
								{
									//										Msg( __LINE__, TEXT(__FILE__), TEXT("Paint this rect = [%d,%d,%d,%d]"),
									// BackgroundRect[i].left, BackgroundRect[i].top, BackgroundRect[i].right, BackgroundRect[i].bottom );
								}
					...
					if (Flicker && hFlicker[0])
					{
						...
						//							Msg( __LINE__, TEXT(__FILE__), TEXT("		Drawing Flicker [%d,%d,%d,%d]."), flicker_rect.left,
						...
						if (hPen1)
						{
							for (int i = 0; i < NUM_BUTTONS; i++)
							{
								if (ButtonList[i])
								{
									...
									MoveToEx(hDC, ButtonList[i]->Return_X_Pos() - 1, ButtonList[i]->Return_Y_Pos() - 1, NULL);
									LineTo(
											hDC,
									...
									LineTo(
											...
											ButtonList[i]->Return_Y_Pos() + ButtonList[i]->Return_Height() + 1);
									LineTo(hDC, ButtonList[i]->Return_X_Pos() - 1, ButtonList[i]->Return_Y_Pos() - 1);
...
//---------------------------------------------------------------
				...
				if (fontptr)
				{
					...
					fontptr->Print(hDC, szWholeString, text_rect, TEXT_COLOR, SHADOW_COLOR, TPF_CENTER_TEXT, TPF_SHADOW);
				}
#endif
			...
			switch (idCtl)
			{
				...
				case IDD_OK4:
					...
					if (!Is_On_CD(PRODUCT_VOLUME_CD1))
					{
						...
						//							result = Prompt_For_CD( window_handle, volume_to_match, IDS_INSERT_CDROM_WITH_VOLUME1,
						// IDS_EXIT_MESSAGE2, &cd_drive );
						result = Prompt_For_CD(
								window_handle,
								volume_to_match,
								"Autorun:InsertCDROMWithVolume1",
								"Autorun:ExitMessage2",
								&cd_drive);
					}
				...
				case IDD_VIEW_DEMO:
					...
					if (!Is_On_CD(PRODUCT_VOLUME_CD2))
					{
						...
						//							result = Prompt_For_CD( window_handle, volume_to_match, IDS_INSERT_CDROM_WITH_VOLUME2,
						// IDS_EXIT_MESSAGE2, &cd_drive );
						result = Prompt_For_CD(
								window_handle,
								volume_to_match,
								AsciiString("Autorun:InsertCDROMWithVolume2"),
								AsciiString("Autorun:ExitMessage2"),
								&cd_drive);
					}
				...
				if (ButtonList[i] && ButtonList[i]->Is_Mouse_In_Region(mouse_x, mouse_y))
				{
					if (ButtonList[i]->Get_State() != DrawButton::PRESSED_STATE)
					{
						...
						Msg(__LINE__,
								...
								ButtonList[i]->Return_Normal_Bitmap(),
								rect.left,
								rect.top,
								rect.right,
								rect.bottom);
		...
		case WM_LBUTTONUP:
		{
			...
			if (found_focus == -1)
			{
				...
				if (ButtonList[focus_index] && (ButtonList[focus_index]->Get_State() != DrawButton::FOCUS_STATE))
				{
					...
					Msg(__LINE__,
							TEXT(__FILE__),
							TEXT("WM_LBUTTONUP -- %s[FOCUS_STATE] = [x=%d, y=%d, w=%d, h=%d]."),
			...
			else
			{
				...
				if (focus_index == found_focus)
				{
					...
					Msg(__LINE__,
							TEXT(__FILE__),
							TEXT("WM_LBUTTONUP -- %s[FOCUS_STATE] = [x=%d, y=%d, w=%d, h=%d]."),
							ButtonList[found_focus]->Return_Normal_Bitmap(),
							rect.left,
							rect.top,
							rect.right,
							rect.bottom);
				...
				else
				{
					...
					if (ButtonList[focus_index])
					{
						...
						Msg(__LINE__,
								TEXT(__FILE__),
								TEXT("WM_LBUTTONUP -- %s[NORMAL_STATE] = [x=%d, y=%d, w=%d, h=%d]."),
								ButtonList[focus_index]->Return_Normal_Bitmap(),
								rect.left,
								rect.top,
								rect.right,
			...
			if (found_focus >= 0)
			{
				if ((ButtonList[found_focus]) && (ButtonList[found_focus]->Get_State() == DrawButton::FOCUS_STATE)
					...
					if (CurrentButton != LastButton)
					{
						...
						for (j = 0; j < NUM_BUTTONS; j++)
						{
							if (ButtonList[j])
							{
								...
								//									Msg( __LINE__, TEXT(__FILE__), TEXT("WM_MOUSEMOVE -- %s[NORMAL_STATE]]."),
						...
						if (w_param & MK_LBUTTON)
						{
							...
							{
								ButtonList[i]->Set_State(DrawButton::PRESSED_STATE);
								//									Msg( __LINE__, TEXT(__FILE__), TEXT("WM_MOUSEMOVE -- %s[PRESSED_STATE]."),
								// ButtonList[i]->Return_Normal_Bitmap());
							}
						}
						else
						{
							...
							if (ButtonList[i] && ButtonList[i]->Get_State() != DrawButton::FOCUS_STATE)
							{
								...
								//									Msg( __LINE__, TEXT(__FILE__), TEXT("WM_MOUSEMOVE -- %s[FOCUS_STATE]."),
								// ButtonList[i]->Return_Normal_Bitmap());
							}
						} // end of if
...
void Debug_Date_And_Time_Stamp(void)
{
	...
	//-------------------------------------------------------------------------
	static const char *Month_Strings[12] = { "January", "February", "March",		 "April",		"May",			"June",
