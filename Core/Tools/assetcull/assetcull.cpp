...
static bool filesEqual(const char *fn1, const char *fn2)
{
	...
	for (; k < s1.st_size;)
	{
		...
		if (cur > sizeof(buf1))
			cur = sizeof(buf1);
		if (fread(buf1, cur, 1, f1) != 1 || fread(buf2, cur, 1, f2) != 1)
			break;
...
static int recursiveCull(FILE *batchFile, const char *dir1, const char *dir2, const char *relDir)
{
