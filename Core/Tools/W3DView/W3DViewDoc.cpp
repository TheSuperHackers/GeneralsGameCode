...
BOOL CW3DViewDoc::LoadSettings(LPCTSTR filename)
{
	...
	if (filename != NULL)
	{
		...
		if (ini_obj.Is_Present("Settings", "SceneLightDistance") && ini_obj.Is_Present("Settings", "SceneLightIntensity")
				&& ini_obj.Is_Present("Settings", "SceneLightAttenStart") && ini_obj.Is_Present("Settings", "SceneLightAttenEnd")
...
BOOL CW3DViewDoc::SaveSettings(LPCTSTR pszFilename, DWORD dwSettingsMask)
{
	...
	if (hFile == NULL)
	{
		...
		::AfxGetMainWnd()->MessageBox(
				"Unable to open file for writing.  Please select another filename.",
				"File Error",
				MB_ICONERROR | MB_OK);
	}
