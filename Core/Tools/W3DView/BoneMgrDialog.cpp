...
BOOL BoneMgrDialogClass::OnInitDialog(void)
{
	...
	pimagelist->Add((HICON)::LoadImage(::AfxGetResourceHandle(), MAKEINTRESOURCE(IDI_FOLDER), IMAGE_ICON, 16, 16, LR_SHARED));

	pimagelist->Add((HICON)::LoadImage(::AfxGetResourceHandle(), MAKEINTRESOURCE(IDI_OBJECT), IMAGE_ICON, 16, 16, LR_SHARED));
...
void BoneMgrDialogClass::Fill_Bone_Item(HTREEITEM hbone_item, int bone_index)
{
	...
	int index = 0;
	for (; index < porig_model->Get_Num_Sub_Objects_On_Bone(bone_index); index++)
	{
