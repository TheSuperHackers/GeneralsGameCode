            ...
            else
            {
                ...
                else
                {
                    ...
                    RenderObjClass *prender_obj =
                        WW3DAssetManager::Get_Instance()->Create_Render_Obj(pmesh_node->Get_Name());
                    ...
                    TextureSettingsDialogClass dialog(
...
void TextureMgrDialogClass::OnPropagate(void)
{
    ...
    if (MessageBox(
            "Warning, this operation will copy the texture settings from the currently selected mesh into all meshes with "
            "the same texture count.  Do you wish to continue?",
            "Texture Propagate",
