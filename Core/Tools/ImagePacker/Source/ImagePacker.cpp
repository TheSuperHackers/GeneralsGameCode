...
Bool ImagePacker::validateImages(void)
{
	...
	for (i = 0; i < m_imageCount; i++)
	{
		...
		//
		if (image->m_size.x > getTargetWidth() || image->m_size.y > getTargetHeight())
		{
			errors = TRUE;
	...
	{
		proceed =
				DialogBox(ApplicationHInstance, (LPCTSTR)IMAGE_ERRORS, TheImagePacker->getWindowHandle(), (DLGPROC)ImageErrorProc);
...
void ImagePacker::writeFinalTextures(void)
{
	...
	for (page = m_pageTail; page; page = page->m_prev)
	{
		...
		sprintf(buffer, "Generating texture #%d of %d.", page->getID(), m_pageCount);
	...
	if (errors == TRUE)
	{
		DialogBox(ApplicationHInstance, (LPCTSTR)PAGE_ERRORS, TheImagePacker->getWindowHandle(), (DLGPROC)PageErrorProc);
...
void ImagePacker::addImagesInDirectory(char *dir)
{
	...
	if (hFile != INVALID_HANDLE_VALUE)
	{
		...
		if (!(item.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) && strcmp(item.cFileName, ".") && strcmp(item.cFileName, ".."))
		{
			len = strlen(item.cFileName);
			if (len > 4 && item.cFileName[len - 4] == '.' && (item.cFileName[len - 3] == 't' || item.cFileName[len - 3] == 'T')
...
Bool ImagePacker::checkOutputDirectory(void)
{
	...
	if (fileCount != 0)
	{
		...
		sprintf(
				buffer,
				"The output directory (%s) must be empty before proceeding.  Delete '%d' files and continue with build process?",
				m_outputDirectory,
				fileCount);
		response = MessageBox(NULL, buffer, "Delete files to continue?", MB_YESNO | MB_ICONWARNING);
		...
		if (hFile != INVALID_HANDLE_VALUE)
		{
			...
			{
				// if this is a file count it
				if (!(item.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) && strcmp(item.cFileName, ".")
...
void ImagePacker::addDirectory(char *path, Bool subDirs)
{
	...
	if (dir == NULL)
	{
		MessageBox(NULL, "Unable to allocate image directory", "Error", MB_OK | MB_ICONERROR);
		return;
	...
	if (dir->m_path == NULL)
	{
		MessageBox(NULL, "Unable to allocate path for directory", "Error", MB_OK | MB_ICONERROR);
		delete dir;
		return;
...
void ImagePacker::addImage(char *path)
{
	...
	if (info == NULL)
	{
		MessageBox(NULL, "Unable to allocate image info", "Error", MB_OK | MB_ICONERROR);
		return;
	...
	if (info->m_path == NULL)
	{
		MessageBox(NULL, "Unable to allcoate image path info", "Error", MB_OK | MB_ICONERROR);
		delete info;
		return;
	...
	// update status
	sprintf(m_statusBuffer, "Loading Image %d of %d.", m_imageCount, m_imagesInDirs);
	statusMessage(m_statusBuffer);
	...
	for (page = m_pageTail; page; page = page->m_prev)
	{
		...
		for (image = page->getFirstImage(); image; image = image->m_nextPageImage)
		{
			...
			fprintf(
					...
					image->m_pagePos.hi.x + 1,
					image->m_pagePos.hi.y + 1);
...
Bool ImagePacker::getSettingsFromDialog(HWND dialog)
{
	...
	if (IsDlgButtonChecked(dialog, RADIO_TARGET_OTHER))
	{
		...
		if (bitCount != 1)
		{
			MessageBox(NULL, "The target image size must be a power of 2.", "Must Be Power Of 2", MB_OK | MB_ICONERROR);
			return FALSE;
	...
	{
		MessageBox(NULL, "Internal Error. Target Size Unknown.", "Error", MB_OK | MB_ICONERROR);
		return FALSE;
	...
	for (i = 0; i < len; i++)
	{
		...
		for (Int j = 0; j < illegalLen; j++)
		{
			if (m_outputFile[i] == illegal[j])
			{
				...
				sprintf(
						buffer,
						"Output filename '%s' contains one or more of the following illegal characters:\n\n%s",
						m_outputFile,
						illegal);
...
Bool ImagePacker::init(void)
{
	...
	if (m_targa == NULL)
	{
		...
		MessageBox(NULL, "ImagePacker can't init, unable to create targa", "Internal Error", MB_OK | MB_ICONERROR);
		return FALSE;
...
Bool ImagePacker::process(void)
{
	...
	if (packImages())
	{
		...
		sprintf(
				m_statusBuffer,
				"Image Packing Complete: '%d' Texture Pages Generated from '%d' Images in '%d' Folder(s)",
				m_pageCount,
				m_imageCount,
				m_dirCount);
		statusMessage(m_statusBuffer);
