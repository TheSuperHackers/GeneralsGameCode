...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformSetClass::Set_Vertex_Color(int index, int color_index, const VertColor &value)
{
...
void MeshDeformSetClass::Update_Current_Data(void)
{
	...
	for (int index = 0; index < m_VertexCount; index++)
	{
		...
		if ((orig.x != current.x) || (orig.y != current.y) || (orig.z != current.z))
		{
			//
...
void MeshDeformSetClass::Update_Key_Frame(int key_frame)
{
	...
	if ((key_frame == m_CurrentKeyFrame) || (verticies.Count() > 0) || (colors.Count() > 0))
	{
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformSetClass::Determine_Interpolation_Indicies(int key_frame, bool position, int &from, int &to, float &state)
{
	/*state = m_State;
	from = -1;
	to = key_frame;
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformSetClass::Apply_Position_Changes(UINT vert, int frame_to_check, Point3 &position, Matrix3 *transform)
{
	//
		...
		for (int index = 0; index < color_to.Count(); index++)
		{
			...
			if (info.index == vert)
			{
				mesh.vertCol[info.color_index] =
						m_pMesh->vertCol[info.color_index] + state * (info.value - m_pMesh->vertCol[info.color_index]);
			}
		}
	}
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformSetClass::Apply_Color_Changes(UINT vert_index, UINT vert_color_index, int frame_to_check, VertColor &color)
{
	//
	// Determine where we should start interpolating this vert
	//
	...
	if (m_pMesh != NULL)
	{
		...
		for (int key_frame = 0; key_frame < m_KeyFrames.Count (); key_frame ++) {
			...
			if (from <= m_CurrentKeyFrame) {
				...
				for (index = 0; index < color_to.Count (); index ++) {
					VERT_INFO &info = color_to[index];
					tri_obj.mesh.vertCol[info.index] = m_pMesh->vertCol[info.index] + state * (info.value -
...
//
///////////////////////////////////////////////////////////////////////////
void MeshDeformSetClass::Save(MeshBuilderClass &builder, Mesh &mesh, MeshDeformSaveSetClass &save_set, Matrix3 *transform)
{
	// Start fresh
	save_set.Reset();
...
IOResult MeshDeformSetClass::Save(ISave *save_obj)
{
	...
	for (int index = 0; (index < key_frames) && (result == IO_OK); index++)
	{
		...
		//
		for (unsigned int pos_index = 0; (pos_index < keyframe_info.VertexCount) && (result == IO_OK); pos_index++)
		{
		...
		//
		for (unsigned int color_index = 0; (color_index < keyframe_info.ColorCount) && (result == IO_OK); color_index++)
		{
...
IOResult MeshDeformSetClass::Load(ILoad *load_obj)
{
	...
	if ((result == IO_OK) && (load_obj->CurChunkID() == DEFORM_CHUNK_SET_INFO))
	{
		...
		//
		for (unsigned int index = 0; (index < set_info.KeyframeCount) && (result == IO_OK); index++)
		{
			KEY_FRAME &key_frame = *(m_KeyFrames[index]);
