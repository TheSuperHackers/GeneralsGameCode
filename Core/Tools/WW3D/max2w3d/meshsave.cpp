...
bool use_simple_rendering(int geo_type)
{
	...
	if ((geo_type == OBSOLETE_W3D_MESH_FLAG_GEOMETRY_TYPE_SHADOW) || (geo_type == W3D_MESH_FLAG_GEOMETRY_TYPE_AABOX)
...
uint32 setup_mesh_attributes(INode *node)
{
	...
	*/
	if (attributes != W3D_MESH_FLAG_GEOMETRY_TYPE_SKIN && attributes != W3D_MESH_FLAG_GEOMETRY_TYPE_CAMERA_ALIGNED
			&& attributes != W3D_MESH_FLAG_GEOMETRY_TYPE_CAMERA_ORIENTED)
	{
	...
	*/
	...
	for (face_index = 0; face_index < mesh.getNumFaces(); face_index++)
	{
		...
		*/
		...
		if ((mtl_to_use != NULL)
				&& ((mtl_to_use->ClassID() == GameMaterialClassID) || (mtl_to_use->ClassID() == PS2GameMaterialClassID)))
		{
			...
			for (pass = 0; pass < MaterialDesc.Pass_Count(); pass++)
			{
				for (stage = 0; stage < W3dMaterialClass::MAX_STAGES; stage++)
				{
					...
					*/
					...
					if (map3d && map3d->Filename && *((unsigned int *)map3d->Filename) == DIFFUSE_COLOR_TEXTURE_PREFIX) // check for
																																																							// prefix
					{
		...
		for (int pass = 0; pass < MaterialDesc.Pass_Count(); pass++)
		{
			...
			for (int stage = 0; stage < W3dMaterialClass::MAX_STAGES; stage++)
			{
				...
				if (map3d && map3d->Filename
						&& (*((unsigned int *)map3d->Filename) & 0xffff00ff) == DIFFUSE_COLOR_TEXTURE_MASK) // check for 'TXC^' prefix
...
void MeshSaveClass::create_materials(Mtl *nodemtl, DWORD wirecolor, char *materialColorTexture)
{
	...
	//////////////////////////////////////////////////////////////////////
	int geo_type = Header.Attributes & W3D_MESH_FLAG_GEOMETRY_TYPE_MASK;
	if ((geo_type == W3D_MESH_FLAG_GEOMETRY_TYPE_AABOX) || (geo_type == W3D_MESH_FLAG_GEOMETRY_TYPE_OBBOX))
	{
			...
			if (MaterialRemapTable[mi] != -1)
			{
				...
				if (err == W3dMaterialDescClass::INCONSISTENT_PASSES)
				{
					sprintf(
							...
							"Materials must have the same number of passes.\n",
							Header.MeshName,
				...
				if (err == W3dMaterialDescClass::INCONSISTENT_SORT_LEVEL)
				{
					sprintf(
							_string1,
							"Exporting Materials for Mesh: %s\nMaterial %s does not have the same Static Sort Level as other materials "
 ...
 *   08/01/1997 GH  : Created.                                                                 *
 *=============================================================================================*/
void MeshSaveClass::compute_physical_constants(INode *inode, Progress_Meter_Class &meter, bool voxelize)
{
