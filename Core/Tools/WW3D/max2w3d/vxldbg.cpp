...
VoxelDebugWindowClass::VoxelDebugWindowClass(VoxelClass *vxl) :
		CurLayer(0), Bitmap(NULL), Voxel(vxl), WindowHWND(0), ViewportHWND(0), LayerSpin(NULL)
{
...
{
	DialogBoxParam(AppInstance, MAKEINTRESOURCE(IDD_VOXEL_DEBUG_DIALOG), NULL, (DLGPROC)_dialog_proc, (LPARAM)this);
}
...
bool VoxelDebugWindowClass::Dialog_Proc(HWND hWnd, UINT message, WPARAM wParam, LPARAM)
{
	...
	switch (message)
	{
		...
		case WM_INITDIALOG:
			...
			LayerSpin = SetupIntSpinner(hWnd, IDC_LAYER_SPIN, IDC_LAYER_EDIT, 0, Voxel->Num_Layers(), 0);
...
void VoxelDebugWindowClass::update_display(void)
{
	...
	*/
	...
	StretchBlt(hdcwindow, x0, y0, x1, y1, hdcdib, 0, 0, Bitmap->Get_Width(), Bitmap->Get_Height(), SRCCOPY);
