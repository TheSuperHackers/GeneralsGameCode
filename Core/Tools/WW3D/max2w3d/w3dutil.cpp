...
RefResult MaterialReferenceMaker::NotifyRefChanged(
    Interval changeInt,
              ...
              if (submaterial != NULL)
              {
                switch (conversion)
                {
                  ...
                  case GAME_TO_STANDARD:
                    ...
                    if (submaterial->ClassID() == GameMaterialClassID)
                    {
                      // Make a reference to the game material to ensure that it is not deleted by the
                      // system.
                  ...
                  case STANDARD_TO_GAME:
                    ...
                    // Change materials to game materials if they were previously game materials before
                    // being converted to standard materials.
                    if (gamenodematerials[gamenodematerialindex].MaterialPtr != NULL)
                    {
...
void SettingsFormClass::Update_All_Instances(void)
{
  ...
  */
  INodeListClass node_list(::GetCOREInterface()->GetRootNode(), ::GetCOREInterface()->GetTime(), &_INodeFilter);
  ...
  switch (message)
  {
    ...
    case WM_COMMAND:
    {
      ...
      */
      ...
      if ((control_id == IDC_HIERARCHY_CHECK) || (control_id == IDC_GEOMETRY_CHECK) || (control_id == IDC_GEOMETRY_HIDE)
          || (control_id == IDC_GEOMETRY_TWO_SIDED) || (control_id == IDC_GEOMETRY_ZNORMALS)
          || (control_id == IDC_GEOMETRY_VERTEX_ALPHA) || (control_id == IDC_GEOMETRY_CAST_SHADOW)
          || (control_id == IDC_GEOMETRY_SHATTERABLE) || (control_id == IDC_GEOMETRY_NPATCH)
          || (control_id == IDC_COLLISION_PHYSICAL) || (control_id == IDC_COLLISION_PROJECTILE)
          || (control_id == IDC_COLLISION_VIS) || (control_id == IDC_COLLISION_CAMERA)
