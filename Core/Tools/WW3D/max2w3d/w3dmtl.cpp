		...
		*/
		...
		for (int stage = 0; stage < MAX_STAGES; stage++)
		{
			if (gamemtl->Get_Texture_Enable(pass, stage) && gamemtl->Get_Texture(pass, stage))
			{
				...
				texinfo.Attributes =
						texinfo.Attributes | ((gamemtl->Get_Texture_Hint(pass, stage) << W3DTEXTURE_HINT_SHIFT) & W3DTEXTURE_HINT_MASK);
...
*/
W3dMaterialDescClass::VertMatClass::VertMatClass(void) : PassIndex(-1), Crc(0), Name(NULL)
{
...
W3dMaterialDescClass::VertMatClass &W3dMaterialDescClass::VertMatClass::operator=(const VertMatClass &that)
{
	if (this != &that)
	{
	...
	*/
	...
	for (int pass = 0; pass < PassCount; pass++)
	{
		remap.VertexMaterialIdx[pass] = Add_Vertex_Material(
				mat.Get_Vertex_Material(pass),
				mat.Get_Mapper_Args(pass, 0),
				mat.Get_Mapper_Args(pass, 1),
				pass,
				name);
...
bool W3dMaterialDescClass::Pass_Uses_Alpha(int pass)
{
	for (int mi = 0; mi < MaterialRemaps.Count(); mi++)
	{
		...
		if ((dst_blend == W3DSHADER_DESTBLENDFUNC_SRC_ALPHA) || (dst_blend == W3DSHADER_DESTBLENDFUNC_ONE_MINUS_SRC_ALPHA)
				|| (src_blend == W3DSHADER_SRCBLENDFUNC_SRC_ALPHA) || (src_blend == W3DSHADER_SRCBLENDFUNC_ONE_MINUS_SRC_ALPHA)
				|| (alpha_test != 0))
		{
			return true;
		}
...
int W3dMaterialDescClass::Add_Vertex_Material(
