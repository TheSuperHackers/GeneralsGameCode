 ...
 *=============================================================================================*/
bool W3dExportClass::Export_Hierarchy(char *name, ChunkSaveClass &csave, Progress_Meter_Class &meter, INode *root)
{
 ...
 *   10/17/2000 gth : Removed the old code-path, we always use an origin now                   *
 *=============================================================================================*/
bool W3dExportClass::Export_Animation(char *name, ChunkSaveClass &csave, Progress_Meter_Class &meter, INode *root)
{
	...
	try
	{
		motion = new MotionClass(
				ExportInterface->theScene,
				root,
				htree,
				ExportOptions,
				FrameRate,
 ...
 *   1999 AJA : Created.                                                                       *
 *=============================================================================================*/
bool W3dExportClass::Export_Damage_Animations(
		char *name,
...
unsigned int houseColorScale[TEAM_COLOR_PALETTE_SIZE] = { 255, 239, 223, 211, 195, 174, 167, 151,
																													135, 123, 107, 91,	79,	 63,	47,	 35 };
 ...
 *=============================================================================================*/
bool W3dExportClass::Export_Geometry(
		char *name,
		ChunkSaveClass &csave,
		Progress_Meter_Class &meter,
		INode *root,
		MeshConnectionsClass **out_connection)
{
	...
	*/
	GeometryExportContextClass
			context(name, csave, world_info, ExportOptions, htree, root, get_origin_list(), CurTime, materialColors);
 ...
 *=============================================================================================*/
bool W3dExportClass::Export_HLod(
		char *name,
	...
	*/
	if (!ExportOptions.ExportHierarchy)
	{
		...
		else
		{
			...
			sprintf(
					buf,
					"Unable to load hierarchy file: %s\nIf this Max file has been moved, please re-select the hierarchy file.",
					HierarchyFilename);
...
bool W3dExportClass::get_export_options(BOOL suppress_prompts)
{
	...
	if (suppress_prompts || retval)
	{
		...
		if ((suppress_prompts == TRUE) && (options->RelativeHierarchyFilename[0] != 0))
		{
			...
			assert(_fullpath(HierarchyFilename, options->RelativeHierarchyFilename, sizeof(HierarchyFilename)));
