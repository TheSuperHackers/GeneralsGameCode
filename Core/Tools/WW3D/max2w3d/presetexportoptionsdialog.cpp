...
PresetExportOptionsDialogClass::PresetExportOptionsDialogClass(Interface *maxinterface, HWND parent_wnd) :
		MaxInterface(maxinterface), Options(NULL), Wnd(NULL), ParentWnd(parent_wnd), CurrentPane(-1)
{
...
//
////////////////////////////////////////////////////////////////////////////////////////
BOOL CALLBACK PresetExportOptionsDialogClass::Real_Message_Proc(HWND wnd, UINT message, WPARAM wparam, LPARAM lparam)
{
...
////////////////////////////////////////////////////////////////////////////////////////
BOOL CALLBACK
		PresetExportOptionsDialogClass::Settings_Pane_Message_Proc(HWND wnd, UINT message, WPARAM wparam, LPARAM lparam)
{
	PresetExportOptionsDialogClass *dialog_obj = NULL;
			...
			switch (LOWORD(wparam))
			{
				...
				case IDC_WHT_BROWSE_BUTTON:
				{
					...
					if (::GetOpenFileName(&ofn))
					{
						...
						//
						Create_Relative_Path(Options->RelativeHierarchyFilename, W3dExportClass::CurrentExportPath, ofn.lpstrFile);
					}
				}
				break;
...
{
	PaneWnds[PANE_HLOD] =
			::CreateDialogParam(AppInstance, MAKEINTRESOURCE(IDD_EXPORT_PANE_HLOD), Wnd, Settings_Pane_Message_Proc, (LPARAM)this);

	PaneWnds[PANE_ANIM_HLOD] = ::CreateDialogParam(
			AppInstance,
			MAKEINTRESOURCE(IDD_EXPORT_PANE_ANIMATED_HLOD),
			Wnd,
			Settings_Pane_Message_Proc,
			(LPARAM)this);
	...
	PaneWnds[PANE_ANIM] = ::CreateDialogParam(
			AppInstance,
			MAKEINTRESOURCE(IDD_EXPORT_PANE_ANIMATION),
			Wnd,
			Settings_Pane_Message_Proc,
			(LPARAM)this);

	PaneWnds[PANE_TERRAIN] = ::CreateDialogParam(
			AppInstance,
			MAKEINTRESOURCE(IDD_EXPORT_PANE_TERRAIN),
			Wnd,
			Settings_Pane_Message_Proc,
			(LPARAM)this);
	...
	PaneWnds[PANE_SKELETON] = ::CreateDialogParam(
			AppInstance,
			MAKEINTRESOURCE(IDD_EXPORT_PANE_SKELETON),
			Wnd,
			Settings_Pane_Message_Proc,
			(LPARAM)this);

	PaneWnds[PANE_MESH] =
			::CreateDialogParam(AppInstance, MAKEINTRESOURCE(IDD_EXPORT_PANE_MESH), Wnd, Settings_Pane_Message_Proc, (LPARAM)this);
...
void PresetExportOptionsDialogClass::Initialize_Controls(void)
{
	...
	for (int index = 0; index < PANE_MAX; index++)
	{
		...
		if (::GetDlgItem(pane_wnd, IDC_RANGE_LOW_SPIN) != NULL)
		{
			...
			low_spin = ::SetupIntSpinner(pane_wnd, IDC_RANGE_LOW_SPIN, IDC_RANGE_LOW_EDIT, startframe, endframe, 0);

			high_spin = ::SetupIntSpinner(pane_wnd, IDC_RANGE_HIGH_SPIN, IDC_RANGE_HIGH_EDIT, startframe, endframe, 0);
