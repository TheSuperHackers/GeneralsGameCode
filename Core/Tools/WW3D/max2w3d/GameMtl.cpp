...
static ParamVersionDesc PassParameterBlockVersions[] = { ParamVersionDesc(PassParameterBlockDescVer0, 34, 0),
 ...
 *   11/23/98   GTH : Created.                                                                 *
 *=============================================================================================*/
RefResult GameMtl::NotifyRefChanged(Interval changeInt, RefTargetHandle hTarget, PartID &partID, RefMessage message)
{
	switch (message)
	{
		case REFMSG_CHANGE:
...
IOResult GameMtl::Load(ILoad *iload)
{
	...
	for (int i = 0; i < W3dMaterialClass::MAX_PASSES; i++)
	{
		iload->RegisterPostLoadCallback(new ParamBlockPLCB(
				PassParameterBlockVersions,
				NUM_OLDVERSIONS,
				&CurrentPassParameterBlockVersion,
				this,
				REF_PASS_PARAMETERS + i));
	}
...
int GameMtl::Compute_PC_Shader_From_PS2_Shader(int pass)
{
	...
	static const int dest_blend[6] = { ST_ZERO, ST_ONE, ST_SRC, 1 - ST_SRC, ST_SRC_ALPHA, 1 - ST_SRC_ALPHA };
