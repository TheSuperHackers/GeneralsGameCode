...
public:
	...
	int DoExport(const TCHAR *name, ExpInterface *ei, Interface *i, BOOL suppressPrompts = FALSE, DWORD options = 0); // Export
																																																										// file
...
private:
	...
	bool Export_Geometry(
			char *name,
			ChunkSaveClass &csave,
			Progress_Meter_Class &meter,
			INode *root = NULL,
			MeshConnectionsClass **out_connection = NULL);
	bool Export_HLod(
			char *name,
	...
	bool Export_Collection(
			...
			INodeListClass &transform_node_list);
