 ...
 *=============================================================================================*/
GameMtlVertexMaterialDlg::GameMtlVertexMaterialDlg(HWND parent, IMtlParams *imp, GameMtl *mtl, int pass) :
	...
	switch (message)
	{
		case WM_INITDIALOG:
		{
			...
			OpacitySpin = SetupFloatSpinner(
					...
					0.0f,
					1.0f,
					TheMtl->Get_Opacity(PassIndex, IParams->GetTime()),
					0.01f);
			TranslucencySpin = SetupFloatSpinner(
					...
					0.0f,
					1.0f,
					TheMtl->Get_Translucency(PassIndex, IParams->GetTime()),
					0.01f);
			ShininessSpin = SetupFloatSpinner(
					...
					1.0f,
					1000.0f,
					TheMtl->Get_Shininess(PassIndex, IParams->GetTime()),
					1.0f);
...
void GameMtlVertexMaterialDlg::ReloadDialog(void)
{
	...
	SendDlgItemMessage(m_hWnd, IDC_MAPPING0_COMBO, CB_SETCURSEL, TheMtl->Get_Mapping_Type(PassIndex, 0), 0);

	SendDlgItemMessage(m_hWnd, IDC_MAPPING1_COMBO, CB_SETCURSEL, TheMtl->Get_Mapping_Type(PassIndex, 1), 0);
