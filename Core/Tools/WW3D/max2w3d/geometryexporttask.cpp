...
public:
	MeshGeometryExportTaskClass(INode *node, GeometryExportContextClass &context) :
			GeometryExportTaskClass(node, context), NameDirty(false), SingleMtl(NULL)
	{
		/*
	...
	MeshGeometryExportTaskClass(const MeshGeometryExportTaskClass &that) :
			GeometryExportTaskClass(that), MeshData(that.MeshData), ExportOptions(that.ExportOptions), NameDirty(false)
	{
	}
	...
	virtual void Export_Geometry(GeometryExportContextClass &context)
	{
		...
		*/
		...
		MeshSaveClass *mesh = new MeshSaveClass(
				Name,
				ContainerName,
				Node,
				&MeshData,
	...
	virtual void Export_Geometry(GeometryExportContextClass &context)
	{
		...
		*/
		CollisionBoxSaveClass *colbox =
				new CollisionBoxSaveClass(Name, ContainerName, Node, ExportSpace, context.CurTime, *context.ProgressMeter);
...
{
public:
	DazzleGeometryExportTaskClass(INode *node, GeometryExportContextClass &context) : GeometryExportTaskClass(node, context) {}
	...
	virtual void Export_Geometry(GeometryExportContextClass &context)
	{
		...
		*/
		DazzleSaveClass *dazzle =
				new DazzleSaveClass(Name, ContainerName, Node, ExportSpace, context.CurTime, *context.ProgressMeter);
		/*
		** Export it.
		*/
...
{
public:
	NullGeometryExportTaskClass(INode *node, GeometryExportContextClass &context) : GeometryExportTaskClass(node, context)
	{
	...
	virtual void Export_Geometry(GeometryExportContextClass &context) { context.ProgressMeter->Add_Increment(); };
...
{
public:
	AggregateGeometryExportTaskClass(INode *node, GeometryExportContextClass &context) : GeometryExportTaskClass(node, context)
	{
		memset(ContainerName, 0, sizeof(ContainerName));
	}
...
{
public:
	ProxyExportTaskClass(INode *node, GeometryExportContextClass &context) : GeometryExportTaskClass(node, context)
	{
		/*
...
GeometryExportTaskClass::GeometryExportTaskClass(const GeometryExportTaskClass &that) :
		BoneIndex(that.BoneIndex), ExportSpace(that.ExportSpace), CurTime(that.CurTime), Node(that.Node)
{
 ...
 *   10/20/2000 gth : Created.                                                                 *
 *=============================================================================================*/
GeometryExportTaskClass *GeometryExportTaskClass::Create_Task(INode *node, GeometryExportContextClass &context)
{
	if (!::Is_Geometry(node))
	{
		return NULL;
		...
		GeometryExportContextClass &context)
{
	...
	*/
	...
	while (i < tasks.Count())
	{
		if ((tasks[i]->Get_Geometry_Type() == GeometryExportTaskClass::MESH) && (!Is_Skin(tasks[i]->Get_Object_Node())))
		{
			/*
			...
			if ((face_smgroup & smgroup) || (face_smgroup == smgroup))
			{
				...
				*/
				...
				for (int vert_index = 0; (vert_index < 3) && !found; vert_index++)
				{
					...
					if ((fabs(delta.x) < EPSILON) && (fabs(delta.y) < EPSILON) && (fabs(delta.z) < EPSILON))
					{
						/*
