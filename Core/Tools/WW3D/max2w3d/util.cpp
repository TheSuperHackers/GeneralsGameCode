...
void Create_Full_Path(char *full_path, const char *curr, const char *rel_path)
{
	...
	const char *rel_p;
	for (rel_p = rel_path; *rel_p == '.' && *(rel_p + 1) == '.' && (*(rel_p + 2) == '/' || *(rel_p + 2) == '\\'); rel_p += 3)
	{
		full_p--;
...
void Create_Relative_Path(char *rel_path, const char *curr, const char *full_path)
{
	...
	for (; *full_p && *full_p == *curr_p || (*full_p == '/' && *curr_p == '\\') || (*full_p == '\\' && *curr_p == '/');
			 full_p++, curr_p++)
		;
	...
	if ((full_type == SLASH_CHAR && curr_type == PLAIN_CHAR) || (curr_type == SLASH_CHAR && full_type == PLAIN_CHAR)
			|| (curr_type == NULL_CHAR && full_type == PLAIN_CHAR)
			|| (curr_type == PLAIN_CHAR && full_type == PLAIN_CHAR && *(full_p - 1) != '/' && *(full_p - 1) != '\\'))
	{
		for (; full_p > up_full
				 && ((*(full_p - 1) != '/' && *(full_p - 1) != '\\') || (*(curr_p - 1) != '/' && *(curr_p - 1) != '\\'));
				 full_p--, curr_p--)
			;
	}
