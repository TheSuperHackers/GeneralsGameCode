...
** local functions
***************************************************************************************/
static void clip_tri_to_slab(Point3 p0, Point3 p1, Point3 p2, float z0, float z1, vertexstruct *outverts, int *setnum);

static void clip_poly(vertexstruct *inverts, int innum, vertexstruct *outverts, int *outnum, const PlaneClass &clipplane);
...
void VoxelLayerClass::Scan_Triangle(Point3 p0, Point3 p1, Point3 p2)
{
	...
	for (i = (int)floor(miny); i <= (int)floor(maxy); i++)
	{
		...
		Draw_Line(_scantab[i].P[LEFT].Pos.x, _scantab[i].P[LEFT].Pos.y, _scantab[i].P[RIGHT].Pos.x, _scantab[i].P[RIGHT].Pos.y);
		//		}
	}
}
