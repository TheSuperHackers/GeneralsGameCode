...
void W3DRadar::reconstructViewBox(void)
{
	...
	TheTacticalView->getScreenCornerWorldPointsAtZ(&world[0], &world[1], &world[2], &world[3], getTerrainAverageZ());
...
//-------------------------------------------------------------------------------------------------
void W3DRadar::radarToPixel(
		const ICoord2D *radar,
		ICoord2D *pixel,
		Int radarUpperLeftX,
		Int radarUpperLeftY,
		Int radarWidth,
...
void W3DRadar::drawViewBox(Int pixelX, Int pixelY, Int width, Int height)
{
	...
	if (ClipLine2D(&start, &end, &clipStart, &clipEnd, &clipRegion))
		TheDisplay->drawLine(clipStart.x, clipStart.y, clipEnd.x, clipEnd.y, lineWidth, topColor);
	...
	radarToPixel(&radar, &end, pixelX, pixelY, width, height);
	if (ClipLine2D(&start, &end, &clipStart, &clipEnd, &clipRegion))
		TheDisplay->drawLine(clipStart.x, clipStart.y, clipEnd.x, clipEnd.y, lineWidth, topColor, bottomColor);
	...
	radarToPixel(&radar, &end, pixelX, pixelY, width, height);
	if (ClipLine2D(&start, &end, &clipStart, &clipEnd, &clipRegion))
		TheDisplay->drawLine(clipStart.x, clipStart.y, clipEnd.x, clipEnd.y, lineWidth, bottomColor);

	// left line
	start = end;
	end = ulStart;
	if (ClipLine2D(&start, &end, &clipStart, &clipEnd, &clipRegion))
		TheDisplay->drawLine(clipStart.x, clipStart.y, clipEnd.x, clipEnd.y, lineWidth, bottomColor, topColor);
...
void W3DRadar::drawSingleBeaconEvent(Int pixelX, Int pixelY, Int width, Int height, Int index)
{
	...
	if (currentFrame > event->fadeFrame)
	{
		a = REAL_TO_UNSIGNEDBYTE(
				(Real)a * (1.0f - (Real)(currentFrame - event->fadeFrame) / (Real)(event->dieFrame - event->fadeFrame)));
...
void W3DRadar::init(void)
{
	...
	// poolify
	m_terrainTexture =
			MSGNEW("TextureClass") TextureClass(m_textureWidth, m_textureHeight, m_terrainTextureFormat, MIP_LEVELS_1);
	DEBUG_ASSERTCRASH(m_terrainTexture, ("W3DRadar: Unable to allocate terrain texture"));

	// allocate our overlay texture
	m_overlayTexture =
			MSGNEW("TextureClass") TextureClass(m_textureWidth, m_textureHeight, m_overlayTextureFormat, MIP_LEVELS_1);
	...
	m_shroudTexture =
			MSGNEW("TextureClass") TextureClass(m_textureWidth, m_textureHeight, m_shroudTextureFormat, MIP_LEVELS_1);
...
void W3DRadar::buildTerrainTexture(TerrainLogic *terrain)
{
	...
	for (y = 0; y < m_textureHeight; y++)
	{
		for (x = 0; x < m_textureWidth; x++)
		{
						...
						for (i = x - waterSamplesAway; i <= x + waterSamplesAway; i++)
						{
							if (i >= 0 && i < m_textureWidth)
							{
								...
								if (terrain->isUnderwater(worldPoint.x, worldPoint.y, NULL, &underwaterZ))
								{
									...
									interpolateColorForHeight(&color, underwaterZ, waterZ, waterZ, m_mapExtent.lo.z);
						...
						for (i = x - samplesAway; i <= x + samplesAway; i++)
						{
							if (i >= 0 && i < m_textureWidth)
							{
								...
								if (workingBridge)
								{
									...
									DEBUG_ASSERTCRASH(
											bridgeTemplate,
											("W3DRadar::buildTerrainTexture - Can't find bridge template for '%s'", bridgeTName.str()));
									...
									//
									Real bridgeHeight = (bridge->peekBridgeInfo()->fromLeft.z + bridge->peekBridgeInfo()->fromRight.z
																			 + bridge->peekBridgeInfo()->toLeft.z + bridge->peekBridgeInfo()->toRight.z)
											/ 4.0f;

									// interpolate the color, but use the bridge height, not the terrain height
									interpolateColorForHeight(&color, bridgeHeight, getTerrainAverageZ(), m_mapExtent.hi.z, m_mapExtent.lo.z);
								...
								else
								{
									...
									interpolateColorForHeight(&color, worldPoint.z, getTerrainAverageZ(), m_mapExtent.hi.z, m_mapExtent.lo.z);
			...
			//
			surface->DrawPixel(x, y, GameMakeColor(color.red * 255, color.green * 255, color.blue * 255, 255));
...
/// The following is an "archive" of an attempt to foil the mapshroud hack... saved for later, since it is too close to
/// release to try it
