...
public:
	...
	virtual Int read(void *pData, Int numBytes) { return (m_file->read(pData, numBytes)); };
};
		...
		const ThingTemplate *thingTemplate)
{
	...
	m_shadowObj = NULL;
	m_runtimeFlags = 0;
	// Note - do NOT set TheKey_objectSelectable on creation - allow it to follow the .ini value unless specified by user
	// action.  jba. [3/20/2003]
	if (props)
	{
		m_properties = *props;
	}
...
Bool WorldHeightMap::ParseHeightMapData(DataChunkInput &file, DataChunkInfo *info, void *userData)
{
	...
	m_seismicUpdateFlag =
			MSGNEW("WorldHeightMap::ParseHeightMapData _ m_seismicUpdateFlag allocated") UnsignedByte[numBytesX * numBytesY];
...
Bool WorldHeightMap::ParseObjectData(DataChunkInput &file, DataChunkInfo *info, void *userData, Bool readDict)
{
	...
	pThisOne = newInstance(MapObject)(loc, name, angle, flags, &d, TheThingFactory->findTemplate(name, FALSE));
	...
	if ((ndx < m_dataSize) && m_tileNdxes)
	{
		...
		if (m_cliffInfoNdxes[ndx])
		{
			...
			for (i = 0; i < this->m_numTextureClasses; i++)
			{
				...
				{
					tilesMatch =
							ndx2 >= m_textureClasses[i].firstTile && ndx2 < m_textureClasses[i].firstTile + m_textureClasses[i].numTiles;
					// tilesMatch = true;
					break;
				}
			}
...
Bool WorldHeightMap::getExtraAlphaUVData(
		Int xIndex,
		Int yIndex,
		float U[4],
		float V[4],
...
*/
void WorldHeightMap::getAlphaUVData(
		Int xIndex,
		Int yIndex,
		float U[4],
		float V[4],
...
Bool WorldHeightMap::setDrawOrg(Int xOrg, Int yOrg)
{
	...
	Bool anythingDifferent =
			(m_drawOriginX != newX) || (m_drawOriginY != newY) || (m_drawWidthX != newWidth) || (m_drawHeightY != newHeight);
...
Bool WorldHeightMap::getRawTileData(Short tileNdx, Int width, UnsignedByte *buffer, Int bufLen)
{
	TileData *pSrc = NULL;
