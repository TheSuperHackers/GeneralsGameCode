...
void W3DShroud::init(WorldHeightMap *pMap, Real worldCellSizeX, Real worldCellSizeY)
{
    ...
    if (pMap)
    {
        ...
        m_numCellsY = REAL_TO_INT_CEIL(
            (Real)(pMap->getYExtent() - 1 - pMap->getBorderSizeInline() * 2) * MAP_XY_FACTOR / m_cellHeight);
    ...
    if (x < m_numCellsX && y < m_numCellsY)
    {
...
#endif
        {
            ...
            UnsignedInt bluepixel =
                (UnsignedInt)((Real)level * ((Real)(TheGlobalData->m_shroudColor.getAsInt() & 0xff) / 255.0f));
...
#endif
                {
                    dataDest[x] = ((dataSrc[x] >> 3) & 0x1f) | (((dataSrc[x] >> 10) & 0x3f) << 5)
