...
void W3DShroud::init(WorldHeightMap *pMap, Real worldCellSizeX, Real worldCellSizeY)
{
	...
	{
		m_numCellsX =
				REAL_TO_INT_CEIL((Real)(pMap->getXExtent() - 1 - pMap->getBorderSizeInline() * 2) * MAP_XY_FACTOR / m_cellWidth);
		m_numCellsY =
				REAL_TO_INT_CEIL((Real)(pMap->getYExtent() - 1 - pMap->getBorderSizeInline() * 2) * MAP_XY_FACTOR / m_cellHeight);
		...
		dstTextureWidth = m_numMaxVisibleCellsX =
				REAL_TO_INT_FLOOR((Real)(pMap->getDrawWidth() - 1) * MAP_XY_FACTOR / m_cellWidth) + 1;
		dstTextureHeight = m_numMaxVisibleCellsY =
				REAL_TO_INT_FLOOR((Real)(pMap->getDrawHeight() - 1) * MAP_XY_FACTOR / m_cellHeight) + 1;
	...
	if (dstTextureWidth != m_dstTextureWidth || dstTextureHeight != m_dstTextureHeight) ///@todo: Check if size has changed -
																																											/// probably never
...
Bool W3DShroud::ReAcquireResources(void)
{
	...
	if (TheGlobalData && TheGlobalData->m_fogOfWarOn)
		m_pDstTexture = MSGNEW("TextureClass")
				TextureClass(m_dstTextureWidth, m_dstTextureHeight, WW3D_FORMAT_A4R4G4B4, MIP_LEVELS_1, TextureClass::POOL_DEFAULT);
	else
#endif
		m_pDstTexture = MSGNEW("TextureClass")
				TextureClass(m_dstTextureWidth, m_dstTextureHeight, WW3D_FORMAT_R5G6B5, MIP_LEVELS_1, TextureClass::POOL_DEFAULT);
...
void W3DShroud::setShroudLevel(Int x, Int y, W3DShroudLevel level, Bool textureOnly)
{
	...
	if (x < m_numCellsX && y < m_numCellsY)
	{
		...
		if (TheGlobalData && TheGlobalData->m_fogOfWarOn)
		{
			...
			UnsignedShort pixel = ((blueVal >> 4) & 0xf) | (((greenVal >> 4) & 0xf) << 4) | (((redVal >> 4) & 0xf) << 8)
					| (((alphaVal >> 4) & 0xf) << 12);
...
#endif
		{
			...
			UnsignedInt greenpixel =
					(UnsignedInt)((Real)level * ((Real)((TheGlobalData->m_shroudColor.getAsInt() & 0xff00) >> 8) / 255.0f));
			UnsignedInt redpixel =
					(UnsignedInt)((Real)level * ((Real)((TheGlobalData->m_shroudColor.getAsInt() & 0xff0000) >> 16) / 255.0f));
...
void W3DShroud::fillShroudData(W3DShroudLevel level)
{
	...
	if (TheGlobalData && TheGlobalData->m_fogOfWarOn)
	{
		...
		pixel = ((blueVal >> 4) & 0xf) | (((greenVal >> 4) & 0xf) << 4) | (((redVal >> 4) & 0xf) << 8)
				| (((alphaVal >> 4) & 0xf) << 12);
	}
	else
#endif
...
void W3DShroud::fillBorderShroudData(W3DShroudLevel level, SurfaceClass *pDestSurface)
{
	...
	for (y = 0; y < m_dstTextureHeight; y++)
	{
		...
		for (x = 0; x < numFullCopies; x++)
		{
			...
			DX8Wrapper::_Copy_DX8_Rects(m_pSrcTexture, &srcRect, 1, pDestSurface->Peek_D3D_Surface(), &dstPoint);
		}
		if (numExtraPixels)
		{
...
void W3DShroud::render(CameraClass *cam)
{
	...
	if (doInit)
	{
		...
		for (Int y = 0; y < desc.Height; y++)
		{
			for (Int x = 0; x < desc.Width; x++)
			{
				...
				if (TheGlobalData && TheGlobalData->m_fogOfWarOn)
				{
					dataDest[x] = ((TheGlobalData->m_shroudColor.getAsInt() >> 4) & 0xf)
							| (((TheGlobalData->m_shroudColor.getAsInt() >> 12) & 0xf) << 4)
							| (((TheGlobalData->m_shroudColor.getAsInt() >> 20) & 0xf) << 8)
							| ((((255 - (dataSrc[x] & 0xff)) >> 4) & 0xf) << 12);
					//					dataDest[x]=((SHROUD_COLOR>>4)&0xf) | (((SHROUD_COLOR>>12)&0xf)<<4) | (((SHROUD_COLOR>>20)&0xf)<<8) |
					//((((255-(dataSrc[x] & 0xff))>>4)&0xf)<<12);
				}
				else
#endif
				{
					dataDest[x] =
							((dataSrc[x] >> 3) & 0x1f) | (((dataSrc[x] >> 10) & 0x3f) << 5) | (((dataSrc[x] >> 19) & 0x1f) << 11);
				}
			}
	...
	Int visStartX =
			REAL_TO_INT_FLOOR((Real)(hm->getDrawOrgX() - hm->getBorderSizeInline()) * MAP_XY_FACTOR / m_cellWidth); // start of
																																																							// rendered
																																																							// heightmap
																																																							// rectangle
	if (visStartX < 0)
	...
	Int visEndX = visStartX + REAL_TO_INT_FLOOR((Real)(hm->getDrawWidth() - 1) * MAP_XY_FACTOR / m_cellWidth) + 1; // size of
																																																								 // rendered
																																																								 // heightmap
																																																								 // rectangle
