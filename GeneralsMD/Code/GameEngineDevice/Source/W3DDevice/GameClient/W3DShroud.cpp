...
void W3DShroud::init(WorldHeightMap *pMap, Real worldCellSizeX, Real worldCellSizeY)
{
  ...
  if (dstTextureWidth != m_dstTextureWidth
      || dstTextureHeight != m_dstTextureHeight) ///@todo: Check if size has changed - probably never
...
void W3DShroud::render(CameraClass *cam)
{
    ...
    for (Int y = 0; y < desc.Height; y++)
    {
      for (Int x = 0; x < desc.Width; x++)
      {
        ...
        if (TheGlobalData && TheGlobalData->m_fogOfWarOn)
        {
          ...
          //					dataDest[x]=((SHROUD_COLOR>>4)&0xf) | (((SHROUD_COLOR>>12)&0xf)<<4) |
          //(((SHROUD_COLOR>>20)&0xf)<<8) | ((((255-(dataSrc[x] & 0xff))>>4)&0xf)<<12);
        }
        else
#endif
        {
  ...
  Int visStartX = REAL_TO_INT_FLOOR(
      (Real)(hm->getDrawOrgX() - hm->getBorderSizeInline()) * MAP_XY_FACTOR
  ...
  Int visEndX = visStartX + REAL_TO_INT_FLOOR((Real)(hm->getDrawWidth() - 1) * MAP_XY_FACTOR / m_cellWidth)
