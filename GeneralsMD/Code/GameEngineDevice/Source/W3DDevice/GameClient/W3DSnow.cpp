...
#define SNOW_BATCH_SIZE \
    2048 // we render at most this many particles per drawprimitive call.  This number * 6 must be less than 65536 to fit
         // into index buffer.
...
Bool W3DSnowManager::ReAcquireResources(void)
{
    ...
    {
        m_indexBuffer =
            NEW_REF(DX8IndexBufferClass, (SNOW_BATCH_SIZE * 6)); // allocate 2 triangles per flake, each with 3 indices.
...
void W3DSnowManager::renderSubBox(RenderInfoClass &rinfo, Int originX, Int originY, Int cubeDimX, Int cubeDimY)
{
    ...
    { // subdivide the box
        ...
        { // subdivide in both directions
            // Upper left
            mmbox.MinCorner.Set(
                originX * m_emitterSpacing - m_cullOverscan,
                (originY + halfY) * m_emitterSpacing - m_cullOverscan,
                m_snowCeiling - m_boxDimensions);
            mmbox.MaxCorner.Set(
                (originX + halfX) * m_emitterSpacing + m_cullOverscan,
                cubeDimY * m_emitterSpacing + m_cullOverscan,
                m_snowCeiling);
            ...
            mmbox.MinCorner.Set(
                ...
                m_snowCeiling - m_boxDimensions);
            mmbox.MaxCorner.Set(
                cubeDimX * m_emitterSpacing + m_cullOverscan,
                cubeDimY * m_emitterSpacing + m_cullOverscan,
                m_snowCeiling);
            if (CollisionMath::Overlap_Test(camera.Get_Frustum(), mmbox) != CollisionMath::OUTSIDE)
...
void W3DSnowManager::render(RenderInfoClass &rinfo)
{
    ...
    m_leafDim =
        45; // cull boxes that are 20x20 emitters in size. Making them much smaller will result in too many draw calls.
...
void W3DSnowManager::renderAsQuads(RenderInfoClass &rinfo, Int cubeOriginX, Int cubeOriginY, Int cubeDimX, Int cubeDimY)
{
    ...
    while (totalPart)
    {
        ...
        DynamicVBAccessClass vb_access(
            ...
            dynamic_fvf_type,
            batchSize * 4); // allocate 4 verts per flake
        {
