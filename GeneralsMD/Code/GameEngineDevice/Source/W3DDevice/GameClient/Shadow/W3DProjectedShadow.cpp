...
Bool W3DProjectedShadowManager::ReAcquireResources(void)
{
	...
	if ((m_dynamicRenderTarget =
					 DX8Wrapper::Create_Render_Target(DEFAULT_RENDER_TARGET_WIDTH, DEFAULT_RENDER_TARGET_HEIGHT, WW3D_FORMAT_A8R8G8B8))
			== NULL)
	{
		m_renderTargetHasAlpha = FALSE;
...
void W3DProjectedShadowManager::queueDecal(W3DProjectedShadow *shadow)
{
	...
	*/
		...
		// rotation. shadow->m_shadowTexture[0]->getDecalUVAxis(&uVector,&vVector); uVector *= 20.0f/dx;//shadow->m_decalRadius;
		// //scale texture to fit object vVector *= 20.0f/dy;//1.2f;//shadow->m_decalRadius;	//scale texture to fit object
		/*		uVector=objXform.Get_X_Vector();
				uVector.Normalize();
				uVector /= decalSizeX + (1.0f+4.0f/64.0f);	//texture has 1 pixel transparent border so we strtech up to make sure
			 solid pixels reach extent.. vVector=objXform.Get_Y_Vector() * -1.0f;	//invert direction since v axis runs right
			 relative to u. vVector.Normalize(); vVector /= decalSizeY + (1.0f+4.0f/64.0f);
		*/
		...
		for (shadow = m_shadowList; shadow; shadow = shadow->m_next)
		{
			if (shadow->m_isEnabled && !shadow->m_isInvisibleEnabled)
			{
				if (shadow->m_type & SHADOW_DECAL)
				{
					...
					{
						flushDecals(lastShadowDecalTexture, lastShadowType); // switched to a new texture, need to render polys using
																																 // last texture.
					...
					for (obj = iter->first(); obj; obj = iter->next())
					{
						...
						for (DrawModule **dm = draw->getDrawModules(); *dm; ++dm)
						{
							...
							if (di)
							{
								...
								if ((robj = w3dDraw->getRenderObject()) != 0 && (!robj->Is_Alpha() || !obj->isKindOf(KINDOF_IMMOBILE))
	...
	if (m_decalList)
	{
		...
		for (shadow = m_decalList; shadow; shadow = shadow->m_next)
		{
			if (shadow->m_isEnabled && !shadow->m_isInvisibleEnabled)
			{
				...
				if (shadow->m_shadowTexture[0] != lastShadowDecalTexture || shadow->m_type != lastShadowType)
				{
					flushDecals(lastShadowDecalTexture, lastShadowType); // switched to a new texture, need to render polys using last
																															 // texture.
					lastShadowDecalTexture = shadow->m_shadowTexture[0];
					lastShadowType = shadow->m_type;
				}
...
Shadow *W3DProjectedShadowManager::addDecal(Shadow::ShadowTypeInfo *shadowInfo)
{
	...
	shadow->m_allowWorldAlign = allowWorldAlign; /// wrap shadow around world geometry - else align perpendicular to local
																							 /// z-axis.
