...
static int FVFTypeIndexList[W3DBufferManager::MAX_FVF] = { D3DFVF_XYZ,
	...
	for (Int i = 0; i < MAX_FVF; i++)
	{
		...
		while (vb)
		{
			...
			vb->m_DX8VertexBuffer =
					NEW_REF(DX8VertexBufferClass, (FVFTypeIndexList[vb->m_format], vb->m_size, DX8VertexBufferClass::USAGE_DEFAULT));
...
W3DBufferManager::W3DVertexBufferSlot *W3DBufferManager::allocateSlotStorage(VBM_FVF_TYPES fvfType, Int size)
{
	...
	if (m_numEmptyVertexBuffersAllocated < MAX_VERTEX_BUFFERS_CREATED)
	{
		...
		pVB->m_DX8VertexBuffer =
				NEW_REF(DX8VertexBufferClass, (FVFTypeIndexList[fvfType], vbSize, DX8VertexBufferClass::USAGE_DEFAULT));
