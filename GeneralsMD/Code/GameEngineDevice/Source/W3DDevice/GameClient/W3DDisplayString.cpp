...
void W3DDisplayString::draw(Int x, Int y, Color color, Color dropColor, Int xDrop, Int yDrop)
{
	...
	//
	if (needNewPolys || x != m_textPos.x || y != m_textPos.y || color != m_currTextColor || dropColor != m_currDropColor)
	{
...
void W3DDisplayString::setFont(GameFont *font)
{
	...
	m_textRendererHotKey.Set_Font(
			static_cast<FontCharsClass *>(TheFontLibrary->getFont(font->nameString, font->pointSize, TRUE)->fontData));
...
void W3DDisplayString::setClipRegion(IRegion2D *region)
{
			...
			|| region->hi.y != m_clipRegion.hi.y)
	{
		...
		m_textRenderer.Set_Clipping_Rect(RectClass(m_clipRegion.lo.x, m_clipRegion.lo.y, m_clipRegion.hi.x, m_clipRegion.hi.y));
