...
void BaseHeightMapRenderObjClass::adjustTerrainLOD(Int adj)
{
    ...
    if (newROBJ)
    {
        ...
        newROBJ->initHeightData(m_map->getDrawWidth(), m_map->getDrawHeight(), m_map, NULL);
...
void BaseHeightMapRenderObjClass::Get_Obj_Space_Bounding_Sphere(SphereClass &sphere) const
{
    ...
    Vector3 ObjSpaceCenter(
        (float)x * 0.5f * MAP_XY_FACTOR,
        (float)y * 0.5f * MAP_XY_FACTOR,
...
// ============================================================================
void BaseHeightMapRenderObjClass::worldBuilderUpdateBridgeTowers(W3DAssetManager *assetManager, SimpleSceneClass *scene)
{
    if (m_bridgeBuffer)
...
used by the world builder since it modifies the terrain too frequently.  The function also assumes that
updateShoreLineTiles() was previously called and inserted the tiles in the correctly sorted order. WARNING!!! Current version
assumes we always sort the entire map!  So don't set parameters to partial updates! */
...
void BaseHeightMapRenderObjClass::updateShorelineTile(Int i, Int j, Int border, WorldHeightMap *pMap)
{
    ...
    waterSide =
        (waterZ0 = TheWaterRenderObj->getWaterHeight(X0, Y0)) > ((terrainZ0 = MAP_HEIGHT_SCALE * pMap->getHeight(i, j)));
    Real X1 = (i - border + 1) * MAP_XY_FACTOR;
    Real Y1 = (j - border + 1) * MAP_XY_FACTOR;
    waterSide |= ((waterZ1 = TheWaterRenderObj->getWaterHeight(X1, Y0))
                  > ((terrainZ1 = MAP_HEIGHT_SCALE * pMap->getHeight(i + 1, j))))
        << 1;
    waterSide |= ((waterZ2 = TheWaterRenderObj->getWaterHeight(X1, Y1))
                  > ((terrainZ2 = MAP_HEIGHT_SCALE * pMap->getHeight(i + 1, j + 1))))
        << 2;
    waterSide |= ((waterZ3 = TheWaterRenderObj->getWaterHeight(X0, Y1))
                  > ((terrainZ3 = MAP_HEIGHT_SCALE * pMap->getHeight(i, j + 1))))
        << 3;

    if (!waterSide || (waterZ0 * waterZ1 * waterZ2 * waterZ3) <= 0)
        return; // all verts are on positive (surface) side of water so don't need blending.  Or one of them is outside the
...
//=============================================================================
Int BaseHeightMapRenderObjClass::initHeightData(
    Int x,
    Int y,
    WorldHeightMap *pMap,
    ...
    for (curScorch = m_numScorches - 1; curScorch >= 0; curScorch--)
    {
        ...
        for (j = minY; j < maxY; j++)
        {
            for (i = minX; i < maxX; i++)
            {
                ...
                Real theZ;
                theZ = amtToFloat
                    + ((float)getClipHeight(i + m_map->getBorderSizeInline(), j + m_map->getBorderSizeInline())
                       * MAP_HEIGHT_SCALE);
...
//=============================================================================
void BaseHeightMapRenderObjClass::addTree(
    DrawableID id,
    Coord3D location,
    Real scale,
    Real angle,
...
//=============================================================================
void BaseHeightMapRenderObjClass::addTerrainBib(Vector3 corners[4], ObjectID id, Bool highlight)
{
    if (m_bibBuffer)
...
/** Adds a terrainBib to the bib buffer.*/
//=============================================================================
void BaseHeightMapRenderObjClass::addTerrainBibDrawable(Vector3 corners[4], DrawableID id, Bool highlight)
{
    if (m_bibBuffer)
