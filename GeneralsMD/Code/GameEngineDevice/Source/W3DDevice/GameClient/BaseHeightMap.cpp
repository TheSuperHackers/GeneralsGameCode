...
void BaseHeightMapRenderObjClass::updateShorelineTile(Int i, Int j, Int border, WorldHeightMap *pMap)
{
	...
	waterSide |=
			((waterZ1 = TheWaterRenderObj->getWaterHeight(X1, Y0)) > ((terrainZ1 = MAP_HEIGHT_SCALE * pMap->getHeight(i + 1, j))))
			<< 1;
	waterSide |= ((waterZ2 = TheWaterRenderObj->getWaterHeight(X1, Y1))
								> ((terrainZ2 = MAP_HEIGHT_SCALE * pMap->getHeight(i + 1, j + 1))))
			<< 2;
	waterSide |=
			((waterZ3 = TheWaterRenderObj->getWaterHeight(X0, Y1)) > ((terrainZ3 = MAP_HEIGHT_SCALE * pMap->getHeight(i, j + 1))))
			<< 3;

	if (!waterSide || (waterZ0 * waterZ1 * waterZ2 * waterZ3) <= 0)
		return; // all verts are on positive (surface) side of water so don't need blending.  Or one of them is outside the water
						// plane bounds (waterHeight <= 0!)
	...
	for (curScorch = m_numScorches - 1; curScorch >= 0; curScorch--)
	{
		...
		for (j = minY; j < maxY; j++)
		{
			for (i = minX; i < maxX; i++)
			{
				...
				Real theZ;
				theZ = amtToFloat
						+ ((float)getClipHeight(i + m_map->getBorderSizeInline(), j + m_map->getBorderSizeInline()) * MAP_HEIGHT_SCALE);
