...
class ScreenDefaultFilter : public W3DFilterInterface
{
  ...
  virtual Bool preRender(
      Bool &skipRender,
      CustomScenePassModes &scenePassMode); ///< Set up at start of render.  Only applies to screen filter shaders.
  virtual Bool postRender(
      FilterModes mode,
      Coord2D &scrollDelta,
      Bool &doExtraRender); ///< Called after render.  Only applies to screen filter shaders.
...
Int MaskTextureShader::set(Int pass)
{
  ...
  Real worldTexelWidth =
      (1.0f - fadeLevel) * 25.0f; // 9 worked well for circle but weird shape requires more stretch to cover.
...
class TerrainShader2Stage : public W3DShaderInterface
{
  ...
  void updateNoise1(
      ...
      Bool doUpdate = true); ///< generate the uv coordinates for Noise1 (i.e clouds)
  void updateNoise2(
      D3DXMATRIX *destMatrix,
      D3DXMATRIX *curViewInverse,
      Bool doUpdate = true); ///< generate the uv coordinates for Noise2 (i.e lightmap)
} terrainShader2Stage;
...
IDirect3DTexture8 *W3DShaderManager::endRenderToTexture(void)
{
  ...
  HRESULT hr = DX8Wrapper::_Get_D3D_Device8()->SetRenderTarget(
      m_oldRenderSurface,
      m_oldDepthSurface); // restore original render target
  DEBUG_ASSERTCRASH(hr == S_OK, ("Set target failed unexpectedly."));
  if (hr == S_OK)
  {
    ...
    case 0:
      ...
      if (W3DShaderManager::getShaderTexture(0))
      {
        ...
        if ((shroud = TheTerrainRenderObject->getShroud()) != 0)
        {
          ...
          if (TheTerrainRenderObject->getMap())
          { // subtract origin position from all coordinates.  Origin is shifted by 1 cell width/height to allow
