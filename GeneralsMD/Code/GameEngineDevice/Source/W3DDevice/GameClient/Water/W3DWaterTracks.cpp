		...
		Int waveTimeOffset)
{
	...
	m_waveInitialHeight = m_waveInitialWidth * waveTypeInfo[m_type].m_initialHeightWidthFraction; ///< height of wave segment
	...
	m_frontSlowDownAcc = -(m_initialVelocity * m_initialVelocity) / (2 * m_waveFinalHeight); // deceleration of wave after it
																																													 // hits land
	...
	m_backSlowDownAcc =
			(2.0f * m_waveInitialHeight
			 / (m_timeToStop * m_timeToStop)); //((m_waveInitialHeight -
																				 // m_velocity*m_timeToStop)*2.0f)/(m_timeToStop*m_timeToStop);
	m_timeToCompress = waveTypeInfo[m_type].m_timeToCompress; // time for back of wave to continue moving forward after front
																														// starts retreating.
...
Int WaterTracksObj::render(DX8VertexBufferClass *vertexBuffer, Int batchStart)
{
	...
	{ // stationary wave
		...
		{ // fading out
			waveAlpha = m_elapsedMs - (m_timeToReachBeach + m_timeToStop - 1000 + m_fadeMs); //(m_totalMs-m_timeToRetreat -m_fadeMs
		...
		{ // start fading up
			...
			waveAlpha = m_elapsedMs - (m_timeToReachBeach + m_timeToStop - 1000); //(m_totalMs-m_timeToRetreat -m_fadeMs -
																																						// m_elapsedMs)/m_fadeMs;
