    ...
    if (surface_desc.Format == D3DFMT_A8R8G8B8)
    {
        ...
        for (tileNdx = 0; tileNdx < buffer->getNumTiles(); tileNdx++)
        {
            ...
            for (j = 0; j < tilePixelExtent; j++)
            {
                ...
                UnsignedByte *pBGRA = ((UnsignedByte *)locked_rect.pBits) + (row)*surface_desc.Width * pixelBytes;
...
#define SC_ALPHA_DETAIL_2X \
    (SHADE_CNST( \
        ...
        ShaderClass::DSTBLEND_ONE_MINUS_SRC_ALPHA, \
        ShaderClass::FOG_DISABLE, \
        ShaderClass::GRADIENT_MODULATE2X, \
        ShaderClass::SECONDARY_GRADIENT_DISABLE, \
        ShaderClass::TEXTURING_ENABLE, \
        ShaderClass::ALPHATEST_ENABLE, \
        ShaderClass::CULL_MODE_ENABLE, \
...
/*
#define SC_ALPHA_DETAIL ( SHADE_CNST(ShaderClass::PASS_LEQUAL, ShaderClass::DEPTH_WRITE_ENABLE,
ShaderClass::COLOR_WRITE_ENABLE, ShaderClass::SRCBLEND_ONE, \
    ShaderClass::DSTBLEND_ZERO, ShaderClass::FOG_DISABLE, ShaderClass::GRADIENT_MODULATE,
ShaderClass::SECONDARY_GRADIENT_DISABLE, ShaderClass::TEXTURING_ENABLE, \
...
/*
#define SC_ALPHA_DETAIL ( SHADE_CNST(ShaderClass::PASS_LEQUAL, ShaderClass::DEPTH_WRITE_DISABLE,
ShaderClass::COLOR_WRITE_ENABLE, ShaderClass::SRCBLEND_SRC_ALPHA, \
    ShaderClass::DSTBLEND_ONE_MINUS_SRC_ALPHA, ShaderClass::FOG_DISABLE, ShaderClass::GRADIENT_MODULATE,
ShaderClass::SECONDARY_GRADIENT_DISABLE, ShaderClass::TEXTURING_ENABLE, \
...
/*
#define SC_ALPHA_MIRROR ( SHADE_CNST(ShaderClass::PASS_LEQUAL, ShaderClass::DEPTH_WRITE_DISABLE,
ShaderClass::COLOR_WRITE_ENABLE, ShaderClass::SRCBLEND_SRC_ALPHA, \
    ShaderClass::DSTBLEND_ONE_MINUS_SRC_ALPHA, ShaderClass::FOG_DISABLE, ShaderClass::GRADIENT_MODULATE,
ShaderClass::SECONDARY_GRADIENT_DISABLE, ShaderClass::TEXTURING_ENABLE, \
    ShaderClass::DETAILCOLOR_DISABLE, ShaderClass::DETAILALPHA_DISABLE, ShaderClass::ALPHATEST_DISABLE,
ShaderClass::CULL_MODE_DISABLE, \ ShaderClass::DETAILCOLOR_DISABLE, ShaderClass::DETAILALPHA_DISABLE) )
...
Int W3DTreeBuffer::getPartitionBucket(const Coord3D &pos) const
{
    ...
    DEBUG_ASSERTCRASH(
        xIndex >= 0 && yIndex >= 0 && xIndex < PARTITION_WIDTH_HEIGHT && yIndex < PARTITION_WIDTH_HEIGHT,
        ("Invalid range."));
...
public:
    ...
    virtual Int read(void *pData, Int numBytes) { return (m_file ? m_file->read(pData, numBytes) : 0); };
};
...
//=============================================================================
UnsignedInt W3DTreeBuffer::doLighting(
        ...
        for (; curTree < m_numTrees; curTree++)
        {
             ...
             *
                    ...
                    if (doPanel) {
                        ...
                        for (j=0; j<6; j++) {
                            ...
                            curVb->z = vLoc.Z;
                            if (doVertexLighting) {
                                curVb->diffuse = doLighting(&vLoc, shadeR, shadeG, shadeB, m_trees[curTree].bounds,
             pDynamicLightsIterator); } else { curVb->diffuse = diffuse;
                            }
                            curVb++;
            ...
            for (i = 0; i < numVertex; i++)
            {
                ...
                else
                {
                    ...
                    {
                        vLoc.X += pVert[i].Z * m_trees[curTree].pushAside * m_trees[curTree].pushAsideCos
                            * m_treeTypes[type].m_data->m_maxOutwardMovement;
                        vLoc.Y += pVert[i].Z * m_trees[curTree].pushAside * m_trees[curTree].pushAsideSin
                            * m_treeTypes[type].m_data->m_maxOutwardMovement;
                    }
                    vLoc.X += loc.X;
                    vLoc.Y += loc.Y;
                ...
                if (doVertexLighting)
                {
                    ...
                    curVb->diffuse = doLighting(&normal, objectLighting, &emissive, vertexDiffuse, 1.0f);
                }
                else
                {
...
#define TREE_RADIUS_APPROX 7.0f
    ...
    DEBUG_ASSERTCRASH(
        xMax >= 0 && yMax >= 0 && xMax <= PARTITION_WIDTH_HEIGHT && yMax <= PARTITION_WIDTH_HEIGHT,
        ("Invalid range."));
    Int i, j;
...
#ifdef USE_STATIC
        m_vertexTree[i] =
            NEW_REF(DX8VertexBufferClass, (DX8_FVF_XYZNDUV1, MAX_TREE_VERTEX + 4, DX8VertexBufferClass::USAGE_DEFAULT));
        m_indexTree[i] = NEW_REF(DX8IndexBufferClass, (MAX_TREE_INDEX + 4, DX8IndexBufferClass::USAGE_DEFAULT));
#else
        m_vertexTree[i] =
            NEW_REF(DX8VertexBufferClass, (DX8_FVF_XYZNDUV1, MAX_TREE_VERTEX + 4, DX8VertexBufferClass::USAGE_DYNAMIC));
...
//=============================================================================
void W3DTreeBuffer::pushAsideTree(DrawableID id, const Coord3D *pusherPos, const Coord3D *pusherDirection, ObjectID pusherID)
{
    Int i;
    for (i = 0; i < m_numTrees; i++)
    {
...
void W3DTreeBuffer::drawTrees(CameraClass *camera, RefRenderObjListIterator *pDynamicLightsIterator)
{
    ...
    if (m_shadow && TheW3DProjectedShadowManager && TheGlobalData->m_useShadowDecals)
    {
        for (curTree = 0; curTree < m_numTrees; curTree++)
        {
            ...
            Real factor = 1.0f;
            if (m_trees[curTree].m_toppleState == TOPPLE_FALLING || m_trees[curTree].m_toppleState == TOPPLE_DOWN)
            {
                continue;
            }
    ...
    for (curTree = 0; curTree < m_numTrees; curTree++)
    {
        ...
        if (m_trees[curTree].m_toppleState == TOPPLE_FALLING || m_trees[curTree].m_toppleState == TOPPLE_FOGGED)
        {
        ...
        else if (m_trees[curTree].m_toppleState == TOPPLE_DOWN)
        {
            if (m_treeTypes[type].m_data->m_killWhenToppled)
            {
                ...
                m_trees[curTree].location.Z -=
                    m_treeTypes[type].m_data->m_sinkDistance / m_treeTypes[type].m_data->m_sinkFrames;
...
//-------------------------------------------------------------------------------------------------
void W3DTreeBuffer::applyTopplingForce(TTree *tree, const Coord3D *toppleDirection, Real toppleSpeed, UnsignedInt options)
{
...
void W3DTreeBuffer::xfer(Xfer *xfer)
{
    ...
    for (i = 0; i < numTrees; i++)
    {
        ...
        xfer->xferUnsignedInt(
            &tree.m_sinkFramesLeft); ///< Toppled trees sink into the terrain & disappear, how many frames left.
