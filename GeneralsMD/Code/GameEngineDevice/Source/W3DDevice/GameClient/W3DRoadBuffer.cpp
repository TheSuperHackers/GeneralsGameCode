...
//=============================================================================
RoadType::RoadType(void) : m_roadTexture(NULL), m_vertexRoad(NULL), m_indexRoad(NULL), m_stackingOrder(0), m_uniqueID(-1)
{
}
...
void RoadType::loadTexture(AsciiString path, Int ID)
{
	...
	// m_roadTexture = pMgr->Get_Texture(path.str(), MIP_LEVELS_3, WW3D_FORMAT_UNKNOWN,true,TextureBaseClass::TEX_REGULAR,
	// false);
	...
	m_vertexRoad = NEW_REF(
			DX8VertexBufferClass,
			(DX8_FVF_XYZDUV1,
			 TheGlobalData->m_maxRoadVertex + 4,
			 (s_dynamic ? DX8VertexBufferClass::USAGE_DYNAMIC : DX8VertexBufferClass::USAGE_DEFAULT)));
	m_indexRoad = NEW_REF(
			DX8IndexBufferClass,
			(TheGlobalData->m_maxRoadIndex + 4,
			 (s_dynamic ? DX8IndexBufferClass::USAGE_DYNAMIC : DX8IndexBufferClass::USAGE_DEFAULT)));
	m_numRoadVertices = 0;
	m_numRoadIndices = 0;
...
//=============================================================================
void W3DRoadBuffer::loadTee(RoadSegment *pRoad, Vector2 loc1, Vector2 loc2, Bool is4Way, Real scale)
{
...
//=============================================================================
void W3DRoadBuffer::loadAlphaJoin(RoadSegment *pRoad, Vector2 loc1, Vector2 loc2, Real scale)
{
	Vector2 roadVector(loc2.X - loc1.X, loc2.Y - loc1.Y);
	Vector2 roadNormal(-roadVector.Y, roadVector.X);
	//	Real roadLen = scale;
...
//=============================================================================
void W3DRoadBuffer::loadY(RoadSegment *pRoad, Vector2 loc1, Vector2 loc2, Real scale)
{
	Vector2 roadVector(loc2.X - loc1.X, loc2.Y - loc1.Y);
	Vector2 roadNormal(-roadVector.Y, roadVector.X);
	//	Real roadLen = scale;
...
//=============================================================================
void W3DRoadBuffer::loadH(RoadSegment *pRoad, Vector2 loc1, Vector2 loc2, Bool flip, Real scale)
{
	Vector2 roadVector(loc2.X - loc1.X, loc2.Y - loc1.Y);
	Vector2 roadNormal(-roadVector.Y, roadVector.X);
	//	Real roadLen = scale;
...
terrain. */
//=============================================================================
void W3DRoadBuffer::loadFloatSection(
		RoadSegment *pRoad,
		Vector2 loc,
		Vector2 roadVector,
		Real halfHeight,
...
to the road normal.  */
//=============================================================================
void W3DRoadBuffer::loadFloat4PtSection(
		RoadSegment *pRoad,
		Vector2 loc,
		Vector2 roadNormal,
		Vector2 roadVector,
	...
	for (i = 0; i <= uCount; i++)
	{
		...
		if (!curColumn.deleted && i != 1)
		{
			...
			for (j = 0; j < vCount; j++)
			{
				...
				Int diffuse = (255 << 24)
						| TheTerrainRenderObject->getStaticDiffuse(
...
void W3DRoadBuffer::preloadRoadsInVertexAndIndexBuffers()
{
	...
	for (curRoad = 0; curRoad < m_numRoads; curRoad++)
	{
		...
		{
			loadY(&m_roads[curRoad], m_roads[curRoad].m_pt1.loc, m_roads[curRoad].m_pt2.loc, m_roads[curRoad].m_scale);
		}
	}
	// Do tees.
	for (curRoad = 0; curRoad < m_numRoads; curRoad++)
	...
	for (curRoad = 0; curRoad < m_numRoads; curRoad++)
	{
		if (m_roads[curRoad].m_type == TEE || m_roads[curRoad].m_type == FOUR_WAY)
		{
			loadTee(
					...
					m_roads[curRoad].m_pt1.loc,
					m_roads[curRoad].m_pt2.loc,
					(m_roads[curRoad].m_type == FOUR_WAY),
					m_roads[curRoad].m_scale);
		}
	}
	// Do joins.
	for (curRoad = 0; curRoad < m_numRoads; curRoad++)
	{
		...
		{
			loadAlphaJoin(&m_roads[curRoad], m_roads[curRoad].m_pt1.loc, m_roads[curRoad].m_pt2.loc, m_roads[curRoad].m_scale);
		}
	}
	updateLighting();
}
...
Bool W3DRoadBuffer::visibilityChanged(const IRegion2D &bounds)
{
	...
	for (curRoad = 0; curRoad < m_numRoads; curRoad++)
	{
		...
		if (m_roads[curRoad].m_pt1.loc.X + halfScale < bounds.lo.x && m_roads[curRoad].m_pt2.loc.X + halfScale < bounds.lo.x)
		{
			newVis = false;
		}
		if (m_roads[curRoad].m_pt1.loc.X - halfScale > bounds.hi.x && m_roads[curRoad].m_pt2.loc.X - halfScale > bounds.hi.x)
		{
			newVis = false;
		}
		// Throw out segs out of view.
		if (m_roads[curRoad].m_pt1.loc.Y + halfScale < bounds.lo.y && m_roads[curRoad].m_pt2.loc.Y + halfScale < bounds.lo.y)
		{
			newVis = false;
		}
		if (m_roads[curRoad].m_pt1.loc.Y - halfScale > bounds.hi.y && m_roads[curRoad].m_pt2.loc.Y - halfScale > bounds.hi.y)
		{
			newVis = false;
		}
		if (newVis != curVis)
...
the join points of the 3 way intersection. */
//=============================================================================
void W3DRoadBuffer::offsetY(TRoadPt *pc1, TRoadPt *pc2, TRoadPt *pc3, Vector2 loc, Vector2 upVector, Real widthInTexture)
{
...
void W3DRoadBuffer::updateLighting(void)
{
	...
	Found:
	When playing a 2 player game both as USA-based armies, if one of the players uses Satellite Spy to reveal an area not yet
	reveal by either player then Alt-Tab's out, and then the enemy send a dozer over to build some structure in the previously
	revealed area, then the game will crash to desktop for the player who Alt-Tab'd.
	...
	Result:
	As soon as the fence is set up, the player who Alt-tab'd would get Zero Hour crashing to desktop with Serious Error
	occured.
	*/
	if (!m_roads)
	{
