...
void W3DSmudgeManager::ReAcquireResources(void)
{
  ...
  m_indexBuffer =
      NEW_REF(DX8IndexBufferClass, (SMUDGE_DRAW_SIZE * 4 * 3)); // allocate 4 triangles per smudge, each with 3 indices.
...
Bool W3DSmudgeManager::testHardwareSupport(void)
{
  ...
  { // we have not done the test yet.
    ...
    Int bufSize = copyRect(
        (unsigned char *)refData,
        sizeof(refData),
        0,
        0,
        BLOCK_SIZE,
        BLOCK_SIZE); // copy area we just rendered using solid color
    if (!bufSize)
    {
      m_hardwareSupportStatus = SMUDGE_SUPPORT_NO;
      return FALSE;
