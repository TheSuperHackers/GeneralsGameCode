...
W3DLaserDraw::W3DLaserDraw(Thing *thing, const ModuleData *moduleData) :
    DrawModule(thing, moduleData), m_line3D(NULL), m_texture(NULL), m_textureAspectRatio(1.0f), m_selfDirty(TRUE)
{
    ...
    for (UnsignedInt segment = 0; segment < data->m_segments; segment++)
    {
        ...
        for (i = data->m_numBeams - 1; i >= 0; i--)
        {
            ...
            if (line)
            {
                ...
                line->Set_Shader(
                    ShaderClass::_PresetAdditiveShader); // pick the alpha blending mode you want - see shader.h for others.
                ...
                if (m_texture)
                {
                    line->Set_Texture_Mapping_Mode(
                        SegLineRendererClass::TILED_TEXTURE_MAP); // this tiles the texture across the line
                }
            ...
            for (Int i = data->m_numBeams - 1; i >= 0; i--)
            {
                ...
                if (m_texture && data->m_tile)
                {
                    ...
                    m_line3D[index]->Set_Texture_Tile_Factor(
                        tileFactor); // number of times to tile texture across each segment
                }
