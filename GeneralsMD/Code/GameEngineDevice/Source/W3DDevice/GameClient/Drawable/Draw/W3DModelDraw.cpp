...
void W3DModelDraw::adjustAnimation(const ModelConditionInfo *prevState, Real prevAnimFraction)
{
	...
	if (numAnims > 0)
	{
		...
		HAnimClass *animHandle = animInfo.getAnimHandle(); // note that this now returns an ADDREFED handle, which must be
...
Bool W3DModelDraw::updateBonesForClientParticleSystems()
{
	...
	if (drawable != NULL && m_curState != NULL && m_renderObject != NULL)
	{
				 ...
				 ++it)
		{
			...
			{
				const Matrix3D boneTransform = m_renderObject->Get_Bone_Transform(boneIndex); // just a little worried about state
																																											// changes
