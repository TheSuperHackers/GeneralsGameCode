...
void ModelConditionInfo::validateTurretInfo() const
{
  ...
  for (int tslot = 0; tslot < MAX_TURRETS; ++tslot)
  {
    ...
    if (tur.m_turretAngleNameKey != NAMEKEY_INVALID)
    {
      ...
      {
        DEBUG_CRASH(
            ("*** ASSET ERROR: TurretBone %s not found! (%s)", KEYNAME(tur.m_turretAngleNameKey).str(), m_modelName.str()));
        tur.m_turretAngleBone = 0;
      }
    }
    else
    ...
    if (tur.m_turretPitchNameKey != NAMEKEY_INVALID)
    {
      ...
      {
        DEBUG_CRASH(
            ("*** ASSET ERROR: TurretBone %s not found! (%s)", KEYNAME(tur.m_turretPitchNameKey).str(), m_modelName.str()));
        tur.m_turretPitchBone = 0;
      }
    }
    else
...
void W3DModelDraw::doStartOrStopParticleSys()
{
  ...
  for (std::vector<ParticleSysTrackerType>::const_iterator it = m_particleSystemIDs.begin(); it != m_particleSystemIDs.end();
       ++it)
...
void W3DModelDraw::rebuildWeaponRecoilInfo(const ModelConditionInfo *state)
{
  ...
  else
  {
    for (wslot = 0; wslot < WEAPONSLOT_COUNT; ++wslot)
    {
      ...
      for (WeaponRecoilInfoVec::iterator it = m_weaponRecoilInfoVec[wslot].begin(); it != m_weaponRecoilInfoVec[wslot].end();
           ++it)
      {
        it->clear();
      }
