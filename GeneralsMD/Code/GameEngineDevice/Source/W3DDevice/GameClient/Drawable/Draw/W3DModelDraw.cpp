...
{
    return (TheGameLogic && TheGameLogic->isInGameLogicUpdate()) || (TheGameState && TheGameState->isInLoadGame());
}
...
void LogClass::dumpMatrix3D(const Matrix3D *m, AsciiString name, AsciiString fname, Int line)
{
    ...
    log("dumpMatrix3D() %s:%d %s", fname.str(), line, name.str());
    for (Int i = 0; i < 3; ++i)
...
void LogClass::dumpReal(Real r, AsciiString name, AsciiString fname, Int line)
{
    ...
    fname.toLower();
    fname = fname.reverseFind('\\') + 1;
    log("dumpReal() %s:%d %s %8.8X (%f)", fname.str(), line, name.str(), AS_INT(r), r);
}
...
static const Int ALL_MAINTAIN_FRAME_FLAGS = (1 << MAINTAIN_FRAME_ACROSS_STATES) | (1 << MAINTAIN_FRAME_ACROSS_STATES2)
...
{
    for (std::vector<ModelConditionFlags>::const_iterator it = m_conditionsYesVec.begin(); it != m_conditionsYesVec.end();
         ++it)
    {
        if (it->test(MODELCONDITION_NIGHT) == (night) && it->test(MODELCONDITION_SNOW) == (snowy))
        {
            return true;
        }
    }
...
//-------------------------------------------------------------------------------------------------
void ModelConditionInfo::validateStuff(RenderObjClass *robj, Real scale, const std::vector<AsciiString> &extraPublicBones)
    const
{
    ...
    {
        for (std::vector<AsciiString>::const_iterator bone_it = extraPublicBones.begin(); bone_it != extraPublicBones.end();
             ++bone_it)
        {
...
void ModelConditionInfo::validateCachedBones(RenderObjClass *robj, Real scale) const
{
    ...
    if (TheGlobalData)
    {
        for (std::vector<AsciiString>::const_iterator it = TheGlobalData->m_standardPublicBones.begin();
             it != TheGlobalData->m_standardPublicBones.end();
             ++it)
        {
            if (!doSingleBoneName(robj, *it, m_pristineBones))
            {
                ...
                // DEBUG_CRASH(("public bone %s (and variations thereof) not found in model
                // %s!",it->str(),m_modelName.str()));
            }
            // else
            //{
    ...
    for (std::vector<AsciiString>::const_iterator it = m_publicBones.begin(); it != m_publicBones.end(); ++it)
    {
        if (!doSingleBoneName(robj, *it, m_pristineBones))
        {
            ...
            DEBUG_CRASH(
                ("*** ASSET ERROR: public bone '%s' (and variations thereof) not found in model %s!",
                 it->str(),
                 m_modelName.str()));
        }
        // else
...
void ModelConditionInfo::validateWeaponBarrelInfo() const
{
    ...
    for (int wslot = 0; wslot < WEAPONSLOT_COUNT; ++wslot)
    {
        ...
        //    ignore this assert if this model is used by a non-projectile-weapon-bearing unit\nModel name = %s.",
        //    m_modelName.str()) );
        // #endif
        ...
        if (fxBoneName.isNotEmpty() || recoilBoneName.isNotEmpty() || mfName.isNotEmpty() || plbName.isNotEmpty())
        {
            ...
            if (m_weaponBarrelInfoVec[wslot].empty())
            {
                ...
                {
                    CRCDEBUG_LOG(
                        ("validateWeaponBarrelInfo() - model name %s (unadorned) wslot %d", m_modelName.str(), wslot));
                    DUMPMATRIX3D(&(info.m_projectileOffsetMtx));
                    BONEPOS_LOG(
                        ("validateWeaponBarrelInfo() - model name %s (unadorned) wslot %d", m_modelName.str(), wslot));
                    BONEPOS_DUMPMATRIX3D(&(info.m_projectileOffsetMtx));
                ...
                {
                    CRCDEBUG_LOG(
                        ("validateWeaponBarrelInfo() - model name %s (unadorned) found nothing", m_modelName.str()));
            ...
            DEBUG_ASSERTCRASH(
                ...
                ("*** ASSET ERROR: No fx bone named '%s' found in model %s!", fxBoneName.str(), m_modelName.str()));
        }
    }
...
void ModelConditionInfo::validateTurretInfo() const
{
    ...
    for (int tslot = 0; tslot < MAX_TURRETS; ++tslot)
    {
        ...
        if (tur.m_turretAngleNameKey != NAMEKEY_INVALID)
        {
            ...
            {
                DEBUG_CRASH(
                    ("*** ASSET ERROR: TurretBone %s not found! (%s)",
...
//-------------------------------------------------------------------------------------------------
enum ParseCondStateType CPP_11( : Int){PARSE_NORMAL, PARSE_DEFAULT, PARSE_TRANSITION, PARSE_ALIAS};
...
void W3DModelDrawModuleData::buildFieldParse(MultiIniFieldParse &p)
{
    ...
    static const FieldParse dataFieldParse[] = {
        ...
        {"ParticlesAttachedToAnimatedBones",
         INI::parseBool,
         NULL,
         offsetof(W3DModelDrawModuleData, m_particlesAttachedToAnimatedBones)},
        ...
        {"IgnoreConditionStates",
         ModelConditionFlags::parseFromINI,
         NULL,
...
//-------------------------------------------------------------------------------------------------
enum AnimParseType CPP_11( : Int){ANIM_NORMAL, ANIM_IDLE};
...
void W3DModelDraw::showSubObject(const AsciiString &name, Bool show)
{
    ...
    {
        Bool found = false;
        for (std::vector<ModelConditionInfo::HideShowSubObjInfo>::iterator it = m_subObjectVec.begin();
             it != m_subObjectVec.end();
             ++it)
        {
...
void W3DModelDrawModuleData::parseConditionState(INI *ini, void *instance, void * /*store*/, const void *userData)
{
    ...
    switch (cst)
    {
        ...
        case PARSE_ALIAS:
        {
            ...
            {
                DEBUG_CRASH(
                    ("You should not specify bits in a state once they are used in IgnoreConditionStates (%s)",
                     TheThingTemplateBeingParsedName.str()));
                throw INI_INVALID_DATA;
            }
        ...
        case PARSE_NORMAL:
        {
            ...
            {
                // it doesn't actually NEED to be first, but it does need to be present, and this is the simplest way to
                // enforce...
                DEBUG_CRASH(
                    ("*** ASSET ERROR: when not using DefaultConditionState, the first ConditionState must be for NONE (%s)",
                     TheThingTemplateBeingParsedName.str()));
                throw INI_INVALID_DATA;
            }
            ...
            {
                DEBUG_CRASH(
                    ("*** ASSET ERROR: duplicate condition states are not currently allowed (%s)",
    ...
    {
        DEBUG_CRASH(
            ("*** ASSET ERROR: Idle Anims should always use ONCE or ONCE_BACKWARDS (%s)",
             TheThingTemplateBeingParsedName.str()));
        throw INI_INVALID_DATA;
    }
...
void W3DModelDraw::doStartOrStopParticleSys()
{
    ...
    for (std::vector<ParticleSysTrackerType>::const_iterator it = m_particleSystemIDs.begin();
         it != m_particleSystemIDs.end();
         ++it)
    // for (std::vector<ParticleSystemID>::const_iterator it = m_particleSystemIDs.begin(); it != m_particleSystemIDs.end();
    // ++it)
    {
...
/** Create shadow resources if not already present. This is used to dynamically enable/disable shadows by the options
 * screen*/
...
{
    if (m_curState != NULL && isAnyMaintainFrameFlagSet(m_curState->m_flags) && m_renderObject != NULL
...
void W3DModelDraw::adjustAnimation(const ModelConditionInfo *prevState, Real prevAnimFraction)
{
    ...
    if (numAnims > 0)
    {
        ...
        HAnimClass *animHandle =
            animInfo.getAnimHandle(); // note that this now returns an ADDREFED handle, which must be released by the caller!
        if (m_renderObject && animHandle)
        {
            ...
            // order is important here: MAINTAIN_FRAME_ACROSS_STATES is overridden by the other bits, above.
                ...
                isAnyMaintainFrameFlagSet(m_curState->m_flags) && prevState && prevState != m_curState
                && isAnyMaintainFrameFlagSet(prevState->m_flags)
                && isCommonMaintainFrameFlagSet(m_curState->m_flags, prevState->m_flags) && prevAnimFraction >= 0.0)
            {
...
*/
static void doHideShowBoneSubObjs(
    Bool state,
    Int numSubObjects,
    Int boneIdx,
...
void ModelConditionInfo::WeaponBarrelInfo::setMuzzleFlashHidden(RenderObjClass *fullObject, Bool hide) const
{
    if (fullObject)
    {
        ...
        {
            DEBUG_CRASH(
                ("*** ASSET ERROR: childObject %s not found in setMuzzleFlashHidden()", m_muzzleFlashBoneName.str()));
        }
    }
}
    ...
    for (int wslot = 0; wslot < WEAPONSLOT_COUNT; ++wslot)
    {
        ...
        for (Int i = 0; i < count; ++i)
        {
            if (barrels[i].m_muzzleFlashBone != 0)
            {
                ...
                // DEBUG_LOG(("adjust muzzleflash %08lx for Draw %08lx state %s to %d at frame
                // %d",subObjToHide,this,m_curState->m_description.str(),hidden?1:0,TheGameLogic->getFrame()));
            ...
            if (barrels[i].m_recoilBone != 0)
            {
                ...
                // %f",recoils[i].m_recoilBone,recoils[i].m_shift));
...
void W3DModelDraw::recalcBonesForClientParticleSystems()
{
    if (m_needRecalcBoneParticleSystems)
    {
        ...
        if (drawable != NULL)
        {
            ...
            {
                for (std::vector<ParticleSysBoneInfo>::const_iterator it = m_curState->m_particleSysBones.begin();
                     it != m_curState->m_particleSysBones.end();
                     ++it)
                {
             ...
             ++it)
        {
            ...
            if ((sys != NULL) && (boneIndex != 0))
            {
                const Matrix3D boneTransform =
                    m_renderObject->Get_Bone_Transform(boneIndex); // just a little worried about state changes
...
void W3DModelDraw::setModelState(const ModelConditionInfo *newState)
{
    ...
    {
        DEBUG_LOG(
            ("REQUEST switching to state %s for obj %s %d",
    ...
    if (m_curState != NULL && newState != NULL)
    {
        ...
        if ((m_curState == newState && m_nextState == NULL) || (m_curState != NULL && m_nextState == newState))
        {
            ...
            {
                DEBUG_LOG(
                    ("IGNORE duplicate state %s for obj %s %d",
                     newState->m_description.str(),
                     getDrawable()->getObject()->getTemplate()->getName().str(),
                     getDrawable()->getObject()->getID()));
            }
        ...
        // check for allow-to-finish implicit transitions
            ...
            newState != m_curState && newState->m_allowToFinishKey != NAMEKEY_INVALID
            && newState->m_allowToFinishKey == m_curState->m_transitionKey && m_renderObject
            && !isAnimationComplete(m_renderObject))
        {
            ...
            {
                DEBUG_LOG(
                    ("ALLOW_TO_FINISH state %s for obj %s %d",
                     newState->m_description.str(),
                     getDrawable()->getObject()->getTemplate()->getName().str(),
                     getDrawable()->getObject()->getID()));
            }
        ...
        // check for a normal->normal state transition
            ...
            newState != m_curState && m_curState->m_transitionKey != NAMEKEY_INVALID
            && newState->m_transitionKey != NAMEKEY_INVALID)
        {
            ...
            if (transState != NULL)
            {
                ...
                {
                    DEBUG_LOG(
                        ("using TRANSITION state %s before requested state %s for obj %s %d",
                         transState->m_description.str(),
                         newState->m_description.str(),
                         getDrawable()->getObject()->getTemplate()->getName().str(),
                         getDrawable()->getObject()->getID()));
                }
    ...
    if (m_curState == NULL || newState->m_modelName != m_curState->m_modelName || turretNamesDiffer(newState, m_curState)
    ...
    )
    {
        ...
        {
            m_renderObject =
                W3DDisplay::m_assetManager->Create_Render_Obj(newState->m_modelName.str(), draw->getScale(), m_hexColor);
        ...
        if (m_renderObject && TheGlobalData->m_makeTrackMarks && !m_trackRenderObject
            && TheTerrainTracksRenderObjClassSystem != NULL && !getW3DModelDrawModuleData()->m_trackFile.isEmpty())
        {
    ...
    Coord3D *turretPitchPos) const
{
    ...
    */
...
#if defined(RTS_DEBUG)
    CRCDEBUG_LOG(("W3DModelDraw::getProjectileLaunchOffset() for %s", stateToUse->getDescription().str()));
#endif
    ...
    DEBUG_ASSERTCRASH(
        stateToUse->m_transitionSig == NO_TRANSITION,
        ("It is never legal to getProjectileLaunchOffset from a Transition state (they vary on a per-client basis)... "
...
//-------------------------------------------------------------------------------------------------
Bool W3DModelDraw::handleWeaponFireFX(
    WeaponSlotType wslot,
    Int specificBarrelToUse,
    const FXList *fxl,
    Real weaponSpeed,
    ...
    Real damageRadius)
{
    ...
    if (fxl)
    {
        if (info.m_fxBone && m_renderObject)
        {
            ...
            else
            {
                ...
                // Unless of course they decide to not have the tracers come out of the windows, but rather go towards the
                target.
    ...
    if (info.m_recoilBone || info.m_muzzleFlashBone)
    {
        ...
        // %d",info.m_muzzleFlashBone,this,m_curState->m_description.str(),TheGameLogic->getFrame()));
...
// this is never defined -- srj
    ...
    if (m_curState != NULL && m_curState->m_transition != NO_TRANSITION && m_nextState != NULL
        && m_nextState->m_transition == NO_TRANSITION)
    {
    ...
    DEBUG_ASSERTCRASH(
        ...
        ("Hmm, setAnimationLoopDuration on a transition state is probably not right... see srj"));
#else
    m_nextStateAnimLoopDuration = NO_NEXT_DURATION;
#endif
    ...
    if (m_curState->m_weaponProjectileHideShowName[slot].isEmpty())
    {
        for (UnsignedInt projectileIndex = 0; projectileIndex < maxCount; projectileIndex++)
        {
            oneEntry.subObjName.format(
                "%s%02d",
                m_curState->m_weaponProjectileLaunchBoneName[slot].str(),
                (projectileIndex + 1));
...
void W3DModelDraw::updateSubObjects()
{
    ...
    if (!m_subObjectVec.empty())
    {
        for (std::vector<ModelConditionInfo::HideShowSubObjInfo>::const_iterator it = m_subObjectVec.begin();
             it != m_subObjectVec.end();
             ++it)
        {
            Int objIndex;
...
void W3DModelDraw::xfer(Xfer *xfer)
{
    ...
    if (version >= 2)
    {
        if (xfer->getXferMode() == XFER_SAVE)
        {
            ...
            if (m_renderObject && m_renderObject->Class_ID() == RenderObjClass::CLASSID_HLOD && m_curState
...
void W3DModelDrawModuleData::xfer(Xfer *x)
{
    ...
    for (ModelConditionVector::iterator it = m_conditionStates.begin(); it != m_conditionStates.end(); ++it)
    {
        ...
        {
            for (PristineBoneInfoMap::iterator bit = info->m_pristineBones.begin(); bit != info->m_pristineBones.end();
                 ++bit)
            {
            ...
            for (i = 0; i < WEAPONSLOT_COUNT; ++i)
            {
                for (ModelConditionInfo::WeaponBarrelInfoVec::iterator wit = info->m_weaponBarrelInfoVec[i].begin();
                     wit != info->m_weaponBarrelInfoVec[i].end();
                     ++wit)
                {
