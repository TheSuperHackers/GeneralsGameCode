    ...
    else // maybe we want to draw rally points, then?
    {
        ...
        for (DrawableListCIt it = selected->begin(); it != selected->end(); ++it)
        {
            ...
            if (obj)
            {
                ...
                // lets test for that very special case of having a listeningoutpost selected, and some enemy drawable
                // moused-over
                                ...
                                if (delta.length() <= obj->getVisionRange()) // is listening outpost close enough to do this?
                                {
                                    ...
                                    if (ai)
                                    {
                                        ...
                                        else // then enemy may be moving to a goal position
                                        {
                                            ...
                                            {
                                                points[numPoints++].Set(
                                                    Vector3(destinationPoint->x, destinationPoint->y, destinationPoint->z));
                                                m_waypointNodeRobj->Set_Position(
                                                    Vector3(destinationPoint->x, destinationPoint->y, destinationPoint->z));
                                                WW3D::Render(*m_waypointNodeRobj, localRinfo);
                                                lineExists = TRUE;
                ...
                if (exitInterface)
                {
                    ...
                    if (rallyPoint)
                    {
                        if (boxWrap)
                        {
                            ...
                            // to do this we find the two corners of the geometry extents that are nearest the natural rally
                            // point
                            ...
                            // if the rallypoint is closer to the wayoutpoint than it is to the natural rally point then we
                            // definitely do not wrap
                            Coord3D rallyToWayOutDelta = wayOutPoint;
                            rallyToWayOutDelta.sub(rallyPoint);
                            if ((100.0f + rallyToWayOutDelta.length()) > wayOutLength)
                            {
                                // if we passed the above idiot test, now lets be sure by testing the dotproduct of the rp
                                // against the wayoutpoint
                                ...
                                if (dot > 0)
                                {
                                    ...
                                    if (pNearElbow) // did we find a nearest corner?
                                    {
                                        ...
                                        if (pFarElbow) // did we find a nearest corner?
                                        {
                                            ...
                                            dot =
                                                firstToRPDelta.x * firstElbowDelta.x + firstToRPDelta.y * firstElbowDelta.y;
