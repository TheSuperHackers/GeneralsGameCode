...
void RTS3DScene::flagOccludedObjects(CameraClass *camera)
{
    ...
    for (Int i = 0; i < m_numPotentialOccludees; i++, occludee++)
    {
        ...
        for (Int j = 0; j < m_numPotentialOccluders; j++, occluder++)
        {
            ...
            Real Beta =
                sphere->Radius * sphere->Radius - (Vector3::Dot_Product(sphere_vector, sphere_vector) - Alpha * Alpha);
...
void RTS3DScene::Visibility_Check(CameraClass *camera)
{
    ...
    if (currentFrame <= TheGlobalData->m_defaultOcclusionDelay)
        currentFrame =
            TheGlobalData->m_defaultOcclusionDelay + 1; // make sure occlusion is enabled when game starts (frame 0).
                ...
                { // need to keep track of occluders and ocludees for subsequent code.
                    ...
                    if (drawInfo && (draw = drawInfo->m_drawable) != NULL)
                    {
                        ...
                        if (draw->getEffectiveOpacity() != 1.0f
                            && m_translucentObjectsCount < TheGlobalData->m_maxVisibleTranslucentObjects)
                        {
                        ...
                        if (TheGlobalData->m_enableBehindBuildingMarkers && TheGameLogic
                            && TheGameLogic->getShowBehindBuildingMarkers())
                        {
                            ...
                            if (draw->isKindOf(KINDOF_STRUCTURE)
                                && m_numPotentialOccluders < TheGlobalData->m_maxVisibleOccluderObjects)
                            { // object which could occlude other objects that need to be visible.
                                // Make sure this object is not translucent so it's not rendered twice (from
                                // m_potentialOccluders and m_translucentObjectsBuffer)
                                ...
                                draw->getObject()
                                && (draw->isKindOf(KINDOF_SCORE) || draw->isKindOf(KINDOF_SCORE_CREATE)
                                    || draw->isKindOf(KINDOF_SCORE_DESTROY) || draw->isKindOf(KINDOF_MP_COUNT_FOR_VICTORY))
                                && (draw->getObject()->getSafeOcclusionFrame()) <= currentFrame
                                && m_numPotentialOccludees < TheGlobalData->m_maxVisibleOccludeeObjects)
                            { // object which could be occluded but still needs to be visible.
                                // We process transucent units twice (also in m_translucentObjectsBuffer) because we need to
                                // see them when occluded.
                                ...
                                && m_numNonOccluderOrOccludee < TheGlobalData->m_maxVisibleNonOccluderOrOccludeeObjects)
                            { // regular object with no custom effects but still needs to be delayed to get the occlusion
        ...
        { // drawable with no object so no way to know if it's shrouded.
            ...
            if (drawInfo->m_shroudStatusObjectID != INVALID_ID)
            {
                ...
                if (shroudObject && shroudObject->getShroudedStatus(localPlayerIndex) >= OBJECTSHROUD_FOGGED)
                    ss = OBJECTSHROUD_SHROUDED; // we will assume that drawables without objects are 'particle' like and
                                                // therefore don't need drawing if fogged/shrouded.
            }
        }
...
void RTS3DScene::updateFixedLightEnvironments(RenderInfoClass &rinfo)
{
    ...
    m_infantryAmbient = Get_Ambient_Light(); // * infantryLightScale;	//for now don't adjust ambient so that we don't lose
...
#define USE_LIGHT_ENV 1
    ...
    for (it.First(&RenderList); !it.Is_Done();)
    {
        ...
        if (robj->Is_Really_Visible())
        {
            ...
            if (drawInfo)
                draw = drawInfo->m_drawable;
#ifdef USE_NON_STENCIL_OCCLUSION
            if (!(draw && drawInfo->m_flags & DrawableInfo::ERF_DELAYED_RENDER)) // model rendering is delayed for some
                                                                                 // reason until end of normal scene
#else
            if (!(draw
                  && drawInfo->m_flags
                      & (DrawableInfo::ERF_DELAYED_RENDER | DrawableInfo::ERF_POTENTIAL_OCCLUDER
                         | DrawableInfo::ERF_IS_NON_OCCLUDER_OR_OCCLUDEE))) // in this mode we delay almost all objects in
                                                                            // order to do correct sorting with stencil.
#endif
                renderOneObject(rinfo, robj, localPlayerIndex);
        }
    }
    ...
    if (terrainObject != NULL && TheParticleSystemManager != NULL && Get_Extra_Pass_Polygon_Mode() == EXTRA_PASS_DISABLE)
    {
...
void renderStenciledPlayerColor(UnsignedInt color, UnsignedInt stencilRef, Bool clear = FALSE)
{
    ...
    { // we want to clear the stencil buffer to some known value whereever a player index is stored
        ...
        DX8Wrapper::Set_DX8_Render_State(
            D3DRS_STENCILPASS,
            D3DSTENCILOP_REPLACE); // pixels which had occluded player colors, get MSB set.
        DX8Wrapper::Set_DX8_Render_State(
            D3DRS_STENCILFAIL,
            D3DSTENCILOP_ZERO); // pixels which had no occluded player colors are cleared.
        DX8Wrapper::Set_DX8_Render_State(
            D3DRS_ZFUNC,
            D3DCMP_NEVER); // fail all access to the frame buffer to improve memory bandwidth
    ...
    if (m_numPotentialOccludees && m_numPotentialOccluders)
    {
        ...
        for (k = 0; k < MAX_PLAYER_COUNT; k++)
        {
            if ((numObjects = lastPlayerObject[k] - &playerObjects[k][0]) != 0)
            {
                ...
                { // assign a new color index to this player
                    ...
                    RGB_To_HSV(
                        hsv,
                        Vector3(((color >> 16) & 0xff) / 255.0f, ((color >> 8) & 0xff) / 255.0f, (color & 0xff) / 255.0f));
                    hsv.Z *= TheGlobalData->m_occludedLuminanceScale;
                    HSV_To_RGB(rgb, hsv);
                ...
                for (Int j = 0; j < numObjects; j++)
                {
                    ...
                    if (drawInfo->m_flags & DrawableInfo::ERF_IS_TRANSLUCENT)
                    {
                        ...
                        DX8Wrapper::Set_DX8_Render_State(D3DRS_STENCILFUNC, D3DCMP_NEVER); // never allow frame buffer
                                                                                           // writes.
                        DX8Wrapper::Set_DX8_Render_State(
                            D3DRS_STENCILFAIL,
                            D3DSTENCILOP_REPLACE); // always replace existing stencil value
        ...
        { // for cases where we have 8 or more visible players, we're only left with 3 bits to store
            ...
            TheW3DShadowManager->setStencilShadowMask(
                0x80808080); // msb indicates occluded player pixels so ignore it when filling screen with shadow
        }
...
void RTS3DScene::flushOccludedObjects(RenderInfoClass &rinfo)
{
    ...
    if (m_occludedObjectsCount)
    {
        ...
        DX8Wrapper::Set_DX8_Render_State(D3DRS_STENCILENABLE, FALSE);
        TheW3DShadowManager->setStencilShadowMask(
            0x80808080); // upper MSB always contains flag indicating occluded player color.
    }
...
void RTS3DScene::Visibility_Check(CameraClass * camera)
{
    ...
    if (currentFrame <= TheGlobalData->m_defaultOcclusionDelay)
        currentFrame = TheGlobalData->m_defaultOcclusionDelay+1;	//make sure occlusion is enabled when game starts (frame
0).
...
//						}
                        //assume normal rendering.
                        drawInfo->m_flags = DrawableInfo::ERF_IS_NORMAL;	//clear any rendering flags that may be in
effect.


                        if (draw->getEffectiveOpacity() != 1.0f && m_translucentObjectsCount <
TheGlobalData->m_maxVisibleTranslucentObjects) {	drawInfo->m_flags |= DrawableInfo::ERF_IS_TRANSLUCENT;	//object is
translucent m_translucentObjectsBuffer[m_translucentObjectsCount++] = robj;
                        }
                        if (TheGlobalData->m_enableBehindBuildingMarkers && TheGameLogic->getShowBehindBuildingMarkers())
                        {
                            //visible drawable. Check if it's either an occluder or occludee
                            if (draw->isKindOf(KINDOF_STRUCTURE) && m_numPotentialOccluders <
TheGlobalData->m_maxVisibleOccluderObjects) {	//object which could occlude other objects that need to be visible.
                                //Make sure this object is not translucent so it's not rendered twice (from
m_potentialOccluders and m_translucentObjectsBuffer) if (drawInfo->m_flags ^ DrawableInfo::ERF_IS_TRANSLUCENT)
                                    m_potentialOccluders[m_numPotentialOccluders++]=robj;
                                drawInfo->m_flags |= DrawableInfo::ERF_POTENTIAL_OCCLUDER;
                            }
                            else
                            if (draw->getObject() &&
                                    (draw->isKindOf(KINDOF_SCORE) || draw->isKindOf(KINDOF_SCORE_CREATE) ||
draw->isKindOf(KINDOF_SCORE_DESTROY) || draw->isKindOf(KINDOF_MP_COUNT_FOR_VICTORY)) &&
                                    (draw->getObject()->getSafeOcclusionFrame()) <= currentFrame && m_numPotentialOccludees <
TheGlobalData->m_maxVisibleOccludeeObjects) {	//object which could be occluded but still needs to be visible.
                                //We process transucent units twice (also in m_translucentObjectsBuffer) because we need to
see them when occluded. m_potentialOccludees[m_numPotentialOccludees++]=robj; drawInfo->m_flags |=
DrawableInfo::ERF_POTENTIAL_OCCLUDEE;
                            }
                            else
                            if (drawInfo->m_flags == DrawableInfo::ERF_IS_NORMAL && m_numNonOccluderOrOccludee <
TheGlobalData->m_maxVisibleNonOccluderOrOccludeeObjects) {	//regular object with no custom effects but still needs to be
delayed to get the occlusion feature to work correctly.
                                //Make sure this object is not translucent so it's not rendered twice (from
m_potentialOccluders and m_translucentObjectsBuffer) if (drawInfo->m_flags ^ DrawableInfo::ERF_IS_TRANSLUCENT)	//make sure
not translucent m_nonOccludersOrOccludees[m_numNonOccluderOrOccludee++]=robj; drawInfo->m_flags |=
