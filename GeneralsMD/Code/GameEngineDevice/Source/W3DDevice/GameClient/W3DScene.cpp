				...
				{ // need to keep track of occluders and ocludees for subsequent code.
					...
					if (drawInfo && (draw = drawInfo->m_drawable) != NULL)
					{
						...
						if (TheGlobalData->m_enableBehindBuildingMarkers && TheGameLogic && TheGameLogic->getShowBehindBuildingMarkers())
						{
							...
							{ // regular object with no custom effects but still needs to be delayed to get the occlusion feature to work
								// correctly.
								// Make sure this object is not translucent so it's not rendered twice (from m_potentialOccluders and
								// m_translucentObjectsBuffer)
		...
		{ // drawable with no object so no way to know if it's shrouded.
			...
			if (drawInfo->m_shroudStatusObjectID != INVALID_ID)
			{
				...
				if (shroudObject && shroudObject->getShroudedStatus(localPlayerIndex) >= OBJECTSHROUD_FOGGED)
					ss = OBJECTSHROUD_SHROUDED; // we will assume that drawables without objects are 'particle' like and therefore
																			// don't need drawing if fogged/shrouded.
			}
		}
	...
	for (it.First(&RenderList); !it.Is_Done();)
	{
		...
		if (robj->Is_Really_Visible())
		{
			...
			if (drawInfo)
				draw = drawInfo->m_drawable;
#ifdef USE_NON_STENCIL_OCCLUSION
			if (!(draw && drawInfo->m_flags & DrawableInfo::ERF_DELAYED_RENDER)) // model rendering is delayed for some reason
																																					 // until end of normal scene
#else
			if (!(draw
						&& drawInfo->m_flags
								& (DrawableInfo::ERF_DELAYED_RENDER | DrawableInfo::ERF_POTENTIAL_OCCLUDER
									 | DrawableInfo::ERF_IS_NON_OCCLUDER_OR_OCCLUDEE))) // in this mode we delay almost all objects in order to
																																			// do correct sorting with stencil.
#endif
				renderOneObject(rinfo, robj, localPlayerIndex);
		}
	}
...
void renderStenciledPlayerColor(UnsignedInt color, UnsignedInt stencilRef, Bool clear = FALSE)
{
	...
	{ // we want to clear the stencil buffer to some known value whereever a player index is stored
		...
		DX8Wrapper::Set_DX8_Render_State(D3DRS_STENCILPASS, D3DSTENCILOP_REPLACE); // pixels which had occluded player colors,
																																							 // get MSB set.
		DX8Wrapper::Set_DX8_Render_State(D3DRS_STENCILFAIL, D3DSTENCILOP_ZERO); // pixels which had no occluded player colors are
																																						// cleared.
		DX8Wrapper::Set_DX8_Render_State(D3DRS_ZFUNC, D3DCMP_NEVER); // fail all access to the frame buffer to improve memory
																																 // bandwidth
...
void RTS3DScene::flushOccludedObjectsIntoStencil(RenderInfoClass &rinfo)
{
	...
	if (m_numPotentialOccludees && m_numPotentialOccluders)
	{
			...
			if ((numObjects = lastPlayerObject[k] - &playerObjects[k][0]) != 0)
			{
				...
				for (Int j = 0; j < numObjects; j++)
				{
					...
					if (drawInfo->m_flags & DrawableInfo::ERF_IS_TRANSLUCENT)
					{
						...
						DX8Wrapper::Set_DX8_Render_State(D3DRS_STENCILFAIL, D3DSTENCILOP_REPLACE); // always replace existing stencil
																																											 // value
		...
		{ // for cases where we have 8 or more visible players, we're only left with 3 bits to store
			...
			TheW3DShadowManager->setStencilShadowMask(0x80808080); // msb indicates occluded player pixels so ignore it when
																														 // filling screen with shadow
		}
...
void RTS3DScene::flushOccludedObjects(RenderInfoClass &rinfo)
{
	...
	if (m_occludedObjectsCount)
	{
		...
		m_occludedObjectsCount = 0;
		DX8Wrapper::Set_DX8_Render_State(D3DRS_STENCILENABLE, FALSE);
		TheW3DShadowManager->setStencilShadowMask(0x80808080); // upper MSB always contains flag indicating occluded player
																													 // color.
	}
