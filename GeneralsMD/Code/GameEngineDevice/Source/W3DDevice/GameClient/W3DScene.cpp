...
void renderStenciledPlayerColor(UnsignedInt color, UnsignedInt stencilRef, Bool clear = FALSE)
{
  ...
  { // we want to clear the stencil buffer to some known value whereever a player index is stored
    ...
    DX8Wrapper::Set_DX8_Render_State(D3DRS_STENCILFUNC,
...
translucent m_translucentObjectsBuffer[m_translucentObjectsCount++] = robj;
            ...
            if (TheGlobalData->m_enableBehindBuildingMarkers && TheGameLogic->getShowBehindBuildingMarkers())
            {
              ...
              if (draw->isKindOf(KINDOF_STRUCTURE) && m_numPotentialOccluders < TheGlobalData->m_maxVisibleOccluderObjects)
              {	//object which could occlude other objects that need to be visible.
                //Make sure this object is not translucent so it's not rendered twice (from m_potentialOccluders and
m_translucentObjectsBuffer) if (drawInfo->m_flags ^ DrawableInfo::ERF_IS_TRANSLUCENT)
                  m_potentialOccluders[m_numPotentialOccluders++]=robj;
                drawInfo->m_flags |= DrawableInfo::ERF_POTENTIAL_OCCLUDER;
              }
              else
...
TheGlobalData->m_maxVisibleOccludeeObjects) {	//object which could be occluded but still needs to be visible.
                //We process transucent units twice (also in m_translucentObjectsBuffer) because we need to see them when
occluded. m_potentialOccludees[m_numPotentialOccludees++]=robj; drawInfo->m_flags |= DrawableInfo::ERF_POTENTIAL_OCCLUDEE;
              }
              else
...
delayed to get the occlusion feature to work correctly.
                //Make sure this object is not translucent so it's not rendered twice (from m_potentialOccluders and
m_translucentObjectsBuffer) if (drawInfo->m_flags ^ DrawableInfo::ERF_IS_TRANSLUCENT)	//make sure not translucent
                  m_nonOccludersOrOccludees[m_numNonOccluderOrOccludee++]=robj;
                drawInfo->m_flags |= DrawableInfo::ERF_IS_NON_OCCLUDER_OR_OCCLUDEE;
              }
            }
