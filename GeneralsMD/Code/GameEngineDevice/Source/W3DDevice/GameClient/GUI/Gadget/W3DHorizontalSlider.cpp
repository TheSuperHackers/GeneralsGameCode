...
void W3DGadgetHorizontalSliderImageDraw(GameWindow *window, WinInstanceData *instData)
{
	...
	if (BitIsSet(instData->getState(), WIN_STATE_HILITED))
	{
		...
		for (i = 0; i < numHighlightBoxes; ++i)
		{
			...
			TheWindowManager
					->winDrawImage(highlightSquare, backgroundStart.x, backgroundStart.y, backgroundEnd.x, backgroundEnd.y);
		}
	}
...
/** Draw horizontal slider with user supplied images */
//=============================================================================
void W3DGadgetHorizontalSliderImageDrawB(GameWindow *window, WinInstanceData *instData)
{
	...
	if (BitIsSet(instData->getState(), WIN_STATE_HILITED))
	{
		...
		tmp.format(
				...
				backgroundEnd.x,
				backgroundEnd.y,
				highlightSquare->getImageWidth(),
				highlightSquare->getImageWidth() * xMulti,
				origin.x,
				size.x);
		tooltip.concat(tmp);
	...
	while (start.x <= origin.x + (s->numTicks * (s->position - s->minVal)) && end.x < origin.x + size.x
				 && s->position != s->minVal)
	{
		TheWindowManager->winDrawImage(fillSquare, start.x, start.y, end.x, end.y);
		start.x = end.x + 2;
...
void W3DGadgetHorizontalSliderImageDrawA(GameWindow *window, WinInstanceData *instData)
{
	...
	if (leftImageLeft == NULL || rightImageLeft == NULL || centerImageLeft == NULL || smallCenterImageLeft == NULL
			|| leftImageRight == NULL || rightImageRight == NULL || centerImageRight == NULL || smallCenterImageRight == NULL)
		return;
