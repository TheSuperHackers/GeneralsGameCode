        ...
        if (life < 15)
        {
            ...
            for (; x < radius; ++x)
            {
                for (Int y = 0; y < radius; ++y)
                {
                    ...
                    if (distance < radius)
                    {
                        ...
                        workspace[(radius + x) + workspaceWidth * (radius + y)] =
                            height + heightMap->getBilinearSampleSeismicZVelocity(centerX + x, centerY + y); // kaleidoscope

                        if (x != 0) // non-zero test prevents cross-shaped double stamp
                        {
                            workspace[(radius - x) + workspaceWidth * (radius + y)] =
                                height + heightMap->getBilinearSampleSeismicZVelocity(centerX - x, centerY + y);
                            if (y != 0)
                                workspace[(radius - x) + workspaceWidth * (radius - y)] =
                                    height + heightMap->getBilinearSampleSeismicZVelocity(centerX - x, centerY - y);
                        }
            ...
            for (x = 0; x < workspaceWidth; ++x)
                for (Int y = 0; y < workspaceWidth; ++y)
                    heightMap->setSeismicZVelocity(
                        centerX - radius + x,
                        centerY - radius + y,
                        workspace[x + workspaceWidth * y]);
...
void W3DTerrainVisual::init(void)
{
    ...
    if (!TheGlobalData->m_headless)
    {
        ...
        m_waterRenderObject->init(
            ...
            TheGlobalData->m_waterExtentX,
            TheGlobalData->m_waterExtentY,
            W3DDisplay::m_3DScene,
            (WaterRenderObjClass::WaterType)TheGlobalData->m_waterType); // create a water plane that's 128x128 units
        m_waterRenderObject->Set_Position(Vector3(
            TheGlobalData->m_waterPositionX,
            TheGlobalData->m_waterPositionY,
    ...
    if (!m_seismicSimulationList.empty())
    {
        ...
        while (it != m_seismicSimulationList.end())
        {
            ...
            if (ssn)
            {
                ...
                DEBUG_ASSERTCRASH(
                    code != SeismicSimulationFilterBase::SEISMIC_STATUS_INVALID,
                    ("Trouble in the Seismic simulator."));
                ...
                for (Int x = border + ssn->m_region.lo.x; x < border + ssn->m_region.hi.x; ++x)
                {
                    for (Int y = border + ssn->m_region.lo.y; y < border + ssn->m_region.hi.y; ++y)
                    {
                        ...
                        {
                            UnsignedByte heightOfOriginal =
                                m_logicHeightMap->getHeight(x, y); // LOGIC, YES DEFINITELY THE LOGIC
                            ...
                            if (heightToUse <= heightOfOriginal)
                            {
                                ...
                                m_clientHeightMap->setSeismicZVelocity(
                                    x,
                                    y,
                                    0.0f); // poof! the dirt hit ground level so stop "falling"
                            }
                            else
                            {
                                ++fallCount;
...
Bool W3DTerrainVisual::load(AsciiString filename)
{
    ...
    m_terrainRenderObject
        ->initHeightData(m_clientHeightMap->getDrawWidth(), m_clientHeightMap->getDrawHeight(), m_clientHeightMap, it);
#else
    m_terrainRenderObject
        ->initHeightData(m_logicHeightMap->getDrawWidth(), m_logicHeightMap->getDrawHeight(), m_logicHeightMap, it);
#endif
...
//-------------------------------------------------------------------------------------------------
Bool W3DTerrainVisual::intersectTerrain(Coord3D *rayStart, Coord3D *rayEnd, Coord3D *result)
{
    Bool hit = FALSE;
    ...
    if (m_terrainRenderObject)
    {
        ...
        LineSegClass lineSeg(Vector3(rayStart->x, rayStart->y, rayStart->z), Vector3(rayEnd->x, rayEnd->y, rayEnd->z));
...
// ------------------------------------------------------------------------------------------------
void W3DTerrainVisual::setWaterGridHeightClamps(const WaterHandle *waterTable, Real minZ, Real maxZ)
{
    if (m_waterRenderObject)
...
// ------------------------------------------------------------------------------------------------
void W3DTerrainVisual::setWaterAttenuationFactors(const WaterHandle *waterTable, Real a, Real b, Real c, Real range)
{
    if (m_waterRenderObject)
...
// ------------------------------------------------------------------------------------------------
void W3DTerrainVisual::setWaterTransform(const WaterHandle *waterTable, Real angle, Real x, Real y, Real z)
{
    if (m_waterRenderObject)
...
// ------------------------------------------------------------------------------------------------
void W3DTerrainVisual::setWaterGridResolution(const WaterHandle *waterTable, Real gridCellsX, Real gridCellsY, Real cellSize)
{
    if (m_waterRenderObject)
...
// ------------------------------------------------------------------------------------------------
void W3DTerrainVisual::getWaterGridResolution(
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void W3DTerrainVisual::addWaterVelocity(Real worldX, Real worldY, Real velocity, Real preferredHeight)
{
    if (m_waterRenderObject)
        ...
        if (m_logicHeightMap->getHeight(x, y) > height)
        {
            ...
            if (m_clientHeightMap)
            {
                if (height < m_clientHeightMap->getHeight(x, y))
                    m_clientHeightMap->setRawHeight(
                        x,
                        y,
                        height); // if the client map is heigher than this height, it will fall down to it anyway!
            }
#endif
        }
    }
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void W3DTerrainVisual::removeTreesAndPropsForConstruction(const Coord3D *pos, const GeometryInfo &geom, Real angle)
{
    if (m_terrainRenderObject)
    {
