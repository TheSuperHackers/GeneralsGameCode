...
void W3DTerrainVisual::init(void)
{
	...
	if (!TheGlobalData->m_headless)
	{
		...
		m_waterRenderObject->Set_Position(
				Vector3(TheGlobalData->m_waterPositionX, TheGlobalData->m_waterPositionY, TheGlobalData->m_waterPositionZ)); // place
																																																										 // water
																																																										 // in
																																																										 // world
		...
		if (m_logicHeightMap->getHeight(x, y) > height)
		{
			...
			if (m_clientHeightMap)
			{
				if (height < m_clientHeightMap->getHeight(x, y))
					m_clientHeightMap->setRawHeight(x, y, height); // if the client map is heigher than this height, it will fall down
																												 // to it anyway!
			}
#endif
		}
