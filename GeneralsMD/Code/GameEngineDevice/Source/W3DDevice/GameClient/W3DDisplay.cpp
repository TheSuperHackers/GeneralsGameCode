...
void StatDumpClass::dumpStats(Bool brief, Bool flagSpikes)
{
    ...
    if (flagSpikes && overallFailedPathfinds > 0)
        fprintf(
            m_fp,
            "                                                                      FAILEDPATHFINDS OUT OF TOLERANCE(0)\n");
    fprintf(m_fp, "\n");
    ...
    if (flagSpikes && slowScript1 * 1000 > 0.2f || slowScript2 * 1000 > 0.2f)
        fprintf(
            m_fp,
            "                                                                      SLOW SCRIPT OUT OF TOLERANCE(0.2)\n");
    fprintf(m_fp, "\n");
    ...
    fprintf(
        ...
        TheTerrainRenderObject->getNumExtraBlendTiles(TRUE),
...
Int W3DDisplay::getDisplayModeCount(void)
{
    ...
    /*	Bool needStencil=false;
        ...
        if (TheGlobalData->m_useShadowVolumes || (TheGlobalData->m_enableBehindBuildingMarkers &&
...
void W3DDisplay::init(void)
{
    ...
    if (!TheGlobalData->m_headless)
    {
        ...
        do
        {
            ...
            renderDeviceError = WW3D::Set_Render_Device(0, getWidth(), getHeight(), getBitDepth(), getWindowed(), true);
...
void W3DDisplay::updateAverageFPS(void)
{
    const Real MaximumFrameTimeCutoff =
        0.5f; // largest frame interval (seconds) we accept before ignoring it as a momentary "spike"
...
void W3DDisplay::gatherDebugStats(void)
{
    ...
    if (s_timeSinceLastUpdateInSecs >= UPDATE_RATE_SECS || TheGlobalData->m_constantDebugUpdate)
    {
        ...
        double drawsPerFrame =
            Debug_Statistics::Get_Draw_Calls(); //(Real)s_drawCallsSinceLastUpdate / (Real)s_framesRenderedSinceLastUpdate;
        double sortPolysPerFrame = Debug_Statistics::Get_Sorting_Polygons(); //(Real)s_sortedPolysSinceLastUpdate /
        ...
        // unibuffer.format( L"FPS: %.2f, %.2fms mapLOD=%d [cumu FPS=%.2f] draws: %.2f sort: %.2f", fps, ms, LOD, cumuFPS,
        ...
        unibuffer2.format(
            L"%.2fms [cumuFPS=%.2f] draws: %d skins: %d sortP: %d skinP: %d LOD %d",
            ms,
            cumuFPS,
            (Int)drawsPerFrame,
...
#else
        // Int LOD = TheGlobalData->m_terrainLOD;
        // unibuffer.format( L"FPS: %.2f, %.2fms mapLOD=%d draws: %.2f sort %.2f", fps, ms, LOD,
        // drawsPerFrame,sortPolysPerFrame);
        unibuffer.format(
            L"FPS: %.2f, %.2fms draws: %.2f skins: %.2f sort %.2f",
            fps,
            ms,
            drawsPerFrame,
            skinDrawsPerFrame,
        ...
        else if (extendedStats > 0)
        {
            extendedStats--;
            unibuffer.format(
                L"FPS: %.2f, %.2fms - OH %.2fms, Console %.2fms, 3D OH %.2fms, Terrain %.2fms, Obs %.2fms, CPU %.2fms",
                fps,
                ms,
                gameOverheadMS,
                consoleMS,
            ...
            if (extendedStats == SHOW_STATS_TIME - 2)
            {
                ...
                sprintf(
                    bufferA,
                    "FPS: %.2f, %.2fms - OH %.2fms, Console %.2fms, 3D OH %.2fms, Terrain %.2fms, Obs %.2fms, CPU %.2fms\n",
                    fps,
                    ms,
                    gameOverheadMS,
                    consoleMS,
        ...
        // polygons this frame
        unibuffer.format(L"Polygons: per frame %d, per second %d", polyPerFrame, (Int)(polyPerFrame * fps));
        ...
        // terrain stats
        unibuffer.format(
            L"3-Way Blends: %d/%d, Shoreline Blends: %d/%d",
            TheTerrainRenderObject->getNumExtraBlendTiles(TRUE),
            TheTerrainRenderObject->getNumExtraBlendTiles(FALSE),
            TheTerrainRenderObject->getNumShoreLineTiles(TRUE),
            TheTerrainRenderObject->getNumShoreLineTiles(FALSE));
        ...
        unibuffer.format(
            L"Mouse position: screen: (%d, %d), world: (%g, %g, %g)",
            mouseIO->pos.x,
            mouseIO->pos.y,
            worldPos.x,
            worldPos.y,
        ...
        {
            unibuffer.format(
                L"IN: %.2f bytes/sec, %.2f packets/sec",
            ...
            unibuffer.format(
                L"OUT: %.2f bytes/sec, %.2f packets/sec",
        ...
        if (draw)
        {
            ...
            if (locoInfo)
            {
                unibuffer2.format(
                    ...
                    turnType,
                    locoInfo->m_accelerationPitch,
                    locoInfo->m_accelerationPitchRate,
                    locoInfo->m_accelerationRoll,
            ...
            if (rcost.getDrawCallCount() > 0)
            {
                unibuffer2.format(
                    ...
                    rcost.getDrawCallCount(),
                    rcost.getShadowDrawCount(),
                    rcost.getSortedMeshCount(),
                    rcost.getSkinMeshCount(),
                    rcost.getBoneCount());
                unibuffer.concat(unibuffer2);
            }
#endif
    ...
    do
    {
        ...
        for (i = 0; i < NUM_SAMPLES; i++)
        {
            ...
            if (i >= NUM_TO_DISCARD)
            {
                ...
                if (i > NUM_TO_DISCARD + 1 && (timeForFrame / ((i + 1) - NUM_TO_DISCARD)) > 2 * maxTimeLimit)
                {
                    i++;
                    break;
                }
        ...
        { // Checking if we have the device before updating views because the heightmap crashes otherwise while
          // trying to refresh the visible terrain geometry.
            ...
            TheParticleSystemManager->update(); // LORENZEN AND WILCZYNSKI MOVED THIS FROM ITS NATIVE POSITION, ABOVE
                                                ...
                                                // TRANSFORM MATRIX, WHILE IT IS STILL VALID (HAVING DONE ITS CLIENT
                                                // TRANSFORMS BUT NOT YET RESETTING TOT HE LOGICAL TRANSFORM) THE RESULT IS
                                                // THAT PARTICLESYSTEMS LINKED TO BONES IN DRAWABLES.OBJECTS MOVE WITH THE
                                                // CLIENT TRANSFORMS, NOW. REVOLUTIONARY! -LORENZEN

            if (TheWaterRenderObj && TheGlobalData->m_waterType == 2)
                TheWaterRenderObj->updateRenderTargetTextures(primaryW3DView->get3DCamera()); // do a render into each
                                                                                              // texture
...
#else
        if ((TheGameLogic->getFrame() % 30 == 1)
            || (!(!TheGameLogic->isGamePaused() && TheGlobalData->m_TiVOFastMode && TheGameLogic->isInReplayGame())))
#endif
        {
            // USE_PERF_TIMER(BigAssRenderLoop)
            static Bool couldRender = true;
            if ((TheGlobalData->m_breakTheMovie == FALSE) && (TheGlobalData->m_disableRender == false)
                    ...
                    == WW3D_ERROR_OK)
            {
                ...
                if (numRenderTargetPolygons || numRenderTargetVertices)
                    Debug_Statistics::Record_DX8_Polys_And_Vertices(
                        numRenderTargetPolygons,
                        numRenderTargetVertices,
                ...
                {
                    // TheSuperHackers @bugfix Mauller 20/07/2025 scale videos based on screen size so they are shown in
...
//=============================================================================
void W3DDisplay::createLightPulse(
    const Coord3D *pos,
...
//=============================================================================
void W3DDisplay::drawLine(Int startX, Int startY, Int endX, Int endY, Real lineWidth, UnsignedInt lineColor)
{
    /// @todo we need to consider the efficiency of the 2D renderer
    m_2DRender->Reset();
    m_2DRender->Enable_Texturing(FALSE);
    m_2DRender->Add_Line(Vector2(startX, startY), Vector2(endX, endY), lineWidth, lineColor);
    m_2DRender->Render();
    ...
    UnsignedInt lineColor2)
{
    ...
    m_2DRender->Reset();
    m_2DRender->Enable_Texturing(FALSE);
    m_2DRender->Add_Line(Vector2(startX, startY), Vector2(endX, endY), lineWidth, lineColor1, lineColor2);
    m_2DRender->Render();
...
//=============================================================================
void W3DDisplay::drawOpenRect(Int startX, Int startY, Int width, Int height, Real lineWidth, UnsignedInt lineColor)
{
...
//=============================================================================
void W3DDisplay::drawFillRect(Int startX, Int startY, Int width, Int height, UnsignedInt color)
{
    /// @todo we need to consider the efficiency of the 2D renderer
    m_2DRender->Reset();
    m_2DRender->Enable_Texturing(FALSE);
    m_2DRender->Add_Rect(RectClass(startX, startY, startX + width, startY + height), 0, 0, color);
...
void W3DDisplay::drawRectClock(Int startX, Int startY, Int width, Int height, Int percent, UnsignedInt color)
{
    ...
    {
        // rectangle #1 & 2
        m_2DRender->Add_Rect(RectClass(startX + width / 2, startY, startX + width, startY + height), 0, 0, color);
        // rectangle #3
        m_2DRender->Add_Rect(RectClass(startX, startY + height / 2, startX + width / 2, startY + height), 0, 0, color);
    ...
    {
        // rectangel #1
        m_2DRender->Add_Rect(RectClass(startX + width / 2, startY, startX + width, startY + height / 2), 0, 0, color);
...
//=============================================================================
void W3DDisplay::drawImage(const Image *image, Int startX, Int startY, Int endX, Int endY, Color color, DrawImageMode mode)
{
    // sanity
    if (image == NULL)
        return;
...
VideoBuffer *W3DDisplay::createVideoBuffer(void)
{
    ...
    if (!TheGlobalData->m_playIntro) //&& TheGameLODManager && (!TheGameLODManager->didMemPass() ||
...
{
    // TheSuperHackers @bugfix Mauller 20/07/2025 scale videos based on screen size so they are shown in their original
    // aspect
...
void W3DDisplay::drawVideoBuffer(VideoBuffer *buffer, Int startX, Int startY, Int endX, Int endY)
{
    ...
    m_2DRender->Add_Quad(RectClass(startX, startY, endX, endY), vbuffer->Rect(0, 0, 1, 1));
    m_2DRender->Render();
}
        ...
        if (setting == CELLSHROUD_SHROUDED)
            ...
            TheTerrainRenderObject->getShroud()->setShroudLevel(
                ...
                (W3DShroudLevel)TheGlobalData
                    ->m_fogAlpha); ///< @todo placeholder to get feedback on logic work while graphic side being decided
        else
...
void W3DDisplay::takeScreenShot(void)
{
    ...
    SurfaceClass *surfaceCopy =
        NEW_REF(SurfaceClass, (DX8Wrapper::_Create_DX8_Surface(surfaceDesc.Width, surfaceDesc.Height, surfaceDesc.Format)));
...
static void drawFramerateBar(void)
{
    ...
    UnsignedInt colorToUse =
        GameMakeColor(REAL_TO_UNSIGNEDBYTE((1.0f - percTime) * 255), REAL_TO_UNSIGNEDBYTE(percTime * 255), 0, 0x7F);
