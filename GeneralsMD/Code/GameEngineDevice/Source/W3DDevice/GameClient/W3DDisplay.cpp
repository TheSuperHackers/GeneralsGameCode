...
{
	const Real MaximumFrameTimeCutoff = 0.5f; // largest frame interval (seconds) we accept before ignoring it as a momentary
																						// "spike"
...
void W3DDisplay::gatherDebugStats(void)
{
	...
	if (s_timeSinceLastUpdateInSecs >= UPDATE_RATE_SECS || TheGlobalData->m_constantDebugUpdate)
	{
		...
		double drawsPerFrame = Debug_Statistics::Get_Draw_Calls(); //(Real)s_drawCallsSinceLastUpdate /
...
void W3DDisplay::setShroudLevel(Int x, Int y, CellShroudStatus setting)
{
	if (TheTerrainRenderObject && TheTerrainRenderObject->getShroud())
	{
		...
		if (setting == CELLSHROUD_SHROUDED)
			...
			TheTerrainRenderObject->getShroud()->setShroudLevel(x, y, (W3DShroudLevel)TheGlobalData->m_fogAlpha); ///< @todo
																																																						///< placeholder
																																																						///< to get
																																																						///< feedback on
																																																						///< logic work
