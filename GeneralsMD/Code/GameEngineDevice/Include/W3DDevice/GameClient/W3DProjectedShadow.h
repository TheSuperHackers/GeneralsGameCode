...
public:
    ...
    virtual Shadow *addDecal(Shadow::ShadowTypeInfo *shadowInfo); ///< add a non-shadow decal which does not follow an
                                                                  ///< object.
    W3DProjectedShadow *addShadow(
        RenderObjClass *robj,
        Shadow::ShadowTypeInfo *shadowInfo,
        Drawable *draw); ///< add a new shadow with texture of given name or that of robj.
    W3DProjectedShadow *createDecalShadow(
        Shadow::ShadowTypeInfo *shadowInfo); ///< add a new shadow with texture of given name or that of robj.
...
public:
    ...
    void setObjPosHistory(const Vector3 &pos)
    {
    ...
    void updateTexture(Vector3 &lightPos); ///< updates the shadow texture image using render object and given light
                                           ///< position.
    void updateProjectionParameters(
        const Matrix3D &cameraXform); ///< recompute projection matrix - needed when light or object moves.
