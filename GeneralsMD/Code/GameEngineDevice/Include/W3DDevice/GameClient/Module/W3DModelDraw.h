...
// typedef std::hash_map< NameKeyType, PristineBoneInfo, rts::hash<NameKeyType>, rts::equal_to<NameKeyType> >
// PristineBoneInfoMap;
...
struct ModelConditionInfo
{
  ...
  inline ModelConditionInfo() { clear(); }
...
//-------------------------------------------------------------------------------------------------
// typedef std::hash_map< TransitionSig, ModelConditionInfo, std::hash<TransitionSig>, std::equal_to<TransitionSig> >
// TransitionMap;
...
#endif
  Int m_defaultState;
  Int m_projectileBoneFeedbackEnabledSlots; ///< Hide and show the launch bone geometries according to clip status
                                            ///< adjustments.
...
class W3DModelDraw : public DrawModule, public ObjectDrawInterface
{
  ...
  virtual void updateDrawModuleSupplyStatus(
      Int maxSupply,
      Int currentSupply); ///< This will do visual feedback on Supplies carried
  ...
  */
  ...
  void doHideShowProjectileObjects(
      UnsignedInt showCount,
      UnsignedInt maxCount,
      WeaponSlotType slot); ///< Means effectively, show m of n.
