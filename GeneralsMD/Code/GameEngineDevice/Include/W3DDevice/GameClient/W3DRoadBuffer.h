...
enum TCorner CPP_11( : Int){
	SEGMENT, CURVE, TEE, FOUR_WAY, THREE_WAY_Y, THREE_WAY_H, THREE_WAY_H_FLIP, ALPHA_JOIN, NUM_JOINS
};
...
protected:
	...
	void loadCurve(RoadSegment *pRoad, Vector2 loc1, Vector2 loc2, Real scale); ///< Fills the index and vertex buffers for
																																							///< drawing 1 fade.
	void loadTee(RoadSegment *pRoad, Vector2 loc1, Vector2 loc2, Bool is4way, Real scale); ///< Fills the index and vertex
																																												 ///< buffers for drawing 1 tee
																																												 ///< intersection.
	void loadY(RoadSegment *pRoad, Vector2 loc1, Vector2 loc2, Real scale); ///< Fills the index and vertex buffers for drawing
																																					///< 1 Y intersection.
	void loadAlphaJoin(RoadSegment *pRoad, Vector2 loc1, Vector2 loc2, Real scale); ///< Fills the index and vertex buffers for
																																									///< drawing 1 alpha blend cap.
	void loadH(RoadSegment *pRoad, Vector2 loc1, Vector2 loc2, Bool flip, Real scale); ///< Fills the index and vertex buffers
																																										 ///< for drawing 1 h tee intersection.
	void loadFloatSection(
			RoadSegment *pRoad,
			Vector2 loc,
			Vector2 roadVector,
			Real height,
	...
	void loadFloat4PtSection(
			...
			Real uScale,
			Real vScale);
	void loadLit4PtSection(
			RoadSegment *pRoad,
			UnsignedShort *ib,
			VertexFormatXYZDUV1 *vb,
			RefRenderObjListIterator *pDynamicLightsIterator);
	void loadRoadsInVertexAndIndexBuffers(void); ///< Fills the index and vertex buffers for drawing.
	void loadLitRoadsInVertexAndIndexBuffers(RefRenderObjListIterator *pDynamicLightsIterator); ///< Fills the index and vertex
																																															///< buffers for drawing.
	void loadRoadSegment(UnsignedShort *ib, VertexFormatXYZDUV1 *vb, RoadSegment *pRoad); ///< Fills the index and vertex
																																												///< buffers for drawing 1 segment.
	void allocateRoadBuffers(void); ///< Allocates the buffers.
	void freeRoadBuffers(void); ///< Frees the index and vertex buffers.
	Bool visibilityChanged(const IRegion2D &bounds); ///< Returns true if some roads are now visible that weren't, or vice
																									 ///< versa.
