...
TextureLoadTaskListClass::TextureLoadTaskListClass(void) : Root()
{
		...
		WW3DFormat dest_format)
{
	...
	IDirect3DTexture8 *d3d_texture = DX8Wrapper::_Create_DX8_Texture(width, height, dest_format, (MipCountType)mips);
...
static bool Is_Format_Compressed(WW3DFormat texture_format, bool allow_compression)
{
	...
	if (texture_format != WW3D_FORMAT_UNKNOWN)
	{
		...
		if (texture_format == WW3D_FORMAT_DXT1 || texture_format == WW3D_FORMAT_DXT2 || texture_format == WW3D_FORMAT_DXT3
				|| texture_format == WW3D_FORMAT_DXT4 || texture_format == WW3D_FORMAT_DXT5)
		{
			compressed = true;
		}
	}
...
IDirect3DTexture8 *TextureLoader::Load_Thumbnail(const StringClass &filename, const Vector3 &hsv_shift) //,WW3DFormat
																																																				// texture_format)
{
		...
		bool compressed)
{
	...
	if (compressed && thumb->Get_Original_Texture_Format() != WW3D_FORMAT_DXT1
			&& thumb->Get_Original_Texture_Format() != WW3D_FORMAT_DXT2 && thumb->Get_Original_Texture_Format() != WW3D_FORMAT_DXT3
...
bool TextureLoadTaskClass::Begin_Uncompressed_Load(void)
{
	...
	{
		WWDEBUG_SAY(
				("Invalid TGA format used in %s - only 24 and 32 bit formats should be used!", Texture->Get_Full_Path().str()));
	}
...
bool TextureLoadTaskClass::Begin_Uncompressed_Load(void)
{
	...
	TextureLoader::Validate_Texture_Size(width, height);
	if (width != ow || height != oh) {
		WWDEBUG_SAY(("Invalid texture size, scaling required. Texture: %s, size: %d x %d -> %d x %d", Texture->Get_Full_Path(),
ow, oh, width, height));
	}
...
void TextureLoadTaskClass::Lock_Surfaces(void)
{
	...
	{
		D3DLOCKED_RECT locked_rect;
		DX8_ErrorCode(Peek_D3D_Texture()->LockRect(i, &locked_rect, NULL, 0));
...
void CubeTextureLoadTaskClass::Lock_Surfaces(void)
{
	for (unsigned int f = 0; f < 6; f++)
	{
		...
		{
			D3DLOCKED_RECT locked_rect;
			DX8_ErrorCode(Peek_D3D_Cube_Texture()->LockRect((D3DCUBEMAP_FACES)f, i, &locked_rect, NULL, 0));
...
void CubeTextureLoadTaskClass::Unlock_Surfaces(void)
{
	for (unsigned int f = 0; f < 6; f++)
	{
		for (unsigned int i = 0; i < MipLevelCount; ++i)
		{
			...
			{
				WWASSERT(ThreadClass::_Get_Current_Thread_ID() == DX8Wrapper::_Get_Main_Thread_ID());
				DX8_ErrorCode(Peek_D3D_Cube_Texture()->UnlockRect((D3DCUBEMAP_FACES)f, i));
			}
			LockedCubeSurfacePtr[f][i] = NULL;
		}
	}
	...
	IDirect3DCubeTexture8 *tex =
			DX8Wrapper::_Create_DX8_Cube_Texture(Width, Height, Format, Texture->MipLevelCount, D3DPOOL_DEFAULT);
...
VolumeTextureLoadTaskClass::VolumeTextureLoadTaskClass() : TextureLoadTaskClass()
{
	// because texture load tasks are pooled, the constructor and destructor
	// don't need to do much. The work of attaching a task to a texture is
	// is done by Init() and Deinit().
...
void VolumeTextureLoadTaskClass::Lock_Surfaces()
{
	for (unsigned int i = 0; i < MipLevelCount; i++)
	{
		...
		DX8_ErrorCode(Peek_D3D_Volume_Texture()->LockBox(i, &locked_box, NULL, 0));
...
void VolumeTextureLoadTaskClass::Unlock_Surfaces()
{
	...
	IDirect3DTexture8 *tex =
			DX8Wrapper::_Create_DX8_Volume_Texture(Width, Height, Depth, Format, Texture->MipLevelCount, D3DPOOL_DEFAULT);
	DX8CALL(UpdateTexture(Peek_D3D_Volume_Texture(), tex));
	Peek_D3D_Volume_Texture()->Release();
	D3DTexture = tex;
	WWDEBUG_SAY(("Created non-managed texture (%s)", Texture->Get_Full_Path()));
