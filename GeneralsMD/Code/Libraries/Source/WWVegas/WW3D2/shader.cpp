...
const Blend srcBlendLUT[ShaderClass::SRCBLEND_MAX] = { Blend(D3DBLEND_ZERO, false),
																											 Blend(D3DBLEND_ONE, false),
																											 Blend(D3DBLEND_SRCALPHA, true),
																											 Blend(D3DBLEND_DESTCOLOR, true) };

const Blend dstBlendLUT[ShaderClass::DSTBLEND_MAX] = { Blend(D3DBLEND_ZERO, false),			Blend(D3DBLEND_ONE, false),
...
void ShaderClass::Apply()
{
				 ...
				 | ShaderClass::MASK_ALPHATEST))
	{
		...
		diff &= ~(
				ShaderClass::MASK_COLORMASK | ShaderClass::MASK_SRCBLEND | ShaderClass::MASK_DSTBLEND | ShaderClass::MASK_ALPHATEST);
		if (!diff)
			return;
	}
	...
	if (diff & pri_mask)
	{
		...
		if (voodoo3 && (PricArg2 == D3DTA_DIFFUSE) && ((SecaOp != D3DTOP_DISABLE) || (SeccOp != D3DTOP_DISABLE)))
		{
