...
ThumbnailManagerClass::ThumbnailManagerClass(const char *thumbnail_filename) :
		ThumbnailMemory(NULL), ThumbnailFileName(thumbnail_filename), PerTextureTimeStampUsed(false), Changed(false), DateTime(0)
{
}
...
ThumbnailManagerClass *ThumbnailManagerClass::Peek_Thumbnail_Manager(const char *thumbnail_filename)
{
	...
	if (GlobalThumbnailManager && GlobalThumbnailManager->ThumbnailFileName == thumbnail_filename)
