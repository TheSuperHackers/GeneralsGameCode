...
void DX8Caps::Compute_Caps(WW3DFormat display_format, const D3DADAPTER_IDENTIFIER8 &adapter_id)
{
    ...
    SupportModAlphaAddClr =
        (Caps.TextureOpCaps & D3DTEXOPCAPS_MODULATEALPHA_ADDCOLOR) == D3DTEXOPCAPS_MODULATEALPHA_ADDCOLOR;
...
void DX8Caps::Check_Depth_Stencil_Support(WW3DFormat display_format, const D3DCAPS8 &caps)
{
    ...
    for (unsigned i = 0; i < WW3D_ZFORMAT_COUNT; ++i)
    {
        ...
        else
        {
            ...
            SupportDepthStencilFormat[i] = SUCCEEDED(Direct3D->CheckDeviceFormat(
                caps.AdapterOrdinal,
                caps.DeviceType,
                d3d_display_format,
...
void DX8Caps::Check_Shader_Support(const D3DCAPS8 &caps)
{
    ...
    DXLOG(
        ("Vertex shader version: %d.%d, pixel shader version: %d.%d\r\n",
         ...
         VertexShaderVersion & 0xff,
         (PixelShaderVersion >> 8) & 0xff,
         PixelShaderVersion & 0xff));
}
            ...
            if (!stricmp(DriverDLL, "nv4_disp.dll") || !stricmp(DriverDLL, "nvdd32.dll"))
            {
                switch (DriverBuildVersion)
                {
                    ...
                    case 1260: // Byon - BYONG
                        DriverVersionStatus = DRIVER_STATUS_BAD;
                        break;
                    case 1241: // Steve Tall gets occasional blue screening with this driver version (blue screen happens in
                               // the driver dll)
                        DriverVersionStatus = DRIVER_STATUS_BAD;
                        break;
...
void DX8Caps::Vendor_Specific_Hacks(const D3DADAPTER_IDENTIFIER8 &adapter_id)
{
    if (VendorId == VENDOR_NVIDIA)
    {
        ...
        if (DeviceId == DEVICE_NVIDIA_GEFORCE2_MX || DeviceId == DEVICE_NVIDIA_GEFORCE2_MX_400)
        {
    ...
    if (VendorId == VENDOR_MATROX)
    {
        ...
        if (DeviceId == DEVICE_MATROX_G400 || DeviceId == DEVICE_MATROX_G550)
        {
