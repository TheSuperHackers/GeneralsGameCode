...
float ParticleBufferClass::LODMaxScreenSizes[17] = {
	NO_MAX_SCREEN_SIZE, NO_MAX_SCREEN_SIZE, NO_MAX_SCREEN_SIZE, NO_MAX_SCREEN_SIZE, NO_MAX_SCREEN_SIZE, NO_MAX_SCREEN_SIZE,
	NO_MAX_SCREEN_SIZE, NO_MAX_SCREEN_SIZE, NO_MAX_SCREEN_SIZE, NO_MAX_SCREEN_SIZE, NO_MAX_SCREEN_SIZE, NO_MAX_SCREEN_SIZE,
...
void ParticleBufferClass::Render_Line(RenderInfoClass &rinfo)
{
	...
	{
		default_diffuse
				.Set(ColorKeyFrameValues[0].X, ColorKeyFrameValues[0].Y, ColorKeyFrameValues[0].Z, AlphaKeyFrameValues[0]);
	}
	...
	if (tmp_points.Count() > 0)
	{
		...
		while (end < count)
		{
			...
			if (end - start > 1)
			{
				LineRenderer->Render(rinfo, Transform, end - start, &(tmp_points[start]), bounding_sphere, &(tmp_diffuse[start]));
			}
			start = end;
		}
	}
	...
	else
	{
		...
		for (i = 0; i < NumColorKeyFrames - 1; i++)
		{
			ColorKeyFrameDeltas[i] =
					(ColorKeyFrameValues[i + 1] - ColorKeyFrameValues[i]) / (float)(ColorKeyFrameTimes[i + 1] - ColorKeyFrameTimes[i]);
		}
		...
		else
		{
			...
			ColorKeyFrameDeltas[i] =
					(new_props.Values[i] - ColorKeyFrameValues[i]) / (new_props.KeyTimes[i] * 1000.0f - (float)ColorKeyFrameTimes[i]);
		}
	...
	else
	{
		...
		for (i = 0; i < NumAlphaKeyFrames - 1; i++)
		{
			AlphaKeyFrameDeltas[i] =
					(AlphaKeyFrameValues[i + 1] - AlphaKeyFrameValues[i]) / (float)(AlphaKeyFrameTimes[i + 1] - AlphaKeyFrameTimes[i]);
		}
		...
		else
		{
			...
			AlphaKeyFrameDeltas[i] =
					(new_props.Values[i] - AlphaKeyFrameValues[i]) / (new_props.KeyTimes[i] * 1000.0f - (float)AlphaKeyFrameTimes[i]);
		}
	...
	else
	{
		...
		for (i = 0; i < NumSizeKeyFrames - 1; i++)
		{
			SizeKeyFrameDeltas[i] =
					(SizeKeyFrameValues[i + 1] - SizeKeyFrameValues[i]) / (float)(SizeKeyFrameTimes[i + 1] - SizeKeyFrameTimes[i]);
		}
		...
		else
		{
			...
			SizeKeyFrameDeltas[i] =
					(new_props.Values[i] - SizeKeyFrameValues[i]) / (new_props.KeyTimes[i] * 1000.0f - (float)SizeKeyFrameTimes[i]);
		}
...
void ParticleBufferClass::Reset_Frames(ParticlePropertyStruct<float> &new_props)
{
	...
	{
		// Create the array if not present
		if ((RenderMode == W3D_EMITTER_RENDER_MODE_LINEGRP_TETRA) || (RenderMode == W3D_EMITTER_RENDER_MODE_LINEGRP_PRISM))
		{
			if (!UCoord)
			{
				UCoord = NEW_REF(ShareBufferClass<float>, (MaxNum, "ParticleBufferClass::UCoord"));
		...
		for (i = 0; i < NumFrameKeyFrames - 1; i++)
		{
			FrameKeyFrameDeltas[i] =
					(FrameKeyFrameValues[i + 1] - FrameKeyFrameValues[i]) / (float)(FrameKeyFrameTimes[i + 1] - FrameKeyFrameTimes[i]);
		}
		...
		else
		{
			...
			FrameKeyFrameDeltas[i] =
					(new_props.Values[i] - FrameKeyFrameValues[i]) / (new_props.KeyTimes[i] * 1000.0f - (float)FrameKeyFrameTimes[i]);
		}
...
void ParticleBufferClass::Update_Visual_Particle_State(void)
{
	...
	for (part = Start; part < sub1_end; part++)
	{
		...
		if (color)
		{
			...
			color[part] = ColorKeyFrameValues[ckey] + ColorKeyFrameDeltas[ckey] * (float)(part_age - ColorKeyFrameTimes[ckey])
		...
		if (alpha)
		{
			...
			alpha[part] = AlphaKeyFrameValues[akey] + AlphaKeyFrameDeltas[akey] * (float)(part_age - AlphaKeyFrameTimes[akey])
		...
		if (frame)
		{
			...
			float tmp_frame = FrameKeyFrameValues[fkey] + FrameKeyFrameDeltas[fkey] * (float)(part_age - FrameKeyFrameTimes[fkey])
		...
		if (ucoord)
		{
			...
			ucoord[part] = FrameKeyFrameValues[fkey] + FrameKeyFrameDeltas[fkey] * (float)(part_age - FrameKeyFrameTimes[fkey])
					+ RandomFrameEntries[part & NumRandomFrameEntriesMinus1];
		}
...
ParticleBufferClass::TailDiffuseTypeEnum ParticleBufferClass::Determine_Tail_Diffuse()
{
	...
	// Alpha
			...
			(shader.Get_Src_Blend_Func() == ShaderClass::SRCBLEND_SRC_ALPHA)
