...
WW3DErrorType ParticleEmitterDefClass::Read_Props(ChunkLoadClass &chunk_load)
{
	...
	if (chunk_load.Open_Chunk() && (chunk_load.Cur_Chunk_ID() == W3D_CHUNK_EMITTER_PROPS))
	{
		...
		if (chunk_load.Read(&info, sizeof(info)) == sizeof(info))
		{
			...
			if (last_keyframe > 0 && m_ColorKeyframes.Values[last_keyframe].X == 0 && m_ColorKeyframes.Values[last_keyframe].Y == 0
...
WW3DErrorType ParticleEmitterDefClass::Save_W3D(ChunkSaveClass &chunk_save)
{
	...
	if (chunk_save.Begin_Chunk(W3D_CHUNK_EMITTER) == TRUE)
	{
		...
		if ((Save_Header(chunk_save) == WW3D_ERROR_OK) && (Save_User_Data(chunk_save) == WW3D_ERROR_OK)
				...
				&& (Save_Props(chunk_save) == WW3D_ERROR_OK) && (Save_Line_Properties(chunk_save) == WW3D_ERROR_OK)
				&& (Save_Rotation_Keyframes(chunk_save) == WW3D_ERROR_OK) && (Save_Frame_Keyframes(chunk_save) == WW3D_ERROR_OK)
				&& (Save_Blur_Time_Keyframes(chunk_save) == WW3D_ERROR_OK) && (Save_Extra_Info(chunk_save) == WW3D_ERROR_OK))
		{
			// Success!
			ret_val = WW3D_ERROR_OK;
		}
...
WW3DErrorType ParticleEmitterDefClass::Save_Props(ChunkSaveClass &chunk_save)
{
	...
	if (chunk_save.Begin_Chunk(W3D_CHUNK_EMITTER_PROPS) == TRUE)
	{
		...
		if (chunk_save.Write(&info, sizeof(info)) == sizeof(info))
		{
			...
			//	Save the keyframes
			//
			if ((Save_Color_Keyframes(chunk_save) == WW3D_ERROR_OK) && (Save_Opacity_Keyframes(chunk_save) == WW3D_ERROR_OK)
