	...
	for (int pass = 0; pass < MAX_PASSES; pass++)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					WWDEBUG_SAY(
							("Unimplemented case: mesh has more than one default vertex material but no alternate vertex materials have "
							 "been defined."));
				}
				Material[pass] = NULL;
			}
		...
		if ((DCGSource[pass] != VertexMaterialClass::MATERIAL) && (ColorArray[0] != NULL))
		{
			...
			for (int vidx = 0; vidx < VertexCount; vidx++)
			{
				...
				else
				{
					if (PassCount != 1)
					{
						set_lighting_to_false = false; // Lighting can only be set to false if ALL passes and ALL materials are
																					 // requesting it
					}
				}
			}
