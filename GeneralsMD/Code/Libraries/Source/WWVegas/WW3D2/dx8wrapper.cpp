...
bool DX8Wrapper::Create_Device(void)
{
  ...
  Vertex_Processing_Behavior = (caps.DevCaps & D3DDEVCAPS_HWTRANSFORMANDLIGHT) ? D3DCREATE_MIXED_VERTEXPROCESSING :
  ...
  HRESULT hr =
      D3DInterface
          ->CreateDevice(CurRenderDevice, WW3D_DEVTYPE, _Hwnd, Vertex_Processing_Behavior, &_PresentParameters, &D3DDevice);
  ...
  if (FAILED(hr))
  {
    ...
    if ((_PresentParameters.BackBufferFormat == D3DFMT_R5G6B5 || _PresentParameters.BackBufferFormat == D3DFMT_X1R5G5B5
    ...
    bool restore_assets)
{
  ...
  */
  ...
  _PresentParameters.SwapEffect = D3DSWAPEFFECT_DISCARD; // IsWindowed ? D3DSWAPEFFECT_DISCARD : D3DSWAPEFFECT_FLIP;		//
                                                         // Shouldn't this be D3DSWAPEFFECT_FLIP?
...
bool DX8Wrapper::Toggle_Windowed(void)
{
  ...
  if (IsInitted)
  {
    ...
    if (curr_res == -1)
    {
      ...
      return Set_Device_Resolution(resolutions[0].Width, resolutions[0].Height, resolutions[0].BitDepth, !IsWindowed, true);
    }
    else
    {
...
bool DX8Wrapper::Registry_Save_Render_Device(const char *sub_key)
{
  ...
  return Registry_Save_Render_Device(
      sub_key,
      CurRenderDevice,
      ResolutionWidth,
      ResolutionHeight,
...
bool DX8Wrapper::Registry_Save_Render_Device(
    const char *sub_key,
    int device,
    int width,
    int height,
...
bool DX8Wrapper::Registry_Load_Render_Device(
    const char *sub_key,
    char *device,
    int device_len,
    int &width,
    ...
    D3DFORMAT *set_zmode)
{
  ...
  */
  ...
  if (bitdepth == 32 && *set_colorbuffer == D3DFMT_X8R8G8B8
      && D3DInterface->CheckDeviceType(0, D3DDEVTYPE_HAL, *set_colorbuffer, D3DFMT_A8R8G8B8, TRUE) == D3D_OK)
...
bool DX8Wrapper::Test_Z_Mode(D3DFORMAT colorbuffer, D3DFORMAT backbuffer, D3DFORMAT zmode)
{
  ...
  if (FAILED(D3DInterface->CheckDeviceFormat(
          D3DADAPTER_DEFAULT,
          WW3D_DEVTYPE,
          colorbuffer,
  ...
  if (vertex_count < 3)
  {
    ...
    switch (render_state.vertex_buffer_types[0])
    {
      ...
      case BUFFER_TYPE_SORTING:
        vertex_count = render_state.vertex_buffers[0]->Get_Vertex_Count() - render_state.index_base_offset
  ...
  switch (render_state.vertex_buffer_types[0])
  {
    ...
    case BUFFER_TYPE_DYNAMIC_DX8:
      switch (render_state.index_buffer_type)
      {
        ...
        case BUFFER_TYPE_DYNAMIC_DX8:
        {
          /*				if ((start_index+render_state.iba_offset+polygon_count*3) >
             render_state.index_buffer->Get_Index_Count()) {	WWASSERT_PRINT(0,"OVERFLOWING INDEX BUFFER");
    ...
    if (render_state_changed & mask)
    {
      SNAPSHOT_SAY(
          ("DX8 - apply texture %d (%s)",
           i,
           render_state.Textures[i] ? render_state.Textures[i]->Get_Full_Path().str() : "NULL"));
    ...
    bool rendertarget)
{
  ...
  if (ret == D3DERR_OUTOFVIDEOMEMORY)
  {
    ...
    else
    {
      ...
      WWDEBUG_SAY(
          ("...Texture creation failed. (%d x %d, format: %s, mips: %d", width, height, format_name.str(), mip_level_count));
    }
  }
  DX8_ErrorCode(ret);
    ...
    D3DPOOL pool)
{
  ...
  unsigned ret = DX8Wrapper::_Get_D3D_Device8()
                     ->CreateTexture(width, height, mip_level_count, D3DUSAGE_DEPTHSTENCIL, zfmt, pool, &texture);
  ...
  if (ret == D3DERR_OUTOFVIDEOMEMORY)
  {
    ...
    ret = DX8Wrapper::_Get_D3D_Device8()
              ->CreateTexture(width, height, mip_level_count, D3DUSAGE_DEPTHSTENCIL, zfmt, pool, &texture);
...
IDirect3DSurface8 *DX8Wrapper::_Create_DX8_Surface(const char *filename_)
{
...

  {
    ...
    if (!myfile->Is_Available())
    {
      ...
      if (ext && (strlen(ext) == 4) && ((ext[1] == 't') || (ext[1] == 'T')) && ((ext[2] == 'g') || (ext[2] == 'G'))
          && ((ext[3] == 'a') || (ext[3] == 'A')))
      {
        ext[1] = 'd';
        ext[2] = 'd';
