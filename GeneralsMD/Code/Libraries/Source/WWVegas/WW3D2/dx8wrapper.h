...
public:
    ...
    */
    ...
    static void Set_Projection_Transform_With_Z_Bias(
    ...
    static void Apply_Render_State_Changes(); // Apply deferred render state changes (will be called automatically by
                                              // Draw...)
...
protected:
    ...
    */
    ...
    static bool Registry_Save_Render_Device(
        const char *sub_key,
        int device,
        int width,
        int height,
        ...
        bool windowed,
        int texture_depth);
    static bool Registry_Load_Render_Device(
        const char *sub_key,
        char *device,
        int device_len,
        int &width,
...
WWINLINE void DX8Wrapper::Set_DX8_Render_State(D3DRENDERSTATETYPE state, unsigned value)
{
    ...
    if (WW3D::Is_Snapshot_Activated())
    {
        ...
        SNAPSHOT_SAY(("DX8 - SetRenderState(state: %s, value: %s)", Get_DX8_Render_State_Name(state), value_name.str()));
    }
#endif
...
WWINLINE RenderStateStruct::RenderStateStruct() : material(0), index_buffer(0)
{
    unsigned i;
    for (i = 0; i < MAX_VERTEX_STREAMS; ++i)
        vertex_buffers[i] = 0;
