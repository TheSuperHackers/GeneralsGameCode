...
inline VertexFormatXYZNDUV2 *DynamicVBAccessClass::WriteLockClass::Get_Formatted_Vertex_Array()
{
	...
	WWASSERT(
			DynamicVBAccess->VertexBuffer->FVF_Info().Get_FVF() == (D3DFVF_XYZ | D3DFVF_NORMAL | D3DFVF_TEX2 | D3DFVF_DIFFUSE));
	return Vertices;
}
...
public:
	...
	DX8VertexBufferClass(
			...
			unsigned vertex_size = 0); // Vertex size not used with FVF formats
	DX8VertexBufferClass(
