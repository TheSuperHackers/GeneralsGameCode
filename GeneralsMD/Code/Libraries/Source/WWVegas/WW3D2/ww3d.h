...
public:
	...
	static WW3DErrorType Set_Render_Device(
			int dev = -1,
			int resx = -1,
			int resy = -1,
			int bits = -1,
	...
	static WW3DErrorType Set_Device_Resolution(
			int w = -1,
			int h = -1,
			int bits = -1,
			int windowed = -1,
	...
	static WW3DErrorType Registry_Save_Render_Device(
			const char *sub_key,
			int device,
			int width,
			int height,
	...
	*/
	static WW3DErrorType Begin_Render(
			bool clear = false,
			bool clearz = true,
	...
	*/
	...
	static void Normalize_Coordinates(int x, int y, float &fx, float &fy); // convert pixel coordinates to 0..1 screen
																																				 // coordinates
