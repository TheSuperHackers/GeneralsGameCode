...
void TextureBaseClass::Invalidate_Old_Unused_Textures(unsigned invalidation_time_override)
{
  ...
  // Zero the texture apply count in this function because this is called every frame...(this wasn't in E&B main branch
  // KJM)
    ...
    TextureFormat(format)
{
  ...
  Poke_Texture(DX8Wrapper::_Create_DX8_Texture(width, height, format, mip_level_count, d3dpool, rendertarget));

  if (pool == POOL_DEFAULT)
  {
    Set_Dirty();
    DX8TextureTrackerClass *track = new DX8TextureTrackerClass(width, height, format, mip_level_count, this, rendertarget);
...
TextureClass::TextureClass(
    ...
    bool allow_reduction) :
    TextureBaseClass(0, 0, mip_level_count), Filter(mip_level_count), TextureFormat(texture_format)
{
...
TextureClass::TextureClass(SurfaceClass *surface, MipCountType mip_level_count) :
    TextureBaseClass(0, 0, mip_level_count), Filter(mip_level_count), TextureFormat(surface->Get_Surface_Format())
{
  IsProcedural = true;
  Initialized = true;
  IsReducible = false;
  ...
  Poke_Texture(DX8Wrapper::_Create_DX8_Texture(surface->Peek_D3D_Surface(), mip_level_count));
  LastAccessed = WW3D::Get_Sync_Time();
}

// ----------------------------------------------------------------------------
TextureClass::TextureClass(IDirect3DBaseTexture8 *d3d_texture) :
    TextureBaseClass(0, 0, ((MipCountType)d3d_texture->GetLevelCount())), Filter((MipCountType)d3d_texture->GetLevelCount())
{
  Initialized = true;
  IsProcedural = true;
  IsReducible = false;
    ...
    */
    if (hastexinfo)
    {
      ...
      newtex->Get_Filter().Set_U_Addr_Mode(
          u_clamp ? TextureFilterClass::TEXTURE_ADDRESS_CLAMP : TextureFilterClass::TEXTURE_ADDRESS_REPEAT);
      bool v_clamp = ((texinfo.Attributes & W3DTEXTURE_CLAMP_V) != 0);
      newtex->Get_Filter().Set_V_Addr_Mode(
          v_clamp ? TextureFilterClass::TEXTURE_ADDRESS_CLAMP : TextureFilterClass::TEXTURE_ADDRESS_REPEAT);
    }
    else
    {
...
ZTextureClass::ZTextureClass(
    ...
    MipCountType mip_level_count,
    PoolType pool) :
    TextureBaseClass(width, height, mip_level_count, pool), DepthStencilTextureFormat(zformat)
{
  D3DPOOL d3dpool = (D3DPOOL)0;
  switch (pool)
  {
  ...
  Poke_Texture(DX8Wrapper::_Create_DX8_ZTexture(width, height, zformat, mip_level_count, d3dpool));

  if (pool == POOL_DEFAULT)
  {
    Set_Dirty();
    DX8ZTextureTrackerClass *track = new DX8ZTextureTrackerClass(width, height, zformat, mip_level_count, this);
    DX8TextureManagerClass::Add(track);
  }
  Initialized = true;
  IsProcedural = true;
    ...
    TextureClass(width, height, format, mip_level_count, pool, rendertarget)
{
  ...
  Poke_Texture(DX8Wrapper::_Create_DX8_Cube_Texture(width, height, format, mip_level_count, d3dpool, rendertarget));
...
VolumeTextureClass::VolumeTextureClass(
    ...
    bool rendertarget,
    bool allow_reduction) :
    TextureClass(width, height, format, mip_level_count, pool, rendertarget), Depth(depth)
{
  Initialized = true;
  IsProcedural = true;
  IsReducible = false;
  ...
  Poke_Texture(DX8Wrapper::_Create_DX8_Volume_Texture(width, height, depth, format, mip_level_count, d3dpool));
...
VolumeTextureClass::VolumeTextureClass(
    ...
    bool allow_compression,
    bool allow_reduction) :
    TextureClass(0, 0, mip_level_count, POOL_MANAGED, false, texture_format), Depth(0)
{
  IsCompressionAllowed = allow_compression;
  InactivationTime = DEFAULT_INACTIVATION_TIME; // Default inactivation time 30 seconds
