...
struct W3dVertexMaterialStruct
{
    ...
    {
        return (
            Attributes == vm.Attributes && Ambient == vm.Ambient && Diffuse == vm.Diffuse && Specular == vm.Specular
...
struct W3dShaderStruct
{
    ...
    {
        return (
            (DepthCompare == that.DepthCompare) && (DepthMask == that.DepthMask) &&
            /*(ColorMask == that.ColorMask) &&       obsolete*/
            (DestBlend == that.DestBlend) &&
            /*(FogFunc == that.FogFunc) &&           obsolete*/
            (PriGradient == that.PriGradient) && (SecGradient == that.SecGradient) && (SrcBlend == that.SrcBlend)
            && (Texturing == that.Texturing) && (DetailColorFunc == that.DetailColorFunc)
            && (DetailAlphaFunc == that.DetailAlphaFunc) &&
            /*(ShaderPreset == that.ShaderPreset) &&  obsolete */
            (AlphaTest == that.AlphaTest));
    }

    inline bool operator!=(const W3dShaderStruct &that) { return !(*this == that); }
};
...
/********************************************************************************
    ...
    W3D_CHUNK_MORPH_ANIMATION							=0x000002C0,	// wrapper for the entire anim
        W3D_CHUNK_MORPHANIM_HEADER,										// W3dMorphAnimHeaderStruct describes playback rate,
number of frames, and type of compression W3D_CHUNK_MORPHANIM_CHANNEL,										// wrapper for a
channel W3D_CHUNK_MORPHANIM_POSENAME,									// name of the other anim which contains the poses
for this morph channel W3D_CHUNK_MORPHANIM_KEYDATA,									// array of W3dMorphAnimKeyStruct's (use
chunk length to determine how many) W3D_CHUNK_MORPHANIM_PIVOTCHANNELDATA,							// uin32 per pivot in the
htree, indicating which channel controls the pivot
...
/********************************************************************************
    ...
    W3D_CHUNK_HLOD										=0x00000700,		// description of an HLod object (see HLodClass)
        W3D_CHUNK_HLOD_HEADER,												// general information such as name and version
        W3D_CHUNK_HLOD_LOD_ARRAY,											// wrapper around the array of objects for each
level of detail W3D_CHUNK_HLOD_SUB_OBJECT_ARRAY_HEADER,					// info on the objects in this level of detail array
            W3D_CHUNK_HLOD_SUB_OBJECT,										// an object in this level of detail array
        W3D_CHUNK_HLOD_AGGREGATE_ARRAY,									// array of aggregates, contains
W3D_CHUNK_SUB_OBJECT_ARRAY_HEADER and W3D_CHUNK_SUB_OBJECT_ARRAY W3D_CHUNK_HLOD_PROXY_ARRAY,
