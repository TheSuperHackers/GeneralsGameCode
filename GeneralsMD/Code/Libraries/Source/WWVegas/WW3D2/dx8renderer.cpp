...
bool DX8TextureCategoryClass::m_gForceMultiply = false; // Forces opaque materials to use the multiply blend - pseudo
	...
	while (renderedVertexCount < VisibleVertexCount)
	{
		mesh = remainingMesh;
		{
			...
			while (mesh != NULL)
			{
				...
				// Example: A skinned mesh was added to the scene then it was attached to a bone without being removed from the
				// scene.
...
void DX8TextureCategoryClass::Render(void)
{
	...
	VertexMaterialClass *vmaterial = (VertexMaterialClass *)Peek_Material(); // ugly cast from const but we'll restore it after
																																					 // changes so okay. -MW
