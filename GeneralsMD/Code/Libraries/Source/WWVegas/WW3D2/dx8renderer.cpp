...
bool DX8TextureCategoryClass::m_gForceMultiply =
    false; // Forces opaque materials to use the multiply blend - pseudo transparent effect.  jba.
// ----------------------------------------------------------------------------
...
void DX8TextureCategoryClass::Log(bool only_visible)
{
  ...
  work.Format("	%8s %8s %6s %6s %6s %5s %s", "idx_cnt", "poly_cnt", "i_offs", "min_vi", "vi_rng", "ident", "name");
  WWDEBUG_SAY((work));
...
class Vertex_Split_Table
{
  ...
  public:
  Vertex_Split_Table(MeshModelClass *mmc_) : mmc(mmc_), npatch_enable(false), allocated_polygon_array(false)
  {
  ...
  const unsigned *Get_Color_Array(unsigned index) const { return mmc->Get_Color_Array(index, false); }
...
DX8SkinFVFCategoryContainer::DX8SkinFVFCategoryContainer(bool sorting) :
    DX8FVFCategoryContainer(DX8_FVF_XYZNUV1, sorting), VisibleVertexCount(0), VisibleSkinHead(NULL), VisibleSkinTail(NULL)
{
}
...
void DX8SkinFVFCategoryContainer::Render(void)
{
  ...
  DynamicVBAccessClass vb(sorting ? BUFFER_TYPE_DYNAMIC_SORTING : BUFFER_TYPE_DYNAMIC_DX8, dynamic_fvf_type, maxVertexCount);
  ...
  while (renderedVertexCount < VisibleVertexCount)
  {
    mesh = remainingMesh;
    {
      ...
      while (mesh != NULL)
      {
        ...
        // Example: A skinned mesh was added to the scene then it was attached to a bone without being removed from
        // the scene.
...
void DX8SkinFVFCategoryContainer::Add_Visible_Skin(MeshClass *mesh)
{
  ...
  {
    DEBUG_CRASH(
        ("Mesh %s is already a visible skin, and we tried to add it again... please notify Mark W or Steven J "
         "immediately!",
         mesh->Get_Name()));
    return;
  }
...
void DX8TextureCategoryClass::Render(void)
{
  ...
  VertexMaterialClass *vmaterial =
      (VertexMaterialClass *)Peek_Material(); // ugly cast from const but we'll restore it after changes so okay. -MW
    ...
    if (!DX8RendererDebugger::Is_Enabled() || !mesh->Is_Disabled_By_Debugger())
    {
      ...
      else
      {
        ...
        { // mesh has material override of some kind
          ...
          if (mesh->Get_User_Data() && *(int *)mesh->Get_User_Data() == RenderObjClass::USER_DATA_MATERIAL_OVERRIDE
              && oldMapper && oldMapper->Mapper_ID() == TextureMapperClass::MAPPER_ID_LINEAR_OFFSET)
          {
...
static void Add_Rigid_Mesh_To_Container(FVFCategoryList *container_list, unsigned fvf, MeshModelClass *mmc)
{
  ...
  bool sorting =
      ((!!mmc->Get_Flag(MeshModelClass::SORT)) && WW3D::Is_Sorting_Enabled() && (mmc->Get_Sort_Level() == SORT_LEVEL_NONE));
...
#ifdef ENABLE_CATEGORY_LOG
  WWDEBUG_SAY(
      ("Registering mesh: %s (%d polys, %d verts + %d gap polygons)",
       mmc->Get_Name(),
       mmc->Get_Polygon_Count(),
       mmc->Get_Vertex_Count(),
