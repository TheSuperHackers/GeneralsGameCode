...
public:
	...
	enum WorldToScreenReturn CPP_11( : Int){ WTS_INSIDE_FRUSTUM = 0, // On the screen (inside frustum of camera)
																					 WTS_OUTSIDE_FRUSTUM, // Return is valid but off the screen (outside frustum of
																																// camera)
...
public:
	...
	virtual FilterModes getViewFilterMode(void)
	{
		return (FilterModes)0;
	} ///< Turns on viewport special effect (black & white mode)
	virtual FilterTypes getViewFilterType(void)
	{
		return (FilterTypes)0;
	} ///< Turns on viewport special effect (black & white mode)
	virtual Bool setViewFilterMode(FilterModes filterMode)
	{
		return FALSE;
	} ///< Turns on viewport special effect (black & white mode)
	...
	virtual void rotateCameraTowardPosition(
			...
			Real easeIn,
			Real easeOut,
			Bool reverseRotation) {}; ///< Rotate camera to face a location.
	...
	virtual void zoom(Real height); ///< Zoom in/out, closer to the ground, limit to min, or farther away from the ground,
																	///< limit to max
	...
	virtual WorldToScreenReturn worldToScreenTriReturn(const Coord3D *w, ICoord2D *s) = 0; ///< Like worldToScreen(), but with
																																												 ///< a more informative return value
	virtual void screenToWorld(const ICoord2D *s, Coord3D *w) = 0; ///< Transform screen coordinate "s" into world coordinate
																																 ///< "w"
	virtual void screenToTerrain(const ICoord2D *screen, Coord3D *world) = 0; ///< transform screen coord to a point on the 3D
																																						///< terrain
	virtual void screenToWorldAtZ(const ICoord2D *s, Coord3D *w, Real z) = 0; ///< transform screen point to world point at the
