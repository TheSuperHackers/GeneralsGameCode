...
enum ParticleSystemID CPP_11( : Int){ INVALID_PARTICLE_SYSTEM_ID = 0 };
...
enum ParticlePriorityType CPP_11( : Int){
  ...
  ALWAYS_RENDER, ///< used for logically important display (not just fluff), so must never be culled, regardless of
                 ///< particle cap, lod, etc
...
class Particle : public MemoryPoolObject, public ParticleInfo
{
  ...
  inline void setIsCulled(Bool enable)
  {
    m_isCulled = enable;
...
static const char *ParticleShaderTypeNames[] = { "NONE", "ADDITIVE", "ALPHA", "ALPHA_TEST", "MULTIPLY", NULL };

static const char *ParticleTypeNames[] = { "NONE", "PARTICLE", "DRAWABLE", "STREAK", "VOLUME_PARTICLE", "SMUDGE", NULL };

static const char *EmissionVelocityTypeNames[] = { "NONE",        "ORTHO",   "SPHERICAL", "HEMISPHERICAL",
                                                   "CYLINDRICAL", "OUTWARD", NULL };

static const char *EmissionVolumeTypeNames[] = { "NONE", "POINT", "LINE", "BOX", "SPHERE", "CYLINDER", NULL };

//"NONE", "FLUFF", "DEBRIS", "NATURE", "WEAPON", "DAMAGE", "SPECIAL"
static const char *ParticlePriorityNames[] = { "NONE",           "WEAPON_EXPLOSION", "SCORCHMARK",
                                               "DUST_TRAIL",     "BUILDUP",          "DEBRIS_TRAIL",
                                               "UNIT_DAMAGE_FX", "DEATH_EXPLOSION",  "SEMI_CONSTANT",
                                               "CONSTANT",       "WEAPON_TRAIL",     "AREA_EFFECT",
                                               "CRITICAL",       "ALWAYS_RENDER",    NULL };

static const char *WindMotionNames[] = { "NONE", "Unused", "PingPong", "Circular", NULL };
...
class ParticleSystem : public MemoryPoolObject, public ParticleSystemInfo
{
  ...
  {
    return (m_particleType == VOLUME_PARTICLE) ? OPTIMUM_VOLUME_PARTICLE_DEPTH : 0;
  }
  ...
  /// called to merge two systems info. If slaveNeedsFullPromotion is true, then the slave needs to be aware of how many
  /// particles to generate as well.
  static ParticleInfo mergeRelatedParticleSystems(
  ...
  virtual Particle *createParticle(
      const ParticleInfo *data,
      ParticlePriorityType priority,
      Bool forceCreate = FALSE); ///< factory method for particles

  const ParticleInfo *generateParticleInfo(
      Int particleNum,
      Int particleCount); ///< generate a new, random set of ParticleInfo
...
class ParticleSystemManager : public SubsystemInterface, public Snapshot
{
  ...
  typedef std::hash_map<AsciiString, ParticleSystemTemplate *, rts::hash<AsciiString>, rts::equal_to<AsciiString> >
      TemplateMap;
  ...
  */
  ParticleSystemID createAttachedParticleSystemID(
      const ParticleSystemTemplate *sysTemplate,
      Object *attachTo,
