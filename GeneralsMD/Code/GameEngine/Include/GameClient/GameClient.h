...
public:
    ...
    virtual void registerDrawable(
        Drawable *draw); ///< Given a drawable, register it with the GameClient and give it a unique ID
    ...
    virtual void createRayEffectByTemplate(
        const Coord3D *start,
        const Coord3D *end,
        const ThingTemplate *tmpl) = 0; ///< create effect needing start and end location
    ...
    virtual void iterateDrawablesInRegion(
        ...
        void *userData); ///< Calls userFunc for each drawable contained within the region

    virtual Drawable *friend_createDrawable(
        const ThingTemplate *thing,
        DrawableStatus statusBits = DRAWABLE_STATUS_NONE) = 0;
...
protected:
    ...
    //	DrawablePtrHash m_drawableHash;															///< Used for DrawableID
    //lookups
...
private:
    ...
    virtual Display *createGameDisplay(
        void) = 0; ///< Factory for Display classes. Called during init to instantiate TheDisplay.
    virtual InGameUI *createInGameUI(
        void) = 0; ///< Factory for InGameUI classes. Called during init to instantiate TheInGameUI
    ...
    virtual TerrainVisual *createTerrainVisual(
        void) = 0; ///< Factory for TerrainVisual classes. Called during init to instance TheTerrainVisual
