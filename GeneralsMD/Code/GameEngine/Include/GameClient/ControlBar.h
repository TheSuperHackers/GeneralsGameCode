...
enum CommandOption CPP_11( : Int){
  ...
  CONTEXTMODE_COMMAND =
      0x00000200, // a context sensitive command mode that requires code to determine whether cursor is valid or not.
  ...
  ATTACK_OBJECTS_POSITION =
      0x00001000, // for weapons that need an object target but attack the position indirectly (like burning trees)
...
static const char *TheCommandOptionNames[] = { "NEED_TARGET_ENEMY_OBJECT",
...
const UnsignedInt COMMAND_OPTION_NEED_TARGET =
    NEED_TARGET_ENEMY_OBJECT | NEED_TARGET_NEUTRAL_OBJECT | NEED_TARGET_ALLY_OBJECT | NEED_TARGET_POS | CONTEXTMODE_COMMAND;
...
enum GUICommandType CPP_11( : Int){
  ...
  GUI_COMMAND_SPECIAL_POWER_FROM_SHORTCUT, ///< do a special power from localPlayer's command center, regardless of
                                           ///< selection
  GUI_COMMAND_SPECIAL_POWER_CONSTRUCT, ///< do a special power using the construct building interface
  GUI_COMMAND_SPECIAL_POWER_CONSTRUCT_FROM_SHORTCUT, ///< do a shortcut special power using the construct building
                                                     ///< interface
...
static const char *TheGuiCommandNames[] = { "NONE",
                                            "DOZER_CONSTRUCT",
...
enum CommandButtonMappedBorderType CPP_11( : Int){
  COMMAND_BUTTON_BORDER_NONE = 0, COMMAND_BUTTON_BORDER_BUILD,  COMMAND_BUTTON_BORDER_UPGRADE,
  COMMAND_BUTTON_BORDER_ACTION,   COMMAND_BUTTON_BORDER_SYSTEM,
...
class CommandButton : public Overridable
{
  ...
  AsciiString m_conflictingLabel; ///< Description for the current command if it can't be selected due to
                                  ///< multually-exclusive choice.
...
//-------------------------------------------------------------------------------------------------
enum ControlBarContext CPP_11( : Int){ CB_CONTEXT_NONE, ///< default view for center bar and portrait window
...
//-------------------------------------------------------------------------------------------------
enum ContextParent CPP_11( : Int){ CP_MASTER, ///< *The* control bar window as a whole
                                   CP_PURCHASE_SCIENCE,
...
enum CommandAvailability CPP_11( : Int){
  COMMAND_RESTRICTED, COMMAND_AVAILABLE, COMMAND_ACTIVE, COMMAND_HIDDEN, COMMAND_NOT_READY, COMMAND_CANT_AFFORD,
};

enum ControlBarStages CPP_11( : Int){ CONTROL_BAR_STAGE_DEFAULT = 0, ///< full view for the world to see
...
class ControlBar : public SubsystemInterface
{
  ...
  CommandAvailability getCommandAvailability(
  ...
  GameWindow *m_contextParent[NUM_CONTEXT_PARENTS]; ///< "parent" window for buttons that are part of the context sensitive
                                                    ///< interface
  ...
  // GameWindow *m_commandMarkers[ MAX_COMMANDS_PER_SET ];			///< When we don't have a command, they want to show an
  // image
  // removed from multiplayer branch
  // void showCommandMarkers( void );													///< function that compare's what's being shown in
  // m_commandWindows and shows the ones that are hidden.
  ...
  static ContainEntry m_containData[MAX_COMMANDS_PER_SET]; ///< inventory buttons integrated into the regular command set
                                                           ///< for buildings/transports
