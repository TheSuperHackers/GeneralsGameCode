...
#define SET_SMUDGE_PARAMETERS(smudge, pos, offset, size, opacity) \
    (smudge->m_pos = pos; smudge->m_offset = offset; smudge->m_size = size; smudge->m_opacity = opacity;)
...
struct Smudge : public DLNodeClass<Smudge>
{
    ...
    smudgeVertex
        m_verts[5]; // 5 vertices of this smudge (in counter-clockwise order, starting at top-left, ending in center.)
};
...
public:
    ...
    inline Int getSmudgeCountLastFrame(void)
    {
