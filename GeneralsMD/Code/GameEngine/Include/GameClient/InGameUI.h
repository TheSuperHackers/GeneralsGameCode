...
class InGameUI : public SubsystemInterface, public Snapshot
{
  ...
  virtual void messageNoFormat(
      const RGBColor *rgbColor,
      const UnicodeString &message); ///< display a colored message to the user
  ...
  virtual void beginAreaSelectHint(
      const GameMessage *msg); ///< Used by HintSpy. An area selection is occurring, start graphical "hint"
  virtual void endAreaSelectHint(
      const GameMessage *msg); ///< Used by HintSpy. An area selection had occurred, finish graphical "hint"
  ...
  virtual void createForceAttackHint(
      const GameMessage *msg); ///< A force attack command has occurred, start graphical "hint"
  ...
  virtual Bool removeSuperweapon(
      ...
      ObjectID id,
      const SpecialPowerTemplate *powerTemplate);
  virtual void objectChangedTeam(
      const Object *obj,
      Int oldPlayerIndex,
      Int newPlayerIndex); // notification for superweapons, etc
  ...
  virtual void setGUICommand(
      const CommandButton *command); ///< the command has been clicked in the UI and needs additional data
  ...
  virtual const DrawableList *getAllSelectedDrawables(
      void) const; ///< Return the list of all the currently selected Drawable IDs.
  virtual const DrawableList *getAllSelectedLocalDrawables(
      void); ///< Return the list of all the currently selected Drawable IDs owned by the current player.
  ...
  virtual Bool areAllObjectsSelected(const std::vector<Object *> &objectsToCheck)
      const; ///< Return true if all of the selected objects are in the drawable list
  ...
  virtual void disregardDrawable(
      Drawable *draw); ///< Drawable is being destroyed, clean up any UI elements associated with it
  ...
  virtual void buildRegion(
      const ICoord2D *anchor,
      const ICoord2D *dest,
      IRegion2D *region); ///< builds a region around the specified coordinates
  ...
  struct MilitarySubtitleData
  {
    ...
    UnsignedInt currentDisplayString; ///< contains the current display string we're on. (also lets us know the last
                                      ///< display string allocated
  ...
  void addMessageText(
      const UnicodeString &formattedMessage,
      const RGBColor *rgbColor = NULL); ///< internal workhorse for adding plain text for messages
  ...
  DrawableID
      m_soloNexusSelectedDrawableID; ///< The drawable of the nexus, if only one angry mob is selected, otherwise, null
