...
static const char *TheShadowNames[] = { "SHADOW_DECAL",
                                        "SHADOW_VOLUME",
                                        "SHADOW_PROJECTION",
...
#define MAX_SHADOW_LIGHTS \
  1 // maximum number of shadow casting light sources in scene - support for more than 1 has been dropped from most code.
...
inline void Shadow::setOpacity(Int value)
{
  ...
  else
  {
    if (m_type & SHADOW_ADDITIVE_DECAL)
    {
      ...
      m_diffuse = REAL_TO_INT(((Real)(m_color & 0xff) * fvalue)) | REAL_TO_INT(((Real)((m_color >> 8) & 0xff) * fvalue))
...
class ProjectedShadowManager
{
  ...
  virtual Shadow *addDecal(
      Shadow::ShadowTypeInfo *shadowInfo) = 0; ///< add a non-shadow decal which does not follow an object.
};
