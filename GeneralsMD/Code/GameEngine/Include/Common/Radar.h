...
//-------------------------------------------------------------------------------------------------
class RadarObject : public MemoryPoolObject, public Snapshot
{
...
public:
	...
	Bool localPixelToRadar(const ICoord2D *pixel, ICoord2D *radar); ///< translate pixel (with UL of radar being (0,0)) to
																																	///< logical radar coords
	Bool screenPixelToWorld(const ICoord2D *pixel, Coord3D *world); ///< translate pixel (with UL of the screen being (0,0)) to
																																	///< world position in the world
	Object *objectUnderRadarPixel(const ICoord2D *pixel); ///< return the object (if any) represented by the pixel coords
																												///< passed in
	void findDrawPositions(Int startX, Int startY, Int width, Int height, ICoord2D *ul, ICoord2D *lr); ///< make translation
																																																		 ///< for screen area of
																																																		 ///< radar square to
																																																		 ///< scaled aspect ratio
																																																		 ///< preserving points
	...
	// radar events
	void createEvent(const Coord3D *world, RadarEventType type, Real secondsToLive = 4.0f); ///< create radar event at location
																																													///< in world
	void createPlayerEvent(Player *player, const Coord3D *world, RadarEventType type, Real secondsToLive = 4.0f); ///< create
																																																								///< radar
																																																								///< event
																																																								///< using
																																																								///< player
...
protected:
	...
	struct RadarEvent
	{
		...
		Bool active; ///< TRUE when event is "active", otherwise it's just historical information in the event array to look
								 ///< through
