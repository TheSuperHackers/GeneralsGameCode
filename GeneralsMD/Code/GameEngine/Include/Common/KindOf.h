...
enum KindOfType CPP_11( : Int){
  ...
  KINDOF_STICK_TO_TERRAIN_SLOPE, ///< should be stuck at ground level, aligned to terrain slope. requires that IMMOBILE bit
                                 ///< is also set.
  ...
  KINDOF_CASH_GENERATOR, ///< used to check if the unit generates cash... checked by cash hackers and whatever else comes
                         ///< up
  KINDOF_DRAWABLE_ONLY, ///< template is used only to create drawables (not Objects)
  KINDOF_MP_COUNT_FOR_VICTORY, ///< If a player loses all his buildings that have this kindof in a multiplayer game, he
                               ///< loses.
  ...
  KINDOF_CAPTURABLE, ///< is "capturable" even if not an enemy (should generally be used only for structures, eg, Tech
                     ///< bldgs)
  ...
  KINDOF_AUTO_RALLYPOINT, ///< When immobile-structure-object is selected, left clicking on ground will set new rally point
                          ///< without requiring command button.
  KINDOF_TECH_BUILDING, ///< Neutral tech building - Oil derrick, Hospital, Radio Station, Refinery.
  KINDOF_POWERED, ///< This object gets the Underpowered disabled condition when its owning player has power consumption
                  ///< exceed supply
  KINDOF_PRODUCED_AT_HELIPAD, ///< ugh... hacky fix for comanche. (srj)
  KINDOF_DRONE, ///< Object drone type -- used for filtering them out of battle plan bonuses, making un-snipable, and
                ///< whatever else may come up.
  KINDOF_CAN_SEE_THROUGH_STRUCTURE, ///< Structure does not block line of sight.
  KINDOF_BALLISTIC_MISSILE, ///< Large ballistic missiles that are specifically large enough to be targetted by base
                            ///< defenses.
  KINDOF_CLICK_THROUGH, ///< Objects with this will never be picked by mouse interactions!
  KINDOF_SUPPLY_SOURCE_ON_PREVIEW, ///< Any thing that we can get "supplies" from that we want to show up on the map
                                   ///< preview
  ...
  KINDOF_REVEALS_ENEMY_PATHS, ///< like the listening outpost... when selected, any enemy drawable will draw show paths
                              ///< when moused over
  ...
  KINDOF_FS_ADVANCED_TECH, ///< Represents each faction's advanced techtree building -- strategy center, propaganda center,
                           ///< and palace.
  ...
  KINDOF_EMP_HARDENED, ///< Like a delivery plane (B52, B3, CargoPlane,etc.)  or a SpectreGunship, which sort-of IS the
                       ///< weapon...
  KINDOF_DEMOTRAP, ///< Added strictly only for disarming purposes. They don't act like mines which have rendering and
                   ///< selection implications!
  KINDOF_CONSERVATIVE_BUILDING, ///< Conservative structures aren't considered part of your base for sneak attack boundary
                                ///< calculations...
...
inline Bool TEST_KINDOFMASK_MULTI(
    const KindOfMaskType &m,
