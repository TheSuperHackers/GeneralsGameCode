...
enum ThingTemplateAudioType CPP_11( : Int){
    ...
    TTAUDIO_soundFalling, ///< This sound is actually called on a unit when it is exiting another.
                          ///< However, there is a soundExit which refers to the container, and this is only used for bombs
                          ///< falling from planes.
...
static const char *BuildableStatusNames[] = {"Yes", "Ignore_Prerequisites", "No", "Only_By_AI", NULL};
...
public:
    ...
    */
    ...
    inline Bool isKindOf(KindOfType t) const { return TEST_KINDOFMASK(m_kindof, t); }
    ...
    inline Bool isAnyKindOf(const KindOfMaskType &anyKindOf) const { return TEST_KINDOFMASK_ANY(m_kindof, anyKindOf); }
    ...
    {
        return (RadarPriorityType)m_radarPriority;
...
protected:
    ...
    {
        return m_audioarray.m_audio[t] ? &m_audioarray.m_audio[t]->m_event : &s_audioEventNoSound;
    }
...
private:
    ...
    AsciiString m_upgradeCameoUpgradeNames[MAX_UPGRADE_CAMEO_UPGRADES]; ///< Use these to find the upgrade images to display
                                                                        ///< on the control bar
    AsciiString m_shadowTextureName; ///< name of texture to use for shadow decal
    AsciiString m_moduleBeingReplacedName; ///< used only during map.ini loading... name (not tag) of Module being replaced,
                                           ///< or empty if not inside ReplaceModule block
    AsciiString m_moduleBeingReplacedTag; ///< used only during map.ini loading... tag (not name) of Module being replaced,
                                          ///< or empty if not inside ReplaceModule block
#ifdef LOAD_TEST_ASSETS
    ...
    // TheSuperHackers @bugfix Caball009/xezon 06/07/2025 No longer copy SparseMatchFinder to prevent copied instances
    // linking to unrelated data. This avoids mismatching in certain maps, for example those that spawn units with veterancy.
    ...
    std::vector<AsciiString> m_buildVariations; /**< if we build a unit of this type via script or ui, randomly choose one
                                                                                                of these templates instead.
                                                   (doesn't apply to MapObject-created items) */
    ...
    Real
        m_shroudClearingRange; ///< Since So many things got added to "Seeing" functionality, we need to split this part out.
    Real m_shroudRevealToAllRange; ///< When > zero, the shroud gets revealed to all players.
    Real
        m_placementViewAngle; ///< when placing buildings this will be the angle of the building when "floating" at the mouse
    Real m_factoryExitWidth; ///< when placing buildings this will be the width of the reserved exit area on the right side.
    Real m_factoryExtraBibWidth; ///< when placing buildings this will be the width of the reserved exit area on the right
                                 ///< side.
    ...
    NameKeyType m_maxSimultaneousLinkKey; ///< If this is not NAMEKEY_INVALID, it indicates that all the templates which have
                                          ///< the same name key should be counted as the same "type" when looking at
    ...
    UnsignedShort
        m_maxSimultaneousOfType; ///< max simultaneous of this unit we can have (per player) at one time. (0 == unlimited)

    // ---- Bool-sized things
    Bool m_maxSimultaneousDeterminedBySuperweaponRestriction; ///< If true, override value in m_maxSimultaneousOfType with
                                                              ///< value from GameInfo::getSuperweaponRestriction()
    ...
    UnsignedByte
        m_crushableLevel; ///< Specifies the level of crushability (must be hit by a crusher greater than this to crush me).
};
