...
static const char *BuildCompletionNames[] = { "INVALID",
...
private:
	...
	AsciiString m_upgradeCameoUpgradeNames[MAX_UPGRADE_CAMEO_UPGRADES]; ///< Use these to find the upgrade images to display on
																																			///< the control bar
	AsciiString m_shadowTextureName; ///< name of texture to use for shadow decal
	AsciiString m_moduleBeingReplacedName; ///< used only during map.ini loading... name (not tag) of Module being replaced, or
																				 ///< empty if not inside ReplaceModule block
	AsciiString m_moduleBeingReplacedTag; ///< used only during map.ini loading... tag (not name) of Module being replaced, or
																				///< empty if not inside ReplaceModule block
#ifdef LOAD_TEST_ASSETS
	...
	// TheSuperHackers @bugfix Caball009/xezon 06/07/2025 No longer copy SparseMatchFinder to prevent copied instances linking
	// to unrelated data. This avoids mismatching in certain maps, for example those that spawn units with veterancy.
	...
	UnsignedShort m_maxSimultaneousOfType; ///< max simultaneous of this unit we can have (per player) at one time. (0 ==
																				 ///< unlimited)
	...
	UnsignedByte m_crushableLevel; ///< Specifies the level of crushability (must be hit by a crusher greater than this to
																 ///< crush me).
};
