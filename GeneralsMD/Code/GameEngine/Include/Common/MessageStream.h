...
enum GameMessageArgumentDataType CPP_11( : Int){
  ARGUMENTDATATYPE_INTEGER,     ARGUMENTDATATYPE_REAL,      ARGUMENTDATATYPE_BOOLEAN,  ARGUMENTDATATYPE_OBJECTID,
  ARGUMENTDATATYPE_DRAWABLEID,  ARGUMENTDATATYPE_TEAMID,    ARGUMENTDATATYPE_LOCATION, ARGUMENTDATATYPE_PIXEL,
  ARGUMENTDATATYPE_PIXELREGION, ARGUMENTDATATYPE_TIMESTAMP, ARGUMENTDATATYPE_WIDECHAR, ARGUMENTDATATYPE_UNKNOWN
};
...
class GameMessage : public MemoryPoolObject
{
  ...
  // They do not get built into RELEASE builds.
    ...
    MSG_META_DEMO_REMOVE_PREREQ, ///< Turn off Prerequisite checks in building legality, works in MULTIPLAYER for all
                                 ///< humans
    ...
    MSG_META_DEMO_TOGGLE_SUPPLY_CENTER_PLACEMENT, ///< start/stop dumping to file all thoughts about placing
                                                  ///< SupplyCenters
    ...
    MSG_META_DEBUG_DUMP_ALL_PLAYER_OBJECTS, ///< Dump numbers of objects owned by each player to the script debug window,
                                            ///< and additional object info
  ...
  /// @todo END section to REMOVE (not disable) for release
    ...
    MSG_INVALID_GUICOMMAND_HINT, ///< posted when the gui command is not valid if the user were to click to attempt to
                                 ///< execute it.
    MSG_AREA_SELECTION_HINT, ///< (pixelRegion) rectangular selection area under construction, not confirmed

    // Command hints
    MSG_DO_ATTACK_OBJECT_HINT, ///< (victim objectID) If clicked, an attack would be ordered, "Current Selection" is
                               ///< assumed
    MSG_IMPOSSIBLE_ATTACK_HINT, ///< we can't do anything, and target is out of range.
    MSG_DO_FORCE_ATTACK_OBJECT_HINT, ///< (victim objectID) If clicked, an attack would be ordered, "Current Selection"
                                     ///< is assumed
    MSG_DO_FORCE_ATTACK_GROUND_HINT, ///< (victim objectID) If clicked, an attack would be ordered, "Current Selection"
                                     ///< is assumed
    ...
    */
    ...
    MSG_BEGIN_DEBUG_NETWORK_MESSAGES =
        1900, ///< network messages that exist only in debug/internal builds. all grouped separately.
...
class GameMessageList : public SubsystemInterface
{
  ...
  virtual void insertMessage(
      GameMessage *msg,
      GameMessage *messageToInsertAfter); // Insert message after messageToInsertAfter.
...
class MessageStream : public GameMessageList
{
  ...
  virtual GameMessage *insertMessage(
      GameMessage::Type type,
      GameMessage *messageToInsertAfter); // Insert message after messageToInsertAfter.
