...
class CommandLineData
{
    ...
    CommandLineData() : m_hasParsedCommandLineForStartup(false), m_hasParsedCommandLineForEngineInit(false) {}
...
public:
    ...
    Real m_drawSkyBox;
    Real m_skyBoxScale;
    Real m_viewportHeightScale; // The height scale of the tactical view ranging 0..1. Used to hide the world behind the
                                // Control Bar.
    Real m_cameraPitch;
    Real m_cameraYaw;
    Real m_cameraHeight;
    ...
    Int m_simulateReplayJobs; ///< Maximum number of processes to use for simulation, or SIMULATE_REPLAYS_SEQUENTIAL for
    ...
    Real m_groupMoveClickToGatherFactor; /** if you take all the selected units and calculate the smallest possible rectangle
                                                                             that contains them all, and click within that,
                                            all the selected units will break formation and gather at the point the user
                                            clicked (if the value is 1.0). If it's 0.0, units will always keep their
                                            formation. If it's <1.0, then the user must click a smaller area within the
    ...
    // TheSuperHackers @feature Mauller 21/06/2025 allow the system time and game time font size to be set, a size of zero
    // disables them
    ...
    Real m_prisonBountyMultiplier; ///< the cost of the unit is multiplied by this and given to the player when prisoners are
                                   ///< returned to the a prison with KINDOF_COLLECTS_PRISON_BOUNTY
    ...
    //	Int m_cheaterHasBeenSpiedIfMyLowestBitIsTrue; ///< says it all.. this lives near other "colors" cause it is
    //masquerading as one
    ...
    Int m_powerBarYellowRange; ///< Red if consumption exceeds production, yellow if consumption this close but under, green
                               ///< if further under
    ...
    UnsignedInt m_networkKeepAliveDelay; ///< The number of seconds between when the connections to each player send a
                                         ///< keep-alive packet.
    UnsignedInt m_networkRunAheadSlack; ///< The amount of slack in the run ahead value. This is the percentage of the
                                        ///< calculated run ahead that is added.
    UnsignedInt m_networkDisconnectTime; ///< The number of milliseconds between when the game gets stuck on a frame for a
                                         ///< network stall and when the disconnect dialog comes up.
    UnsignedInt m_networkPlayerTimeoutTime; ///< The number of milliseconds between when a player's last keep alive command
                                            ///< was recieved and when they are considered disconnected from the game.
    UnsignedInt
        m_networkDisconnectScreenNotifyTime; ///< The number of milliseconds between when the disconnect screen comes up and
                                             ///< when the other players are notified that we are on the disconnect screen.
