...
public:
	...
	Int m_powerBarYellowRange; ///< Red if consumption exceeds production, yellow if consumption this close but under, green if
														 ///< further under
	...
	UnsignedInt m_networkPlayerTimeoutTime; ///< The number of milliseconds between when a player's last keep alive command was
																					///< recieved and when they are considered disconnected from the game.
	UnsignedInt m_networkDisconnectScreenNotifyTime; ///< The number of milliseconds between when the disconnect screen comes
																									 ///< up and when the other players are notified that we are on the
																									 ///< disconnect screen.
