...
enum ObjectShroudStatus CPP_11( : Int){
    ...
    OBJECTSHROUD_INVALID_BUT_PREVIOUS_VALID, ///< indeterminate state, will recompute, BUT previous status is valid, don't
...
enum CommandSourceType CPP_11( : Int){
    ...
    CMD_FROM_SCRIPT,
    CMD_FROM_AI,
    CMD_FROM_DOZER, // Special rare command when the dozer originates a command to attack a mine. Mines are not
                    // ai-attackable, and it seems deceitful for the dozer to generate a player or script command. jba.
