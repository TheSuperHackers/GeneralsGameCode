...
class Player : public Snapshot
{
  ...
  Bool hasUpgradeInProduction(
      const UpgradeTemplate *upgradeTemplate); ///< does player have this upgrade in progress right now
  Upgrade *addUpgrade(
  ...
  */
  ...
  void applyBattlePlanBonusesForObject(
      Object *obj) const; // New object or converted object gaining our current battle plan bonuses.
  void removeBattlePlanBonusesForObject(Object *obj) const; // Object left team
  void applyBattlePlanBonusesForPlayerObjects(
      const BattlePlanBonuses *bonus); // Battle plan bonuses changing, so apply to all of our objects!
  ...
  */
  ...
  void addToPriorityBuildList(
      ...
      Coord3D *pos,
      Real angle); ///< Adds this to the build list.	 Used for factories placed instead of in build list.
  ...
  */
  ...
  ProductionChangeMap
      m_productionCostChanges; ///< Map to keep track of Faction specific discounts or penalties on prices of units
  ProductionChangeMap
      m_productionTimeChanges; ///< Map to keep track of Faction specific discounts or penalties on build times of units
  ...
  ScienceVec
      m_sciencesDisabled; ///< (SAVE) sciences that we are not permitted to purchase "yet". Controlled by mission scripts.
