...
enum LegalBuildCode CPP_11( : Int){
	LBC_OK = 0, LBC_RESTRICTED_TERRAIN,		 LBC_NOT_FLAT_ENOUGH, LBC_OBJECTS_IN_THE_WAY, LBC_NO_CLEAR_PATH,
	LBC_SHROUD, LBC_TOO_CLOSE_TO_SUPPLIES, LBC_GENERIC_FAILURE,
};
...
public:
	...
	enum LocalLegalToBuildOptions
	{
		...
		NO_OBJECT_OVERLAP = 0X00000004, ///< Can't overlap enemy objects, or locally controled objects that can't move out of the
																		///< way
		...
		FAIL_STEALTHED_WITHOUT_FEEDBACK = 0x00000080 ///< USE WITH IGNORE_STEALTHED except it will fail without BIB feedback
																								 ///< (when clicking to place).
	};
...
public:
	...
	virtual void buildObjectLineNow(
			Object *constructorObject,
			const ThingTemplate *what,
			const Coord3D *start,
			const Coord3D *end,
	...
	virtual LegalBuildCode isLocationLegalToBuild(
			const Coord3D *worldPos,
			const ThingTemplate *build,
	...
	/// query if we can build at this location
	virtual LegalBuildCode isLocationClearOfObjects(
			const Coord3D *worldPos,
			const ThingTemplate *build,
	...
	virtual TileBuildInfo *buildTiledLocations(
