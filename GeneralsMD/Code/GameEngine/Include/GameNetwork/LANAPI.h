...
static const Int m_lanMaxOptionsLength = MAX_PACKET_SIZE
		- (8 + (g_lanGameNameLength + 1) * 2 + 4 + (g_lanPlayerNameLength + 1) * 2 + (g_lanLoginNameLength + 1)
...
public:
	...
	virtual void RequestGameOptions(AsciiString gameOptions, Bool isPublic, UnsignedInt ip = 0) = 0; ///< Change the game
																																																	 ///< options
	...
	virtual void OnPlayerJoin(Int slot, UnicodeString playerName) = 0; ///< Someone else joined our game (host only; joiners
																																		 ///< get a slotlist)
	...
	virtual void OnChat(UnicodeString player, UnsignedInt ip, UnicodeString message, ChatType format) = 0; ///< Chat message
																																																				 ///< from someone
	...
	//	virtual void OnSlotList( ReturnType ret, LANGameInfo *theGame ) = 0;															///< Slotlist for a
	// game in setup
...
public:
	...
	virtual void OnPlayerJoin(Int slot, UnicodeString playerName); ///< Someone else joined our game (host only; joiners get a
																																 ///< slotlist)
	...
	virtual void OnChat(UnicodeString player, UnsignedInt ip, UnicodeString message, ChatType format); ///< Chat message from
																																																		 ///< someone
	...
	// virtual void OnSlotList( ReturnType ret, LANGameInfo *theGame );															///< Slotlist for a game in
	// setup
