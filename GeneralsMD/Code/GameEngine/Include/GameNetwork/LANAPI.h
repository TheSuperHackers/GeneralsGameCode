...
class LANAPIInterface : public SubsystemInterface
{
  ...
  virtual void RequestGameOptions(
  ...
  virtual void OnPlayerJoin(
      Int slot,
      UnicodeString playerName) = 0; ///< Someone else joined our game (host only; joiners get a slotlist)
...
class LANAPI : public LANAPIInterface
{
  ...
  virtual void OnPlayerJoin(
      Int slot,
      UnicodeString playerName); ///< Someone else joined our game (host only; joiners get a slotlist)
  ...
  // virtual void OnSlotList( ReturnType ret, LANGameInfo *theGame );															///< Slotlist for a game
  // in setup
