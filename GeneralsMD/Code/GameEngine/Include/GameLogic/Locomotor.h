...
//-------------------------------------------------------------------------------------------------
enum LocomotorAppearance CPP_11( : Int){ LOCO_LEGS_TWO, LOCO_WHEELS_FOUR, LOCO_TREADS, LOCO_HOVER, LOCO_THRUST, LOCO_WINGS,
																				 LOCO_CLIMBER, // human climber - backs down cliffs.
																				 LOCO_OTHER,		LOCO_MOTORCYCLE };
...
static const char *TheLocomotorAppearanceNames[] = { "TWO_LEGS", "FOUR_WHEELS", "TREADS", "HOVER",			"THRUST",
																										 "WINGS",		 "CLIMBER",			"OTHER",	"MOTORCYCLE",
...
static const char *TheLocomotorBehaviorZNames[] = { "NO_Z_MOTIVE_FORCE",
																										"SEA_LEVEL",
...
private:
	...
	*/
	...
	Real m_preferredHeightDamping; ///< how aggressively to adjust to preferred height: 1.0 = very much so, 0.1 = gradually,
																 ///< etc
	Real m_circlingRadius; ///< for flying things, the radius at which they circle their "maintain" destination. (pos = cw, neg
												 ///< = ccw, 0 = smallest possible)
...
public:
	...
	void locoUpdate_moveTowardsPosition(
			Object *obj,
	...
	*/
	...
	inline Real getPreferredHeight() const
	{
	...
	inline Real getAccelPitchLimit() const
	{
		return m_template->m_accelPitchLimit;
	} ///< Maximum amount we will pitch up or down under acceleration (including recoil.)
	inline Real getDecelPitchLimit() const
	{
		return m_template->m_decelPitchLimit;
	} ///< Maximum amount we will pitch down under deceleration (including recoil.)
	inline Real getBounceKick() const
	{
		return m_template->m_bounceKick;
	} ///< How much simulating rough terrain "bounces" a wheel up.
	inline Real getPitchStiffness() const
	{
		return m_template->m_pitchStiffness;
	...
	inline Real getPitchByZVelCoef() const
	{
		return m_template->m_pitchByZVelCoef;
	...
	inline Real getForwardAccelCoef() const
	{
		return m_template->m_forwardAccelCoef;
	} ///< How much we pitch in response to acceleration.
	inline Real getLateralAccelCoef() const
	{
		return m_template->m_lateralAccelCoef;
	} ///< How much we roll in response to acceleration.
	inline Real getUniformAxialDamping() const
	{
		return m_template->m_uniformAxialDamping;
	} ///< How much we roll in response to acceleration.
	...
	inline Real getRudderCorrectionDegree() const
	{
		return m_template->m_rudderCorrectionDegree;
	} ///< How much we roll in response to acceleration.
	inline Real getRudderCorrectionRate() const
	{
		return m_template->m_rudderCorrectionRate;
	} ///< How much we roll in response to acceleration.
	inline Real getElevatorCorrectionDegree() const
	{
		return m_template->m_elevatorCorrectionDegree;
	} ///< How much we roll in response to acceleration.
	inline Real getElevatorCorrectionRate() const
	{
		return m_template->m_elevatorCorrectionRate;
	} ///< How much we roll in response to acceleration.
	...
	{
		DEBUG_ASSERTCRASH(
				!(speed <= 0.0f && m_template->m_appearance == LOCO_THRUST),
				("THRUST locos may not have zero speeds!"));
		m_maxSpeed = speed;
	}
...
protected:
	void moveTowardsPositionLegs(
			Object *obj,
			PhysicsBehavior *physics,
			const Coord3D &goalPos,
			Real onPathDistToGoal,
			Real desiredSpeed);
	void moveTowardsPositionLegsWander(
			Object *obj,
			PhysicsBehavior *physics,
			const Coord3D &goalPos,
			Real onPathDistToGoal,
			Real desiredSpeed);
	void moveTowardsPositionClimb(
			Object *obj,
			PhysicsBehavior *physics,
			const Coord3D &goalPos,
			Real onPathDistToGoal,
			Real desiredSpeed);
	void moveTowardsPositionWheels(
			Object *obj,
			PhysicsBehavior *physics,
			const Coord3D &goalPos,
			Real onPathDistToGoal,
			Real desiredSpeed);
	void moveTowardsPositionTreads(
			Object *obj,
			PhysicsBehavior *physics,
			const Coord3D &goalPos,
			Real onPathDistToGoal,
			Real desiredSpeed);
	void moveTowardsPositionOther(
			Object *obj,
			PhysicsBehavior *physics,
			const Coord3D &goalPos,
			Real onPathDistToGoal,
			Real desiredSpeed);
	void moveTowardsPositionHover(
			Object *obj,
			PhysicsBehavior *physics,
			const Coord3D &goalPos,
			Real onPathDistToGoal,
			Real desiredSpeed);
	void moveTowardsPositionThrust(
			Object *obj,
			PhysicsBehavior *physics,
			const Coord3D &goalPos,
			Real onPathDistToGoal,
			Real desiredSpeed);
	void moveTowardsPositionWings(
			Object *obj,
			PhysicsBehavior *physics,
			const Coord3D &goalPos,
			Real onPathDistToGoal,
...
private:
	...
	inline void setFlag(LocoFlag f, Bool b)
	{
		if (b)
			m_flags |= (1 << f);
		else
