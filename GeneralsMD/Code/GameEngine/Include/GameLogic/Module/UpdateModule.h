...
//-------------------------------------------------------------------------------------------------
enum UpdateSleepTime CPP_11( : Int){ UPDATE_SLEEP_INVALID = 0,
                                     UPDATE_SLEEP_NONE = 1,
...
enum SleepyUpdatePhase CPP_11( : Int){ // reserve 2 bits for phase. this still leaves us 30 bits for frame counter,
...
{
  public:
  static void buildFieldParse(MultiIniFieldParse &p) { BehaviorModuleData::buildFieldParse(p); }
};
...
class UpdateModule : public BehaviorModule, public UpdateModuleInterface
{
  ...
  */
  ...
  UpdateSleepTime frameToSleepTime(
      UnsignedInt frame1,
  ...
  DisabledMaskType getDisabledTypesToProcess() const { return DISABLEDMASK_NONE; }
...
#define UPDATEMODULE_FRIEND_DECLARATOR virtual
  ...
  UPDATEMODULE_FRIEND_DECLARATOR UnsignedInt friend_getPriority() const { return m_nextCallFrameAndPhase; }

  UPDATEMODULE_FRIEND_DECLARATOR UnsignedInt friend_getNextCallFrame() const { return m_nextCallFrameAndPhase >> 2; }
  ...
  UPDATEMODULE_FRIEND_DECLARATOR Int friend_getIndexInLogic() const { return m_indexInLogic; }

  UPDATEMODULE_FRIEND_DECLARATOR void friend_setIndexInLogic(Int i) { m_indexInLogic = i; }

  UPDATEMODULE_FRIEND_DECLARATOR const Object *friend_getObject() const { return getObject(); }
};
inline UpdateModule::UpdateModule(Thing *thing, const ModuleData *moduleData) :
    BehaviorModule(thing, moduleData), m_indexInLogic(-1), m_nextCallFrameAndPhase(0)
{
  // nothing
}
...
{
  public:
  virtual void projectileLaunchAtObjectOrPosition(
      const Object *victim,
      ...
      const ParticleSystemTemplate *exhaustSysOverride) = 0; ///< launch the projectile at the given victim
  virtual void projectileFireAtObjectOrPosition(
      const Object *victim,
      const Coord3D *victimPos,
      const WeaponTemplate *detWeap,
      const ParticleSystemTemplate *exhaustSysOverride) = 0;
  ...
  virtual void setFramesTillCountermeasureDiversionOccurs(
      UnsignedInt frames) = 0; ///< Number of frames till missile diverts to countermeasures.
...
class DockUpdateInterface
{
  ...
  */
  ...
  virtual Bool action(
      Object *docker,
      Object *drone = NULL) = 0; ///< Perform your specific action on me.  Returning FALSE means there is nothing for you
                                 ///< to do so I should leave

  virtual void cancelDock(Object *docker) = 0; ///< Clear me from any reserved points, and if I was the reason you were
                                               ///< Busy, you aren't anymore.
  ...
  virtual void setDockCrippled(
      Bool setting) = 0; ///< Game Logic can set me as inoperative.  I get to decide what that means.

  virtual Bool isAllowPassthroughType() = 0; ///< Not all docks allow you to path through them in your AIDock machine

  virtual Bool isRallyPointAfterDockType() = 0; ///< A minority of docks want to give you a final command to their rally
                                                ///< point
};
...
{
  public:
  virtual Bool isExitBusy() const = 0; ///< Contain style exiters are getting the ability to space out exits, so ask this
                                       ///< before reserveDoor as a kind of no-commitment check.
  virtual ExitDoorType reserveDoorForExit(
      const ThingTemplate *objType,
      Object *specificObject) = 0; ///< All types can answer if they are free to exit or not, and you can ask about a
                                   ///< specific guy or just exit anything in general
  virtual void exitObjectViaDoor(
      Object *newObj,
      ExitDoorType exitDoor) = 0; ///< Here is the object for you to exit to the world in your own special way
  virtual void exitObjectByBudding(Object *newObj, Object *budHost) = 0; ///< puts new spawn on top of an existing one
  virtual void unreserveDoorForExit(
      ExitDoorType exitDoor) = 0; ///< if you get permission to exit, but then don't/can't call exitObjectViaDoor, you
                                  ///< should call this to "give up" your permission
