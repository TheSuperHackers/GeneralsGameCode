...
///////////////////////////////////////////////////////////////////////////////////////////////////
enum ProductionID CPP_11( : Int){PRODUCTIONID_INVALID = 0};

enum ProductionType CPP_11( : Int){PRODUCTION_INVALID = 0, PRODUCTION_UNIT, PRODUCTION_UPGRADE};
...
public:
    ...
    {
        return m_productionQuantityTotal - m_productionQuantityProduced;
...
protected:
    ...
    Int m_framesUnderConstruction; ///< counter for how many frames we've been under construction (incremented once per
                                   ///< update)
    Int m_productionQuantityTotal; ///< it is now possible to construct multiple units simultaneously.
    Int m_productionQuantityProduced; ///< And we need to allow pausing within an entry, so we keep track of number of
                                      ///< sub-successes
...
public:
    ...
    UnsignedInt m_constructionCompleteDuration; ///< in frames, how long we state in "construction complete" condition after
                                                ///< making something
    std::vector<QuantityModifier>
        m_quantityModifiers; ///< Quantity modifiers modify the number of specified object to created whenever produced.
...
public:
    ...
    // These functions keep track of the special power construction of a new building via a special power instead of standard
    // production interface. This was added for the sneak attack building functionality.
...
public:
    ...
    {
        return getProductionUpdateModuleData()->m_disabledTypesToProcess;
    }
    ...
    virtual UnsignedInt countUnitTypeInQueue(
        const ThingTemplate *unitType) const; ///< count number of units with matching unit type in the production queue
    ...
    // These functions keep track of the special power construction of a new building via a special power instead of standard
    // production interface. This was added for the sneak attack building functionality.
    virtual const CommandButton *getSpecialPowerConstructionCommandButton() const
    {
        return m_specialPowerConstructionCommandButton;
    }
    virtual void setSpecialPowerConstructionCommandButton(const CommandButton *commandButton)
    {
        m_specialPowerConstructionCommandButton = commandButton;
    }
...
protected:
    ...
    {
        UnsignedInt
            m_doorOpenedFrame; ///< for producer objects that have a door open/close animation when they make something
    ...
    const CommandButton *m_specialPowerConstructionCommandButton; ///< In a mode to construct a specific building via a
                                                                  ///< special power. (NO NEED TO SAVE DATA ON THIS FIELD)
