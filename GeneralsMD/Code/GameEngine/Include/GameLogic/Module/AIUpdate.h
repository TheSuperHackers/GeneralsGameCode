...
enum MoodActionAdjustment CPP_11( : Int){
    ...
    MAA_Affect_Range_Bitmask =
        (MAA_Affect_Range_IgnoreAll | MAA_Affect_Range_WaitForAttack | MAA_Affect_Range_Alert
...
protected:
    ...
    virtual void privateExecuteRailedTransport(
        CommandSourceType cmdSource); ///< execute next leg in railed transport sequence
...
public:
    ...
    {
        return TheGameLogic->getFrame() - m_pathTimestamp;
...
public:
    ...
    inline const Coord3D *friend_getGoalPathPosition(Int index) const
    {
        return getStateMachine()->getGoalPathPosition(index);
    }
...
private:
    ...
    CommandSourceType
        m_lastCommandSource; /**< Keep track of the source of the last command we got.
