...
enum LocomotorSetType CPP_11( : Int){
	...
	LOCOMOTORSET_NORMAL = 0,	 LOCOMOTORSET_NORMAL_UPGRADED, LOCOMOTORSET_FREEFALL, LOCOMOTORSET_WANDER, LOCOMOTORSET_PANIC,
...
static const char *TheLocomotorSetNames[] = { "SET_NORMAL", "SET_NORMAL_UPGRADED", "SET_FREEFALL",	 "SET_WANDER",
																							"SET_PANIC",	"SET_TAXIING",				 "SET_SUPERSONIC", "SET_SLUGGISH",
...
static const char *TheAutoAcquireEnemiesNames[] = { "YES", "STEALTHED", "NO", "NOTWHILEATTACKING", "ATTACK_BUILDINGS",
...
//-------------------------------------------------------------------------------------------------
enum MoodMatrixAction CPP_11( : Int){ MM_Action_Idle, MM_Action_Move, MM_Action_Attack, MM_Action_AttackMove };
...
enum MoodActionAdjustment CPP_11( : Int){
	...
	MAA_Affect_Range_Bitmask =
			(MAA_Affect_Range_IgnoreAll | MAA_Affect_Range_WaitForAttack | MAA_Affect_Range_Alert | MAA_Affect_Range_Aggressive)
};
...
public:
	...
	Bool m_forbidPlayerCommands; ///< Should isAllowedToRespondToAiCommands() filter out commands from the player, thus making
															 ///< it ai-controllable only?
...
enum AIFreeToExitType CPP_11( : Int) // Note - written out in save/load xfer, don't change these numbers.  jba.
		{ FREE_TO_EXIT = 0, NOT_FREE_TO_EXIT = 1, WAIT_TO_EXIT = 2 };
...
protected:
	...
	virtual void privateMoveToPosition(const Coord3D *pos, CommandSourceType cmdSource); ///< move to given position(s)
	...
	virtual void privateMoveToAndEvacuateAndExit(const Coord3D *pos, CommandSourceType cmdSource); ///< move to given position
																																																 ///< & unload transport.
	virtual void privateIdle(CommandSourceType cmdSource); ///< Enter idle state.
	virtual void privateTightenToPosition(const Coord3D *pos, CommandSourceType cmdSource); ///< move to given position(s)
																																													///< tightening the formation.
	virtual void privateFollowWaypointPath(const Waypoint *way, CommandSourceType cmdSource); ///< start following the path
																																														///< from the given point
	virtual void privateFollowWaypointPathAsTeam(const Waypoint *way, CommandSourceType cmdSource); ///< start following the
																																																	///< path from the given
																																																	///< point
	virtual void privateFollowWaypointPathExact(const Waypoint *way, CommandSourceType cmdSource); ///< start following the
																																																 ///< path from the given
																																																 ///< point
	virtual void privateFollowWaypointPathAsTeamExact(const Waypoint *way, CommandSourceType cmdSource); ///< start following
																																																			 ///< the path from the
																																																			 ///< given point
	virtual void privateFollowPath(
			const std::vector<Coord3D> *path,
			Object *ignoreObject,
			CommandSourceType cmdSource,
			Bool exitProduction); ///< follow the path defined by the given array of points
	...
	virtual void privateForceAttackObject(Object *victim, Int maxShotsToFire, CommandSourceType cmdSource); ///< attack given
																																																					///< object
	virtual void privateGuardRetaliate(
			Object *victim,
			const Coord3D *pos,
			Int maxShotsToFire,
			CommandSourceType cmdSource); ///< retaliate and attack attacker -- but with guard restrictions
	virtual void privateAttackTeam(const Team *team, Int maxShotsToFire, CommandSourceType cmdSource); ///< attack the given
																																																		 ///< team
	virtual void privateAttackPosition(const Coord3D *pos, Int maxShotsToFire, CommandSourceType cmdSource); ///< attack given
																																																					 ///< spot
	virtual void privateAttackMoveToPosition(
			const Coord3D *pos,
			Int maxShotsToFire,
			CommandSourceType cmdSource); ///< attack move to the given location
	virtual void privateAttackFollowWaypointPath(
			const Waypoint *way,
			Int maxShotsToFire,
			Bool asTeam,
			CommandSourceType cmdSource); ///< attack move along the following waypoint path, potentially as a team
	...
	virtual void privateReturnPrisoners(Object *prison, CommandSourceType cmdSource); ///< return picked up prisoners to the
																																										///< 'prison'
#endif
	...
	virtual void privateExitInstantly(Object *objectToExit, CommandSourceType cmdSource); ///< get out of this Object this
																																												///< frame
	virtual void privateEvacuate(Int exposeStealthUnits, CommandSourceType cmdSource); ///< empty its contents
	virtual void privateEvacuateInstantly(Int exposeStealthUnits, CommandSourceType cmdSource); ///< empty its contents this
																																															///< frame
	virtual void privateExecuteRailedTransport(CommandSourceType cmdSource); ///< execute next leg in railed transport sequence
	virtual void privateGoProne(const DamageInfo *damageInfo, CommandSourceType cmdSource); ///< life altering state change, if
																																													///< this AI can do it
	virtual void privateGuardTunnelNetwork(GuardMode guardMode, CommandSourceType cmdSource); ///< guard the given spot
	virtual void privateGuardPosition(const Coord3D *pos, GuardMode guardMode, CommandSourceType cmdSource); ///< guard the
																																																					 ///< given spot
	virtual void privateGuardObject(Object *objectToGuard, GuardMode guardMode, CommandSourceType cmdSource); ///< guard the
																																																						///< given object
	virtual void privateGuardArea(
			const PolygonTrigger *areaToGuard,
			GuardMode guardMode,
			CommandSourceType cmdSource); ///< guard the given area
	...
	virtual void privateCommandButtonPosition(
...
public:
	...
	virtual Object *getEnterTarget(); ///< if we are trying to enter something (see enter() above), what is the thing in
																		///< question? (or null)
	...
	*/
	...
	Bool isAircraftThatAdjustsDestination(void) const; ///< True if is aircraft that doesn't stack destinations (missles for
																										 ///< example do stack destinations.)
	...
	*/
	...
	// "Planning Mode" -----------------------------------------------------------------------------------
	Bool queueWaypoint(const Coord3D *pos); ///< add waypoint to end of move list. return true if success, false if queue was
																					///< full and those the waypoint not added
	...
	// Pathfinding ---------------------------------------------------------------------------------------
private:
	Bool computePath(PathfindServicesInterface *pathfinder, Coord3D *destination); ///< computes path to destination, returns
																																								 ///< false if no path
	Bool computeAttackPath(
			PathfindServicesInterface *pathfinder,
			const Object *victim,
			const Coord3D *victimPos); ///< computes
																 ///< path to
																 ///< attack
																 ///< the
																 ///< current
...
public:
	...
	void requestAttackPath(ObjectID victimID, const Coord3D *victimPos); ///< computes path to attack the current target,
	...
	Bool isQuickPathAvailable(const Coord3D *destination) const; ///< does a path (using quick pathfind) exist between us and
																															 ///< the destination
	Int getNumFramesBlocked(void) const { return m_blockedFrames; }
	Bool isBlockedAndStuck(void) const { return m_isBlockedAndStuck; }
	Bool canComputeQuickPath(void); ///< Returns true if we can quickly comput a path.  Usually missiles & the like that just
	...
	virtual Bool processCollision(PhysicsBehavior *physics, Object *other); ///< Returns true if the physics collide should
																																					///< apply the force.  Normally not.  jba.
...
public:
	...
	inline Object *getGoalObject()
	{
		return getStateMachine()->getGoalObject();
	} ///< return the id of the current state of the machine
	inline const Coord3D *getGoalPosition() const
	{
	...
	inline void clearMoveOutOfWay(void)
	{
...
private:
	...
	*/
	...
	Int m_surrenderedPlayerIndex; ///< if surrendered, the playerindex to whom we are surrendered, or -1 if available for
																///< everyone
#endif
	...
	Bool m_isFinalGoal; ///< True if this path ends at our destination (as opposed to an intermediate point on a waypoint
											///< path).
	...
	Bool m_randomlyOffsetMoodCheck; ///< If true, randomly offset the mood check rate next time, to avoid "spiking" of ai
																	///< checks
