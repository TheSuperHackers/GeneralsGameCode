...
public:
    ...
    {
        return UpdateModule::getInterfaceMask() | (MODULEINTERFACE_CONTAIN) | (MODULEINTERFACE_COLLIDE)
            | (MODULEINTERFACE_DIE) | (MODULEINTERFACE_DAMAGE);
    }
    ...
    virtual void orderAllPassengersToExit(
        ...
        Bool instantly); ///< All of the smarts of exiting are in the passenger's AIExit. removeAllFrommContain is a last
    ...
    virtual void markAllPassengersDetected(); ///< Cool game stuff got added to the system calls since this layer didn't
    ...
    virtual Bool isEnclosingContainerFor(
        const Object *obj) const; ///< Does this type of Contain Visibly enclose its contents?
    ...
    {
        m_passengerAllowedToFire = permission;
    ...
    {
        return FALSE;
    } ///< Contain style exiters are getting the ability to space out exits, so ask this before reserveDoor as a kind of
      ///< no-commitment check.
    ...
    virtual Bool getNaturalRallyPoint(Coord3D &rallyPoint, Bool offset = TRUE)
        const; ///< get the natural "rally point" for units to move towards
...
protected:
    virtual void monitorConditionChanges(
        void); ///< check to see if we need to update our occupant postions from a model change or anything else
    ...
    void removeFromContainViaIterator(
        ContainedItemsList::iterator it,
        Bool exposeStealthUnits = FALSE); ///< remove item from contain list
