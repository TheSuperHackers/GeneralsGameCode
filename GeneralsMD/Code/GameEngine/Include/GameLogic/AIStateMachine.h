...
enum AIStateType CPP_11( : Int){
  ...
  AI_BUSY, ///< This is a state that things will be in when they are busy doing random stuff that doesn't require AI
           ///< interaction.
...
class AIStateMachine : public StateMachine
{
  ...
  StateReturnType setTemporaryState(
      StateID newStateID,
      Int frameLimitCoount); ///< change the temporary state of the machine, and number of frames limit.
...
class AIAttackApproachTargetState : public AIInternalMoveToState
{
  ...
  Bool m_stopIfInRange; ///< If true, we check and stop as soon we can fire.  Used when we have to path to the object
                        ///< instead of to a firing position.
