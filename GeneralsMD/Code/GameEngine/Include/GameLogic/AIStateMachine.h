 ...
 */
enum AIStateType CPP_11( : Int){ AI_IDLE,
																 ...
																 AI_ATTACK_AND_FOLLOW_OBJECT, ///< attempt to kill GoalObject, following it if necessary (and
																															///< possible)
																 AI_DEAD,
																 ...
																 AI_ATTACKFOLLOW_WAYPOINT_PATH_AS_INDIVIDUALS, ///< Attack-Follow down a waypoint path as
																																							 ///< individuals
																 ...
																 AI_MOVE_AWAY_FROM_REPULSORS, ///< Civilians are running away from repulsors. (enemies or
																															///< dead civs, usually) jba
																 AI_WANDER_IN_PLACE, ///< Civilians just wander around a spot, rather than along a path.
																 AI_BUSY, ///< This is a state that things will be in when they are busy doing random stuff
																					///< that doesn't require AI interaction.
																 AI_EXIT_INSTANTLY, ///< exit this obj, without waiting -- do it in the onEnter! This frame!
																 AI_GUARD_RETALIATE, ///< attacks attacker but with restrictions (hybrid of attack and
																										 ///< guard).
...
public:
	...
	StateReturnType setTemporaryState(StateID newStateID, Int frameLimitCoount); ///< change the temporary state of the
																																							 ///< machine, and number of frames limit.
...
public:
	// Attack states.
	enum StateType CPP_11( : Int){ CHASE_TARGET, ///< Chase a moving target (optionally following it)
...
private:
	...
	Bool m_stopIfInRange; ///< If true, we check and stop as soon we can fire.  Used when we have to path to the object instead
												///< of to a firing position.
	Bool m_isInitialApproach; ///< If true, we can attack other units along the way. We will check for them every N frames (N
														///< specified in AI.ini)
...
public:
	AIPickUpCrateState(StateMachine *machine) : AIInternalMoveToState(machine, "AIAttackPickUpCrateState")
	{
		// we're setting m_isInitialApproach to true in the constructor because we want the first pass
		// through this state to allow a unit to attack incidental targets (if it is turreted)
	}
...
public:
	AIFollowWaypointPathState(StateMachine *machine, Bool asGroup) :
			m_isFollowWaypointPathState(true), m_moveAsGroup(asGroup), AIInternalMoveToState(machine, "AIFollowWaypointPathState")
	{
		m_angle = 0.0f;
	}
...
public:
	AIFollowWaypointPathExactState(StateMachine *machine, Bool asGroup) :
			m_moveAsGroup(asGroup), m_lastWaypoint(NULL), AIInternalMoveToState(machine, "AIFollowWaypointPathExactState")
	{
	}
	virtual StateReturnType onEnter();
	virtual void onExit(StateExitType status);
...
public:
	AIFollowPathState(StateMachine *machine, AsciiString name = "AIFollowPathState") :
			m_adjustFinal(true), m_adjustFinalOverride(false), m_index(0), m_retryCount(10), AIInternalMoveToState(machine, name)
	{
	}
	virtual StateReturnType onEnter();
	virtual void onExit(StateExitType status);
...
public:
	AIAttackFireWeaponState(StateMachine *machine, NotifyWeaponFiredInterface *att) :
			State(machine, "AIAttackFireWeaponState"), m_att(att)
	{
	}
	virtual Bool isAttack() const { return TRUE; }
	virtual StateReturnType update();
...
public:
	AIGuardState(StateMachine *machine) : State(machine, "AIGuardState"), m_guardMachine(NULL) { m_guardMachine = NULL; }
	//~AIGuardState();
...
public:
	AIHuntState(StateMachine *machine) : State(machine, "AIHuntState"), m_huntMachine(NULL) { m_nextEnemyScanTime = 0; }
	//~AIHuntState();
	virtual Bool isAttack() const;
	virtual StateReturnType onEnter();
	virtual void onExit(StateExitType status);
...
public:
	AIAttackAreaState(StateMachine *machine) :
			State(machine, "AIAttackAreaState"), m_attackMachine(NULL), m_nextEnemyScanTime(0)
	{
	}
	//~AIAttackAreaState();
