...
public:
    ...
    void optimize(
        ...
        Bool blocked); ///< Optimize the path to discard redundant nodes
...
public:
    ...
    Bool setTypeAsObstacle(
        Object *obstacle,
        Bool isFence,
        const ICoord2D &pos); ///< flag this cell as an obstacle, from the given one
    ...
    Bool isObstaclePresent(
        ObjectID objID) const; ///< return true if the given object ID is registered as an obstacle in this cell
...
public:
    ...
    void blockCalculateZones(
        PathfindCell **map,
        PathfindLayer layers[],
        const IRegion2D &bounds); ///< Does zone calculations.
...
public:
    ...
    Bool clientSafeQuickDoesPathExist(
        const LocomotorSet &locomotorSet,
        const Coord3D *from,
        const Coord3D *to); ///< Can we build any path at all between the locations	(terrain & buildings check - fast)
    ...
    void updateGoal(
        ...
        PathfindLayerEnum layer); ///< Update the given mobile unit's cells in the map
    ...
    PathfindCell *getClippedCell(
        PathfindLayerEnum layer,
        const Coord3D *pos); ///< Given a position, return associated grid cell
    ...
    Bool isViewBlockedByObstacle(
        const Object *obj,
        const Object *objOther); ///< Return true if the straight line between the given points contains any obstacle, and
                                 ///< thus blocks vision
    ...
    Int clearCellForDiameter(
        Bool crusher,
        Int cellX,
        Int cellY,
        PathfindLayerEnum layer,
        Int pathDiameter); ///< Return true if given position is a valid movement location
...
protected:
    ...
    */
    ...
    void classifyObjectFootprint(
        Object *obj,
        Bool insert); /** Classify the cells under the given object
                                                                                      If 'insert' is true, object is being
                         added If 'insert' is false, object is being removed */
    void internal_classifyObjectFootprint(
        Object *obj,
        Bool
            insert); /** Classify the cells under the given object
    ...
    void classifyUnitFootprint(
        Object *obj,
        Bool insert,
        Bool remove,
        Bool update); /** Classify the cells under the given object If 'insert' is true, object is being added */
    ...
    Path *buildHierachicalPath(
        const Coord3D *fromPos,
        PathfindCell *goalCell); ///< Work backwards from goal cell to construct final path

    void prependCells(
        Path *path,
        const Coord3D *fromPos,
        PathfindCell *goalCell,
        Bool center); ///< Add pathfind cells to a path.
