...
public:
	...
	void setMinVeterancyLevel(VeterancyLevel newLevel, Bool provideFeedback = TRUE); ///< Set Level to AT LEAST this... if we
																																									 ///< are already >= this level, do
																																									 ///< nothing.
	void addExperiencePoints(Int experienceGain, Bool canScaleForBonus = TRUE); ///< Gain this many exp.
	Bool gainExpForLevel(Int levelsToGain, Bool canScaleForBonus = TRUE); ///< Gain enough exp to gain a level. return false if
																																				///< can't gain a level.
