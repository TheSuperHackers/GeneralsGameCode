...
public:
	...
	*/
	...
	Bool attemptHealingFromSoleBenefactor(Real amount, const Object *source, UnsignedInt duration); ///< for the non-stacking
																																																	///< healers like ambulance
																																																	///< and propaganda
	ObjectID getSoleHealingBenefactor(void) const;

	Real estimateDamage(DamageInfoInput &damageInfo) const;
	void kill(DamageType damageType = DAMAGE_UNRESISTABLE, DeathType deathType = DEATH_NORMAL); ///< kill the object with an
	...
	void doTempWeaponBonus(WeaponBonusConditionType status, UnsignedInt duration); ///< At this level, we just pass this on to
																																								 ///< our helper

	void scoreTheKill(const Object *victim); ///< I just killed this object.
	void onVeterancyLevelChanged(VeterancyLevel oldLevel, VeterancyLevel newLevel, Bool provideFeedback = TRUE); ///< I just
																																																							 ///< achieved
																																																							 ///< this
																																																							 ///< level
																																																							 ///< right
	...
	Bool affectedByUpgrade(const UpgradeTemplate *upgradeT) const; ///< can the object even "have" this upgrade, will it do
																																 ///< something?
	...
	*/
	...
	Bool hasAnyDamageWeapon() const; // Kris: a should be used for real weapons that directly inflict damage... not deploy,
																	 // hack, etc.
...
private:
	...
	enum ObjectPrivateStatusBits
	{
		...
		UNDETECTED_DEFECTOR = (1 << 1), ///< set to true when I defect from my team; set to false when I attack anything or when
																		///< time runs out
		CAPTURED = (1 << 2), ///< set to true if I've been captured, otherwise, its false. (Note: Never becomes false once it's
												 ///< true)
	...
	PhysicsBehavior *m_physics; ///< physics interface (if any), cached for handy access. (duplicate of entry in the module
															///< array!)
