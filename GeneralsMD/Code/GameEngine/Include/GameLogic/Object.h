...
enum CrushSquishTestType CPP_11( : Int){TEST_CRUSH_ONLY, TEST_SQUISH_ONLY, TEST_CRUSH_OR_SQUISH};
...
public:
    ...
    */
    ...
    {
        return m_status.test(OBJECT_STATUS_DESTROYED);
    } ///< Returns TRUE if object has been destroyed
    Bool isAirborneTarget() const
    {
        return m_status.test(OBJECT_STATUS_AIRBORNE_TARGET);
    ...
    Bool attemptHealingFromSoleBenefactor(
        Real amount,
        const Object *source,
        UnsignedInt duration); ///< for the non-stacking healers like ambulance and propaganda
    ObjectID getSoleHealingBenefactor(void) const;

    Real estimateDamage(DamageInfoInput &damageInfo) const;
    void kill(
        DamageType damageType = DAMAGE_UNRESISTABLE,
        DeathType deathType = DEATH_NORMAL); ///< kill the object with an optional type of damage and death.
    ...
    void doTempWeaponBonus(
        WeaponBonusConditionType status,
        UnsignedInt duration); ///< At this level, we just pass this on to our helper

    void scoreTheKill(const Object *victim); ///< I just killed this object.
    void onVeterancyLevelChanged(
        VeterancyLevel oldLevel,
        VeterancyLevel newLevel,
        Bool provideFeedback = TRUE); ///< I just achieved this level right this moment
    ...
    SpecialPowerUpdateInterface *findSpecialPowerWithOverridableDestinationActive(
        SpecialPowerType type = SPECIAL_INVALID) const;
    ...
    Bool affectedByUpgrade(
        const UpgradeTemplate *upgradeT) const; ///< can the object even "have" this upgrade, will it do something?
    ...
    void onPartitionCellChange(); ///< We have moved a 'significant' amount, so do maintenence that can be considered
                                  ///< 'cell-based'
    void handlePartitionCellMaintenance(); ///< Undo and redo all shroud actions.  Call when something has changed, like
                                           ///< position or ownership or Death
    ...
    void doSpecialPower(
        ...
        Bool forced = false); ///< execute power
    void doSpecialPowerAtObject(
        const SpecialPowerTemplate *specialPowerTemplate,
        Object *obj,
        UnsignedInt commandOptions,
        Bool forced = false); ///< execute power
    void doSpecialPowerAtLocation(
        const SpecialPowerTemplate *specialPowerTemplate,
        const Coord3D *loc,
        Real angle,
        UnsignedInt commandOptions,
    ...
    */
    ...
    Bool hasAnyDamageWeapon()
        const; // Kris: a should be used for real weapons that directly inflict damage... not deploy, hack, etc.
    ...
    {
        return m_weaponSet.getNthCommandSourceMask(wSlot);
    }
    ...
    */
    ...
    Int getMultiLogicalBonePosition(
    ...
    inline UnsignedInt getSafeOcclusionFrame(void)
    {
        return m_safeOcclusionFrame;
    } //< this is an object specific frame at which it's safe to enable building occlusion.
...
private:
    ...
    enum ObjectPrivateStatusBits
    {
        ...
        UNDETECTED_DEFECTOR =
            (1 << 1), ///< set to true when I defect from my team; set to false when I attack anything or when time runs out
        CAPTURED =
            (1
             << 2), ///< set to true if I've been captured, otherwise, its false. (Note: Never becomes false once it's true)
    ...
    SightingInfo
        *m_partitionLastThreat; ///< Where and for whom I last delt with threat, so I can undo its effects when I stop
    ...
    PhysicsBehavior
        *m_physics; ///< physics interface (if any), cached for handy access. (duplicate of entry in the module array!)
    ...
    UnsignedInt m_soleHealingBenefactorExpirationFrame; ///< on what frame can I accept healing (thus to switch) from a new
                                                        ///< benefactor
    ///////////////////////////////////
    ...
    UnsignedInt m_safeOcclusionFrame; ///< flag used by occlusion renderer so it knows when objects have exited their
                                      ///< production building.
