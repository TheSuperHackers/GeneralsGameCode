...
enum WeaponAffectsMaskType CPP_11( : Int){
  ...
  WEAPON_DOESNT_AFFECT_AIRBORNE =
      0x40, // Radius damage doesn't affect airborne units, unless they are the primary target. (used for poison fields.)
};
...
enum WeaponCollideMaskType CPP_11( : Int){
  ...
  WEAPON_COLLIDE_STRUCTURES =
      0x0004, // this is "all structures EXCEPT for structures belonging to the projectile's controller".
...
class WeaponTemplate : public MemoryPoolObject
{
  ...
  */
  ...
  std::vector<Coord2D>
      m_scatterTargets; ///< instead of pure randomness, this is the list of places I will randomly choose from to attack
  ...
  Int m_shotsPerBarrel; ///< If non zero, don't cycle through your launch points every shot, mod the shot by this to get
                        ///< even chucks of firing
  ...
  Bool m_leechRangeWeapon; ///< once the weapon has fired once at the proper range, the weapon gains unlimited range for
                           ///< the remainder of the attack cycle
...
class Weapon : public MemoryPoolObject, public Snapshot
{
  ...
  */
  ...
  /// Some AI needs to know when the soonest I can possibly next shoot.  Bonuses have been figured into this number
  /// already.
  ...
  */
  ...
  void processRequestAssistance(
      const Object *requestingObject,
      Object *victimObject); ///< Weapons can call for extra attacks from nearby objects
  ...
  std::vector<Int>
      m_scatterTargetsUnused; ///< A running memory of which targets I've used, so I can shoot them all at random
  Bool m_pitchLimited;
