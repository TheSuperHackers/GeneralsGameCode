...
//-------------------------------------------------------------------------------------------------
enum WeaponReloadType CPP_11( : Int){ AUTO_RELOAD, NO_RELOAD, RETURN_TO_BASE_TO_RELOAD };

#ifdef DEFINE_WEAPONRELOAD_NAMES
static const char *TheWeaponReloadNames[] = { "YES", "NO", "RETURN_TO_BASE", NULL };
#endif

//-------------------------------------------------------------------------------------------------
enum WeaponPrefireType CPP_11( : Int){ PREFIRE_PER_SHOT, ///< Use the prefire delay for every shot we make
...
static const char *TheWeaponPrefireNames[] = { "PER_SHOT", "PER_ATTACK", "PER_CLIP", NULL };
#endif
...
enum WeaponAffectsMaskType CPP_11( : Int){
	...
	WEAPON_DOESNT_AFFECT_SIMILAR = 0x20, // Doesn't affect others that are the same as us (like other terrorists for a
																			 // terrorist bomb to prevent chain reaction)
	WEAPON_DOESNT_AFFECT_AIRBORNE = 0x40, // Radius damage doesn't affect airborne units, unless they are the primary target.
																				// (used for poison fields.)
};

// #ifdef DEFINE_WEAPONAFFECTSMASK_NAMES ; Removed protection so other clases can use these strings... not sure why this was
// protected in the 1st place
static const char *TheWeaponAffectsMaskNames[] = { "SELF",		"ALLIES",			 "ENEMIES",			 "NEUTRALS",
																									 "SUICIDE", "NOT_SIMILAR", "NOT_AIRBORNE", NULL };
// #endif
...
enum WeaponCollideMaskType CPP_11( : Int){
	...
	WEAPON_COLLIDE_STRUCTURES = 0x0004, // this is "all structures EXCEPT for structures belonging to the projectile's
	// controller".
...
static const char *TheWeaponCollideMaskNames[] = { "ALLIES",
																									 "ENEMIES",
																									 "STRUCTURES",
																									 "SHRUBBERY",
																									 "PROJECTILES",
...
// existing values!
//
enum WeaponBonusConditionType CPP_11( : Int){ // The access and use of this enum has the bit shifting built in, so this is a
																							// 0,1,2,3,4,5 enum
																							...
																							WEAPONBONUSCONDITION_FANATICISM, // FOR THE NEW GC INFANTRY GENERAL... adds to
																																							 // nationalism
...
static const char *TheWeaponBonusFieldNames[] = { "DAMAGE", "RADIUS", "RANGE", "RATE_OF_FIRE", "PRE_ATTACK", NULL };
#endif
...
struct HistoricWeaponDamageInfo
{
	...
	HistoricWeaponDamageInfo(UnsignedInt f, const Coord3D &l) : frame(f), location(l) {}
};
...
public:
	...
	*/
	...
	inline const ObjectCreationList *getProjectileDetonationOCL(VeterancyLevel v) const
	{
		return m_projectileDetonationOCLs[v];
	}
...
private:
	...
	std::vector<Coord2D> m_scatterTargets; ///< instead of pure randomness, this is the list of places I will randomly choose
																				 ///< from to attack
	...
	const ObjectCreationList *m_projectileDetonationOCLs[LEVEL_COUNT]; ///< Post-loaded lookup of name string for ease and
																																		 ///< speed (and subsystem init order)
	...
	UnsignedInt m_continuousFireCoastFrames; ///< How long after we should have shot should we start to wind down from
	...
	Int m_shotsPerBarrel; ///< If non zero, don't cycle through your launch points every shot, mod the shot by this to get even
												///< chucks of firing
	...
	Bool m_damageDealtAtSelfPosition; ///< if T, weapon damage is done at source's position, not victim's pos. (useful for
																		///< suicide weapons.)
	...
	UnsignedInt m_historicBonusTime; ///< if 'count' instances of this weapon do damage within 'time' and 'radius' from each
																	 ///< other, fire the historic bonus weapon
	...
	Bool m_leechRangeWeapon; ///< once the weapon has fired once at the proper range, the weapon gains unlimited range for the
													 ///< remainder of the attack cycle
	...
	Int m_preAttackDelay; ///< doesn't attack until preAttack delay is finish (triggering detonation, aiming a snipe shot,
												///< etc.)
	Real m_continueAttackRange; ///< if nonzero: when you destroy something, look for a similar obj controlled by same player
															///< to attack (used mainly for mine-clearing)
	Real m_infantryInaccuracyDist; ///< When this weapon is used against infantry, it can randomly miss by as much as this
																 ///< distance.
...
public:
	...
	void fireProjectileDetonationWeapon(
			const Object *source,
			Object *target,
	...
	*/
	...
	Real estimateWeaponDamage(const Object *source, const Coord3D *pos) { return estimateWeaponDamage(source, NULL, pos); }

	void onWeaponBonusChange(const Object *source); ///< Our Object's weapon bonus changed, so we need to update to reflect
	...
	Bool isGoalPosWithinAttackRange(
			const Object *source,
	...
	Bool isSourceObjectWithGoalPositionWithinAttackRange(
			const Object *source,
			const Coord3D *goalPos,
			const Object *target,
			const Coord3D *targetPos) const;
	...
	*/
	...
	void newProjectileFired(
			const Object *sourceObj,
			const Object *projectile,
			const Object *victimObj,
			const Coord3D *victimPos); ///< I just made this projectile and may need to keep track of it
...
protected:
	...
	void processRequestAssistance(const Object *requestingObject, Object *victimObject); ///< Weapons can call for extra
																																											 ///< attacks from nearby objects
...
private:
	...
	UnsignedInt m_whenLastReloadStarted; ///< the frame the current reload/between-shots began. (only valid if status is
																			 ///< RELOADING_CLIP or BETWEEN_FIRING_SHOTS)
	...
	std::vector<Int> m_scatterTargetsUnused; ///< A running memory of which targets I've used, so I can shoot them all at
																					 ///< random
	Bool m_pitchLimited;
...
public:
	...
	*/
	...
	void handleProjectileDetonation(
			const WeaponTemplate *w,
			const Object *source,
			const Coord3D *pos,
			WeaponBonusConditionFlags extraBonusFlags,
			Bool inflictDamage = TRUE);
