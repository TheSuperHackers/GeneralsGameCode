...
static const char *TheWeaponSlotTypeNames[] = { "PRIMARY",
																								"SECONDARY",
																								"TERTIARY",
...
//-------------------------------------------------------------------------------------------------
enum WeaponSetConditionType CPP_11( : Int){ WSF_INVALID = -1,

																						WSF_NONE = 0,			WSF_FIRING, WSF_BETWEEN, WSF_RELOADING, WSF_PREATTACK,
...
public:
	inline WeaponTemplateSet() { clear(); }
...
public:
	...
	{
		return (m_totalDamageTypeMask.test(typeToDeal) && (m_totalDamageTypeMask.count() == 1));
	}
	...
	inline UnsignedInt getNthCommandSourceMask(WeaponSlotType n) const
	{
		return m_curWeaponTemplateSet ? m_curWeaponTemplateSet->getNthCommandSourceMask(n) : NULL;
	}
	...
	*/
	...
	// When calling this function, all conditions must be validated to the point where we have decided that we wish to attack
	// the object (faction checks, etc). Now, we are determining if the attack itself is able to be performed!
	...
	*/
	Bool chooseBestWeaponForTarget(
			const Object *obj,
			const Object *victim,
