...
class TAiData : public Snapshot
{
  ...
  */
  ...
  Real m_skirmishBaseDefenseExtraDistance; ///< instead of building base defenses right on the template bounding circle,
                                           ///< push them out this much.
  ...
  Bool m_attackUsesLineOfSight; // If true, attack for units with KINDOF_ATTACK_NEEDS_LINE_OF_SIGHT  uses line of sight.
                                // jba.

  Bool m_attackIgnoreInsignificantBuildings; // If true, attack for ALL UNITS ignores buildings that are hostile that are
                                             // not significant. jkmcd
...
class AI : public SubsystemInterface, public Snapshot
{
  ...
  static void parseSideInfo(
      INI *ini,
      void *instance,
      void *store,
      const void *userData); ///< Parse the image part of the INI file
  static void parseSkirmishBuildList(
      INI *ini,
      void *instance,
      void *store,
      const void *userData); ///< Parse the image part of the INI file
  static void parseStructure(
      INI *ini,
      void *instance,
      void *store,
      const void *userData); ///< Parse the image part of the INI file
  static void parseSkillSet(
      INI *ini,
      void *instance,
      void *store,
      const void *userData); ///< Parse the image part of the INI file
  static void parseScience(
      INI *ini,
      void *instance,
      void *store,
      const void *userData); ///< Parse the image part of the INI file
...
enum AttitudeType CPP_11( : Int){
  ATTITUDE_SLEEP = -2, ATTITUDE_PASSIVE = -1,   ATTITUDE_NORMAL = 0,
  ATTITUDE_ALERT = 1,  ATTITUDE_AGGRESSIVE = 2, ATTITUDE_INVALID = 3
...
class AIGroup : public MemoryPoolObject, public Snapshot
{
  ...
  void groupMoveToAndEvacuateAndExit(
      const Coord3D *pos,
  ...
  void groupTightenToPosition(
      const Coord3D *pos,
      Bool addWaypoint,
      CommandSourceType cmdSource); ///< move to given position(s)
  void groupFollowWaypointPath(
      const Waypoint *way,
      CommandSourceType cmdSource); ///< start following the path from the given point
  void groupFollowWaypointPathAsTeam(
      const Waypoint *way,
      CommandSourceType cmdSource); ///< start following the path from the given point
  void groupFollowWaypointPathExact(
      const Waypoint *way,
      CommandSourceType cmdSource); ///< start following the path from the given point
  void groupFollowWaypointPathAsTeamExact(
      const Waypoint *way,
      CommandSourceType cmdSource); ///< start following the path from the given point
  void groupFollowPath(
      const std::vector<Coord3D> *path,
      Object *ignoreObject,
      CommandSourceType cmdSource); ///< follow the path defined by the given array of points
  ...
  void groupAttackMoveToPosition(
      const Coord3D *pos,
      Int maxShotsToFire,
      CommandSourceType cmdSource); ///< Attack move to the location
  ...
  void groupGuardArea(
      ...
      GuardMode guardMode,
      CommandSourceType cmdSource); ///< guard an area
