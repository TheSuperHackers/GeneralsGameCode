...
public:
    ...
    ScriptGroup *duplicateAndQualify(
        const AsciiString &qualifier,
        const AsciiString &playerTemplateName,
        const AsciiString &newPlayerName) const; // note, duplicates just this node, not the full list.
...
public:
    ...
    OrCondition *duplicateAndQualify(
        const AsciiString &qualifier,
        const AsciiString &playerTemplateName,
...
public:
    enum ScriptActionType
    {
        ...
        CAMERA_MOD_SET_ROLLING_AVERAGE, ///< Sets the number of frames to average changes (angle, positoin) to smooth out a
                                        ///< camera movement.
        ...
        TEAM_STOP_AND_DISBAND, ///< Make a team stop and be merged into the default player team. Also, mark the whole team
                               ///< recruitable
        ...
        NAMED_FIRE_WEAPON_FOLLOWING_WAYPOINT_PATH, ///< Fires a specific weapon that follows waypoint path and detonates at
                                                   ///< the final waypoint.
        ...
        SKIRMISH_FIRE_SPECIAL_POWER_AT_MOST_COST, ///< Fire a power or superweapon at the enemy's most costly concentration
                                                  ///< of structures & units..
        ...
        SKIRMISH_PERFORM_COMMANDBUTTON_ON_MOST_VALUABLE_OBJECT, ///< Send a single unit/ all available to perform a command
                                                                ///< button action on the most valuable target
        SKIRMISH_WAIT_FOR_COMMANDBUTTON_AVAILABLE_ALL, ///< In a sequential script, stay on this step until the specified
                                                       ///< command button is ready for all units.
        SKIRMISH_WAIT_FOR_COMMANDBUTTON_AVAILABLE_PARTIAL, ///< In a sequential script, stay on this step until the specified
                                                           ///< command button is ready for one unit on the team.
        ...
        TEAM_ALL_USE_COMMANDBUTTON_ON_NEAREST_ENEMY_UNIT, ///< Tell the team to use the commandbutton on the nearest enemy
                                                          ///< unit
        TEAM_ALL_USE_COMMANDBUTTON_ON_NEAREST_GARRISONED_BUILDING, ///< Tell the team to use the commandbutton on the nearest
                                                                   ///< enemy garrisoned building
        TEAM_ALL_USE_COMMANDBUTTON_ON_NEAREST_KINDOF, ///< Tell the team to use the commandbutton on the nearest enemy that
                                                      ///< has kindof set
        TEAM_ALL_USE_COMMANDBUTTON_ON_NEAREST_ENEMY_BUILDING, ///< Tell the team to use the commandbutton on the nearest
                                                              ///< enemy building
        TEAM_ALL_USE_COMMANDBUTTON_ON_NEAREST_ENEMY_BUILDING_CLASS, ///< Tell the team to use the commandbutton on the
                                                                    ///< nearest enemy building class
        TEAM_ALL_USE_COMMANDBUTTON_ON_NEAREST_OBJECTTYPE, ///< Tell the team to use the commandbutton on the nearest enemy
                                                          ///< object of type
        TEAM_PARTIAL_USE_COMMANDBUTTON, ///< Tell some percentage of the team to use the commandbutton on
        TEAM_CAPTURE_NEAREST_UNOWNED_FACTION_UNIT, ///< Tell the team to capture the nearest unowned faction unit (unit whose
                                                   ///< pilot has been killed by Jarmen Kell)
        PLAYER_CREATE_TEAM_FROM_CAPTURED_UNITS, ///< Tell the player to create a new named team from all the units who have
                                                ///< been captured.
        ...
        TEAM_WAIT_FOR_NOT_CONTAINED_ALL, ///< In a sequential script, stay on this step until the entire team is no longer
                                         ///< contained
        TEAM_WAIT_FOR_NOT_CONTAINED_PARTIAL, ///< In a sequential script, stay on this step until at least one member of the
                                             ///< team is no longer contained
        ...
        PLAYER_EXCLUDE_FROM_SCORE_SCREEN, ///< This player should be listed in the score screen.  Should only be used in
                                          ///< campaign games.
        ...
        NAMED_SET_EVAC_LEFT_OR_RIGHT, ///< LORENZEN -- Which side of the garrisoned unit (LIKELY A TRAIN) do you want units
                                      ///< to evacuate from?
        ...
        NAMED_USE_COMMANDBUTTON_ABILITY_USING_WAYPOINT_PATH, ///< Added for particle cannon to have beam follow waypoint
                                                             ///< path.
    ...
    ScriptAction *duplicateAndQualify(
        const AsciiString &qualifier,
        const AsciiString &playerTemplateName,
        const AsciiString &newPlayerName) const;
...
protected: // Note - If you add any member vars, you must take them into account in duplicate() and updateFrom(), as well as
           // file read/write.
    // snapshot methods
    virtual void crc(Xfer *xfer);
    virtual void xfer(Xfer *xfer);
...
public:
    ...
    Parameter(ParameterType type, int val = 0) : m_initialized(false), m_paramType(type), m_int(val), m_real(0)
    {
        m_coord.x = 0;
        m_coord.y = 0;
        m_coord.z = 0;
...
public:
    enum ConditionType
    {
        ...
        SKIRMISH_PLAYER_HAS_COMPARISON_GARRISONED, // True if there are comparison to garrisoned buildings (by player) on the
                                                   // map
...
public:
    ...
    ScriptList *duplicateAndQualify(
        const AsciiString &qualifier,
        const AsciiString &playerTemplateName,
        const AsciiString &newPlayerName) const;
