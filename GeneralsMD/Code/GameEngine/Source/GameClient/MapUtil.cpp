...
// Bool MapCache::addMap( AsciiString dirName, AsciiString fname, WinTimeStamp timestamp, UnsignedInt filesize, Bool
// isOfficial )
    ...
    while (numMapsListed < TheMapCache->size())
    {
        ...
        while (tempit != tempCache.end())
        {
            ...
            */
            ...
            if (it->first.startsWithNoCase(mapDir.str()) && isMultiplayer == it->second.m_isMultiplayer
...
Image *getMapPreviewImage(AsciiString mapName)
{
    ...
    TextureClass::MIP_LEVELS_1 ); uv.lo.x = 0.0f; uv.lo.y = 1.0f; uv.hi.x = 1.0f; uv.hi.y = 0.0f; mapPreviewImage->setStatus(
    IMAGE_STATUS_RAW_TEXTURE ); mapPreviewImage->setRawTextureData( texture ); mapPreviewImage->setUV( &uv );
