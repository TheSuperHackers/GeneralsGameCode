            ...
            if (build && TheInGameUI->isPlacementAnchored())
            {
                ...
                lbc = TheBuildAssistant->isLocationLegalToBuild(
                    ...
                    BuildAssistant::USE_QUICK_PATHFIND | BuildAssistant::TERRAIN_RESTRICTIONS | BuildAssistant::CLEAR_PATH
                        ...
                        | BuildAssistant::IGNORE_STEALTHED | BuildAssistant::FAIL_STEALTHED_WITHOUT_FEEDBACK,
                    builderObj,
                    NULL);
                if (lbc == LBC_OK)
                {
                    ...
                    if (builderObj)
                    {
                        ...
                        if (puInterface)
                        {
                            ...
                            if (commandButton)
                            {
                                // If we get this far, then we aren't going to really build the object using the production
                                // update interface. Instead, we're going to trigger the special power to create it magically
                                // without a dozer/worker.
                                placeMsg = TheMessageStream->appendMessage(GameMessage::MSG_DO_SPECIAL_POWER_AT_LOCATION);
                                placeMsg->appendIntegerArgument(commandButton->getSpecialPowerTemplate()
                                                                    ->getID()); // The ID of the special power template.
                                ...
                                placeMsg->appendObjectIDArgument(
                                    builderObj->getID()); // The source object responsible for firing the special.
