...
GameMessageDisposition LookAtTranslator::translateGameMessage(const GameMessage *msg)
{
	...
	switch (t)
	{
		...
		case GameMessage::MSG_FRAME_TICK:
		{
				...
				if (m_isScrolling)
				{
					...
					switch (m_scrollType)
					{
						case SCROLL_RMB:
						{
							...
							// TheSuperHackers @fix Mauller 16/06/2025 fix RMB scrolling to allow it to scale with the user adjusted scroll
							// factor
							Coord2D vec;
							vec.x = (m_currentPos.x - m_anchor.x);
							vec.y = (m_currentPos.y - m_anchor.y);
							// TheSuperHackers @info calculate the length of the vector to obtain the movement speed before the vector is
							// normalized
							float vecLength = vec.length();
							vec.normalize();
							offset.x = TheGlobalData->m_horizontalScrollSpeedFactor * fpsRatio * vecLength * vec.x * SCROLL_MULTIPLIER
									* TheGlobalData->m_keyboardScrollFactor;
							offset.y = TheGlobalData->m_verticalScrollSpeedFactor * fpsRatio * vecLength * vec.y * SCROLL_MULTIPLIER
									* TheGlobalData->m_keyboardScrollFactor;
						}
						break;
						case SCROLL_KEY:
						{
							if (scrollDir[DIR_UP])
							{
								offset.y -= TheGlobalData->m_verticalScrollSpeedFactor * fpsRatio * SCROLL_AMT
										* TheGlobalData->m_keyboardScrollFactor;
							}
							...
							{
								offset.x -= TheGlobalData->m_horizontalScrollSpeedFactor * fpsRatio * SCROLL_AMT
										* TheGlobalData->m_keyboardScrollFactor;
							}
			...
			// (TheGlobalData->m_saveCameraInReplay && (TheGameLogic->isInMultiplayerGame() || TheGameLogic->isInSinglePlayerGame()
