...
//-------------------------------------------------------------------------------------------------
MetaEventTranslator::MetaEventTranslator() : m_lastKeyDown(MK_NONE), m_lastModState(0)
{
...
GameMessageDisposition MetaEventTranslator::translateGameMessage(const GameMessage *msg)
{
	...
	if (t == GameMessage::MSG_RAW_KEY_DOWN || t == GameMessage::MSG_RAW_KEY_UP)
	{
		...
		{
			DEBUG_ASSERTCRASH(
					map->m_meta > GameMessage::MSG_BEGIN_META_MESSAGES && map->m_meta < GameMessage::MSG_END_META_MESSAGES,
			...
			{
				// DEBUG_LOG(("Frame %d: MetaEventTranslator::translateGameMessage() Mods-only change: %s", TheGameLogic->getFrame(),
							...
							))
			{
				if (keyState & KEY_STATE_AUTOREPEAT)
				{
					...
					// DEBUG_LOG(("Frame %d: MetaEventTranslator::translateGameMessage() auto-repeat: %s", TheGameLogic->getFrame(),
					// findGameMessageNameByType(map->m_meta)));
				}
				else
				{
					// THIS IS A GREASY HACK... MESSAGE SHOULD BE HANDLED IN A TRANSLATOR, BUT DURING CINEMATICS THE TRANSLATOR IS
					// DISABLED
					...
					/*GameMessage *metaMsg =*/TheMessageStream->appendMessage(map->m_meta);
					// DEBUG_LOG(("Frame %d: MetaEventTranslator::translateGameMessage() normal: %s", TheGameLogic->getFrame(),
					// findGameMessageNameByType(map->m_meta)));
				}
				disp = DESTROY_MESSAGE;
				break;
			...
			case GameMessage::MSG_RAW_MOUSE_RIGHT_BUTTON_UP:
			{
				...
				if (t == GameMessage::MSG_RAW_MOUSE_LEFT_BUTTON_UP)
				{
					if (m_nextUpShouldCreateDoubleClick[index])
						newMessage =
								TheMessageStream->insertMessage(GameMessage::MSG_MOUSE_LEFT_DOUBLE_CLICK, const_cast<GameMessage *>(msg));
					else
				...
				else if (t == GameMessage::MSG_RAW_MOUSE_MIDDLE_BUTTON_UP)
				{
					if (m_nextUpShouldCreateDoubleClick[index])
						newMessage =
								TheMessageStream->insertMessage(GameMessage::MSG_MOUSE_MIDDLE_DOUBLE_CLICK, const_cast<GameMessage *>(msg));
					else
						newMessage =
								TheMessageStream->insertMessage(GameMessage::MSG_MOUSE_MIDDLE_CLICK, const_cast<GameMessage *>(msg));
					m_nextUpShouldCreateDoubleClick[index] = FALSE;
				}
				else if (t == GameMessage::MSG_RAW_MOUSE_RIGHT_BUTTON_UP)
				{
					if (m_nextUpShouldCreateDoubleClick[index])
						newMessage =
								TheMessageStream->insertMessage(GameMessage::MSG_MOUSE_RIGHT_DOUBLE_CLICK, const_cast<GameMessage *>(msg));
					else
...
/*static */ void MetaMap::generateMetaMap()
{
	...
	}
	{
		...
		if (map->m_key == MK_NONE)
		{
			...
			map->m_description =
					TheGameText->FETCH_OR_SUBSTITUTE("GUI:SelectNextIdleWorkerDescription", L"Select the next idle worker");
