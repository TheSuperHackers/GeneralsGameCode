...
CBCommandStatus ControlBar::processCommandTransitionUI(GameWindow *control, GadgetGameMessage gadgetMessage)
{
  ...
  {
    if (m_currContext != CB_CONTEXT_NONE && m_currContext != CB_CONTEXT_OBSERVER_INFO
 ...
 * our GUI commands */
//-------------------------------------------------------------------------------------------------
CBCommandStatus ControlBar::processCommandUI(GameWindow *control, GadgetGameMessage gadgetMessage)
{
  ...
  if (m_currContext != CB_CONTEXT_MULTI_SELECT && commandButton->getCommandType() != GUI_COMMAND_PURCHASE_SCIENCE
  ...
  if (BitIsSet(commandButton->getOptions(), COMMAND_OPTION_NEED_TARGET))
  {
    ...
    switch (commandButton->getCommandType())
    {
      ...
      case GUI_COMMAND_UNIT_BUILD:
      {
        ...
        DEBUG_ASSERTCRASH(
            whatToBuild,
            ("Undefined BUILD command for object '%s'", commandButton->getThingTemplate()->getName().str()));
        ...
        else if (cmt != CANMAKE_OK)
        {
          DEBUG_ASSERTCRASH(
              0,
              ("Cannot create '%s' because the factory object '%s' returns false for canMakeUnit",
        ...
        if (pu == NULL)
        {
          DEBUG_ASSERTCRASH(
              0,
              ("Cannot create '%s' because the factory object '%s' is not capable of producting units",
               whatToBuild->getName().str(),
               factory->getTemplate()->getName().str()));
          break;
      ...
      {
        Int i;
        // TheSuperHackers @fix Caball009/Mauller 23/05/2025 Fix uninitialized variable and control lookup behaviour
        // to prevent a buffer-overflow when the control container is empty
        ObjectID objID = INVALID_ID;
        ...
        // if the control container returns an object ID but the object is not found, remove the control entry and
        // exit
