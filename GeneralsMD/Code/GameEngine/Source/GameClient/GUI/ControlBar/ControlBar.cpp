...
void CommandSet::parseCommandButton(INI *ini, void *instance, void *store, const void *userData)
{
  ...
  // sanity
  DEBUG_ASSERTCRASH(buttonIndex < MAX_COMMANDS_PER_SET, ("parseCommandButton: button index '%d' out of range", buttonIndex));
...
void ControlBar::init(void)
{
  ...
  if (TheWindowManager)
  {
    ...
    for (i = 0; i < MAX_PURCHASE_SCIENCE_RANK_3; i++)
    {
      ...
      id = TheNameKeyGenerator->nameToKey(windowName.str());
      m_sciencePurchaseWindowsRank3[i] = TheWindowManager->winGetWindowFromId(m_contextParent[CP_PURCHASE_SCIENCE], id);
    ...
    for (i = 0; i < MAX_PURCHASE_SCIENCE_RANK_8; i++)
    {
      ...
      id = TheNameKeyGenerator->nameToKey(windowName.str());
      m_sciencePurchaseWindowsRank8[i] = TheWindowManager->winGetWindowFromId(m_contextParent[CP_PURCHASE_SCIENCE], id);
  ...
  for (i = 0; i < m_currentlyUsedSpecialPowersButtons; i++)
  {
    ...
    else
    {
      ...
      else if (commandButton->getCommandType() == GUI_COMMAND_SELECT_ALL_UNITS_OF_TYPE)
      {
        ...
        Object *obj =
            ThePlayerList->getLocalPlayer()->findAnyExistingObjectWithThingTemplate(commandButton->getThingTemplate());
        if (!obj)
        {
          continue;
        }
