...
const FieldParse CommandButton::s_commandButtonFieldParseTable[] = {
    ...
    {"MaxShotsToFire", INI::parseInt, nullptr,
     offsetof(CommandButton, m_maxShotsToFire)},
    {"Science", INI::parseScienceVector, nullptr,
     offsetof(CommandButton, m_science)},
    {"SpecialPower", INI::parseSpecialPowerTemplate, nullptr,
     offsetof(CommandButton, m_specialPower)},
    {"TextLabel", INI::parseAsciiString, nullptr,
     offsetof(CommandButton, m_textLabel)},
    {"DescriptLabel", INI::parseAsciiString, nullptr,
     offsetof(CommandButton, m_descriptionLabel)},
    {"PurchasedLabel", INI::parseAsciiString, nullptr,
     offsetof(CommandButton, m_purchasedLabel)},
    {"ConflictingLabel", INI::parseAsciiString, nullptr,
    ...
    {"CursorName", INI::parseAsciiString, nullptr,
     offsetof(CommandButton, m_cursorName)},
    {"InvalidCursorName", INI::parseAsciiString, nullptr,
    ...
    {"UnitSpecificSound", INI::parseAudioEventRTS, nullptr,
...
void ControlBar::markUIDirty(void) {
  ...
  if (m_consecutiveDirtyFrames > 20) {
    DEBUG_CRASH(("Serious flaw in interface system! Either new code or INI has "
                 "caused the interface to be marked dirty every frame. This "
                 "problem actually causes the interface to completely lockup "
                 "not allowing you to click normal game buttons."));
  }
...
Relationship
ControlBar::getCurrentlyViewedPlayerRelationship(const Team *team) {
...
void ControlBar::populatePurchaseScience(Player *player) {
  ...
  if (commandSet1 == nullptr || commandSet3 == nullptr ||
      commandSet8 == nullptr)
    return;
  ...
  for (i = 0; i < MAX_PURCHASE_SCIENCE_RANK_1; i++) {
    ...
    if (commandButton == nullptr ||
        BitIsSet(commandButton->getOptions(), SCRIPT_ONLY)) {
  ...
  /*
     ...
     "GeneralsExpPoints.wnd:StaticTextLevel" ) ); if(win)
          {
                  tempUS.format(TheGameText->fetch("SCIENCE:Rank"),
     player->getRankLevel()); GadgetStaticTextSetText(win, tempUS);
          }
  */
...
Bool CommandButton::isValidToUseOn(const Object *sourceObj,
  ...
  if (BitIsSet(m_options, NEED_TARGET_POS)) {
    return TheActionManager->canDoSpecialPowerAtLocation(
        sourceObj, &pos, commandSource, m_specialPower, nullptr, m_options,
        false);
  }
...
Bool CommandButton::isValidObjectTarget(const Drawable *source,
  ...
  return isValidObjectTarget(source ? source->getObject() : nullptr,
                             target ? target->getObject() : nullptr);
}
...
const FieldParse CommandSet::m_commandSetFieldParseTable[] = {
    ...
    {"1", CommandSet::parseCommandButton, (void *)nullptr,
    ...
    {"12", CommandSet::parseCommandButton, (void *)11,
     offsetof(CommandSet, m_command)},
    {"13", CommandSet::parseCommandButton, (void *)12,
     offsetof(CommandSet, m_command)},
    {"14", CommandSet::parseCommandButton, (void *)13,
     offsetof(CommandSet, m_command)},
    {"15", CommandSet::parseCommandButton, (void *)14,
     offsetof(CommandSet, m_command)},
    {"16", CommandSet::parseCommandButton, (void *)15,
     offsetof(CommandSet, m_command)},
    {"17", CommandSet::parseCommandButton, (void *)16,
     offsetof(CommandSet, m_command)},
    {"18", CommandSet::parseCommandButton, (void *)17,
     offsetof(CommandSet, m_command)},
    {nullptr, nullptr, nullptr, 0}
...
void CommandButton::copyButtonTextFrom(const CommandButton *button,
                                       Bool shortcutButton,
                                       Bool markUIDirtyIfChanged) const {
  // This function was added to change the strings when you upgrade from a
  // DaisyCutter to a MOAB. All other special powers are the same.
  Bool change = FALSE;
  if (shortcutButton) {
    // Not the best code, but conflicting label means shortcut label (most won't
    // have any string specified).
    if (button->getConflictingLabel().isNotEmpty() &&
        m_textLabel.compare(button->getConflictingLabel())) {
      m_textLabel = button->getConflictingLabel();
      change = TRUE;
    }
  } else {
    // Copy the text from the purchase science button if it exists (most won't).
    if (button->getTextLabel().isNotEmpty() &&
        m_textLabel.compare(button->getTextLabel())) {
      m_textLabel = button->getTextLabel();
      change = TRUE;
    }
  }
  if (button->getDescriptionLabel().isNotEmpty() &&
      m_descriptionLabel.compare(button->getDescriptionLabel())) {
...
void CommandSet::parseCommandButton(INI *ini, void *instance, void *store,
  ...
  DEBUG_ASSERTCRASH(
      ...
      ("parseCommandButton: button index '%d' out of range", buttonIndex));
...
ControlBar::ControlBar(void) {
  ...
  updateCommandBarBorderColors(GAME_COLOR_UNDEFINED, GAME_COLOR_UNDEFINED,
...
//-------------------------------------------------------------------------------------------------
/** Initialize the control bar, this is our interface to the context sensitive
 * GUI */
//-------------------------------------------------------------------------------------------------
void ControlBar::init(void) {
  ...
  ini.loadFileDirectory("Data\\INI\\Default\\CommandButton", INI_LOAD_OVERWRITE,
                        nullptr);
  ini.loadFileDirectory("Data\\INI\\CommandButton", INI_LOAD_OVERWRITE,
                        nullptr);
  ...
  if (TheWindowManager) {
    ...
    m_contextParent[CP_PURCHASE_SCIENCE] = TheWindowManager->winGetWindowFromId(
        nullptr, id); // m_scienceLayout->getFirstWindow();
    ...
    GameWindow *win = TheWindowManager->winGetWindowFromId(
        nullptr,
        TheNameKeyGenerator->nameToKey("ControlBar.wnd:ButtonOptions"));
    if (win) {
    ...
    win = TheWindowManager->winGetWindowFromId(
        nullptr, TheNameKeyGenerator->nameToKey("ControlBar.wnd:ButtonLarge"));
    if (win) {
    ...
    win = TheWindowManager->winGetWindowFromId(
        nullptr, TheNameKeyGenerator->nameToKey("ControlBar.wnd:PowerWindow"));
    if (win) {
      win->winSetTooltipFunc(commandButtonTooltip);
    }
    win = TheWindowManager->winGetWindowFromId(
    ...
    win = TheWindowManager->winGetWindowFromId(
        nullptr, TheNameKeyGenerator->nameToKey("ControlBar.wnd:GeneralsExp"));
    if (win) {
      win->winSetTooltipFunc(commandButtonTooltip);
    }

    m_radarAttackGlowWindow = TheWindowManager->winGetWindowFromId(
        nullptr, TheNameKeyGenerator->nameToKey("ControlBar.wnd:WinUAttack"));
...
void ControlBar::reset(void) {
  ...
  // do not destroy the rally drawable, it will get destroyed with everything
  // else during a reset
  m_rallyPointDrawableID = INVALID_DRAWABLE_ID;
...
void ControlBar::update(void) {
  ...
  if (m_isObserverCommandBar) {
    ...
    if (TheInGameUI->getSelectCount() > 1) {
      ...
      // TheSuperHackers @fix Mauller 07/04/2025 The first access to this can
  ...
  if (ThePlayerList && ThePlayerList->getLocalPlayer() &&
    ...
    GameWindow *win =
        TheWindowManager->winGetWindowFromId(nullptr, beaconPlacementButtonID);
    if (win) {
...
void ControlBar::onDrawableDeselected(Drawable *draw) {
  ...
  // obviously be over now because we are no longer selecting the dozer or
  // worker
  //
...
const Image *ControlBar::getStarImage(void) {
  ...
  GameWindow *win = TheWindowManager->winGetWindowFromId(
      nullptr, TheNameKeyGenerator->nameToKey("ControlBar.wnd:ButtonGeneral"));
  if (!win)
    return nullptr;
  if (!m_genStarFlash) {
...
//-------------------------------------------------------------------------------------------------
CommandButton *
ControlBar::newCommandButtonOverride(CommandButton *buttonToOverride) {
  if (!buttonToOverride) {
    return nullptr;
  }
...
/*static*/ void ControlBar::parseCommandSetDefinition(INI *ini) {
  ...
  DEBUG_ASSERTCRASH(
      commandSet,
      ("parseCommandSetDefinition: Unable to allocate set '%s'", name.str()));
...
void ControlBar::switchToContext(ControlBarContext context, Drawable *draw) {
  ...
  case CB_CONTEXT_NONE: {
    ...
    for (int i = 0; i < MAX_COMMANDS_PER_SET; i++) {
      // the implementation won't necessarily use the max number of windows
      // possible
      if (m_commandWindows[i]) {
  ...
  default: {
    ...
    DEBUG_ASSERTCRASH(
        0, ("ControlBar::switchToContext, unknown context '%d'", context));
    break;
  }
  }
 ...
 * NOTE that parent may be nullptr, it only helps to speed up the search for a
...
void ControlBar::setControlBarSchemeByPlayer(Player *p) {
  ...
  GameWindow *buttonPlaceBeacon =
      TheWindowManager->winGetWindowFromId(nullptr, buttonPlaceBeaconID);
  GameWindow *buttonIdleWorker =
      TheWindowManager->winGetWindowFromId(nullptr, buttonIdleWorkerID);
  GameWindow *buttonGeneral =
      TheWindowManager->winGetWindowFromId(nullptr, buttonGeneralID);
...
void ControlBar::updateCommandBarBorderColors(Color build, Color action,
...
void ControlBar::updateSlotExitImage(const Image *image) {
  ...
  // Kris:
  // Other than this being a completely ridiculously retarded idea, I'm not
  // inclined to recode this in a better way, yikes! Btw, I DID NOT CODE THIS!
  // But this is what this does: The button images are overridden by a faction
  // specific icon. The proper way to fix this would be to make a commandbutton
  // option and loop through all buttons on init to replace the icon. We need a
  // system like this for neutral buildings which can have a different empty
  // inventory icon based on the faction player.
...
void ControlBar::triggerRadarAttackGlow(void) {
  ...
  if (BitIsSet(m_radarAttackGlowWindow->winGetStatus(), WIN_STATUS_ENABLED) ==
      TRUE)
...
void ControlBar::updateRadarAttackGlow(void) {
  ...
  if (m_remainingRadarAttackGlowFrames % RADAR_ATTACK_GLOW_NUM_TIMES == 0) {
    m_radarAttackGlowWindow->winEnable(
        !BitIsSet(m_radarAttackGlowWindow->winGetStatus(), WIN_STATUS_ENABLED));
  }
}
void ControlBar::initSpecialPowershortcutBar(Player *player) {
  ...
  for (i = 0; i < MAX_SPECIAL_POWER_SHORTCUTS; i++) {
    ...
    m_specialPowerShortcutButtons[i]->winSetStatus(
        WIN_STATUS_USE_OVERLAY_STATES);
    // Oh god... this is a total hack for shortcut buttons to handle rendering
...
void ControlBar::populateSpecialPowerShortcut(Player *player) {
  ...
  if (!player || !player->getPlayerTemplate() || !player->isLocalPlayer() ||
      m_currentlyUsedSpecialPowersButtons == 0 ||
      m_specialPowerShortcutButtons == nullptr ||
      m_specialPowerShortcutButtonParents == nullptr)
    return;
  for (i = 0; i < MAX_SPECIAL_POWER_SHORTCUTS; ++i) {
  ...
  for (i = 0; i < m_currentlyUsedSpecialPowersButtons; i++) {
    ...
    if (commandButton == nullptr) {
      ...
      if (BitIsSet(commandButton->getOptions(), NEED_UPGRADE)) {
        ...
        if (upgrade && !ThePlayerList->getLocalPlayer()->hasUpgradeComplete(
                           upgrade->getUpgradeMask())) {
          // Kris: 8/13/03 - Don't show shortcut buttons that require upgrades
          // we don't have. As far as I know, only the radar van scan has this.
          // The MOAB is handled differently (sciences).
          continue;
        }
      }
      ...
      if (BitIsSet(commandButton->getOptions(), NEED_SPECIAL_POWER_SCIENCE)) {
        ...
        if (!power) {
          // Should have the power.. button is probably missing the SpecialPower
          // = xxx entry.
          DEBUG_CRASH(("CommandButton %s needs a SpecialPower entry, but it's "
        ...
        Object *obj = ThePlayerList->getLocalPlayer()
        ...
        if (power->getRequiredScience() != SCIENCE_INVALID) {
          if (player->hasScience(power->getRequiredScience()) == FALSE) {
            ...
            if (bestIndex != -1) {
              ...
              commandSet1 =
                  findCommandSet(player->getPlayerTemplate()
                                     ->getPurchaseScienceCommandSetRank1());
              commandSet3 =
                  findCommandSet(player->getPlayerTemplate()
                                     ->getPurchaseScienceCommandSetRank3());
              ...
              for (i = 0; !found && i < MAX_PURCHASE_SCIENCE_RANK_1; i++) {
                ...
                if (command &&
                    command->getCommandType() == GUI_COMMAND_PURCHASE_SCIENCE) {
                  // All purchase sciences specify a single science.
                  if (command->getScienceVec().empty()) {
                    DEBUG_CRASH(("Commandbutton %s is a purchase science "
                                 "button without any science! Please add it.",
                                 command->getName().str()));
      ...
      } else if (commandButton->getCommandType() ==
                 GUI_COMMAND_SELECT_ALL_UNITS_OF_TYPE) {
        // Make sure we actually have an object of type that we want to be able
        // to select.
        Object *obj = ThePlayerList->getLocalPlayer()
...
void ControlBar::updateSpecialPowerShortcut(void) {
  ...
  Bool hasAnyShortcutSpecialPower =
      ThePlayerList->getLocalPlayer()->hasAnyShortcutSpecialPower();

  Bool hasValidShortcutButton =
      hasShortcutSelectionButtons || hasAnyShortcutSpecialPower;

  if (hasValidShortcutButton && m_specialPowerShortcutParent->winIsHidden() &&
      m_contextParent[CP_MASTER] &&
      !m_contextParent[CP_MASTER]->winIsHidden()) {
    showSpecialPowerShortcut();
    animateSpecialPowerShortcut(TRUE);
  ...
  for (Int i = 0; i < m_currentlyUsedSpecialPowersButtons; i++) {
    ...
    if (spTemplate) {
      ...
      Object *obj = ThePlayerList->getLocalPlayer()
                        ->findAnyExistingObjectWithThingTemplate(
                            command->getThingTemplate());
      if (obj) {
        // Make command available if it isn't a special power template shortcut
        // power.
        ...
        obj = ThePlayerList->getLocalPlayer()
                  ->findMostReadyShortcutSpecialPowerForThing(
                      command->getThingTemplate(), mostReadyPercentage);
        if (obj) {
          // Ugh... hacky.
          // Look for a command button for a special power and if so, then get
          // the command availability for it.
          const CommandSet *commandSet =
              findCommandSet(obj->getCommandSetString());
          if (commandSet) {
            for (Int commandIndex = 0; commandIndex < MAX_COMMANDS_PER_SET;
                 commandIndex++) {
              const CommandButton *evalButton =
                  commandSet->getCommandButton(commandIndex);
              GameWindow *evalButtonWin = m_commandWindows[commandIndex];
              if (evalButton &&
                  evalButton->getCommandType() == GUI_COMMAND_SPECIAL_POWER) {
                // We want to evaluate the special powerbutton... but apply the
                // clock overlay to our button!
                availability =
                    getCommandAvailability(evalButton, obj, evalButtonWin, win);
                break;
              }
            }
          }
  ...
  for (Int i = 0; i < m_currentlyUsedSpecialPowersButtons; i++) {
    ...
    for (int i = 0; i < MAX_SPECIAL_POWER_SHORTCUTS; i++) {
      if (!m_shortcutDisplayStrings[i]) {
        // m_shortcutDisplayStrings[ i ] =
        // TheDisplayStringManager->newDisplayString();
        // m_shortcutDisplayStrings[ i ]->setFont( TheFontLibrary->getFont(
        // "Arial", 16, false ) );
      }
      ...
      Int numReady = 0;
      if (spTemplate) {
        numReady = ThePlayerList->getLocalPlayer()
      ...
      if (numReady > 1) // Lorenzen changed... Displaying a "1" is superfluous
      {
        ...
        // TheControlBar->m_shortcutDisplayStrings[ i ]->draw( 600, i * 40 + 40,
        // GameMakeColor(255,255,255,255), GameMakeColor(0,0,0,0), 0, 0 );
      } else {
        UnicodeString unibuffer;
        GadgetButtonSetText(win, unibuffer);
        // TheDisplayStringManager->freeDisplayString( m_shortcutDisplayStrings[
        // i ] ); m_shortcutDisplayStrings[ i ] = nullptr;
      }
    }
  }
}
...
void ControlBar::setScaledViewportHeight() {
  TheTacticalView->setHeight(TheDisplay->getHeight() *
                             TheGlobalData->m_viewportHeightScale);
}
