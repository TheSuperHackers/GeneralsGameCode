...
void ControlBar::markUIDirty(void)
{
    ...
    {
        DEBUG_CRASH(
            ("Serious flaw in interface system! Either new code or INI has caused the interface to be marked dirty every "
             "frame. This problem actually causes the interface to completely lockup not allowing you to click normal game "
             "buttons."));
    }
...
void ControlBar::populatePurchaseScience(Player *player)
{
    ...
    commandSet1 = TheControlBar->findCommandSet(
        player->getPlayerTemplate()->getPurchaseScienceCommandSetRank1()); // TEMP WILL CHANGE TO PROPER WAY ONCE WORKING
    commandSet3 = TheControlBar->findCommandSet(
        player->getPlayerTemplate()->getPurchaseScienceCommandSetRank3()); // TEMP WILL CHANGE TO PROPER WAY ONCE WORKING
    commandSet8 = TheControlBar->findCommandSet(
        player->getPlayerTemplate()->getPurchaseScienceCommandSetRank8()); // TEMP WILL CHANGE TO PROPER WAY ONCE WORKING
    ...
    if (commandSet1 == NULL || commandSet3 == NULL || commandSet8 == NULL)
        return;
            ...
            if (!commandButton->getScienceVec().empty())
            {
                ...
                else
                {
                    ...
                    if (!player->hasScience(st) && TheScienceStore->playerHasPrereqsForScience(player, st)
                        && TheScienceStore->getSciencePurchaseCost(st) <= player->getSciencePurchasePoints())
                    {
    ...
    /*
        win = TheWindowManager->winGetWindowFromId( m_contextParent[ CP_PURCHASE_SCIENCE ], TheNameKeyGenerator->nameToKey(
...
//-------------------------------------------------------------------------------------------------
Bool CommandButton::isValidToUseOn(
    ...
    CommandSourceType commandSource) const
{
    ...
    if (BitIsSet(m_options, COMMAND_OPTION_NEED_OBJECT_TARGET))
    {
        return TheActionManager
            ->canDoSpecialPowerAtObject(sourceObj, targetObj, commandSource, m_specialPower, m_options, false);
    }

    if (BitIsSet(m_options, NEED_TARGET_POS))
    {
        return TheActionManager
            ->canDoSpecialPowerAtLocation(sourceObj, &pos, commandSource, m_specialPower, NULL, m_options, false);
    }
...
void ControlBar::init(void)
{
    ...
    if (TheWindowManager)
    {
        ...
        m_contextParent[CP_PURCHASE_SCIENCE] =
            TheWindowManager->winGetWindowFromId(NULL, id); // m_scienceLayout->getFirstWindow();
        ...
        for (i = 0; i < MAX_RIGHT_HUD_UPGRADE_CAMEOS; i++)
        {
            ...
            id = TheNameKeyGenerator->nameToKey(windowName.str());
            m_rightHUDUpgradeCameos[i] = TheWindowManager->winGetWindowFromId(m_rightHUDWindow, id);
        ...
        GameWindow *win =
            TheWindowManager->winGetWindowFromId(NULL, TheNameKeyGenerator->nameToKey("ControlBar.wnd:ButtonOptions"));
        if (win)
        {
        ...
        m_radarAttackGlowWindow =
            TheWindowManager->winGetWindowFromId(NULL, TheNameKeyGenerator->nameToKey("ControlBar.wnd:WinUAttack"));

        win = TheWindowManager->winGetWindowFromId(
            NULL,
            TheNameKeyGenerator->nameToKey(AsciiString("ControlBar.wnd:BackgroundMarker")));
        ...
        m_genStarOn =
            TheMappedImageCollection ? (Image *)TheMappedImageCollection->findImageByName("BarButtonGenStarON") : NULL;
        m_genStarOff =
            TheMappedImageCollection ? (Image *)TheMappedImageCollection->findImageByName("BarButtonGenStarOFF") : NULL;
        m_genStarFlash = TRUE;
...
void ControlBar::update(void)
{
    ...
    if (m_currentSelectedDrawable == NULL)
    {
        ...
        DEBUG_ASSERTCRASH(
            m_currContext == CB_CONTEXT_NONE,
            ("ControlBar::update no selection, but not we're not showing the default NONE context"));
        return;
...
const Image *ControlBar::getStarImage(void)
{
    ...
    GameWindow *win =
        TheWindowManager->winGetWindowFromId(NULL, TheNameKeyGenerator->nameToKey("ControlBar.wnd:ButtonGeneral"));
    if (!win)
        return NULL;
    if (!m_genStarFlash)
    {
...
void ControlBar::evaluateContextUI(void)
{
    ...
    if (!TheInGameUI->areSelectedObjectsControllable())
    {
        ...
        if (obj->getControllingPlayer() && obj->getControllingPlayer()->getPlayerTemplate()
            && obj->getControllingPlayer()->getPlayerTemplate()->getBeaconTemplate().compare(obj->getTemplate()->getName())
                == 0)
        {
...
/*static*/ void ControlBar::parseCommandSetDefinition(INI *ini)
{
    ...
    else if (ini->getLoadType() != INI_LOAD_CREATE_OVERRIDES)
    {
        ...
        // command set.
        DEBUG_CRASH(
            ("[LINE: %d in '%s'] Duplicate commandset %s found!", ini->getLineNum(), ini->getFilename().str(), name.str()));
        throw INI_INVALID_DATA;
...
//-------------------------------------------------------------------------------------------------
CBCommandStatus ControlBar::processContextSensitiveButtonClick(GameWindow *button, GadgetGameMessage gadgetMessage)
{
...
/** Process a button click for the context sensitive GUI */
//-------------------------------------------------------------------------------------------------
CBCommandStatus ControlBar::processContextSensitiveButtonTransition(GameWindow *button, GadgetGameMessage gadgetMessage)
{
    // call command processing method
 ...
 * window ID */
//-------------------------------------------------------------------------------------------------
void ControlBar::setControlCommand(
    const AsciiString &buttonWindowName,
    GameWindow *parent,
    const CommandButton *commandButton)
{
...
void ControlBar::updateBuildQueueDisabledImages(const Image *image)
{
    ...
    for (i = 0; i < MAX_BUILD_QUEUE_BUTTONS; i++)
    {
        ...
        m_queueData[i].control = TheWindowManager->winGetWindowFromId(m_contextParent[CP_BUILD_QUEUE], buildQueueIDs[i]);
...
void ControlBar::showPurchaseScience(void)
{
    ...
    // m_generalsScreenAnimate->registerGameWindow( m_contextParent[ CP_PURCHASE_SCIENCE ], WIN_ANIMATION_SLIDE_TOP, TRUE,
    // 200 );
}
...
void ControlBar::switchControlBarStage(ControlBarStages stage)
{
    ...
    switch (stage)
    {
        ...
        default:
            DEBUG_ASSERTCRASH(
                FALSE,
                ("ControlBar::switchControlBarStage we were passed in a stage that's not supported %d", stage));
    }
}
...
{
    GameWindow *win =
        TheWindowManager->winGetWindowFromId(NULL, TheNameKeyGenerator->nameToKey("ControlBar.wnd:ButtonLarge"));
    if (!win)
        return;
...
void ControlBar::initSpecialPowershortcutBar(Player *player)
{
    ...
    if (!player || !pt || !player->isLocalPlayer() || pt->getSpecialPowerShortcutButtonCount() == 0
        || pt->getSpecialPowerShortcutWinName().isEmpty() || !player->isPlayerActive())
        return;
    ...
    for (i = 0; i < MAX_SPECIAL_POWER_SHORTCUTS; i++)
    {
        ...
        id = TheNameKeyGenerator->nameToKey(windowName.str());
        m_specialPowerShortcutButtons[i] = TheWindowManager->winGetWindowFromId(m_specialPowerShortcutParent, id);
        ...
        id = TheNameKeyGenerator->nameToKey(windowName.str());
        m_specialPowerShortcutButtonParents[i] = TheWindowManager->winGetWindowFromId(m_specialPowerShortcutParent, id);
    } // end for i
}
...
void ControlBar::populateSpecialPowerShortcut(Player *player)
{
    ...
    commandSet = TheControlBar->findCommandSet(
        player->getPlayerTemplate()->getSpecialPowerShortcutCommandSet()); // TEMP WILL CHANGE TO PROPER WAY ONCE WORKING
    if (!commandSet)
        return;
    // populate the button with commands defined
    Int currentButton = 0;
    ...
    for (i = 0; i < m_currentlyUsedSpecialPowersButtons; i++)
    {
        ...
        else
        {
            ...
            if (BitIsSet(commandButton->getOptions(), NEED_SPECIAL_POWER_SCIENCE))
            {
                ...
                Object *obj = ThePlayerList->getLocalPlayer()->findMostReadyShortcutSpecialPowerOfType(
                    commandButton->getSpecialPowerTemplate()->getSpecialPowerType());
                if (!obj)
                {
                    continue;
                }
                ...
                if (power->getRequiredScience() != SCIENCE_INVALID)
                {
                    ...
                    else
                    {
                        ...
                        if (bestIndex != -1)
                        {
                            ...
                            commandSet1 = TheControlBar->findCommandSet(
                                player->getPlayerTemplate()->getPurchaseScienceCommandSetRank1());
                            commandSet3 = TheControlBar->findCommandSet(
                                player->getPlayerTemplate()->getPurchaseScienceCommandSetRank3());
                            commandSet8 = TheControlBar->findCommandSet(
                                player->getPlayerTemplate()->getPurchaseScienceCommandSetRank8());
                            ...
                            for (i = 0; !found && i < MAX_PURCHASE_SCIENCE_RANK_1; i++)
                            {
                                ...
                                if (command && command->getCommandType() == GUI_COMMAND_PURCHASE_SCIENCE)
                                {
                                    ...
                                    {
                                        DEBUG_CRASH(
                                            ("Commandbutton %s is a purchase science button without any science! Please add "
                                             "it.",
                                             command->getName().str()));
                                    }
            ...
            else if (commandButton->getCommandType() == GUI_COMMAND_SELECT_ALL_UNITS_OF_TYPE)
            {
                ...
                Object *obj = ThePlayerList->getLocalPlayer()->findAnyExistingObjectWithThingTemplate(
    ...
    if (m_contextParent[CP_MASTER] && !m_contextParent[CP_MASTER]->winIsHidden()
...
{
    if (!m_specialPowerShortcutParent || !m_specialPowerShortcutButtons || !ThePlayerList
    ...
    if (hasValidShortcutButton && m_specialPowerShortcutParent->winIsHidden() && m_contextParent[CP_MASTER]
        && !m_contextParent[CP_MASTER]->winIsHidden())
    {
        ...
        !hasValidShortcutButton && !m_specialPowerShortcutParent->winIsHidden()
    ...
    for (Int i = 0; i < m_currentlyUsedSpecialPowersButtons; i++)
    {
        ...
        else if (command->getCommandType() == GUI_COMMAND_SELECT_ALL_UNITS_OF_TYPE)
        {
            ...
            Object *obj =
                ThePlayerList->getLocalPlayer()->findAnyExistingObjectWithThingTemplate(command->getThingTemplate());
            if (obj)
            {
                ...
                obj = ThePlayerList->getLocalPlayer()->findMostReadyShortcutSpecialPowerForThing(
...
void ControlBar::drawSpecialPowerShortcutMultiplierText()
{
    for (Int i = 0; i < m_currentlyUsedSpecialPowersButtons; i++)
    {
        ...
        for (int i = 0; i < MAX_SPECIAL_POWER_SHORTCUTS; i++)
        {
            ...
            if (spTemplate)
            {
                numReady = ThePlayerList->getLocalPlayer()->countReadyShortcutSpecialPowersOfType(
            ...
            if (numReady > 1) // Lorenzen changed... Displaying a "1" is superfluous
            {
                ...
                // TheControlBar->m_shortcutDisplayStrings[ i ]->draw( 600, i * 40 + 40, GameMakeColor(255,255,255,255),
...
void ControlBar::animateSpecialPowerShortcut(Bool isOn)
{
    ...
    if (isOn)
    {
        ...
        m_animateWindowManagerForGenShortcuts
            ->registerGameWindow(m_specialPowerShortcutParent, WIN_ANIMATION_SLIDE_RIGHT, TRUE, 500, 0);
    }
    else
    {
...
{
    if (TheScriptEngine->isGameEnding() || !m_specialPowerShortcutParent || !m_specialPowerShortcutButtons || !ThePlayerList
        || !ThePlayerList->getLocalPlayer())
        return;
    Bool dontAnimate = TRUE;
    for (Int i = 0; i < m_currentlyUsedSpecialPowersButtons; ++i)
