...
void ControlBar::populatePurchaseScience(Player *player)
{
	...
	if (!player || !player->getPlayerTemplate() || player->getPlayerTemplate()->getPurchaseScienceCommandSetRank1().isEmpty()
		...
		return;
	commandSet1 = TheControlBar->findCommandSet(player->getPlayerTemplate()->getPurchaseScienceCommandSetRank1()); // TEMP WILL
																																																								 // CHANGE TO
																																																								 // PROPER
																																																								 // WAY ONCE
																																																								 // WORKING
	commandSet3 = TheControlBar->findCommandSet(player->getPlayerTemplate()->getPurchaseScienceCommandSetRank3()); // TEMP WILL
																																																								 // CHANGE TO
																																																								 // PROPER
																																																								 // WAY ONCE
																																																								 // WORKING
	commandSet8 = TheControlBar->findCommandSet(player->getPlayerTemplate()->getPurchaseScienceCommandSetRank8()); // TEMP WILL
																																																								 // CHANGE TO
																																																								 // PROPER
																																																								 // WAY ONCE
																																																								 // WORKING
...
void ControlBar::populateSpecialPowerShortcut(Player *player)
{
	...
	if (player->getPlayerTemplate()->getSpecialPowerShortcutCommandSet().isEmpty())
		return;
	commandSet = TheControlBar->findCommandSet(player->getPlayerTemplate()->getSpecialPowerShortcutCommandSet()); // TEMP WILL
																																																								// CHANGE TO
																																																								// PROPER WAY
																																																								// ONCE
																																																								// WORKING
						...
						if (bestIndex != -1)
						{
							...
							for (i = 0; !found && i < MAX_PURCHASE_SCIENCE_RANK_1; i++)
							{
								...
								if (command && command->getCommandType() == GUI_COMMAND_PURCHASE_SCIENCE)
								{
									...
									{
										DEBUG_CRASH(
												("Commandbutton %s is a purchase science button without any science! Please add it.",
												 command->getName().str()));
									}
