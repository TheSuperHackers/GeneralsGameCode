								...
								&& commandButton->getCommandType() != GUI_COMMAND_OBJECT_UPGRADE)
						{
							...
							else
							{
								...
								if (bestIndex != -1)
								{
									...
									commandSet1 =
											TheControlBar->findCommandSet(player->getPlayerTemplate()->getPurchaseScienceCommandSetRank1());
									commandSet3 =
											TheControlBar->findCommandSet(player->getPlayerTemplate()->getPurchaseScienceCommandSetRank3());
									commandSet8 =
											TheControlBar->findCommandSet(player->getPlayerTemplate()->getPurchaseScienceCommandSetRank8());
...
void ControlBar::populateBuildQueue(Object *producer)
{
	...
	for (production = pu->firstProduction(); production; production = pu->nextProduction(production))
	{
		...
		if (production->getProductionType() == PRODUCTION_UNIT)
		{
			...
			//
			//		image = TheMappedImageCollection->findImageByName( production->getProductionObject()->getInventoryImageName(
			// INV_IMAGE_DISABLED ) ); 		GadgetButtonSetDisabledImage( m_queueData[ windowIndex ].control, image );
	...
	for (i = 0; i < MAX_COMMANDS_PER_SET; i++)
	{
		...
		if (BitIsSet(command->getOptions(), CHECK_LIKE))
		{
			...
			DEBUG_ASSERTCRASH(
					...
					("updateContextCommand: Error, gadget window for command '%s' is not check-like!", command->getName().str()));
		...
		Bool forceDisabledEvaluation) const
{
	...
	if (disabled && !forceDisabledEvaluation)
	{
		...
		if (commandType != GUI_COMMAND_SELL && commandType != GUI_COMMAND_EVACUATE && commandType != GUI_COMMAND_EXIT_CONTAINER
				&& commandType != GUI_COMMAND_BEACON_DELETE && commandType != GUI_COMMAND_SET_RALLY_POINT
				&& commandType != GUI_COMMAND_STOP && commandType != GUI_COMMAND_SWITCH_WEAPON)
		{
	...
	switch (command->getCommandType())
	{
		...
		case GUI_COMMAND_FIRE_WEAPON:
		{
			...
			// DEBUG_ASSERTLOG( w, ("Unit %s's CommandButton %s is trying to access weaponslot %d, but doesn't have a weapon there
			// in its FactionUnit ini entry.", 	obj->getTemplate()->getName().str(), command->getName().str(),
			...
			if (w == NULL // No weapon
					...
					|| w->getPossibleNextShotFrame() == now // Weapon ready, but could fire this exact frame (handle button flicker
																									// since it may be going to fire anyway)
					/// @todo srj -- not sure why this next check is necessary, but the Comanche missile buttons will flicker without
					/// it. figure out someday.
					/// @todo ml  -- and note: that the "now-1" below causes zero-clip-reload weapons to never be ready, so I added
					/// this If you make changes to this code, make sure that the DragonTank's firewall weapon can be retargeted while
					/// active, that is, while the tank is squirting out flames all over the floor, you can click the firewall button
					/// (or "F"), and re-target the firewall without having to stop or move in-betwen.. Thanks for reading
					|| (w->getPossibleNextShotFrame() == now - 1))
			{
				if (w != NULL)
				{
					// only draw the clock when reloading a clip, not when merely between shots, since that's usually a tiny amount of
					// time
		...
		case GUI_COMMAND_SPECIAL_POWER_CONSTRUCT_FROM_SHORTCUT:
		{
			...
			if (mod == NULL)
			{
				...
				DEBUG_CRASH(
						("Object %s does not contain special power module (%s) to execute.  Did you forget to add it to the object INI?",
