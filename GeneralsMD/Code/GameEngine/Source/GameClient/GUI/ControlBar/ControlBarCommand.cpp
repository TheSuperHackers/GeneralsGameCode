                    ...
                    if (power && power->getRequiredScience() != SCIENCE_INVALID)
                    {
                            ...
                            && commandButton->getCommandType() != GUI_COMMAND_OBJECT_UPGRADE)
                        {
                            ...
                            else
                            {
                                ...
                                Int bestIndex = -1;
                                ScienceType science;
                                for (size_t scienceIndex = 0; scienceIndex < commandButton->getScienceVec().size();
                                     ++scienceIndex)
                                {
                                ...
                                if (bestIndex != -1)
                                {
                                    ...
                                    for (i = 0; !found && i < MAX_PURCHASE_SCIENCE_RANK_1; i++)
                                    {
                                        ...
                                        if (command && command->getCommandType() == GUI_COMMAND_PURCHASE_SCIENCE)
                                        {
                                            ...
                                            {
                                                DEBUG_CRASH(
                                                    ("Commandbutton %s is a purchase science button without any science! "
                                                     "Please add it.",
                                                     command->getName().str()));
                                            }
...
void ControlBar::populateBuildQueue(Object *producer)
{
    ...
    Int windowIndex = 0;
    const Image *image;
    for (production = pu->firstProduction(); production; production = pu->nextProduction(production))
    {
        ...
        if (production->getProductionType() == PRODUCTION_UNIT)
        {
            ...
            // No longer used.
            // image = TheMappedImageCollection->findImageByName( production->getProductionObject()->getInventoryImageName(
            // INV_IMAGE_HILITE ) ); GadgetButtonSetHiliteSelectedImage( m_queueData[ windowIndex ].control, image ); image =
            // TheMappedImageCollection->findImageByName( production->getProductionObject()->getInventoryImageName(
            // INV_IMAGE_PUSHED ) ); GadgetButtonSetHiliteImage( m_queueData[ windowIndex ].control, image );
            ...
            //production->getProductionObject()->getInventoryImageName( INV_IMAGE_DISABLED ) ); 		GadgetButtonSetDisabledImage(
            //m_queueData[ windowIndex ].control, image );
        ...
        else
        {
            ...
            //  queue button graphic we already have in place
            //
            //		image = TheMappedImageCollection->findImageByName( ut->getQueueImageName(
            //UpgradeTemplate::UPGRADE_DISABLED ) ); 		GadgetButtonSetDisabledImage( m_queueData[ windowIndex ].control, image
            //);
...
void ControlBar::updateContextCommand(void)
{
    ...
    if (contain && contain->getContainMax() > 0 && m_lastRecordedInventoryCount != contain->getContainCount())
    {
    ...
    for (i = 0; i < MAX_COMMANDS_PER_SET; i++)
    {
        ...
        if (BitIsSet(command->getOptions(), CHECK_LIKE))
        {
            ...
            DEBUG_ASSERTCRASH(
                BitIsSet(win->winGetStatus(), WIN_STATUS_CHECK_LIKE),
    ...
    Bool forceDisabledEvaluation) const
{
    ...
    // If we modify the button (like a gadget clock overlay), then sometimes we may wish to apply it to a specific different
    // button. But if we don't specify anything (default), then make them the same.
    if (!applyToWin)
    {
        applyToWin = win;
    }
    ...
    // It's possible for command buttons to be a single use only type of a button -- like detonating a nuke from a convoy
    // truck.
    ...
    if (disabled && BitIsSet(command->getOptions(), IGNORES_UNDERPOWERED)
        && obj->getDisabledFlags().test(DISABLED_UNDERPOWERED) && obj->getDisabledFlags().count() == 1)
    {
        disabled = false;
    }
    ...
    if (disabled && !forceDisabledEvaluation)
    {
        ...
        if (commandType != GUI_COMMAND_SELL && commandType != GUI_COMMAND_EVACUATE
            && commandType != GUI_COMMAND_EXIT_CONTAINER && commandType != GUI_COMMAND_BEACON_DELETE
            && commandType != GUI_COMMAND_SET_RALLY_POINT && commandType != GUI_COMMAND_STOP
    ...
    if (BitIsSet(command->getOptions(), NEED_UPGRADE))
    {
        ...
        if (upgradeT)
        {
            ...
            else if (upgradeT->getUpgradeType() == UPGRADE_TYPE_OBJECT && obj->hasUpgrade(upgradeT) == FALSE)
            {
                return COMMAND_RESTRICTED;
            }
        }
    ...
    switch (command->getCommandType())
    {
        case GUI_COMMAND_DOZER_CONSTRUCT:
        {
            ...
            if (whatToBuild)
            {
                ...
                if (bStatus == BSTATUS_NO
                    || (bStatus == BSTATUS_ONLY_BY_AI && obj->getControllingPlayer()->getPlayerType() != PLAYER_COMPUTER))
                    return COMMAND_HIDDEN;
            }
        ...
        case GUI_COMMAND_FIRE_WEAPON:
        {
            ...
            // DEBUG_ASSERTLOG( w, ("Unit %s's CommandButton %s is trying to access weaponslot %d, but doesn't have a weapon
            // there in its FactionUnit ini entry.", 	obj->getTemplate()->getName().str(), command->getName().str(),
            ...
            if (w == NULL // No weapon
                ...
                || w->getPossibleNextShotFrame() == now // Weapon ready, but could fire this exact frame (handle button
                                                        // flicker since it may be going to fire anyway)
                /// @todo srj -- not sure why this next check is necessary, but the Comanche missile buttons will flicker
                /// without it. figure out someday.
                /// @todo ml  -- and note: that the "now-1" below causes zero-clip-reload weapons to never be ready, so I
                /// added this If you make changes to this code, make sure that the DragonTank's firewall weapon can be
                /// retargeted while active, that is, while the tank is squirting out flames all over the floor, you can
                /// click the firewall button (or "F"), and re-target the firewall without having to stop or move in-betwen..
                /// Thanks for reading
                || (w->getPossibleNextShotFrame() == now - 1))
            {
                if (w != NULL)
                {
                    // only draw the clock when reloading a clip, not when merely between shots, since that's usually a tiny
                    // amount of time
        ...
        case GUI_COMMAND_SPECIAL_POWER_CONSTRUCT_FROM_SHORTCUT:
        {
            // sanity
            DEBUG_ASSERTCRASH(
                command->getSpecialPowerTemplate() != NULL,
            ...
            if (mod == NULL)
            {
                ...
                DEBUG_CRASH(
                    ("Object %s does not contain special power module (%s) to execute.  Did you forget to add it to the "
                     "object INI?",
            ...
            else if (mod->isReady() == FALSE)
            {
                ...
                SpecialAbilityUpdate *spUpdate =
                    obj->findSpecialAbilityUpdate(command->getSpecialPowerTemplate()->getSpecialPowerType()))
            {
        ...
        case GUI_COMMAND_SWITCH_WEAPON:
        {
            ...
            DEBUG_ASSERTCRASH(
                w,
                ("Unit %s's CommandButton %s is trying to access weaponslot %d, but doesn't have a weapon there in its "
                 "FactionUnit ini entry.",
                 obj->getTemplate()->getName().str(),
                 command->getName().str(),
                 (Int)command->getWeaponSlot()));
