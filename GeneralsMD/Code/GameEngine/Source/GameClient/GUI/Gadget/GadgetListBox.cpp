...
{
	memcpy(&list->selections[i], &list->selections[(i + 1)], ((list->listLength - i) * sizeof(Int)));
...
//=============================================================================
WindowMsgHandledType GadgetListBoxInput(GameWindow *window, UnsignedInt msg, WindowMsgData mData1, WindowMsgData mData2)
{
	ListboxData *list = (ListboxData *)window->winGetUserData();
	WinInstanceData *instData = window->winGetInstanceData();
	...
	switch (msg)
	{
			...
			switch (mData1)
			{
				...
				case KEY_SPACE:
				{
					if (BitIsSet(mData2, KEY_STATE_UP))
					{
						...
						TheWindowManager
								->winSendSystemMsg(window->winGetOwner(), GLM_DOUBLE_CLICKED, (WindowMsgData)window, list->selectPos);
					} // end if
		...
		case GWM_LEFT_UP:
		{
					...
					&& (i == oldPos || (oldPos == -1 && (i >= 0 && i < list->endPos))))
			{
				...
				if (oldPos == -1)
					...
					temp = oldPos;
				TheWindowManager->winSendSystemMsg(window->winGetOwner(), GLM_DOUBLE_CLICKED, (WindowMsgData)window, temp);
				// break;
			}
		...
		case GWM_RIGHT_UP:
		{
			...
			rc.mouseY = mousey;
			TheWindowManager
					->winSendSystemMsg(window->winGetOwner(), GLM_RIGHT_CLICKED, (WindowMsgData)window, (WindowMsgData)&rc);
			break;
		...
		case GWM_MOUSE_ENTERING:
		{
			if (BitIsSet(instData->getStyle(), GWS_MOUSE_TRACK))
			{
				...
				TheWindowManager->winSendSystemMsg(window->winGetOwner(), GBM_MOUSE_ENTERING, (WindowMsgData)window, 0);
				// TheWindowManager->winSetFocus( window );
		...
		case GWM_MOUSE_LEAVING:
		{
			if (BitIsSet(instData->getStyle(), GWS_MOUSE_TRACK))
			{
				...
				TheWindowManager->winSendSystemMsg(window->winGetOwner(), GBM_MOUSE_LEAVING, (WindowMsgData)window, 0);
			} // end if
		...
		case GWM_LEFT_DRAG:
			...
			if (BitIsSet(instData->getStyle(), GWS_MOUSE_TRACK))
				TheWindowManager->winSendSystemMsg(window->winGetOwner(), GGM_LEFT_DRAG, (WindowMsgData)window, 0);
			break;
...
//=============================================================================
WindowMsgHandledType GadgetListBoxMultiInput(GameWindow *window, UnsignedInt msg, WindowMsgData mData1, WindowMsgData mData2)
{
	ListboxData *list = (ListboxData *)window->winGetUserData();
	WinInstanceData *instData = window->winGetInstanceData();
	...
	switch (msg)
	{
		...
		case GWM_LEFT_UP:
			// case GWM_LEFT_CLICK:
			{
				...
				TheWindowManager->winSendSystemMsg(window->winGetOwner(), GLM_SELECTED, (WindowMsgData)window, selectPos);
				break;
...
WindowMsgHandledType GadgetListBoxSystem(GameWindow *window, UnsignedInt msg, WindowMsgData mData1, WindowMsgData mData2)
{
	...
	switch (msg)
	{
		...
		case GLM_DEL_ENTRY:
		{
			...
			memcpy(&list->listData[mData1], &list->listData[(mData1 + 1)], (list->endPos - mData1 - 1) * sizeof(ListEntryRow));
		...
		case GLM_ADD_ENTRY:
		{
			...
			if (success)
			{
				...
				{
					addedIndex =
							addEntry((UnicodeString *)addInfo->data, mData2, addInfo->row, addInfo->column, window, addInfo->overwrite);
				}
				else if (addInfo->type == LISTBOX_IMAGE)
				{
					addedIndex = addImageEntry(
							(const Image *)addInfo->data,
							mData2,
							addInfo->row,
							addInfo->column,
							window,
			...
			if (success)
			{
				if (list->autoScroll)
				{
					while (TRUE)
					{
						...
						if (row == -1)
						{
							if (list->listData[(list->insertPos - 1)].listHeight >= (list->displayPos + list->displayHeight))
								adjustDisplay(window, 1, TRUE);
							else
								break;
						}
						...
						{
							if (list->listData[(row)].listHeight >= (list->displayPos + list->displayHeight))
								adjustDisplay(window, 1, TRUE);
							else
								break;
						}
		...
		case GLM_SET_SELECTION:
		{
			...
			else
			{
				...
				{
					TheWindowManager->winSendSystemMsg(window, GLM_UPDATE_DISPLAY, list->selectPos, 0);
				}
				else if (list->listData[list->selectPos].listHeight > (list->displayPos + list->displayHeight))
				{
					if (list->selectPos > 0)
		...
		case GLM_SCROLL_BUFFER:
		{
			...
			// copy the cells up
			//
			memcpy(list->listData, &list->listData[mData1], (list->endPos - mData1) * sizeof(ListEntryRow));
		...
		case GWM_INPUT_FOCUS:
		{
			...
			TheWindowManager->winSendSystemMsg(window->winGetOwner(), GGM_FOCUS_CHANGE, mData1, window->winGetWindowId());
...
Int GadgetListBoxAddEntryText(GameWindow *listbox, UnicodeString text, Color color, Int row, Int column, Bool overwrite)
{
	...
	// DEBUG_ASSERTLOG(!listData->scrollIfAtEnd, ("Adding line %d (orig end was %d, newEntryOffset is %d, (%d-%d)?=%d,
	// isFull=%d/%d ll=%d, end=%d", index, oldBottomIndex, newEntryOffset, index, oldBottomIndex, newEntryOffset, wasFull,
	// GadgetListBoxIsFull(listbox), listData->listLength, listData->endPos));
...
/** Add a new string entry into the listbox at the insert position */
//=============================================================================
Int GadgetListBoxAddEntryImage(
		GameWindow *listbox,
		const Image *image,
		Int row,
		Int column,
...
Int GadgetListBoxAddEntryImage(GameWindow *listbox, const Image *image, Int row, Int column, Bool overwrite, Color color)
{
