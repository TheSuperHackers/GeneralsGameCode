...
static Int addEntry(UnicodeString *string, Int color, Int row, Int column,
  ...
  ListEntryRow *listRow = &list->listData[row];
  // Here I've decided to just overwrite what's in the row, if that's not what
...
WindowMsgHandledType GadgetListBoxInput(GameWindow *window, UnsignedInt msg,
  ...
  case GWM_CHAR: {
    ...
    case KEY_SPACE: {
      ...
      if (BitIsSet(mData2, KEY_STATE_UP)) {
        ...
        TheWindowManager->winSendSystemMsg(
            window->winGetOwner(), GLM_DOUBLE_CLICKED, (WindowMsgData)window,
            list->selectPos);
      }
    ...
    default: {
      ...
      if (BitIsSet(mData2, KEY_STATE_DOWN)) {
        ...
        // only search the max number of times so that we're not looping over
        // and over
        for (Int i = 0; i < list->endPos; ++i) {
          ...
          for (j = 0; j < TheKeyboard->MAX_KEY_STATES; ++j) {
            if (dString->getText().getCharAt(0) ==
                TheKeyboard->getPrintableKey((KeyDefType)mData1, j)) {
  ...
  case GWM_RIGHT_DOWN:
    doAudioFeedback(window);
    break; // if we're in game, we want to eat this message because we're right
  ...
  case GWM_RIGHT_UP: {
    ...
    rc.mouseX = mousex;
    rc.mouseY = mousey;
    TheWindowManager->winSendSystemMsg(window->winGetOwner(), GLM_RIGHT_CLICKED,
                                       (WindowMsgData)window,
                                       (WindowMsgData)&rc);
    break;
  }
    ...
    if (success) {
      if (addInfo->type == LISTBOX_TEXT) {
        ...
        addedIndex = addImageEntry(
            (const Image *)addInfo->data, mData2, addInfo->row, addInfo->column,
            window, addInfo->overwrite, addInfo->width, addInfo->height);
      } else
        success = FALSE;
    }
...
Int GadgetListBoxAddEntryText(GameWindow *listbox, UnicodeString text,
  ...
  // %d, newEntryOffset is %d, (%d-%d)?=%d, isFull=%d/%d ll=%d, end=%d", index,
  // oldBottomIndex, newEntryOffset, index, oldBottomIndex, newEntryOffset,
  // wasFull, GadgetListBoxIsFull(listbox), listData->listLength,
  // listData->endPos));
...
void GadgetListboxCreateScrollbar(GameWindow *listbox) {
  ...
  listData->downButton = TheWindowManager->gogoGadgetPushButton(
      ...
      buttonHeight, &winInstData, nullptr, TRUE);
...
void GadgetListBoxAddMultiSelect(GameWindow *listbox) {
  ...
  DEBUG_ASSERTCRASH(listboxData && listboxData->selections == nullptr,
