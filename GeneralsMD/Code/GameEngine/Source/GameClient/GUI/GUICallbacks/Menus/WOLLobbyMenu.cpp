			...
			switch (resp.peerResponseType)
			{
				...
				case PeerResponse::PEERRESPONSE_STAGINGROOM:
				{
					...
					switch (resp.stagingRoom.action)
					{
						case PEER_CLEAR:
							TheGameSpyInfo->clearStagingRoomList();
							// TheGameSpyInfo->addText( UnicodeString(L"gameList: PEER_CLEAR"), GameSpyColor[GSCOLOR_DEFAULT],
							// listboxLobbyChat );
							break;
						case PEER_ADD:
						case PEER_UPDATE:
						{
							...
							if (serverOk)
							{
								...
								if (resp.stagingRoom.action == PEER_ADD)
								{
									...
									// TheGameSpyInfo->addText( UnicodeString(L"gameList: PEER_ADD"), GameSpyColor[GSCOLOR_DEFAULT],
									// listboxLobbyChat );
								}
								else
								{
									TheGameSpyInfo->updateStagingRoom(room);
									// TheGameSpyInfo->addText( UnicodeString(L"gameList: PEER_UPDATE"), GameSpyColor[GSCOLOR_DEFAULT],
									// listboxLobbyChat );
								}
							}
							else
							{
								...
								// TheGameSpyInfo->addText( UnicodeString(L"gameList: PEER_UPDATE FAILED"), GameSpyColor[GSCOLOR_DEFAULT],
								// listboxLobbyChat );
							}
							break;
						}
						case PEER_REMOVE:
							room.setID(resp.stagingRoom.id);
							TheGameSpyInfo->removeStagingRoom(room);
							// TheGameSpyInfo->addText( UnicodeString(L"gameList: PEER_REMOVE"), GameSpyColor[GSCOLOR_DEFAULT],
							// listboxLobbyChat );
							break;
						default:
					...
					if (selectedID > 0)
					{
						...
						if (srmIt != srm->end())
						{
									...
									|| roomToJoin->getIniCRC() != TheGlobalData->m_iniCRC)
							{
								// bad crc.  don't go.
								DEBUG_LOG(
										("WOLLobbyMenuSystem - CRC mismatch with the game I'm trying to join. My CRC's - EXE:0x%08X INI:0x%08X  "
										 "Their CRC's - EXE:0x%08x INI:0x%08x",
