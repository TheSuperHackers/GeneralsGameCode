...
static NameKeyType comboBoxPlayerID[MAX_SLOTS] = { NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID,
                                                   NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID };

static NameKeyType comboBoxColorID[MAX_SLOTS] = { NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID,
                                                  NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID };

static NameKeyType comboBoxPlayerTemplateID[MAX_SLOTS] = { NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID,
                                                           NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID,
                                                           NAMEKEY_INVALID, NAMEKEY_INVALID };

static NameKeyType comboBoxTeamID[MAX_SLOTS] = { NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID,
                                                 NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID, NAMEKEY_INVALID };
...
static GameWindow *comboBoxPlayer[MAX_SLOTS] = { NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL };

static GameWindow *comboBoxColor[MAX_SLOTS] = { NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL };

static GameWindow *comboBoxPlayerTemplate[MAX_SLOTS] = { NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL };

static GameWindow *comboBoxTeam[MAX_SLOTS] = { NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL };
...
Bool SkirmishPreferences::write(void)
{
  ...
  //	NameKeyType sliderGameSpeedID = TheNameKeyGenerator->nameToKey( AsciiString(
...
void CheckForCDAtGameStart(gameStartCallback callback)
{
  if (!IsFirstCDPresent())
  {
    ...
    ExMessageBoxOkCancel(
        TheGameText->fetch("GUI:InsertCDPrompt"),
...
/////////////////////////////////////////////////////
void MapSelectorTooltip(GameWindow *window, WinInstanceData *instData, UnsignedInt mouse)
{
  Int x, y;
...
void positionStartSpotControls(
    GameWindow *win,
    ...
    GameWindow *buttonMapStartPositions[])
{
  ...
  for (Int i = 0; i < MAX_SLOTS; ++i)
  {
    ...
    if (gadgetPos.x > tempPos.x && gadgetPos.x < tempPos.x + gadgetSize.x && gadgetPos.y > tempPos.y
        && gadgetPos.y < tempPos.y + gadgetSize.y)
    {
...
void positionAdditionalImages(MapMetaData *mmd, GameWindow *mapWindow, Bool force)
{
  ...
  while (it != mmd->m_supplyPositions.end())
  {
    ...
    // When we actually draw the map, save off it's screen position and use that instead of the map window's
    // position/size
    Real position;
  ...
  else
  {
    ...
    for (; i < mmd.m_numPlayers && mmd.m_isMultiplayer; ++i)
    {
      ...
      else
      {
        DEBUG_ASSERTCRASH(
            FALSE,
            ("positionStartSpots:: someone messed with the map cash.  We couldn't find waypoint <%s> in map <%s>",
             waypointName.str(),
             lowerMap.str()));
      }
    }
...
void updateMapStartSpots(GameInfo *myGame, GameWindow *buttonMapStartPositions[], Bool onLoadScreen)
{
  ...
  for (i = 0; i < MAX_SLOTS; ++i)
  {
    ...
    if (onLoadScreen)
    {
      if (gs->getApparentStartPos() >= 0 && gs->getApparentStartPos() < mmd.m_numPlayers
...
void InitSkirmishGameGadgets(void)
{
  ...
  checkBoxLimitSuperweaponsID =
      TheNameKeyGenerator->nameToKey(AsciiString("SkirmishGameOptionsMenu.wnd:CheckboxLimitSuperweapons"));
  ...
  for (; i < MAX_SLOTS; i++)
  {
    ...
    //		buttonStartPosition[i] = TheWindowManager->winGetWindowFromId( parentSkirmishGameOptions,
    // buttonStartPositionID[i] ); 		DEBUG_ASSERTCRASH(buttonStartPosition[i], ("Could not find the
...
void updateSkirmishGameOptions(void)
{
  ...
  DEBUG_ASSERTCRASH(
      index < itemCount,
      ("Could not find new starting cash amount %d in list", TheSkirmishGameInfo->getStartingCash().countMoney()));
}
...
void SkirmishGameOptionsMenuInit(WindowLayout *layout, void *userData)
{
  ...
  GameWindow *win =
      TheWindowManager->winGetWindowFromId(NULL, TheNameKeyGenerator->nameToKey("SkirmishGameOptionsMenu.wnd:SubParent"));
  if (win)
    win->winHide(TRUE);
  buttonPushed = FALSE;
} // void SkirmishGameOptionsMenuInit( WindowLayout *layout, void *userData )
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType SkirmishGameOptionsMenuInput(
    GameWindow *window,
    UnsignedInt msg,
    ...
    case GWM_CHAR:
    {
      ...
      switch (key)
      {
        ...
        case KEY_ESC:
        {
          ...
          {
            TheWindowManager->winSendSystemMsg(window, GBM_SELECTED, (WindowMsgData)buttonExit, buttonExitID);
          } // end if
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType SkirmishGameOptionsMenuSystem(
    GameWindow *window,
    UnsignedInt msg,
    WindowMsgData mData1,
    WindowMsgData mData2)
{
  ...
  switch (msg)
  {
    ...
    case GBM_SELECTED:
    {
      GameWindow *control = (GameWindow *)mData1;
      Int controlID = control->winGetWindowId();
      ///				static NameKeyType buttonResetFPSID = TheNameKeyGenerator->nameToKey( AsciiString(
      ...
      //				{
      //					static NameKeyType sliderGameSpeedID = TheNameKeyGenerator->nameToKey( AsciiString(
      //"SkirmishGameOptionsMenu.wnd:SliderGameSpeed" ) ); 					GameWindow *sliderGameSpeed =
      // TheWindowManager->winGetWindowFromId( parentSkirmishGameOptions, sliderGameSpeedID ); GadgetSliderSetPosition(
...
void populateSkirmishBattleHonors(void)
{
  ...
  UnicodeString uStr;
  GameWindow *streakWindow =
      TheWindowManager->winGetWindowFromId(NULL, NAMEKEY("SkirmishGameOptionsMenu.wnd:StaticTextStreakValue"));
  if (streakWindow)
  {
    uStr.format(L"%d", stats.getWinStreak());
    GadgetStaticTextSetText(streakWindow, uStr);
  }
  GameWindow *bestStreakWindow =
      TheWindowManager->winGetWindowFromId(NULL, NAMEKEY("SkirmishGameOptionsMenu.wnd:StaticTextBestStreakValue"));
  if (bestStreakWindow)
  {
    uStr.format(L"%d", stats.getBestWinStreak());
    GadgetStaticTextSetText(bestStreakWindow, uStr);
  }
  GameWindow *winsWindow =
      TheWindowManager->winGetWindowFromId(NULL, NAMEKEY("SkirmishGameOptionsMenu.wnd:StaticTextWinsValue"));
  if (winsWindow)
  {
    uStr.format(L"%d", stats.getWins());
    GadgetStaticTextSetText(winsWindow, uStr);
  }
  GameWindow *lossesWindow =
      TheWindowManager->winGetWindowFromId(NULL, NAMEKEY("SkirmishGameOptionsMenu.wnd:StaticTextLossesValue"));
  if (lossesWindow)
  {
  ...
  if (stats.getCHINACampaignComplete(DIFFICULTY_HARD))
  {
    InsertBattleHonor(
        list,
        TheMappedImageCollection->findImageByName("ChinaCampaign_G"),
        TRUE,
  ...
  else if (stats.getCHINACampaignComplete(DIFFICULTY_NORMAL))
  {
    InsertBattleHonor(
        list,
        TheMappedImageCollection->findImageByName("ChinaCampaign_S"),
        TRUE,
        BATTLE_HONOR_CAMPAIGN_CHINA,
        row,
        column);
  ...
  else if (stats.getCHINACampaignComplete(DIFFICULTY_EASY))
  {
    InsertBattleHonor(
        list,
        TheMappedImageCollection->findImageByName("ChinaCampaign_B"),
        TRUE,
        BATTLE_HONOR_CAMPAIGN_CHINA,
        row,
        column);
  ...
  else
  {
    InsertBattleHonor(
        list,
        TheMappedImageCollection->findImageByName("ChinaCampaign_B"),
        (honors & BATTLE_HONOR_CAMPAIGN_CHINA),
        BATTLE_HONOR_CAMPAIGN_CHINA,
        row,
        column);
  }
  ...
  if (stats.getGLACampaignComplete(DIFFICULTY_HARD))
  {
    InsertBattleHonor(
        list,
        TheMappedImageCollection->findImageByName("GLACampaign_G"),
        TRUE,
  ...
  else if (stats.getGLACampaignComplete(DIFFICULTY_NORMAL))
  {
    InsertBattleHonor(
        list,
        TheMappedImageCollection->findImageByName("GLACampaign_S"),
        TRUE,
        BATTLE_HONOR_CAMPAIGN_GLA,
        row,
        column);
  ...
  else if (stats.getGLACampaignComplete(DIFFICULTY_EASY))
  {
    InsertBattleHonor(
        list,
        TheMappedImageCollection->findImageByName("GLACampaign_B"),
        TRUE,
        BATTLE_HONOR_CAMPAIGN_GLA,
        row,
        column);
  ...
  else
  {
    InsertBattleHonor(
        list,
        TheMappedImageCollection->findImageByName("GLACampaign_B"),
        (honors & BATTLE_HONOR_CAMPAIGN_GLA),
        BATTLE_HONOR_CAMPAIGN_GLA,
        row,
        column);
  }
  ...
  if (stats.getUSACampaignComplete(DIFFICULTY_HARD))
  {
    InsertBattleHonor(
        list,
        TheMappedImageCollection->findImageByName("USACampaign_G"),
        TRUE,
  ...
  else if (stats.getUSACampaignComplete(DIFFICULTY_NORMAL))
  {
    InsertBattleHonor(
        list,
        TheMappedImageCollection->findImageByName("USACampaign_S"),
        TRUE,
        BATTLE_HONOR_CAMPAIGN_USA,
        row,
        column);
  ...
  else if (stats.getUSACampaignComplete(DIFFICULTY_EASY))
  {
    InsertBattleHonor(
        list,
        TheMappedImageCollection->findImageByName("USACampaign_B"),
        TRUE,
        BATTLE_HONOR_CAMPAIGN_USA,
        row,
        column);
  ...
  else
  {
    InsertBattleHonor(
        list,
        TheMappedImageCollection->findImageByName("USACampaign_B"),
        (honors & BATTLE_HONOR_CAMPAIGN_USA),
        BATTLE_HONOR_CAMPAIGN_USA,
        row,
        column);
  }
  ...
  if (completedOnHard)
  {
    InsertBattleHonor(
        list,
        TheMappedImageCollection->findImageByName("Challenge_Gold"),
        TRUE,
  ...
  else if (completedOnNormal)
  {
    InsertBattleHonor(
        list,
        TheMappedImageCollection->findImageByName("Challenge_Silver"),
        TRUE,
        BATTLE_HONOR_CHALLENGE_MODE,
        row,
        column);
  ...
  else if (completedOnEasy)
  {
    InsertBattleHonor(
        list,
        TheMappedImageCollection->findImageByName("Challenge_Bronz"),
        TRUE,
        BATTLE_HONOR_CHALLENGE_MODE,
        row,
        column);
  ...
  InsertBattleHonor(
      list,
      TheMappedImageCollection->findImageByName("HonorBattleTank"),
  ...
  if (streak >= 1000)
  {
    InsertBattleHonor(
        list,
        TheMappedImageCollection->findImageByName("HonorStreak_1000"),
        TRUE,
        BATTLE_HONOR_STREAK,
        row,
        column,
  ...
  else if (streak >= 500)
  {
    InsertBattleHonor(
        list,
        TheMappedImageCollection->findImageByName("HonorStreak_500"),
        TRUE,
        BATTLE_HONOR_STREAK,
        row,
        column,
  ...
  else if (streak >= 100)
  {
    InsertBattleHonor(
        list,
        TheMappedImageCollection->findImageByName("HonorStreak_100"),
        TRUE,
        BATTLE_HONOR_STREAK,
        row,
        column,
  ...
  else if (streak >= 25)
  {
    InsertBattleHonor(
        list,
        TheMappedImageCollection->findImageByName("HonorStreak_G"),
        TRUE,
        BATTLE_HONOR_STREAK,
        row,
        column,
  ...
  else if (streak >= 10)
  {
    InsertBattleHonor(
        list,
        TheMappedImageCollection->findImageByName("HonorStreak_S"),
        TRUE,
        BATTLE_HONOR_STREAK,
        row,
        column,
  ...
  else if (streak >= 3)
  {
    InsertBattleHonor(
        list,
        TheMappedImageCollection->findImageByName("HonorStreak_B"),
        TRUE,
        BATTLE_HONOR_STREAK,
        row,
        column,
  ...
  if (totalWins >= 10000)
  {
    InsertBattleHonor(
        list,
        TheMappedImageCollection->findImageByName("Domination_10000"),
        TRUE,
  ...
  else if (totalWins >= 1000)
  {
    InsertBattleHonor(
        list,
        TheMappedImageCollection->findImageByName("Domination_1000"),
        TRUE,
        BATTLE_HONOR_DOMINATION,
        row,
        column,
  ...
  else if (totalWins >= 500)
  {
    InsertBattleHonor(
        list,
        TheMappedImageCollection->findImageByName("Domination_500"),
        TRUE,
        BATTLE_HONOR_DOMINATION,
        row,
        column,
  ...
  else if (totalWins >= 100)
  {
    InsertBattleHonor(
        list,
        TheMappedImageCollection->findImageByName("Domination_100"),
        TRUE,
        BATTLE_HONOR_DOMINATION,
        row,
        column,
  ...
  InsertBattleHonor(
      list,
      TheMappedImageCollection->findImageByName("Ultimate"),
      perfect,
  ...
  InsertBattleHonor(list, TheMappedImageCollection->findImageByName("Loyalty_China"), (honors &
  BATTLE_HONOR_LOYALTY_CHINA), BATTLE_HONOR_LOYALTY_CHINA, row, column); InsertBattleHonor(list,
  TheMappedImageCollection->findImageByName("Loyalty_GLA"), (honors & BATTLE_HONOR_LOYALTY_GLA), BATTLE_HONOR_LOYALTY_GLA,
  row, column);
  */
