    ...
    for (Int i = 0; i < stepsPerUpdate; i++)
    {
        ...
        if (pos < NUM_GENERALS && !buttonGeneralPosition[pos]->winIsHidden())
        {
            ...
            if (playerTemplate)
                GadgetCheckBoxSetEnabledImage(
                    buttonGeneralPosition[pos],
                    TheMappedImageCollection->findImageByName(playerTemplate->getMedallionSelected()));
        }
        ...
        if (--pos > 0 && pos < NUM_GENERALS && !buttonGeneralPosition[pos]->winIsHidden())
        {
            ...
            if (playerTemplate)
                GadgetCheckBoxSetEnabledImage(
                    buttonGeneralPosition[pos],
                    TheMappedImageCollection->findImageByName(playerTemplate->getMedallionHilite()));
        }
        ...
        if (--pos > 0 && pos < NUM_GENERALS && !buttonGeneralPosition[pos]->winIsHidden())
        {
            ...
            if (playerTemplate)
                GadgetCheckBoxSetEnabledImage(
                    buttonGeneralPosition[pos],
                    TheMappedImageCollection->findImageByName(playerTemplate->getMedallionNormal()));
        }
    ...
    for (Int i = 0; i < frames; i++)
    {
        if (bioTextPosition < bioTotalLength)
        {
            ...
            else
            {
                ...
                wChar =
                    bioLine4.getCharAt(bioTextPosition - bioLine1.getLength() - bioLine2.getLength() - bioLine3.getLength());
...
void ChallengeMenuInit(WindowLayout *layout, void *userData)
{
    ...
    bioLine3Entry =
        TheWindowManager->winGetWindowFromId(parentMenu, bioBirthplaceEntryID); // this window has been repurposed
    ...
    GameWindow *winGadgetParent =
        TheWindowManager->winGetWindowFromId(NULL, TheNameKeyGenerator->nameToKey("ChallengeMenu.wnd:GadgetParent"));
    if (winGadgetParent)
        ...
        case GBM_SELECTED:
        {
            ...
            if (buttonIndex != -1)
            {
                ...
                if (lastButtonIndex != -1)
                {
                    ...
                    GameWindow *lastControl =
                        TheWindowManager->winGetWindowFromId(NULL, buttonGeneralPositionID[lastButtonIndex]);
