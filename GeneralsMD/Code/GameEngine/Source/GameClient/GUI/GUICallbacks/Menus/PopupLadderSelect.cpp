...
WindowMsgHandledType PopupLadderSelectSystem(GameWindow *window, UnsignedInt msg, WindowMsgData mData1, WindowMsgData mData2)
{
    switch (msg)
    {
        ...
        case GBM_SELECTED:
        {
            ...
            else if (controlID == buttonPasswordOkID)
            {
                ...
                if (pass.isNotEmpty()) // password ok
                {
                    ...
                    DEBUG_LOG(
                        ("pass is %s, crypted pass is %s, comparing to %s",
                         pass.str(),
                         cryptPass.str(),
                         li->cryptedPassword.str()));
        ...
        case GLM_DOUBLE_CLICKED:
        {
            ...
            {
                TheWindowManager
                    ->winSendSystemMsg(parent, GBM_SELECTED, (WindowMsgData)buttonOk, buttonOk->winGetWindowId());
            }
            break;
        }
                ...
                if (srmIt != srm->end())
                {
                    ...
                    if (linfo)
                    {
                        WindowLayout *rcLayout =
                            TheWindowManager->winCreateLayout(AsciiString("Menus/PopupLadderDetails.wnd"));
                        if (!rcLayout)
                            break;
