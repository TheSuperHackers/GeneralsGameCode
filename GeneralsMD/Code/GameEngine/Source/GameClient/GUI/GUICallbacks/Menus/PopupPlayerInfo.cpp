...
void BattleHonorTooltip(GameWindow *window, WinInstanceData *instData, UnsignedInt mouse)
{
    ...
    if (BitIsSet(battleHonor, BATTLE_HONOR_NOT_GAINED))
    {
        if (BitIsSet(battleHonor, BATTLE_HONOR_LOYALTY_USA))
            ...
            TheMouse
                ->setCursorTooltip(TheGameText->fetch("TOOLTIP:BattleHonorLoyaltyChinaDisabled"), -1, NULL, tooltipWidth);
            ...
            TheMouse
                ->setCursorTooltip(TheGameText->fetch("TOOLTIP:BattleHonorCampaignChinaDisabled"), -1, NULL, tooltipWidth);
            ...
            TheMouse->setCursorTooltip(
                TheGameText->fetch("TOOLTIP:BattleHonorCampaignChallengeDisabled"),
                -1,
                NULL,
                tooltipWidth);
            ...
            TheMouse
                ->setCursorTooltip(TheGameText->fetch("TOOLTIP:BattleHonorGlobalGeneralDisabled"), -1, NULL, tooltipWidth);
            ...
            TheMouse
                ->setCursorTooltip(TheGameText->fetch("TOOLTIP:BattleHonorStreakOnlineDisabled"), -1, NULL, tooltipWidth);
            ...
            TheMouse->setCursorTooltip(
                TheGameText->fetch("TOOLTIP:BattleHonorDominationOnlineDisabled"),
                -1,
                NULL,
                tooltipWidth);
    }
    else
    {
        ...
        else if (BitIsSet(battleHonor, BATTLE_HONOR_STREAK_ONLINE))
        {
            if (extraValue >= 1000)
                TheMouse
                    ->setCursorTooltip(TheGameText->fetch("TOOLTIP:BattleHonorStreak1000Online"), -1, NULL, tooltipWidth);
            else if (extraValue >= 500)
                ...
                TheMouse->setCursorTooltip(
                    TheGameText->fetch("TOOLTIP:BattleHonorStreakOnlineDisabled"),
                    -1,
                    NULL,
                    tooltipWidth);
        }
        else if (BitIsSet(battleHonor, BATTLE_HONOR_DOMINATION))
        {
            if (extraValue >= 10000)
                ...
                TheMouse
                    ->setCursorTooltip(TheGameText->fetch("TOOLTIP:BattleHonorDominationDisabled"), -1, NULL, tooltipWidth);
        }
        else if (BitIsSet(battleHonor, BATTLE_HONOR_DOMINATION_ONLINE))
        {
            if (extraValue >= 10000)
                TheMouse->setCursorTooltip(
                    TheGameText->fetch("TOOLTIP:BattleHonorDomination10000Online"),
                    -1,
                    NULL,
                    tooltipWidth);
            else if (extraValue >= 1000)
                TheMouse->setCursorTooltip(
                    TheGameText->fetch("TOOLTIP:BattleHonorDomination1000Online"),
                    -1,
                    NULL,
                    tooltipWidth);
            else if (extraValue >= 500)
                TheMouse
                    ->setCursorTooltip(TheGameText->fetch("TOOLTIP:BattleHonorDomination500Online"), -1, NULL, tooltipWidth);
            else if (extraValue >= 100)
                TheMouse
                    ->setCursorTooltip(TheGameText->fetch("TOOLTIP:BattleHonorDomination100Online"), -1, NULL, tooltipWidth);
            else
                TheMouse->setCursorTooltip(
                    TheGameText->fetch("TOOLTIP:BattleHonorDominationOnlineDisabled"),
                    -1,
...
static void populateBattleHonors(
    const PSPlayerStats &stats,
    Int battleHonors,
    Int gamesInRow,
    Int lastGen,
    Int challenge,
    GameWindow *list)
{
    ...
    InsertBattleHonor(
        list,
        TheMappedImageCollection->findImageByName("FairPlay"),
        isFairPlayer,
        BATTLE_HONOR_FAIR_PLAY,
        row,
        column);
    ...
    InsertBattleHonor(
        ...
        BitIsSet(battleHonors, BATTLE_HONOR_AIR_WING),
    ...
    InsertBattleHonor(
        ...
        BitIsSet(battleHonors, BATTLE_HONOR_BATTLE_TANK),
    ...
    InsertBattleHonor(
        ...
        BitIsSet(battleHonors, BATTLE_HONOR_APOCALYPSE),
    ...
    if (totalWins >= 10000)
    {
        InsertBattleHonor(
            ...
            TRUE,
            BATTLE_HONOR_DOMINATION_ONLINE,
            row,
            column,
            uStr,
            totalWins);
    ...
    InsertBattleHonor(
        list,
        TheMappedImageCollection->findImageByName("GlobalGen"),
        BitIsSet(battleHonors, BATTLE_HONOR_GLOBAL_GENERAL),
    ...
    /*
    Bool isLoyal;
    isLoyal = ThePlayerTemplateStore->getNthPlayerTemplate(lastGen)->getSide().compareNoCase( "america") == 0 && gamesInRow
    >= 20; InsertBattleHonor(list, TheMappedImageCollection->findImageByName("Loyalty_USA"), isLoyal,
        BATTLE_HONOR_LOYALTY_USA, row, column);
    isLoyal = ThePlayerTemplateStore->getNthPlayerTemplate(lastGen)->getSide().compareNoCase( "china") == 0 && gamesInRow >=
    20; InsertBattleHonor(list, TheMappedImageCollection->findImageByName("Loyalty_China"), isLoyal,
        BATTLE_HONOR_LOYALTY_CHINA, row, column);
    isLoyal = ThePlayerTemplateStore->getNthPlayerTemplate(lastGen)->getSide().compareNoCase( "gla") == 0 && gamesInRow >=
    20; InsertBattleHonor(list, TheMappedImageCollection->findImageByName("Loyalty_GLA"), isLoyal, BATTLE_HONOR_LOYALTY_GLA,
    row, column);
        */

    // insertBattleHonor(list, TheMappedImageCollection->findImageByName("Endurance"), BitIsSet(battleHonors,
    // BATTLE_HONOR_ENDURANCE), BATTLE_HONOR_ENDURANCE, row, column);

    /*
    InsertBattleHonor(list, TheMappedImageCollection->findImageByName("Campaign_USA"), BitIsSet(battleHonors,
    BATTLE_HONOR_CAMPAIGN_USA), BATTLE_HONOR_CAMPAIGN_USA, row, column); InsertBattleHonor(list,
    TheMappedImageCollection->findImageByName("Campaign_China"), BitIsSet(battleHonors, BATTLE_HONOR_CAMPAIGN_CHINA),
        BATTLE_HONOR_CAMPAIGN_CHINA, row, column);
    InsertBattleHonor(list, TheMappedImageCollection->findImageByName("Campaign_GLA"), BitIsSet(battleHonors,
    BATTLE_HONOR_CAMPAIGN_GLA), BATTLE_HONOR_CAMPAIGN_GLA, row, column);
    */
...
void PopulatePlayerInfoWindows(AsciiString parentWindowName)
{
    ...
    if (win)
    {
        ...
        uStr.format(
            TheGameText->fetch("GUI:PlayerStatistics"),
            lookAtPlayerName.c_str(),
            TheGameText->fetch(localeID).str());
    ...
    if (win && TheRankPointValues)
    {
        ...
        else
        {
            GadgetProgressBarSetProgress(
                win,
                100 * INT_TO_REAL(rankPoints - TheRankPointValues->m_ranks[currentRank])
                    / (TheRankPointValues->m_ranks[currentRank + 1] - TheRankPointValues->m_ranks[currentRank]));
        }
    }
            ...
            switch (resp.responseType)
            {
                ...
                case PSResponse::PSRESPONSE_PLAYERSTATS:
                {
                    ...
                    */
                    if (resp.player.id == TheGameSpyInfo->getLocalProfileID())
                    {
                        ...
                        DEBUG_LOG(
                            ("PEERREQUEST_PUSHSTATS: stats will be %d,%d,%d,%d,%d,%d",
                             req.statsToPush.locale,
                             req.statsToPush.wins,
                             req.statsToPush.losses,
                             req.statsToPush.rankPoints,
                             req.statsToPush.side,
                             req.statsToPush.preorder));
                    ...
                    DEBUG_LOG(
                        ("PopulatePlayerInfoWindows() - lookAtPlayerID is %d, got %d", lookAtPlayerID, resp.player.id));
                    PopulatePlayerInfoWindows("PopupPlayerInfo.wnd");
                    // GadgetListBoxAddEntryText(listboxInfo, UnicodeString(L"Got info!"), GameSpyColor[GSCOLOR_DEFAULT],
                    // -1);
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType GameSpyPlayerInfoOverlayInput(
    GameWindow *window,
    UnsignedInt msg,
        ...
        case GWM_CHAR:
        {
            ...
            switch (key)
            {
                ...
                case KEY_ESC:
                {
                    ...
                    {
                        TheWindowManager->winSendSystemMsg(window, GBM_SELECTED, (WindowMsgData)buttonClose, buttonCloseID);
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType GameSpyPlayerInfoOverlaySystem(
    GameWindow *window,
    UnsignedInt msg,
    WindowMsgData mData1,
    WindowMsgData mData2)
    ...
    switch (msg)
    {
        ...
        case GBM_SELECTED:
        {
            ...
            else if (controlID == buttonDeleteAccountID)
            {
                ...
                GameSpyCloseOverlay(GSOVERLAY_PLAYERINFO);
                MessageBoxYesNo(
                    TheGameText->fetch("GUI:DeleteAccount"),
