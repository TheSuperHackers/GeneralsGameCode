...
void initialHide(void)
{
    ...
    if (win)
        win->winHide(TRUE);
    win =
        TheWindowManager->winGetWindowFromId(parentMainMenu, TheNameKeyGenerator->nameToKey("MainMenu.wnd:WinFactionChina"));
    if (win)
        win->winHide(TRUE);
...
void MainMenuInit(WindowLayout *layout, void *userData)
{
    ...
    instData.setTooltipText(UnicodeString(L"Only Used in Debug and Internal!"));
    buttonCampaign =
        TheWindowManager->gogoGadgetPushButton(parentMainMenu, WIN_STATUS_ENABLED, 25, 54, 180, 26, &instData, NULL, TRUE);
#endif
    ...
    */
    ...
    GameWindow *rule = TheWindowManager->winGetWindowFromId(
        parentMainMenu,
        TheNameKeyGenerator->nameToKey(AsciiString("MainMenu.wnd:MainMenuRuler")));
    if (rule)
        rule->winHide(TRUE);
...
void DeclineResolution()
{
    ...
    if (TheDisplay
            ->setDisplayMode(oldDispSettings.xRes, oldDispSettings.yRes, oldDispSettings.bitDepth, oldDispSettings.windowed))
    {
        dispChanged = FALSE;
...
void MainMenuUpdate(WindowLayout *layout, void *userData)
{
    ...
    */
    ...
    //		pendingDropDown = DROPDOWN_NONE;
    //	}
    //	else if(localAnimateWindowManager && dropDown == DROPDOWN_NONE && pendingDropDown == DROPDOWN_NONE &&
    //localAnimateWindowManager->isReversed() && localAnimateWindowManager->isFinished())
    //	{
    //		localAnimateWindowManager->reset();
    ...
    switch (msg)
    {
        ...
        case GBM_SELECTED:
        {
            ...
            else if (control == buttonCompressTest)
            {
                ...
                if (TheTransitionHandler->isFinished() && controlID != buttonEasyID && controlID != buttonMediumID
                    && controlID != buttonHardID)
                {
            ...
            else if (controlID == worldBuilderID)
            {
                ...
                if (_spawnl(_P_NOWAIT, "WorldBuilderD.exe", "WorldBuilderD.exe", NULL) < 0)
                    MessageBoxOk(
                        TheGameText->fetch("GUI:WorldBuilder"),
                        TheGameText->fetch("GUI:WorldBuilderLoadFailed"),
                        NULL);
#else
            ...
            {
                // If we ever want to add a dialog before we exit out of the game, uncomment this line and kill the
                // quitCallback() line below.
                // #if defined(RTS_DEBUG)
            ...
            /*			else if(controlID == buttonTRAININGID)
                        {
                            ...
                            GameWindow *win = TheWindowManager->winGetWindowFromId(parentMainMenu,
            TheNameKeyGenerator->nameToKey("MainMenu.wnd:WinFactionTraining")); if(win) win->winHide(TRUE);
                            TheTransitionHandler->reverse("MainMenuSinglePlayerMenuBackTraining");
                            TheTransitionHandler->setGroup("MainMenuDifficultyMenuTraining");
                            campaignSelected = TRUE;
                            showLogo = FALSE;
