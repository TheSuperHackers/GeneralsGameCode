...
AsciiString GameSpyLoginPreferences::getDateForEmail(
		AsciiString email,
		AsciiString &month,
		AsciiString &date,
		AsciiString &year)
...
void WOLLoginMenuInit(WindowLayout *layout, void *userData)
{
	...
	{
		TheWindowManager->winSendSystemMsg(parentWOLLogin, GBM_SELECTED, (WindowMsgData)buttonTOS, buttonTOSID);
	}
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType WOLLoginMenuInput(GameWindow *window, UnsignedInt msg, WindowMsgData mData1, WindowMsgData mData2)
{
	switch (msg)
	{
		// --------------------------------------------------------------------------------------------
...
#define DATE_BUFFER_SIZE 256
	...
	GetDateFormat(LOCALE_SYSTEM_DEFAULT, 0, NULL, "yyyy", dateBuffer, DATE_BUFFER_SIZE);
	...
	GetDateFormat(LOCALE_SYSTEM_DEFAULT, 0, NULL, "MM", dateBuffer, DATE_BUFFER_SIZE);
	...
	GetDateFormat(LOCALE_SYSTEM_DEFAULT, 0, NULL, "dd", dateBuffer, DATE_BUFFER_SIZE);
	sysVal = atoi(dateBuffer);
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType WOLLoginMenuSystem(GameWindow *window, UnsignedInt msg, WindowMsgData mData1, WindowMsgData mData2)
{
			...
			else if (controlID == buttonCreateAccountID)
			{
			 ...
			 // actually attempt to create an account based on info entered
					...
					if (!email.isEmpty() && !login.isEmpty() && !password.isEmpty())
					{
						...
						DEBUG_LOG(
								("before create: TheGameSpyInfo->stuff(%s/%s/%s)",
			...
			else if (controlID == buttonLoginID)
			{
				...
				if (GameSpyUseProfiles)
				{
					...
					if (!email.isEmpty() && !login.isEmpty() && !password.isEmpty())
					{
						...
						TheGameSpyInfo->setLocalPassword(password);
						DEBUG_LOG(
								("before login: TheGameSpyInfo->stuff(%s/%s/%s)",
								 TheGameSpyInfo->getLocalBaseName().str(),
								 TheGameSpyInfo->getLocalEmail().str(),
								 TheGameSpyInfo->getLocalPassword().str()));
