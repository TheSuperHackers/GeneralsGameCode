...
*/
	...
	if (screenType == SCORESCREEN_INTERNET)
	{
		...
		if (TheGameSpyGame && !TheGameSpyGame->getUseStats() && !TheGameSpyGame->isQMGame()) // QuickMatch games always record
																																												 // stats
		...
		if (localID)
		{
			...
			if (player->isLocalPlayer())
			{
				...
				if (localSlot)
				{
					...
					DEBUG_LOG(
							("Game ended on frame %d - TheGameLogic->getFrame()=%d", lastFrameOfGame - 1, TheGameLogic->getFrame() - 1));
					...
					if (gameEndedInDisconnect)
					{
						if (latestHumanInGame == TheNetwork->getPingFrame())
						{
							...
							if (TheNetwork->getPingsRecieved() < max(1, TheNetwork->getPingsSent() / 2)) /// @todo: what's a good percent
																																													 /// of pings to have gotten?
							{
					...
					DEBUG_LOG(
							("After game built scud:%d, cannon:%d, nuke:%d", stats.builtSCUD, stats.builtParticleCannon, stats.builtNuke));
					...
					if (!TheNetwork->sawCRCMismatch() && !gameEndedInDisconnect && !TheVictoryConditions->isLocalAlliedDefeat()
...
void grabSinglePlayerInfo(void)
{
	...
	if (localPlayer)
	{
		...
		{
			const Image *image =
					TheMappedImageCollection->findImageByName(ThePlayerList->getLocalPlayer()->getPlayerTemplate()->getScoreScreen());
			if (image)
			{
				parent->winSetEnabledImage(0, image);
				parent->winSetStatus(parent->winGetStatus() | WIN_STATUS_IMAGE);
