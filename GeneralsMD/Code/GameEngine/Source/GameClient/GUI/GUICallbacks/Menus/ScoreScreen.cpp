...
void ScoreScreenInit(WindowLayout *layout, void *userData)
{
    ...
    {
        DEBUG_LOG(
            ("ScoreScreenInit(): TheGameSpyInfo->stuff(%s/%s/%s)",
    ...
    listboxChatWindowScoreScreenID =
        TheNameKeyGenerator->nameToKey(AsciiString("ScoreScreen.wnd:ListboxChatWindowScoreScreen"));
    listboxAcademyWindowScoreScreenID =
        TheNameKeyGenerator->nameToKey(AsciiString("ScoreScreen.wnd:ListboxWarschoolAdvice"));
    ...
    staticTextGameSaved = TheWindowManager->winGetWindowFromId(
        parent,
        TheNameKeyGenerator->nameToKey("ScoreScreen.wnd:StaticTextGameSaveComplete"));
    backdrop = TheWindowManager->winGetWindowFromId(parent, backdropID);
    challengePortrait =
        TheWindowManager->winGetWindowFromId(parent, TheNameKeyGenerator->nameToKey("ScoreScreen.wnd:BigPortrait"));
    challengeWinLossText =
        TheWindowManager->winGetWindowFromId(parent, TheNameKeyGenerator->nameToKey("ScoreScreen.wnd:ChallengeWinLossText"));
    challengeRemarks =
        TheWindowManager->winGetWindowFromId(parent, TheNameKeyGenerator->nameToKey("ScoreScreen.wnd:GeneralRemarks"));
    gadgetParent =
        TheWindowManager->winGetWindowFromId(parent, TheNameKeyGenerator->nameToKey("ScoreScreen.wnd:GadgetParent"));
    ...
    Bool isChallengeCampaign =
        TheCampaignManager->getCurrentCampaign() ? TheCampaignManager->getCurrentCampaign()->isChallengeCampaign() : FALSE;
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType ScoreScreenInput(GameWindow *window, UnsignedInt msg, WindowMsgData mData1, WindowMsgData mData2)
{
    switch (msg)
    {
        // --------------------------------------------------------------------------------------------
        case GWM_CHAR:
        {
            ...
            switch (key)
            {
                ...
                case KEY_ESC:
                {
                    ...
                    {
                        TheWindowManager->winSendSystemMsg(window, GBM_SELECTED, (WindowMsgData)buttonOk, buttonOkID);
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType ScoreScreenSystem(GameWindow *window, UnsignedInt msg, WindowMsgData mData1, WindowMsgData mData2)
{
...
void PlayMovieAndBlock(AsciiString movieTitle)
{
    ...
    if (videoBuffer == NULL || !videoBuffer->allocate(videoStream->width(), videoStream->height()))
    {
        delete videoBuffer;
        videoBuffer = NULL;
                ...
                if (player->getAcademyStats()->calculateAcademyAdvice(&info))
                {
                    for (UnsignedInt i = 0; i < info.numTips; i++)
                    {
                        GadgetListBoxAddEntryText(
                            listboxAcademyWindowScoreScreen,
                            info.advice[i],
...
*/
    ...
    //			if(bIt !=  TheGameSpyInfo->getBuddyMap()->end()  || (TheGameSpyInfo->getLocalProfileID() ==
    //blah->second.m_profileID))
    //			{
    //				// already our buddy
    //				win->winHide(TRUE);
    //			}
    ...
    if (screenType == SCORESCREEN_SKIRMISH && player->isLocalPlayer())
    {
        if (TheGameInfo->isSandbox()
            || !(TheVictoryConditions->isLocalAlliedDefeat() || TheVictoryConditions->isLocalAlliedVictory()))
        {
        ...
        if (localID)
        {
            ...
            if (player->isLocalPlayer())
            {
                ...
                if (localSlot)
                {
                    ...
                    DEBUG_LOG(
                        ("Game ended on frame %d - TheGameLogic->getFrame()=%d",
                         lastFrameOfGame - 1,
                    ...
                    for (i = 0; i < MAX_SLOTS; ++i)
                    {
                        const GameSlot *slot = TheGameInfo->getConstSlot(i);
                        DEBUG_LOG(
                            ("latestHumanInGame=%d, slot->isOccupied()=%d, slot->disconnected()=%d, slot->isAI()=%d, "
                        ...
                        if (slot->isOccupied() && !slot->disconnected() && i != localSlotNum
                            && (slot->isAI()
                                || (slot->lastFrameInGame() >= lastFrameOfGame /*TheGameLogic->getFrame()*/ - 1)))
                        {
                    ...
                    if (gameEndedInDisconnect)
                    {
                        if (latestHumanInGame == TheNetwork->getPingFrame())
                        {
                            ...
                            if (TheNetwork->getPingsRecieved() < max(
                                    1,
                                    TheNetwork->getPingsSent()
                                        / 2)) /// @todo: what's a good percent of pings to have gotten?
                            {
                    ...
                    {
                        DEBUG_LOG(
                            ("Not sending results - we didn't finish a game. %d", TheVictoryConditions->getEndFrame()));
                        return;
                    }
                    ...
                    if (TheVictoryConditions->getEndFrame() < LOGICFRAMES_PER_SECOND * 25
                    ...
                    if (stats.id == 0)
                    {
                        ...
                        DEBUG_CRASH(
                            ("populatePlayerInfo() - not tracking stats - we haven't gotten the original stuff yet"));
                        return;
                    }
                    ...
                    for (i = 0; i < MAX_SLOTS; ++i)
                    {
                        if (TheGameSpyGame->getConstSlot(i)->isOccupied()
                            && TheGameSpyGame->getConstSlot(i)->getPlayerTemplate() != PLAYERTEMPLATE_OBSERVER)
                            ++gameSize;
                    }
                    switch (gameSize)
                    {
                    ...
                    DEBUG_LOG(
                        ("Before game built scud:%d, cannon:%d, nuke:%d",
                         stats.builtSCUD,
                    ...
                    DEBUG_LOG(
                        ("After game built scud:%d, cannon:%d, nuke:%d",
                         stats.builtSCUD,
                         stats.builtParticleCannon,
                         stats.builtNuke));
                    ...
                    if (!TheNetwork->sawCRCMismatch() && !gameEndedInDisconnect
                        && !TheVictoryConditions->isLocalAlliedDefeat() && TheVictoryConditions->getEndFrame())
                    {
                    ...
                    DEBUG_LOG(
                        ("populatePlayerInfo() - tracking stats for %s/%s/%s",
                         TheGameSpyInfo->getLocalBaseName().str(),
                         TheGameSpyInfo->getLocalEmail().str(),
                         TheGameSpyInfo->getLocalPassword().str()));
...
void grabMultiPlayerInfo(void)
{
    ...
    DEBUG_ASSERTCRASH(
        count == playerCount,
        ("For some reason we added %d players to the scores map, but only read %d", playerCount, count));
}
    ...
    if (localPlayer)
    {
        ...
        if (fact != NULL)
        {
            const Image *image = TheMappedImageCollection->findImageByName(
                ThePlayerList->getLocalPlayer()->getPlayerTemplate()->getScoreScreen());
            if (image)
            {
                parent->winSetEnabledImage(0, image);
                parent->winSetStatus(parent->winGetStatus() | WIN_STATUS_IMAGE);
