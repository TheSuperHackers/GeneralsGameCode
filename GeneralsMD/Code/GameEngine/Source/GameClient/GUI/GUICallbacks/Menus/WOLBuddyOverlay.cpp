...
void InitBuddyControls(Int type)
{
	...
	switch (type)
	{
		...
		default:
			DEBUG_ASSERTCRASH(
					FALSE,
					("Well, you really shouldn't have gotten here, if you really care about GUI Bugs, search for this string, you you "
					 "don't care, call chris (who probably doesn't care either"));
	}
}
			...
			if (selected >= 0)
			{
						...
						&& !ThePlayerList->getLocalPlayer()->isPlayerActive())
				{
					...
					for (Int i = 0; i < MAX_SLOTS; ++i)
					{
						...
						{
							// can't send to someone in our game if we're dead/observing.  security breach and all that.  no seances for
							// you.
		...
		WindowMsgData mData2)
{
	switch (msg)
	{
		...
		case GBM_SELECTED:
		{
			...
			DEBUG_ASSERTCRASH(
					rcData,
					("WOLBuddyOverlayRCMenuSystem GBM_SELECTED:: we're attempting to read the GameSpyRCMenuData from the window, but "
