    ...
    for (Int i = 0; i < MAX_SLOTS; i++)
    {
        ...
        if (slot->isAI())
        {
            switch (slot->getState())
            {
                ...
                {
                    addedRow =
                        GadgetListBoxAddEntryText(listBoxPlayers, TheGameText->fetch("GUI:EasyAI"), playerColor, -1, 1);
                    break;
                }
                case SLOT_MED_AI:
                {
                    addedRow =
                        GadgetListBoxAddEntryText(listBoxPlayers, TheGameText->fetch("GUI:MediumAI"), playerColor, -1, 1);
                    break;
                }
                case SLOT_BRUTAL_AI:
                {
                    addedRow =
                        GadgetListBoxAddEntryText(listBoxPlayers, TheGameText->fetch("GUI:HardAI"), playerColor, -1, 1);
                    break;
                }
                default:
                    break;
