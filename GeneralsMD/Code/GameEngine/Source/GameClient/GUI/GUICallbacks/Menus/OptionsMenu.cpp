...
static void setDefaults(void)
{
    ...
    if ((TheGameLogic->isInGame() == FALSE) || (TheGameLogic->isInShellGame() == TRUE))
    {
        ...
        switch (level)
        {
            ...
            default:
                DEBUG_ASSERTCRASH(
                    FALSE,
                    ("Tried to set comboBoxDetail to a value of %d ", TheGameLODManager->getStaticLODLevel()));
        };
    }
    ...
    if ((TheGameLogic->isInGame() == FALSE) || (TheGameLogic->isInShellGame() == TRUE) && !TheGameSpyInfo)
    {
        ...
        for (Int i = 0; i < numResolutions; ++i)
        {
            ...
            if (xres == DEFAULT_DISPLAY_WIDTH
                && yres == DEFAULT_DISPLAY_HEIGHT) // keep track of default mode in case we need it.
            {
                defaultResIndex = i;
                break;
            }
    ...
    Real maxVolume =
        MAX(TheAudio->getAudioSettings()->m_defaultSoundVolume, TheAudio->getAudioSettings()->m_default3DSoundVolume);
...
static void saveOptions(void)
{
    ...
    if (index == CUSTOMDETAIL)
    {
        ...
        (*pref)["BuildingOcclusion"] =
            TheWritableGlobalData->m_enableBehindBuildingMarkers ? AsciiString("yes") : AsciiString("no");
    ...
    //-------------------------------------------------------------------------------------------------
    // HTTP Proxy
    GameWindow *textEntryHTTPProxy =
        TheWindowManager->winGetWindowFromId(NULL, NAMEKEY("OptionsMenu.wnd:TextEntryHTTPProxy"));
    ...
    GameWindow *textEntryFirewallPortOverride =
        TheWindowManager->winGetWindowFromId(NULL, NAMEKEY("OptionsMenu.wnd:TextEntryFirewallPortOverride"));
    ...
    (*pref)["UseDoubleClickAttackMove"] =
        TheWritableGlobalData->m_doubleClickAttackMove ? AsciiString("yes") : AsciiString("no");
...
void OptionsMenuInit(WindowLayout *layout, void *userData)
{
    ...
    enum AliasingMode CPP_11( : Int){OFF = 0, LOW, HIGH, NUM_ALIASING_MODES};
        ...
        case GBM_SELECTED:
        {
            ...
            else if (controlID == buttonAccept)
            {
                ...
                if (!TheGameLogic->isInGame() || TheGameLogic->isInShellGame())
                    destroyQuitMenu(); // if we're in a game, the change res then enter the same kind of game, we nee the
                                       // quit menu to be gone.
