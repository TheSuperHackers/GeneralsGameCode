...
static GameWindow *comboBoxDisabledLadder =
    NULL; // enable and disable this, but never use it.  it is a stand-in for comboBoxLadder for when there are no ladders
...
static Int MAX_DISCONNECTS[MAX_DISCONNECTS_COUNT] =
    {MAX_DISCONNECTS_ANY, MAX_DISCONNECTS_5, MAX_DISCONNECTS_10, MAX_DISCONNECTS_25, MAX_DISCONNECTS_50};
    ...
    for (std::list<AsciiString>::const_iterator it = maps.begin(); it != maps.end(); ++it)
    {
        ...
        if (md && md->m_numPlayers >= numPlayers)
        {
            ...
            GadgetListBoxAddEntryText(
                listboxMapSelect,
                displayName,
                GameSpyColor[(isSelected) ? GSCOLOR_MAP_SELECTED : GSCOLOR_MAP_UNSELECTED],
                index,
                1);
...
void WOLQuickMatchMenuInit(WindowLayout *layout, void *userData)
{
    ...
    if (TheGameSpyGame && TheGameSpyGame->isGameInProgress())
    {
        ...
        if (TheGameSpyInfo->isDisconnectedAfterGameStart(&disconReason))
        {
            ...
            DEBUG_LOG(
                ("WOLQuickMatchMenuInit() - game was in progress, and we were disconnected, so pop immediate back to main "
                 "menu"));
    ...
    if (TheLadderList->getStandardLadders()->size() == 0 && TheLadderList->getSpecialLadders()->size() == 0
    ...
    GameWindow *staticTextTitle =
        TheWindowManager->winGetWindowFromId(parentWOLQuickMatch, NAMEKEY("WOLQuickMatchMenu.wnd:StaticTextTitle"));
    if (staticTextTitle)
    {
        UnicodeString tmp;
        ...
        while (allowedMessages-- && !sawImportantMessage && TheGameSpyPeerMessageQueue->getResponse(resp))
        {
            ...
            switch (resp.peerResponseType)
            {
                ...
                case PeerResponse::PEERRESPONSE_JOINGROUPROOM:
                    ...
                    {
                        TheGameSpyInfo->addText(UnicodeString(L"Joined group room"), GameSpyColor[GSCOLOR_DEFAULT],
                    quickmatchTextWindow);
                    }
                    else
                    {
                        TheGameSpyInfo->addText(UnicodeString(L"Didn't join group room"), GameSpyColor[GSCOLOR_DEFAULT],
                    quickmatchTextWindow);
                    }
                    */
                    break;
                ...
                case PeerResponse::PEERRESPONSE_QUICKMATCHSTATUS:
                {
                    ...
                    {
                        case QM_IDLE:
                            // TheGameSpyInfo->addText(UnicodeString(L"Status: QM_IDLE"), GameSpyColor[GSCOLOR_DEFAULT],
                            // quickmatchTextWindow);
                            break;
                        case QM_JOININGQMCHANNEL:
                            TheGameSpyInfo->addText(
                                TheGameText->fetch("QM:JOININGQMCHANNEL"),
                                GameSpyColor[GSCOLOR_DEFAULT],
                                quickmatchTextWindow);
                            break;
                        case QM_LOOKINGFORBOT:
                            TheGameSpyInfo->addText(
                                TheGameText->fetch("QM:LOOKINGFORBOT"),
                                GameSpyColor[GSCOLOR_DEFAULT],
                                quickmatchTextWindow);
                            break;
                        case QM_SENTINFO:
                            TheGameSpyInfo->addText(
                                TheGameText->fetch("QM:SENTINFO"),
                                GameSpyColor[GSCOLOR_DEFAULT],
                                quickmatchTextWindow);
                            break;
                        case QM_WORKING:
                        ...
                        case QM_WIDENINGSEARCH:
                            TheGameSpyInfo->addText(
                                TheGameText->fetch("QM:WIDENINGSEARCH"),
                                GameSpyColor[GSCOLOR_DEFAULT],
                                quickmatchTextWindow);
                            buttonWiden->winEnable(FALSE);
                            break;
                        ...
                        case QM_INCHANNEL:
                            TheGameSpyInfo->addText(
                                TheGameText->fetch("QM:INCHANNEL"),
                                GameSpyColor[GSCOLOR_DEFAULT],
                                quickmatchTextWindow);
                            break;
                        ...
                        case QM_STARTINGGAME:
                            TheGameSpyInfo->addText(
                                TheGameText->fetch("QM:STARTINGGAME"),
                                GameSpyColor[GSCOLOR_DEFAULT],
                                quickmatchTextWindow);
                            break;
                        case QM_COULDNOTFINDBOT:
                            TheGameSpyInfo->addText(
                                TheGameText->fetch("QM:COULDNOTFINDBOT"),
                                GameSpyColor[GSCOLOR_DEFAULT],
                                quickmatchTextWindow);
                            buttonWiden->winEnable(FALSE);
                            buttonStart->winHide(FALSE);
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType WOLQuickMatchMenuInput(GameWindow *window, UnsignedInt msg, WindowMsgData mData1, WindowMsgData mData2)
{
    switch (msg)
    {
        // --------------------------------------------------------------------------------------------
                ...
                case KEY_ESC:
                {
                    ...
                    if (BitIsSet(state, KEY_STATE_UP))
                    {
                        if (!buttonBack->winIsHidden())
                            TheWindowManager
                                ->winSendSystemMsg(window, GBM_SELECTED, (WindowMsgData)buttonBack, buttonBackID);
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType WOLQuickMatchMenuSystem(GameWindow *window, UnsignedInt msg, WindowMsgData mData1, WindowMsgData mData2)
{
    ...
    switch (msg)
    {
        ...
        case GBM_SELECTED:
        {
            ...
            if (controlID == buttonStopID)
            {
                ...
                enableOptionsGadgets(TRUE);
                TheGameSpyInfo->addText(
                    TheGameText->fetch("GUI:QMAborted"),
                    GameSpyColor[GSCOLOR_DEFAULT],
                    quickmatchTextWindow);
            }
            ...
            else if (controlID == buttonStartID)
            {
                ...
                else if (index == PLAYERTEMPLATE_RANDOM)
                {
                    ...
                    Int randomTries = 0; // Rare to hit Random 10 times in a row, but if it does then random will be
                                         // converted to a set side by the very bug this tries to fix, so no harm done.
            ...
            else if (controlID == buttonSelectAllMapsID)
            {
                ...
                for (Int i = 0; i < numMaps; ++i)
                {
                    ...
                    GadgetListBoxAddEntryText(
                        listboxMapSelect,
                        GadgetListBoxGetText(listboxMapSelect, i, 1),
                        GameSpyColor[GSCOLOR_MAP_SELECTED],
                        i,
                        1);
                }
            ...
            else if (controlID == buttonSelectNoMapsID)
            {
                ...
                for (Int i = 0; i < numMaps; ++i)
                {
                    ...
                    GadgetListBoxAddEntryText(
                        listboxMapSelect,
                        GadgetListBoxGetText(listboxMapSelect, i, 1),
                        GameSpyColor[GSCOLOR_MAP_UNSELECTED],
                        i,
                        1);
                }
