...
static GameWindow *comboBoxDisabledLadder = NULL; // enable and disable this, but never use it.  it is a stand-in for
...
static Int MAX_DISCONNECTS[MAX_DISCONNECTS_COUNT] = { MAX_DISCONNECTS_ANY,
																											MAX_DISCONNECTS_5,
																											MAX_DISCONNECTS_10,
																											MAX_DISCONNECTS_25,
																											MAX_DISCONNECTS_50 };
...
void WOLQuickMatchMenuInit(WindowLayout *layout, void *userData)
{
	...
	if (TheGameSpyGame && TheGameSpyGame->isGameInProgress())
	{
		...
		if (TheGameSpyInfo->isDisconnectedAfterGameStart(&disconReason))
		{
			...
			DEBUG_LOG(
					("WOLQuickMatchMenuInit() - game was in progress, and we were disconnected, so pop immediate back to main menu"));
		...
		case GBM_SELECTED:
		{
			...
			else if (controlID == buttonStartID)
			{
				...
				else if (index == PLAYERTEMPLATE_RANDOM)
				{
					...
					Int randomTries = 0; // Rare to hit Random 10 times in a row, but if it does then random will be converted to a set
															 // side by the very bug this tries to fix, so no harm done.
