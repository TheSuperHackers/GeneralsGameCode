            ...
            switch (key)
            {
                ...
                case KEY_ESC:
                {
                    ...
                    if (BitIsSet(state, KEY_STATE_UP))
                    {
                        ...
                        TheWindowManager->winSendSystemMsg(window, GBM_SELECTED, (WindowMsgData)button, buttonBackKey);
...
static void processLoadButtonPress(GameWindow *window)
{
    ...
    if (selectedGameInfo)
    {
        ...
        {
            // GameWindow *buttonFrame  = TheWindowManager->winGetWindowFromId( parent, NAMEKEY(
            // "PopupSaveLoad.wnd:MenuButtonFrame" ) ); GameWindow *listboxGames = TheWindowManager->winGetWindowFromId(
            // parent, NAMEKEY( "PopupSaveLoad.wnd:ListboxGames" ) ); GameWindow *loadConfirm  =
            // TheWindowManager->winGetWindowFromId( parent, NAMEKEY( "PopupSaveLoad.wnd:LoadConfirmParent" ) );
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType SaveLoadMenuSystem(GameWindow *window, UnsignedInt msg, WindowMsgData mData1, WindowMsgData mData2)
{
    switch (msg)
    {
        // --------------------------------------------------------------------------------------------
        ...
        case GBM_SELECTED:
        {
            ...
            else if (controlID == buttonSaveKey)
            {
                // sanity
                DEBUG_ASSERTCRASH(
                    currentLayoutType == SLLT_SAVE_AND_LOAD || currentLayoutType == SLLT_SAVE_ONLY,
                    ("SaveLoadMenuSystem - layout type '%d' does not allow saving", currentLayoutType));
                ...
                else
                {
                    ...
                    // GameWindow *overwriteConfirm  = TheWindowManager->winGetWindowFromId( parent, NAMEKEY(
            ...
            else if (controlID == buttonDeleteKey)
            {
                ...
                if (selectedGameInfo)
                {
                    ...
                    // GameWindow *deleteConfirm = TheWindowManager->winGetWindowFromId( parent, NAMEKEY(
            ...
            else if (controlID == buttonOverwriteCancel || controlID == buttonOverwriteConfirm)
            {
                ...
                if (controlID == buttonOverwriteConfirm)
                {
                    ...
                    DEBUG_ASSERTCRASH(
                        selectedGameInfo,
                        ("SaveLoadMenuSystem: Internal error, listbox entry to overwrite game has no item data set into "
                         "listbox element"));
            ...
            else if (controlID == buttonSaveDescConfirm)
            {
                ...
                // GameWindow *entryDesc = TheWindowManager->winGetWindowFromId( saveDesc, NAMEKEY(
                // "PopupSaveLoad.wnd:EntryDesc" ) );
            ...
            {
                // hide confirm dialog
                // GameWindow *loadConfirm  = TheWindowManager->winGetWindowFromId( parent, NAMEKEY(
