...
GameWindow *GameWindow::findPrevLeaf(void)
{
	...
	if (leaf->m_prev)
	{
		...
		while (leaf->m_child && BitIsSet(leaf->m_status, WIN_STATUS_TAB_STOP) == FALSE)
		{
			leaf = leaf->m_child;
...
GameWindow *GameWindow::findNextLeaf(void)
{
	...
	if (leaf->m_next)
	{
		...
		for (leaf = leaf->m_next; leaf; leaf = leaf->m_child)
			if (leaf->m_child == NULL || BitIsSet(leaf->m_status, WIN_STATUS_TAB_STOP))
				return leaf;
...
Int GameWindow::winBringToTop(void)
{
	...
	else
	{
		...
		for (current = TheWindowManager->winGetWindowList(); current != this; current = current->m_next)
			if (current == NULL)
