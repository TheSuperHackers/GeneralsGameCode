...
void SinglePlayerLoadScreen::init(GameInfo *game)
{
  ...
  /*
    ...
    "SinglePlayerLoadScreen.wnd:WindowCameo1" ) )); DEBUG_ASSERTCRASH(m_cameoWindow1, ("Can't initialize the m_cameoWindow1
    for the single player loadscreen")); m_cameoWindow1->winHide(TRUE); m_cameoWindow1->winEnable(FALSE);
    ...
    "SinglePlayerLoadScreen.wnd:WindowCameo2" ) )); DEBUG_ASSERTCRASH(m_cameoWindow2, ("Can't initialize the m_cameoWindow2
    for the single player loadscreen")); m_cameoWindow2->winHide(TRUE); m_cameoWindow2->winEnable(FALSE);
    ...
    "SinglePlayerLoadScreen.wnd:WindowCameo3" ) )); DEBUG_ASSERTCRASH(m_cameoWindow3, ("Can't initialize the m_cameoWindow3
    for the single player loadscreen")); m_cameoWindow3->winHide(TRUE); m_cameoWindow3->winEnable(FALSE);
    ...
    "SinglePlayerLoadScreen.wnd:WindowHead" ) )); DEBUG_ASSERTCRASH(m_headMovie, ("Can't initialize the m_headMovie for the
    single player loadscreen")); m_headMovie->winHide(TRUE); m_cameoFrame = TheWindowManager->winGetWindowFromId(
    ...
    const GeneralPersona *generalOpponent)
{
  ...
  if (frame > FRAME_TELETYPE_START && (frame % TELETYPE_UPDATE_FREQ) == 0)
  {
    ...
    textPosBigNameLeft =
        updateTeletypeText(1, m_bioBigNameEntryLeft, TheGameText->fetch(generalPlayer->getBioName()), textPosBigNameLeft);
    ...
    textPosBigNameRight = updateTeletypeText(
        ...
        textPosBigNameRight);
    //		textPosAgeRight = updateTeletypeText( 1, m_bioAgeEntryRight, TheGameText->fetch(generalOpponent->getBioDOB()),
    // textPosAgeRight);
