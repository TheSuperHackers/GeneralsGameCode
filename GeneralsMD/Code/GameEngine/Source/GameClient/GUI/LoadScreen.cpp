...
TheGameText->fetch(TheCampaignManager->getCurrentMission()->m_cameoImageName[0]));
    ...
    else if( frame > STATE_ANIM_CAMEO1_TRASITION_CAMEO2 && frame < STATE_ANIM_CAMEO2)
    {
        ...
        Real percent = INT_TO_REAL((frame - STATE_ANIM_CAMEO1_TRASITION_CAMEO2)) / (STATE_ANIM_CAMEO2 -
STATE_ANIM_CAMEO1_TRASITION_CAMEO2); m_cameoFrame->winSetPosition(startPos.x + (endPos.x - startPos.x) * percent, endPos.y);
    }
...
TheGameText->fetch(TheCampaignManager->getCurrentMission()->m_cameoImageName[1]));
    ...
    else if( frame > STATE_ANIM_CAMEO2_TRASITION_CAMEO3 && frame < STATE_ANIM_CAMEO3)
    {
        ...
        //extrapolate between start and end pos
        Real percent = INT_TO_REAL((frame - STATE_ANIM_CAMEO2_TRASITION_CAMEO3)) / (STATE_ANIM_CAMEO3 -
STATE_ANIM_CAMEO2_TRASITION_CAMEO3); m_cameoFrame->winSetPosition(startPos.x + (endPos.x - startPos.x) * percent, endPos.y);
    }
...
void SinglePlayerLoadScreen::init(GameInfo *game)
{
    ...
    m_progressBar = TheWindowManager->winGetWindowFromId(
        m_loadScreen,
        TheNameKeyGenerator->nameToKey(AsciiString("SinglePlayerLoadScreen.wnd:ProgressLoad")));
    ...
    m_percent = TheWindowManager->winGetWindowFromId(
        m_loadScreen,
        TheNameKeyGenerator->nameToKey(AsciiString("SinglePlayerLoadScreen.wnd:Percent")));
    ...
    m_objectiveWin = TheWindowManager->winGetWindowFromId(
        m_loadScreen,
        TheNameKeyGenerator->nameToKey(AsciiString("SinglePlayerLoadScreen.wnd:ObjectivesWin")));
    ...
    for (; i < MAX_OBJECTIVE_LINES; ++i)
    {
        ...
        DEBUG_ASSERTCRASH(
            m_objectiveLines[i],
            ("Can't initialize the m_objectiveLines[%d] for the single player loadscreen", i));
    ...
    m_location = TheWindowManager->winGetWindowFromId(
        m_loadScreen,
        TheNameKeyGenerator->nameToKey(AsciiString("SinglePlayerLoadScreen.wnd:StaticTextCameoText3")));
    ...
    /*
        m_cameoWindow1 = TheWindowManager->winGetWindowFromId( m_loadScreen,TheNameKeyGenerator->nameToKey( AsciiString(
       "SinglePlayerLoadScreen.wnd:WindowCameo1" ) )); DEBUG_ASSERTCRASH(m_cameoWindow1, ("Can't initialize the
       m_cameoWindow1 for the single player loadscreen")); m_cameoWindow1->winHide(TRUE); m_cameoWindow1->winEnable(FALSE);
        ...
        m_cameoWindow2 = TheWindowManager->winGetWindowFromId( m_loadScreen,TheNameKeyGenerator->nameToKey( AsciiString(
       "SinglePlayerLoadScreen.wnd:WindowCameo2" ) )); DEBUG_ASSERTCRASH(m_cameoWindow2, ("Can't initialize the
       m_cameoWindow2 for the single player loadscreen")); m_cameoWindow2->winHide(TRUE); m_cameoWindow2->winEnable(FALSE);
        ...
        m_cameoWindow3 = TheWindowManager->winGetWindowFromId( m_loadScreen,TheNameKeyGenerator->nameToKey( AsciiString(
       "SinglePlayerLoadScreen.wnd:WindowCameo3" ) )); DEBUG_ASSERTCRASH(m_cameoWindow3, ("Can't initialize the
       m_cameoWindow3 for the single player loadscreen")); m_cameoWindow3->winHide(TRUE); m_cameoWindow3->winEnable(FALSE);
        ...
        m_headMovie = TheWindowManager->winGetWindowFromId( m_loadScreen,TheNameKeyGenerator->nameToKey( AsciiString(
       "SinglePlayerLoadScreen.wnd:WindowHead" ) )); DEBUG_ASSERTCRASH(m_headMovie, ("Can't initialize the m_headMovie for
       the single player loadscreen")); m_headMovie->winHide(TRUE); m_cameoFrame = TheWindowManager->winGetWindowFromId(
       m_loadScreen,TheNameKeyGenerator->nameToKey( AsciiString( "SinglePlayerLoadScreen.wnd:WindowHiliteCameo" ) ));
        DEBUG_ASSERTCRASH(m_cameoFrame, ("Can't initialize the m_cameoFrame for the single player loadscreen"));
        m_cameoFrame->winHide(TRUE);
        m_cameoText = TheWindowManager->winGetWindowFromId( m_loadScreen,TheNameKeyGenerator->nameToKey( AsciiString(
       "SinglePlayerLoadScreen.wnd:StaticTextCameoText" ) )); DEBUG_ASSERTCRASH(m_cameoText, ("Can't initialize the
       m_cameoText for the single player loadscreen"));
    ...
    */
    ...
    GameWindow *backgroundWin = TheWindowManager->winGetWindowFromId(
        m_loadScreen,
...
// accepts the number of chars to advance, the window we're concerned with, the total text for final display, and the current
// position of the readout returns the updated position of the readout
...
void ChallengeLoadScreen::activatePieces(
    Int frame,
    const GeneralPersona *generalPlayer,
    const GeneralPersona *generalOpponent)
{
    ...
    {
        textPosNameLeft =
            updateTeletypeText(1, m_bioNameEntryLeft, TheGameText->fetch(generalPlayer->getBioName()), textPosNameLeft);
        textPosBigNameLeft = updateTeletypeText(
            1,
            m_bioBigNameEntryLeft,
            TheGameText->fetch(generalPlayer->getBioName()),
            textPosBigNameLeft);
        //		textPosAgeLeft = updateTeletypeText( 1, m_bioAgeEntryLeft, TheGameText->fetch(generalPlayer->getBioDOB()),
        //textPosAgeLeft);
        textPosBirthplaceLeft = updateTeletypeText(
            1,
            m_bioBirthplaceEntryLeft,
            TheGameText->fetch(generalPlayer->getBioRank()),
            textPosBirthplaceLeft);
        textPosStrategyLeft = updateTeletypeText(
            1,
            m_bioStrategyEntryLeft,
            TheGameText->fetch(generalPlayer->getBioStrategy()),
            textPosStrategyLeft);

        textPosNameRight =
            updateTeletypeText(1, m_bioNameEntryRight, TheGameText->fetch(generalOpponent->getBioName()), textPosNameRight);
        textPosBigNameRight = updateTeletypeText(
            1,
            m_bioBigNameEntryRight,
            TheGameText->fetch(generalOpponent->getBioName()),
            textPosBigNameRight);
        //		textPosAgeRight = updateTeletypeText( 1, m_bioAgeEntryRight,
        //TheGameText->fetch(generalOpponent->getBioDOB()), textPosAgeRight);
        textPosBirthplaceRight = updateTeletypeText(
            1,
            m_bioBirthplaceEntryRight,
            TheGameText->fetch(generalOpponent->getBioRank()),
            textPosBirthplaceRight);
        textPosStrategyRight = updateTeletypeText(
            1,
            m_bioStrategyEntryRight,
...
void ChallengeLoadScreen::init(GameInfo *game)
{
    ...
    DEBUG_ASSERTCRASH(
        mission->m_generalName.isNotEmpty(),
        ("No GeneralName associated with this mission, check Campaign.ini"));
    ...
    m_progressBar = TheWindowManager->winGetWindowFromId(
        m_loadScreen,
        TheNameKeyGenerator->nameToKey(AsciiString("ChallengeLoadScreen.wnd:ProgressLoad")));
    DEBUG_ASSERTCRASH(m_progressBar, ("Can't initialize the progressbar for the single player loadscreen"));
    GadgetProgressBarSetProgress(m_progressBar, 0);
...
void ShellGameLoadScreen::init(GameInfo *game)
{
    ...
    m_progressBar = TheWindowManager->winGetWindowFromId(
        m_loadScreen,
        TheNameKeyGenerator->nameToKey(AsciiString("ShellGameLoadScreen.wnd:ProgressLoad")));
    DEBUG_ASSERTCRASH(m_progressBar, ("Can't initialize the progressbar for the single player loadscreen"));
    GadgetProgressBarSetProgress(m_progressBar, 0);
...
void MultiPlayerLoadScreen::init(GameInfo *game)
{
    ...
    m_mapPreview = TheWindowManager->winGetWindowFromId(
        m_loadScreen,
        TheNameKeyGenerator->nameToKey("MultiplayerLoadScreen.wnd:WinMapPreview"));
    ...
    m_portraitLocalGeneral = TheWindowManager->winGetWindowFromId(
        m_loadScreen,
        TheNameKeyGenerator->nameToKey("MultiplayerLoadScreen.wnd:LocalGeneralPortrait"));
    m_portraitLocalGeneral->winSetEnabledImage(0, portrait);
    m_featuresLocalGeneral = TheWindowManager->winGetWindowFromId(
        m_loadScreen,
        TheNameKeyGenerator->nameToKey("MultiplayerLoadScreen.wnd:LocalGeneralFeatures"));
    AsciiString features = pt->getGeneralFeatures();
    GadgetStaticTextSetText(
        m_featuresLocalGeneral,
        TheGameText->fetch(features.isEmpty() ? AsciiString("GUI:PlayerObserver") : pt->getGeneralFeatures()));
    m_nameLocalGeneral = TheWindowManager->winGetWindowFromId(
        m_loadScreen,
        TheNameKeyGenerator->nameToKey("MultiplayerLoadScreen.wnd:LocalGeneralName"));
    ...
    for (i = 0; i < MAX_SLOTS; ++i)
    {
        ...
        m_buttonMapStartPosition[i] =
            TheWindowManager->winGetWindowFromId(m_loadScreen, TheNameKeyGenerator->nameToKey(winName));
        DEBUG_ASSERTCRASH(
            m_buttonMapStartPosition[i],
            ("Can't initialize the MapStart Positions for the MultiplayerLoadScreen loadscreen"));
...
void MultiPlayerLoadScreen::processProgress(Int playerId, Int percentage)
{
    ...
    // DEBUG_LOG(("Percentage %d was passed in for Player %d (in loadscreen position %d)", percentage, playerId,
    // m_playerLookup[playerId]));
...
void GameSpyLoadScreen::init(GameInfo *game)
{
    ...
    m_portraitLocalGeneral = TheWindowManager->winGetWindowFromId(
        m_loadScreen,
        TheNameKeyGenerator->nameToKey("GameSpyLoadScreen.wnd:LocalGeneralPortrait"));
    m_portraitLocalGeneral->winSetEnabledImage(0, portrait);
    m_featuresLocalGeneral = TheWindowManager->winGetWindowFromId(
        m_loadScreen,
        TheNameKeyGenerator->nameToKey("GameSpyLoadScreen.wnd:LocalGeneralFeatures"));
    AsciiString features = pt->getGeneralFeatures();
    GadgetStaticTextSetText(
        m_featuresLocalGeneral,
        TheGameText->fetch(features.isEmpty() ? AsciiString("GUI:PlayerObserver") : pt->getGeneralFeatures()));
    m_nameLocalGeneral = TheWindowManager->winGetWindowFromId(
        m_loadScreen,
        TheNameKeyGenerator->nameToKey("GameSpyLoadScreen.wnd:LocalGeneralName"));
    GadgetStaticTextSetText(m_nameLocalGeneral, localName);
    ...
    for (i = 0; i < MAX_SLOTS; ++i)
    {
        ...
        DEBUG_ASSERTCRASH(
            m_buttonMapStartPosition[i],
            ("Can't initialize the MapStart Positions for the GameSpyLoadScreen loadscreen"));
        ...
        m_playerTotalDisconnects[i] =
            TheWindowManager->winGetWindowFromId(m_loadScreen, TheNameKeyGenerator->nameToKey(winName));
        DEBUG_ASSERTCRASH(
            m_playerTotalDisconnects[i],
            ("Can't initialize the m_playerTotalDisconnects for the GameSpyLoadScreen loadscreen"));

        //		// Load the Player's m_playerFavoriteFactions
        //		winName.format( "GameSpyLoadScreen.wnd:StaticTextFavoriteFaction%d",i);
        //		m_playerFavoriteFactions[i] = TheWindowManager->winGetWindowFromId(
        //m_loadScreen,TheNameKeyGenerator->nameToKey( winName )); 		DEBUG_ASSERTCRASH(m_playerFavoriteFactions[i], ("Can't
        //initialize the StaticTextFavoriteFaction for the GameSpyLoadScreen loadscreen"));
        ...
        DEBUG_ASSERTCRASH(
            m_playerWinLosses[i],
            ("Can't initialize the m_playerWinLosses for the GameSpyLoadScreen loadscreen"));
        ...
        m_playerOfficerMedal[i] =
            TheWindowManager->winGetWindowFromId(m_loadScreen, TheNameKeyGenerator->nameToKey(winName));
        DEBUG_ASSERTCRASH(
            m_playerOfficerMedal[i],
            ("Can't initialize the m_playerOfficerMedal for the GameSpyLoadScreen loadscreen"));
        ...
        m_playerWinLosses[netSlot]->winSetEnabledTextColors(
            houseColor,
            m_playerWinLosses[netSlot]->winGetEnabledTextBorderColor());
        // favoriteFaction
        Int numGames = 0;
        Int favorite = 0;
...
void MapTransferLoadScreen::init(GameInfo *game)
{
    ...
    for (i = 0; i < MAX_SLOTS; ++i)
    {
        ...
        m_progressText[netSlot]->winSetEnabledTextColors(
            houseColor,
            m_progressText[netSlot]->winGetEnabledTextBorderColor());

        if ((i == 0 || (TheGameInfo->getConstSlot(i)->isHuman() && TheGameInfo->getConstSlot(i)->hasMap()))
