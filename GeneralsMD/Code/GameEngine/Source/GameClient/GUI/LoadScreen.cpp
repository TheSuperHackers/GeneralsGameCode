...
void SinglePlayerLoadScreen::init(GameInfo *game)
{
	...
	/*
		...
		"SinglePlayerLoadScreen.wnd:StaticTextCameoText" ) )); DEBUG_ASSERTCRASH(m_cameoText, ("Can't initialize the m_cameoText
		for the single player loadscreen"));
...
void GameSpyLoadScreen::init(GameInfo *game)
{
	...
	for (i = 0; i < MAX_SLOTS; ++i)
	{
		...
		//		m_playerFavoriteFactions[i] = TheWindowManager->winGetWindowFromId( m_loadScreen,TheNameKeyGenerator->nameToKey(
		// winName )); 		DEBUG_ASSERTCRASH(m_playerFavoriteFactions[i], ("Can't initialize the StaticTextFavoriteFaction for the
