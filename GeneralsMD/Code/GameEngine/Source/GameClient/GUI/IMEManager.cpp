...
class IMEManager : public IMEManagerInterface
{
  ...
  Int m_compositionCharsDisplayed; ///< number of temporary composition characters displayed that need to be replaced with
                                   ///< result string.
...
IMEManager::MessageInfo IMEManager::m_mainMessageInfo[] = { { "WM_IME_SETCONTEXT", WM_IME_SETCONTEXT },
...
IMEManager::MessageInfo IMEManager::m_notifyInfo[] = { { "IMN_CLOSESTATUSWINDOW", IMN_CLOSESTATUSWINDOW },
...
IMEManager::MessageInfo IMEManager::m_controlInfo[] = { { "IMC_GETCANDIDATEPOS", IMC_GETCANDIDATEPOS },
...
IMEManager::MessageInfo IMEManager::m_setContextInfo[] = { { "CANDIDATEWINDOW1", ISC_SHOWUICANDIDATEWINDOW },
...
IMEManager::MessageInfo IMEManager::m_setCmodeInfo[] = { { "ALPHANUMERIC", IME_CMODE_ALPHANUMERIC },
...
IMEManager::MessageInfo IMEManager::m_setSmodeInfo[] = { { "NONE", IME_SMODE_NONE },
    ...
    case WM_IME_NOTIFY:
    {
      ...
      switch (wParam)
      {
        ...
        /*
                  ...
                  case IMN_SETCANDIDATEPOS:        //This message is sent when the IME is about to move the candidate
           window. case IMN_SETSTATUSWINDOWPOS:     //This message is sent when the status window position in the
           input context is updated. return FALSE; case IMN_OPENSTATUSWINDOW:       //This message is sent when an
           IME is about to create the status window. DEBUG_LOG(("Open Status Window")); return FALSE; case
           IMN_CLOSESTATUSWINDOW:      //This message is sent to the application when an input method editor (IME) is
           about to close the status window. DEBUG_LOG(("Close Status Window")); return FALSE; case
           IMN_SETOPENSTATUS:          //This message is sent when the open status of the input context is updated.
                  case IMN_SETCOMPOSITIONFONT:     //This message is sent when the font of the input context is
           updated. case IMN_SETCOMPOSITIONWINDOW:   //This message is sent when the style or position of the
           composition window is updated. case IMN_PRIVATE:                //This message is for your own use, it
           seems.
        */
        default:
          m_result = 1;
  ...
  if (m_context)
  {
    ...
    else
    {
      ...
      if (result > 0)
      {
        ...
        // msg_assert ( (int)strlen(tempBuf) >= convRes ,("bad DBCS string: DBCS = %d chars, Wide = %d chars",
        // strlen(tempBuf), convRes));
  ...
  for (Int i = 0, candidate = 1; i < 32; i++, candidate = candidate << 1)
  {
    if (candidateFlags & candidate)
    {
      ...
      if (ok && clist->dwStyle != IME_CAND_UNKNOWN && clist->dwStyle != IME_CAND_CODE)
      {
        ...
        if ((clist->dwPageStart > clist->dwSelection) || (clist->dwSelection >= clist->dwPageStart + clist->dwPageSize))
        {
...
#ifdef DEBUG_IME
        DEBUG_LOG(
            ("IME: Candidate Update: Candidates = %d, pageSize = %d pageStart = %d, selected = %d",
             m_candidateCount,
             m_pageStart,
             m_pageSize,
             m_selectedIndex));
