...
//-----------------------------------------------------------------------------
Campaign::Campaign(void) : m_isChallengeCampaign(FALSE)
{
  m_missions.clear();
...
Int CampaignManager::getCurrentMissionNumber(void)
{
  ...
  if (m_currentCampaign)
  {
    ...
    for (it = m_currentCampaign->m_missions.begin(); it != m_currentCampaign->m_missions.end(); ++it)
    {
      number++;
...
void CampaignManager::loadPostProcess(void)
{
  ...
  {
    DEBUG_CRASH(
        ("TheChallengeGenerals singleton does not exist. This loaded game will not have a working Continue button for "
         "GC mode."));
    return;
  }
