...
void Anim2DTemplate::parseNumImages(INI *ini, void *instance, void *store, const void *userData)
{
  ...
  {
    DEBUG_CRASH(
        ("Anim2DTemplate::parseNumImages - Invalid animation '%s', animations must have '%d' or more frames defined",
...
/*static*/ void Anim2DTemplate::parseImageSequence(INI *ini, void *instance, void *store, const void *userData)
{
  ...
  {
    DEBUG_CRASH(
        ("Anim2DTemplate::parseImageSequence - You must specify the number of animation frames for animation '%s' "
         "*BEFORE* specifying the image sequence name",
         animTemplate->getName().str()));
    throw INI_INVALID_DATA;
  ...
  for (Int i = 0; i < animTemplate->getNumFrames(); ++i)
  {
    ...
    {
      DEBUG_CRASH(
          ("Anim2DTemplate::parseImageSequence - Image '%s' not found for animation '%s'.  Check the number of images "
...
void Anim2DTemplate::storeImage(const Image *image)
{
  ...
  DEBUG_CRASH(
      ("Anim2DTemplate::storeImage - Unable to store image '%s' into animation '%s' because the animation is setup to "
...
{
  // sanity
  DEBUG_ASSERTCRASH(m_images != NULL, ("Anim2DTemplate::getFrame - Image data is NULL for animation '%s'", getName().str()));

  // sanity
  if (frameNumber < 0 || frameNumber >= m_numFrames)
  {
    DEBUG_CRASH(("Anim2DTemplate::getFrame - Illegal frame number '%d' for animation '%s'", frameNumber, getName().str()));
    return NULL;
...
void Anim2D::setCurrentFrame(UnsignedShort frame)
{
  ...
  DEBUG_ASSERTCRASH(
      TheGameLogic != NULL,
      ("Anim2D::setCurrentFrame - TheGameLogic must exist to use animation instances (%s)", m_template->getName().str()));

  // sanity
  DEBUG_ASSERTCRASH(
      frame >= 0 && frame < m_template->getNumFrames(),
      ("Anim2D::setCurrentFrame - Illegal frame number '%d' in animation", frame, m_template->getName().str()));
...
void Anim2D::reset(void)
{
  ...
  switch (m_template->getAnimMode())
  {
    ...
    default:
      DEBUG_CRASH(
          ("Anim2D::reset - Unknown animation mode '%d' for '%s'", m_template->getAnimMode(), m_template->getName().str()));
      break;
...
void Anim2D::tryNextFrame(void)
{
  ...
  DEBUG_ASSERTCRASH(
      TheGameLogic != NULL,
      ("Anim2D::tryNextFrame - TheGameLogic must exist to use animation instances (%s)", m_template->getName().str()));
...
void Anim2D::draw(Int x, Int y)
{
  ...
  DEBUG_ASSERTCRASH(
      image != NULL,
      ("Anim2D::draw - Image not found for frame '%d' on animation '%s'", m_currentFrame, m_template->getName().str()));
...
{
  // search the list
  for (Anim2DTemplate *animTemplate = m_templateList; animTemplate; animTemplate = animTemplate->friend_getNextTemplate())
  {
...
void Anim2DCollection::registerAnimation(Anim2D *anim)
{
  ...
  // sanity
  DEBUG_ASSERTCRASH(
      anim->m_collectionSystemNext == NULL && anim->m_collectionSystemPrev == NULL,
      ("Registering animation instance, instance '%s' is already in a system", anim->getAnimTemplate()->getName().str()));
