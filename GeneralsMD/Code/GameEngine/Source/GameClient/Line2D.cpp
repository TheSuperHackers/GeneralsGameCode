 ...
 */
//-------------------------------------------------------------------------------------------------
Bool ClipLine2D(ICoord2D *p1, ICoord2D *p2, ICoord2D *c1, ICoord2D *c2, IRegion2D *clipRegion)
{
	Int x1, y1, x2, y2;
	Int clipLeft;
	Int clipRight;
...
// Subject 1.03
Bool IntersectLine2D(const Coord2D *a, const Coord2D *b, const Coord2D *c, const Coord2D *d, Coord2D *intersection)
{
	if (!a || !b || !c || !d)
	{
...
Bool PointInsideRect2D(const Coord2D *bl, const Coord2D *tl, const Coord2D *br, const Coord2D *tr, const Coord2D *inputPoint)
{
...
Bool PointInsideRect3D(const Coord3D *bl, const Coord3D *tl, const Coord3D *br, const Coord3D *tr, const Coord3D *inputPoint)
{
...
void ShortestDistancePointToSegment2D(
		const Coord2D *a,
		const Coord2D *b,
		const Coord2D *pt,
		Real *outDistance,
		...
		Real *outU)
{
	...
	// General case
	Real u = ((pt->x - a->x) * (b->x - a->x) + (pt->y - a->y) * (b->y - a->y)) / sqr(segAB.length());
