...
/*static*/ void Drawable::initStaticImages()
{
    ...
    s_animationTemplates[ICON_BATTLEPLAN_BOMBARD] =
        TheAnim2DCollection->findTemplate(TheDrawableIconNames[ICON_BATTLEPLAN_BOMBARD]);
    s_animationTemplates[ICON_BATTLEPLAN_HOLDTHELINE] =
        TheAnim2DCollection->findTemplate(TheDrawableIconNames[ICON_BATTLEPLAN_HOLDTHELINE]);
    s_animationTemplates[ICON_BATTLEPLAN_SEARCHANDDESTROY] =
        TheAnim2DCollection->findTemplate(TheDrawableIconNames[ICON_BATTLEPLAN_SEARCHANDDESTROY]);
    ...
    s_animationTemplates[ICON_ENTHUSIASTIC_SUBLIMINAL] =
        TheAnim2DCollection->findTemplate(TheDrawableIconNames[ICON_ENTHUSIASTIC_SUBLIMINAL]);
...
//-------------------------------------------------------------------------------------------------
Drawable::Drawable(const ThingTemplate *thingTemplate, DrawableStatus statusBits) : Thing(thingTemplate)
{
    ...
    m_constructDisplayString->setFont(TheFontLibrary->getFont(
    ...
    *m = NULL;
    ...
    if (cuMI.getCount())
    {
        ...
        for (modIdx = 0; modIdx < cuMI.getCount(); ++modIdx)
        {
            ...
            if (thingTemplate->isKindOf(KINDOF_SHRUBBERY) && !TheGlobalData->m_useTreeSway
    ...
    if (obj)
    {
        ...
        if (obj->isDisabled())
        {
            if (!obj->isKindOf(KINDOF_PRODUCED_AT_HELIPAD) &&
                ...
                (obj->isDisabledByType(DISABLED_HACKED) || obj->isDisabledByType(DISABLED_PARALYZED)
...
//-------------------------------------------------------------------------------------------------
Int Drawable::getCurrentClientBonePositions(
    const char *boneNamePrefix,
    Int startIndex,
    Coord3D *positions,
    Matrix3D *transforms,
    Int maxBones) const
{
    ...
    for (const DrawModule **dm = getDrawModules(); *dm; ++dm)
    {
        ...
        if (di)
        {
            Int subcount = di->getCurrentBonePositions(boneNamePrefix, startIndex, positions, transforms, maxBones);
...
void Drawable::updateDrawable(void)
{
    ...
    if (m_prevTintStatus != m_tintStatus) // edge test
    {
        ...
        else if (testTintStatus(TINT_STATUS_FRENZY))
        {
            ...
            m_colorTintEnvelope
                ->play(isKindOf(KINDOF_INFANTRY) ? &FRENZY_COLOR_INFANTRY : &FRENZY_COLOR, 30, 30, SUSTAIN_INDEFINITELY);
        }
...
void Drawable::calcPhysicsXformHoverOrWings(const Locomotor *locomotor, PhysicsXformInfo &info)
{
    ...
    m_locoInfo->m_pitchRate +=
        ((-PITCH_STIFFNESS * m_locoInfo->m_pitch) + (-PITCH_DAMPING * m_locoInfo->m_pitchRate)); // spring/damper
    m_locoInfo->m_rollRate +=
        ((-ROLL_STIFFNESS * m_locoInfo->m_roll) + (-ROLL_DAMPING * m_locoInfo->m_rollRate)); // spring/damper
    ...
    m_locoInfo->m_accelerationPitchRate +=
        ((-PITCH_STIFFNESS * (m_locoInfo->m_accelerationPitch))
         + (-PITCH_DAMPING * m_locoInfo->m_accelerationPitchRate)); // spring/damper
...
void Drawable::calcPhysicsXformTreads(const Locomotor *locomotor, PhysicsXformInfo &info)
{
    ...
    if (overlapped)
    {
        ...
        if (centerDistSqr < sqr(maxCenterDist))
        {
            ...
            Bool flat = false;
            if (overlapped->isKindOf(KINDOF_LOW_OVERLAPPABLE) || overlapped->isKindOf(KINDOF_INFANTRY)
    ...
    if (damageInfo)
    {
        if (obj->getBodyModule()->getLastDamageTimestamp() > m_lastDamageTimestamp
        ...
        if (DO_WHEELS)
        {
            ...
            if (pos->z - hheight > -MAX_SUSPENSION_EXTENSION)
            {
                m_locoInfo->m_wheelInfo.m_rearLeftHeightOffset +=
                    (MAX_SUSPENSION_EXTENSION - m_locoInfo->m_wheelInfo.m_rearLeftHeightOffset) / 2.0f;
                m_locoInfo->m_wheelInfo.m_rearRightHeightOffset +=
                    (MAX_SUSPENSION_EXTENSION - m_locoInfo->m_wheelInfo.m_rearRightHeightOffset) / 2.0f;
            }
            else
            {
                m_locoInfo->m_wheelInfo.m_rearLeftHeightOffset +=
...
#define WHEEL_SMOOTHNESS 10.0f // higher numbers add smaller angles, make it more "smooth"
        m_locoInfo->m_wheelInfo.m_wheelAngle +=
            (newInfo.m_wheelAngle - m_locoInfo->m_wheelInfo.m_wheelAngle) / WHEEL_SMOOTHNESS;
        ...
        if (newInfo.m_frontLeftHeightOffset < m_locoInfo->m_wheelInfo.m_frontLeftHeightOffset)
        {
            ...
            m_locoInfo->m_wheelInfo.m_frontLeftHeightOffset +=
                (newInfo.m_frontLeftHeightOffset - m_locoInfo->m_wheelInfo.m_frontLeftHeightOffset) / 2.0f;
        }
        else
        {
        ...
        if (newInfo.m_frontRightHeightOffset < m_locoInfo->m_wheelInfo.m_frontRightHeightOffset)
        {
            ...
            m_locoInfo->m_wheelInfo.m_frontRightHeightOffset +=
                (newInfo.m_frontRightHeightOffset - m_locoInfo->m_wheelInfo.m_frontRightHeightOffset) / 2.0f;
        }
        else
        {
        ...
        if (newInfo.m_rearLeftHeightOffset < m_locoInfo->m_wheelInfo.m_rearLeftHeightOffset)
        {
            // If it's going down, dampen the movement a bit
            m_locoInfo->m_wheelInfo.m_rearLeftHeightOffset +=
                (newInfo.m_rearLeftHeightOffset - m_locoInfo->m_wheelInfo.m_rearLeftHeightOffset) / 2.0f;
        }
        else
        {
        ...
        if (newInfo.m_rearRightHeightOffset < m_locoInfo->m_wheelInfo.m_rearRightHeightOffset)
        {
            // If it's going down, dampen the movement a bit
            m_locoInfo->m_wheelInfo.m_rearRightHeightOffset +=
                (newInfo.m_rearRightHeightOffset - m_locoInfo->m_wheelInfo.m_rearRightHeightOffset) / 2.0f;
        }
        else
        {
...
void Drawable::calcPhysicsXformMotorcycle(const Locomotor *locomotor, PhysicsXformInfo &info)
{
    ...
    // THis logic had recently been added to Drawable::applyPhysicsXform(), which was naughty, since it clamped the roll in
    // every drawable in the game Now only motorcycles enjoy this constraint
...
void Drawable::draw(View *view)
{
    if (testTintStatus(TINT_STATUS_FRENZY) == FALSE)
    {
        ...
        else if (m_secondMaterialPassOpacity > VERY_TRANSPARENT_MATERIAL_PASS_OPACITY) // keep fading any add'l material
                                                                                       // unless something has set it to zero
...
void Drawable::drawIconUI(void)
{
    if (TheGameLogic->getDrawIconUI() && (TheScriptEngine->getFade() == ScriptEngine::FADE_NONE))
    {
        ...
        if (obj->isEffectivelyDead()
            || obj->isKindOf(KINDOF_IGNORED_IN_GUI)) // object explicitly wants nothing to do with these icons, so...
            return;
...
void Drawable::setEmoticon(const AsciiString &name, Int duration)
{
    ...
    if (animTemplate)
    {
        DEBUG_ASSERTCRASH(
            ...
            ("Drawable::setEmoticon - Emoticon isn't empty, need to refuse to set or destroy the old one in favor of the "
             "new one"));
...
void Drawable::drawBattlePlans(const IRegion2D *healthBarRegion)
{
    ...
    if (player && player->getNumBattlePlansActive() > 0 && player->doesObjectQualifyForBattlePlan(obj))
    {
        if (player->getBattlePlansActiveSpecific(PLANSTATUS_BOMBARDMENT))
        {
            ...
            {
                getIconInfo()->m_icon[ICON_BATTLEPLAN_BOMBARD] =
                    newInstance(Anim2D)(s_animationTemplates[ICON_BATTLEPLAN_BOMBARD], TheAnim2DCollection);
            }
            // Int barHeight = healthBarRegion.hi.y - healthBarRegion.lo.y;
        ...
        if (player->getBattlePlansActiveSpecific(PLANSTATUS_HOLDTHELINE))
        {
            ...
            {
                getIconInfo()->m_icon[ICON_BATTLEPLAN_HOLDTHELINE] =
                    newInstance(Anim2D)(s_animationTemplates[ICON_BATTLEPLAN_HOLDTHELINE], TheAnim2DCollection);
            }
            // draw the icon
        ...
        if (player->getBattlePlansActiveSpecific(PLANSTATUS_SEARCHANDDESTROY))
        {
            ...
            {
                getIconInfo()->m_icon[ICON_BATTLEPLAN_SEARCHANDDESTROY] =
                    newInstance(Anim2D)(s_animationTemplates[ICON_BATTLEPLAN_SEARCHANDDESTROY], TheAnim2DCollection);
            }
            // draw the icon
...
void Drawable::drawEnthusiastic(const IRegion2D *healthBarRegion)
{
    ...
    if (obj->testWeaponBonusCondition(WEAPONBONUSCONDITION_ENTHUSIASTIC) == TRUE && healthBarRegion != NULL)
    {
    ...
    if (ai->isDemoralized())
    {
        ...
        if (healthBarRegion)
        {
            ...
            if (getIconInfo()->m_icon[ICON_DEMORALIZED] == NULL)
                getIconInfo()->m_icon[ICON_DEMORALIZED] =
                    newInstance(Anim2D)(s_animationTemplates[ICON_DEMORALIZED], TheAnim2DCollection);
...
void Drawable::drawBombed(const IRegion2D *healthBarRegion)
{
    ...
    if (obj->testWeaponSetFlag(WEAPONSET_CARBOMB) && obj->getControllingPlayer() == ThePlayerList->getLocalPlayer())
    {
        if (!getIconInfo()->m_icon[ICON_CARBOMB])
            getIconInfo()->m_icon[ICON_CARBOMB] =
                newInstance(Anim2D)(s_animationTemplates[ICON_CARBOMB], TheAnim2DCollection);
    ...
    if (update)
    {
        ...
        if (target)
        {
            if (update->isTimedBomb())
            {
                ...
                {
                    getIconInfo()->m_icon[ICON_BOMB_REMOTE] =
                        newInstance(Anim2D)(s_animationTemplates[ICON_BOMB_REMOTE], TheAnim2DCollection);
                    getIconInfo()->m_icon[ICON_BOMB_TIMED] =
                        newInstance(Anim2D)(s_animationTemplates[ICON_BOMB_TIMED], TheAnim2DCollection);
...
void Drawable::drawDisabled(const IRegion2D *healthBarRegion)
{
    ...
    //                   7/
    if (obj->isDisabledByType(DISABLED_HACKED) || obj->isDisabledByType(DISABLED_PARALYZED)
        ...
        || obj->isDisabledByType(DISABLED_UNDERPOWERED))
    {
        ...
        {
            getIconInfo()->m_icon[ICON_DISABLED] =
                newInstance(Anim2D)(s_animationTemplates[ICON_DISABLED], TheAnim2DCollection);
        }
        ...
        && (isSelected() || (TheInGameUI && (TheInGameUI->getMousedOverDrawableID() == getID()))))
    {
        ...
        else // red to green
        {
            ...
            if (healthRatio >= 0.5f)
            {
                ...
                //				outlineColor = GameMakeColor( (255 - ((healthRatio - 0.5f) / 0.5f) * 255) * 0.5, 255 * 0.5,
                //0, 255 );
            }
            else
            {
                ...
                //				outlineColor = GameMakeColor( 255 * 0.5, (255 - ((0.5f - healthRatio) / 0.5f) * 255) * 0.5,
                //0, 255 );
            }
        ...
        TheDisplay->drawOpenRect(
            healthBarRegion->lo.x,
            healthBarRegion->lo.y,
            healthBoxWidth,
            healthBoxHeight,
        ...
        TheDisplay->drawFillRect(
            healthBarRegion->lo.x + 1,
            healthBarRegion->lo.y + 1,
            (healthBoxWidth - 2) * healthRatio,
            healthBoxHeight - 2,
            color);
    } // end if
...
//-------------------------------------------------------------------------------------------------
Bool Drawable::handleWeaponFireFX(
    WeaponSlotType wslot,
    Int specificBarrelToUse,
    const FXList *fxl,
    Real weaponSpeed,
...
void Drawable::mangleCustomAudioName(DynamicAudioEventInfo *audioToMangle) const
{
    ...
    customizedName.format(
        " CUSTOM %d ",
        (Int)getID()); // Note space at beginning prevents collision with any names from INI file
        ...
        else
        {
            ...
            for (UnsignedShort j = 0; j < moduleCount; ++j)
            {
                ...
                if (module == NULL)
                {
                    ...
                    DEBUG_CRASH((
                        "Drawable::xferDrawableModules - Module '%s' was indicated in file, but not found on Drawable %s %d",
...
void Drawable::xfer(Xfer *xfer)
{
    ...
    if (xfer->getXferMode() == XFER_LOAD)
    {
        ...
        else
        {
            if (objectID != INVALID_ID)
            {
                ...
                DEBUG_CRASH(
                    ("Drawable::xfer - Drawable '%s' is not attached to an object but should be attached to object '%s' "
                     "with id '%d'",
                     getTemplate()->getName().str(),
    ...
    if (version < 2)
    {
        ...
        DEBUG_ASSERTCRASH(xfer->getXferMode() == XFER_LOAD, ("Drawable::xfer - Writing an old format!!!"));
        ...
        if (customized)
        {
            ...
            if (xfer->getXferMode() == XFER_LOAD)
            {
                ...
                else
                {
                    ...
                    {
                        DEBUG_CRASH(
                            ("Load failed to load customized ambient sound because sound '%s' no longer exists",
                             baseInfoName.str()));

                        // Keep trying to load if we possibly can... Don't completely ruin save files just because an old
                        // sound entry in the INI files was removed or renamed
