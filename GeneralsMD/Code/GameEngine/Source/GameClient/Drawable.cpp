...
void Drawable::draw(View *view)
{
	if (testTintStatus(TINT_STATUS_FRENZY) == FALSE)
	{
		...
		else if (m_secondMaterialPassOpacity > VERY_TRANSPARENT_MATERIAL_PASS_OPACITY) // keep fading any add'l material unless
...
void Drawable::drawIconUI(void)
{
	if (TheGameLogic->getDrawIconUI() && (TheScriptEngine->getFade() == ScriptEngine::FADE_NONE))
	{
		...
		if (obj->isEffectivelyDead() || obj->isKindOf(KINDOF_IGNORED_IN_GUI)) // object explicitly wants nothing to do with these
																																					// icons, so...
			return;
...
void Drawable::setEmoticon(const AsciiString &name, Int duration)
{
	...
	if (animTemplate)
	{
		DEBUG_ASSERTCRASH(
				...
				("Drawable::setEmoticon - Emoticon isn't empty, need to refuse to set or destroy the old one in favor of the new "
				 "one"));
...
void Drawable::drawBombed(const IRegion2D *healthBarRegion)
{
	...
	if (obj->testWeaponSetFlag(WEAPONSET_CARBOMB) && obj->getControllingPlayer() == TheControlBar->getCurrentlyViewedPlayer())
	{
...
void Drawable::mangleCustomAudioName(DynamicAudioEventInfo *audioToMangle) const
{
	...
	customizedName.format(" CUSTOM %d ", (Int)getID()); // Note space at beginning prevents collision with any names from INI
																											// file
