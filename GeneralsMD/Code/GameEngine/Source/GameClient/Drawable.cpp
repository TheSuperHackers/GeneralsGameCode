...
static const char *TheDrawableIconNames[] = { "DefaultHeal",
                                              "StructureHeal",
                                              "VehicleHeal",
...
void Drawable::calcPhysicsXformTreads(const Locomotor *locomotor, PhysicsXformInfo &info)
{
  ...
  if (overlapped || m_locoInfo->m_overlapZ <= 0.0f)
  {
    ...
    m_locoInfo->m_rollRate +=
        ((-ROLL_STIFFNESS * (m_locoInfo->m_roll - groundRoll)) + (-ROLL_DAMPING * m_locoInfo->m_rollRate)); // spring/damper
  }
    ...
    if (customized)
    {
      ...
      if (xfer->getXferMode() == XFER_LOAD)
      {
        ...
        else
        {
          ...
          {
            DEBUG_CRASH(
                ("Load failed to load customized ambient sound because sound '%s' no longer exists", baseInfoName.str()));
