...
void GameClient::init(void)
{
    ...
    if (TheMessageStream)
    {
        ...
        m_translators[m_numTranslators++] =
            TheMessageStream->attachTranslator(MSGNEW("GameClientSubsystem") WindowTranslator, 10);
        m_translators[m_numTranslators++] =
            TheMessageStream->attachTranslator(MSGNEW("GameClientSubsystem") MetaEventTranslator, 20);
        m_translators[m_numTranslators++] =
            TheMessageStream->attachTranslator(MSGNEW("GameClientSubsystem") HotKeyTranslator, 25);
        m_translators[m_numTranslators++] =
            TheMessageStream->attachTranslator(MSGNEW("GameClientSubsystem") PlaceEventTranslator, 30);
        m_translators[m_numTranslators++] =
            TheMessageStream->attachTranslator(MSGNEW("GameClientSubsystem") GUICommandTranslator, 40);
        m_translators[m_numTranslators++] =
            TheMessageStream->attachTranslator(MSGNEW("GameClientSubsystem") SelectionTranslator, 50);
        m_translators[m_numTranslators++] =
            TheMessageStream->attachTranslator(MSGNEW("GameClientSubsystem") LookAtTranslator, 60);
        m_translators[m_numTranslators] =
            TheMessageStream->attachTranslator(MSGNEW("GameClientSubsystem") CommandTranslator, 70);
        ...
        m_translators[m_numTranslators++] =
            TheMessageStream->attachTranslator(MSGNEW("GameClientSubsystem") HintSpyTranslator, 100);
        ...
        //
        m_translators[m_numTranslators++] =
            TheMessageStream->attachTranslator(MSGNEW("GameClientSubsystem") GameClientMessageDispatcher, 999999999);
    }
...
void GameClient::reset(void)
{
    ...
    // TheSuperHackers @fix Mauller 13/04/2025 Reset the drawable id so it does not keep growing over the lifetime of the
    // game.
    m_nextDrawableID = (DrawableID)1;
...
#endif
        {
            // localPlayerIndex=TheGhostObjectManager->getLocalPlayerIndex();	//always use the first local player set since
            // normally can't change.  Doesn't work with debug "CTRL_SHIFT_SPACE"
...
void GameClient::preloadAssets(TimeOfDay timeOfDay)
{
    ...
    DEBUG_LOG(
        ("Preloading memory dwAvailPageFile %d --> %d : %d",
         before.dwAvailPageFile,
         after.dwAvailPageFile,
         before.dwAvailPageFile - after.dwAvailPageFile));
    DEBUG_LOG(
        ("Preloading memory dwAvailPhys     %d --> %d : %d",
         before.dwAvailPhys,
         after.dwAvailPhys,
         before.dwAvailPhys - after.dwAvailPhys));
    DEBUG_LOG(
        ("Preloading memory dwAvailVirtual  %d --> %d : %d",
         before.dwAvailVirtual,
         after.dwAvailVirtual,
...
static Bool shouldSaveDrawable(const Drawable *draw)
{
    if (draw->testDrawableStatus(DRAWABLE_STATUS_NO_SAVE))
    {
        ...
        {
            DEBUG_CRASH(
                ("You should not ever set DRAWABLE_STATUS_NO_SAVE for a Drawable with an object. (%s)",
...
void GameClient::xfer(Xfer *xfer)
{
    ...
    if (xfer->getXferMode() == XFER_SAVE)
    {
        ...
        for (draw = getDrawableList(); draw; draw = draw->getNextDrawable())
        {
            ...
            {
                DEBUG_CRASH(
                    ("GameClient::xfer - Drawable TOC entry not found for '%s'", draw->getTemplate()->getName().str()));
    ...
    else
    {
        ...
        for (UnsignedShort i = 0; i < drawableCount; ++i)
        {
            ...
            {
                DEBUG_CRASH(
                    ("GameClient::xfer - Unrecognized thing template '%s', skipping.  ENGINEERS - Are you *sure* it's OK to "
                     "be ignoring this object from the save file???  Think hard about it!",
                     tocEntry->name.str()));
                xfer->skip(dataSize);
                continue;
            ...
            if (objectID != INVALID_ID)
            {
                ...
                {
                    DEBUG_CRASH(
                        ("GameClient::xfer - Cannot find object '%d' that is supposed to be attached to this drawable '%s'",
                         objectID,
                ...
                {
                    DEBUG_CRASH(
                        ("GameClient::xfer - There is no drawable attached to the object '%s' (%d) and there should be",
            ...
            else
            {
                ...
                if (draw == NULL)
                {
                    DEBUG_CRASH(("GameClient::xfer - Unable to create drawable for '%s'", thingTemplate->getName().str()));
                    throw SC_INVALID_DATA;
