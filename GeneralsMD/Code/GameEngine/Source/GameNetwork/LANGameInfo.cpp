...
Bool ParseGameOptionsString(LANGameInfo *game, AsciiString options)
{
  ...
  if (ParseAsciiStringToGameInfo(game, options))
  {
    ...
    if (!TheLAN->AmIHost() && isInGame)
    {
      ...
      // DEBUG_LOG(("wasInGame:%d isInGame:%d hadMap:%d hasMap:%d oldMap:%s newMap:%s", wasInGame, isInGame, hadMap,
      // hasMap, oldMap.str(), game->getMap().str()));
    ...
    for (i = 0; i < MAX_SLOTS; ++i)
    {
      ...
      if (slot->isHuman())
      {
        ...
        // DEBUG_LOG(("Restored %ls@%ls for %ls", slot->getUser()->getLogin().str(),
        // slot->getUser()->getHost().str(), slot->getName().str()));
      }
    }
