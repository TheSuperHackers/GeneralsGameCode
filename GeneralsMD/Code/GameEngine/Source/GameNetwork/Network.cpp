...
protected:
    ...
    void processFrameSynchronizedNetCommand(
        NetCommandRef *msg); ///< If there is a network command that needs to be executed at the same frame number on all
                             ///< clients, it happens here.
    void processRunAheadCommand(
        NetRunAheadCommandMsg *msg); ///< Do what needs to be done when we get a new run ahead command.
    void processDestroyPlayerCommand(
        NetDestroyPlayerCommandMsg *msg); ///< Do what needs to be done when we need to destroy a player.
    void endOfGameCheck(); ///< Checks to see if its ok to leave this game.  If it is, send the apropriate command to the
                           ///< game logic.
...
void Network::setSawCRCMismatch(void)
{
    ...
    DEBUG_LOG(
        ("Latest frame for mismatch = %d GameLogic frame = %d",
         TheGameLogic->getFrame() - m_runAhead - 1,
...
void Network::parseUserList(const GameInfo *game)
{
    ...
    m_conMgr->zeroFrames(
        1,
        m_runAhead - 1); ///< we zero out m_runAhead frames +1 because the game actually starts at frame 1.
}
    ...
    while (msg != NULL)
    {
        ...
        { // Is this something we should be sending to the other players?
            ...
            TheCommandList->removeMessage(
                msg); // This does not destroy msg's prev and next pointers, so they should still be valid.
            deleteInstance(msg);
        }
        else
        {
...
Bool Network::processCommand(GameMessage *msg)
{
    ...
    if ((msg->getType() == GameMessage::MSG_CLEAR_GAME_DATA) && (m_localStatus == NETLOCALSTATUS_INGAME))
    {
        Int executionFrame = getExecutionFrame();
        DEBUG_LOG(
            ("Network::processCommand - local player leaving, executionFrame = %d, player leaving on frame %d",
             executionFrame,
             executionFrame + 1));
        ...
        m_conMgr->processFrameTick(
            executionFrame + 1); // since we send it for executionFrame+1, we need to process both ticks
...
void Network::RelayCommandsToCommandList(UnsignedInt frame)
{
    ...
    while (msg != NULL)
    {
        ...
        {
            // DEBUG_LOG(("Network::RelayCommandsToCommandList - appending command %d of type %s to command list on frame
            // %d", msg->getCommand()->getID(), ((NetGameCommandMsg
            // *)msg->getCommand())->constructGameMessage()->getCommandAsString(), TheGameLogic->getFrame()));
    ...
    for (std::list<Int>::iterator selfDestructIt = m_playersToDisconnect.begin();
         selfDestructIt != m_playersToDisconnect.end();
         ++selfDestructIt)
    {
...
void Network::processFrameSynchronizedNetCommand(NetCommandRef *msg)
{
    ...
    if (cmdMsg->getNetCommandType() == NETCOMMANDTYPE_PLAYERLEAVE)
    {
        ...
        {
            DEBUG_LOG(
                ("Network::processFrameSynchronizedNetCommand - Local player left the game on frame %d.",
                 TheGameLogic->getFrame()));
        ...
        {
            DEBUG_LOG(
                ("Network::processFrameSynchronizedNetCommand - Packet router left the game on frame %d",
                 TheGameLogic->getFrame()));
        }
        else
        {
            DEBUG_LOG(
                ("Network::processFrameSynchronizedNetCommand - Client left the game on frame %d",
                 TheGameLogic->getFrame()));
        }
    }
    else if (cmdMsg->getNetCommandType() == NETCOMMANDTYPE_RUNAHEAD)
    {
        ...
        DEBUG_LOG_LEVEL(
            DEBUG_LEVEL_NET,
            ("command to set run ahead to %d and frame rate to %d on frame %d actually executed on frame %d",
             netmsg->getRunAhead(),
             netmsg->getFrameRate(),
             netmsg->getExecutionFrame(),
             TheGameLogic->getFrame()));
    }
    else if (cmdMsg->getNetCommandType() == NETCOMMANDTYPE_DESTROYPLAYER)
    {
        ...
        // DEBUG_LOG(("CRC command (%8.8X) on frame %d actually executed on frame %d", netmsg->getCRC(),
...
void Network::processDestroyPlayerCommand(NetDestroyPlayerCommandMsg *msg)
{
    ...
    DEBUG_LOG(
        ("Saw DestroyPlayer from %d about %d for frame %d on frame %d",
         msg->getPlayerID(),
         msg->getPlayerIndex(),
...
Bool Network::timeForNewFrame()
{
    ...
    if (m_conMgr != NULL)
    {
        ...
        Real runAheadPercentage = m_runAhead
            * (TheGlobalData->m_networkRunAheadSlack
               / (Real)100.0); // If we are at least 50% into our slack, we need to slow down.
        if (cushion < runAheadPercentage)
        {
            ...
            //			DEBUG_LOG(("Average cushion = %f, run ahead percentage = %f.  Adjusting frameDelay from %I64d to
            //%I64d", cushion, runAheadPercentage, oldFrameDelay, frameDelay));
            m_didSelfSlug = TRUE;
            //		} else {
    ...
    {
        //		DEBUG_LOG(("Allowing a new frame, frameDelay = %I64d, curTime - m_nextFrameTime = %I64d", frameDelay, curTime
        //- m_nextFrameTime));
