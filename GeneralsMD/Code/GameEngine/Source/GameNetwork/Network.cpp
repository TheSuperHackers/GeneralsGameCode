...
class Network : public NetworkInterface
{
  ...
  void processFrameSynchronizedNetCommand(NetCommandRef *msg); ///< If there is a network command that needs to be executed
...
void Network::processFrameSynchronizedNetCommand(NetCommandRef *msg)
{
  ...
  if (cmdMsg->getNetCommandType() == NETCOMMANDTYPE_PLAYERLEAVE)
  {
    ...
    {
      DEBUG_LOG(
          ("Network::processFrameSynchronizedNetCommand - Client left the game on frame %d", TheGameLogic->getFrame()));
    }
  }
...
Bool Network::timeForNewFrame()
{
  ...
  if (m_conMgr != NULL)
  {
    ...
    if (cushion < runAheadPercentage)
    {
      ...
      //			DEBUG_LOG(("Average cushion = %f, run ahead percentage = %f.  Adjusting frameDelay from %I64d to %I64d",
      // cushion, runAheadPercentage, oldFrameDelay, frameDelay));
      m_didSelfSlug = TRUE;
      //		} else {
