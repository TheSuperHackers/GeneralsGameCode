		...
		*/
		...
		for (Int i = 0; i < MAX_SLOTS; ++i)
		{
			...
			if (ip)
			{
				...
				*/
				{
					user.format(",%s@%d.%d.%d.%d:%d", tmpUserName.str(), PRINTF_IP_AS_4_INTS(ip), TheNAT->getSlotPort(i));
				}
...
AsciiString GameSpyGameInfo::generateGameResultsPacket(void)
{
	...
	AsciiString results;
	results.format(
			"seed=%d,slotNum=%d,sawDesync=%d,sawGameEnd=%d,winningTeam=%d,disconEnd=%d,duration=%d,numPlayers=%d,isQM=%d",
			getSeed(),
			localSlotNum,
	...
	for (i = 0; i < MAX_SLOTS; ++i)
	{
		...
		if (p)
		{
			...
			playerStr.format(
					",player%d=%s,playerID%d=%d,locale%d=%s",
					playerID,
					playerName.str(),
					playerID,
					gsPlayerID,
			...
			results.concat(playerStr);
			playerStr.format(
					",fps%d=%d,cash%d=%d,capturedTech%d=%d,discon%d=%d",
					playerID,
					fps,
					playerID,
					earnings,
