        ...
        if (m_pingFrame < TheGameLogic->getFrame())
        {
            ...
            if ((curTime - m_lastFrameTime) > 10000) /// @todo: plug in some better measure here
            {
                ...
                if (ThePinger)
                {
                    ...
                    DEBUG_LOG(
                        ("DisconnectManager::update() - requesting %d pings of %d from %s",
                         req.repetitions,
                         req.timeout,
                         req.hostname.c_str()));
                }
        ...
        if (ThePinger)
        {
            ...
            while (ThePinger->getResponse(resp))
            {
                if (m_pingFrame != TheGameLogic->getFrame())
                {
                    ...
                    DEBUG_LOG(
                        ("DisconnectManager::update() - discarding ping of %d from %s (%d reps)",
                         resp.avgPing,
                         resp.hostname.c_str(),
                         resp.repetitions));
                }
                else
                {
                    // right frame
                    DEBUG_LOG(
                        ("DisconnectManager::update() - keeping ping of %d from %s (%d reps)",
                         resp.avgPing,
                         resp.hostname.c_str(),
                         resp.repetitions));
        ...
        if (conMgr->isPlayerConnected(i))
        {
            ...
            if (slot != -1)
            {
                ...
                if (m_haveNotifiedOtherPlayersOfCurrentFrame == FALSE)
                {
                    ...
                    DEBUG_LOG(
                        ("DisconnectManager::updateDisconnectStatus - curTime = %d, m_timeOfDisconnectScreenOn = %d, "
                         "curTime - m_timeOfDisconnectScreenOn = %d",
                         curTime,
                ...
                if ((newTime < 0) || (isPlayerVotedOut(slot, conMgr) == TRUE))
                {
                    newTime = 0;
                    DEBUG_LOG(
                        ("DisconnectManager::updateDisconnectStatus - player %d(translated slot %d) has been voted out or "
                         "timed out",
                         i,
                         slot));
                    if (allOnSameFrame(conMgr) == TRUE)
                    {
                        ...
                        if (isLocalPlayerNextPacketRouter(conMgr) == TRUE)
                        {
                            ...
                            DEBUG_LOG(
                                ("DisconnectManager::updateDisconnectStatus - about to do the disconnect procedure for "
                                 "player %d",
                                 i));
                        ...
                        {
                            DEBUG_LOG(
                                ("DisconnectManager::updateDisconnectStatus - local player is not the next packet router"));
                        }
                    }
                    else
                    {
...
void DisconnectManager::updateWaitForPacketRouter(ConnectionManager *conMgr)
{
    /*
        ...
        if (newTime < 0) {
            ...
            DEBUG_LOG(("DisconnectManager::updateWaitForPacketRouter - timed out waiting for new packet router, quitting
       game")); TheNetwork->quitGame();
        }
...
void DisconnectManager::processDisconnectPlayer(NetCommandMsg *msg, ConnectionManager *conMgr)
{
    ...
    DEBUG_LOG(
        ("DisconnectManager::processDisconnectPlayer - Got disconnect player command from player %d.  Disconnecting player "
         "%d on frame %d",
         msg->getPlayerID(),
         cmdMsg->getDisconnectSlot(),
         cmdMsg->getDisconnectFrame()));
    DEBUG_ASSERTCRASH(
        TheGameLogic->getFrame() == cmdMsg->getDisconnectFrame(),
...
void DisconnectManager::processPacketRouterQuery(NetCommandMsg *msg, ConnectionManager *conMgr)
{
    ...
    DEBUG_LOG(
        ("DisconnectManager::processPacketRouterQuery - got a packet router query command from player %d",
         msg->getPlayerID()));
    ...
    if (conMgr->getPacketRouterSlot() == conMgr->getLocalPlayerID())
    {
        ...
        DEBUG_LOG(
            ("DisconnectManager::processPacketRouterQuery - We are the new packet router, responding with an packet router "
             "ack. Local player is %d",
             ackmsg->getPlayerID()));
    ...
    {
        DEBUG_LOG(
            ("DisconnectManager::processPacketRouterQuery - We are NOT the new packet router, these are not the droids "
             "you're looking for."));
    }
}
...
void DisconnectManager::processPacketRouterAck(NetCommandMsg *msg, ConnectionManager *conMgr)
{
    ...
    DEBUG_LOG(
        ("DisconnectManager::processPacketRouterAck - got packet router ack command from player %d", msg->getPlayerID()));
    ...
    if (conMgr->getPacketRouterSlot() == cmdMsg->getPlayerID())
    {
        ...
        while ((currentPacketRouterSlot != conMgr->getPacketRouterFallbackSlot(currentPacketRouterIndex))
        ...
        m_disconnectState = DISCONNECTSTATETYPE_SCREENON; ///< set it to screen on so that the next call to AllCommandsReady
                                                          ///< can set up everything for the next frame properly.
    }
}
...
void DisconnectManager::processDisconnectVote(NetCommandMsg *msg, ConnectionManager *conMgr)
{
    ...
    DEBUG_LOG(
        ("DisconnectManager::processDisconnectVote - Got a disconnect vote for player %d command from player %d",
         cmdMsg->getSlot(),
         cmdMsg->getPlayerID()));
...
void DisconnectManager::processDisconnectFrame(NetCommandMsg *msg, ConnectionManager *conMgr)
{
    ...
    {
        DEBUG_LOG(
            ("DisconnectManager::processDisconnectFrame - Got two disconnect frames without an intervening disconnect "
    ...
    DEBUG_LOG(
        ("DisconnectManager::processDisconnectFrame - Got a disconnect frame for player %d, frame = %d, local player is %d, "
         "local disconnect frame = %d, command id = %d",
         cmdMsg->getPlayerID(),
         cmdMsg->getDisconnectFrame(),
    ...
    if (playerID == conMgr->getLocalPlayerID())
    {
        ...
        for (Int i = 0; i < MAX_SLOTS; ++i)
        {
            if (i != playerID)
            {
                ...
                if (isPlayerInGame(transSlot, conMgr) == TRUE)
                {
                    ...
                    {
                        DEBUG_LOG(
                            ("DisconnectManager::processDisconnectFrame - I have more frames than player %d, my frame = %d, "
                             "their frame = %d",
                             i,
                             m_disconnectFrames[conMgr->getLocalPlayerID()],
        ...
        (m_disconnectFrames[playerID] < m_disconnectFrames[conMgr->getLocalPlayerID()])
        && (m_disconnectFramesReceived[playerID] == TRUE))
    {
        DEBUG_LOG((
            "DisconnectManager::processDisconnectFrame - I have more frames than player %d, my frame = %d, their frame = %d",
            playerID,
            m_disconnectFrames[conMgr->getLocalPlayerID()],
            m_disconnectFrames[playerID]));
...
void DisconnectManager::processDisconnectScreenOff(NetCommandMsg *msg, ConnectionManager *conMgr)
{
    ...
    DEBUG_LOG(
        ("DisconnectManager::processDisconnectScreenOff - got a screen off command from player %d for frame %d",
         cmdMsg->getPlayerID(),
         cmdMsg->getNewFrame()));
    ...
    {
        DEBUG_LOG(
            ("DisconnectManager::processDisconnectScreenOff - resetting the disconnect screen status for player %d",
             playerID));
        m_disconnectFramesReceived[playerID] = FALSE;
        m_disconnectFrames[playerID] = newFrame; // just in case we get packets out of order and the disconnect screen off
                                                 // message gets here before the disconnect frame message.

        DEBUG_LOG(
            ("DisconnectManager::processDisconnectScreenOff - about to call resetPlayersVotes for player %d", playerID));
...
void DisconnectManager::applyDisconnectVote(Int slot, UnsignedInt frame, Int fromSlot, ConnectionManager *conMgr)
{
    ...
    DEBUG_LOG(
        ("DisconnectManager::applyDisconnectVote - added a vote to disconnect slot %d, from slot %d, for frame %d, current "
         "votes are %d",
         slot,
         fromSlot,
         frame,
...
{
    DEBUG_LOG(
        ("DisconnectManager::disconnectPlayer - Disconnecting slot number %d on frame %d", slot, TheGameLogic->getFrame()));
...
void DisconnectManager::sendDisconnectCommand(Int slot, ConnectionManager *conMgr)
{
    ...
    DEBUG_LOG(
        ("DisconnectManager::sendDisconnectCommand - Sending disconnect command for slot number %d for frame %d",
         slot,
         disconnectFrame));
...
void DisconnectManager::recalculatePacketRouterIndex(ConnectionManager *conMgr)
{
    Int currentPacketRouterSlot = conMgr->getPacketRouterSlot();
    m_currentPacketRouterIndex = 0;
    while ((currentPacketRouterSlot != conMgr->getPacketRouterFallbackSlot(m_currentPacketRouterIndex))
    ...
    for (Int i = 0; (i < MAX_SLOTS) && (retval == TRUE); ++i)
    {
        ...
        if ((conMgr->isPlayerConnected(i) == TRUE) && (isPlayerInGame(transSlot, conMgr) == TRUE))
        {
            ...
            if ((m_disconnectFramesReceived[i] == TRUE)
                && (m_disconnectFrames[conMgr->getLocalPlayerID()] != m_disconnectFrames[i]))
            {
...
void DisconnectManager::sendPlayerDestruct(Int slot, ConnectionManager *conMgr)
{
    ...
    DEBUG_LOG(
        ("Queueing DestroyPlayer %d for frame %d on frame %d as command %d",
         slot,
         TheNetwork->getExecutionFrame() + 1,
...
void DisconnectManager::resetPlayersVotes(Int playerID, UnsignedInt frame, ConnectionManager *conMgr)
{
    ...
    for (Int i = 0; i < MAX_SLOTS; ++i)
    {
        ...
        {
            DEBUG_LOG(
                ("DisconnectManager::resetPlayersVotes - resetting player %d's vote for player %d from frame %d on frame %d",
                 playerID,
                 i,
                 m_playerVotes[i][playerID].frame,
                 frame));
