...
void ConnectionManager::processFile(NetFileCommandMsg *msg)
{
	...
	if (realFileName.isEmpty())
	{
		...
		// in other words is bogus and points outside of the approved target directory, avoid an arbitrary file overwrite
		// vulnerability by simply returning and let the transfer time out.
...
void ConnectionManager::ackCommand(NetCommandRef *ref, UnsignedInt localSlot)
{
	...
	else
	{
		...
		if ((m_packetRouterSlot >= 0) && (m_packetRouterSlot < MAX_SLOTS))
		{
			...
			{
				//				DEBUG_LOG(("ConnectionManager::ackCommand - acking command %d from player %d to packet router.", commandID,
				// m_packetRouterSlot));
...
void ConnectionManager::updateRunAhead(Int oldRunAhead, Int frameRate, Bool didSelfSlug, Int nextExecutionFrame)
{
	...
	if ((lasttimesent == 0) || ((curTime - lasttimesent) > TheGlobalData->m_networkRunAheadMetricsTime))
	{
		if (m_localSlot == m_packetRouterSlot)
		{
			...
			if (didSelfSlug)
			{
				// DEBUG_LOG(("ConnectionManager::updateRunAhead - local player run ahead metrics, fps = %d, actual fps = %d, latency
				// = %f, didSelfSlug = true", m_fpsAverages[m_localSlot], m_frameMetrics.getAverageFPS(),
			...
			newRunAhead += (newRunAhead * TheGlobalData->m_networkRunAheadSlack) / 100; // Add in 10% of slack to the run ahead in
...
void ConnectionManager::sendLocalCommand(NetCommandMsg *msg, UnsignedByte relay /* = 0xff by default*/)
{
	...
	else
	{
		...
		UnsignedByte temprelay = relay & ~(1 << m_localSlot); // Tell the packet router to relay the message to everyone but
																													// myself. Hopefully the packet router is smart enough to not send
																													// it to slots that are not in the game.

		m_connections[m_packetRouterSlot]->sendNetCommandMsg(msg, temprelay); // This will create a new copy of netmsg for this
																																					// connection.
...
void ConnectionManager::sendSingleFrameToPlayer(UnsignedInt playerID, UnsignedInt frame)
{
	if ((TheGameLogic->getFrame() - FRAMES_TO_KEEP) > frame)
	{
		DEBUG_LOG_LEVEL(
				DEBUG_LEVEL_NET,
				("ConnectionManager::sendSingleFrameToPlayer - player %d requested frame %d when we are on frame %d, this is too "
				 "far in the past.",
				 playerID,
				 frame,
				 TheGameLogic->getFrame()));
		...
		{ // no need to send his own commands to him.
			...
			if (list != NULL)
			{
				...
				while (ref != NULL)
				{
					DEBUG_LOG_LEVEL(
							DEBUG_LEVEL_NET,
							("ConnectionManager::sendFrameDataToPlayer - sending command %d from player %d to player %d using relay 0x%x",
			...
			DEBUG_LOG_LEVEL(
					DEBUG_LEVEL_NET,
					("ConnectionManager::sendFrameDataToPlayer - sending frame info from player %d to player %d for frame %d with "
					 "command count %d and ID %d and relay %d",
					 i,
					 playerID,
					 msg->getExecutionFrame(),
					 msg->getCommandCount(),
