...
void ConnectionManager::init()
{
...
#endif
    m_packetRouterSlot = 0; /// @todo The LAN/WOL interface should be telling us who the packet router is based on machine
                            /// specs passed around through game options.
    for (i = 0; i < MAX_SLOTS; ++i)
    {
    ...
    {
        m_latencyAverages[i] =
            0.0; // using zero since all floating point standards should be able to specify 0.0 accurately.
    }
...
void ConnectionManager::zeroFrames(UnsignedInt startingFrame, UnsignedInt numFrames)
{
    for (Int i = 0; i < MAX_SLOTS; ++i)
    {
        if (m_frameData[i] != NULL)
        {
            //			DEBUG_LOG(("Calling zeroFrames on player %d, starting frame %d, numFrames %d", i, startingFrame,
            //numFrames));
...
Bool ConnectionManager::processNetCommand(NetCommandRef *ref)
{
    ...
    if ((msg->getNetCommandType() == NETCOMMANDTYPE_ACKSTAGE1) || (msg->getNetCommandType() == NETCOMMANDTYPE_ACKSTAGE2)
    ...
    {
        DEBUG_LOG(
            ("ConnectionManager::processNetCommand - got a TimeOut GameStart net command from player %d",
             msg->getPlayerID()));
    ...
    {
        DEBUG_LOG(
            ("ConnectionManager::processNetCommand - got a Load Complete net command from player %d", msg->getPlayerID()));
...
void ConnectionManager::processWrapper(NetCommandRef *ref)
{
    ...
    DEBUG_LOG_LEVEL(
        DEBUG_LEVEL_NET,
        ("ConnectionManager::processWrapper() - wrapped commandID is %d, commandID is %d", commandID, wrapperMsg->getID()));
    Int origProgress = 0;
    ...
    DEBUG_LOG_LEVEL(
        DEBUG_LEVEL_NET,
        ("ConnectionManager::processWrapper() - origProgress[%d] == %d for command %d",
         m_localSlot,
         origProgress,
         commandID));
    ...
    if (fcIt != s_fileCommandMap.end())
    {
        ...
        DEBUG_LOG_LEVEL(
            DEBUG_LEVEL_NET,
            ("ConnectionManager::processWrapper() - newProgress[%d] == %d for command %d",
             m_localSlot,
             newProgress,
             commandID));
...
void ConnectionManager::processRunAheadMetrics(NetRunAheadMetricsCommandMsg *msg)
{
    ...
    if ((player >= 0) && (player < MAX_SLOTS) && (isPlayerConnected(player)))
    {
        ...
        // DEBUG_LOG(("ConnectionManager::processRunAheadMetrics - player %d, fps = %d, latency = %f", player,
        // msg->getAverageFps(), msg->getAverageLatency()));
...
void ConnectionManager::processDisconnectChat(NetDisconnectChatCommandMsg *msg)
{
    ...
    //	DEBUG_LOG(("ConnectionManager::processDisconnectChat - got message from player %d, message is %ls", playerID,
    //unitext.str()));
...
void ConnectionManager::processChat(NetChatCommandMsg *msg)
{
    ...
    unitext.format(L"[%ls] %ls", name.str(), msg->getText().str());
    //	DEBUG_LOG(("ConnectionManager::processChat - got message from player %d (mask %8.8X), message is %ls", playerID,
    //msg->getPlayerMask(), unitext.str()));
...
void ConnectionManager::processFile(NetFileCommandMsg *msg)
{
    ...
    UnicodeString log;
    log.format(
        L"Saw file transfer: '%hs' of %d bytes from %d",
    ...
    {
        // TheSuperHackers @security slurmlord 18/06/2025 As the file name/path from the NetFileCommandMsg failed to
        // normalize, in other words is bogus and points outside of the approved target directory, avoid an arbitrary file
        // overwrite vulnerability by simply returning and let the transfer time out.
...
{
    DEBUG_LOG(
        ("ConnectionManager::processFileAnnounce() - expecting '%s' (%s) in command %d",
         msg->getPortableFilename().str(),
         msg->getRealFilename().str(),
         msg->getFileID()));
...
{
    DEBUG_LOG(("ConnectionManager::processFileProgress() - command %d is at %d%%", msg->getFileID(), msg->getProgress()));
...
void ConnectionManager::processFrameInfo(NetFrameCommandMsg *msg)
{
    ...
    if ((playerID >= 0) && (playerID < MAX_SLOTS))
    {
        ...
        {
            //			DEBUG_LOG(("ConnectionManager::processFrameInfo - player %d, frame %d, command count %d, received on
            //frame %d", playerID, msg->getExecutionFrame(), msg->getCommandCount(), TheGameLogic->getFrame()));
...
void ConnectionManager::processAckStage1(NetCommandMsg *msg)
{
    ...
    if (doDebug == TRUE)
    {
        DEBUG_LOG_LEVEL(
            DEBUG_LEVEL_NET,
            ("ConnectionManager::processAck - processing ack for command %d from player %d",
             ((NetAckStage1CommandMsg *)msg)->getCommandID(),
             playerID));
    }
#endif
    ...
    else
    {
        DEBUG_ASSERTCRASH(
            ...
            ("ConnectionManager::processAck - %d is an invalid player number", playerID));
    }
...
void ConnectionManager::processAckStage2(NetCommandMsg *msg)
{
    ...
    if (ref != NULL)
    {
        ...
        // commandID));
        DEBUG_ASSERTCRASH(
            (m_localSlot == playerID),
            ("Found a command in the pending commands list that wasn't originated by the local player"));
    ...
    {
        // DEBUG_LOG(("ConnectionManager::processAckStage2 - Couldn't find command %d from player %d in the pending commands
        // list.", commandID, playerID));
    }

    ref = m_relayedCommands->findMessage(commandID, playerID);
    if (ref != NULL)
    {
        // DEBUG_LOG(("ConnectionManager::processAckStage2 - found command ID %d from player %d in the relayed commands
        // list.", commandID, playerID));
        ...
        if (relay == 0)
        {
            // DEBUG_LOG(("ConnectionManager::processAckStage2 - relay is 0, removing command from the relayed commands
            // list."));
...
PlayerLeaveCode ConnectionManager::processPlayerLeave(NetPlayerLeaveCommandMsg *msg)
{
    ...
    {
        DEBUG_LOG(
            ("ConnectionManager::processPlayerLeave() - setQuitting() on player %d on frame %d",
             playerID,
    ...
    {
        DEBUG_LOG(
            ("ConnectionManager::processPlayerLeave - setQuitFrame on player %d for frame %d",
             playerID,
...
void ConnectionManager::handleLocalPlayerLeaving(UnsignedInt frame)
{
    ...
    DEBUG_ASSERTCRASH(
        ...
        ("ConnectionManager::handleLocalPlayerLeaving, packet router is %d, illegal value.", m_packetRouterSlot));
...
void ConnectionManager::ackCommand(NetCommandRef *ref, UnsignedInt localSlot)
{
    ...
    if (sendRelay == 0)
    {
        ...
        if (doDebug)
        {
            DEBUG_LOG_LEVEL(
                DEBUG_LEVEL_NET,
                ("ConnectionManager::ackCommand - doing ack both for command %d from player %d",
                 bothmsg->getCommandID(),
                 bothmsg->getOriginalPlayerID()));
        }
#endif
    }
    else
    {
        ...
        if (doDebug)
        {
            DEBUG_LOG_LEVEL(
                DEBUG_LEVEL_NET,
                ("ConnectionManager::ackCommand - doing ack stage 1 for command %d from player %d",
                 stage1msg->getCommandID(),
                 stage1msg->getOriginalPlayerID()));
        }
#endif
    }
    ...
    {
        // The local connection may be the packet router, in that case, the connection would be NULL.  So do something about
        // it!
        if ((m_packetRouterSlot >= 0) && (m_packetRouterSlot < MAX_SLOTS))
        {
            if (m_connections[m_packetRouterSlot] != NULL)
            {
                //				DEBUG_LOG(("ConnectionManager::ackCommand - acking command %d from player %d to packet
                //router.", commandID, m_packetRouterSlot));
            ...
            else if (m_localSlot == m_packetRouterSlot)
            {
                ...
                if ((msg->getPlayerID() >= 0) && (msg->getPlayerID() < MAX_SLOTS))
                {
                    if (m_connections[msg->getPlayerID()] != NULL)
                    {
                        //						DEBUG_LOG(("ConnectionManager::ackCommand - acking command %d from player %d
                        //directly to player.", commandID, msg->getPlayerID()));
                        m_connections[msg->getPlayerID()]->sendNetCommandMsg(ackmsg, 1 << msg->getPlayerID());
                    }
                    else
                    {
                        //					DEBUG_ASSERTCRASH(m_connections[msg->getPlayerID()] != NULL, ("Connection to
                        //player is NULL"));
                    }
                }
                else
...
void ConnectionManager::sendRemoteCommand(NetCommandRef *msg)
{
    ...
    DEBUG_LOG_LEVEL(
        DEBUG_LEVEL_NET,
        ("ConnectionManager::sendRemoteCommand - sending net command %d of type %s from player %d, relay is 0x%x",
         msg->getCommand()->getID(),
         GetNetCommandTypeAsString(msg->getCommand()->getNetCommandType()),
    ...
    if ((relay & (1 << m_localSlot)) && (m_frameData[msg->getCommand()->getPlayerID()] != NULL))
    {
        if (IsCommandSynchronized(msg->getCommand()->getNetCommandType()))
        {
            DEBUG_LOG_LEVEL(
                DEBUG_LEVEL_NET,
                ("ConnectionManager::sendRemoteCommand - adding net command of type %s to player %d for frame %d",
                 GetNetCommandTypeAsString(msg->getCommand()->getNetCommandType()),
                 msg->getCommand()->getPlayerID(),
    ...
    for (Int i = 0; i < MAX_SLOTS; ++i)
    {
        if ((relay & (1 << i)) && ((m_connections[i] != NULL) && (m_connections[i]->isQuitting() == FALSE)))
        {
            DEBUG_LOG_LEVEL(
                DEBUG_LEVEL_NET,
                ("ConnectionManager::sendRemoteCommand - relaying command %d to player %d", msg->getCommand()->getID(), i));
...
//				DEBUG_LOG(("ConnectionManager::update - calling doSend on connection %d", i));
        ...
        if ((m_frameData[i] != NULL) && (m_frameData[i]->getIsQuitting() == TRUE))
        {
            ...
            {
                DEBUG_LOG(
                    ("ConnectionManager::update - deleting frame data for slot %d on quitting frame %d",
                     i,
                     m_frameData[i]->getQuitFrame()));
                deleteInstance(m_frameData[i]);
                m_frameData[i] = NULL;
            }
        }
...
void ConnectionManager::updateRunAhead(Int oldRunAhead, Int frameRate, Bool didSelfSlug, Int nextExecutionFrame)
{
    ...
    if ((lasttimesent == 0) || ((curTime - lasttimesent) > TheGlobalData->m_networkRunAheadMetricsTime))
    {
        if (m_localSlot == m_packetRouterSlot)
        {
            ...
            if (didSelfSlug)
            {
                // DEBUG_LOG(("ConnectionManager::updateRunAhead - local player run ahead metrics, fps = %d, actual fps = %d,
                // latency = %f, didSelfSlug = true", m_fpsAverages[m_localSlot], m_frameMetrics.getAverageFPS(),
                // m_latencyAverages[m_localSlot]));
            }
            else
            {
                // DEBUG_LOG(("ConnectionManager::updateRunAhead - local player run ahead metrics, fps = %d, latency = %f,
                // didSelfSlug = false", m_fpsAverages[m_localSlot], m_latencyAverages[m_localSlot]));
            }
            Int minFps;
            Int minFpsPlayer;
            ...
            DEBUG_LOG_LEVEL(
                DEBUG_LEVEL_NET,
                ("ConnectionManager::updateRunAhead - max latency = %f, min fps = %d, min fps player = %d old FPS = %d",
                 getMaximumLatency(),
                 minFps,
                 minFpsPlayer,
                 frameRate));
            ...
            newRunAhead += (newRunAhead * TheGlobalData->m_networkRunAheadSlack)
                / 100; // Add in 10% of slack to the run ahead in case of network hiccups.
        ...
        else
        {
            ...
            if (didSelfSlug)
            {
                // DEBUG_LOG(("ConnectionManager::updateRunAhead - average latency = %f, average fps = %d, actual fps = %d,
                // didSelfSlug = true", m_frameMetrics.getAverageLatency(), m_frameMetrics.getAverageFPS(),
                // m_frameMetrics.getAverageFPS()));
            }
            else
            {
                // DEBUG_LOG(("ConnectionManager::updateRunAhead - average latency = %f, average fps = %d, didSelfSlug =
                // false", m_frameMetrics.getAverageLatency(), m_frameMetrics.getAverageFPS()));
            }
...
void ConnectionManager::processFrameTick(UnsignedInt frame)
{
    ...
    DEBUG_LOG_LEVEL(
        DEBUG_LEVEL_NET,
        ("ConnectionManager::processFrameTick - sending frame info for frame %d, ID %d, command count %d",
         frame,
         msg->getID(),
         commandCount));
...
{
    if (CommandRequiresDirectSend(msg) || (m_packetRouterSlot < 0) || (m_packetRouterSlot >= MAX_SLOTS)
    ...
    DEBUG_LOG_LEVEL(
        DEBUG_LEVEL_NET,
        ("ConnectionManager::sendLocalCommand - sending net command %d of type %s",
         msg->getID(),
    ...
    if (relay & (1 << m_localSlot))
    {
        DEBUG_LOG_LEVEL(
            DEBUG_LEVEL_NET,
            ("ConnectionManager::sendLocalCommand - adding net command of type %s to player %d for frame %d",
             GetNetCommandTypeAsString(msg->getNetCommandType()),
             msg->getPlayerID(),
        ...
        for (Int i = 0; i < MAX_SLOTS; ++i)
        {
            ...
            if (((m_connections[i] != NULL) && (m_connections[i]->isQuitting() == FALSE)) && (relay & (1 << i)))
            {
                ...
                m_connections[i]->sendNetCommandMsg(
                    msg,
                    temprelay); // This will create a new copy of netmsg for this connection.
            }
        }
    }
    else
    {
        ...
        UnsignedByte temprelay = relay & ~(1 << m_localSlot); // Tell the packet router to relay the message to everyone but
                                                              // myself. Hopefully the packet router is smart enough to not
                                                              // send it to slots that are not in the game.
...
void ConnectionManager::sendLocalCommandDirect(NetCommandMsg *msg, UnsignedByte relay)
{
    ...
    if (((relay & (1 << m_localSlot)) != 0) && (m_frameData[m_localSlot] != NULL))
    {
        if (IsCommandSynchronized(msg->getNetCommandType()) == TRUE)
        {
            DEBUG_LOG_LEVEL(
                DEBUG_LEVEL_NET,
                ("ConnectionManager::sendLocalCommandDirect - adding net command of type %s to player %d for frame %d",
                 GetNetCommandTypeAsString(msg->getNetCommandType()),
                 msg->getPlayerID(),
                 msg->getExecutionFrame()));
            m_frameData[m_localSlot]->addNetCommandMsg(msg);
    ...
    for (Int i = 0; i < MAX_SLOTS; ++i)
    {
        if ((relay & (1 << i)) != 0)
        {
            if ((m_connections[i] != NULL) && (m_connections[i]->isQuitting() == FALSE))
            {
                ...
                DEBUG_LOG_LEVEL(
                    DEBUG_LEVEL_NET,
                    ("ConnectionManager::sendLocalCommandDirect - Sending direct command %d of type %s to player %d",
                     msg->getID(),
                     GetNetCommandTypeAsString(msg->getNetCommandType()),
                     i));
            }
...
Bool ConnectionManager::allCommandsReady(UnsignedInt frame, Bool justTesting /* = FALSE */)
{
    ...
    for (; (i < MAX_SLOTS) && retval; ++i)
    {
        ...
        {
            /*
                        if (!(m_frameData[i]->allCommandsReady(frame, (frame != commandsReadyDebugSpewage) && (justTesting ==
            FALSE)))) { if ((frame != commandsReadyDebugSpewage) && (justTesting == FALSE)) {
            ...
            */
            ...
            frameRetVal =
                m_frameData[i]->allCommandsReady(frame, (frame != commandsReadyDebugSpewage) && (justTesting == FALSE));
    ...
    for (Int i = 0; i < MAX_SLOTS; ++i)
    {
        if (m_frameData[i] != NULL)
        {
            ...
            if (frame > FRAMES_TO_KEEP)
            {
                ...
                DEBUG_LOG_LEVEL(
                    DEBUG_LEVEL_NET,
                    ("getFrameCommandList - called reset frame on player %d for frame %d", i, frame - FRAMES_TO_KEEP));
            }
        }
    }
...
PlayerLeaveCode ConnectionManager::disconnectPlayer(Int slot)
{
    ...
    if (TheGameInfo)
    {
        ...
        {
            DEBUG_LOG(
                ("ConnectionManager::disconnectPlayer(%d) - slot is last in the game on frame %d",
                 slot,
                 TheGameLogic->getFrame()));
...
void ConnectionManager::parseUserList(const GameInfo *game)
{
    ...
    while ( (userStr=strtok_r(listPtr, ",", &listPos)) != NULL )
    {
        ...
        {
            DEBUG_LOG(("ConnectionManager::parseUserList - (numUsers = %d) FAILED parseUserList with list [%s]", numUsers,
buf)); return;
        }
...
void ConnectionManager::sendChat(UnicodeString text, Int playerMask, UnsignedInt executionFrame)
{
    ...
    DEBUG_LOG_LEVEL(
        DEBUG_LEVEL_NET,
        ("Chat message has ID of %d, mask of %8.8X, text of %ls", msg->getID(), msg->getPlayerMask(), msg->getText().str()));
...
UnsignedShort ConnectionManager::sendFileAnnounce(AsciiString path, UnsignedByte playerMask)
{
    ...
    DEBUG_LOG_LEVEL(
        DEBUG_LEVEL_NET,
        ("ConnectionManager::sendFileAnnounce() - creating announce message with ID of %d from %d to mask %X for '%s' going "
         "to %X as command %d",
         announceMsg->getID(),
         announceMsg->getPlayerID(),
         announceMask,
         announceMsg->getRealFilename().str(),
         announceMsg->getPlayerMask(),
...
void ConnectionManager::sendFile(AsciiString path, UnsignedByte playerMask, UnsignedShort commandID)
{
    ...
    Int compressedLen = path.endsWith(".tga") ?
        CompressionManager::getMaxCompressedSize(len, CompressionManager::getPreferredCompression()) :
        0;
    ...
    DEBUG_LOG_LEVEL(
        DEBUG_LEVEL_NET,
        ("ConnectionManager::sendFile() - creating file message with ID of %d for '%s' going to %X from %d, size of %d",
         fileMsg->getID(),
         fileMsg->getRealFilename().str(),
         playerMask,
         fileMsg->getPlayerID(),
         fileMsg->getFileLength()));
...
void ConnectionManager::debugPrintConnectionCommands()
{
    ...
    for (Int i = 0; i < MAX_SLOTS; ++i)
    {
        if (m_connections[i] != NULL)
        {
            DEBUG_LOG_LEVEL(
                DEBUG_LEVEL_NET,
                ("ConnectionManager::debugPrintConnectionCommands - commands for connection %d", i));
            m_connections[i]->debugPrintCommands();
        }
    }
...
void ConnectionManager::notifyOthersOfCurrentFrame(Int frame)
{
    ...
    DEBUG_LOG_LEVEL(
        DEBUG_LEVEL_NET,
        ("ConnectionManager::notifyOthersOfCurrentFrame - sending disconnect frame of %d, command ID = %d",
         frame,
         msg->getID()));
    sendLocalCommandDirect(msg, 0xff ^ (1 << m_localSlot));
...
void ConnectionManager::sendFrameDataToPlayer(UnsignedInt playerID, UnsignedInt startingFrame)
{
    DEBUG_LOG_LEVEL(
        DEBUG_LEVEL_NET,
        ("ConnectionManager::sendFrameDataToPlayer - sending frame data to player %d starting with frame %d",
         playerID,
         startingFrame));
    ...
    DEBUG_LOG_LEVEL(
        DEBUG_LEVEL_NET,
        ("ConnectionManager::sendFrameDataToPlayer - done sending commands to player %d", playerID));
}
...
void ConnectionManager::sendSingleFrameToPlayer(UnsignedInt playerID, UnsignedInt frame)
{
    if ((TheGameLogic->getFrame() - FRAMES_TO_KEEP) > frame)
    {
        DEBUG_LOG_LEVEL(
            DEBUG_LEVEL_NET,
            ("ConnectionManager::sendSingleFrameToPlayer - player %d requested frame %d when we are on frame %d, this is "
             "too far in the past.",
             playerID,
             frame,
             TheGameLogic->getFrame()));
    ...
    for (Int i = 0; i < MAX_SLOTS; ++i)
    {
        if ((m_frameData[i] != NULL) && (i != playerID))
        { // no need to send his own commands to him.
            ...
            if (list != NULL)
            {
                ...
                while (ref != NULL)
                {
                    DEBUG_LOG_LEVEL(
                        DEBUG_LEVEL_NET,
                        ("ConnectionManager::sendFrameDataToPlayer - sending command %d from player %d to player %d using "
                         "relay 0x%x",
            ...
            DEBUG_LOG_LEVEL(
                DEBUG_LEVEL_NET,
                ("ConnectionManager::sendFrameDataToPlayer - sending frame info from player %d to player %d for frame %d "
