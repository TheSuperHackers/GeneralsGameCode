...
//			}
			...
			// The network slack pushes the run ahead up to the next RA value when the latency is within the slack percentage of the current RA value
			Real networklatency =  getMaximumLatency() * ( 1.0f + ( (Real)TheGlobalData->m_networkRunAheadSlack / 100.0f ) );
			Int newRunAhead = ceilf( networklatency * (Real)minFps );

			// TheSuperHackers @info if the runahead goes bellow 3 logic frames it can start to introduce stutter
