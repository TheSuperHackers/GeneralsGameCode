        ...
        for (UnsignedInt node = 0; node < m_numNodes; ++node)
        {
            if (m_myConnections[node] == TRUE)
            {
                ...
                if (slot != NULL)
                {
                    ...
                    DEBUG_LOG(
                        ("NAT::connectionUpdate - sending keep alive to node %d at %d.%d.%d.%d:%d",
                         node,
    ...
    for (Int i = 0; i < MAX_MESSAGES; ++i)
    {
        if (m_transport->m_inBuffer[i].length > 0)
        {
...
#endif
            DEBUG_LOG(
                ("NAT::connectionUpdate - got a packet from %d.%d.%d.%d:%d, length = %d",
                 PRINTF_IP_AS_4_INTS(ip),
                 m_transport->m_inBuffer[i].port,
                 m_transport->m_inBuffer[i].length));
            ...
            if (memcmp(data, "PROBE", strlen("PROBE")) == 0)
            {
                ...
                {
                    DEBUG_LOG(
                        ("NAT::connectionUpdate - probe was sent by our target, setting connection state %d to done.",
                         m_targetNodeNumber));
...
#endif
                        DEBUG_LOG(
                            ("NAT::connectionUpdate - incomming packet has different from address than we expected, "
                             "incoming: %d.%d.%d.%d expected: %d.%d.%d.%d",
                    ...
                    {
                        DEBUG_LOG(
                            ("NAT::connectionUpdate - incoming packet came from a different port than we expected, "
                             "incoming: %d expected: %d",
                             m_transport->m_inBuffer[i].port,
                             targetSlot->getPort()));
        ...
        else
        {
            if (timeGetTime() >= m_manglerRetryTime)
            {
                ...
                else
                {
                    ...
                    {
                        DEBUG_LOG(
                            ("NAT::connectionUpdate - trying to send to the mangler again. mangler address: %d.%d.%d.%d, "
                             "from port: %d, packet ID:%d",
        ...
        if ((m_slotList != NULL) && (m_slotList[i] != NULL))
        {
            if ((m_slotList[i]->getNATBehavior() & FirewallHelperClass::FIREWALL_TYPE_NETGEAR_BUG) != 0)
            {
                if (otherNetgearNum == -1)
                {
                    ...
                    Int nodeindex = 0;
                    while ((m_connectionPairs[m_connectionPairIndex][0][nodeindex] == -1)
                    ...
                    DEBUG_LOG(
                        ("NAT::establishConnectionPaths - first netgear in pair. assigning node %d to slot %d (%ls)",
                         nodeindex,
                         i,
                ...
                else
                {
                    ...
                    otherNetgearNum = -1;
                    DEBUG_LOG(
                        ("NAT::establishConnectionPaths - second netgear in pair. assigning node %d to slot %d (%ls)",
                         nodeindex,
                         i,
                         m_slotList[i]->getName().str()));
                }
...
void NAT::attachSlotList(GameSlot *slotList[], Int localSlot, UnsignedInt localIP)
{
    ...
    DEBUG_LOG(
        ("NAT::attachSlotList - initting the transport socket with address %d.%d.%d.%d:%d",
         PRINTF_IP_AS_4_INTS(m_localIP),
         getSlotPort(localSlot)));
        ...
        if (targetNodeNumber != -1)
        {
            if (i == m_localNodeNumber)
            {
                ...
                UnsignedInt targetSlotIndex =
                    m_connectionNodes[(m_connectionPairs[m_connectionPairIndex][m_connectionRound][i])].m_slotIndex;
                ...
                DEBUG_LOG(
                    ("NAT::doThisConnectionRound - Target slot index = %d (%ls)",
                     targetSlotIndex,
                     m_slotList[targetSlotIndex]->getName().str()));
                DEBUG_LOG(
                    ("NAT::doThisConnectionRound - Target slot has NAT behavior 0x%8X, local slot has NAT behavior 0x%8X",
                ...
                DEBUG_LOG(
                    ("NAT::doThisConnectionRound - Target slot has IP %d.%d.%d.%d  Local slot has IP %d.%d.%d.%d",
                    ...
                    && ((localSlot->getNATBehavior() & FirewallHelperClass::FIREWALL_TYPE_NETGEAR_BUG) != 0))
                {
                    ...
                    DEBUG_LOG(
                        ("NAT::doThisConnectionRound - Local node has a netgear and the target node does not, need to delay "
                         "our probe."));
...
void NAT::sendMangledSourcePort()
{
    ...
    if (targetSlot->getIP() == localSlot->getIP())
    {
        ...
        DEBUG_LOG(
            ("NAT::sendMangledSourcePort - I am %ls, target is %ls, my IP is %d.%d.%d.%d, target IP is %d.%d.%d.%d",
             localSlot->getName().str(),
    ...
    {
        DEBUG_LOG(
            ("NAT::sendMangledSourcePort - our firewall doesn't NAT based on destination address, checking for old "
    ...
    DEBUG_LOG(
        ("NAT::sendMangledSourcePort - mangler %s address is %d.%d.%d.%d",
         manglerName,
         PRINTF_IP_AS_4_INTS(m_manglerAddress)));
...
void NAT::gotMangledPort(Int nodeNumber, UnsignedShort mangledPort)
{
    ...
    {
        DEBUG_LOG(
            ("NAT::gotMangledPort - got a mangled port number for someone that isn't my target. node = %d, target node = %d",
             nodeNumber,
             m_targetNodeNumber));
        return;
    }
    ...
    DEBUG_LOG(
        ("NAT::gotMangledPort - got mangled port number %d from our target node (%ls)",
         mangledPort,
         targetSlot->getName().str()));
    if (((localSlot->getNATBehavior() & FirewallHelperClass::FIREWALL_TYPE_NETGEAR_BUG) == 0) || (m_beenProbed == TRUE)
        || (((localSlot->getNATBehavior() & FirewallHelperClass::FIREWALL_TYPE_NETGEAR_BUG) != 0)
            && ((targetSlot->getNATBehavior() & FirewallHelperClass::FIREWALL_TYPE_NETGEAR_BUG) != 0)))
    {
#ifdef DEBUG_LOGGING
        UnsignedInt ip = targetSlot->getIP();
#endif
        DEBUG_LOG(
            ("NAT::gotMangledPort - don't have a netgear or we have already been probed, or both my target and I have a "
             "netgear, send a PROBE. Sending to %d.%d.%d.%d:%d",
             PRINTF_IP_AS_4_INTS(ip),
             targetSlot->getPort()));
...
void NAT::gotInternalAddress(Int nodeNumber, UnsignedInt address)
{
    ...
    {
        DEBUG_LOG((
            "NAT::gotInternalAddress - got a internal address for someone that isn't my target. node = %d, target node = %d",
            nodeNumber,
            m_targetNodeNumber));
        return;
    }
    ...
    if (localSlot->getIP() == targetSlot->getIP())
    {
        ...
        DEBUG_LOG(
            ("NAT::gotInternalAddress - target and local players have same external address, using internal address."));
...
void NAT::processGlobalMessage(Int slotNum, const char *options)
{
    ...
    else if (!strncmp(ptr, "CONNDONE", strlen("CONNDONE")))
    {
        ...
        */
        ...
        if (m_connectionPairs[m_connectionPairIndex][m_connectionRound][node] == sendingNode)
        {
            ...
            {
                DEBUG_LOG(
                    ("NAT::processGlobalMessage - node %d's connection is complete, setting connection state to done",
                     node));
        ...
        {
            DEBUG_LOG(
                ("NAT::processGlobalMessage - got a connection done message that isn't from this round. node: %d sending "
                 "node: %d",
                 node,
                 sendingNode));
        }
    ...
    else if (!strncmp(ptr, "PORT", strlen("PORT")))
    {
        ...
        DEBUG_LOG(
            ("NAT::processGlobalMessage - got port message from node %d, port: %d, internal address: %d.%d.%d.%d",
             node,
             port,
