		...
		if (m_transport->m_inBuffer[i].length > 0)
		{
			...
			if (memcmp(data, "PROBE", strlen("PROBE")) == 0)
			{
				...
				if (fromNode == m_targetNodeNumber)
				{
...
#endif
						DEBUG_LOG(
								("NAT::connectionUpdate - incomming packet has different from address than we expected, incoming: "
					...
					{
						DEBUG_LOG(
								("NAT::connectionUpdate - incoming packet came from a different port than we expected, incoming: %d "
								 "expected: %d",
								 m_transport->m_inBuffer[i].port,
								 targetSlot->getPort()));
		...
		else
		{
			if (timeGetTime() >= m_manglerRetryTime)
			{
				...
				else
				{
					...
					{
						DEBUG_LOG(
								("NAT::connectionUpdate - trying to send to the mangler again. mangler address: %d.%d.%d.%d, from port: %d, "
								 "packet ID:%d",
		...
		if (targetNodeNumber != -1)
		{
			if (i == m_localNodeNumber)
			{
						...
						&& ((localSlot->getNATBehavior() & FirewallHelperClass::FIREWALL_TYPE_NETGEAR_BUG) != 0))
				{
					...
					DEBUG_LOG(
							("NAT::doThisConnectionRound - Local node has a netgear and the target node does not, need to delay our "
							 "probe."));
...
void NAT::sendMangledSourcePort()
{
	...
	{
		DEBUG_LOG(
				("NAT::sendMangledSourcePort - our firewall doesn't NAT based on destination address, checking for old connections "
				 "from this address"));
...
void NAT::processGlobalMessage(Int slotNum, const char *options)
{
	...
	else if (!strncmp(ptr, "CONNDONE", strlen("CONNDONE")))
	{
		...
		*/
		...
		{
			DEBUG_LOG(
					("NAT::processGlobalMessage - got a connection done message that isn't from this round. node: %d sending node: %d",
					 node,
					 sendingNode));
		}
	}
