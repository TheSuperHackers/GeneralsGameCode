...
void PopulatePlayerTemplateComboBox(Int comboBox, GameWindow *comboArray[],
  ...
  for (Int c = 0; c < numPlayerTemplates; ++c) {
    ...
    // This is also enforced at game loading (GameLogic.cpp and
    // UserPreferences.cpp).
    // @todo: unlock these when something rad happens
    Bool disallowLockedGenerals = TRUE;
    const GeneralPersona *general =
        TheChallengeGenerals->getGeneralByTemplateName(fac->getName());
...
formatMoneyForStartingCashComboBox(const Money &moneyAmount) {
  UnicodeString rtn;
  rtn.format(TheGameText->fetch("GUI:StartingMoneyFormat"),
             moneyAmount.countMoney());
  return rtn;
}
...
void PopulateStartingCashComboBox(GameWindow *comboBox, GameInfo *myGame) {
  ...
  const MultiplayerStartingMoneyList &startingCashMap =
      TheMultiplayerSettings->getStartingMoneyList();
  Int currentSelectionIndex = -1;

  MultiplayerStartingMoneyList::const_iterator it = startingCashMap.begin();
  for (; it != startingCashMap.end(); it++) {
    Int newIndex = GadgetComboBoxAddEntry(
        comboBox, formatMoneyForStartingCashComboBox(*it),
        comboBox->winGetEnabled() ? comboBox->winGetEnabledTextColor()
                                  : comboBox->winGetDisabledTextColor());
  ...
  if (currentSelectionIndex == -1) {
    ...
    currentSelectionIndex = GadgetComboBoxAddEntry(
        comboBox, formatMoneyForStartingCashComboBox(myGame->getStartingCash()),
        comboBox->winGetEnabled() ? comboBox->winGetEnabledTextColor()
                                  : comboBox->winGetDisabledTextColor());
    GadgetComboBoxSetItemData(comboBox, currentSelectionIndex,
                              (void *)it->countMoney());
  }
  ...
  if (myGame) {
    for (int i = 0; i < MAX_SLOTS; i++) {
      ...
      if ((comboColor[i] != nullptr) &&
          BitIsSet(comboColor[i]->winGetStatus(), WIN_STATUS_ENABLED))
      ...
      if (comboColor[i] != nullptr) {
        ...
        for (idx = 0; idx < max; ++idx) {
          Int color =
              (Int)(intptr_t)GadgetComboBoxGetItemData(comboColor[i], idx);
      ...
      if (comboTeam[i] != nullptr) {
        ...
        for (idx = 0; idx < max; ++idx) {
          Int team =
              (Int)(intptr_t)GadgetComboBoxGetItemData(comboTeam[i], idx);
      ...
      if (comboPlayerTemplate[i] != nullptr) {
        ...
        for (idx = 0; idx < max; ++idx) {
          Int playerTemplate = (Int)(intptr_t)GadgetComboBoxGetItemData(
              comboPlayerTemplate[i], idx);
