...
{
    DEBUG_LOG(
        ("LANAPI::LANAPI() - max game option size is %d, sizeof(LANMessage)=%d, MAX_PACKET_SIZE=%d",
        ...
        if (m_transport->m_inBuffer[i].length > 0)
        {
            ...
            switch (msg->LANMessageType)
            {
                ...
                case LANMessage::MSG_REQUEST_LOCATIONS: // Hey, where is everybody?
                    DEBUG_LOG(
                        ("LANAPI::update - got a MSG_REQUEST_LOCATIONS from %d.%d.%d.%d", PRINTF_IP_AS_4_INTS(senderIP)));
                ...
                case LANMessage::MSG_REQUEST_GAME_INFO:
                    DEBUG_LOG(
                        ("LANAPI::update - got a MSG_REQUEST_GAME_INFO from %d.%d.%d.%d", PRINTF_IP_AS_4_INTS(senderIP)));
                ...
                case LANMessage::MSG_REQUEST_GAME_LEAVE: // I'm outa here!
                    DEBUG_LOG(
                        ("LANAPI::update - got a MSG_REQUEST_GAME_LEAVE from %d.%d.%d.%d", PRINTF_IP_AS_4_INTS(senderIP)));
                    handleRequestGameLeave(msg, senderIP);
                    break;
                case LANMessage::MSG_REQUEST_LOBBY_LEAVE: // I'm outa here!
                    DEBUG_LOG(
                        ("LANAPI::update - got a MSG_REQUEST_LOBBY_LEAVE from %d.%d.%d.%d", PRINTF_IP_AS_4_INTS(senderIP)));
    ...
    if (m_currentGame && !(m_currentGame->getIsDirectConnect()))
    {
        if (m_currentGame->getIP(0) == m_localIP
            || (m_currentGame->isGameInProgress() && TheNetwork
                && TheNetwork->isPacketRouter())) // if we're in game we should reply if we're the packet router
        {
            LANMessage reply;
