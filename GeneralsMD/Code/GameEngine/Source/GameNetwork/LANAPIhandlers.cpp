...
/*			if (msg->GameToJoin.iniCRC != TheGlobalData->m_iniCRC ||
            ...
            {
                DEBUG_LOG(("LANAPI::handleRequestJoin - join denied because of CRC mismatch. CRCs are them/us INI:%X/%X
   exe:%X/%X", msg->GameToJoin.iniCRC, TheGlobalData->m_iniCRC, msg->GameToJoin.exeCRC, TheGlobalData->m_exeCRC));
                reply.LANMessageType = LANMessage::MSG_JOIN_DENY;
