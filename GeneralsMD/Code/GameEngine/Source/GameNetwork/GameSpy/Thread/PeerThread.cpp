...
const char *PeerThreadClass::s_values[6] =
    {s_valueBuffers[0], s_valueBuffers[1], s_valueBuffers[2], s_valueBuffers[3], s_valueBuffers[4], s_valueBuffers[5]};
    ...
    try
    {
        ...
        **********/
        ...
        while (running)
        {
            ...
            {
                DEBUG_LOG(
                    ("TheGameSpyPeerMessageQueue->getRequest() got request of type %d", incomingRequest.peerRequestType));
                switch (incomingRequest.peerRequestType)
                {
                    ...
                    case PeerRequest::PEERREQUEST_CREATESTAGINGROOM:
                    {
                        ...
                        // WideCharStringToMultiByte(incomingRequest.text.c_str()).c_str(), MAX_SLOTS,
                        // incomingRequest.password.c_str(), qr2Sock, preferredQRPort, createRoomCallback, (void *)&res,
                        // PEERTrue);
                        DEBUG_LOG(
                        ...
                        if (m_sawEndOfEnumPlayers)
                        {
                            if (m_sawMatchbot)
                            {
                                ...
                                snprintf(
                                    ...
                                    64,
                                    "\\IP\\%d",
                                    ntohl(peerGetLocalIP(peer))); // not ntohl(localIP), as we need EXTERNAL address for
                                                                  // proper NAT negotiation!
                                msg.append(buf);
    ...
    void *param)
{
    ...
    {
        DEBUG_ASSERTCRASH(
            nick && strcmp(nick, "(END)") == 0,
            ("roomKeyChangedCallback bad values = nick:%X:%s, key:%X:%s, val:%X:%s", nick, nick, key, key, val, val));
        return;
    }
    ...
    void *param)
{
    ...
    {
        DEBUG_LOG(
            ("Bailing from listingGamesCallback() - success=%d, name=%X, server=%X, msg=%X", success, name, server, msg));
        return;
    }
    if (!name)
        name = "bogus";
    ...
    if (server && (msg == PEER_ADD || msg == PEER_UPDATE))
    {
        ...
        DEBUG_LOG(
            ("Raw stuff: [%s] [%s] [%s] [%d] [%d] [%d]", verStr, exeStr, iniStr, hasPassword, allowObservers, usesStats));
