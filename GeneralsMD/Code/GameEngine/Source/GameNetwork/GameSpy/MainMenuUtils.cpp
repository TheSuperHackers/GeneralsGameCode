...
void StartDownloadingPatches(void)
{
    ...
    if (TheDownloadManager)
    {
        ...
        while (it != queuedDownloads.end())
        {
            QueuedDownload q = *it;
            TheDownloadManager
                ->queueFileForDownload(q.server, q.userName, q.password, q.file, q.localFile, q.regKey, q.tryResume);
...
{
    ghttpGet("http://gamestats.gamespy.com/ccgenzh/display.html", GHTTPFalse, overallStatsCallback, NULL);
}
...
{
    ghttpGet(
        "http://launch.gamespyarcade.com/software/launch/arcadecount2.dll?svcname=ccgenzh",
        GHTTPFalse,
