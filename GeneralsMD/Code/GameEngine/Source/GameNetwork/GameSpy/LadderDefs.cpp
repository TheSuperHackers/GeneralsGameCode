...
static LadderInfo *parseLadder(AsciiString raw)
{
	...
	while (raw.nextToken(&line, "\n"))
	{
		...
		*/
		...
		{
			DEBUG_LOG(
					("Saw a ladder: name=%ls, addr=%s:%d, players=%dv%d, pass=%s, replay=%d, homepage=%s",
					 lad->name.str(),
					 lad->address.str(),
					 lad->port,
					 lad->playersPerTeam,
					 lad->playersPerTeam,
					 lad->cryptedPassword.str(),
					 lad->submitReplay,
					 lad->homepageURL.str()));
			// end of a ladder
			if (lad->playersPerTeam >= 1 && lad->playersPerTeam <= MAX_SLOTS / 2)
			{
				...
				else
				{
					...
					for (AsciiStringListIterator it = validFactions.begin(); it != validFactions.end(); ++it)
					{
						...
						DEBUG_LOG(
								("Faction %s has marker %s corresponding to str %ls",
								 faction.str(),
								 marker.str(),
