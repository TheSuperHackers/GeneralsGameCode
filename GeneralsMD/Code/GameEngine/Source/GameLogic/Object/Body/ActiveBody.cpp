                ...
                && !contain->isImmuneToClearBuildingAttacks())
            {
                ...
                if (items)
                {
                    for (ContainedItemsList::const_iterator it = items->begin();
                         (it != items->end()) && (numKilled < killsToMake);
                         it++)
                    {
...
//		if (damageInfo->in.m_damageType == DAMAGE_SURRENDER && m_currentHealth <= 0.0f &&
//obj->isKindOf(KINDOF_CAN_SURRENDER))
//		{
        ...
        && controllingPlayer->getPlayerType() == PLAYER_HUMAN)
    {
        if (shouldRetaliateAgainstAggressor(obj, damager))
        {
            ...
            SimpleObjectIterator *iter =
                ThePartitionManager
                    ->iterateObjectsInRange(obj->getPosition(), distance, FROM_CENTER_2D, filters, ITER_FASTEST);
