    ...
    if (!isForcedAttack(attackType))
    {
        ...
        if (victimContain)
        {
            const Player *victimApparentController =
                victimContain->getApparentControllingPlayer(source->getControllingPlayer());
    ...
    if (hasAnyDamageWeapon())
    {
        ...
        for (Int i = first; i >= last; --i)
        {
            ...
            if (weapon && weapon->estimateWeaponDamage(source, victim))
            {
                ...
                if (weapon->getDamageType() == DAMAGE_KILLPILOT && source->isKindOf(KINDOF_HERO)
                    && m_curWeapon == PRIMARY_WEAPON && specificSlot == (WeaponSlotType)-1)
                {
                    continue;
                }
            ...
            for (ContainedItemsList::const_iterator it = items->begin(); it != items->end(); ++it)
            {
                ...
                if (garrisonedMember->isAbleToAttack())
                {
                    CanAttackResult result =
                        garrisonedMember->getAbleToUseWeaponAgainstTarget(attackType, victim, pos, commandSource);
