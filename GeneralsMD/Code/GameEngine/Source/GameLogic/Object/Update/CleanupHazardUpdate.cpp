...
void CleanupHazardUpdate::onObjectCreated()
{
	...
	{
		DEBUG_CRASH(
				("CleanupHazardUpdate for %s doesn't have a valid weapon template", getObject()->getTemplate()->getName().str()));
		return;
	}
	...
	{
		DEBUG_CRASH(
				("CleanupHazardUpdate for %s requires the scan range (%.1f) being larger than the firing range (%.1f)",
...
Object *CleanupHazardUpdate::scanClosestTarget()
{
	...
	if (m_moveRange > 0.0f)
	{
		...
		bestTargetInRange =
				ThePartitionManager->getClosestObject(&m_pos, data->m_scanRange + m_moveRange, FROM_CENTER_2D, filters);
	}
	else
	{
