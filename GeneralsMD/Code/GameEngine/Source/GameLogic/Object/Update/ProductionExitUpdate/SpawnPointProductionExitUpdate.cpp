...
void SpawnPointProductionExitUpdate::exitObjectViaDoor(Object *newObj, ExitDoorType exitDoor)
{
  ...
  if (creationObject)
  {
    ...
    createPoint.z =
        TheTerrainLogic ? TheTerrainLogic->getLayerHeight(createPoint.x, createPoint.y, creationObject->getLayer()) : 0.0f;
...
void SpawnPointProductionExitUpdate::initializeBonePositions()
{
  ...
  m_spawnPointCount =
      myDrawable->getPristineBonePositions(md->m_spawnPointBoneNameData.str(), 1, NULL, boneTransforms, MAX_SPAWN_POINTS);
...
void SpawnPointProductionExitUpdate::xfer(Xfer *xfer)
{
  ...
  //
  //	Bool m_bonesInitialized;													///< To prevent creation bugs, only init the World coords when
  // first asked for one 	Int m_spawnPointCount;														///< How many in the array are actually live and
  // valid 	Coord3D m_worldCoordSpawnPoints[MAX_SPAWN_POINTS];///< Where my little friends will be created 	Real
  // m_worldAngleSpawnPoints[MAX_SPAWN_POINTS];		///< And what direction they should face
