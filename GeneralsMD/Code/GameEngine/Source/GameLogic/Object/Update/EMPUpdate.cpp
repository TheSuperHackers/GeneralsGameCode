...
void EMPUpdate::doDisableAttack(void)
{
    ...
    if (radius > 0.0f)
    {
        iter = ThePartitionManager->iterateObjectsInRange(pos, radius, FROM_BOUNDINGSPHERE_3D);
    ...
    for (; curVictim != NULL; curVictim = iter ? iter->nextWithNumeric(&curVictimDistSqr) : NULL)
    {
        if (curVictim != object)
        {
            ...
            if (!curVictim->isKindOf(KINDOF_VEHICLE) && !curVictim->isKindOf(KINDOF_STRUCTURE)
...
void LeafletDropBehavior::doDisableAttack(void)
{
    ...
    for (; curVictim != NULL; curVictim = iter ? iter->nextWithNumeric(&curVictimDistSqr) : NULL)
    {
        if (curVictim != object)
        {
            if (!(curVictim->isKindOf(KINDOF_INFANTRY)
                  || curVictim->isKindOf(KINDOF_VEHICLE))) // both commuters and pedestrians
                continue;
