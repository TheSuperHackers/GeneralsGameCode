...
/*static*/ void ParticleUplinkCannonUpdateModuleData::buildFieldParse(MultiIniFieldParse &p)
{
  ...
  static const FieldParse dataFieldParse[] = {
    ...
    { "OuterEffectBoneName",
      ...
      NULL,
      offsetof(ParticleUplinkCannonUpdateModuleData, m_outerEffectBaseBoneName) },
    { "OuterEffectNumBones",
    ...
    { "OuterNodesLightFlareParticleSystem",
      INI::parseAsciiString,
      NULL,
      offsetof(ParticleUplinkCannonUpdateModuleData, m_outerNodesLightFlareParticleSystemName) },
    { "OuterNodesMediumFlareParticleSystem",
      INI::parseAsciiString,
      NULL,
      offsetof(ParticleUplinkCannonUpdateModuleData, m_outerNodesMediumFlareParticleSystemName) },
    { "OuterNodesIntenseFlareParticleSystem",
      INI::parseAsciiString,
      NULL,
      offsetof(ParticleUplinkCannonUpdateModuleData, m_outerNodesIntenseFlareParticleSystemName) },
    ...
    { "ConnectorMediumLaserName",
      INI::parseAsciiString,
      NULL,
      offsetof(ParticleUplinkCannonUpdateModuleData, m_connectorMediumLaserNameName) },
    { "ConnectorIntenseLaserName",
      INI::parseAsciiString,
      NULL,
      offsetof(ParticleUplinkCannonUpdateModuleData, m_connectorIntenseLaserNameName) },
    { "ConnectorMediumFlare",
      INI::parseAsciiString,
      NULL,
      offsetof(ParticleUplinkCannonUpdateModuleData, m_connectorMediumFlareParticleSystemName) },
    { "ConnectorIntenseFlare",
      INI::parseAsciiString,
      NULL,
      offsetof(ParticleUplinkCannonUpdateModuleData, m_connectorIntenseFlareParticleSystemName) },
    ...
    { "LaserBaseLightFlareParticleSystemName",
      INI::parseAsciiString,
      NULL,
      offsetof(ParticleUplinkCannonUpdateModuleData, m_laserBaseLightFlareParticleSystemName) },
    { "LaserBaseMediumFlareParticleSystemName",
      INI::parseAsciiString,
      NULL,
      offsetof(ParticleUplinkCannonUpdateModuleData, m_laserBaseMediumFlareParticleSystemName) },
    { "LaserBaseIntenseFlareParticleSystemName",
      INI::parseAsciiString,
      NULL,
      offsetof(ParticleUplinkCannonUpdateModuleData, m_laserBaseIntenseFlareParticleSystemName) },
    ...
    { "DelayBetweenLaunchFX",
      INI::parseDurationUnsignedInt,
      NULL,
      offsetof(ParticleUplinkCannonUpdateModuleData, m_framesBetweenLaunchFXRefresh) },
    ...
    { "DamageType",
      DamageTypeFlags::parseSingleBitFromINI,
      NULL,
    ...
    { "UnpackToIdleSoundLoop",
      INI::parseAsciiString,
      NULL,
      offsetof(ParticleUplinkCannonUpdateModuleData, m_unpackToReadySoundName) },
    ...
    { "DamagePulseRemnantObjectName",
      INI::parseAsciiString,
      NULL,
      offsetof(ParticleUplinkCannonUpdateModuleData, m_damagePulseRemnantObjectName) },
    ...
    { "ManualFastDrivingSpeed",
      INI::parseReal,
      NULL,
      offsetof(ParticleUplinkCannonUpdateModuleData, m_manualFastDrivingSpeed) },
...
void ParticleUplinkCannonUpdate::onObjectCreated()
{
  ...
  {
    DEBUG_CRASH(
        ("%s object's ParticleUplinkCannonUpdate lacks access to the SpecialPowerTemplate. Needs to be specified in ini.",
...
//-------------------------------------------------------------------------------------------------
Bool ParticleUplinkCannonUpdate::initiateIntentToDoSpecialPower(
...
UpdateSleepTime ParticleUplinkCannonUpdate::update()
{
  ...
  if (m_startAttackFrame != 0 && m_startAttackFrame <= now)
  {
    ...
    {
      if (me->isDisabledByType(DISABLED_UNDERPOWERED) || me->isDisabledByType(DISABLED_EMP)
          || me->isDisabledByType(DISABLED_SUBDUED) || me->isDisabledByType(DISABLED_HACKED))
      {
    ...
    if (isFiring)
    {
      if (!m_manualTargetMode && !m_scriptedWaypointMode)
      {
        ...
        Real cxDistance =
            (factor * data->m_swathOfDeathDistance) - (data->m_swathOfDeathDistance * 0.5f); // cx is cartesian x
        ...
        dotProduct = __min(0.99999f, __max(-0.99999f, dotProduct)); // Account for numerical errors.  Also, acos(-1.00000) is
                                                                    // coming out QNAN on the superweapon general map.  Heh.
      ...
      else
      {
        ...
        if (m_scriptedWaypointMode
            || m_lastDrivingClickFrame - m_2ndLastDrivingClickFrame < data->m_doubleClickToFastDriveDelay)
        {
        ...
        if (distance < speed)
        {
          ...
          if (m_scriptedWaypointMode)
          {
            ...
            if (way)
            {
              ...
              // TheSuperHackers @bugfix Caball009 27/06/2025 Check if the last way point has been reached
              // before attempting to access the next way point.
              if (way)
              {
                m_nextDestWaypointID = way->getID();
      ...
      if (beam)
      {
        ...
        if (update)
        {
          ...
          // The logical laser radius for the damage should probably be part of
      ...
      DEBUG_ASSERTCRASH(
          logicalLaserRadius == visualLaserRadius,
          ("ParticleUplinkCannonUpdate's laser radius does not match LaserUpdate's laser radius - will cause mismatch "
           "in VS6 retail compatible builds"));
#endif
      ...
      if (m_nextScorchMarkFrame <= now)
      {
        ...
        ThePartitionManager->doShroudReveal(
            m_currentTargetPosition.x,
            m_currentTargetPosition.y,
            data->m_revealRange,
            me->getControllingPlayer()->getPlayerMask());
      ...
      if (m_nextDamagePulseFrame <= now)
      {
        ...
        ObjectIterator *iter =
            ThePartitionManager->iterateObjectsInRange(&m_currentTargetPosition, damageRadius, FROM_CENTER_2D, filters);
...
Bool ParticleUplinkCannonUpdate::calculateDefaultInformation()
{
  ...
  int numBones = obj->getMultiLogicalBonePosition(
      data->m_outerEffectBaseBoneName.str(),
      data->m_outerEffectNumBones,
      bonePositions,
      boneMatrices,
      FALSE);
  ...
  {
    DEBUG_CRASH(
        ("Particle cannon requires %d outer node bones, but can only find %d bones.",
         data->m_outerEffectNumBones,
         numBones));
    m_invalidSettings = TRUE;
    return FALSE;
...
Bool ParticleUplinkCannonUpdate::calculateUpBonePositions()
{
  ...
  if (draw)
  {
    if (data->m_connectorBoneName.isNotEmpty()
        && draw->getCurrentClientBonePositions(data->m_connectorBoneName.str(), 0, &pos, &mtx, 1))
    {
      obj->convertBonePosToWorldPos(&pos, &mtx, &m_connectorNodePosition, &mtx);
    }
    if (data->m_connectorBoneName.isNotEmpty()
        && draw->getCurrentClientBonePositions(data->m_fireBoneName.str(), 0, &pos, &mtx, 1))
    {
  ...
  switch (newStatus)
  {
    case STATUS_IDLE:
    {
      ...
      obj->clearModelConditionFlags(
          MAKE_MODELCONDITION_MASK3(MODELCONDITION_PACKING, MODELCONDITION_UNPACKING, MODELCONDITION_DEPLOYED));
      TheAudio->removeAudioEvent(m_powerupSound.getPlayingHandle());
      TheAudio->removeAudioEvent(m_unpackToReadySound.getPlayingHandle());
      TheAudio->removeAudioEvent(m_firingToIdleSound.getPlayingHandle());
      TheAudio->removeAudioEvent(m_annihilationSound.getPlayingHandle());
    ...
    case STATUS_PREPARING:
    {
      ...
      obj->clearAndSetModelConditionFlags(
          MAKE_MODELCONDITION_MASK2(MODELCONDITION_PACKING, MODELCONDITION_DEPLOYED),
    ...
    case STATUS_ALMOST_READY:
    {
      ...
      obj->clearAndSetModelConditionFlags(
          MAKE_MODELCONDITION_MASK2(MODELCONDITION_PACKING, MODELCONDITION_UNPACKING),
