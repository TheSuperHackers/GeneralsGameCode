	...
	if (m_startAttackFrame != 0 && m_startAttackFrame <= now)
	{
		...
		if (isFiring)
		{
			if (!m_manualTargetMode && !m_scriptedWaypointMode)
			{
				...
				Real cxDistance = (factor * data->m_swathOfDeathDistance) - (data->m_swathOfDeathDistance * 0.5f); // cx is cartesian
																																																					 // x
				...
				if (distance < speed)
				{
					...
					if (m_scriptedWaypointMode)
					{
						...
						if (way)
						{
							...
							// TheSuperHackers @bugfix Caball009 27/06/2025 Check if the last way point has been reached before attempting
							// to access the next way point.
							if (way)
							{
								m_nextDestWaypointID = way->getID();
			...
			DEBUG_ASSERTCRASH(
					...
					("ParticleUplinkCannonUpdate's laser radius does not match LaserUpdate's laser radius - will cause mismatch in "
					 "VS6 retail compatible builds"));
#endif
