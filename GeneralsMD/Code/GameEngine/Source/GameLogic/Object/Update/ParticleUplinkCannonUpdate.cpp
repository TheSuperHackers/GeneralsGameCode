    ...
    if (m_startAttackFrame != 0 && m_startAttackFrame <= now)
    {
        ...
        if (isFiring)
        {
            if (!m_manualTargetMode && !m_scriptedWaypointMode)
            {
                ...
                dotProduct =
                    __min(0.99999f, __max(-0.99999f, dotProduct)); // Account for numerical errors.  Also, acos(-1.00000) is
            ...
            m_currentTargetPosition.z =
                TheTerrainLogic->getGroundHeight(m_currentTargetPosition.x, m_currentTargetPosition.y);
