...
static const ModelConditionFlagType theOpeningFlags[DOOR_COUNT_MAX] = { MODELCONDITION_DOOR_1_OPENING,
																																				MODELCONDITION_DOOR_2_OPENING,
																																				MODELCONDITION_DOOR_3_OPENING,
																																				MODELCONDITION_DOOR_4_OPENING };

static const ModelConditionFlagType theClosingFlags[DOOR_COUNT_MAX] = { MODELCONDITION_DOOR_1_CLOSING,
																																				MODELCONDITION_DOOR_2_CLOSING,
																																				MODELCONDITION_DOOR_3_CLOSING,
																																				MODELCONDITION_DOOR_4_CLOSING };

static const ModelConditionFlagType theWaitingOpenFlags[DOOR_COUNT_MAX] = { MODELCONDITION_DOOR_1_WAITING_OPEN,
																																						MODELCONDITION_DOOR_2_WAITING_OPEN,
																																						MODELCONDITION_DOOR_3_WAITING_OPEN,
																																						MODELCONDITION_DOOR_4_WAITING_OPEN };

static const ModelConditionFlagType theWaitingToCloseFlags[DOOR_COUNT_MAX] = { MODELCONDITION_DOOR_1_WAITING_TO_CLOSE,
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
ProductionUpdate::ProductionUpdate(Thing *thing, const ModuleData *moduleData) : UpdateModule(thing, moduleData)
{
...
Bool ProductionUpdate::queueUpgrade(const UpgradeTemplate *upgrade)
{
	...
	if (upgrade->getUpgradeType() == UPGRADE_TYPE_PLAYER && TheUpgradeCenter->canAffordUpgrade(player, upgrade) == FALSE)
		return FALSE;
	else if (
			upgrade->getUpgradeType() == UPGRADE_TYPE_OBJECT
			&& (getObject()->hasUpgrade(upgrade) == TRUE || getObject()->affectedByUpgrade(upgrade) == FALSE))
		return FALSE;
...
void ProductionUpdate::cancelUpgrade(const UpgradeTemplate *upgrade)
{
	...
	{
		if (production->m_type == PRODUCTION_UPGRADE && production->m_upgradeToResearch == upgrade)
			break;
...
Bool ProductionUpdate::queueCreateUnit(const ThingTemplate *unitType, ProductionID productionID)
{
	...
	for (std::vector<QuantityModifier>::const_iterator it = data->m_quantityModifiers.begin();
			 it != data->m_quantityModifiers.end();
			 ++it)
	{
...
void ProductionUpdate::cancelAllUnitsOfType(const ThingTemplate *unitType)
{
	...
	{
		// are we at the one we want get rid of it
		if (production->getProductionType() == PRODUCTION_UNIT && production->getProductionObject() == unitType)
		{
...
UpdateSleepTime ProductionUpdate::update(void)
{
	...
	production->m_percentComplete =
			INT_TO_REAL(production->m_framesUnderConstruction) / INT_TO_REAL(totalProductionFrames) * 100.0f;
	...
	if (production->m_percentComplete >= 100.0f)
	{
		...
		if (production->m_type == PRODUCTION_UNIT)
		{
			...
			if (exitInterface)
			{
				...
				//
				Int numberToTry = production->getProductionQuantityRemaining(); // this will change midloop, so it can't be the For
																																				// clause
				for (int i = 0; i < numberToTry; i++)
				{
					...
					if (exitDoor != DOOR_NONE_AVAILABLE)
					{
						...
						if (d->m_numDoorAnimations == 0 || door == NULL || door->m_doorWaitOpenFrame != 0)
						{
							...
							{
								// Call the voice created for the 1st object -- because it's possible to create multiple objects like
								// redguards!
			...
			else
			{
				...
				DEBUG_ASSERTCRASH(
						0,
						("Cannot create '%s', there is no ExitUpdate interface defined for producer object '%s'",
...
{
	if (production->m_type == PRODUCTION_UNIT && production->m_exitDoor != DOOR_NONE_AVAILABLE)
	{
		ExitInterface *exitInterface = getObject()->getObjectExitInterface();
		if (exitInterface)
		{
...
Bool ProductionUpdate::isUpgradeInQueue(const UpgradeTemplate *upgrade) const
{
	...
	for (production = firstProduction(); production; production = nextProduction(production))
		if (production->getProductionType() == PRODUCTION_UPGRADE && production->getProductionUpgrade() == upgrade)
			return TRUE;
	...
	for (i = 0; i < productionLimit; i++)
	{
		...
		if (m_productionQueue)
		{
			...
			else
			{
				...
				DEBUG_CRASH(
						("ProductionUpdate::cancelAndRefundAllProduction - Unknown production type '%d'",
