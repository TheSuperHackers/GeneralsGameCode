...
Bool SpecialAbilityUpdate::isPowerCurrentlyInUse(const CommandButton *command) const
{
	if (command)
	{
				...
				&& command->getSpecialPowerTemplate()->getSpecialPowerType() == SPECIAL_REMOTE_CHARGES)
		{
			...
			{
				// This is the detonate charge button. Treat it backwards saying it's in use when we don't have any special objects
				// (charges). That way, the button will be grayed out.
...
void SpecialAbilityUpdate::onExit(Bool cleanup)
{
	...
	//  This is the althernate way to one-at-a-time BlackLotus' specials; we'll keep it commented her until Dustin decides, or
	//  until 12/10/02 setBusy( FALSE );// My owner is no longer using me
	...
	switch (spTemplate->getSpecialPowerType())
	{
		...
		case SPECIAL_BLACKLOTUS_CAPTURE_BUILDING:
			...
			if (spmInterface && spTemplate->getSpecialPowerType() == SPECIAL_INFANTRY_CAPTURE_BUILDING)
			{
				...
				// resets in triggerAbilityEffect()
			...
			if (m_doDisableFXParticles)
			{
				...
				if (tmp)
				{
					...
					if (sys)
					{
						...
						sys->setSystemLifetime(data->m_effectDuration * durationInterleaveFactor); // lifetime of the system, not the
																																											 // particles
					}
				}
			}
		...
		if (ai)
		{
			...
			if (obj)
			{
				...
				if (contPlayer)
				{
					...
					Object *mine = ThePartitionManager
														 ->getClosestObject(&pos, data->m_fleeRangeAfterCompletion, FROM_CENTER_2D, filters); // could
																																																									// be
																																																									// null.
																																																									// this
																																																									// is
