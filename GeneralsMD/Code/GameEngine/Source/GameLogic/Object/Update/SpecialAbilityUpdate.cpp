    ...
    SpecialPowerUpdateModule(thing, moduleData)
{
    ...
    //  This is the althernate way to one-at-a-time BlackLotus' specials; we'll keep it commented her until Dustin decides,
    //  or until 12/10/02 setBusy( FALSE );
}
...
UpdateSleepTime SpecialAbilityUpdate::update(void)
{
                ...
                switch (data->m_specialPowerTemplate->getSpecialPowerType())
                {
                    ...
                    case SPECIAL_BOOBY_TRAP:
                    {
                        if (target->testStatus(OBJECT_STATUS_STEALTHED)
                            && (target->testStatus(OBJECT_STATUS_DETECTED) == FALSE))
                        {
    ...
    if (!isPreparationComplete())
    {
        ...
        if (isPersistentAbility()
            && getDoesPersistenceRequireRecharge()) // unless I intend to persist in this ability's effect, whereupon I must
                                                    // verify that power is recharged
...
Bool SpecialAbilityUpdate::isPowerCurrentlyInUse(const CommandButton *command) const
{
    if (command)
    {
        if (command->getSpecialPowerTemplate()
            && command->getSpecialPowerTemplate()->getSpecialPowerType() == SPECIAL_REMOTE_CHARGES)
        {
            if (!BitIsSet(command->getOptions(), CONTEXTMODE_COMMAND))
            {
                // This is the detonate charge button. Treat it backwards saying it's in use when we don't have any special
                // objects (charges). That way, the button will be grayed out.
    ...
    if (m_packingState != STATE_NONE)
    {
        ...
        if ((m_packingState == STATE_PACKING || m_packingState == STATE_PACKED) && command
            && command->getSpecialPowerTemplate()->getReloadTime() == 0)
            return false;
...
void SpecialAbilityUpdate::onExit(Bool cleanup)
{
    ...
    //  This is the althernate way to one-at-a-time BlackLotus' specials; we'll keep it commented her until Dustin decides,
    //  or until 12/10/02 setBusy( FALSE );// My owner is no longer using me
    ...
    if (m_animFrames > 0)
    {
        ...
        if (m_animFrames == 0)
        {
            ...
            getObject()->clearModelConditionFlags(
                MAKE_MODELCONDITION_MASK2(MODELCONDITION_UNPACKING, MODELCONDITION_PACKING));
...
void SpecialAbilityUpdate::startPacking(Bool success)
{
    ...
    Real variation =
        GameLogicRandomValueReal(1.0f - data->m_packUnpackVariationFactor, 1.0f + data->m_packUnpackVariationFactor);
...
Bool SpecialAbilityUpdate::isWithinStartAbilityRange() const
{
    ...
    if (fDistSquared <= fStartRangeSquared)
    {
        if (range == 0.0f && m_targetID != INVALID_ID)
        {
            ...
            ObjectIterator *iter =
                ThePartitionManager->iteratePotentialCollisions(self->getPosition(), self->getGeometryInfo(), 0.0f);
        ...
        if (data->m_approachRequiresLOS)
        {
            ...
            ObjectIterator *iter =
                ThePartitionManager
                    ->iterateObjectsInRange(self, range, FROM_BOUNDINGSPHERE_2D, filters, ITER_SORTED_NEAR_TO_FAR);
            MemoryPoolObjectHolder hold(iter);
    ...
    switch (spTemplate->getSpecialPowerType())
    {
        ...
        case SPECIAL_BLACKLOTUS_STEAL_CASH_HACK:
        {
            ...
            if (target)
            {
                ...
                // Warn the victim so he might have a chance to react!
                if (spTemplate->getSpecialPowerType() == SPECIAL_BLACKLOTUS_CAPTURE_BUILDING && target
...
Bool SpecialAbilityUpdate::continuePreparation()
{
    ...
    switch (spTemplate->getSpecialPowerType())
    {
        ...
        case SPECIAL_BLACKLOTUS_CAPTURE_BUILDING:
            ...
            {
                // these keep resetting the recharge timer while in use. (srj) // only for infantry capture, not black lotus
                // which resets in triggerAbilityEffect()
...
void SpecialAbilityUpdate::triggerAbilityEffect()
{
    ...
    Int skillPoints =
        data->m_skillPointsForTriggering != -1 ? data->m_skillPointsForTriggering : data->m_awardXPForTriggering;
    if (skillPoints > 0)
    {
                ...
                if (tmp)
                {
                    ...
                    if (sys)
                    {
                        ...
                        sys->setSystemLifetime(
                            data->m_effectDuration * durationInterleaveFactor); // lifetime of the system, not the particles
                    }
                }
            }
            break;
            ...
            else
            {
                ...
                if (charge)
                {
                    ...
                    if (!update)
                    {
                        DEBUG_ASSERTCRASH(
                            0,
                            ("Unit '%s' attempted to place remote charge but the charge '%s' requires a StickyBombUpdate "
                             "module.",
                             object->getTemplate()->getName().str(),
                             charge->getTemplate()->getName().str()));
                        killSpecialObjects();
            ...
            if (obj)
            {
                ...
                if (contPlayer)
                {
                    ...
                    Object *mine = ThePartitionManager->getClosestObject(
                        &pos,
                        data->m_fleeRangeAfterCompletion,
                        FROM_CENTER_2D,
