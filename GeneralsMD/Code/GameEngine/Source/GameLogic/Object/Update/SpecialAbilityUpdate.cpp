...
UpdateSleepTime SpecialAbilityUpdate::update(void)
{
  ...
  if (!isPreparationComplete())
  {
    ...
    if (isPersistentAbility() && getDoesPersistenceRequireRecharge()) // unless I intend to persist in this ability's effect,
                                                                      // whereupon I must verify that power is recharged
...
Bool SpecialAbilityUpdate::isWithinStartAbilityRange() const
{
  ...
  if (fDistSquared <= fStartRangeSquared)
  {
    ...
    if (data->m_approachRequiresLOS)
    {
      ...
      ObjectIterator *iter =
          ThePartitionManager->iterateObjectsInRange(self, range, FROM_BOUNDINGSPHERE_2D, filters, ITER_SORTED_NEAR_TO_FAR);
      MemoryPoolObjectHolder hold(iter);
