...
/*static*/ void PhysicsBehaviorModuleData::buildFieldParse(MultiIniFieldParse &p)
{
    ...
    static const FieldParse dataFieldParse[] = {
        ...
        {"VehicleCrashesIntoBuildingWeaponTemplate",
         ...
         NULL,
         offsetof(PhysicsBehaviorModuleData, m_vehicleCrashesIntoBuildingWeaponTemplate)},
        {"VehicleCrashesIntoNonBuildingWeaponTemplate",
         INI::parseWeaponTemplate,
         NULL,
         offsetof(PhysicsBehaviorModuleData, m_vehicleCrashesIntoNonBuildingWeaponTemplate)},
...
// TheSuperHackers @info helmutbuhler 06/05/2025 This debug mutates the code to become CRC incompatible
    ...
    // DEBUG_ASSERTCRASH(!(_isnan(m_accel.x) || _isnan(m_accel.y) || _isnan(m_accel.z)), ("PhysicsBehavior::applyForce accel
    // NAN!")); DEBUG_ASSERTCRASH(!(_isnan(m_vel.x) || _isnan(m_vel.y) || _isnan(m_vel.z)), ("PhysicsBehavior::applyForce vel
    // NAN!")); DEBUG_ASSERTCRASH(fabs(force->z) < 3, ("unlikely z-force"));
#ifdef SLEEPY_PHYSICS
...
void PhysicsBehavior::applyFrictionalForces()
{
    ...
    Bool deckTaxiing = getObject()->testStatus(OBJECT_STATUS_DECK_HEIGHT_OFFSET) && getObject()->getAI()
...
UpdateSleepTime PhysicsBehavior::update()
{
    ...
    if (!obj->isDisabledByType(DISABLED_HELD))
    {
        ...
        if (_isnan(mtx.Get_X_Translation()) || _isnan(mtx.Get_Y_Translation()) || _isnan(mtx.Get_Z_Translation()))
        {
        ...
        if (getIsStunned())
        {
            if ((fabs(m_vel.x) < STUN_RELIEF_EPSILON && fabs(m_vel.y) < STUN_RELIEF_EPSILON
        ...
        if (mtx.Get_Z_Translation() <= groundZ)
        {
            ...
            // When a stunned object hits the ground the first time, chage it's model state from stunned flailing to just
            // stunned.
    ...
    if (getFlag(WAS_AIRBORNE_LAST_FRAME) && !airborneAtEnd && !getFlag(IMMUNE_TO_FALLING_DAMAGE))
    {
        ...
        if (netSpeed > 0.0f && m_pui == NULL)
        {
                ...
                && (fabs(m_vel.y) <= TINY_DELTA || fabs(activeVelZ / m_vel.y) >= MIN_ANGLE_TAN))
            {
                ...
                // %s!",damageAmt,damageInfo.out.m_actualDamageDealt,
...
{
#ifdef SLEEPY_PHYSICS
    if (isZero3D(m_vel) && isZero3D(m_accel) && !getFlag(HAS_PITCHROLLYAW) && !isMotive()
        && (getObject()->getLayer() == LAYER_GROUND && !getObject()->isAboveTerrain()) && getCurrentOverlap() == INVALID_ID
        && getPreviousOverlap() == INVALID_ID && getFlag(UPDATE_EVER_RUN))
    {
...
void PhysicsBehavior::onCollide(Object *other, const Coord3D *loc, const Coord3D *normal)
{
    ...
    if (ai != NULL && ai->getIgnoredObstacleID() == other->getID())
    {
        ...
        if (obj->isKindOf(KINDOF_INFANTRY) && other->isDisabledByType(DISABLED_UNMANNED))
        {
            ...
            // This could be done via a special/new module, but doing it here doesn't require a new module update to every
            // infantry.
    ...
    if (getAllowCollideForce())
    {
        ...
        if (otherImmobile && !obj->isDestroyed())
        {
            ...
            if (delta.z < 0.0f && obj->getPosition()->z >= TheGlobalData->m_defaultStructureRubbleHeight)
            {
                ...
                else
                {
                    ...
                    {
                        TheWeaponStore->createAndFireTempWeapon(
                            getPhysicsBehaviorModuleData()->m_vehicleCrashesIntoNonBuildingWeaponTemplate,
                            obj,
                            obj->getPosition());
                    }
                }
 ...
 *  * The crusherMe is !CRUSHER.																			Result: return
 * false
...
Bool PhysicsBehavior::checkForOverlapCollision(Object *other)
{
    ...
    if (selfCrushingOther && selfBeingCrushed)
    {
        ...
        DEBUG_CRASH(
            ("%s (Crusher:%d, Crushable:%d) is attempting to crush %s (Crusher:%d, Crushable:%d) but it is reciprocating -- "
             ...
             crusherMe->getCrusherLevel(),
             crusherMe->getCrushableLevel(),
             crusheeOther->getTemplate()->getName().str(),
             crusheeOther->getCrusherLevel(),
             crusheeOther->getCrushableLevel()));
        return false;
    }
    ...
    if (!crusherPhysics->wasPreviouslyOverlapped(crusheeOther))
    {
        ...
        damageInfo.in.m_amount =
            0.0f; // yes, that's right -- we don't want to do damage, just to trigger the minor DamageFX, if any
