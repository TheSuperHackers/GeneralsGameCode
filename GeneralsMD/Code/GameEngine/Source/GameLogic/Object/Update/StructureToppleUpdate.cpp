...
void StructureToppleUpdate::beginStructureTopple(const DamageInfo *damageInfo)
{
	...
	if (d)
	{
		...
		m_delayBurstLocation.x = building->getPosition()->x + explosionRadius * WWMath::CosTrig(toppleAngle);
		m_delayBurstLocation.y = building->getPosition()->y + explosionRadius * WWMath::SinTrig(toppleAngle);
...
UpdateSleepTime StructureToppleUpdate::update()
{
	...
	if (m_toppleState == TOPPLESTATE_TOPPLING) {
		UnsignedInt now = TheGameLogic->getFrame();
		Real toppleAcceleration = TOPPLE_ACCELERATION_FACTOR * (WWMath::SinTrig(m_accumulatedAngle) * (1.0 - m_structuralIntegrity));
...
//	static const Real THETA_CEILING = PI/8; // This weapon won't do any damage until theta is less than this value.
	...
	Real minorComponent = building->getGeometryInfo().getMinorRadius() * WWMath::CosTrig(angle);
	Real majorComponent = building->getGeometryInfo().getMajorRadius() * WWMath::SinTrig(angle);
	...
	Real maxDistance = m_buildingHeight * (1.0 - WWMath::SinTrig(theta));
...
//	Coord3D target;
	...
	for (; j < maxDistance; j += WEAPON_SPACING_PERPENDICULAR) {
		jcos = j * WWMath::CosTrig(toppleAngle);
		jsin = j * WWMath::SinTrig(toppleAngle);
		doDamageLine(building, wt, jcos, jsin, facingWidth, toppleAngle);
	}


	jcos = maxDistance * WWMath::CosTrig(toppleAngle);
	jsin = maxDistance * WWMath::SinTrig(toppleAngle);
	doDamageLine(building, wt, jcos, jsin, facingWidth, toppleAngle);
...
void StructureToppleUpdate::doDamageLine(Object *building, const WeaponTemplate* wt, Real jcos, Real jsin, Real facingWidth, Real toppleAngle)
{
	...
	{
		target.x = building->getPosition()->x + jcos + (i * WWMath::SinTrig(toppleAngle));
		target.y = building->getPosition()->y + jsin + (i * WWMath::CosTrig(toppleAngle));
	...
	target.x = building->getPosition()->x + jcos + (facingWidth * WWMath::SinTrig(toppleAngle));
	target.y = building->getPosition()->y + jsin + (facingWidth * WWMath::CosTrig(toppleAngle));
	target.z = TheTerrainLogic->getGroundHeight(target.x, target.y);
