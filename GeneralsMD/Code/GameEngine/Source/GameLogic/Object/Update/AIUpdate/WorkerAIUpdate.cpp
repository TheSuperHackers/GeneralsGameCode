...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
WorkerAIUpdate::WorkerAIUpdate(Thing *thing, const ModuleData *moduleData) : AIUpdateInterface(thing, moduleData)
...
//-------------------------------------------------------------------------------------------------
Object *WorkerAIUpdate::construct(
...
Bool WorkerAIUpdate::gainOneBox(Int remainingStock)
{
	...
	if (remainingStock == 0)
	{
		Object *bestWarehouse =
				getObject()->getControllingPlayer()->getResourceGatheringManager()->findBestSupplyWarehouse(getObject());
...
WorkerStateMachine::WorkerStateMachine(Object *owner) : StateMachine(owner, "WorkerStateMachine")
{
	...
	defineState(
			AS_SUPPLY_TRUCK,
			newInstance(ActAsSupplyTruckState)(this),
			INVALID_STATE_ID,
			INVALID_STATE_ID,
			asTruckConditions);
}
...
Bool WorkerStateMachine::supplyTruckSubMachineReadyToLeave(State *thisState, void *userData)
{
	...
	return !supplyTruckSubMachineWantsToEnter(thisState, NULL) && update->isSupplyTruckBrainActiveAndBusy();
}
