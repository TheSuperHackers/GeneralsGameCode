...
void RailroadBehavior::onCollide(Object *other, const Coord3D *loc, const Coord3D *normal)
{
	...
	if (other->isKindOf(KINDOF_STRUCTURE)) // now be careful here, we ignore buildings, except for hostile, nasty ones that can
																				 // blow us up
	{
	...
	const RailroadBehaviorModuleData *modData = getRailroadBehaviorModuleData();
	if (m_conductorState == WAIT_AT_STATION && (m_pullInfo.speed < modData->m_runningGarrisonSpeedMax)) // they can grab on
																																																			// safely
	{
...
UpdateSleepTime RailroadBehavior::update(void)
{
	...
	if (m_wantsToBeLeadCarraige > FRAMES_UNPULLED_LONG_ENOUGH_TO_UNHITCH) // if this flag survived until now, I have lost my
																																				// puller
	{
