...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
RailroadBehavior::RailroadBehavior(Thing *thing, const ModuleData *moduleData) : PhysicsBehavior(thing, moduleData)
{
...
void RailroadBehavior::onCollide(Object *other, const Coord3D *loc, const Coord3D *normal)
{
    ...
    if (other->isKindOf(KINDOF_STRUCTURE)) // now be careful here, we ignore buildings, except for hostile, nasty ones that
                                           // can blow us up
    {
        // If it is a civilian building like a tunnel or a train station, let it be
        // but if it is a faction building, kill it!
        if (other->isKindOf(KINDOF_FS_POWER) || other->isKindOf(KINDOF_FS_FACTORY) || other->isKindOf(KINDOF_FS_BASE_DEFENSE)
            || other->isKindOf(KINDOF_FS_TECHNOLOGY) || other->isKindOf(KINDOF_REBUILD_HOLE))
        {
    ...
    if (m_conductorState == WAIT_AT_STATION
        && (m_pullInfo.speed < modData->m_runningGarrisonSpeedMax)) // they can grab on safely
    {
    ...
    if (m_conductorState == WAIT_AT_STATION
        || (m_conductorState == COAST && m_pullInfo.speed < modData->m_runningGarrisonSpeedMax) || !m_isLocomotive)
    {
        //  AIUpdateInterface *ai = other->getAI();
        //	  if ( ai )
...
UpdateSleepTime RailroadBehavior::update(void)
{
    ...
    if (m_wantsToBeLeadCarraige
        > FRAMES_UNPULLED_LONG_ENOUGH_TO_UNHITCH) // if this flag survived until now, I have lost my puller
    {
    ...
    if (m_isLeadCarraige)
    {
        ...
        FindPosByPathDistance(&conductorPullInfo.towHitchPosition, conductorPullInfo.trackDistance, m_track->m_length);
            ...
            else
            {
                DEBUG_ASSERTCRASH(
                    ...
                    ("%s could not hitch a %s without a RailroadBehavior... \nwhat kind of nutty conductor are you? \nThe "
                     "next carriage would have been a %s.",
...
void RailroadBehavior::hitchNewCarriagebyProximity(ObjectID locoID, TrainTrack *track)
{
    ...
    if (closeCarriage)
    {
        ...
        else
        {
            DEBUG_ASSERTCRASH(
                rb,
                ("%s is attempting to hitch carriage, %s without a RailroadBehavior... \nwhat kind of nutty conductor are "
                 "you? ",
                 self->getTemplate()->getName().str(),
...
void RailroadBehavior::updatePositionTrackDistance(PullInfo *pullerInfo, PullInfo *myInfo)
{
    ...
    FindPosByPathDistance(
        &myInfo->towHitchPosition,
        myInfo->trackDistance, // the look ahead
        m_track->m_length);

    Coord3D carPosition;
    FindPosByPathDistance(&carPosition, myInfo->trackDistance, m_track->m_length, TRUE);
                    ...
                    if (m_isLocomotive) // since only locomotives care about such things
                    {
                        ...
                        if (edge)
                        {
                            ...
                            else if (thisPoint->m_isDisembark)
                            {
                                ...
                                thisPoint->m_isPingPong && conductorPullInfo.m_mostRecentSpecialPointHandle != handleFound)
                            {
