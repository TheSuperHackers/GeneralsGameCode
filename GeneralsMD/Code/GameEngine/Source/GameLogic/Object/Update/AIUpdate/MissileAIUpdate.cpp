...
void MissileAIUpdate::doKillState()
{
	...
	if (isIdle()) {
		...
		if (getGoalObject()!=nullptr) {
			...
			//DEBUG_LOG(("Distance to target %f, closeEnough %f", WWMath::Sqrt(distanceToTargetSq), closeEnough));
...
UpdateSleepTime MissileAIUpdate::update()
{
	...
	{
		Real distThisTurn = WWMath::Sqrt(sqr(newPos.x-m_prevPos.x) + sqr(newPos.y-m_prevPos.y) + sqr(newPos.z-m_prevPos.z));
