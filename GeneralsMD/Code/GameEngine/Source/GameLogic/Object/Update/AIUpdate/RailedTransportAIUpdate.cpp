...
void RailedTransportAIUpdate::pickAndMoveToInitialLocation(void)
{
	...
	DEBUG_ASSERTCRASH(
			...
			("No suitable starting waypoint path could be found for '%s'", us->getTemplate()->getName().str()));
...
UpdateSleepTime RailedTransportAIUpdate::update(void)
{
	...
	{
		// sanity
		DEBUG_ASSERTCRASH(m_currentPath != INVALID_PATH, ("RailedTransportAIUpdate: Invalid current path '%s'", m_currentPath));
...
void RailedTransportAIUpdate::aiDoCommand(const AICommandParms *parms)
{
	...
	if (parms->m_cmdSource == CMD_FROM_PLAYER && parms->m_cmd != AICMD_EXECUTE_RAILED_TRANSPORT
