...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
POWTruckAIUpdate::POWTruckAIUpdate(Thing *thing, const ModuleData *moduleData) : AIUpdateInterface(thing, moduleData)
{
    m_aiMode = AUTOMATIC;
...
void POWTruckAIUpdate::setTask(POWTruckTask task, Object *taskObject)
{
    ...
    if ((task == POW_TRUCK_TASK_COLLECTING_TARGET || task == POW_TRUCK_TASK_RETURNING_PRISONERS) && taskObject == NULL)
    {
    ...
    if (task == POW_TRUCK_TASK_COLLECTING_TARGET)
    {
        ...
        DEBUG_ASSERTCRASH(
            taskObject->getAIUpdateInterface(),
            ("POWTruckAIUpdate::setTask - '%s' has no ai module", taskObject->getTemplate()->getName().str()));
...
void POWTruckAIUpdate::updateWaiting(void)
{
    ...
    DEBUG_ASSERTCRASH(ai, ("POWTruckAIUpdate::updateWaiting - '%s' has no ai", us->getTemplate()->getName().str()));
...
void POWTruckAIUpdate::updateFindTarget(void)
{
    ...
    DEBUG_ASSERTCRASH(
        m_aiMode != MANUAL,
        ("POWTruckAIUpdate::updateFindTarget - We shouldn't be here with a manual ai mode"));
    if (m_aiMode == MANUAL)
        return;
    ...
    Object *us = getObject();
    AIUpdateInterface *ai = us->getAIUpdateInterface();
    DEBUG_ASSERTCRASH(ai, ("POWTruckAIUpdate::updateFindTarget - '%s' has no ai", us->getTemplate()->getName().str()));
...
static void putContainedInPrison(Object *obj, void *userData)
{
    ...
    // sanity
    DEBUG_ASSERTCRASH(
        returnData != NULL && returnData->source != NULL && returnData->dest != NULL,
...
void POWTruckAIUpdate::updateReturnPrisoners(void)
{
    Object *us = getObject();
    AIUpdateInterface *ai = us->getAIUpdateInterface();
    DEBUG_ASSERTCRASH(ai, ("POWTruckAIUpdate::updateReturnPrisoners - '%s' has no AI", us->getTemplate()->getName().str()));
...
void POWTruckAIUpdate::doReturnPrisoners(void)
{
    ...
    Object *us = getObject();
    AIUpdateInterface *ai = us->getAIUpdateInterface();
    DEBUG_ASSERTCRASH(ai, ("POWTruckAIUpdate::doReturnPrisoners - '%s' has no AI", us->getTemplate()->getName().str()));
...
Object *POWTruckAIUpdate::findBestTarget(void)
{
    ...
    DEBUG_ASSERTCRASH(ai, ("POWTruckAIUpdate::findBestTarget- '%s' has no AI", us->getTemplate()->getName().str()));
    ...
    for (other = TheGameLogic->getFirstObject(); other; other = other->getNextObject())
    {
        ...
        if (closestTarget == NULL || distSq < closestTargetDistSq)
        {
            ...
            if (TheAI->pathfinder()->quickDoesPathExist(ai->getLocomotorSet(), us->getPosition(), other->getPosition())
                == TRUE)
            {
...
static void putPrisonersInPrison(Object *obj, void *userData)
{
    ...
    prisonUnloadData->bounty +=
        TheGlobalData->m_prisonBountyMultiplier * obj->getTemplate()->calcCostToBuild(prisonerOwningPlayer);
...
void POWTruckAIUpdate::unloadPrisonersToPrison(Object *prison)
{
    ...
    DEBUG_ASSERTCRASH(
        prison->getContain(),
        ("POWTruckAIUpdate::unloadPrisonersToPrison - '%s' has no contain", prison->getTemplate()->getName().str()));
    DEBUG_ASSERTCRASH(
        prison->getContain()->asOpenContain(),
        ("POWTruckAIUpdate::unloadPrisonersToPrison - '%s' has no OPEN contain", prison->getTemplate()->getName().str()));
    DEBUG_ASSERTCRASH(
        truckContain,
        ("POWTruckAIUpdate::unloadPrisonersToPrison - '%s' has no contain", us->getTemplate()->getName().str()));
    DEBUG_ASSERTCRASH(
        truckContain->asOpenContain(),
        ("POWTruckAIUpdate::unloadPrisonersToPrison - '%s' has no OPEN contain", us->getTemplate()->getName().str()));
    ...
    if (prison->isKindOf(KINDOF_COLLECTS_PRISON_BOUNTY) && prisonUnloadData.bounty > 0)
    {
        Player *player = us->getControllingPlayer();
