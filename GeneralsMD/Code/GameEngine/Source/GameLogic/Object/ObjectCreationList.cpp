            ...
            if (ai)
            {
                if (m_startAtMaxSpeed && createOwner)
                {
                    ...
                    if (physics)
                    {
                        ...
                        Real maxSpeed =
                            ai->getCurLocomotor()->getMaxSpeedForCondition(transport->getBodyModule()->getDamageState());
        ...
        UnsignedInt lifetimeFrames) const
    {
        ...
        if (m_containInsideSourceObject)
        {
            ...
            else
            {
                DEBUG_ASSERTCRASH(
                    FALSE,
                    ("A OCL with ContainInsideSourceObject failed the contain and is killing the new object."));
        ...
        if (m_diesOnBadLand && obj)
        {
                ...
                && obj->getLayer() == LAYER_GROUND)
            {
                ...
                damageInfo.in.m_damageType =
                    DAMAGE_WATER; // use this instead of UNRESISTABLE so we don't get a dusty damage effect
