...
//-------------------------------------------------------------------------------------------------
// void ObjectCreationNugget::create( const Object* primaryObj, const Coord3D *primary, const Coord3D *secondary, Real angle,
// UnsignedInt lifetimeFrames ) const
//{
//	create( primaryObj, primary ? primary->getPosition() : NULL, secondary ? secondary->getPosition() : NULL, angle,
// lifetimeFrames );
//}

//-------------------------------------------------------------------------------------------------
// This one is called only when we have a nugget that doesn't care about createOwner.
Object *ObjectCreationNugget::create(
...
class AttackNugget : public ObjectCreationNugget
{
  ...
  public:
  AttackNugget() : m_numberOfShots(1), m_weaponSlot(PRIMARY_WEAPON), m_deliveryDecalRadius(0) {}
      ...
      UnsignedInt lifetimeFrames = 0) const
  {
    ...
    for (UnsignedInt formationIndex = 0; formationIndex < m_formationSize; formationIndex++)
    {
      ...
      if (createOwner)
      {
        ...
        if (m_delayDeliveryFramesMax > 0)
        {
          transport->setDisabledUntil(
              DISABLED_DEFAULT,
              TheGameLogic->getFrame() + GameLogicRandomValue(0, m_delayDeliveryFramesMax));
        }
      }
      else
      {
      ...
      if (ai)
      {
        ...
        {
          startPos.z =
              TheTerrainLogic->getGroundHeight(startPos.x, startPos.y) + ai->getCurLocomotor()->getPreferredHeight();
          transport->setPosition(&startPos);
        }

        const ThingTemplate *putInContainerTmpl =
            m_putInContainerName.isEmpty() ? NULL : TheThingFactory->findTemplate(m_putInContainerName);
        for (std::vector<Payload>::const_iterator it = m_payload.begin(); it != m_payload.end(); ++it)
        {
          ...
          {
            DEBUG_CRASH(
                ("DeliverPayloadNugget::create() -- %s couldn't create %s (template not found).",
                 transport->getTemplate()->getName().str(),
                 it->m_payloadName.str()));
            return NULL;
          }
          for (int i = 0; i < it->m_payloadCount; ++i)
          {
            ...
            {
              DEBUG_CRASH(
                  ("DeliverPayload: transport %s is full, or not valid for the payload %s!",
                   m_transportName.str(),
                   it->m_payloadName.str()));
            }
          }
...
class ApplyRandomForceNugget : public ObjectCreationNugget
{
  ...
  public:
  ApplyRandomForceNugget() : m_spinRate(0.0f), m_minMag(0.0f), m_maxMag(0.0f), m_minPitch(0.0f), m_maxPitch(0.0f) {}
...
//-------------------------------------------------------------------------------------------------
enum DebrisDisposition CPP_11( : Int){ LIKE_EXISTING = 0x00000001,  ON_GROUND_ALIGNED = 0x00000002,
                                       SEND_IT_FLYING = 0x00000004, SEND_IT_UP = 0x00000008,
                                       SEND_IT_OUT = 0x00000010,    RANDOM_FORCE = 0x00000020,
                                       FLOATING = 0x00000040,       INHERIT_VELOCITY = 0x00000080,
                                       WHIRLING = 0x00000100 };

static const char *DebrisDispositionNames[] = {
  "LIKE_EXISTING", "ON_GROUND_ALIGNED", "SEND_IT_FLYING",   "SEND_IT_UP", "SEND_IT_OUT",
...
class GenericObjectCreationNugget : public ObjectCreationNugget
{
  ...
  virtual Object *create(const Object *primary, const Object *secondary, UnsignedInt lifetimeFrames = 0) const
  {
    if (primary)
    {
      ...
      return reallyCreate(
          primary->getPosition(),
          primary->getTransformMatrix(),
          primary->getOrientation(),
          primary,
          lifetimeFrames);
    }
    else
      ...
      UnsignedInt lifetimeFrames) const
  {
    ...
    if (lup)
    {
      ...
      {
        // They will both be zero if no lifetime was specified in the OCL.  It could be in the Object so don't mess
        // with it. So the OCL listing will override the Object listing for lifetime, but ONLY if there is one.
      ...
      for (DrawModule **dm = obj->getDrawable()->getDrawModules(); *dm; ++dm)
      {
        ...
        if (di)
        {
          ...
          if (m_animSets.size() > 0)
          {
            ...
            di->setAnimNames(
                m_animSets[which].m_animInitial,
                m_animSets[which].m_animFlying,
                m_animSets[which].m_animFinal,
                m_fxFinal);
          }
        }
      }
    ...
    {
      DEBUG_LOG(
          ("Object %s inherits veterancy level %d from %s",
    ...
    if (m_diesOnBadLand && obj)
    {
      ...
      if (TheTerrainLogic->isUnderwater(riderPos->x, riderPos->y, &waterZ, &terrainZ) && riderPos->z <= waterZ + 10.0f
      ...
      if (obj->isOffMap() || (cellType == PathfindCell::CELL_CLIFF) || (cellType == PathfindCell::CELL_WATER)
      ...
      UnsignedInt lifetimeFrames) const
  {
    ...
    if (m_requiresLivePlayer
        && (!sourceObj || !sourceObj->getControllingPlayer() || !sourceObj->getControllingPlayer()->isPlayerActive()))
...
//-------------------------------------------------------------------------------------------------
Object *ObjectCreationList::createInternal(
    const Object *primaryObj,
    const Coord3D *primary,
    const Coord3D *secondary,
    Bool createOwner,
    UnsignedInt lifetimeFrames) const
{
  DEBUG_ASSERTCRASH(
      primaryObj != NULL,
      ("You should always call OCLs with a non-null primary Obj, even for positional calls, to get team ownership right"));
