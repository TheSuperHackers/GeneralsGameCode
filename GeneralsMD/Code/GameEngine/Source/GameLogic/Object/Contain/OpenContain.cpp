...
/*static*/ void OpenContainModuleData::buildFieldParse(MultiIniFieldParse &p)
{
	...
	static const FieldParse dataFieldParse[] = {
		...
		{ "WeaponBonusPassedToPassengers",
			INI::parseBool,
			NULL,
			offsetof(OpenContainModuleData, m_weaponBonusPassedToPassengers) },
...
OpenContain::~OpenContain()
{
	...
	DEBUG_ASSERTCRASH(
			m_containList.empty(),
			("OpenContain %s: destroying a container that still has items in it!", getObject()->getTemplate()->getName().str()));

	// sanity
	DEBUG_ASSERTCRASH(
			m_xferContainIDList.empty(),
			("OpenContain %s: m_xferContainIDList is not empty but should be", getObject()->getTemplate()->getName().str()));
}
...
void OpenContain::addOrRemoveObjFromWorld(Object *obj, Bool add)
{
	...
	if (obj->isKindOf(KINDOF_STRUCTURE))
		DEBUG_LOG(
				("WARNING: Containing/Removing structures like '%s' is potentially a very expensive and slow operation",
...
void OpenContain::addToContain(Object *rider)
{
	...
	{
		DEBUG_LOG(
				("'%s' is trying to contain '%s', but '%s' is already contained by '%s'",
				 getObject()->getTemplate()->getName().str(),
...
void OpenContain::removeFromContain(Object *rider, Bool exposeStealthUnits)
{
	...
	{
		DEBUG_LOG(
				("'%s' is trying to un-contain '%s', but '%s' is really contained by '%s'",
				 getObject()->getTemplate()->getName().str(),
				 rider->getTemplate()->getName().str(),
				 rider->getTemplate()->getName().str(),
...
Bool OpenContain::isValidContainerFor(const Object *obj, Bool checkCapacity) const
{
	...
	if (obj->isAnyKindOf(modData->m_allowInsideKindOf) == FALSE || obj->isAnyKindOf(modData->m_forbidInsideKindOf) == TRUE)
	{
		return false;
	}
	...
	switch (r)
	{
		...
		default:
			DEBUG_CRASH(
					("isValidContainerFor: Undefined relationship (%d) between '%s' and '%s'",
					 r,
					 getObject()->getTemplate()->getName().str(),
					 obj->getTemplate()->getName().str()));
			return FALSE;
