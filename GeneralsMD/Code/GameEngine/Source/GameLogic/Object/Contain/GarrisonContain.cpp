...
void GarrisonContain::redeployOccupants(void)
{
	...
	// points without clearing the list (and thus forgetting where everyone contained was stationed)... just a handy reminder.
	m_stationGarrisonPointsInitialized = FALSE;
...
#define HUGE_FRAME_IN_FUTURE (LOGICFRAMES_PER_SECOND * 1000)
	...
	else
	{
		...
		DEBUG_ASSERTCRASH(
				...
				("GarrisonContain::update - Objects with garrison contain can be spec'd as 'mobile' in the INI. Do you really want "
				 "to do this?"));
	}
		...
		if (!foundHisSpot && !pickAStationForMe(contained))
		{
			DEBUG_ASSERTCRASH(
					...
					("GarrisonContain::positionObjectsAtStationGarrisonPoints found something terribly wrong... \nthere is either a "
					 "station point shortage, or some other bug."));
		}
...
void GarrisonContain::exitObjectViaDoor(Object *exitObj, ExitDoorType exitDoor)
{
	...
	if (m_evacDisposition == EVAC_TO_LEFT || m_evacDisposition == EVAC_TO_RIGHT)
	{
		...
		doorPosition.X = GameLogicRandomValueReal(-containerHalfLength / 4, containerHalfLength / 4); // a rectangular pocket to
																																																	// act as the "doorway"
