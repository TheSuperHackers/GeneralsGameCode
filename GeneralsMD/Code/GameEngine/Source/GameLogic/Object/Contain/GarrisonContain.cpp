...
void GarrisonContain::putObjectAtGarrisonPoint(Object *obj, ObjectID targetID, Int conditionIndex, Int pointIndex)
{
    ...
    // sanity
    if (obj == NULL || pointIndex < 0 || pointIndex >= MAX_GARRISON_POINTS || conditionIndex < 0
        || conditionIndex >= MAX_GARRISON_POINT_CONDITIONS)
    {
    ...
    {
        DEBUG_CRASH(("GarrisonContain::putObjectAtGarrisonPoint - Garrison Point '%d' is not empty", pointIndex));
        return;
...
Int GarrisonContain::findConditionIndex(void)
{
    ...
    switch (bodyDamage)
    {
        ...
        default:
            ...
            DEBUG_CRASH(("GarrisonContain::findConditionIndex - Unknown body damage type '%d'", bodyDamage));
            break;
...
Bool GarrisonContain::calcBestGarrisonPosition(Coord3D *sourcePos, const Coord3D *targetPos)
{
    ...
    DEBUG_ASSERTCRASH(
        modData->m_isEnclosingContainer,
        ("calcBestGarrisonPosition... SHOULD NOT GET HERE, since this container is non-enclosing"));
#endif
...
void GarrisonContain::putObjectAtBestGarrisonPoint(Object *obj, Object *target, const Coord3D *targetPos)
{
    ...
    Int placeIndex = findClosestFreeGarrisonPointIndex(conditionIndex, targetPos);
    DEBUG_ASSERTCRASH(
        placeIndex != GARRISON_INDEX_INVALID,
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
GarrisonContain::GarrisonContain(Thing *thing, const ModuleData *moduleData) : OpenContain(thing, moduleData)
{
    Int i, j;
...
void GarrisonContain::removeInvalidObjectsFromGarrisonPoints(void)
{
    ...
    DEBUG_ASSERTCRASH(
        modData->m_isEnclosingContainer,
        ("removeinvalidobjFromGarrisonPoint... SHOULD NOT GET HERE, since this container is non-enclosing"));
#endif
    Object *obj;
...
void GarrisonContain::addValidObjectsToGarrisonPoints(void)
{
    ...
    DEBUG_ASSERTCRASH(
        modData->m_isEnclosingContainer,
        ("addvalidobjtoGarrisonPoint... SHOULD NOT GET HERE, since this container is non-enclosing"));
#endif
...
void GarrisonContain::trackTargets(void)
{
    if (!isEnclosingContainerFor(0))
        return; // since ina non-enclosing container, objects fire from their station points, instead of being juggled around
                // between garrison firepoints
...
void GarrisonContain::redeployOccupants(void)
{
    ...
    // ATTENTION... setting this false allows each redeployOccupants() call to create fresh station points, based on the new
    // transform if anything wierd ever happens, like rotating buildings and such, we will need a way of transforming the
    // points without clearing the list (and thus forgetting where everyone contained was stationed)... just a handy
    // reminder.
    m_stationGarrisonPointsInitialized = FALSE;
...
void GarrisonContain::updateEffects(void)
{
    ...
    DEBUG_ASSERTCRASH(
        modData->m_isEnclosingContainer,
        ("updateeffects... SHOULD NOT GET HERE, since this container is non-enclosing"));
#endif
        ...
        if (obj->getLastShotFiredFrame() == currentFrame - 1)
        {
            ...
            if (garrisonIndex != GARRISON_INDEX_INVALID)
            {
                ...
                if (effect)
                {
                    ...
                    if (passengerWeapon
                        && passengerWeapon->getDamageType() != DAMAGE_POISON) // No muzzle flash with poison weapon
                    {
    ...
    for (Int i = 0; i < MAX_GARRISON_POINTS; ++i)
    {
        if (m_garrisonPointData[i].effect && m_garrisonPointData[i].lastEffectFrame != 0
...
#define HUGE_FRAME_IN_FUTURE (LOGICFRAMES_PER_SECOND * 1000)
    ...
    else
    {
        ...
        DEBUG_ASSERTCRASH(
            ...
            ("GarrisonContain::update - Objects with garrison contain can be spec'd as 'mobile' in the INI. Do you really "
             "want to do this?"));
    }
...
void GarrisonContain::positionObjectsAtStationGarrisonPoints()
{
    ...
    for (ContainedItemsList::const_iterator it = containList.begin(); it != containList.end(); ++it)
    {
        ...
        for (std::vector<StationPointData>::const_iterator pt = m_stationPointList.begin(); pt != m_stationPointList.end();
             ++pt)
        {
        ...
        if (!foundHisSpot && !pickAStationForMe(contained))
        {
            DEBUG_ASSERTCRASH(
                foundHisSpot,
                ("GarrisonContain::positionObjectsAtStationGarrisonPoints found something terribly wrong... \nthere is "
                 "either a station point shortage, or some other bug."));
        }
...
Bool GarrisonContain::pickAStationForMe(const Object *obj)
{
    ...
    DEBUG_ASSERTCRASH(
        foundVacancy,
        ("GarrisonContain::pickAStationForMe is all kinds of bad... \n there was no vacancy found for a newly contained "
         "object."));
...
void GarrisonContain::removeObjectFromStationPoint(const Object *obj)
{
    ...
    DEBUG_ASSERTCRASH(
        foundOccupant,
        ("GarrisonContain::removeObjectFromStationPoint is all kinds of bad... \n the contained object was not found in "
         "station point list."));
}
...
void GarrisonContain::recalcApparentControllingPlayer(void)
{
    ...
    if (draw)
    {
        ...
        if (getContainCount() > 0)
        {
            ...
            if (detected
                || (getApparentControllingPlayer(ThePlayerList->getLocalPlayer()) == getObject()->getControllingPlayer()))
            {
...
void GarrisonContain::loadGarrisonPoints(void)
{
    ...
    DEBUG_ASSERTCRASH(
        modData->m_isEnclosingContainer,
        ("loadGarrisonPoints... SHOULD NOT GET HERE, since this container is non-enclosing"));
    ...
    //
    {
        ...
        count =
            structure->getMultiLogicalBonePosition("FIREPOINT", MAX_GARRISON_POINTS, m_garrisonPoint[conditionIndex], NULL);
...
void GarrisonContain::validateRallyPoint(void)
{
    ...
    if (m_rallyValid == FALSE)
    {
        ...
        options.ignoreObject = getObject();
        options.relationshipObject = getObject();
        m_rallyValid = ThePartitionManager->findPositionAround(getObject()->getPosition(), &options, &m_exitRallyPoint);
    } // end if
...
void GarrisonContain::exitObjectViaDoor(Object *exitObj, ExitDoorType exitDoor)
{
    ...
    if (m_evacDisposition == EVAC_TO_LEFT || m_evacDisposition == EVAC_TO_RIGHT)
    {
        ...
        doorPosition.X = GameLogicRandomValueReal(
            -containerHalfLength / 4,
            containerHalfLength / 4); // a rectangular pocket to act as the "doorway"
        ...
        else
        {
            DEBUG_CRASH(
                ("DeliverPayload: PutInContainer %s is full, or not valid for the payload %s!",
                 object->getName().str(),
                 self->m_initialRoster.templateName.str()));
        }
    }
}
