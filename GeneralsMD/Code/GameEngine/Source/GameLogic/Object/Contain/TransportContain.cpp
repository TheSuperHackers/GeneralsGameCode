...
void TransportContainModuleData::buildFieldParse(MultiIniFieldParse &p)
{
    ...
    static const FieldParse dataFieldParse[] = {
        ...
        {"DestroyRidersWhoAreNotFreeToExit",
         INI::parseBool,
         NULL,
         offsetof(TransportContainModuleData, m_destroyRidersWhoAreNotFreeToExit)},
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
TransportContain::TransportContain(Thing *thing, const ModuleData *moduleData) : OpenContain(thing, moduleData)
{
    m_extraSlotsInUse = 0;
                    ...
                    if (weapon)
                    {
                        if (weapon->getTemplate()->isContactWeapon() == FALSE
                            && weapon->isDamageWeapon()
                                == TRUE) // THIS MAY NEED TO CHECK MORE WEAPON ATTRIBUTES TO WORK BEST
                        {
...
Bool TransportContain::isPassengerAllowedToFire(ObjectID id) const
{
    ...
    if (heWhoContainsMe)
    {
        ...
        DEBUG_ASSERTCRASH(
            hisContain,
            ("TransportContain::isPassengerAllowedToFire()... CONTAINER WITHOUT A CONTAIN! AARRGH!"));
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
Bool TransportContain::isExitBusy() const ///< Contain style exiters are getting the ability to space out exits, so ask this
...
void TransportContain::onCapture(Player *oldOwner, Player *newOwner)
{
    if (oldOwner != newOwner)
    {
        ...
        {
            // If this vehicle was sniped, then instantly eject everyone (otherwise, the next guy to eject will recapture
            // it).
            removeAllContained();
        }
        else
