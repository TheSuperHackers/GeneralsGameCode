...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
MinefieldBehavior::MinefieldBehavior(Thing *thing, const ModuleData *moduleData) : UpdateModule(thing, moduleData)
{
...
UpdateSleepTime MinefieldBehavior::update()
{
	...
	for (Int i = 0; i < MAX_IMMUNITY; ++i)
	{
		...
		if (TheGameLogic->findObjectByID(m_immunes[i].id) == NULL || now > m_immunes[i].collideTime + 2)
		{
	...
	if (m_draining)
	{
		...
		damageInfo.in.m_amount =
				(obj->getBodyModule()->getMaxHealth() * d->m_healthPercentToDrainPerSecond) / LOGICFRAMES_PER_SECOND;
...
void MinefieldBehavior::onDamage(DamageInfo *damageInfo)
{
	...
	for (;;)
	{
		...
		Int virtualMinesExpected = damageInfo->in.m_damageType == DAMAGE_HEALING ? REAL_TO_INT_FLOOR(virtualMinesExpectedF) :
...
void MinefieldBehavior::xfer(Xfer *xfer)
{
	...
	{
		DEBUG_CRASH(
				("MinefieldBehavior::xfer - MAX_IMMUNITY has changed size, you must version this code and then you can remove this "
				 "error message"));
