...
void PropagandaTowerBehavior::doScan(void)
{
  ...
  if (overlord)
  {
    ...
    // The PropTowers that ride on the backs of overlord tanks want to suppress their fx if the overlord is stealthed to
    // the local player The PropTowers that ride on the bellies of Helixes also suppress their fx when hiding stealthed
    // (never happens?) The PropTowers must do nothing, scanning-or-FX-wise when they are inside something... there are
    // two cases
