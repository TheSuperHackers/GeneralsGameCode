...
UpdateSleepTime BattleBusSlowDeathBehavior::update(void)
{
  ...
  else
  {
    ...
    if (m_penaltyDeathFrame != 0)
    {
      ...
      {
        // No salvation, see if the timer is up to be killed.  Penalty prevents effects, Extra 4 prevents this die
        // module from recalling. Can't use Suicided death because a Terrorist actually inflicts Suicide deaths on
        // others.
