...
/*static*/ void BattleBusSlowDeathBehaviorModuleData::buildFieldParse(MultiIniFieldParse &p)
{
	...
	static const FieldParse dataFieldParse[] = {
		...
		{ "OCLStartUndeath",
			...
			NULL,
			offsetof(BattleBusSlowDeathBehaviorModuleData, m_oclStartUndeath) },
		...
		{ "PercentDamageToPassengers",
			...
			NULL,
			offsetof(BattleBusSlowDeathBehaviorModuleData, m_percentDamageToPassengers) },
		{ "EmptyHulkDestructionDelay",
			INI::parseDurationUnsignedInt,
			NULL,
			offsetof(BattleBusSlowDeathBehaviorModuleData, m_emptyHulkDestructionDelay) },
...
UpdateSleepTime BattleBusSlowDeathBehavior::update(void)
{
	...
	else
	{
		...
		if (m_penaltyDeathFrame != 0)
		{
			...
			{
				// No salvation, see if the timer is up to be killed.  Penalty prevents effects, Extra 4 prevents this die module
				// from recalling. Can't use Suicided death because a Terrorist actually inflicts Suicide deaths on others.
