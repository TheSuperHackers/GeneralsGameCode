...
UpdateSleepTime SpawnBehavior::update(void)
{
	...
	if (!m_initialBurstTimesInited)
	{
		...
		Bool runtimeProduced = getObject()->getProducerID() != INVALID_ID; // this was produced by a production module, rather
																																			 // than a script or worldbuilder
	...
	for (objectIDListIterator it = m_spawnIDs.begin(); it != m_spawnIDs.end(); ++it)
	{
		...
		if (obj)
		{
			...
			switch (result)
			{
				...
				default:
					DEBUG_CRASH(
							("SpawnBehavior::getCanAnySlavesAttackSpecificTarget encountered unhandled CanAttackResult of %d. Treating as "
							 "not possible...",
							 result));
					break;
			}
	...
	for (objectIDListIterator it = m_spawnIDs.begin(); it != m_spawnIDs.end(); ++it)
	{
		...
		if (obj)
		{
			...
			switch (result)
			{
				...
				default:
					DEBUG_CRASH(
							("SpawnBehavior::getCanAnySlavesUseWeaponAgainstTarget encountered unhandled CanAttackResult of %d. Treating "
							 "as not possible...",
							 result));
					break;
			}
