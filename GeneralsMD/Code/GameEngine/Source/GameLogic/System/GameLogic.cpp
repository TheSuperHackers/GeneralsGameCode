...
enum
{
    ...
    LOAD_PROGRESS_LOOP_INITIAL_NETWORK_BUILDINGS =
        LOAD_PROGRESS_MAX_ALL_THE_FREAKN_OBJECTS + 1, // Increment the next one by at least MAX_SLOTS
...
static Object *placeObjectAtPosition(
    Int slotNum,
    ...
    const PlayerTemplate *pTemplate)
{
    ...
    if (obj)
    {
        ...
        CRCDEBUG_LOG(
            ("Placed an object for %ls at pos (%g,%g,%g)", pPlayer->getPlayerDisplayName().str(), pos.x, pos.y, pos.z));
        DUMPCOORD3D(&pos);
...
static void placeNetworkBuildingsForPlayer(
    Int slotNum,
    const GameSlot *pSlot,
    Player *pPlayer,
    const PlayerTemplate *pTemplate)
{
    ...
    DEBUG_ASSERTCRASH(
        ...
        ("no starting building type for player %d (playertemplate %ls)", slotNum, pTemplate->getDisplayName().str()));
    ...
    for (i = 0; i < MAX_SLOTS; ++i)
    {
               ...
               && (playerTemplateIdx < 0 || playerTemplateIdx >= ThePlayerTemplateStore->getPlayerTemplateCount()))
        {
            DEBUG_ASSERTCRASH(
                ...
                ("Non-random bad playerTemplate %d in slot %d", playerTemplateIdx, i));
#ifdef MORE_RANDOM
...
void GameLogic::startNewGame(Bool loadingSaveGame)
{
    ...
    if (loadingSaveGame == FALSE)
    {
        ...
        {
            DEBUG_CRASH(
                ("FATAL SAVE/LOAD ERROR! - Setting a pristine map name that refers to a map in the save directory.  The "
                 "pristine map should always refer to the ORIGINAL map in the Maps directory, if the pristine map string is "
                 "corrupt then map.ini files will not load correctly."));
    ...
    if (game)
    {
        ...
        for (int i = 0; i < MAX_SLOTS; ++i)
        {
            ...
            d.setBool(
                ...
                slot->isHuman() && (slot->getName().compare(game->getSlot(game->getLocalSlotNum())->getName().str()) == 0));
    ...
    ThePartitionManager->init();
    ThePartitionManager
        ->refreshShroudForLocalPlayer(); // Can't do this until after init, and doesn't seem right to do in init
    ...
    ThePartitionManager->revealMapForPlayerPermanently(
        ThePlayerList->findPlayerWithNameKey(TheNameKeyGenerator->nameToKey("ReplayObserver"))->getPlayerIndex());
    DEBUG_LOG((
        "Reveal shroud for %ls whose index is %d",
        ThePlayerList->findPlayerWithNameKey(TheNameKeyGenerator->nameToKey("ReplayObserver"))->getPlayerDisplayName().str(),
        ThePlayerList->findPlayerWithNameKey(TheNameKeyGenerator->nameToKey("ReplayObserver"))->getPlayerIndex()));
    ...
    if (game)
    {
        for (int i = 0; i < MAX_SLOTS; ++i)
        {
            ...
            {
                DEBUG_LOG(
                    ("Clearing shroud for observer %s in playerList slot %d", playerName.str(), player->getPlayerIndex()));
    ...
    if (TheGameLODManager->getStaticLODLevel() == STATIC_GAME_LOD_CUSTOM && TheGlobalData->m_useShadowVolumes)
    {
    ...
    else
    {
        for (pMapObj = MapObject::getFirstMapObject(); pMapObj; pMapObj = pMapObj->getNext())
        {
            ...
            DEBUG_ASSERTCRASH(
                pMapObj->getProperties()->getType(TheKey_originalOwner) == Dict::DICT_ASCIISTRING,
                ("unit %s has no original owner specified (obsolete map file)", pMapObj->getName().str()));
    ...
    if (game && !loadingSaveGame)
    {
        for (int i = 0; i < MAX_SLOTS; ++i)
        {
            ...
            else
            {
                ...
                // Trouble was that skirmish games would get no command centers upon start, if this was set true in a
                // GameSpyMenu
                if (isInInternetGame())
                {
    ...
    if (isChallengeCampaign)
    {
        ...
        if (placeholderThePlayer)
        {
            PlayerMaskType thePlayerEnemysMask =
                ThePlayerList->getPlayersWithRelationship(placeholderThePlayer->getPlayerIndex(), ALLOW_ENEMIES);
        ...
        else
        {
            ...
            for (Int i = 0; i < ThePlayerList->getPlayerCount(); i++)
            {
                ...
                if (thatPlayer == localPlayer)
                {
                    ...
                    thatPlayer != ThePlayerList->getNeutralPlayer()
    ...
    if (loadingSaveGame == FALSE && isInSinglePlayerGame())
    {
        for (Int i = 0; i < MAX_PLAYER_COUNT; ++i)
        {
            ...
            if (pPlayer)
            {
                ...
                DEBUG_LOG(
                    ("GameLogic::startNewGame() - adding m_rankPointsToAddAtGameStart==%d to player %d(%ls)",
    ...
    else
    {
        ...
        if (m_gameMode == GAME_REPLAY)
        {
            ThePlayerList->setLocalPlayer(
                ThePlayerList->findPlayerWithNameKey(TheNameKeyGenerator->nameToKey("ReplayObserver")));
            TheRadar->forceOn(TRUE);
            ThePartitionManager->refreshShroudForLocalPlayer();
            TheControlBar->setControlBarSchemeByPlayer(ThePlayerList->getLocalPlayer());
            DEBUG_LOG(
                ("Start of a replay game %ls, %d",
...
// If defined, the game times various units.
    ...
    fputs(
        "Full,100*ms,NoPart-NoSpawn,,No Spawn,100*ms,Logic,100*ms,Thing,Model,Kind,Side,DrawCalls All,DrawCalls "
...
void GameLogic::processCommandList(CommandList *list)
{
    ...
    if (m_shouldValidateCRCs && !TheNetwork->sawCRCMismatch())
    {
        ...
        if (sawCRCMismatch)
        {
            ...
            for (std::map<Int, UnsignedInt>::const_iterator crcIt = m_cachedCRCs.begin(); crcIt != m_cachedCRCs.end();
                 ++crcIt)
            {
                Player *player = ThePlayerList->getNthPlayer(crcIt->first);
                DEBUG_LOG(
                    ("CRC from player %d (%ls) = %X",
                     crcIt->first,
...
#define SLEEPY_DEBUG
    ...
    {
        DEBUG_ASSERTCRASH(
            m_sleepyUpdates[i]->friend_getIndexInLogic() == i,
            ("index mismatch: expected %d, got %d", i, m_sleepyUpdates[i]->friend_getIndexInLogic()));
...
void GameLogic::friend_awakenUpdateModule(Object *obj, UpdateModulePtr u, UnsignedInt whenToWakeUp)
{
    ...
    {
        DEBUG_CRASH(
            ("You should not call setWakeFrame() from inside your update(), because it will be ignored, in favor of the "
             "return code from update."));
        return;
    }
...
#define DO_SINGLE_KIND
    ...
    if (timeFrames > 0)
    {
        ...
        sprintf(
            remark,
            "All %f: (%d ms) for %d %s's\n",
            timeAll,
            (Int)(timeAll * 1000 / TIME_FRAMES),
            TOTAL_UNITS,
            thingName.str());
        DEBUG_LOG((remark));
        ...
        if (g_UT_commaLog)
        {
            ...
            sprintf(
                remark,
                "%f,%d,%f,%d,%f,%d,%f,%d,%s,%s,%s,%s,%f,%f,%f\n",
                timeAll,
                (Int)(timeAll * 1000 / TIME_FRAMES),
                timeNoPart,
                (Int)(timeNoPart * 1000 / TIME_FRAMES),
                timeNoSpawn,
                (Int)(timeNoSpawn * 1000 / TIME_FRAMES),
                timeLogic,
                (Int)(timeLogic * 1000 / TIME_FRAMES),
                thingName.str(),
                modelName.str(),
                type.str(),
                sides[side].str(),
    ...
    if (g_UT_startTiming && g_UT_curThing && !g_UT_gotUnit)
    {
        ...
        while (!g_UT_gotUnit)
        {
            ...
            //			if (btt->getName() endsWith("GLAAngryMobMolotovCocktailProjectileObject")) continue; // 100 overloads
            //system. 			if (btt->getName().startsWith("Firestorm"))	continue;	// 100 crashes 			if
            //(btt->getName().startsWith("Avalanche"))	continue;	// 100 crashes 			if
            //(btt->getName().startsWith("InfernoTankShell"))	continue;	// 100 crashes
            //
            //			if (btt->getName() endsWith("ChinaArtilleryBarrageShell") continue; // 100 takes really, freaking
            //long. Doesn't crash jba. 			if (btt->getName() endsWith("ChinaTankOverlordBattleBunker") continue; // 100 seems to
            //hang gth. 			if (btt->getName() endsWith("ChinaTankOverlordPropagandaTower") continue; // 100 seems to hang gth.
...
void GameLogic::update(void)
{
    ...
    if (m_startNewGame && !TheDisplay->isMoviePlaying())
    {
        ...
        sprintf(
            Buf,
            "Texture=%f, Anim=%f, CreateRobj=%f, Load3DAssets=%f",
    ...
    Bool isMPGameOrReplay =
        (TheRecorder && TheRecorder->isMultiplayer() && getGameMode() != GAME_SHELL && getGameMode() != GAME_NONE);
...
#ifdef DEBUG_CRC
    if ((g_crcModuleDataFromClient && !isInGameLogicUpdate()) || (g_crcModuleDataFromLogic && isInGameLogicUpdate()))
    {
...
void GameLogic::processProgressComplete(Int playerId)
{
    ...
    {
        DEBUG_LOG(
            ("GameLogic::processProgressComplete, playerId %d is marked TRUE already yet we're trying to mark him as true "
             "again",
             playerId));
        return;
    }
...
// ------------------------------------------------------------------------------------------------
Bool GameLogic::findControlBarOverride(const AsciiString &commandSetName, Int slot, ConstCommandButtonPtr &commandButton)
    const
{
    char buf[256];
    buf[0] = '0' + slot;
...
void GameLogic::prepareLogicForObjectLoad(void)
{
    ...
    DEBUG_ASSERTCRASH(
        getFirstObject() == NULL,
        ("GameLogic::prepareLogicForObjectLoad - There are still objects loaded in the engine, but it should be empty (Top "
         "is '%s')",
...
void GameLogic::xfer(Xfer *xfer)
{
    ...
    else
    {
        ...
        for (UnsignedInt i = 0; i < objectCount; ++i)
        {
            ...
            {
                DEBUG_CRASH(
                    ("GameLogic::xfer - Unrecognized thing template name '%s', skipping.  ENGINEERS - Are you *sure* it's "
                     "OK to be ignoring this object from the save file???  Think hard about it!",
                     tocEntry->name.str()));
    ...
    if (version >= 3)
    {
        ...
        {
            DEBUG_CRASH(
                ("GameLogic::xfer - Polygon trigger count mismatch.  Save file has a count of '%d', but map had '%d' "
                 "triggers",
                 sanityTriggerCount,
                 triggerCount));
            throw SC_INVALID_DATA;
        ...
        else
        {
            ...
            for (UnsignedInt i = 0; i < triggerCount; ++i)
            {
                ...
                if (poly == NULL)
                {
                    DEBUG_CRASH(("GameLogic::xfer - Unable to find polygon trigger with id '%d'", triggerID));
                    throw SC_INVALID_DATA;
    ...
    if (version >= 7)
    {
        if (xfer->getXferMode() == XFER_SAVE)
        {
            for (BuildableMap::const_iterator it = m_thingTemplateBuildableOverrides.begin();
                 it != m_thingTemplateBuildableOverrides.end();
                 ++it)
            {
    ...
    if (version >= 8)
    {
        ...
        if (xfer->getXferMode() == XFER_SAVE)
        {
            for (ControlBarOverrideMap::const_iterator it = m_controlBarOverrides.begin(); it != m_controlBarOverrides.end();
                 ++it)
            {
                AsciiString name = it->first;
