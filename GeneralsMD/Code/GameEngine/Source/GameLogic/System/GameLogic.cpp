    ...
    const PlayerTemplate *pTemplate)
{
  ...
  if (obj)
  {
    ...
    CRCDEBUG_LOG(("Placed an object for %ls at pos (%g,%g,%g)", pPlayer->getPlayerDisplayName().str(), pos.x, pos.y, pos.z));
    DUMPCOORD3D(&pos);
...
#define DO_SINGLE_KIND
  ...
  if (g_UT_startTiming && g_UT_curThing && !g_UT_gotUnit)
  {
    ...
    while (!g_UT_gotUnit)
    {
      ...
      static char *illegalTemplateNames[] = { "EMPPulseBomb",
      ...
      //
      //			if (btt->getName() endsWith("ChinaArtilleryBarrageShell") continue; // 100 takes really, freaking long.
      // Doesn't crash jba. 			if (btt->getName() endsWith("ChinaTankOverlordBattleBunker") continue; // 100 seems to hang
      // gth. 			if (btt->getName() endsWith("ChinaTankOverlordPropagandaTower") continue; // 100 seems to hang gth. if
      //(btt->getName() endsWith("ChinaTankOverlordGattlingCannon") continue; // 100 seems to hang gth. 			if
      //(btt->getName().startsWith("CINE")) continue; 			if (btt->getName() endsWith("GLAInfantryAngryMobNexus") continue;
      //
      //      //missiondisk perps
      //
  ...
  for (BehaviorModule **b = obj->getBehaviorModules(); *b; ++b)
  {
...
#endif
    {
      DEBUG_ASSERTCRASH(
          ...
          ("you may not specify a zero initial sleep time for sleepy modules (%d %d)", u->friend_getNextCallFrame(), now));
      pushSleepyUpdate(u);
    }
  }
}
