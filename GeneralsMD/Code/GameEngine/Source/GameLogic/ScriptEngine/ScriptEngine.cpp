    ...
    if (xfer->getXferMode() == XFER_SAVE)
    {
        if (m_priorityMap)
        {
            ...
            DEBUG_ASSERTCRASH(
                ...
                ("AttackPriorityInfo::xfer - Mismatch in priority map size.  Size() method returned '%d' but actual "
                 "iteration count was '%d'",
                 priorityMapCount,
                 count));
...
void ScriptEngine::reset(void)
{
    ...
    if (TheSidesList)
    {
        for (numToDump = 0; numToDump < 10; numToDump++)
        {
            ...
            {
                DEBUG_LOG(
                    ("   SCRIPT %s total time %f seconds,\n        evaluated %d times, avg execution %2.3f msec (Goal less "
                     "than 0.05)",
...
/* dump out the named objects table.  For extremely intense debug only.  jba. :P
    for (VecNamedRequestsIt it = m_namedObjects.begin(); it != m_namedObjects.end(); ++it) {
        ...
        if (obj && obj->getAIUpdateInterface())
            DEBUG_LOG(("%s=%x('%s'), isDead%d", name.str(), obj, obj->getName().str(),
...
Player *ScriptEngine::getPlayerFromAsciiString(const AsciiString &playerString)
{
    ...
    if (playerString == LOCAL_PLAYER || (playerString == THE_PLAYER && is_GeneralsChallengeContext))
        ...
        // However, they've also built many of their single player maps with this string, where "ThePlayer" is not intended
        // as an alias.
...
Team *ScriptEngine::getTeamNamed(const AsciiString &teamName)
{
    ...
    if (teamName == TEAM_THE_PLAYER && is_GeneralsChallengeContext)
        // Designers have built their Generals' Challenge maps, referencing "teamThePlayer" meaning the local player's
        // default (parent) team. However, they've also built many of their single player maps with this string, where
        // "teamThePlayer" is not intended as an alias.
        ...
        for (pCondition = pOr->getFirstAndCondition(); pCondition; pCondition = pCondition->getNext())
        {
            ...
            for (i = 0; i < pCondition->getNumParameters(); i++)
            {
                if (Parameter::TEAM == pCondition->getParameter(i)->getParameterType())
                {
                    ...
                    {
                        singletonTeamName =
                            teamName; // Singleton team - use if it is the only one, but can have multiple of these.
                    }
                    else
                    {
                ...
                case 0x07:
                {
                    ...
                    /*
                                        ...
                                        while (true)
                                        {
                                            ...
                                            //before removing the system, make sure to remove all of its particles
                       individually while (sys->getParticleCount() > 0) {
