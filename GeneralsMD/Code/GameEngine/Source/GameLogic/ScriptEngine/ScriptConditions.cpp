    ...
    Parameter *pTypeParm)
{
    ...
    {
        return (
            theTeam->someInsideSomeOutside(pTrig, (UnsignedInt)pTypeParm->getInt())
...
{
    return !(
        evaluateTeamInsideAreaEntirely(pTeamParm, pTriggerParm, pTypeParm)
...
Bool ScriptConditions::evaluateEnemySighted(Parameter *pItemParm, Parameter *pAllianceParm, Parameter *pPlayerParm)
{
    ...
    SimpleObjectIterator *iter = ThePartitionManager->iterateObjectsInRange(theObj, visionRange, FROM_CENTER_2D, filters);
...
Bool ScriptConditions::evaluateNamedDiscovered(Parameter *pItemParm, Parameter *pPlayerParm)
{
    ...
    if (theObj->getStatusBits().test(OBJECT_STATUS_STEALTHED) && !theObj->getStatusBits().test(OBJECT_STATUS_DETECTED)
...
Bool ScriptConditions::evaluateTeamDiscovered(Parameter *pTeamParm, Parameter *pPlayerParm)
{
    ...
    for (DLINK_ITERATOR<Object> iter = theTeam->iterate_TeamMemberList(); !iter.done(); iter.advance())
    {
        ...
        // If we are stealthed we are not visible.
        if (pObj->getStatusBits().test(OBJECT_STATUS_STEALTHED) && !pObj->getStatusBits().test(OBJECT_STATUS_DETECTED)
    ...
    Parameter *pUnitParm)
{
    ...
    if (pUnitParm)
    {
        ...
        if (!pUnit)
        {
            // we cared about the source object, but it is dead.  No sense checking anymore, since we don't know it's
            // objectID anymore. :P
            return FALSE;
        }
    ...
    if (pPlayer)
    {
        return TheScriptEngine
            ->isSpecialPowerTriggered(pPlayer->getPlayerIndex(), pSpecialPowerParm->getString(), TRUE, sourceID);
    }
    return FALSE;
}
    ...
    Parameter *pUnitParm)
{
    ...
    {
        return TheScriptEngine
            ->isSpecialPowerMidway(pPlayer->getPlayerIndex(), pSpecialPowerParm->getString(), TRUE, sourceID);
    }
    return FALSE;
}
    ...
    Parameter *pUnitParm)
{
    ...
    {
        return TheScriptEngine
            ->isSpecialPowerComplete(pPlayer->getPlayerIndex(), pSpecialPowerParm->getString(), TRUE, sourceID);
    }
    return FALSE;
}
...
//-------------------------------------------------------------------------------------------------
Bool ScriptConditions::evaluatePlayerUnitCondition(
    Condition *pCondition,
    Parameter *pPlayerParm,
    Parameter *pComparisonParm,
    Parameter *pCountParm,
    ...
    Parameter *pValueParm)
{
    ...
    SimpleObjectIterator *iter =
        ThePartitionManager->iterateObjectsInRange(&center, distance, FROM_CENTER_2D, filters, ITER_FASTEST);
    MemoryPoolObjectHolder hold(iter);
    ...
    for (Object *them = iter->first(); them; them = iter->next())
    {
        ...
        SupplyWarehouseDockUpdate *warehouseModule =
            (SupplyWarehouseDockUpdate *)them->findUpdateModule(key_warehouseUpdate);
        if (!warehouseModule)
        {
            continue;
        }
...
Bool ScriptConditions::evaluateSkirmishPlayerTechBuildingWithinDistancePerimeter(
    Condition *pCondition,
    Parameter *pSkirmishPlayerParm,
    Parameter *pDistanceParm,
    Parameter *pLocationParm)
...
Bool ScriptConditions::evaluateCondition(Condition *pCondition)
{
    switch (pCondition->getConditionType())
    {
        ...
        case Condition::TEAM_INSIDE_AREA_PARTIALLY:
            return evaluateTeamInsideAreaPartially(
        ...
        case Condition::TEAM_INSIDE_AREA_ENTIRELY:
            return evaluateTeamInsideAreaEntirely(
                pCondition->getParameter(0),
                pCondition->getParameter(1),
                pCondition->getParameter(2));
        case Condition::TEAM_OUTSIDE_AREA_ENTIRELY:
            return evaluateTeamOutsideAreaEntirely(
                pCondition->getParameter(0),
                pCondition->getParameter(1),
                pCondition->getParameter(2));
        ...
        case Condition::PLAYER_HAS_CREDITS:
            return evaluatePlayerHasCredits(
                pCondition->getParameter(0),
                pCondition->getParameter(1),
                pCondition->getParameter(2));
        ...
        case Condition::ENEMY_SIGHTED:
        {
            ...
            DEBUG_ASSERTCRASH(
                numParameters == 3,
                ("'Condition: [Unit] Unit has sighted a(n) friendly/neutral/enemy unit belonging to a side.' has too few "
                 "parameters. Please fix in WB. (jkmcd)"));
            ...
            return evaluateEnemySighted(
                pCondition->getParameter(0),
                pCondition->getParameter(1),
                pCondition->getParameter(2));
        }
        case Condition::TYPE_SIGHTED:
            return evaluateTypeSighted(
                pCondition->getParameter(0),
                pCondition->getParameter(1),
                pCondition->getParameter(2));
        ...
        case Condition::MISSION_ATTEMPTS:
            return evaluateMissionAttempts(
                pCondition->getParameter(0),
                pCondition->getParameter(1),
                pCondition->getParameter(2));
        ...
        case Condition::TEAM_ENTERED_AREA_ENTIRELY:
            return evaluateTeamEnteredAreaEntirely(
                pCondition->getParameter(0),
                pCondition->getParameter(1),
                pCondition->getParameter(2));
        case Condition::TEAM_ENTERED_AREA_PARTIALLY:
            return evaluateTeamEnteredAreaPartially(
                pCondition->getParameter(0),
                pCondition->getParameter(1),
                pCondition->getParameter(2));
        case Condition::TEAM_EXITED_AREA_ENTIRELY:
            return evaluateTeamExitedAreaEntirely(
                pCondition->getParameter(0),
                pCondition->getParameter(1),
                pCondition->getParameter(2));
        case Condition::TEAM_EXITED_AREA_PARTIALLY:
            return evaluateTeamExitedAreaPartially(
                pCondition->getParameter(0),
                pCondition->getParameter(1),
                pCondition->getParameter(2));
        ...
        case Condition::PLAYER_BUILT_UPGRADE_FROM_NAMED:
            return evaluateUpgradeFromUnitComplete(
                pCondition->getParameter(0),
                pCondition->getParameter(1),
                pCondition->getParameter(2));
        ...
        case Condition::PLAYER_HAS_COMPARISON_UNIT_TYPE_IN_TRIGGER_AREA:
        {
            ...
            DEBUG_ASSERTCRASH(
                numParameters == 5,
                ("'Condition: [Player] has (comparison) unit type in an area' has too few parameters. Please fix in WB. "
                 "(jkmcd)"));
        ...
        case Condition::PLAYER_HAS_COMPARISON_UNIT_KIND_IN_TRIGGER_AREA:
        {
            ...
            DEBUG_ASSERTCRASH(
                numParameters == 5,
                ("'Condition: [Player] has (comparison) kind of unit or structure in an area' has too few parameters. "
                 "Please fix in WB. (jkmcd)"));
