		...
		Parameter *pUnitParm)
{
	...
	if (pUnitParm)
	{
		...
		if (!pUnit)
		{
			// we cared about the source object, but it is dead.  No sense checking anymore, since we don't know it's objectID
			// anymore. :P
			return FALSE;
		}
...
Bool ScriptConditions::evaluateCondition(Condition *pCondition)
{
	switch (pCondition->getConditionType())
	{
		...
		case Condition::PLAYER_HAS_COMPARISON_UNIT_TYPE_IN_TRIGGER_AREA:
		{
			...
			DEBUG_ASSERTCRASH(
					numParameters == 5,
					("'Condition: [Player] has (comparison) unit type in an area' has too few parameters. Please fix in WB. (jkmcd)"));
		...
		case Condition::PLAYER_HAS_COMPARISON_UNIT_KIND_IN_TRIGGER_AREA:
		{
			...
			DEBUG_ASSERTCRASH(
					numParameters == 5,
					("'Condition: [Player] has (comparison) kind of unit or structure in an area' has too few parameters. Please fix "
					 "in WB. (jkmcd)"));
