...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
Bridge::Bridge(BridgeInfo &theInfo, Dict *props, AsciiString bridgeTemplateName) : m_bridgeInfo(theInfo)
{
...
Bool LineInRegion(const Coord2D *p1, const Coord2D *p2, const Region2D *clipRegion)
{
  ...
  // Line is visible
  return (
      x1 >= clipLeft && x1 <= clipRight && y1 >= clipTop && y1 <= clipBottom && x2 >= clipLeft && x2 <= clipRight
...
{
  return (pt->x >= clipRegion->lo.x && pt->y >= clipRegion->lo.y && pt->x <= clipRegion->hi.x && pt->y <= clipRegion->hi.y);
}
          ...
          for (obj = TheGameLogic->getFirstObject(); obj; obj = obj->getNextObject())
          {
            if (obj->getLayer() == m_layer)
            {
              ...
              if (TheTerrainLogic->objectInteractsWithBridgeLayer(obj, obj->getLayer(), considerBridgeHealth))
              {
                ...
                // might sink thru the eyecandy of bridge drbris, looking funny. so now we just
                // indiscriminately kill everything that was on the bridge, regardless of height they might
                // fall.
                // Real deltaHeight = obj->getPosition()->z -
                // TheTerrainLogic->getGroundHeight(obj->getPosition()->x, obj->getPosition()->y); if
                // (deltaHeight>PATHFIND_CELL_SIZE_F * 0.5f)
                {
...
void TerrainLogic::update(void)
{
  ...
  if (m_numWaterToUpdate)
  {
    ...
    Bool finalTransition, doDamageThisFrame = (TheGameLogic->getFrame() % LOGICFRAMES_PER_SECOND) == 0;
...
void TerrainLogic::enableWaterGrid(Bool enable)
{
  ...
  if (enable == TRUE)
  {
    ...
    {
      DEBUG_CRASH(
          ("!!!!!! Deformable water won't work because there was no group of vertex water data defined in "
           "GameData.INI for this map name '%s' !!!!!! (C. Day)",
...
void TerrainLogic::addWaypoint(MapObject *pMapObj)
{
  ...
  Waypoint *pWay = newInstance(
      Waypoint)(pMapObj->getWaypointID(), pMapObj->getWaypointName(), &loc, label1, label2, label3, biDirectional);
...
void makeAlignToNormalMatrix(Real angle, const Coord3D &pos, const Coord3D &normal, Matrix3D &mtx)
{
  ...
  */
  ...
  DEBUG_ASSERTCRASH(
      ...
      ("dot is not zero (%f)", fabs(x.x * z.x + x.y * z.y + x.z * z.z)));
...
void TerrainLogic::deleteBridge(Bridge *bridge)
{
  ...
  {
    for (Bridge *otherBridge = getFirstBridge(); otherBridge; otherBridge = otherBridge->getNext())
    {
      //
...
// ------------------------------------------------------------------------------------------------
void TerrainLogic::setWaterHeight(const WaterHandle *water, Real height, Real damageAmount, Bool forcePathfindUpdate)
{
  // sanity
  if (water == NULL)
    return;
  ...
  if (damageAmount > 0.0f && height > previousHeight)
  {
    ...
    Real maxDist = sqrt(affectedRegion.width() * affectedRegion.width() + affectedRegion.height() * affectedRegion.height());

    // scan the objects in the area of the water affected
    ObjectIterator *iter = ThePartitionManager->iterateObjectsInRange(&center, maxDist, FROM_CENTER_2D, NULL);
...
// ------------------------------------------------------------------------------------------------
void TerrainLogic::changeWaterHeightOverTime(
    const WaterHandle *water,
    Real finalHeight,
    ...
    Real damageAmount)
{
  ...
  m_waterToUpdate[m_numWaterToUpdate].changePerFrame =
      (finalHeight - currentHeight) / (LOGICFRAMES_PER_SECOND * transitionTimeInSeconds);
  ...
  if (version >= 2)
  {
    ...
    for (UnsignedInt i = 0; i < m_numWaterToUpdate; ++i)
    {
      ...
      else if (xfer->getXferMode() == XFER_LOAD)
      {
        ...
        {
          DEBUG_CRASH(
              ("TerrainLogic::xfer - Unable to find polygon trigger for water table with trigger ID '%d'", triggerID));
