...
{
	for (PolygonTrigger *poly = PolygonTrigger::getFirstPolygonTrigger(); poly; poly = poly->getNext())
...
Bool PolygonTrigger::ParsePolygonTriggersDataChunk(DataChunkInput &file, DataChunkInfo *info, void *userData)
{
	...
	while (count > 0)
	{
		...
		if (numPoints < 2)
		{
			DEBUG_LOG(("Deleting polygon trigger '%s' with %d points.", pTrig->getTriggerName().str(), numPoints));
