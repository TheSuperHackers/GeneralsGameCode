        ...
        if (m_targetIsBldg)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    Real offset =
                        min(obj->getGeometryInfo().getBoundingCircleRadius(),
...
AIStateMachine::AIStateMachine(Object *obj, AsciiString name) : StateMachine(obj, name)
{
    ...
    defineState(
        ...
        newInstance(AIFollowWaypointPathState)(this, false),
        AI_IDLE,
        AI_IDLE);
    defineState(
...
static Bool cannotPossiblyAttackObject(State *thisState, void *userData)
{
    ...
    if (obj && victim)
    {
        ...
        CanAttackResult result =
            obj->getAbleToAttackSpecificObject(attackType, victim, obj->getAI()->getLastCommandSource());
...
Bool AIAttackApproachTargetState::computePath()
{
    ...
    if (getMachineGoalObject())
    {
        ...
        CRCDEBUG_LOG(
            ("AIAttackApproachTargetState::computePath - bailing after repathing for object %d",
             getMachineOwner()->getID()));
        return true;
    }
    else
...
StateReturnType AIAttackMoveToState::update()
{
    ...
    if (ret != STATE_CONTINUE)
    {
        ...
        Real distSqr =
            sqr(owner->getPosition()->x - m_pathGoalPosition.x) + sqr(owner->getPosition()->y - m_pathGoalPosition.y);
...
StateReturnType AIAttackAimAtTargetState::onEnter()
{
    ...
    { // non enclosing garrison containers do not use firepoints. Lorenzen, 6/11/03
        ...
        inFiringRange =
            weapon->isWithinAttackRange(source, targetPos); // See, I can be attacking the ground.  Perfectly valid.
    else
...
StateReturnType AIAttackAimAtTargetState::update()
{
    ...
    // no else here!
    {
        ...
        const Real REL_THRESH =
            0.035f; // about 2 degrees. (getRelativeAngle2D is current only accurate to about 1.25 degrees)
...
StateReturnType AIAttackFireWeaponState::update()
{
    ...
    */
    ...
    if (m_att->isAttackingObject())
    {
        ...
        // victim in our attack state machine. When we move immediately to the aim state in the same frame as the transfer
        // (after this call in fact), the victim was still pointing to the building and not the hole we transferred to. This
        // code fixes that.
...
void AIAttackState::xfer(Xfer *xfer)
{
    ...
    Bool										m_follow;
    Bool										m_isAttackingObject;								// if false, attacking
    position AttackExitConditionsInterface*	m_attackParameters;					///< these are not owned by this, and will
    not be deleted on destruction Bool										m_isForceAttacking
    */
} // end xfer
...
StateReturnType AIAttackState::onEnter()
{
    ...
    // Kris: Jan 12, 2005
    // Don't allow units under construction to attack! The selection/action manager system was responsible for preventing
    // this from ever happening, but failed in two cases which I fixed. This is an extra check to mitigate cheats.
    ...
    if (goal)
    {
        ...
        if (!TheActionManager->canEnterObject(obj, goal, obj->getAI()->getLastCommandSource(), CHECK_CAPACITY))
        {
            /*
                ...
                3. have infantry  units garrison a neutral building, before they garrison the building have some enemy units
               garrison it first.

                Expected result: Based on test plan: Instead of just stopping when enemy units garrison building first, they
               should continue and attack the building.
            */
...
StateReturnType AIExitState::update()
{
    ...
    if (goal)
    {
        ...
        if (getMachine()->getCurrentStateID() != getID())
            return STATE_CONTINUE; // Not sucess, because exitViaDoor has changed us to FollowPath, and if we say Success,
                                   // our machine will think FollowPath succeeded
        else
            return STATE_SUCCESS;
    }
    else
...
StateReturnType AIHuntState::update()
{
    ...
    getMachine()->lock(
        "AIHuntState::update"); // The idle state in the sub machine can sometimes acquire targets.
...
StateReturnType AIAttackAreaState::update()
{
    ...
    getMachine()->lock(
        "AIAttackAreaState::update"); // The idle state in the sub machine can sometimes acquire targets.
                                      // It is important to not switch out of this state via a sub machine call. jba.
    /*
        Note the use of CONVERT_SLEEP_TO_CONTINUE; even if the sub-machine
        sleeps, we still need to be called every frame.
