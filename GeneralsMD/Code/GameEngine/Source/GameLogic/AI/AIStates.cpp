	...
	*/
	...
	if (obj->isKindOf(KINDOF_IMMOBILE) == FALSE)
	{
		if (obj->isKindOf(KINDOF_PORTABLE_STRUCTURE) && obj->isKindOf(KINDOF_CAN_ATTACK))
		{
			...
			*/
			defineState(
					AttackStateMachine::CHASE_TARGET,
...
StateReturnType AIRappelState::update()
{
	...
	if (pos->z <= m_destZ)
	{
		...
		{
			DEBUG_ASSERTCRASH(
					TheActionManager->canEnterObject(obj, bldg, obj->getAI()->getLastCommandSource(), COMBATDROP_INTO),
			...
			else
			{
				...
				else
				{
					...
					Real offset =
							min(obj->getGeometryInfo().getBoundingCircleRadius(), bldg->getGeometryInfo().getBoundingCircleRadius());
...
AIStateMachine::AIStateMachine(Object *obj, AsciiString name) : StateMachine(obj, name)
{
	DEBUG_ASSERTCRASH(
			getOwner(),
			("An AI State Machine '%s' was constructed without an owner, please tell JKMCD", name.str()));
	DEBUG_ASSERTCRASH(
			getOwner()->getAI(),
			("An AI State Machine '%s' was constructed without an AIUpdateInterface, please tell JKMCD", name.str()));
	...
	defineState(
			...
			newInstance(AIFollowWaypointPathExactState)(this, true),
			AI_IDLE,
			AI_IDLE);
	defineState(
			AI_FOLLOW_WAYPOINT_PATH_AS_INDIVIDUALS_EXACT,
			newInstance(AIFollowWaypointPathExactState)(this, false),
			AI_IDLE,
			AI_IDLE);
	...
	defineState(
			...
			newInstance(AIMoveAndEvacuateState)(this),
			AI_MOVE_AND_DELETE,
			AI_MOVE_AND_DELETE);
...
Bool outOfWeaponRangeObject(State *thisState, void *userData)
{
	...
	if (victim && weapon)
	{
		...
		{
			viewBlocked =
					TheAI->pathfinder()->isAttackViewBlockedByObstacle(obj, *obj->getPosition(), victim, *victim->getPosition());
		}
...
AIIdleState::AIIdleState(StateMachine *machine, AIIdleState::AIIdleTargetingType shouldLookForTargets) :
		State(machine, "AIIdleState"), m_shouldLookForTargets(shouldLookForTargets == AIIdleState::LOOK_FOR_TARGETS)
{
	m_inited = FALSE;
...
StateReturnType AIIdleState::update()
{
	...
	if (m_shouldLookForTargets && !getMachine()->isLocked())
	{
		...
		if (!obj->isDisabledByType(DISABLED_PARALYZED) && !obj->isDisabledByType(DISABLED_UNMANNED)
				&& !obj->isDisabledByType(DISABLED_EMP) && !obj->isDisabledByType(DISABLED_SUBDUED)
...
StateReturnType AIInternalMoveToState::onEnter()
{
	...
	if (physics && physics->isMotive())
	{
		...
		ModelConditionFlagType modelConditionFlag =
				(cell && cell->getType() != PathfindCell::CELL_CLIFF) ? MODELCONDITION_MOVING : MODELCONDITION_CLIMBING;
...
void AIInternalMoveToState::onExit(StateExitType status)
{
	...
	if (ai)
	{
		...
		if (obj->getTeam() && ai->isDoingGroundMovement() && ai->getCurLocomotor() && ai->getCurLocomotor()->isUltraAccurate())
		{
...
StateReturnType AIInternalMoveToState::update()
{
	...
	if (ai->getCurLocomotor() && (onPathDistToGoal < ai->getCurLocomotor()->getCloseEnoughDist()))
	{
		...
		DEBUG_ASSERTLOG(
				!getMachine()->getWantsDebugOutput(),
				("AIInternalMoveToState::update: reached end of path, exiting state with success"));
		// Kris: 7/01/03 (Temporary debug hook for units not being able to leave maps)
		if (blah)
		{
			blah = blah;
...
static Bool canPursue(Object *source, Weapon *weapon, Object *victim)
{
	...
	if (TheAI->getAiData()->m_aiCrushesInfantry)
	{
		if (source->getControllingPlayer() && (source->getControllingPlayer()->getPlayerType() == PLAYER_COMPUTER)
...
Bool AIAttackApproachTargetState::computePath()
{
	...
	if (getMachineGoalObject())
	{
		...
		{
			CRCDEBUG_LOG(
					("AIAttackApproachTargetState::computePath - bailing because victim in same place for object %d",
					 getMachineOwner()->getID()));
			return true;
		}
		...
		{
			CRCDEBUG_LOG(
					("AIAttackApproachTargetState::computePath - bailing because of no weapon for object %d",
					 getMachineOwner()->getID()));
			return false;
		}
		...
		CRCDEBUG_LOG(
				("AIAttackApproachTargetState::computePath - requestAttackPath() for object %d", getMachineOwner()->getID()));
		ai->requestAttackPath(victim->getID(), &pos);
		m_stopIfInRange = false; // we have calculated a position to shoot from, so go there.

		CRCDEBUG_LOG(
				("AIAttackApproachTargetState::computePath - bailing after repathing for object %d", getMachineOwner()->getID()));
		return true;
	}
	else
	{
		...
		{
			CRCDEBUG_LOG(
					("AIAttackApproachTargetState::computePath - bailing because we're aiming for a fixed position for object %d",
					 getMachineOwner()->getID()));
		...
		CRCDEBUG_LOG(
				("AIAttackApproachTargetState::computePath - bailing after repathing at a fixed position for object %d",
				 getMachineOwner()->getID()));
		return true;
	}

	CRCDEBUG_LOG(
			("AIAttackApproachTargetState::computePath - bailing at end of function for object %d", getMachineOwner()->getID()));
	return true;
}
...
StateReturnType AIAttackApproachTargetState::onEnter()
{
	...
	if (victim)
	{
		...
		if (weapon->isWithinAttackRange(source, victim))
		{
			...
			{
				viewBlocked = TheAI->pathfinder()
													->isAttackViewBlockedByObstacle(source, *source->getPosition(), victim, *victim->getPosition());
			}
			if (!viewBlocked)
			{
				return STATE_SUCCESS;
...
StateReturnType AIAttackApproachTargetState::updateInternal()
{
	...
	else
	{
		...
		if (m_stopIfInRange && weapon && weapon->isWithinAttackRange(source, &m_goalPosition))
		{
			...
			{
				viewBlocked =
						TheAI->pathfinder()->isAttackViewBlockedByObstacle(source, *source->getPosition(), NULL, m_goalPosition);
			}
			if (!viewBlocked)
			{
				return STATE_SUCCESS;
...
StateReturnType AIAttackPursueTargetState::onEnter()
{
	...
	if (source->isKindOf(KINDOF_PROJECTILE))
	{
		// CRCDEBUG_LOG(("AIAttackPursueTargetState::onEnter() - is a projectile for object %d (%s)", getMachineOwner()->getID(),
	...
	if (!m_isAttackingObject)
	{
		// CRCDEBUG_LOG(("AIAttackPursueTargetState::onEnter() - not attacking for object %d (%s)", getMachineOwner()->getID(),
		// getMachineOwner()->getTemplate()->getName().str()));
	...
	if (victim)
	{
		...
		if (!canPursue(source, weapon, victim))
		{
			// CRCDEBUG_LOG(("AIAttackPursueTargetState::onEnter() - can't pursue for object %d (%s)", getMachineOwner()->getID(),
			// getMachineOwner()->getTemplate()->getName().str()));
			return STATE_SUCCESS;
		}
	}
	...
	{
		// CRCDEBUG_LOG(("AIAttackPursueTargetState::onEnter() - no victim for object %d (%s)", getMachineOwner()->getID(),
		// getMachineOwner()->getTemplate()->getName().str()));
	...
	{
		// CRCDEBUG_LOG(("AIAttackPursueTargetState::onEnter() - no turret for object %d (%s)", getMachineOwner()->getID(),
		// getMachineOwner()->getTemplate()->getName().str()));
...
StateReturnType AIAttackPursueTargetState::updateInternal()
{
	...
	if (victim)
	{
		...
		{
			viewBlocked =
					TheAI->pathfinder()->isAttackViewBlockedByObstacle(source, *source->getPosition(), victim, *victim->getPosition());
		}
		if (!viewBlocked && victim->getPhysics() && weapon->isWithinAttackRange(source, victim))
		{
			...
			// DEBUG_LOG(("VS %f, OS %f, goal %f", victim->getPhysics()->getForwardSpeed2D(),
...
{
	// contained by AIAttackState, so no separate timer
	// CRCDEBUG_LOG(("AIAttackPursueTargetState::onExit() for object %d (%s)", getMachineOwner()->getID(),
	// getMachineOwner()->getTemplate()->getName().str()));
	AIInternalMoveToState::onExit(status);
...
// TheSuperHackers @info helmutbuhler 05/05/2025 This debug mutates the code to become CRC incompatible
					...
					DEBUG_LOG(
							("Breaking out of follow waypoint path %s of %s",
...
StateReturnType AIAttackAimAtTargetState::update()
{
	...
	// no else here!
	{
		Real relAngle = m_isAttackingObject ? ThePartitionManager->getRelativeAngle2D(source, victim) :
		...
		// DEBUG_LOG(("AIM: desired %f, actual %f, delta %f, aimDelta %f, goalpos %f %f",rad2deg(obj->getOrientation() +
...
StateReturnType AIAttackFireWeaponState::update()
{
	...
	*/
	...
	if (m_att->isAttackingObject())
	{
		...
		// Fixes cases where some units couldn't transfer their attack to a different object. One example was Colonel Burton
		// attacking any GLA structure. When the structure was destroyed becoming a hole, Burton would stop attacking. Even
		// though there is code to transfer attackers (AIUpdateInterface::transferAttack), it is unable to modify our current
		// victim in our attack state machine. When we move immediately to the aim state in the same frame as the transfer (after
		// this call in fact), the victim was still pointing to the building and not the hole we transferred to. This code fixes
		// that.
		...
		if (continueRange > 0.0f && victim
				&& (victim->isDestroyed() || victim->isEffectivelyDead()
						|| (victim->isKindOf(KINDOF_MINE) && victim->testStatus(OBJECT_STATUS_MASKED))))
		{
			...
			if (originalVictimPos)
			{
				...
				victim = ThePartitionManager->getClosestObject(originalVictimPos, continueRange, FROM_CENTER_2D, filters); // could
																																																									 // be
																																																									 // null.
																																																									 // this is
																																																									 // ok.
		...
		m_victimTeam(NULL)
{
	...
	{
		DEBUG_LOG(
				("Creating attack state follow %d, attacking object %d, force attacking %d",
				 follow,
				 attackingObject,
				 forceAttacking));
	}
...
void AIAttackState::xfer(Xfer *xfer)
{
	...
	if (hasMachine && m_attackMachine == NULL)
	{
		...
		m_attackMachine = newInstance(
				AttackStateMachine)(getMachineOwner(), this, "AIAttackMachine", m_follow, m_isAttackingObject, m_isForceAttacking);
	}
	if (hasMachine)
	{
	...
	AttackExitConditionsInterface*	m_attackParameters;					///< these are not owned by this, and will not be deleted on
	destruction Bool										m_isForceAttacking
	*/
} // end xfer
...
StateReturnType AIAttackState::onEnter()
{
	...
	m_attackMachine =
			newInstance(AttackStateMachine)(source, this, "AIAttackMachine", m_follow, m_isAttackingObject, m_isForceAttacking);
...
Object *AIAttackSquadState::chooseVictim(void)
{
	...
	switch (difficulty)
	{
		...
		case DIFFICULTY_NORMAL:
		{
			...
			Object *victim =
					ThePartitionManager->getClosestObject(getMachineOwner(), HUGE_DIST, FROM_CENTER_2D, filters, NULL, NULL);
			return victim;
			break;
		}
	...
	if (goal)
	{
		...
		if (!TheActionManager->canEnterObject(obj, goal, obj->getAI()->getLastCommandSource(), CHECK_CAPACITY))
		{
			/*
				...
				3. have infantry  units garrison a neutral building, before they garrison the building have some enemy units garrison
				it first.

				Expected result: Based on test plan: Instead of just stopping when enemy units garrison building first, they should
				continue and attack the building.
			*/
			if (obj->getRelationship(goal) == ENEMIES && obj->getAI())
			{
				CanAttackResult result =
						TheActionManager->getCanAttackObject(obj, goal, obj->getAI()->getLastCommandSource(), ATTACK_NEW_TARGET);
				if (result == ATTACKRESULT_POSSIBLE || result == ATTACKRESULT_POSSIBLE_AFTER_MOVING)
				{
...
StateReturnType AIExitState::update()
{
	...
	if (goal)
	{
		...
		ExitDoorType exitDoor =
				goalExitInterface ? goalExitInterface->reserveDoorForExit(obj->getTemplate(), obj) : DOOR_NONE_NEEDED;
		if (exitDoor == DOOR_NONE_AVAILABLE)
			return STATE_FAILURE;

		goalExitInterface->exitObjectViaDoor(obj, exitDoor);
		if (getMachine()->getCurrentStateID() != getID())
			return STATE_CONTINUE; // Not sucess, because exitViaDoor has changed us to FollowPath, and if we say Success, our
...
StateReturnType AIExitInstantlyState::onEnter()
{
	...
	if (goal)
	{
		...
		return STATE_CONTINUE; // Not success, because exitViaDoor has changed us to FollowPath, and if we say Success, our
													 // machine will think FollowPath succeeded
	}
	else
	{
...
StateReturnType AIGuardState::update()
{
	...
	// (this can happen for units which never auto-reload, like the Raptor)
	Object *owner = getMachineOwner();
	if (owner->getAI()->getJetAIUpdate() && owner->isOutOfAmmo() && !owner->isKindOf(KINDOF_PROJECTILE)
...
StateReturnType AIAttackAreaState::update()
{
	...
	getMachine()->lock("AIAttackAreaState::update"); // The idle state in the sub machine can sometimes acquire targets.
																									 // It is important to not switch out of this state via a sub machine call.
																									 // jba.
	/*
		Note the use of CONVERT_SLEEP_TO_CONTINUE; even if the sub-machine
		sleeps, we still need to be called every frame.
