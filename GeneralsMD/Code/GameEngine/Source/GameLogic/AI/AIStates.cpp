...
AICommandParms::AICommandParms(AICommandType cmd, CommandSourceType cmdSource)
    : m_cmd(cmd), m_cmdSource(cmdSource), m_obj(nullptr), m_otherObj(nullptr),
      m_team(nullptr), m_waypoint(nullptr), m_polygon(nullptr), m_intValue(0),
      m_commandButton(nullptr), m_path(nullptr) {
  m_pos.zero();
  m_coords.clear();
}
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AttackStateMachine::crc(Xfer *xfer) { StateMachine::crc(xfer); }
...
AttackStateMachine::AttackStateMachine(Object *obj, AIAttackState *att,
  ...
  static const StateConditionInfo objectConditionsNormal[] = {
      StateConditionInfo(outOfWeaponRangeObject,
                         AttackStateMachine::CHASE_TARGET, nullptr),
    ...
    if (obj->isKindOf(KINDOF_PORTABLE_STRUCTURE) &&
      ...
      static const StateConditionInfo portableStructureChaseConditions[] = {
          StateConditionInfo(inWeaponRangeObject,
                             AttackStateMachine::AIM_AT_TARGET, nullptr),
          StateConditionInfo(nullptr, INVALID_STATE_ID, nullptr)};
...
static Object *findEnemyInContainer(Object *killer, Object *bldg) {
  const ContainedItemsList *items =
      bldg->getContain() ? bldg->getContain()->getContainedItemsList()
                         : nullptr;
  if (items) {
...
StateReturnType AIRappelState::onEnter() {
  ...
  if (bldg == nullptr || bldg->isEffectivelyDead() ||
...
AIStateMachine::AIStateMachine(Object *obj, AsciiString name)
  ...
  defineState(AI_ATTACK_POSITION,
              newInstance(AIAttackState)(this, false, false, false, nullptr),
              AI_IDLE, AI_IDLE);
  defineState(AI_ATTACK_OBJECT,
              newInstance(AIAttackState)(this, false, true, false, nullptr),
              AI_IDLE, AI_IDLE);
  defineState(AI_FORCE_ATTACK_OBJECT,
              newInstance(AIAttackState)(this, false, true, true, nullptr),
              AI_IDLE, AI_IDLE);

  defineState(AI_ATTACK_AND_FOLLOW_OBJECT,
              newInstance(AIAttackState)(this, true, true, false, nullptr),
              AI_IDLE, AI_IDLE);
  defineState(AI_ATTACK_SQUAD, newInstance(AIAttackSquadState)(this, nullptr),
              AI_IDLE, AI_IDLE);
  ...
  defineState(AI_EXIT_INSTANTLY, newInstance(AIExitInstantlyState)(this),
              AI_IDLE, AI_IDLE);
  ...
  defineState(AI_GUARD_RETALIATE, newInstance(AIGuardRetaliateState)(this),
              AI_IDLE, AI_IDLE);
...
//----------------------------------------------------------------------------------------------------------
AIStateMachine::~AIStateMachine() { deleteInstance(m_goalSquad); }

// ------------------------------------------------------------------------------------------------
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIStateMachine::crc(Xfer *xfer) { StateMachine::crc(xfer); }
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void AIStateMachine::loadPostProcess(void) { StateMachine::loadPostProcess(); }
...
//-extraLogging
  ...
  if (!idle && TheGlobalData->m_extraLogging)
    DEBUG_LOG(("%d - %s::update() start - %s", TheGameLogic->getFrame(),
               getCurrentStateName().str(),
               getOwner()->getTemplate()->getName().str()));
#endif
  // end -extraLogging
...

{
  ...
  if (getWantsDebugOutput()) {
    ...
    DEBUG_LOG_RAW(("%d '%s' -(TEMP)- '%s' %x exit ", TheGameLogic->getFrame(),
...
Bool outOfWeaponRangeObject(State *thisState, void *userData) {
  ...
  if (victim && weapon) {
    ...
    if (!weapon->hasLeechRange() && viewBlocked) {
      ...
      // blocked for attacking %d (%s)", 	obj->getID(),
    ...
    if (!weapon->hasLeechRange() && !weapon->isWithinAttackRange(obj, victim)) {
      ...
      // range for attacking %d (%s)", 	obj->getID(),
      //obj->getTemplate()->getName().str(), 	victim->getID(),
      //victim->getTemplate()->getName().str()));
      return true;
    }
...
Bool outOfWeaponRangePosition(State *thisState, void *userData) {
  ...
  if (weapon && pos) {
    ...
    if (onGround) {
      viewBlocked = TheAI->pathfinder()->isAttackViewBlockedByObstacle(
          obj, *obj->getPosition(), nullptr, *pos);
    }
    if (viewBlocked) {
      return true;
    }
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void AIInternalMoveToState::loadPostProcess(void) { startMoveSound(); }
...
StateReturnType AIInternalMoveToState::update() {
  ...
  // If we're deployed, don't move! But keep the state around until we can
  // packup (edge case). if( obj->testStatus( OBJECT_STATUS_DEPLOYED ) )
  //{
  //	return STATE_CONTINUE;
  // }
  ...
  if (m_waitingForPath) {
    ...
    if (thePath == nullptr) {
      // Kris: 7/01/03 (Temporary debug hook for units not being able to leave
      // maps)
      if (blah) {
        blah = blah;
      }
  ...
  // DEBUG_LOG(("onPathDistToGoal = %f %s",onPathDistToGoal,
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIAttackMoveStateMachine::crc(Xfer *xfer) { StateMachine::crc(xfer); }
...
StateReturnType AIMoveToState::update() {
  ...
  if (goalObj) {
    ...
    Bool gotPhysics =
        obj->getPhysics() != nullptr && goalObj->getPhysics() != nullptr;
    ...
    // %f",m_goalPosition.x,m_goalPosition.y,m_goalPosition.z));
  } else {
    Bool isMissile = obj->isKindOf(KINDOF_PROJECTILE);
    if (isMissile) {
...
Bool AIAttackApproachTargetState::computePath() {
  ...
  if (ai->isBlockedAndStuck()) {
    ...
    // recomputing for object %d", getMachineOwner()->getID()));
  }
  if (m_waitingForPath)
    return true;
  ...
  /// doesn't compute on the same frame (MSB)
  if (!forceRepath &&
    ...
    // of min time for object %d", getMachineOwner()->getID()));
    return true;
  }
  ...
  if (getMachineGoalObject()) {
    ...
    if (!forceRepath && isSamePosition(source->getPosition(), &m_prevVictimPos,
      ...
      CRCDEBUG_LOG(("AIAttackApproachTargetState::computePath - bailing "
                    "because victim in same place for object %d",
    ...
    if (!weapon) {
      CRCDEBUG_LOG(("AIAttackApproachTargetState::computePath - bailing "
                    "because of no weapon for object %d",
                    getMachineOwner()->getID()));
      return false;
    }
    ...
    CRCDEBUG_LOG(("AIAttackApproachTargetState::computePath - "
                  "requestAttackPath() for object %d",
                  getMachineOwner()->getID()));
    ai->requestAttackPath(victim->getID(), &pos);
    m_stopIfInRange =
        false; // we have calculated a position to shoot from, so go there.

    CRCDEBUG_LOG(("AIAttackApproachTargetState::computePath - bailing after "
                  "repathing for object %d",
                  getMachineOwner()->getID()));
    return true;
  } else {
    // goal position.
    ...
    if (!forceRepath) {
      CRCDEBUG_LOG(("AIAttackApproachTargetState::computePath - bailing "
                    "because we're aiming for a fixed position for object %d",
                    getMachineOwner()->getID()));
    ...
    CRCDEBUG_LOG(("AIAttackApproachTargetState::computePath - bailing after "
                  "repathing at a fixed position for object %d",
                  getMachineOwner()->getID()));
    return true;
  }

  CRCDEBUG_LOG(("AIAttackApproachTargetState::computePath - bailing at end of "
                "function for object %d",
                getMachineOwner()->getID()));
  return true;
}
...
StateReturnType AIAttackApproachTargetState::onEnter() {
  ...
  if (victim) {
    ...
    // crawler). Kris (July 2003): If we are retaliating... don't fail out!
    if (ai->getCurrentStateID() != AI_GUARD_RETALIATE) {
      if (source->getControllingPlayer()->getPlayerType() == PLAYER_HUMAN) {
        ...
        // Computer player.  Don't chase aircraft, unless we're hunting. jba
        // [8/27/2003]
        Bool hunt = ai->getCurrentStateID() == AI_HUNT;
        if (!hunt && victim->isKindOf(KINDOF_AIRCRAFT) &&
            victim->isAirborneTarget()) {
          return STATE_FAILURE;
        }
      }
    }
    ...
    // Attacking a position.  For a variety of reasons, we need to destroy any
    // existing path or we spin. jba. [8/25/2003]
    ai->destroyPath();
  }
  ...
  // computePath() for object %d", getMachineOwner()->getID()));
...
StateReturnType AIAttackApproachTargetState::updateInternal() {
  ...
  if (victim) {
    ...
    if (victim->testStatus(OBJECT_STATUS_STEALTHED) &&
        ...
        !victim->testStatus(OBJECT_STATUS_DISGUISED)) {
      // If obj is stealthed, can no longer approach.
      // TheSuperHackers @bugfix Stubbjax 19/11/2025 Except when disarming
      // stealthed mines or traps.
#if RETAIL_COMPATIBLE_CRC
      return STATE_FAILURE;
#else
      const Bool isTargetingMine = weapon &&
                                   weapon->getDamageType() == DAMAGE_DISARM &&
                                   (victim->isKindOf(KINDOF_MINE) ||
                                    victim->isKindOf(KINDOF_BOOBY_TRAP) ||
                                    victim->isKindOf(KINDOF_DEMOTRAP));
    ...
    // computePath() to victim for object %d", getMachineOwner()->getID()));
    if (computePath() == false)
      return STATE_SUCCESS;
    ...
    // find a good spot to shoot from
    // CRCDEBUG_LOG(("AIAttackApproachTargetState::updateInternal() - calling
    // computePath() to position for object %d", getMachineOwner()->getID()));
    if (m_stopIfInRange && weapon &&
        weapon->isWithinAttackRange(source, &m_goalPosition)) {
      Bool viewBlocked = false;
      if (ai->isDoingGroundMovement()) {
        viewBlocked = TheAI->pathfinder()->isAttackViewBlockedByObstacle(
            source, *source->getPosition(), nullptr, m_goalPosition);
      }
      if (!viewBlocked) {
        return STATE_SUCCESS;
      }
...
void AIAttackApproachTargetState::onExit(StateExitType status) {
  ...
  if (ai) {
    ...
    // can have spastic users that click the target repeatedly. This will
...
StateReturnType AIAttackPursueTargetState::onEnter() {
  ...
  if (source->isKindOf(KINDOF_PROJECTILE)) {
    ...
    // object %d (%s)", getMachineOwner()->getID(),
  ...
  if (getMachine()->isGoalObjectDestroyed()) {
    ...
    // destroyed for object %d (%s)", getMachineOwner()->getID(),
    // getMachineOwner()->getTemplate()->getName().str()));
    return STATE_SUCCESS; // Already killed victim.
  }
  if (!m_isAttackingObject) {
  ...
  // (ie we auto-acquired), we don't want to chase the unit. Kris (July 2003):
  // If we are retaliating... don't succeed out!
  if (ai->getCurrentStateID() != AI_GUARD_RETALIATE) {
    if (source->getControllingPlayer()->getPlayerType() == PLAYER_HUMAN) {
      if (ai->getLastCommandSource() == CMD_FROM_AI) {
        return STATE_SUCCESS;
...
StateReturnType AIAttackPursueTargetState::updateInternal() {
  ...
  if (victim) {
    ...
    if (code != STATE_CONTINUE) {
      ...
      // internal update() for object %d (%s)", getMachineOwner()->getID(),
      // getMachineOwner()->getTemplate()->getName().str()));
      return STATE_SUCCESS; // Always return state success, as state failure
                            // exits the attack.
      // we may need to aim & do another approach if the target moved.  jba.
    ...
    if (tur == TURRET_INVALID) {
      ...
      // for object %d (%s)", getMachineOwner()->getID(),
      // getMachineOwner()->getTemplate()->getName().str()));
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIPickUpCrateState::crc(Xfer *xfer) { AIInternalMoveToState::crc(xfer); }
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIFollowPathState::crc(Xfer *xfer) { AIInternalMoveToState::crc(xfer); }
...
StateReturnType AIAttackMoveToState::update() {
  ...
  if (m_attackMoveMachine->isInIdleState()) {
    ...
    if (nextObjectToAttack != nullptr) {
      ...
      shouldRepathThisFrame = false; // we're about to drop out of this
                                     // function, but this is semantic emphasis.
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIMoveAndDeleteState::crc(Xfer *xfer) { AIInternalMoveToState::crc(xfer); }
...
#define NO_ROTATE_OFFSETS
  ...
  if (extent.isInRegionNoZ(&dest)) {
    // The waypoint is on the map.  Check & see if the adjusted position is off
    // map [8/28/2003]
  ...
  if (m_moveAsGroup &&
    ...
    if (getAdjustsDestination() && ai->isDoingGroundMovement()) {
      if (!TheAI->pathfinder()->adjustDestination(obj, ai->getLocomotorSet(),
                                                  &m_goalPosition)) {
        if (m_currentWaypoint) {
// TheSuperHackers @info helmutbuhler 05/05/2025 This debug mutates the code to
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIWanderState::crc(Xfer *xfer) { AIFollowWaypointPathState::crc(xfer); }
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIWanderInPlaceState::crc(Xfer *xfer) { AIInternalMoveToState::crc(xfer); }
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIPanicState::crc(Xfer *xfer) { AIFollowWaypointPathState::crc(xfer); }
...
StateReturnType AIAttackAimAtTargetState::onEnter() {
  ...
  ContainModuleInterface *contain =
      containedBy ? containedBy->getContain() : nullptr;

  if (containedBy && weapon && contain &&
      contain->isEnclosingContainerFor(
          source)) { // non enclosing garrison containers do not use firepoints.
                     // Lorenzen, 6/11/03
    if (victim) {
      inFiringRange =
  ...
  if (sourceAI->areTurretsLinked()) {
    ...
    for (Int i = 0; i < MAX_TURRETS; i++) {
      if (m_isAttackingObject) {
        sourceAI->setTurretTargetObject((WhichTurretType)i, victim,
                                        m_isForceAttacking);
      } else {
        sourceAI->setTurretTargetPosition((WhichTurretType)i,
    ...
    if (tur != TURRET_INVALID) {
      ...
      // GS moved contact weapon check in here, because Success can never be
      // given to a unit in this state using a turret to attack.  Check out
      // ::update and you will see.
...
StateReturnType AIAttackAimAtTargetState::update() {
  ...
  if (source->isDisabledByType(DISABLED_HELD)) {
    ...
    // NO BAD WRONG!!! How can this be the one spot to convert an Object to a
    // center pos?  We have an attack object check for a reason!  The center and
    // the edge can be very far apart.
    //		const Coord3D *pos = m_isAttackingObject ? victim->getPosition()
    //: getMachineGoalPosition();
    ...
    if (m_isAttackingObject)
      ...
      inRange =
          weapon ? weapon->isWithinAttackRange(source, getMachineGoalPosition())
                 : FALSE;
...
void AIAttackAimAtTargetState::onExit(StateExitType status) {
  ...
  getMachineOwner()->clearStatus(
      MAKE_OBJECT_STATUS_MASK(OBJECT_STATUS_IS_AIMING_WEAPON));
...
StateReturnType AIAttackFireWeaponState::update() {
  ...
  */
  ...
  if (m_att->isAttackingObject()) {
    // Since it is very late in the project, and there is no call for such
    // code... there is currently no support here for linked turrets, as regards
    // Attacking Objects (victims) If the concept of linked turrets is further
    // developed then God help you, and put more code right here that lookl like
    // the //LINKED TURRETS// block, below

    obj->fireCurrentWeapon(victim);

    // Kris: October 21, 2003 - Patch 1.01
    // Fixes cases where some units couldn't transfer their attack to a
    // different object. One example was Colonel Burton attacking any GLA
    // structure. When the structure was destroyed becoming a hole, Burton would
    // stop attacking. Even though there is code to transfer attackers
    // (AIUpdateInterface::transferAttack), it is unable to modify our current
    // victim in our attack state machine. When we move immediately to the aim
    // state in the same frame as the transfer (after this call in fact), the
    // victim was still pointing to the building and not the hole we transferred
    // to. This code fixes that.
    ...
    if (continueRange > 0.0f && victim &&
      ...
      const Coord3D *originalVictimPos =
          m_att ? m_att->getOriginalVictimPos() : nullptr;
    ...
    { // it doesn;t matter which weapon slot is locked, current or whatever
      for (Int slot = PRIMARY_WEAPON; slot < WEAPONSLOT_COUNT;
           slot++) { // were firing with all barrels
        Weapon *weapon = obj->getWeaponInWeaponSlot((WeaponSlotType)slot);
        if (weapon) {
          if (weapon->fireWeapon(
                  obj, getMachineGoalPosition())) // fire() returns 'reloaded'
            obj->releaseWeaponLock(
                LOCKED_TEMPORARILY); // unlock, 'cause we're loaded
...
void AIAttackFireWeaponState::onExit(StateExitType status) {
  // contained by AIAttackState, so no separate timer
  Object *obj = getMachineOwner();
  obj->clearStatus(MAKE_OBJECT_STATUS_MASK2(OBJECT_STATUS_IS_FIRING_WEAPON,
...
AIAttackState::AIAttackState(StateMachine *machine, Bool follow,
    ...
    : State(machine, "AIAttackState"), m_attackMachine(nullptr),
      m_attackParameters(attackParameters), m_lockedWeaponOnEnter(nullptr),
      m_follow(follow), m_isAttackingObject(attackingObject),
      m_isForceAttacking(forceAttacking), m_victimTeam(nullptr) {
  m_originalVictimPos.zero();
#ifdef STATE_MACHINE_DEBUG
  if (machine->getWantsDebugOutput()) {
    DEBUG_LOG(("Creating attack state follow %d, attacking object %d, force "
               "attacking %d",
...
void AIAttackState::loadPostProcess(void) {
  ...
  m_lockedWeaponOnEnter =
      source->isCurWeaponLocked() ? source->getCurrentWeapon() : nullptr;
}
...
Bool AIAttackState::chooseWeapon() {
  ...
  Bool found = FALSE;
  //	if (victim) // Pardon?  We still need to pick a weapon if we are
  ...
  // %s",source->getTemplate()->getName().str()));
  //	}
...
StateReturnType AIAttackState::onEnter() {
  ...
  // Kris: Jan 12, 2005
  // Don't allow units under construction to attack! The selection/action
  // manager system was responsible for preventing this from ever happening, but
  // failed in two cases which I fixed. This is an extra check to mitigate
  // cheats.
  ...
  // object %d", getMachineOwner()->getID()));
  m_attackMachine =
  ...
  // Something can happen to make none of our weapons work.  Return failure, or
  // we will start shooting our Primary (default pick) regardless of legality.
  ...
  if (curWeapon) {
    ...
    if (curWeapon->getContinueAttackRange() > 0.0f)
      source->setStatus(
          MAKE_OBJECT_STATUS_MASK(OBJECT_STATUS_IGNORING_STEALTH));
  }
...
void AIAttackState::onExit(StateExitType status) {
  ...
  Object *obj = getMachineOwner();
  obj->clearStatus(MAKE_OBJECT_STATUS_MASK4(
      OBJECT_STATUS_IS_FIRING_WEAPON, OBJECT_STATUS_IS_AIMING_WEAPON,
      OBJECT_STATUS_IS_ATTACKING, OBJECT_STATUS_IGNORING_STEALTH));
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIAttackThenIdleStateMachine::crc(Xfer *xfer) { StateMachine::crc(xfer); }
...
Object *AIAttackSquadState::chooseVictim(void) {
  ...
  case DIFFICULTY_NORMAL: {
    ...
    Object *victim = ThePartitionManager->getClosestObject(
        getMachineOwner(), HUGE_DIST, FROM_CENTER_2D, filters, nullptr,
        nullptr);
    return victim;
    break;
  }
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void AIEnterState::crc(Xfer *xfer) { AIInternalMoveToState::crc(xfer); }
...
static bool hasVerticalOverlap(const Object *a, const Object *b) {
  ...
  const float aUpper =
      aLower + a->getGeometryInfo().getMaxHeightAbovePosition();
  const float bLower = b->getPosition()->z;
  const float bUpper =
      bLower + b->getGeometryInfo().getMaxHeightAbovePosition();
...
StateReturnType AIEnterState::update() {
  ...
  if (goal) {
    ...
    if (goal->getContainedBy() != nullptr && goal->isAboveTerrain() &&
        !obj->isAboveTerrain()) {
      return STATE_FAILURE;
    }
...
#else
  // TheSuperHackers @bugfix Stubbjax 05/11/2025 Check for vertical overlap when
  // entering containers. This prevents levitating or airborne units from
  // entering containers they are not actually touching.
...
StateReturnType AIExitState::update() {
  ...
  if (goal) {
    ...
    //  as removeFromContain is only in the Contain type.  I'm splitting the
    //  names in shame.
    ...
    if (getMachine()->getCurrentStateID() != getID())
      return STATE_CONTINUE; // Not success, because exitViaDoor has changed us
...
StateReturnType AIExitInstantlyState::onEnter() {
  ...
  if (goal) {
    ...
    return STATE_CONTINUE; // Not success, because exitViaDoor has changed us to
                           // FollowPath, and if we say Success, our machine
                           // will think FollowPath succeeded
  } else {
    return STATE_FAILURE;
  }
}
...
StateReturnType AIGuardState::update() {
  ...
  // %08lx",TheGameLogic->getFrame(),getMachineOwner()));
  ...
  // (this can happen for units which never auto-reload, like the Raptor)
  Object *owner = getMachineOwner();
  if (owner->getAI()->getJetAIUpdate() && owner->isOutOfAmmo() &&
      !owner->isKindOf(KINDOF_PROJECTILE) &&
      !owner->getTemplate()->isEnterGuard()) {
    DEBUG_CRASH(("Hmm, this should probably never happen, since this case "
                 "should be intercepted by JetAIUpdate"));
    return STATE_FAILURE;
  }
...
void AIGuardRetaliateState::xfer(Xfer *xfer) {
  ...
  if (hasMachine && m_guardRetaliateMachine == nullptr) {
    ...
    m_guardRetaliateMachine =
        newInstance(AIGuardRetaliateMachine)(getMachineOwner());
  }
  if (hasMachine) {
...
StateReturnType AIGuardRetaliateState::update() {
  // DEBUG_LOG(("AIGuardRetaliateState frame %d:
  // %08lx",TheGameLogic->getFrame(),getMachineOwner()));
...
StateReturnType AIHuntState::update() {
  ...
  getMachine()->lock(
      "AIHuntState::update"); // The idle state in the sub machine can sometimes
                              // acquire targets. It is important to not switch
                              // out of this state via a sub machine call. jba.
  /*
          Note the use of CONVERT_SLEEP_TO_CONTINUE; even if the sub-machine
          sleeps, we still need to be called every frame.
  */
...
StateReturnType AIAttackAreaState::update() {
  ...
  getMachine()->lock(
      "AIAttackAreaState::update"); // The idle state in the sub machine can
                                    ...
                                    // important to not switch out of this state
                                    // via a sub machine call. jba.
  /*
          Note the use of CONVERT_SLEEP_TO_CONTINUE; even if the sub-machine
          sleeps, we still need to be called every frame.
  */
