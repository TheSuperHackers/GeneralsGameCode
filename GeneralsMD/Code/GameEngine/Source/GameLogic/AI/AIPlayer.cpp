    ...
    for (BuildListInfo *info = m_player->getBuildList(); info; info = info->getNext())
    {
        ...
        if (curGatherers >= desiredGatherers)
        {
            ...
            if (supplyCenter)
            {
                ...
                Object *supplySource =
                    ThePartitionManager->getClosestObject(&center, radius, FROM_BOUNDINGSPHERE_2D, filters);
                if (!supplySource)
                {
                    // No supplies.
                    continue;
                ...
                if (dx * dx + dy * dy < radSqr)
                {
                    ...
                    if (pObj->isKindOf(KINDOF_COMMANDCENTER))
                    {
                        if (!includeMilitaryUnits)
                            ...
                            value = value / 10; // Command centers cannot be killed by any superweapon, so we don't want to
                                                // target them as highly. jba.
                    }
                    if (pObj->isKindOf(KINDOF_FS_SUPERWEAPON))
                    {
                        if (!includeMilitaryUnits)
                            ...
                            value = value / 10; // Superweapons cannot be killed by any superweapon, so we don't want to
                                                // target them as highly. jba.
                    }
                    if (applyNegValue)
                    {
        ...
        if (orders)
        {
            ...
            if (team->m_team->getPrototype()->getTemplateInfo()->m_executeActions)
            {
                const Script *script = TheScriptEngine->findScriptByName(
                    team->m_team->getPrototype()->getTemplateInfo()->m_productionCondition);
    ...
    if (teamProto)
    {
        ...
        for (i = 0; i < teamProto->getTemplateInfo()->m_numUnitsInfo; i++)
        {
            ...
            if (thing)
            {
                ...
                {
                    Object *unit =
                        theTeam->tryToRecruit(thing, &teamProto->getTemplateInfo()->m_homeLocation, recruitRadius);
                    if (unit)
                    {
                        unitsRecruited++;
...
void AIPlayer::xfer(Xfer *xfer)
{
    ...
    else
    {
        ...
        {
            DEBUG_CRASH(
                ("AIPlayer::xfer - TeamBuildQueue head is not NULL, you should delete it or something before loading a new "
                 "list"));
    ...
    else
    {
        ...
        {
            DEBUG_CRASH(
                ("AIPlayer::xfer - TeamReadyQueue head is not NULL, you should delete it or something before loading a new "
                 "list"));
            throw SC_INVALID_DATA;
...
void TeamInQueue::xfer(Xfer *xfer)
{
    ...
    else
    {
        ...
        {
            DEBUG_CRASH(
                ("TeamInQueue::xfer - m_workOrders should be NULL but isn't.  Perhaps you should blow it away before "
                 "loading"));
            throw SC_INVALID_DATA;
