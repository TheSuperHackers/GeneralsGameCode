...
void Pathfinder::classifyFence(Object *obj, Bool insert)
{
	...
	const Real STEP_SIZE = PATHFIND_CELL_SIZE_F * 0.5f; // in theory, should be PATHFIND_CELL_SIZE_F exactly, but needs to be
																											// smaller to avoid aliasing problems
...
void Pathfinder::internal_classifyObjectFootprint(Object *obj, Bool insert)
{
	...
	switch (obj->getGeometryInfo().getGeomType())
	{
		case GEOMETRY_BOX:
		{
			...
			const Real STEP_SIZE = PATHFIND_CELL_SIZE_F * 0.5f; // in theory, should be PATHFIND_CELL_SIZE_F exactly, but needs to
						...
						centerInCell))
		{
			...
			if (!viewBlocked)
			{
				...
				if (obj->isKindOf(KINDOF_VEHICLE))
				{
					...
					Int cellLimit = 12; // Magic number, yes I know - jba.   It is about 4 * size of an average vehicle width (3 cells)
															// [8/15/2003]
					while (cur)
					{
						cellLimit--;
