...
void Path::optimize(const Object *obj, LocomotorSurfaceTypeMask acceptableSurfaces, Bool blocked)
{
    ...
    while (anchor != getLastNode())
    {
        ...
        const Int ALLOWED_STEPS =
            3; // we can optimize 3 steps to or from a bridge.  Otherwise, we need to insert a point. jba.
    ...
    ClosestPointOnPathInfo &out)
{
    ...
    if (closeNode && closeNode->getNextOptimized())
    {
        ...
        if (TheAI->pathfinder()
                ->isLinePassable(obj, locomotorSet.getValidSurfaces(), out.layer, pos, *nextNodePos, false, true))
        {
        ...
        for (j = info.cell.y - info.radius; j < info.cell.y + numCellsAbove; j++)
        {
            ...
            if (cell)
            {
                ...
                else
                {
                    ...
                    {
                        if (obj->getAIUpdateInterface()
                            && obj->getAIUpdateInterface()->getIgnoredObstacleID() == unit->getID())
                        {
...
#pragma message("AHHHH!, forced calls to pathzonerefresh still in code...  notify M Lorenzen")
    ...
    if (pathsFound > 0)
    {
        ...
        {
            DEBUG_LOG(
                ("%d Pathfind queue: %d paths, %d cells --",
    ...
    while (m_openList != NULL)
    {
        ...
        for (int i = 0; i < numNeighbors; i++)
        {
            ...
            if (newCell != goalCell)
            {
                ...
                // See how wide the cell is.
                clearDiameter =
                    clearCellForDiameter(crusher, newCellCoord.x, newCellCoord.y, newCell->getLayer(), pathDiameter);
    ...
    Bool closestOK)
{
    ...
    while (m_openList != NULL)
    {
        ...
        if (parentCell->getLayer() == LAYER_GROUND)
        {
            parentZone =
                m_zoneManager
                    .getBlockZone(locomotorSurface, crusher, parentCell->getXIndex(), parentCell->getYIndex(), m_map);
        }
        else
        {
    ...
    while (m_openList != NULL)
    {
                ...
                centerInCell))
        {
            if (!startedStuck || validMovementPosition(isCrusher, locomotorSet.getValidSurfaces(), parentCell))
            {
                ...
                distSqr += (parentCell->getCostSoFar() * (parentCell->getCostSoFar() * COST_TO_DISTANCE_FACTOR_SQR))
                    * pathCostMultiplier;
                if (distSqr < closestDistanceSqr)
                {
        ...
        if (count > 5000)
        {
            ...
            DEBUG_LOG(
                ("%d Pathfind(findClosestPath) chugged from (%f,%f) to (%f,%f) --",
                 ...
                 to->y));
            DEBUG_LOG(
                ("Unit '%s', time %f", obj->getTemplate()->getName().str(), (::GetTickCount() - startTimeMS) / 1000.0f));
#ifdef INTENSE_DEBUG
