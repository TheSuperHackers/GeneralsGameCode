...
void AIGroup::groupSell( CommandSourceType cmdSource )
{
	...
	groupObjectsCopy.resize(m_memberListSize);

	// TheSuperHackers @bugfix Mauller 26/06/2025 when sellObject is called, the objects in this AiGroup get removed from it. This happens within the Object::deselectObject() function.
	// This invalidates the local AIGroup object from under the call to groupSell, therefore we need to make a local copy of the objects that need selling.
	for( i = m_memberList.begin(); i != m_memberList.end(); /*empty*/ )
	{
