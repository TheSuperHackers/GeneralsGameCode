...
AIGuardRetaliateMachine::AIGuardRetaliateMachine(Object *owner) :
		StateMachine(owner, "AIGuardRetaliateMachine"), m_nemesisToAttack(INVALID_ID)
{
	...
	defineState(
			AI_GUARD_RETALIATE_ATTACK_AGGRESSOR,
			newInstance(AIGuardRetaliateAttackAggressorState)(this),
			AI_GUARD_RETALIATE_RETURN,
			AI_GUARD_RETALIATE_RETURN);
	defineState(
			AI_GUARD_RETALIATE_RETURN,
			newInstance(AIGuardRetaliateReturnState)(this),
			AI_GUARD_RETALIATE_IDLE,
			AI_GUARD_RETALIATE_INNER,
			attackAggressors);
	defineState(
			AI_GUARD_RETALIATE_IDLE,
			newInstance(AIGuardRetaliateIdleState)(this),
			AI_GUARD_RETALIATE_INNER,
			EXIT_MACHINE_WITH_SUCCESS,
			attackAggressors);
	defineState(
			AI_GUARD_RETALIATE_INNER,
			newInstance(AIGuardRetaliateInnerState)(this),
			AI_GUARD_RETALIATE_OUTER,
			AI_GUARD_RETALIATE_OUTER);
	defineState(
			AI_GUARD_RETALIATE_OUTER,
			newInstance(AIGuardRetaliateOuterState)(this),
			AI_GUARD_RETALIATE_GET_CRATE,
			AI_GUARD_RETALIATE_GET_CRATE);
	defineState(
			AI_GUARD_RETALIATE_GET_CRATE,
			newInstance(AIGuardRetaliatePickUpCrateState)(this),
			AI_GUARD_RETALIATE_RETURN,
			AI_GUARD_RETALIATE_RETURN);
}
...
StateReturnType AIGuardRetaliateInnerState::onEnter(void)
{
	...
	else
	{
		...
		m_exitConditions.m_conditionsToConsider =
				(GuardRetaliateExitConditions::ATTACK_ExitIfOutsideRadius | GuardRetaliateExitConditions::ATTACK_ExitIfNoUnitFound);
...
StateReturnType AIGuardRetaliateOuterState::onEnter(void)
{
	...
	m_exitConditions.m_conditionsToConsider =
			(GuardRetaliateExitConditions::ATTACK_ExitIfExpiredDuration | GuardRetaliateExitConditions::ATTACK_ExitIfOutsideRadius
...
StateReturnType AIGuardRetaliateAttackAggressorState::onEnter(void)
{
	...
	if (!nemesis && body && body->getLastDamageInfo()->in.m_sourceID
			&& body->getLastDamageInfo()->in.m_damageType != DAMAGE_HEALING)
	{
