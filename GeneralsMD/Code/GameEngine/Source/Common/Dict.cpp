...
Dict::DictPair *Dict::ensureUnique(int numPairsNeeded, Bool preserveData, DictPair *pairToTranslate)
{
	...
	if (numPairsNeeded > 0)
	{
		...
		DEBUG_ASSERTCRASH(
				numPairsNeeded <= MAX_LEN,
				("Dict::ensureUnique exceeds max pairs length %d with requested length %d", MAX_LEN, numPairsNeeded));
...
Dict::Dict(Int numPairsToPreAllocate) : m_data(0)
{
	...
	*/
	DEBUG_ASSERTCRASH(
			sizeof(Bool) <= sizeof(void *) && sizeof(Int) <= sizeof(void *) && sizeof(Real) <= sizeof(void *)
					&& sizeof(AsciiString) <= sizeof(void *) && sizeof(UnicodeString) <= sizeof(void *),
