...
Bool ActionManager::canGetRepairedAt(const Object *obj, const Object *repairDest, CommandSourceType commandSource)
{
    ...
    if (obj->isKindOf(KINDOF_AIRCRAFT))
    {
        ...
        if (!obj->isAboveTerrain() || repairDest->isKindOf(KINDOF_FS_AIRFIELD) == FALSE)
            return FALSE;
    }
    else
    {
    ...
    CanEnterType mode)
{
    ...
    if (obj->isKindOf(KINDOF_AIRCRAFT) && objectToEnter->isKindOf(KINDOF_FS_AIRFIELD))
    {
        if (obj->getStatusBits().test(OBJECT_STATUS_DECK_HEIGHT_OFFSET)
            && obj->getCarrierDeckHeight() >= obj->getPosition()->z)
        {
            return FALSE;
        }
    ...
    AbleToAttackType attackType)
{
    ...
    if (obj->isKindOf(KINDOF_SPAWNS_ARE_THE_WEAPONS))
    {
        ...
        else if (result == ATTACKRESULT_NOT_POSSIBLE) // oh dear me. The wierd case of a garrisoncontainer being a
...
Bool ActionManager::canCaptureBuilding(const Object *obj, const Object *objectToCapture, CommandSourceType commandSource)
{
    ...
    //); 		if ( cashSPI && cashSPI->isBusy() ) 			return FALSE;
    //	}
    ...
    Bool checkSourceRequirements)
{
    ...
    //); 	if ( captureSPI && captureSPI->isBusy() ) 		return FALSE; 	SpecialPowerModuleInterface *cashSPI =
    //obj->findSpecialPowerModuleInterface( SPECIAL_BLACKLOTUS_STEAL_CASH_HACK ); 	if ( cashSPI && cashSPI->isBusy() ) 		return
    //FALSE;
...
Bool ActionManager::canStealCashViaHacking(const Object *obj, const Object *objectToHack, CommandSourceType commandSource)
{
    ...
    //  or until 12/10/02
    //	SpecialPowerModuleInterface *captureSPI = obj->findSpecialPowerModuleInterface( SPECIAL_BLACKLOTUS_CAPTURE_BUILDING
    //); 	if ( captureSPI && captureSPI->isBusy() ) 		return FALSE; 	SpecialPowerModuleInterface *disableSPI =
    //obj->findSpecialPowerModuleInterface( SPECIAL_BLACKLOTUS_DISABLE_VEHICLE_HACK ); 	if ( disableSPI &&
        ...
        switch (spTemplate->getSpecialPowerType())
        {
            ...
            case SPECIAL_TANKHUNTER_TNT_ATTACK:
                if (target->isKindOf(KINDOF_STRUCTURE)
                    || (target->isKindOf(KINDOF_VEHICLE) && !target->isKindOf(KINDOF_AIRCRAFT)))
                {
                    return true;
                }
                break;
                ...
                if (target->isKindOf(KINDOF_STRUCTURE) || target->isKindOf(KINDOF_VEHICLE))
                {
                    ...
                    if (spUpdate)
                    {
                        ...
                        if (spUpdate->getSpecialObjectCount() < spUpdate->getSpecialObjectMax())
                        {
                            ...
                            // We also don't want to allow a unit that can place timed charges on a building to be able to
                            // place remote charges (or vice-versa). So we're going to look for the other special ability
                            // update and reject if the other one has it planted...
