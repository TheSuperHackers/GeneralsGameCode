...
const char *TheAcademyClassificationTypeNames[] = { "ACT_NONE", "ACT_UPGRADE_RADAR", "ACT_SUPERPOWER", NULL };
...
void AcademyStats::init(const Player *player)
{
	...
	if (!plyrTemplate
			|| (plyrTemplate->getBaseSide().compareNoCase("USA") && plyrTemplate->getBaseSide().compareNoCase("China")
...
void AcademyStats::recordProduction(const Object *obj, const Object *constructer)
{
	...
	// 18) Built secondary income building?
	if (obj->isKindOf(KINDOF_MONEY_HACKER) || obj->isKindOf(KINDOF_FS_BLACK_MARKET)
