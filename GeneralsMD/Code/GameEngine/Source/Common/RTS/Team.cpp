...
void TeamFactory::addTeamPrototypeToList(TeamPrototype *team)
{
	...
	if (it != m_prototypes.end())
	{
		DEBUG_ASSERTCRASH(
				(*it).second == team,
				("TeamFactory::addTeamPrototypeToList: Team %s already exists... skipping.", team->getName().str()));
...
void TeamFactory::xfer(Xfer *xfer)
{
	...
	{
		DEBUG_CRASH(("TeamFactory::xfer - Prototype count mismatch '%d should be '%d'", prototypeCount, m_prototypes.size()));
	...
	prototype->getControllingPlayer()->getPlayerIndex() ); for( DLINK_ITERATOR<Team> teamIt =
	prototype->iterate_TeamInstanceList(); !teamIt.done(); teamIt.advance() )
		{
			team = teamIt.cur();
	...
	if (!m_retrievedGenericScripts)
	{
		...
		for (Int i = 0; i < MAX_GENERIC_SCRIPTS; ++i)
		{
			...
			if (!m_teamTemplate.m_teamGenericScripts[i].isEmpty())
			{
				...
				{
					DEBUG_CRASH(
							("We attempted to find a generic script, but couldn't. ('%s')", m_teamTemplate.m_teamGenericScripts[i].str()));
				}
			}
...
Team::~Team()
{
	...
	// DEBUG_ASSERTCRASH(!(getControllingPlayer() && getControllingPlayer()->getDefaultTeam()==this),("I am still someones
...
Object *Team::getTeamTargetObject(void)
{
	...
	if (target)
	{
		...
		if (target->testStatus(OBJECT_STATUS_STEALTHED) && !target->testStatus(OBJECT_STATUS_DETECTED)
...
// ------------------------------------------------------------------------
void Team::countObjectsByThingTemplate(
		Int numTmplates,
		const ThingTemplate *const *things,
		Bool ignoreDead,
		Int *counts,
...
void Team::killTeam(void)
{
	...
	// TheSuperHackers @bugfix Mauller 20/07/2025 the neutral player has no player template so we need to check for a null
	// template
	...
	const ThingTemplate *beaconTemplate =
			playerTemplate ? TheThingFactory->findTemplate(playerTemplate->getBeaconTemplate()) : NULL;
...
void Team::xfer(Xfer *xfer)
{
	...
	if (teamID != m_id)
	{
		DEBUG_CRASH(("Team::xfer - TeamID mismatch.  Xfered '%d' but should be '%d'", teamID, m_id));
		throw SC_INVALID_DATA;
	...
	for (DLINK_ITERATOR<Object> objIt = iterate_TeamMemberList(); objIt.done() == FALSE; objIt.advance())
		memberCount++;
