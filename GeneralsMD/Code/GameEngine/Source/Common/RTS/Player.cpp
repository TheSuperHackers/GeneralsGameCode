  ...
  if (yes)
  {
    ...
    if (ThePartitionManager)
    {
      ...
      for (Object *object = iter->first(); object; object = iter->next())
      {
        ...
        {
          // KM -- August 2002:
          // Added support for disguised objects, based on relationships, we either show the real color or the
          // disguised color.
...
Bool Player::addSkillPoints(Int delta)
{
  ...
  Int pointCap =
      TheRankInfoStore->getRankInfo(levelCap)->m_skillPointsNeeded; // Cap at lowest point of cap level, not highest.
...
void Player::removeKindOfProductionCostChange(KindOfMaskType kindOf, Real percent)
{
  ...
  DEBUG_ASSERTCRASH(
      FALSE,
      ("removeKindOfProductionCostChange was called with kindOf=%d and percent=%f. We could not find the entry in the "
       "list with these variables. CLH.",
       kindOf,
       percent));
}
