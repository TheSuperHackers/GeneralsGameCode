...
void dumpBattlePlanBonuses(
		const BattlePlanBonuses *b,
		AsciiString name,
		const Player *p,
		const Object *o,
		AsciiString fname,
...
{
	CRCDEBUG_LOG(
			("dumpBattlePlanBonuses() %s:%d %s\n  Player %d(%ls) object %d(%s) armor:%g/%8.8X bombardment:%d, holdTheLine:%d, "
			 "searchAndDestroy:%d sight:%g/%8.8X, valid:%s invalid:%s",
			 fname.str(),
			 line,
			 name.str(),
			 (p) ? p->getPlayerIndex() : -1,
			 ...
			 b->m_bombardment,
			 b->m_holdTheLine,
			 b->m_searchAndDestroy,
			 b->m_sightRangeScalar,
...
void Player::initFromDict(const Dict *d)
{
	...
	DEBUG_ASSERTCRASH(
			pt != NULL,
			("PlayerTemplate %s not found -- this is an obsolete map (please open and resave in WB)", tmplname.str()));
	...
	if (d->getBool(TheKey_playerIsSkirmish, &exists))
	{
		...
		DEBUG_ASSERTCRASH(
				skirmish,
				("Could not find skirmish player for side %s... quietly making into nonskirmish.", getSide().str()));
		if (!skirmish)
			forceHuman = true;
	}
			...
			for (i = 0; i < TheSidesList->getNumSkirmishSides(); i++)
			{
				...
				if (pt && pt->getSide() == mySide)
				{
					qualTemplatePlayerName.format(
							"%s%d",
							TheSidesList->getSkirmishSideInfo(i)->getDict()->getAsciiString(TheKey_playerName).str(),
							m_mpStartIndex);
					found = true;
					break;
				}
	...
	if (skirmish)
	{
		...
		for (skirmishNdx = 0; skirmishNdx < TheSidesList->getNumSkirmishSides(); skirmishNdx++)
		{
			AsciiString templateName =
					TheSidesList->getSkirmishSideInfo(skirmishNdx)->getDict()->getAsciiString(TheKey_playerFaction);
			pt = ThePlayerTemplateStore->findPlayerTemplate(NAMEKEY(templateName));
			if (pt && pt->getSide() == mySide)
			{
				qualTemplatePlayerName.format(
		...
		else
		{
			...
			AsciiString qualifier;
			qualifier.format("%d", m_mpStartIndex);
			ScriptList *scripts = TheSidesList->getSkirmishSideInfo(skirmishNdx)
																->getScriptList()
			...
			AsciiString originalPlayerName =
					TheSidesList->getSkirmishSideInfo(skirmishNdx)->getDict()->getAsciiString(TheKey_playerName);
			for (i = 0; i < TheSidesList->getNumSkirmishTeams(); i++)
			{
				if (TheSidesList->getSkirmishTeamInfo(i)->getDict()->getAsciiString(TheKey_teamOwner) == originalPlayerName)
				{
					...
					NameKeyType nameKeys[] = { TheKey_teamOnCreateScript,				 TheKey_teamOnIdleScript,
	...
	if (yes)
	{
		...
		if (ThePartitionManager)
		{
			...
			for (Object *object = iter->first(); object; object = iter->next())
			{
				...
				{
					// KM -- August 2002:
					// Added support for disguised objects, based on relationships, we either show the real color or the disguised
					// color.
...
void Player::onStructureConstructionComplete(Object *builder, Object *structure, Bool isRebuild)
{
	...
	if (structure->hasSpecialPower(SPECIAL_NEUTRON_MISSILE) || structure->hasSpecialPower(NUKE_SPECIAL_NEUTRON_MISSILE)
...
{
	for (PlayerTeamList::const_iterator it = m_playerTeamPrototypes.begin(); it != m_playerTeamPrototypes.end(); ++it)
	{
		if (team == *it)
...
//=============================================================================
void Player::countObjectsByThingTemplate(
		Int numTmplates,
...
void Player::transferAssetsFromThat(Player *that)
{
	...
	for (PlayerTeamList::iterator it = that->m_playerTeamPrototypes.begin(); it != that->m_playerTeamPrototypes.end(); ++it)
	{
		for (DLINK_ITERATOR<Team> iter = (*it)->iterate_TeamInstanceList(); !iter.done(); iter.advance())
		{
			Team *team = iter.cur();
...
void Player::garrisonAllUnits(CommandSourceType source)
{
	...
	ObjectIterator *iterBuilding =
			ThePartitionManager->iterateObjectsInRange(&pos, 1e9f, FROM_CENTER_3D, filters, ITER_SORTED_NEAR_TO_FAR);
...
Bool Player::addSkillPoints(Int delta)
{
	...
	Int pointCap = TheRankInfoStore->getRankInfo(levelCap)->m_skillPointsNeeded; // Cap at lowest point of cap level, not
																																							 // highest.
...
static void countExisting(Object *obj, void *userData)
{
	...
	if ((typeCountData->type && typeCountData->type->isEquivalentTo(obj->getTemplate()))
			|| (typeCountData->linkKey != NAMEKEY_INVALID && obj->getTemplate() != NULL
					&& typeCountData->linkKey == obj->getTemplate()->getMaxSimultaneousLinkKey()))
	{
		typeCountData->count++;
	}
...
{
	return (
			!TheVictoryConditions->hasSinglePlayerBeenDefeated(this) && !m_isPlayerDead && !TheInGameUI->isClientQuiet()
...
void Player::removeRadar(Bool disableProof)
{
	...
	DEBUG_ASSERTCRASH(
			m_radarCount > 0,
			("removeRadar: An Object is taking its radar away, but the player radar count says they don't have radar!"));
	--m_radarCount;
...
void Player::friend_applyDifficultyBonusesForObject(Object *obj, Bool apply) const
{
	if (TheGameLogic->isInSinglePlayerGame())
	{
		...
		static const WeaponBonusConditionType wbonus[PLAYERTYPE_COUNT][DIFFICULTY_COUNT] = {
			{ WEAPONBONUSCONDITION_SOLO_HUMAN_EASY, WEAPONBONUSCONDITION_SOLO_HUMAN_NORMAL, WEAPONBONUSCONDITION_SOLO_HUMAN_HARD },
...
static void localApplyBattlePlanBonusesToObject(Object *obj, void *userData)
{
	...
	if (isProjectile)
	{
		...
		DEBUG_LOG(
				("Object is a projectile - looking at object %d (%s) instead",
				 (objectToValidate) ? objectToValidate->getID() : INVALID_ID,
				 (objectToValidate) ? objectToValidate->getTemplate()->getName().str() : "<No Object>"));
	}
		...
		if (!objectToValidate->isAnyKindOf(bonus->m_invalidKindOf))
		{
			...
			if (!isProjectile)
			{
				...
				if (bonus->m_armorScalar != 1.0f)
				{
					...
					CRCDEBUG_LOG(
							("Applying armor scalar of %g (%8.8X) to object %d (%ls) owned by player %d",
							 bonus->m_armorScalar,
							 AS_INT(bonus->m_armorScalar),
...
void Player::removeKindOfProductionCostChange(KindOfMaskType kindOf, Real percent)
{
	...
	DEBUG_ASSERTCRASH(
			FALSE,
			("removeKindOfProductionCostChange was called with kindOf=%d and percent=%f. We could not find the entry in the list "
			 "with these variables. CLH.",
			 kindOf,
			 percent));
}
...
void Player::xfer(Xfer *xfer)
{
	...
	if ((tunnelTrackerPresent == TRUE && m_tunnelSystem == NULL) || (tunnelTrackerPresent == FALSE && m_tunnelSystem != NULL))
	{
	...
	if (xfer->getXferMode() == XFER_SAVE)
	{
		...
		// save each item
		for (it = m_kindOfPercentProductionChangeList.begin(); it != m_kindOfPercentProductionChangeList.end(); ++it)
		{
			// get entry data
			entry = *it;
