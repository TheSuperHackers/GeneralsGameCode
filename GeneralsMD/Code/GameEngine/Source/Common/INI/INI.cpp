        ...
        while (m_endOfFile == FALSE)
        {
            ...
            if (token)
            {
                ...
                else
                {
                    DEBUG_ASSERTCRASH(
                        0,
                        ("[LINE: %d - FILE: '%s'] Unknown block '%s'", getLineNum(), getFilename().str(), token));
    ...
    else
    {
        ...
        while (p != m_buffer + INI_MAX_CHARS_PER_LINE)
        {
            ...
            *p = m_readBuffer[m_readBufferNext++];
            ...
            DEBUG_ASSERTCRASH(
                *p != '\t',
                ("tab characters are not allowed in INI files (%s). please check your editor settings. Line Number %d",
                 m_filename.str(),
                 getLineNum()));
        ...
        *p = 0;
        ...
        if (p == m_buffer + INI_MAX_CHARS_PER_LINE)
        {
            DEBUG_ASSERTCRASH(
                0,
                ("Buffer too small (%d) and was truncated, increase INI_MAX_CHARS_PER_LINE", INI_MAX_CHARS_PER_LINE));
    ...
    if (token != NULL)
    {
        ...
        else
        {
            ...
            if ((strLen = strlen(token)) > 1)
            {
                ...
                // Check for end of quoted string.  Checking here fixes cases where quoted string on same line with other
                // data.
            ...
            else
            {
                ...
                for (int ptIdx = 0; ptIdx < parseTableList.getCount(); ++ptIdx)
                {
                    ...
                    INIFieldParseProc parse =
                        findFieldParse(parseTableList.getNthFieldParse(ptIdx), field, offset, userData);
                    if (parse)
                    {
                        ...
                        {
                            DEBUG_CRASH(
                                ("[LINE: %d - FILE: '%s'] Error reading field '%s' of block '%s'",
                                 INI::getLineNum(),
                                 INI::getFilename().str(),
                                 field,
                                 m_curBlockStart));
...
void INI::parseGameClientRandomVariable(INI *ini, void * /*instance*/, void *store, const void * /*userData*/)
{
    ...
    if (token)
        type = (GameClientRandomVariable::DistributionType)INI::scanIndexList(
            token,
            GameClientRandomVariable::DistributionTypeNames);
