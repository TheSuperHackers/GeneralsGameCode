...
void RecorderClass::logGameStart(AsciiString options)
{
	...
	time(&startTime);
	UnsignedInt fileSize = m_replayFile->size();
	// move to appropriate offset
	if ( m_replayFile->seek(startTimeOffset, File::seekMode::START) == startTimeOffset )
	{
		// save off start time
		replay_time_t tmp = (replay_time_t)startTime;
		m_replayFile->write(&tmp, sizeof(replay_time_t));
	}
	// move back to end of stream
#ifdef DEBUG_CRASHING
	Int res =
#endif
		m_replayFile->seek(fileSize, File::seekMode::START);
...
void RecorderClass::logPlayerDisconnect(UnicodeString player, Int slot)
{
	...
	UnsignedInt fileSize = m_replayFile->size();
	// move to appropriate offset
	if ( m_replayFile->seek(disconOffset + slot*sizeof(Bool), File::seekMode::START) == (disconOffset + slot*sizeof(Bool)) )
	{
		// save off discon status
		Bool b = TRUE;
		m_replayFile->write(&b, sizeof(Bool));
	}
	// move back to end of stream
#ifdef DEBUG_CRASHING
	Int res =
...
void RecorderClass::logCRCMismatch( void )
{
	...
	UnsignedInt fileSize = m_replayFile->size();
	// move to appropriate offset
	if ( m_replayFile->seek(desyncOffset, File::seekMode::START) == desyncOffset )
	{
...
void RecorderClass::logGameEnd( void )
{
	...
	UnsignedInt fileSize = m_replayFile->size();
	// move to appropriate offset
	if ( m_replayFile->seek(endTimeOffset, File::seekMode::START) == endTimeOffset )
	{
		// save off end time
	...
	// move to appropriate offset
	if ( m_replayFile->seek(frameCountOffset, File::seekMode::START) == frameCountOffset )
	{
		// save off frameCount
		m_replayFile->write(&frameCount, sizeof(UnsignedInt));
	}
	// move back to end of stream
#ifdef DEBUG_CRASHING
	Int res =
...
void RecorderClass::updateRecord() 
{
	...
	if (needFlush) {
		DEBUG_ASSERTCRASH(m_replayFile != NULL, ("RecorderClass::updateRecord() - unexpected call to fflush(m_replayFile)"));
		m_replayFile->flush();
	}
}
...
void RecorderClass::startRecording(GameDifficulty diff, Int originalGameMode, Int rankPoints, Int maxFPS) {
	DEBUG_ASSERTCRASH(m_replayFile == NULL, ("Starting to record game while game is in progress."));
	...
	m_replayFile = TheFileSystem->openFile(filepath.str(), File::CREATE | File::WRITE);
	if (m_replayFile == NULL) {
		DEBUG_ASSERTCRASH(m_replayFile != NULL, ("Failed to create replay file"));
		return;
	}
	// TheSuperHackers @info the original fprintf ignores the null terminator
	m_replayFile->write("GENREP", sizeof("GENREP") - 1);
	...
	m_replayFile->write(&time, sizeof(replay_time_t));	// reserve space for start time
	m_replayFile->write(&time, sizeof(replay_time_t));	// reserve space for end time

	UnsignedInt frames = 0;
	m_replayFile->write(&frames, sizeof(UnsignedInt));	// reserve space for duration in frames

	Bool dataFlags = FALSE;
	m_replayFile->write(&dataFlags, sizeof(Bool));	// reserve space for flag (true if we desync)
	m_replayFile->write(&dataFlags, sizeof(Bool));	// reserve space for flag (true if we quit early)
	for (Int i=0; i<MAX_SLOTS; ++i)
	{
		m_replayFile->write(&dataFlags, sizeof(Bool));	// reserve space for flag (true if player i disconnects)
	}

	// Print out the name of the replay.
	UnicodeString replayName;
	replayName = TheGameText->fetch("GUI:LastReplay");
	m_replayFile->write(replayName.str(), (replayName.getLength() * sizeof(WideChar)) + sizeof(WideChar) );

	// Date and Time
	SYSTEMTIME systemTime;
	GetLocalTime( &systemTime );
	m_replayFile->write(&systemTime, sizeof(SYSTEMTIME));
	...
	m_replayFile->write(versionString.str(), (versionString.getLength() * sizeof(WideChar)) + sizeof(WideChar) );
	m_replayFile->write(versionTimeString.str(), (versionTimeString.getLength() * sizeof(WideChar)) + sizeof(WideChar) );
	m_replayFile->write(&versionNumber, sizeof(UnsignedInt));
	m_replayFile->write(&(TheGlobalData->m_exeCRC), sizeof(UnsignedInt));
	m_replayFile->write(&(TheGlobalData->m_iniCRC), sizeof(UnsignedInt));

	// Number of players
	/*
	Int numPlayers = ThePlayerList->getPlayerCount();
	fwrite(&numPlayers, sizeof(numPlayers), 1, m_replayFile);
	*/
	...
	m_replayFile->write(theSlotList.str(), theSlotList.getLength() + sizeof(Char));

	// TheSuperHackers @info This is like this because life is pain and the original implementation hates us
	Char indexString[2] = "";
	snprintf(indexString, 2, "%d", localIndex);
	m_replayFile->write(&indexString, 2 * sizeof(Char));
	...
	for (Int i = 0; i < numPlayers; ++i) {
		...
		fwprintf(m_replayFile, L"%s", name.str());
		fputwc(0, m_replayFile);
		UnicodeString faction = player->getFaction()->getFactionDisplayName();
		fwprintf(m_replayFile, L"%s", faction.str());
		fputwc(0, m_replayFile);
		Int color = player->getColor()->getAsInt();
		fwrite(&color, sizeof(color), 1, m_replayFile);
		Int team = 0;
		Int startingSpot = 0;
		fwrite(&startingSpot, sizeof(Int), 1, m_replayFile);
		fwrite(&team, sizeof(Int), 1, m_replayFile);
	}
	*/

	// Write the game difficulty.
	m_replayFile->write(&diff, sizeof(Int));

	// Write original game mode
	m_replayFile->write(&originalGameMode, sizeof(originalGameMode));

	// Write rank points to add at game start
	m_replayFile->write(&rankPoints, sizeof(rankPoints));

	// Write maxFPS chosen
	m_replayFile->write(&maxFPS, sizeof(maxFPS));

	DEBUG_LOG(("RecorderClass::startRecording() - diff=%d, mode=%d, FPS=%d", diff, originalGameMode, maxFPS));

	/*
	// Write the map name.
	fprintf(m_replayFile, "%s", (TheGlobalData->m_mapName).str());
	fputc(0, m_replayFile);
	*/
...
void RecorderClass::writeToFile(GameMessage * msg) {
	...
	m_replayFile->write(&frame, sizeof(frame));

	// Write the command type
	GameMessage::Type type = msg->getType();
	m_replayFile->write(&type, sizeof(type));

	// Write the player index
	Int playerIndex = msg->getPlayerIndex();
	m_replayFile->write(&playerIndex, sizeof(playerIndex));
	...
	m_replayFile->write(&numTypes, sizeof(numTypes));
	...
	while (argType != NULL) {
		...
		m_replayFile->write(&argTypeCount, sizeof(argTypeCount));
...
//		if (lasttype != type) {
//			fwrite(&type, sizeof(type), 1, m_replayFile);
//			lasttype = type;
//		}
...
void RecorderClass::writeArgument(GameMessageArgumentDataType type, const GameMessageArgumentType arg) {
	...
	switch (type) {
		...
		case ARGUMENTDATATYPE_INTEGER:
			m_replayFile->write( &(arg.integer), sizeof(Int) );
			break;
		case ARGUMENTDATATYPE_REAL:
			m_replayFile->write( &(arg.real), sizeof(Real) );
			break;
		case ARGUMENTDATATYPE_BOOLEAN:
			m_replayFile->write( &(arg.boolean), sizeof(Bool) );
			break;
		case ARGUMENTDATATYPE_OBJECTID:
			m_replayFile->write( &(arg.objectID), sizeof(ObjectID) );
			break;
		case ARGUMENTDATATYPE_DRAWABLEID:
			m_replayFile->write( &(arg.drawableID), sizeof(DrawableID) );
			break;
		case ARGUMENTDATATYPE_TEAMID:
			m_replayFile->write( &(arg.teamID), sizeof(UnsignedInt) );
			break;
		case ARGUMENTDATATYPE_LOCATION:
			m_replayFile->write( &(arg.location), sizeof(Coord3D) );
			break;
		case ARGUMENTDATATYPE_PIXEL:
			m_replayFile->write( &(arg.pixel), sizeof(ICoord2D) );
			break;
		case ARGUMENTDATATYPE_PIXELREGION:
			m_replayFile->write( &(arg.pixelRegion), sizeof(IRegion2D) );
			break;
		case ARGUMENTDATATYPE_TIMESTAMP:
			m_replayFile->write( &(arg.timestamp), sizeof(UnsignedInt) );
			break;
		case ARGUMENTDATATYPE_WIDECHAR:
			m_replayFile->write( &(arg.wChar), sizeof(WideChar) );
			break;
		default:
