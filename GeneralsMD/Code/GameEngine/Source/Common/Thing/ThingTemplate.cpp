...
const FieldParse ThingTemplate::s_objectFieldParseTable[] = {
    ...
    //{ "InventoryImageEnabled",	INI::parseAsciiString,	NULL,		offsetof( ThingTemplate, m_inventoryImage[
    //INV_IMAGE_ENABLED ] ) }, { "InventoryImageDisabled",	INI::parseAsciiString,	NULL,		offsetof( ThingTemplate,
    //m_inventoryImage[ INV_IMAGE_DISABLED ] ) }, { "InventoryImageHilite",		INI::parseAsciiString,	NULL,		offsetof(
    //ThingTemplate, m_inventoryImage[ INV_IMAGE_HILITE ] ) }, { "InventoryImagePushed",		INI::parseAsciiString,	NULL,
    ...
    {"MaxSimultaneousLinkKey",
     ...
     NULL,
     offsetof(ThingTemplate, m_maxSimultaneousLinkKey)},
...
Bool ModuleInfo::clearModuleDataWithTag(const AsciiString &tagToClear, AsciiString &clearedModuleNameOut)
{
    ...
    for (std::vector<Nugget>::iterator it = m_info.begin(); it != m_info.end(); /* empty */)
    {
        if (it->m_moduleTag == tagToClear)
        {
            DEBUG_ASSERTCRASH(
                !cleared,
                ("Hmm, multiple clears in ModuleInfo::clearModuleDataWithTag, should this be possible?"));
    ...
    while (it != m_info.end())
    {
        if ((it->interfaceMask & interfaceMask) != 0 && it->copiedFromDefault)
        {
            ...
            else if (it->overrideableByLikeKind)
            {
                ...
                if (oldName == newName // we will dump this instance, since the INI author requested a specific one of the
                                       // same class
                    || disallowed // or, we just do not Add these special overrideables to these kinds of templates, so just
                                  // dump it
                    || candidate == FALSE)
                {
                    it = m_info.erase(it);
...
void ThingTemplate::parseModuleName(INI *ini, void *instance, void *store, const void *userData)
{
    ...
    {
        DEBUG_CRASH(
            ("[LINE: %d - FILE: '%s'] Module tag not found for module '%s' on thing template '%s'.  Module tags are "
             "required and must be unique for all modules within an object definition",
             ini->getLineNum(),
             ini->getFilename().str(),
             tokenStr.str(),
             self->getName().str()));
...
void ThingTemplate::validate()
{
    ...
    if (isKindOf(KINDOF_STRUCTURE))
    {
        ...
        {
            DEBUG_CRASH(
                ("Structure %s has no armor, but probably should (StructureArmor) -- please fix it.)", getName().str()));
        }
