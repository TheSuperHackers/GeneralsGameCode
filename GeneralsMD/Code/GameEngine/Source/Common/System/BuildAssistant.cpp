  ...
  for (it = m_sellList.begin(); it != m_sellList.end(); /*empty*/)
  {
    ...
    if (obj->getConstructionPercent() <= -50.0f)
    {
      ...
      if (player)
      {
        ...
        // 0 is the init, and means you have no override.  We would be marked unsellable if someone wanted us to not
        // sell
        if (obj->getTemplate()->getRefundValue() != 0)
          sellValue = obj->getTemplate()->getRefundValue();
        else
          sellValue = REAL_TO_UNSIGNEDINT(obj->getTemplate()->calcCostToBuild(player) * TheGlobalData->m_sellPercentage);
...
//-------------------------------------------------------------------------------------------------
Object *BuildAssistant::buildObjectNow(
    ...
    Player *owningPlayer)
{
  ...
  {
    DEBUG_ASSERTCRASH(
        constructorObject->getControllingPlayer() == owningPlayer,
...
//-------------------------------------------------------------------------------------------------
void BuildAssistant::buildObjectLineNow(
    Object *constructorObject,
    const ThingTemplate *what,
    const Coord3D *start,
    const Coord3D *end,
    ...
    Player *owningPlayer)
{
  ...
  tileBuildInfo = buildTiledLocations(what, angle, start, end, objectSize, maxObjects, constructorObject);
...
static void checkSampleBuildLocation(const Coord3D *samplePoint, void *userData)
{
  ...
  else
  {
    ...
    if ((type == PathfindCell::CELL_WATER) || (type == PathfindCell::CELL_CLIFF) || (type == PathfindCell::CELL_IMPASSABLE))
    {
      sampleData->terrainRestricted = true;
    }
  }
    ...
    void *funcUserData)
{
  ...
  if (build->getTemplateGeometryInfo().getGeomType() == GEOMETRY_BOX)
  {
    ...
    halfFootprintWidth = build->getTemplateGeometryInfo().getMajorRadius();
      ...
      build->getTemplateGeometryInfo().getGeomType() == GEOMETRY_SPHERE
  ...
  Real x, y;
  Vector3 v;
  for (y = -halfFootprintHeight; y < halfFootprintHeight + sampleResolution; y += sampleResolution)
  {
    // snap it to the actual extent since we can go over by one sample resolution
    if (y > halfFootprintHeight)
      y = halfFootprintHeight;

    for (x = -halfFootprintWidth; x < halfFootprintWidth + sampleResolution; x += sampleResolution)
    {
      // snap it to the actual extent since we can go over by one sample resolution
...
LegalBuildCode BuildAssistant::isLocationClearOfObjects(
    ...
    Player *thePlayer)
{
  ObjectIterator *iter = ThePartitionManager->iteratePotentialCollisions(worldPos, build->getTemplateGeometryInfo(), angle);
  Object *them;
  ...
  for (them = iter->first(); them; them = iter->next())
  {
    ...
    if (BitIsSet(options, IGNORE_STEALTHED))
    {
      if (rel != ALLIES)
      {
        if (them->testStatus(OBJECT_STATUS_STEALTHED) && !them->testStatus(OBJECT_STATUS_DETECTED)
            && !them->testStatus(OBJECT_STATUS_DISGUISED))
        {
  ...
  for (them = iter2->first(); them; them = iter2->next())
  {
    ...
    if (ThePartitionManager
            ->geomCollidesWithGeom(them->getPosition(), hisBounds, them->getOrientation(), worldPos, myBounds, angle))
    {
      TheTerrainVisual->addFactionBib(them, true);
      return LBC_OBJECTS_IN_THE_WAY;
    }
    ...
    if (them->isKindOf(KINDOF_IMMOBILE))
    {
      ...
      if (checkMyExit
          && ThePartitionManager
                 ->geomCollidesWithGeom(them->getPosition(), hisBounds, them->getOrientation(), &myExitPos, myGeom, angle))
      {
        TheTerrainVisual->addFactionBib(them, true);
        return LBC_OBJECTS_IN_THE_WAY;
      }
      ...
      if (checkHisExit
          && ThePartitionManager
                 ->geomCollidesWithGeom(&hisExitPos, hisGeom, them->getOrientation(), worldPos, myBounds, angle))
      {
        TheTerrainVisual->addFactionBib(them, true);
        return LBC_OBJECTS_IN_THE_WAY;
      }
      ...
      if (checkMyExit && checkHisExit
          && ThePartitionManager
                 ->geomCollidesWithGeom(&hisExitPos, hisGeom, them->getOrientation(), &myExitPos, myGeom, angle))
      {
        TheTerrainVisual->addFactionBib(them, true);
        return LBC_OBJECTS_IN_THE_WAY;
      }
    ...
    Player *player)
{
  ...
  {
    {
      ...
      DEBUG_ASSERTCRASH(
          playerIndex >= 0,
          ("isLocationLegalToBuild() needs a builderObject with a team to check for shroud"));
  ...
  if (BitIsSet(options, TERRAIN_RESTRICTIONS))
  {
    ...
    iterateFootprint(build, angle, worldPos, 3 * sampleResolution, checkSampleBuildLocation, &sampleData);
    ...
    iterateFootprint(build, angle, worldPos, sampleResolution, checkSampleBuildLocation, &sampleData);
    if (sampleData.terrainRestricted == TRUE)
      return LBC_RESTRICTED_TERRAIN;
    // check if the height across the whole footprint area is too varied (not flat enough)
    if (sampleData.hiZ - sampleData.loZ > TheGlobalData->m_allowedHeightVariationForBuilding)
...
Bool BuildAssistant::isPossibleToMakeUnit(Object *builder, const ThingTemplate *whatToBuild) const
{
  ...
  if (commandSet == NULL)
  {
    DEBUG_ASSERTLOG(
        0,
        ("Can't build a '%s' from the builder '%s' because '%s' doesn't have any command set defined",
...
CanMakeType BuildAssistant::canMakeUnit(Object *builder, const ThingTemplate *whatToBuild) const
{
  ...
  if (pu && pu->getSpecialPowerConstructionCommandButton()
      && pu->getSpecialPowerConstructionCommandButton()->getThingTemplate() == whatToBuild)
  {
    return CANMAKE_OK;
  }
...
// ------------------------------------------------------------------------------------------------
void BuildAssistant::clearRemovableForConstruction(const ThingTemplate *whatToBuild, const Coord3D *pos, Real angle)
{
  ObjectIterator *iter = ThePartitionManager->iteratePotentialCollisions(pos, whatToBuild->getTemplateGeometryInfo(), angle);
  MemoryPoolObjectHolder hold(iter);
  for (Object *them = iter->first(); them; them = iter->next())
  {
    ...
    Player *playerToBuild)
{
  ...
  ObjectIterator *iter = ThePartitionManager->iteratePotentialCollisions(pos, gi, angle);
