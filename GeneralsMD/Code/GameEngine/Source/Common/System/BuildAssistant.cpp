        ...
        if (obj->getConstructionPercent() <= -50.0f)
        {
            ...
            if (player)
            {
                ...
                if (obj->getTemplate()->getRefundValue() != 0)
                    ...
                    sellValue =
                        REAL_TO_UNSIGNEDINT(obj->getTemplate()->calcCostToBuild(player) * TheGlobalData->m_sellPercentage);
...
{
    ObjectIterator *iter =
        ThePartitionManager->iteratePotentialCollisions(worldPos, build->getTemplateGeometryInfo(), angle);
    Object *them;
...
{
    ObjectIterator *iter =
        ThePartitionManager->iteratePotentialCollisions(pos, whatToBuild->getTemplateGeometryInfo(), angle);
    MemoryPoolObjectHolder hold(iter);
    for (Object *them = iter->first(); them; them = iter->next())
    {
