...
void GUIEdit::selectWindowsInRegion(IRegion2D *region)
{
	...
	while (window)
	{
		...
		if (origin.x >= region->lo.x && origin.y >= region->lo.y && origin.x + size.x <= region->hi.x
...
void GUIEdit::readFontFile(const char *filename)
{
	...
	for (Int i = 0; i < fontCount; i++)
	{
		...
		if (font == NULL)
		{
			...
			sprintf(
					buffer,
					"Warning: The font '%s' Size: '%d' Bold: '%d', specified in the config file could not be loaded.  Does that font "
					"exist?",
					fontBuffer,
					size,
					bold);
...
static GameWindow *pointInChild(Int x, Int y, GameWindow *win)
{
	...
	for (child = win->winGetChild(); child; child = child->winGetNext())
	{
		...
		if (x >= origin.x && x <= origin.x + tempX && y >= origin.y && y <= origin.y + tempY
...
GameWindow *GUIEdit::getWindowAtPos(Int x, Int y)
{
	...
	for (window = TheWindowManager->winGetWindowList(); window; window = window->winGetNext())
	{
		// get window region
	...
	if (pick)
	{
		...
		if (parent)
		{
			if (BitIsSet(
							parent->winGetStyle(),
							GWS_VERT_SLIDER | GWS_HORZ_SLIDER | GWS_SCROLL_LISTBOX | GWS_COMBO_BOX | GWS_TAB_CONTROL))
			{
				...
				//
				if (BitIsSet(parent->winGetStyle(), GWS_HORZ_SLIDER | GWS_VERT_SLIDER))
				{
					GameWindow *grandParent = parent->winGetParent();

					if (grandParent && BitIsSet(grandParent->winGetStyle(), GWS_SCROLL_LISTBOX))
						pick = grandParent;
 ...
 * locations */
//=============================================================================
void GUIEdit::clipCreationParamsToParent(GameWindow *parent, Int *x, Int *y, Int *width, Int *height)
{
...
//=============================================================================
GameWindow *GUIEdit::newWindow(UnsignedInt windowStyle, GameWindow *parent, Int x, Int y, Int width, Int height)
{
	...
	switch (windowStyle)
	{
		...
		// ------------------------------------------------------------------------
		default:
			MessageBox(m_appHWnd, "Unknown window type to newWindow()", "ERROR", MB_OK | MB_ICONERROR);
			break;
...
//=============================================================================
GameWindow *GUIEdit::newUserWindow(GameWindow *parent, Int x, Int y, Int width, Int height)
{
	...
	window = TheWindowManager->winCreate(parent, status, x, y, width, height, NULL, NULL);
...
//=============================================================================
GameWindow *GUIEdit::newPushButton(GameWindow *parent, Int x, Int y, Int width, Int height)
{
	GameWindow *window;
	...
	window = TheWindowManager->gogoGadgetPushButton(parent, status, x, y, width, height, &instData, NULL, TRUE);
...
//=============================================================================
GameWindow *GUIEdit::newCheckBox(GameWindow *parent, Int x, Int y, Int width, Int height)
{
	GameWindow *window;
	WinInstanceData instData;
	UnsignedInt status = WIN_STATUS_ENABLED | WIN_STATUS_IMAGE;
	...
	// create gadget
	window = TheWindowManager->gogoGadgetCheckbox(parent, status, x, y, width, height, &instData, NULL, TRUE);
...
//=============================================================================
GameWindow *GUIEdit::newRadioButton(GameWindow *parent, Int x, Int y, Int width, Int height)
{
	...
	// create gadget
	window = TheWindowManager->gogoGadgetRadioButton(parent, status, x, y, width, height, &instData, &radioData, NULL, TRUE);
...
//=============================================================================
GameWindow *GUIEdit::newTabControl(GameWindow *parent, Int x, Int y, Int width, Int height)
{
	...
	// create gadget
	window =
			TheWindowManager->gogoGadgetTabControl(parent, status, x, y, width, height, &instData, &tabControlData, NULL, TRUE);
...
//=============================================================================
GameWindow *GUIEdit::newHorizontalSlider(GameWindow *parent, Int x, Int y, Int width, Int height)
{
	GameWindow *window;
	WinInstanceData instData;
	UnsignedInt status = WIN_STATUS_ENABLED | WIN_STATUS_IMAGE;
	...
	// make control
	window = TheWindowManager->gogoGadgetSlider(parent, status, x, y, width, height, &instData, &sliderData, NULL, TRUE);
...
//=============================================================================
GameWindow *GUIEdit::newVerticalSlider(GameWindow *parent, Int x, Int y, Int width, Int height)
{
	GameWindow *window;
	WinInstanceData instData;
	UnsignedInt status = WIN_STATUS_ENABLED | WIN_STATUS_IMAGE;
...
//=============================================================================
GameWindow *GUIEdit::newProgressBar(GameWindow *parent, Int x, Int y, Int width, Int height)
{
	GameWindow *window;
	WinInstanceData instData;
	UnsignedInt status = WIN_STATUS_ENABLED | WIN_STATUS_IMAGE;
	...
	// make control
	window = TheWindowManager->gogoGadgetProgressBar(parent, status, x, y, width, height, &instData, NULL, TRUE);
...
//=============================================================================
GameWindow *GUIEdit::newComboBox(GameWindow *parent, Int x, Int y, Int width, Int height)
{
	GameWindow *window;
	WinInstanceData instData;
	UnsignedInt status = WIN_STATUS_ENABLED | WIN_STATUS_IMAGE;
	...
	// create the control
	window = TheWindowManager->gogoGadgetComboBox(parent, status, x, y, width, height, &instData, comboData, NULL, TRUE);
...
/** Create a new list box */
//=============================================================================
GameWindow *GUIEdit::newListbox(GameWindow *parent, Int x, Int y, Int width, Int height)
{
	GameWindow *window;
	WinInstanceData instData;
	UnsignedInt status = WIN_STATUS_ENABLED | WIN_STATUS_IMAGE;
	...
	// make control
	window = TheWindowManager->gogoGadgetListBox(parent, status, x, y, width, height, &instData, &listData, NULL, TRUE);
...
//=============================================================================
GameWindow *GUIEdit::newTextEntry(GameWindow *parent, Int x, Int y, Int width, Int height)
{
	GameWindow *window;
	WinInstanceData instData;
	UnsignedInt status = WIN_STATUS_ENABLED | WIN_STATUS_IMAGE;
	...
	// make control
	window = TheWindowManager->gogoGadgetTextEntry(parent, status, x, y, width, height, &instData, &entryData, NULL, TRUE);
...
//=============================================================================
GameWindow *GUIEdit::newStaticText(GameWindow *parent, Int x, Int y, Int width, Int height)
{
	GameWindow *window;
	WinInstanceData instData;
	UnsignedInt status = WIN_STATUS_ENABLED | WIN_STATUS_IMAGE;
	...
	// make control
	window = TheWindowManager->gogoGadgetStaticText(parent, status, x, y, width, height, &instData, &textData, NULL, TRUE);
...
Bool GUIEdit::menuExit(void)
{
	...
	if (m_unsaved)
	{
		result = MessageBox(m_appHWnd, "Save file before quitting?", "Save?", MB_YESNOCANCEL);
		if (result == IDCANCEL)
		...
		else if (result == IDYES)
		{
			...
			//
			if (success == FALSE)
				MessageBox(m_appHWnd, "File not saved.  If you continue to exit all data will be LOST!", "File Not Saved", MB_OK);
...
Bool GUIEdit::menuNew(void)
{
	...
	if (m_unsaved == TRUE)
	{
		...
		result = MessageBox(m_appHWnd, "Current data is not saved.  Save before proceeding?", "Save?", MB_YESNOCANCEL);
		if (result == IDCANCEL)
			return TRUE; // no error
		else if (result == IDYES)
		{
...
Bool GUIEdit::menuCopy(void)
{
	...
	if (select == NULL)
	{
		MessageBox(m_appHWnd, "You must have windows selected before you can copy them.", "No Windows Selected", MB_OK);
		return TRUE;
...
Bool GUIEdit::menuCut(void)
{
	...
	if (select == NULL)
	{
		MessageBox(m_appHWnd, "You must have windows selected before you can cut.", "No Windows Selected", MB_OK);
		return TRUE;
 ...
 * windows */
//=============================================================================
void GUIEdit::dragMoveSelectedWindows(ICoord2D *dragOrigin, ICoord2D *dragDest)
{
	WindowSelectionEntry *select;
	GameWindow *window;
 ...
 * and on the screen */
//=============================================================================
void GUIEdit::computeSafeLocation(GameWindow *window, Int x, Int y, Int *safeX, Int *safeY)
{
	Int dx, dy;
	ICoord2D origin, size;
	GameWindow *parent;
 ...
 */
//=============================================================================
void GUIEdit::computeSafeSizeLocation(
		GameWindow *window,
		Int newX,
		Int newY,
		Int newWidth,
 ...
 * window should be */
//=============================================================================
void GUIEdit::computeResizeLocation(
		EditMode resizeMode,
		GameWindow *window,
		...
		ICoord2D *resultSize)
{
	...
	// sanity
	if (window == NULL || resizeOrigin == NULL || resizeDest == NULL || resultLoc == NULL || resultSize == NULL)
		return;
