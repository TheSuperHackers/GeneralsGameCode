...
//=============================================================================
LRESULT CALLBACK EditWindow::editProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
		// ------------------------------------------------------------------------
			...
			switch (controlID)
			{
				...
				case POPUP_MENU_BRING_TO_TOP:
				{
					...
					switch (controlID)
					{
						...
						// ----------------------------------------------------------------
						case POPUP_MENU_NEW_HORIZONTAL_SLIDER:
							TheEditor->newWindow(GWS_HORZ_SLIDER, window, pos.x, pos.y, 20 * TheEditor->getGridResolution(), GADGET_SIZE);
							break;

						// ----------------------------------------------------------------
						case POPUP_MENU_NEW_VERTICAL_SLIDER:
							TheEditor->newWindow(GWS_VERT_SLIDER, window, pos.x, pos.y, GADGET_SIZE, 20 * TheEditor->getGridResolution());
							break;

						// ----------------------------------------------------------------
						case POPUP_MENU_NEW_PROGRESS_BAR:
							TheEditor->newWindow(GWS_PROGRESS_BAR, window, pos.x, pos.y, 40 * TheEditor->getGridResolution(), GADGET_SIZE);
							break;

						// ----------------------------------------------------------------
						case POPUP_MENU_NEW_TEXT_ENTRY:
							TheEditor->newWindow(GWS_ENTRY_FIELD, window, pos.x, pos.y, 20 * TheEditor->getGridResolution(), 25);
							break;
...
void EditWindow::init(UnsignedInt clientWidth, UnsignedInt clientHeight)
{
	...
	// create the window
	m_editWindowHWnd = CreateWindowEx(
	...
	if (WW3D::Set_Render_Device(0, m_size.x, m_size.y, m_bitDepth, TRUE) != WW3D_ERROR_OK)
	{
		assert(0);
		shutdown();
		return;
...
//=============================================================================
void EditWindow::mouseEvent(UnsignedInt windowsMessage, WPARAM wParam, LPARAM lParam)
{
	Int x = LOWORD(lParam);
	Int y = HIWORD(lParam);
	...
	if (windowsMessage == WM_MOUSEMOVE)
	{
		...
		sprintf(buffer, "Mouse Location (X = %d, Y = %d)", mousePrint.x, mousePrint.y);
	...
	switch (windowsMessage)
	{
		...
		case WM_LBUTTONDOWN:
		{
			...
			//
			if (TheEditor->getMode() == MODE_DRAG_MOVE && TheEditor->selectionCount() == 1
			...
			//
			if (TheEditor->getMode() == MODE_RESIZE_TOP_LEFT || TheEditor->getMode() == MODE_RESIZE_BOTTOM_RIGHT
					|| TheEditor->getMode() == MODE_RESIZE_TOP_RIGHT || TheEditor->getMode() == MODE_RESIZE_BOTTOM_LEFT
					|| TheEditor->getMode() == MODE_RESIZE_TOP || TheEditor->getMode() == MODE_RESIZE_BOTTOM
					|| TheEditor->getMode() == MODE_RESIZE_RIGHT || TheEditor->getMode() == MODE_RESIZE_LEFT)
			{
		...
		case WM_LBUTTONUP:
		{
			...
			else if (m_resizingWindow)
			{
				...
				if (window)
				{
					...
					TheEditor->computeResizeLocation(TheEditor->getMode(), window, &m_resizeOrigin, &dest, &resultLoc, &resultSize);
...
//=============================================================================
Bool EditWindow::inCornerTolerance(ICoord2D *dest, ICoord2D *source, Int tolerance)
{
	IRegion2D region;
	...
	// check if in region
	if (dest->x >= region.lo.x && dest->x <= region.hi.x && dest->y >= region.lo.y && dest->y <= region.hi.y)
		return TRUE;
...
//=============================================================================
Bool EditWindow::inLineTolerance(ICoord2D *dest, ICoord2D *lineStart, ICoord2D *lineEnd, Int tolerance)
{
	IRegion2D region;
...
void EditWindow::drawUIFeedback(void)
{
	...
	if (TheEditor->getShowHiddenOutlines())
		drawHiddenOutlines(TheWindowManager->winGetWindowList(), GameMakeColor(color, 64, 64, 255));

	// draw see-thru window outlines if requested
	if (TheEditor->getShowSeeThruOutlines())
		drawSeeThruOutlines(TheWindowManager->winGetWindowList(), GameMakeColor(64, 64, color, 255));
	...
	if (m_dragSelecting)
	{
		...
		drawOpenRect(m_selectRegion.lo.x, m_selectRegion.lo.y, width, height, selectBoxWidth, selectBoxColor);
	...
	while (select)
	{
		...
		// draw a box on the window
		drawOpenRect(origin.x, origin.y, size.x, size.y, windowSelectWidth, windowSelectColor);
	...
	if (TheEditor->getMode() == MODE_DRAG_MOVE || TheEditor->getMode() == MODE_KEYBOARD_MOVE)
	{
		...
		while (select)
		{
			...
			if (change.x || change.y)
			{
				...
				// keep location legal
				TheEditor->computeSafeLocation(window, moveLoc.x, moveLoc.y, &safeLoc.x, &safeLoc.y);
				...
				drawOpenRect(safeLoc.x, safeLoc.y, size.x, size.y, outlineWidth, outlineColor);
			}
	...
	if (m_resizingWindow)
	{
		...
		//
		TheEditor->computeResizeLocation(TheEditor->getMode(), window, &m_resizeOrigin, &dest, &loc, &size);
...
void EditWindow::drawGrid(void)
{
	...
	Color color = GameMakeColor(gridColor->red, gridColor->green, gridColor->blue, gridColor->alpha);
...
//=============================================================================
void EditWindow::drawLine(Int startX, Int startY, Int endX, Int endY, Real lineWidth, UnsignedInt lineColor)
{
	m_2DRender->Reset();
 ...
 * specified color */
//=============================================================================
void EditWindow::drawOpenRect(Int startX, Int startY, Int width, Int height, Real lineWidth, UnsignedInt lineColor)
{
	m_2DRender->Reset();
	m_2DRender->Enable_Texturing(FALSE);
 ...
 * specified color */
//=============================================================================
void EditWindow::drawFillRect(Int startX, Int startY, Int width, Int height, UnsignedInt color)
{
	m_2DRender->Reset();
	m_2DRender->Enable_Texturing(FALSE);
...
//=============================================================================
void EditWindow::drawImage(const Image *image, Int startX, Int startY, Int endX, Int endY, Color color)
{
	// sanity
	if (image == NULL)
		return;
