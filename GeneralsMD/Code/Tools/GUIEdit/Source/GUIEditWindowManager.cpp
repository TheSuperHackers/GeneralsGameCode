...
//-------------------------------------------------------------------------------------------------
Bool GUIEditWindowManager::isWindowInClipboard(GameWindow *window, GameWindow **list)
{
	GameWindow *other;
...
//-------------------------------------------------------------------------------------------------
void GUIEditWindowManager::linkToClipboard(GameWindow *window, GameWindow **list)
{
	// sanity
...
//-------------------------------------------------------------------------------------------------
void GUIEditWindowManager::unlinkFromClipboard(GameWindow *window, GameWindow **list)
{
...
//-------------------------------------------------------------------------------------------------
GameWindow *GUIEditWindowManager::winCreate(
		GameWindow *parent,
		UnsignedInt status,
		Int x,
		Int y,
		...
		WinInstanceData *instData)
{
	...
	window = W3DGameWindowManager::winCreate(parent, status, x, y, width, height, system, instData);
...
void GUIEditWindowManager::validateClipboardNames(GameWindow *root)
{
	...
	while (nameOK == FALSE)
	{
		...
		if (++sanityLoop > 99999999)
		{
			...
			sprintf(
					buffer,
					"Unable to make unique name for '%s', please manually edit the name before saving.",
...
void GUIEditWindowManager::pasteClipboard(void)
{
	...
	if (m_clipboard == NULL)
	{
		MessageBox(TheEditor->getWindowHandle(), "Cannot perform paste, the clipboard is empty.", "Clipboard Empty", MB_OK);
		return;
	...
	for (window = m_clipboardDup; window; window = next)
	{
		...
		TheEditor->computeSafeLocation(window, pos.x, pos.y, &safePos.x, &safePos.y);
...
void InstDrawCopy(WinInstanceData *instData, WinInstanceData *sourceInstData)
{
	...
	memcpy(&instData->m_enabledDrawData, &sourceInstData->m_enabledDrawData, sizeof(WinDrawData) * MAX_DRAW_DATA);
	memcpy(&instData->m_disabledDrawData, &sourceInstData->m_disabledDrawData, sizeof(WinDrawData) * MAX_DRAW_DATA);
	memcpy(&instData->m_hiliteDrawData, &sourceInstData->m_hiliteDrawData, sizeof(WinDrawData) * MAX_DRAW_DATA);
}

//-------------------------------------------------------------------------------------------------
/** Duplciate a window and all its children */
//-------------------------------------------------------------------------------------------------
GameWindow *GUIEditWindowManager::duplicateWindow(GameWindow *source, GameWindow *parent)
{
	...
	if (BitIsSet(style, GWS_PUSH_BUTTON))
	{
		duplicate =
				TheWindowManager
						->gogoGadgetPushButton(parent, status, pos.x, pos.y, size.x, size.y, &instDataCopy, source->winGetFont(), FALSE);
	...
	else if (BitIsSet(style, GWS_CHECK_BOX))
	{
		duplicate =
				TheWindowManager
						->gogoGadgetCheckbox(parent, status, pos.x, pos.y, size.x, size.y, &instDataCopy, source->winGetFont(), FALSE);
	...
	else if (BitIsSet(style, GWS_USER_WINDOW))
	{
		...
		duplicate = TheWindowManager->winCreate(parent, status, pos.x, pos.y, size.x, size.y, NULL, &instDataCopy);

	} // end else if
	else
	{
		MessageBox(TheEditor->getWindowHandle(), "Cannot duplicate window, undefined style.", "Internal Error", MB_OK);
		assert(0);
	...
	if (duplicate == NULL)
	{
		MessageBox(TheEditor->getWindowHandle(), "Unable to duplicate window", "Internal Error", MB_OK);
		assert(0);
		memset(&instDataCopy, 0, sizeof(instDataCopy)); // see comment below
		return NULL;
...
void GUIEditWindowManager::makeChildOf(GameWindow *target, GameWindow *parent)
{
	...
	{
		MessageBox(TheEditor->getWindowHandle(), "You cannot give a gadget control a child.", "Child Not Allowed", MB_OK);
		return;
	...
	ICoord2D parentPos;
	parent->winGetScreenPosition(&parentPos.x, &parentPos.y);
	target->winSetPosition(screenPosBeforeMove.x - parentPos.x, screenPosBeforeMove.y - parentPos.y);
	...
	// kee the location legal
	TheEditor->computeSafeLocation(target, origin.x, origin.y, &safeLoc.x, &safeLoc.y);
...
//-------------------------------------------------------------------------------------------------
void GUIEditWindowManager::moveAheadOf(GameWindow *windowToMove, GameWindow *aheadOf)
{
	// sanity
	...
	windowToMove->winGetScreenPosition(&beforeMoveScreenPos.x, &beforeMoveScreenPos.y);
	...
	windowToMove->winSetPosition(beforeMoveScreenPos.x - parentPos.x, beforeMoveScreenPos.y - parentPos.y);
	...
	// kee the location legal
	TheEditor->computeSafeLocation(windowToMove, origin.x, origin.y, &safeLoc.x, &safeLoc.y);
