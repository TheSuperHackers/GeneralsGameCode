...
void ChunkTableClass::AddItem(CListCtrl *List, int &Counter, const char *name, W3dShaderStruct *shader)
{
	static const char *_depth_compare[] = {
		"Pass Never", "Pass Less", "Pass Equal", "Pass Less or Equal", "Pass Greater", "Pass Not Equal", "Pass Greater or Equal",
		"Pass Always"
	};
	...
	static const char *_destblend[] = {
		"Zero", "One", "Src Color", "One Minus Src Color", "Src Alpha", "One Minus Src Alpha", "Src Color Prefog"
	};
	...
	static const char *_detailcolor[] = { "Disable", "Detail", "Scale", "InvScale",		"Add",
...
{
	static const char *_chntypes[] = { "X Translation", "Y Translation", "Z Translation", "X Rotation",
...
void ChunkTableClass::List_W3D_CHUNK_EMITTER_ROTATION_KEYFRAMES(ChunkItem *Item, CListCtrl *List)
{
	...
	W3dEmitterRotationKeyframeStruct *key =
			(W3dEmitterRotationKeyframeStruct *)((char *)Item->Data + sizeof(W3dEmitterRotationHeaderStruct));
	char buf[256];
...
void ChunkTableClass::List_W3D_CHUNK_EMITTER_BLUR_TIME_KEYFRAMES(ChunkItem *Item, CListCtrl *List)
{
	...
	W3dEmitterBlurTimeKeyframeStruct *key =
			(W3dEmitterBlurTimeKeyframeStruct *)((char *)Item->Data + sizeof(W3dEmitterBlurTimeHeaderStruct));
	char buf[256];
	for (unsigned int i = 0; i < header->KeyframeCount + 1; i++)
	{
		sprintf(buf, "Time[%d]", i);
...
ChunkTableClass::ChunkTableClass()
{
	...
	NewType(
			...
			"OBSOLETE_W3D_CHUNK_EMITTER_OPACITY_KEYFRAME",
			List_W3D_CHUNK_EMITTER_OPACITY_KEYFRAME);
	NewType(
	...
	NewType(
			...
			"W3D_CHUNK_PRELIT_LIGHTMAP_MULTI_PASS",
			List_W3D_CHUNK_PRELIT_LIGHTMAP_MULTI_PASS,
			true);
	NewType(
			W3D_CHUNK_PRELIT_LIGHTMAP_MULTI_TEXTURE,
			"W3D_CHUNK_PRELIT_LIGHTMAP_MULTI_TEXTURE",
			List_W3D_CHUNK_PRELIT_LIGHTMAP_MULTI_TEXTURE,
			true);
...
ChunkItem::ChunkItem(ChunkLoadClass &cload)
{
	...
	// if the chunktype indicates that it has member chunks then do not load this chunk's data. Have the external caller do
	// that.
