...
WorldHeightMapEdit::WorldHeightMapEdit(ChunkInputStream *pStrm) :
		WorldHeightMap(pStrm), m_warnTooManyTex(false), m_warnTooManyBlend(false)
{
...
void WorldHeightMapEdit::loadBaseImages(void)
{
	...
	for (terrain = TheTerrainTypes->firstTerrain(); terrain; terrain = TheTerrainTypes->nextTerrain(terrain))
	{
...
void WorldHeightMapEdit::getTexClassNeighbors(Int xIndex, Int yIndex, Int textureClass, Int *pSideCount, Int *pTotalCount)
{
	Int i, j;
	int sideCount = 0;
	int totalCount = 0;
...
Int WorldHeightMapEdit::getTileNdxForClass(Int xIndex, Int yIndex, Int textureClass)
{
	...
	if (textureClass >= 0 && textureClass < m_numGlobalTextureClasses)
	{
		...
		tileNdx = firstTile + ((xIndex / 2) % width) + width * ((yIndex / 2) % width); // 2 terrain cells across tile so divide
																																									 // by 2
...
void WorldHeightMapEdit::blendTile(Int xIndex, Int yIndex, Int srcXIndex, Int srcYIndex, Int textureClass, Int edgeClass)
{
	...
	{ // get index of sub-tile that would show up here if we continued tiling src.
		...
		DEBUG_ASSERTCRASH((blendTileNdx / 4 < m_numBitmapTiles), ("oops")); // check it falls into one of the 64x64 tiles of
																																				// textures used on map.
	}
...
#define MATCH(x) (curInfo->x == pBlendInfo->x)
		if (MATCH(blendNdx) && MATCH(horiz) && MATCH(vert) && MATCH(rightDiagonal) && MATCH(leftDiagonal) && MATCH(longDiagonal)
...
Updates m_blendTileNdxes[] for this coordinate with index into m_blendedTiles[]*/
void WorldHeightMapEdit::blendSpecificTiles(
		Int xIndex,
		Int yIndex,
		Int srcXIndex,
		Int srcYIndex,
...
*/
void WorldHeightMapEdit::blendToThisClass(Int xIndex, Int yIndex, Int textureClass, Int edgeClass)
{
	Int sides, total;
	...
	else if (sides == 2)
	{
		...
		blendSpecificTiles(xIndex, yIndex, srcXIndex, srcYIndex, curTileNdx, blendTileNdx, true, edgeClass);
	}
}
...
Bool WorldHeightMapEdit::floodFill(Int xIndex, Int yIndex, Int textureClass, Bool doReplace)
{
	...
	if (!textureExistsInMap && doReplace)
	{
		if (m_globalTextureClasses[textureClass].numTiles <= m_globalTextureClasses[curTileClass].numTiles)
		{
			...
			{
				REF_PTR_SET(
						m_sourceTiles[m_textureClasses[localClass].firstTile + i],
...
void WorldHeightMapEdit::showTileStatusInfo(void)
{
	...
	CString tmp;
	tmp.Format(
			"Base tiles used %d of %d (%d%%)\n",
			m_numBitmapTiles,
			availableTiles,
			(m_numBitmapTiles * 100 + 50) / availableTiles);
	message += tmp;
	tmp.Format(
			"Blend tiles used %d of %d (%d%%)\n",
			m_numBlendedTiles,
			availableBlends,
...
*/
Bool WorldHeightMapEdit::resize(
		Int newXSize,
		Int newYSize,
		Int newHeight,
		Int newBorder,
...
void WorldHeightMapEdit::dbgVerifyAfterUndo(void)
{
	...
	for (i = 0; i < m_numTextureClasses; i++)
	{
		...
		if (m_textureClasses[i].globalTextureClass >= 0)
		{
			...
			DEBUG_ASSERTCRASH(
					(m_globalTextureClasses[m_textureClasses[i].globalTextureClass].forDebugOnly_fileTextureClass == i),
					("oops"));
		}
	}
...
void WorldHeightMapEdit::updateForAdjacentCliffs(Int xIndex, Int yIndex, UnsignedByte *pProcessed, TCliffInfo &cliffInfo)
{
...
void WorldHeightMapEdit::updateFlatCellForAdjacentCliffs(Int xIndex, Int yIndex, Int curTileClass, UnsignedByte *pProcessed)
{
	TCliffInfo tmpCliff;
	Bool lock0 = false;
	Bool lock1 = false;
