    ...
    WorldHeightMap(pStrm), m_warnTooManyTex(false), m_warnTooManyBlend(false)
{
    ...
    for (i = 0; i < m_numGlobalTextureClasses; i++)
    {
        for (j = 0; j < m_numTextureClasses; j++)
        {
            ...
            {
                DEBUG_ASSERTCRASH(
                    m_textureClasses[j].globalTextureClass == -1,
...
#define MATCH(x) (curInfo->x == pBlendInfo->x)
        if (MATCH(blendNdx) && MATCH(horiz) && MATCH(vert) && MATCH(rightDiagonal) && MATCH(leftDiagonal)
            && MATCH(longDiagonal) && MATCH(inverted) && MATCH(customBlendEdgeClass))
        {
            // got a match.
            return k;
        }
