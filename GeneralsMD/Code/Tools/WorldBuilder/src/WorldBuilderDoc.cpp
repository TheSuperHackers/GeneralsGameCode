...
static Bool IndexInRect(
    ...
    const Coord3D *tr,
    CPoint *index);
static Bool AddUniqueAndNeighbors(
    CWorldBuilderDoc *pDoc,
    const Coord3D *bl,
    const Coord3D *tl,
    const Coord3D *br,
...
public:
    ...
    virtual void flush(void)
    {
        ...
        DEBUG_LOG(
            ("Compressed %d bytes to %d bytes - compression of %g%%",
             m_totalBytes,
             compressedLen,
...
#define FIX_TEAM(key) \
    ...
    for (pMapObj = MapObject::getFirstMapObject(); pMapObj; pMapObj = pMapObj->getNext())
    {
        ...
        if (pMapObj->isScorch() || pMapObj->isWaypoint() || pMapObj->isLight() || pMapObj->getFlag(FLAG_ROAD_FLAGS)
        ...
        if (exists)
        {
            ...
            if (teamInfo)
            {
                ...
                if (pSide)
                {
                    ...
                    {
                        DEBUG_LOG(
                            ("Player '%s' Faction '%s' could not be found in sides list!",
                             playername.str(),
                             tmplname.str()));
                        if (tmplname.startsWith("FactionFundamentalist"))
                        {
...
//=============================================================================
//=============================================================================
Bool CWorldBuilderDoc::getAllIndexesInRect(
    const Coord3D *bl,
    const Coord3D *br,
    const Coord3D *tl,
    const Coord3D *tr,
...
void CWorldBuilderDoc::addWaypointLink(Int waypointID1, Int waypointID2)
{
    ...
    for (i = 0; i < m_numWaypointLinks; i++)
    {
        if (m_waypointLinks[i].waypoint1 == waypointID1 && m_waypointLinks[i].waypoint2 == waypointID2)
        {
            ...
            if (tt)
            {
                if (tt->getEditorSorting() == ES_VEHICLE || tt->getEditorSorting() == ES_INFANTRY)
                {
                    ...
                    fprintf(
                        theLogFile,
                        ", %s",
                        d->getBool(TheKey_objectRecruitableAI, &exists) ? "AIRecruitable" : "Not AIRecruitable");
                    fprintf(
                        theLogFile,
                        ", %s",
                        d->getBool(TheKey_objectSelectable, &exists) ? "Selectable" : "Not Selectable");
            ...
            for (Int i = 0; i < numTeams; i++)
            {
                ...
                if (ti->getDict()->getAsciiString(TheKey_teamOwner) == name)
                {
                    ...
                    fprintf(
                        ...
                        teamName.str(),
                        waypoint.str(),
                        static_cast<LPCSTR>(pri),
                        trigger.str());
                    fprintf(theLogFile, "  UNITS:");
                    fprintUnit(
                        theLogFile,
                        ti->getDict(),
                        TheKey_teamUnitMinCount1,
                        TheKey_teamUnitMaxCount1,
                        TheKey_teamUnitType1);
                    fprintUnit(
                        theLogFile,
                        ti->getDict(),
                        TheKey_teamUnitMinCount2,
                        TheKey_teamUnitMaxCount2,
                        TheKey_teamUnitType2);
                    fprintUnit(
                        theLogFile,
                        ti->getDict(),
                        TheKey_teamUnitMinCount3,
                        TheKey_teamUnitMaxCount3,
                        TheKey_teamUnitType3);
                    fprintUnit(
                        theLogFile,
                        ti->getDict(),
                        TheKey_teamUnitMinCount4,
                        TheKey_teamUnitMaxCount4,
                        TheKey_teamUnitType4);
                    fprintUnit(
                        theLogFile,
                        ti->getDict(),
                        TheKey_teamUnitMinCount5,
                        TheKey_teamUnitMaxCount5,
                        TheKey_teamUnitType5);
                    fprintUnit(
                        theLogFile,
                        ti->getDict(),
                        TheKey_teamUnitMinCount6,
                        TheKey_teamUnitMaxCount6,
                        TheKey_teamUnitType6);
                    fprintUnit(
                        theLogFile,
                        ti->getDict(),
