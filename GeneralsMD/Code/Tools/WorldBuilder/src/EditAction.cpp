...
EditAction::EditAction(CWnd *pParent /*=NULL*/) : CDialog(EditAction::IDD, pParent)
{
...
BOOL EditAction::OnInitDialog()
{
    ...
    ScreenToClient(&rect);
    m_actionTreeView.Create(
        TVS_HASLINES | TVS_LINESATROOT | TVS_HASBUTTONS | TVS_SHOWSELALWAYS | TVS_DISABLEDRAGDROP | WS_TABSTOP,
        rect,
        this,
        IDC_ACTION_TREE);
        ...
        if (pHdr->hwndFrom == m_myEditCtrl.m_hWnd)
        {
            if (pHdr->code == EN_LINK)
            {
                ...
                if (pLink->msg == WM_LBUTTONDOWN)
                {
                    ...
                    if (match)
                    {
                        if (m_action->getParameter(m_curEditParameter)->getParameterType()
                            == Parameter::COMMANDBUTTON_ABILITY)
                        {
                            EditParameter::edit(
                                m_action->getParameter(m_curEditParameter),
                                0,
                                m_action->getParameter(0)->getString());
                        }
                        else
                        {
