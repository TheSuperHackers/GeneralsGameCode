	...
	for (Int i = 0; i < numChildren; i++)
	{
		if (GameLogicRandomValueReal(0.0f, 1.0f) < childChance)
		{
			for (retry = 0; retry < maxTries; retry++)
			{
				...
				if (dir.x * prevDir.x + dir.y * prevDir.y >= 0.0f && normal.z > flatTolerance
						&& fabs(childPos.z - baseHeight) < heightTolerance && childPos.x > 0 && childPos.y > 0 && childPos.x < bounds.x
						&& childPos.y < bounds.y && (!localIsUnderwater(childPos.x, childPos.y)))
					break;
			}
...
static Bool _positionIsTooCliffyForTrees(Coord3D pos)
{
	...
	if (((pos.z / otherPos.z / 1) > MAX_TREE_RISE_OVER_RUN) || ((otherPos.z / pos.z / 1) > MAX_TREE_RISE_OVER_RUN))
	{
		return true;
	}
	...
	if (((pos.z / otherPos.z / SQRT_2) > MAX_TREE_RISE_OVER_RUN) || ((otherPos.z / pos.z / SQRT_2) > MAX_TREE_RISE_OVER_RUN))
	{
		return true;
	}
