...
void LayersList::removeLayerNamed(IN AsciiString layerToRemove)
{
	...
	for (ListMapObjectPtrIt obj = layerIt->objectsInLayer.begin(); obj != layerIt->objectsInLayer.end();
...
void LayersList::mergeLayerInto(IN ListLayerIt src, IN ListLayerIt dst)
{
	...
	for (ptpIt = src->polygonTriggersInLayer.begin(); ptpIt != src->polygonTriggersInLayer.end();
			 ptpIt = src->polygonTriggersInLayer.begin())
	{
...
void LayersList::updateUIFromList(void)
{
	...
	for (ListLayerIt layersIt = mLayers.begin(); layersIt != mLayers.end(); ++layersIt)
	{
		...
		for (ListPolygonTriggerPtrIt triggerIt = layersIt->polygonTriggersInLayer.begin();
				 triggerIt != layersIt->polygonTriggersInLayer.end();
				 ++triggerIt)
		{
...
Bool LayersList::findMapObjectAndList(IN MapObject *objectToFind, OUT ListLayerIt *layerIt, OUT ListMapObjectPtrIt *objectIt)
{
	...
	for (ListLayerIt layersIt = mLayers.begin(); layersIt != mLayers.end(); ++layersIt)
	{
		for (ListMapObjectPtrIt objPtrIt = layersIt->objectsInLayer.begin(); objPtrIt != layersIt->objectsInLayer.end();
				 ++objPtrIt)
		{
...
Bool LayersList::findPolygonTriggerAndList(
		IN PolygonTrigger *triggerToFind,
		OUT ListLayerIt *layerIt,
		OUT ListPolygonTriggerPtrIt *triggerIt)
{
	if (!triggerToFind)
	{
		return false;
	...
	for (ListLayerIt layersIt = mLayers.begin(); layersIt != mLayers.end(); ++layersIt)
	{
		for (ListPolygonTriggerPtrIt triggerPtrIt = layersIt->polygonTriggersInLayer.begin();
				 triggerPtrIt != layersIt->polygonTriggersInLayer.end();
				 ++triggerPtrIt)
		{
...
void LayersList::removeMapObjectFromLayer(
		IN MapObject *objectToRemove,
...
void LayersList::removePolygonTriggerFromLayer(
		IN PolygonTrigger *triggerToRemove,
		IN ListLayerIt *layerIt,
		IN ListPolygonTriggerPtrIt *triggerIt)
{
	if (!triggerToRemove)
	{
