...
Int DrawObject::initData(void)
{
    ...
    m_vertexBufferTile1 =
        NEW_REF(DX8VertexBufferClass, (DX8_FVF_XYZDUV1, m_numTriangles * 3, DX8VertexBufferClass::USAGE_DYNAMIC));
    m_vertexBufferTile2 =
        NEW_REF(DX8VertexBufferClass, (DX8_FVF_XYZDUV1, m_numTriangles * 3, DX8VertexBufferClass::USAGE_DYNAMIC));

    m_vertexFeedback =
        NEW_REF(DX8VertexBufferClass, (DX8_FVF_XYZDUV1, NUM_FEEDBACK_VERTEX, DX8VertexBufferClass::USAGE_DYNAMIC));
...
void DrawObject::updateRampVB(void)
{
    ...
    */
    ...
    BuildRectFromSegmentAndWidth(&m_rampStartPoint, &m_rampEndPoint, m_rampWidth, &coordBL, &coordTL, &coordBR, &coordTR);
...
Int DrawObject::updateVB(DX8VertexBufferClass *pVB, Int color, Bool doArrow, Bool doDiamond)
{
    ...
    Real factor = TheGlobalData->m_terrainAmbient[0].red + TheGlobalData->m_terrainAmbient[0].green
    ...
    static const Int highlightColors[NUM_HIGHLIGHT] = {((255 << 8) + (255 << 16)), ((255 << 16)), (255 << 8)};
        ...
        case GEOMETRY_BOX:
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                for (int corner = 0; corner < 4; corner++)
                {
                    ...
                    {
                        m_lineRenderer
                            ->Add_Line(Vector2(start.x, start.y), Vector2(end.x, end.y), BOUNDING_BOX_LINE_WIDTH, color);
                    }
                }
...
void DrawObject::addCircleToLineRenderer(
...
void DrawObject::Render(RenderInfoClass &rinfo)
{
    ...
    if (m_drawObjects || m_drawWaypoints || m_drawBoundingBoxes || m_drawSightRanges || m_drawWeaponRanges
        || m_drawSoundRanges || m_drawTestArtHighlight)
    {
...
void BuildRectFromSegmentAndWidth(
    const Coord3D *start,
    const Coord3D *end,
    Real width,
    Coord3D *outBL,
