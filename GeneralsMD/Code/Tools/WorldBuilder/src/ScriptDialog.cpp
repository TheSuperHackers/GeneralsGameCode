...
ScriptDialog::ScriptDialog(CWnd *pParent /*=NULL*/) : CDialog(ScriptDialog::IDD, pParent)
{
...
HTREEITEM ScriptDialog::findItem(ListType sel, Bool failSafe)
{
    ...
    for (script = pTree->GetChildItem(group); script != NULL; script = pTree->GetNextSiblingItem(script))
    {
        ...
        DEBUG_ASSERTCRASH(
            lt.m_objType == ListType::SCRIPT_IN_PLAYER_TYPE || lt.m_objType == ListType::SCRIPT_IN_GROUP_TYPE,
            ("Not script"));
...
void ScriptDialog::OnSave()
{
    ...
    CFileDialog fileDlg(false, ".scb", NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, "Script files (.scb)|*.scb||", this);
...
{
    CFileDialog fileDlg(true, ".scb", NULL, 0, "Script files (.scb)|*.scb||", this);
        ...
        try
        {
            ...
            for (i = 0; i < count; i++)
            {
                ...
                if (curSide >= m_sides.getNumSides())
                {
                    curSide = 0;
                    ::AfxMessageBox(
                        "Imported scripts came from more players than exist in this map.  Additional scripts moved to "
                        "Neutral player.");
                }
...
Bool ScriptDialog::ParseObjectDataChunk(DataChunkInput &file, DataChunkInfo *info, void *userData)
{
    ...
    pThisOne = newInstance(MapObject)(loc, name, angle, flags, &d, TheThingFactory->findTemplate(name));
...
Bool ScriptDialog::ParseTeamsDataChunk(DataChunkInput &file, DataChunkInfo *info, void *userData)
{
    ...
    while (!file.atEndOfChunk())
    {
        ...
        else
        {
            AsciiString warning;
            warning.format(
                "Importing team %s of player %s.  Player %s doesn't exist, Select player..",
                teamName.str(),
                player.str(),
                player.str());
...
Bool ScriptDialog::ParsePolygonTriggersDataChunk(DataChunkInput &file, DataChunkInfo *info, void *userData)
{
    ...
    while (count > 0)
    {
        ...
        for (pCurrentTrigger = PolygonTrigger::getFirstPolygonTrigger(); pCurrentTrigger;
             pCurrentTrigger = pCurrentTrigger->getNext())
        {
...
void ScriptDialog::doDropOn(HTREEITEM hDrag, HTREEITEM hTarget)
{
    ...
    if (pScript)
    {
        ...
        if (drag.m_objType == target.m_objType && drag.m_playerIndex == target.m_playerIndex
            && drag.m_groupIndex == target.m_groupIndex && drag.m_scriptIndex < target.m_scriptIndex)
        {
    ...
    else if (drag.m_objType == ListType::GROUP_TYPE)
    {
        ...
        pSL->deleteGroup(pGroup);
        if (drag.m_objType != ListType::SCRIPT_IN_PLAYER_TYPE && drag.m_playerIndex == target.m_playerIndex
