...
public:
	...
	virtual Int iterateDrawablesInRegion(
	...
	virtual void screenToWorld(const ICoord2D *s, Coord3D *w) {}; ///< Transform screen coordinate "s" into world coordinate
																																///< "w"
	virtual void screenToTerrain(const ICoord2D *screen, Coord3D *world) {}; ///< transform screen coord to a point on the 3D
																																					 ///< terrain
	virtual void screenToWorldAtZ(const ICoord2D *s, Coord3D *w, Real z) {}; ///< transform screen point to world point at the
																																					 ///< specified world Z value
	...
	virtual void resetCamera(const Coord3D *location, Int frames, Real easeIn, Real easeOut) {}; ///< Move camera to location,
																																															 ///< and reset to default
																																															 ///< angle & zoom.
	virtual void rotateCamera(Real rotations, Int frames, Real easeIn, Real easeOut) {}; ///< Rotate camera about current
																																											 ///< viewpoint.
	...
	virtual void cameraModRollingAverage(Int framesToAverage) {}; ///< Number of frames to average movement for current camera
																																///< movement.
	virtual void cameraModFinalTimeMultiplier(Int finalMultiplier) {}; ///< Final time multiplier for current camera movement.
	virtual void cameraModFinalPitch(Real finalPitch, Real easeIn, Real easeOut) {}; ///< Final pitch for current camera
																																									 ///< movement.
	...
	virtual void setLocation(const ViewLocation *location) {}; ///< set the view's current location from to the view location
																														 ///< object
	...
	virtual void rotateCameraTowardPosition(
			const Coord3D *pLoc,
			Int milliseconds,
			Real easeIn,
			Real easeOut,
			Bool reverseRotation) {}; ///< Rotate camera to face an object, and hold on it
		...
		m_showSoundCircles(false)
{
	...
	TheWritableGlobalData->m_use3WayTerrainBlends =
			(::AfxGetApp()->GetProfileInt(MAIN_FRAME_SECTION, "ShowExtraBlends", 1) > 1 ? 2 : 1);
...
static Real getHeightAroundPos(WBHeightMap *heightMap, Real x, Real y)
{
	...
	terrainHeightMax =
			max(terrainHeightMax, heightMap->getHeightMapHeight(x + TERRAIN_SAMPLE_SIZE, y - TERRAIN_SAMPLE_SIZE, NULL));
	terrainHeightMax =
			max(terrainHeightMax, heightMap->getHeightMapHeight(x - TERRAIN_SAMPLE_SIZE, y - TERRAIN_SAMPLE_SIZE, NULL));
	terrainHeightMax =
			max(terrainHeightMax, heightMap->getHeightMapHeight(x + TERRAIN_SAMPLE_SIZE, y + TERRAIN_SAMPLE_SIZE, NULL));
				...
				if (md)
				{
					...
					if (m_heightMapRenderObj)
					{
						...
						TheTerrainRenderObject
								->addTree((DrawableID)(Int)pMapObj, pos, scale, pMapObj->getAngle(), 0.0f /*no random scaling*/, md);
					}
				}
			}
		}
...
void WbView3d::updateFenceListObjects(MapObject *pObject)
{
	...
	for (pMapObj = pObject; pMapObj; pMapObj = pMapObj->getNext())
	{
		...
		if (!renderObj)
		{
			...
			if (renderObj == NULL && modelName.isEmpty() == FALSE && strncmp(modelName.str(), "No ", 3))
			{
...
Bool WbView3d::docToViewCoords(Coord3D curPt, CPoint *newPt)
{
	...
	//
	Int sx, sy;
	W3DLogicalScreenToPixelScreen(screen.X, screen.Y, &sx, &sy, rClient.right - rClient.left, rClient.bottom - rClient.top);
...
int WbView3d::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	AfxGetApp()->WriteProfileInt("GameOptions", "cloudMap", TheGlobalData->m_useCloudMap); // Just in case it wasn't already
																																												 // there
...
void WbView3d::drawLabels(HDC hdc)
{
		...
		for (pMapObj = MapObject::getFirstMapObject(); pMapObj; pMapObj = pMapObj->getNext())
		{
			...
			if (pMapObj->isWaypoint() && m_showWaypoints)
			{
				...
				pos.z = m_heightMapRenderObj->getHeightMapHeight(pos.x, pos.y, NULL);
					...
					pMapObj->getThingTemplate() && !(pMapObj->getFlags() & (FLAG_ROAD_FLAGS | FLAG_BRIDGE_FLAGS))
	...
	{ // Draw Lines to indicate the direction of each light source
		...
		for (Int lIndex = 0; lIndex < MAX_GLOBAL_LIGHTS; lIndex++)
		{
			...
			worldEnd.Set(
					selectedPos.x - m_lightDirection[lIndex].x * selectedRadius,
...
void WbView3d::OnViewShowSoftWater()
{
	...
	{ // we just turned it on, so recompute shoreline tiles since they may not exist.
		TheTerrainRenderObject->updateShorelineTiles(
				0,
				0,
				WbDoc()->GetHeightMap()->getXExtent() - 1,
				WbDoc()->GetHeightMap()->getYExtent() - 1,
				WbDoc()->GetHeightMap());
	}
