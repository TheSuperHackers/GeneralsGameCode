...
public:
  ...
  WorldToScreenReturn worldToScreenTriReturn( const Coord3D *w, ICoord2D *s ) override { return WTS_INVALID; };	///< Transform world coordinate "w" into screen coordinate "s"
	...
	void moveCameraTo(const Coord3D *o, Int frames, Int shutter,
														Bool orient, Real easeIn, Real easeOut) override {lookAt(o);};
	void moveCameraAlongWaypointPath(Waypoint *way, Int frames, Int shutter,
														Bool orient, Real easeIn, Real easeOut) override {};
	Bool isCameraMovementFinished() override {return true;};
	void resetCamera(const Coord3D *location, Int frames, Real easeIn, Real easeOut) override {}; ///< Move camera to location, and reset to default angle & zoom.
	void rotateCamera(Real rotations, Int frames, Real easeIn, Real easeOut) override {}; ///< Rotate camera about current viewpoint.
	void rotateCameraTowardObject(ObjectID id, Int milliseconds, Int holdMilliseconds, Real easeIn, Real easeOut) override {};	///< Rotate camera to face an object, and hold on it
	void cameraModFinalZoom(Real finalZoom, Real easeIn, Real easeOut) override {};			 ///< Final zoom for current camera movement.
	...
	void cameraModFinalPitch(Real finalPitch, Real easeIn, Real easeOut) override {};		 ///< Final pitch for current camera movement.
	...
	void zoomCamera( Real finalZoom, Int milliseconds, Real easeIn, Real easeOut ) override {};
	void pitchCamera( Real finalPitch, Int milliseconds, Real easeIn, Real easeOut ) override {};
