...
void MapObjectProps::_DictToPrebuiltUpgrades(void)
{
    ...
    do
    {
        ...
        if (exists)
        {
            ...
            {
                DEBUG_CRASH(
                    ("Object claims '%s', but it wasn't found in the list of possible upgrades.", upgradeString.str()));
                ++upgradeNum;
                continue;
            }
            pBox->SetSel(selNdx);
...
void MapObjectProps::clearCustomizeFlag(CWorldBuilderDoc *pDoc, MultipleUndoable *ownerUndoable)
{
    ...
    pUndo = new DictItemUndoable(
        getAllSelectedDictsData(),
        empty,
        TheKey_objectSoundAmbientMaxRange,
        m_allSelectedDicts.size(),
        pDoc,
        false);
    ownerUndoable->addUndoable(pUndo);
    ...
    pUndo = new DictItemUndoable(
        getAllSelectedDictsData(),
        empty,
        TheKey_objectSoundAmbientLooping,
        m_allSelectedDicts.size(),
        pDoc,
        false);
    ownerUndoable->addUndoable(pUndo);
    ...
    pUndo = new DictItemUndoable(
        getAllSelectedDictsData(),
        empty,
        TheKey_objectSoundAmbientLoopCount,
        m_allSelectedDicts.size(),
        pDoc,
        false);
    ownerUndoable->addUndoable(pUndo);
...
void MapObjectProps::attachedSoundToDict(void)
{
    ...
    if (pDoc != NULL)
    {
        ...
        DictItemUndoable *pDictUndo = new DictItemUndoable(
            getAllSelectedDictsData(),
            newDict,
            TheKey_objectSoundAmbient,
            m_allSelectedDicts.size(),
            pDoc,
            true);
...
void MapObjectProps::dictToLoopCount()
{
    ...
    CButton *customizeCheckbox = (CButton *)GetDlgItem(IDC_CUSTOMIZE_CHECKBOX);
    CButton *loopingCheckbox = (CButton *)GetDlgItem(IDC_LOOPING_CHECKBOX);
    if (customizeCheckbox == NULL || customizeCheckbox->GetCheck() == 0 || loopingCheckbox == NULL
        || loopingCheckbox->GetCheck() == 0)
    {
...
void MapObjectProps::dictToEnabled()
{
    ...
    if (currentString == NO_SOUND_STRING || (soundIndex == m_defaultEntryIndex && m_defaultIsNone))
    {
        enableCheckbox->SetCheck(0);
        enableCheckbox->EnableWindow(false);
        return;
