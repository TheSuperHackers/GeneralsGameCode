...
static void SpitLights()
{
    ...
    for (; time < 4; time++)
    {
        for (Int light = 0; light < 3; light++)
        {
            ...
            DEBUG_LOG(
                ("TerrainObjectsLighting%sAmbient%s = R:%d G:%d B:%d", times[time], lights[light], redA, greenA, blueA));
            DEBUG_LOG(
                ("TerrainObjectsLighting%sDiffuse%s = R:%d G:%d B:%d", times[time], lights[light], redD, greenD, blueD));
    ...
    for (time = 0; time < 4; time++)
    {
        for (Int light = 0; light < 3; light++)
        {
            ...
            DEBUG_LOG(
                ("TheGlobalData->m_terrainLighting[%d][%d].ambient.green = %0.4ff;", theTime, light, tl.ambient.green));
            ...
            DEBUG_LOG(
                ("TheGlobalData->m_terrainLighting[%d][%d].diffuse.green = %0.4ff;", theTime, light, tl.diffuse.green));
            ...
            DEBUG_LOG(
                ("TheGlobalData->m_terrainObjectsLighting[%d][%d].ambient.red = %0.4ff;", theTime, light, tl.ambient.red));
            DEBUG_LOG(
                ("TheGlobalData->m_terrainObjectsLighting[%d][%d].ambient.green = %0.4ff;",
                 theTime,
                 light,
                 tl.ambient.green));
            DEBUG_LOG(
                ("TheGlobalData->m_terrainObjectsLighting[%d][%d].ambient.blue = %0.4ff;", theTime, light, tl.ambient.blue));

            DEBUG_LOG(
                ("TheGlobalData->m_terrainObjectsLighting[%d][%d].diffuse.red = %0.4ff;", theTime, light, tl.diffuse.red));
            DEBUG_LOG(
                ("TheGlobalData->m_terrainObjectsLighting[%d][%d].diffuse.green = %0.4ff;",
                 theTime,
                 light,
                 tl.diffuse.green));
            DEBUG_LOG(
                ("TheGlobalData->m_terrainObjectsLighting[%d][%d].diffuse.blue = %0.4ff;", theTime, light, tl.diffuse.blue));

            DEBUG_LOG(
                ("TheGlobalData->m_terrainObjectsLighting[%d][%d].lightPos.x = %0.4ff;", theTime, light, tl.lightPos.x));
            DEBUG_LOG(
                ("TheGlobalData->m_terrainObjectsLighting[%d][%d].lightPos.y = %0.4ff;", theTime, light, tl.lightPos.y));
            DEBUG_LOG(
                ("TheGlobalData->m_terrainObjectsLighting[%d][%d].lightPos.z = %0.4ff;", theTime, light, tl.lightPos.z));
