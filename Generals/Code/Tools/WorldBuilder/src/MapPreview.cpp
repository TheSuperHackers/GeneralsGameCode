...
void MapPreview::interpolateColorForHeight(RGBColor *color, Real height, Real hiZ, Real midZ, Real loZ)
{
...
void MapPreview::buildMapPreviewTexture(CString tgaName)
{
	...
	for (y = 0; y < MAP_PREVIEW_HEIGHT; y++)
	{
		for (x = 0; x < MAP_PREVIEW_WIDTH; x++)
		{
					...
					if (j >= 0 && j < MAP_PREVIEW_HEIGHT)
					{
						for (i = x - samplesAway; i <= x + samplesAway; i++)
						{
							if (i >= 0 && i < MAP_PREVIEW_WIDTH)
							{
								...
								interpolateColorForHeight(&color, z, maxHeight, getTerrainAverageZ, minHeight);
			...
			//
			m_pixelBuffer[y][x] = 255 | (REAL_TO_INT(color.red * 255) << 8) | (REAL_TO_INT(color.green * 255) << 16)
