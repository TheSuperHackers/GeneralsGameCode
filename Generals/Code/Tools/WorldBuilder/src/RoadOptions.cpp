...
void RoadOptions::SelectConnected(void)
{
    ...
    while (changed)
    {
        ...
        for (std::list<MapObject *>::iterator it = roadSegs.begin(); it != roadSegs.end(); ++it)
        {
            ...
            for (std::list<MapObject *>::iterator connected = connectedSegs.begin(); connected != connectedSegs.end();
                 ++connected)
            {
                ...
                if (qd1 < MAP_XY_FACTOR / 100 || qd2 < MAP_XY_FACTOR / 100 || qd3 < MAP_XY_FACTOR / 100
                    || qd4 < MAP_XY_FACTOR / 100)
                {
