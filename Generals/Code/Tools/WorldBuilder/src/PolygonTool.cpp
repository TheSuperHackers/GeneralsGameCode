...
/// Constructor
PolygonTool::PolygonTool(void) : Tool(ID_POLYGON_TOOL, IDC_POLYGON), m_poly_plusCursor(NULL), m_poly_moveCursor(NULL)
{
}
...
void PolygonTool::startMouseDown(TTrackingMode m, CPoint viewPt, WbView *pView, CWorldBuilderDoc *pDoc)
{
    ...
    else
    {
        ...
        else if (m_poly_dragPointNdx != -1)
        {
            ...
            ModifyPolygonPointUndoable *pUndo =
                new ModifyPolygonPointUndoable(m_poly_curSelectedPolygon, m_poly_dragPointNdx);
            pDoc->AddAndDoUndoable(pUndo);
            REF_PTR_RELEASE(pUndo); // belongs to pDoc now.
        }
        ...
        else
        {
            ...
            InsertPolygonPointUndoable *pUndo =
                new InsertPolygonPointUndoable(m_poly_curSelectedPolygon, iDocPt, m_poly_dragPointNdx);
            pDoc->AddAndDoUndoable(pUndo);
            REF_PTR_RELEASE(pUndo); // belongs to pDoc now.
        }
    }
