...
static void loadSchemeDataToDialog(HWND hWndDialog)
{
	...
	SetDlgItemText(hWndDialog, STATIC_CURRENT_SCHEME, theScheme->getSchemeFilename());

	// load the state combo box with every option available
	SendDlgItemMessage(hWndDialog, COMBO_STATE, CB_RESETCONTENT, 0, 0);
	LoadStateCombo(
			GWS_PUSH_BUTTON | GWS_RADIO_BUTTON | GWS_CHECK_BOX | GWS_PROGRESS_BAR | GWS_SCROLL_LISTBOX | GWS_HORZ_SLIDER
					| GWS_VERT_SLIDER | GWS_STATIC_TEXT | GWS_ENTRY_FIELD | GWS_USER_WINDOW | GWS_COMBO_BOX,
	...
	for (i = FIRST_VALID_IDENTIFIER; i < NUM_STATE_IDENTIFIERS; i++)
	{
		...
		assert(info);
		StoreImageAndColor(info->stateID, info->image, info->color, info->borderColor);
...
static void saveData(HWND hWndDialog)
{
	...
	for (i = FIRST_VALID_IDENTIFIER; i < NUM_STATE_IDENTIFIERS; i++)
	{
		...
		assert(info);
		theScheme->storeImageAndColor(info->stateID, info->image, info->color, info->borderColor);
...
//=============================================================================
static LRESULT CALLBACK layoutSchemeCallback(HWND hWndDialog, UINT message, WPARAM wParam, LPARAM lParam)
{
	Int returnCode;
		...
		case WM_COMMAND:
		{
			...
			switch (controlID)
			{
				...
				case BUTTON_APPLY_SCHEME:
				{
					...
					result = MessageBox(
							NULL,
							"This will apply these scheme color and image settings to ALL windows and gadgets currently loaded in the "
							"edit window.  Are you sure you want to proceed?",
							"Are You Sure?",
				...
				case BUTTON_LOAD:
				{
					...
					if (theScheme && filename)
					{
						...
						{
							MessageBox(TheEditor->getWindowHandle(), "Unable to open scheme file.", "Error", MB_OK);
...
void LayoutScheme::openDialog(void)
{
	...
	DialogBox(
			...
			(LPCTSTR)LAYOUT_SCHEME_DIALOG,
			TheEditor->getWindowHandle(),
			(DLGPROC)layoutSchemeCallback);
...
//=============================================================================
void LayoutScheme::storeImageAndColor(StateIdentifier id, const Image *image, Color color, Color borderColor)
{
	// sanity
	if (id < 0 || id >= NUM_STATE_IDENTIFIERS)
	{
...
{
	FILE *fp;
	UnsignedByte colorR, colorG, colorB, colorA, bColorR, bColorG, bColorB, bColorA;
	...
	if (fp == NULL)
	{
		...
		MessageBox(TheEditor->getWindowHandle(), "Unable to open scheme for for saving.  Read only?", "Save Error", MB_OK);
		return FALSE;
	...
	char fontBuffer[256];
	if (m_font)
		sprintf(fontBuffer, "\"%s\" Size: %d Bold: %d", m_font->nameString.str(), m_font->pointSize, m_font->bold);
	else
	...
	for (Int i = 0; i < NUM_STATE_IDENTIFIERS; i++)
	{
		...
		GameGetColorComponents(info->borderColor, &bColorR, &bColorG, &bColorB, &bColorA);

		// output it
		fprintf(
				fp,
				"%d: Image: %s Color: (%d,%d,%d,%d) Border: (%d,%d,%d,%d)\n",
				i,
				info->image ? info->image->getName().str() : "NONE",
				colorR,
				colorG,
				colorB,
				colorA,
...
Bool LayoutScheme::loadScheme(char *filename)
{
	...
	if (fscanf(fp, "Window Layout Scheme: Version '%d'\n", &version) == 1)
	{
		if (version != SCHEME_VERSION)
		{
			...
			MessageBox(TheEditor->getWindowHandle(), "Old layout version, cannot open.", "Old File", MB_OK);
			return FALSE;
	...
	if (fscanf(fp, "Number of states: %i\n", &numStates) == 1)
	{
		for (Int i = 0; i < numStates; i++)
		{
			...
			if (fscanf(
							fp,
							"%d: Image: %s Color: (%hhu,%hhu,%hhu,%hhu) Border: (%hhu,%hhu,%hhu,%hhu)\n",
							&state,
							imageBuffer,
							&colorR,
							&colorG,
