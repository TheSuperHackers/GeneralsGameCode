...
{
	if (strcmp(token, "ENABLEDDRAWDATA") == 0 || strcmp(token, "LISTBOXENABLEDUPBUTTONDRAWDATA") == 0
			|| strcmp(token, "LISTBOXENABLEDDOWNBUTTONDRAWDATA") == 0 || strcmp(token, "LISTBOXENABLEDSLIDERDRAWDATA") == 0
			|| strcmp(token, "COMBOBOXDROPDOWNBUTTONENABLEDDRAWDATA") == 0 || strcmp(token, "COMBOBOXEDITBOXENABLEDDRAWDATA") == 0
			|| strcmp(token, "COMBOBOXLISTBOXENABLEDDRAWDATA") == 0 || strcmp(token, "SLIDERTHUMBENABLEDDRAWDATA") == 0)
		return TRUE;
...
{
	if (strcmp(token, "DISABLEDDRAWDATA") == 0 || strcmp(token, "LISTBOXDISABLEDUPBUTTONDRAWDATA") == 0
			|| strcmp(token, "LISTBOXDISABLEDDOWNBUTTONDRAWDATA") == 0 || strcmp(token, "LISTBOXDISABLEDSLIDERDRAWDATA") == 0
			|| strcmp(token, "COMBOBOXDROPDOWNBUTTONDISABLEDDRAWDATA") == 0
			|| strcmp(token, "COMBOBOXEDITBOXDISABLEDDRAWDATA") == 0 || strcmp(token, "COMBOBOXLISTBOXDISABLEDDRAWDATA") == 0
...
{
	if (strcmp(token, "HILITEDRAWDATA") == 0 || strcmp(token, "LISTBOXHILITEUPBUTTONDRAWDATA") == 0
			|| strcmp(token, "LISTBOXHILITEDOWNBUTTONDRAWDATA") == 0 || strcmp(token, "LISTBOXHILITESLIDERDRAWDATA") == 0
			|| strcmp(token, "COMBOBOXLISTBOXHILITEDRAWDATA") == 0 || strcmp(token, "COMBOBOXEDITBOXHILITEDRAWDATA") == 0
			|| strcmp(token, "COMBOBOXDROPDOWNBUTTONHILITEDRAWDATA") == 0 || strcmp(token, "SLIDERTHUMBHILITEDRAWDATA") == 0)
		return TRUE;
	...
	for (i = 0; i < MAX_DRAW_DATA; i++)
	{
		...
		if (i == 0)
			sprintf(
					...
					token,
					image ? image->getName().str() : "NoImage",
					r,
					g,
					b,
					a,
...
static Bool saveListboxData(GameWindow *window, FILE *fp, Int dataIndent)
{
	...
	if (listData == NULL)
	{
		DEBUG_LOG(("No listbox data to save for window '%d'", window->winGetWindowId()));
		assert(0);
		return FALSE;
...
static Bool saveComboBoxData(GameWindow *window, FILE *fp, Int dataIndent)
{
	...
	if (comboData == NULL)
	{
		DEBUG_LOG(("No comboData data to save for window '%d'", window->winGetWindowId()));
		assert(0);
		return FALSE;
...
static Bool saveRadioButtonData(GameWindow *window, FILE *fp, Int dataIndent)
{
	...
	if (radioData == NULL)
	{
		DEBUG_LOG(("No radio button data to save for window '%d'", window->winGetWindowId()));
		assert(0);
		return FALSE;
...
static Bool saveSliderData(GameWindow *window, FILE *fp, Int dataIndent)
{
	...
	if (sliderData == NULL)
	{
		DEBUG_LOG(("No slider data in window to save for window %d", window->winGetWindowId()));
		assert(0);
		return FALSE;
...
static Bool saveStaticTextData(GameWindow *window, FILE *fp, Int dataIndent)
{
	...
	if (textData == NULL)
	{
		DEBUG_LOG(("No text data in window to save for window %d", window->winGetWindowId()));
		assert(0);
		return FALSE;
...
static Bool saveTextEntryData(GameWindow *window, FILE *fp, Int dataIndent)
{
	...
	if (entryData == NULL)
	{
		DEBUG_LOG(("No text entry data in window to save for window %d", window->winGetWindowId()));
		assert(0);
		return FALSE;
...
static Bool saveGadgetData(GameWindow *window, FILE *fp, Int dataIndent)
{
	...
	else if (BitIsSet(window->winGetStyle(), GWS_VERT_SLIDER | GWS_HORZ_SLIDER))
...
void GUIEdit::validateNames(GameWindow *root, char *filename, Bool *valid)
{
	...
	if (strlen(filename) >= MAX_WINDOW_NAME_LEN)
	{
		sprintf(
				offendingNames,
				"Filename '%s' is too long.  The max is '%d' for filename AND window name!\n",
				filename,
				MAX_WINDOW_NAME_LEN);
		*valid = FALSE;
		return;
	...
	if (TheEditor->isNameDuplicate(TheWindowManager->winGetWindowList(), root, instData->m_decoratedNameString))
	{
...
Bool GUIEdit::saveData(char *filePathAndFilename, char *filename)
{
	...
	sprintf(
			offendingNames,
			"\nOne or more window names are illegal.  A window name PLUS the filename must be under '%d' characters and names "
