...
//=============================================================================
static LRESULT CALLBACK listboxPropertiesCallback(HWND hWndDialog, UINT message, WPARAM wParam, LPARAM lParam)
{
	Int returnCode;
					...
					if (window)
					{
						...
						if (newColumns > 1)
						{
							...
							if (strlen(percentages) == 0)
							{
								MessageBox(
										NULL,
										"You have specified a column amount greater then 1, please enter the same about of percentages",
										"whoops",
							...
							while (token != NULL)
							{
								...
								if (i > newColumns && token)
								{
									...
									sprintf(
											whoopsMsg,
											"You have Specified %d columns but I have read in more then that for the percentages, please double "
											"check your data",
											newColumns);
								...
								else if (i < newColumns && !token)
								{
									...
									sprintf(
											whoopsMsg,
											"You have Specified %d columns but I have read in only %d for the percentages, please double check "
											"your data",
											newColumns,
											i);
									MessageBox(NULL, whoopsMsg, "Whoops", MB_OK | MB_ICONSTOP | MB_APPLMODAL);
