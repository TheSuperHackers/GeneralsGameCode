...
protected:
	...
	void renderObjectList(const RadarObject *listHead, TextureClass *texture, Bool calcHero = FALSE); ///< render an object
																																																		///< list to the texture
	void interpolateColorForHeight(
			RGBColor *color,
			Real height,
			Real hiZ,
			Real midZ,
	...
	void radarToPixel(
			...
			Int radarUpperLeftY,
			Int radarWidth,
			Int radarHeight); ///< convert radar coord to pixel location
