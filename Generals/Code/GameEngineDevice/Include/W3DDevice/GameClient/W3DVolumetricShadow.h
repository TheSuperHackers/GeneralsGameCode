...
public:
	...
	void reset(void);
	W3DVolumetricShadow *addShadow(RenderObjClass *robj, Shadow::ShadowTypeInfo *shadowInfo, Drawable *draw); ///< adds shadow
																																																						///< caster to
																																																						///< rendering
																																																						///< system.
...
protected:
	...
	void updateOptimalExtrusionPadding(void); ///< for immobile objects, figure out the length of extrusion needed to hit
																						///< ground - expensive!
	...
	void updateMeshVolume(
			Int meshIndex,
			Int lightIndex,
			const Matrix3D *meshXform,
			const AABoxClass &meshBox,
			float floorZ); ///< update shadow volume of this mesh.
	...
	void RenderMeshVolumeBounds(Int meshIndex, Int lightIndex, const Matrix3D *meshXform); ///< render bounding volume around
																																												 ///< shadow volume - for debug use.

	void setLightPosHistory(Int lightIndex, Int meshIndex, Vector3 &pos)
	{
		m_lightPosHistory[lightIndex][meshIndex] = pos;
	...
	Real m_extraExtrusionPadding; ///< amount to extrude shadow beyond where it would normally stop (ground level below
																///< object).
