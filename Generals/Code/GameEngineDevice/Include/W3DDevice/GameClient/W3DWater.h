...
class WaterRenderObjClass : public Snapshot, public RenderObjClass
{
  ...
  void changeGridHeight(
      Real x,
      Real y,
      Real delta); ///< change height of grid at world point including neighbors within falloff.
  void setGridChangeAttenuationFactors(
      Real a,
      Real b,
      Real c,
      Real range); ///< adjust falloff parameters for grid change method.
  ...
  inline Bool worldToGridSpace(
      Real worldX,
      Real worldY,
      Real &gridX,
      Real &gridY); ///< convert from world coordinates to grid's local coordinate system.
