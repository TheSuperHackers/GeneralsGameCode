...
void StatDumpClass::dumpStats()
{
  ...
  fprintf(
      m_fp,
      "Draws: %d Skins: %d SortedPolys: %d SkinPolys: %d\n",
  ...
  fprintf(
      m_fp,
      "  3-Way Blends: %d, Shoreline Blends: %d\n",
...
void W3DDisplay::gatherDebugStats(void)
{
  ...
  if (s_timeSinceLastUpdateInSecs >= UPDATE_RATE_SECS || TheGlobalData->m_constantDebugUpdate)
  {
    ...
    unibuffer2.format(
        L"%.2fms [cumuFPS=%.2f] draws: %d skins: %d sortP: %d skinP: %d",
        ms,
        cumuFPS,
        (Int)drawsPerFrame,
    ...
    // terrain stats
    unibuffer.format(
        L"3-Way Blends: %d, Shoreline Blends: %d",
        TheTerrainRenderObject->getNumExtraBlendTiles(),
        TheTerrainRenderObject->getNumShoreLineTiles());
