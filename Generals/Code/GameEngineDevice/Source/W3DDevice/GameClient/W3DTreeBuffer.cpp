    ...
    for (curTree = 0; curTree < m_numTrees; curTree++)
    {
        ...
        if (doPanel)
        {
            ...
            for (j = 0; j < 6; j++)
            {
                ...
                {
                    curVb->diffuse =
                        doLighting(&vLoc, shadeR, shadeG, shadeB, m_trees[curTree].bounds, pDynamicLightsIterator);
                }
                else
                {
...
{
    m_vertexTree =
        NEW_REF(DX8VertexBufferClass, (DX8_FVF_XYZDUV1, MAX_TREE_VERTEX + 4, DX8VertexBufferClass::USAGE_DYNAMIC));
