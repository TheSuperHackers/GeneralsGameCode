...
Bool W3DProjectedShadowManager::ReAcquireResources(void)
{
    ...
    if ((m_dynamicRenderTarget =
             DX8Wrapper::Create_Render_Target(DEFAULT_RENDER_TARGET_WIDTH, DEFAULT_RENDER_TARGET_HEIGHT, true))
        == NULL)
    {
        m_renderTargetHasAlpha = FALSE;
    ...
    if (TheTerrainRenderObject)
    {
        ...
        { // flush the buffer by drawing the contents and re-locking again
            if (shadowVertexBufferD3D
                    ->Lock(0, numVerts * sizeof(SHADOW_VOLUME_VERTEX), (unsigned char **)&pvVertices, D3DLOCK_DISCARD)
                != D3D_OK)
                return 0;
            nShadowVertsInBuf = 0;
        ...
        { // flush the buffer by drawing the contents and re-locking again
            if (shadowIndexBufferD3D->Lock(0, numIndex * sizeof(short), (unsigned char **)&pvIndices, D3DLOCK_DISCARD)
                != D3D_OK)
                return 0;
...
void W3DProjectedShadowManager::queueDecal(W3DProjectedShadow *shadow)
{
    ...
    if (TheTerrainRenderObject)
    {
        ...
        */
        ...
        {	//use the object's bounding box to determine shadow extent
            ...
            Real newExtentX = fabs(uVector * box.Extent) + fabs(uVector * box.Center);	//get new extent for object
    orientation Real newExtentY = fabs(vVector * box.Extent) + fabs(vVector * box.Center);  //get new extent for object
    orientation
    ...
    */
        ...
        { // flush the buffer by drawing the contents and re-locking again
            ...
            if (shadowDecalVertexBufferD3D
                    ->Lock(0, numVerts * sizeof(SHADOW_DECAL_VERTEX), (unsigned char **)&pvVertices, D3DLOCK_DISCARD)
                != D3D_OK)
                return;
        ...
        // code to deal with rotated shadows based on sun direction, fix this later.  For now shadow rotates with object
        // rotation. shadow->m_shadowTexture[0]->getDecalUVAxis(&uVector,&vVector); uVector
        // *= 20.0f/dx;//shadow->m_decalRadius;	//scale texture to fit object vVector
        // *= 20.0f/dy;//1.2f;//shadow->m_decalRadius;	//scale texture to fit object
        /*		uVector=objXform.Get_X_Vector();
                uVector.Normalize();
                uVector /= decalSizeX + (1.0f+4.0f/64.0f);	//texture has 1 pixel transparent border so we strtech up to make
           sure solid pixels reach extent.. vVector=objXform.Get_Y_Vector() * -1.0f;	//invert direction since v axis runs
           right relative to u. vVector.Normalize(); vVector /= decalSizeY + (1.0f+4.0f/64.0f);
        */
        ...
        { // flush the buffer by drawing the contents and re-locking again
            ...
            if (shadowDecalIndexBufferD3D->Lock(0, numIndex * sizeof(short), (unsigned char **)&pvIndices, D3DLOCK_DISCARD)
                != D3D_OK)
                return;
        ...
        for (shadow = m_shadowList; shadow; shadow = shadow->m_next)
        {
            if (shadow->m_isEnabled && !shadow->m_isInvisibleEnabled)
            {
                if (shadow->m_type & SHADOW_DECAL)
                {
                    ...
                    if (shadow->m_shadowTexture[0] != lastShadowDecalTexture || shadow->m_type != lastShadowType)
                    {
                        flushDecals(
                            lastShadowDecalTexture,
                            lastShadowType); // switched to a new texture, need to render polys using last texture.
                    ...
                    for (obj = iter->first(); obj; obj = iter->next())
                    {
                        ...
                        for (DrawModule **dm = draw->getDrawModules(); *dm; ++dm)
                        {
                            ...
                            if (di)
                            {
                                ...
                                ///@todo: don't apply shadows to translcuent objects unless they are MOBILE - hack to get
                                ///tanks to work.
                                if ((robj = w3dDraw->getRenderObject()) != 0
                                    && (!robj->Is_Alpha() || !obj->isKindOf(KINDOF_IMMOBILE)) && robj != shadow->m_robj
                                    && robj->Is_Really_Visible())
                                {
...
Shadow *W3DProjectedShadowManager::addDecal(Shadow::ShadowTypeInfo *shadowInfo)
{
    ...
    shadow->m_allowWorldAlign =
        allowWorldAlign; /// wrap shadow around world geometry - else align perpendicular to local z-axis.
...
/** Generic function which can be used to create arbitrary decals that follow the renderObject but don't have to be used for
shadows. Some examples: Scorch marks, blood, stains, selection/status indicators, etc.*/
...
Int W3DShadowTexture::init(RenderObjClass *robj)
{
    ...
    TextureClass *new_texture =
        MSGNEW("TextureClass") TextureClass(surface_desc.Width, surface_desc.Height, surface_desc.Format, MIP_LEVELS_1);
