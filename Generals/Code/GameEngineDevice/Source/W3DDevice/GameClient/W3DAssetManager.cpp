...
//---------------------------------------------------------------------
W3DPrototypeClass::W3DPrototypeClass(RenderObjClass *proto, const AsciiString &name) : Proto(proto), Name(name)
{
    assert(Proto);
    Proto->Add_Ref();
}
...
static inline void Munge_Render_Obj_Name(
    char *newname,
    const char *oldname,
    float scale,
    const int color,
...
static void remapPalette16Bit(SurfaceClass::SurfaceDescription *sd, UnsignedShort *palette, unsigned int color)
{
    ...
    Vector3 rgb,
        v_color(
            (float)((color >> 16) & 0xff) / 255.0f / 255.0f,
...
//---------------------------------------------------------------------
static void remapTexture16Bit(
    Int dx,
    Int dy,
    Int pitch,
    SurfaceClass::SurfaceDescription *sd,
    UnsignedShort *palette,
    UnsignedShort *data,
    unsigned int color)
{
    UnsignedShort pal[TEAM_COLOR_PALETTE_SIZE];
...
static void remapAlphaTexture16Bit(
    Int dx,
    Int dy,
    Int pitch,
    SurfaceClass::SurfaceDescription *sd,
...
//---------------------------------------------------------------------
static void remapAlphaTexture32Bit(
    Int dx,
    Int dy,
    Int pitch,
    SurfaceClass::SurfaceDescription *sd,
    ...
    for (y = 0; y < dy; y++)
    {
        for (x = 0; x < dx; x++)
        {
            ...
            { // some house color needs to show through
                ...
                RGB_To_HSV(
                    hsv,
                    Vector3(((pixel >> 16) & 0xff) / 255.0f, ((pixel >> 8) & 0xff) / 255.0f, (pixel & 0xff) / 255.0f));
                hsv.X = hsv_color.X;
                hsv.Y *= hsv_color.Y;
                HSV_To_RGB(rgb, hsv);
#else
...
void W3DAssetManager::Report_Used_Textures(void)
{
    ...
    */
    ...
    for (ite.First(); !ite.Is_Done(); ite.Next())
    {
        ...
        if (tex->Num_Refs() <= 1)
        {
            /*		temp_textures[count++]=tex;
                    if (count==256) {
                        ...
                        count=0;
                        ite.First();	// iterator doesn't support modifying the hash table while iterating, so start from
               the
                                            // beginning.
                    }*/
        }
        ...
        {
            DEBUG_LOG(
                ("**Texture \"%s\" referenced %d times on map reset", tex->Get_Texture_Name().str(), tex->Num_Refs() - 1));
        }
    }
...
TextureClass * W3DAssetManager::Get_Texture_With_HSV_Shift(const char * filename, const Vector3 &hsv_shift, MipCountType
mip_level_count)
{
