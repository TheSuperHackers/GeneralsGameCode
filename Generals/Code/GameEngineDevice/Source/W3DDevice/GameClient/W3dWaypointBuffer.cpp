                    ...
                    if (exitInterface)
                    {
                                ...
                                FALSE)) // FALSE means "without the extra offset"
                        {
                            if (!naturalRallyPoint.equals(exitPoint))
                            {
                                points[numPoints].Set(
                                    Vector3(naturalRallyPoint.x, naturalRallyPoint.y, naturalRallyPoint.z));
                                numPoints++;
                            }
                            else
                            {
                        ...
                        if (rallyPoint)
                        {
                            if (boxWrap)
                            {
                                ...
                                // to do this we find the two corners of the geometry extents that are nearest the natural
                                // rally point
                                ...
                                // if the rallypoint is closer to the wayoutpoint than it is to the natural rally point then
                                // we definitely do not wrap
                                Coord3D rallyToWayOutDelta = wayOutPoint;
                                rallyToWayOutDelta.sub(rallyPoint);
                                if ((100.0f + rallyToWayOutDelta.length()) > wayOutLength)
                                {
                                    // if we passed the above idiot test, now lets be sure by testing the dotproduct of the
                                    // rp against the wayoutpoint
                                    ...
                                    if (dot > 0)
                                    {
                                        ...
                                        Coord2D *pNearElbow =
                                            NULL; // find the closest corner to the rallyPoint same end as door
                                        Coord2D *pFarElbow = NULL; // find the closest corner to the rallypoint away from
                                                                   // door
                                        ...
                                        if (pNearElbow) // did we find a nearest corner?
                                        {
                                            ...
                                            {
                                                // but let's test the dot of the first elbow against the rally point to find
                                                // out
