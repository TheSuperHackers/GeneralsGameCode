	...
	if (progress)
	{
		...
		if (barColor != WIN_COLOR_UNDEFINED)
		{
			...
			if (end.x - start.x > 1)
			{
				...
				TheWindowManager
						->winDrawLine(GameMakeColor(255, 255, 255, 255), WIN_DRAW_LINE_WIDTH, start.x, start.y, end.x, start.y);
				TheWindowManager
						->winDrawLine(GameMakeColor(200, 200, 200, 255), WIN_DRAW_LINE_WIDTH, start.x, start.y, start.x, end.y);
			}
...
{
	ICoord2D origin, size, start, end;
	const Image *backLeft, *backRight, *backCenter, *barRight, *barCenter; //*backSmallCenter,*barLeft,, *barSmallCenter;
	Int progress = (Int)window->winGetUserData();
	Int xOffset, yOffset;
	Int i;
	...
	// sanity
	if (backLeft == NULL || backRight == NULL || backCenter == NULL || barRight == NULL)
