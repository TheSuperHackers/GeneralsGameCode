...
void W3DGadgetPushButtonDraw(GameWindow *window, WinInstanceData *instData)
{
    ...
    {
        TheWindowManager->winOpenRect(border, WIN_DRAW_LINE_WIDTH, start.x, start.y, end.x, end.y);
    ...
    if (color != WIN_COLOR_UNDEFINED)
    {
        ...
        TheWindowManager->winFillRect(color, WIN_DRAW_LINE_WIDTH, start.x, start.y, end.x, end.y);
    ...
    if (pData)
    {
        ...
        if (pData->drawClock)
        {
            ...
            {
                TheDisplay
                    ->drawRemainingRectClock(origin.x, origin.y, size.x, size.y, pData->percentClock, pData->colorClock);
            }
...
void W3DGadgetPushButtonImageDraw(GameWindow *window, WinInstanceData *instData)
{
    ...
    if (GadgetButtonGetMiddleEnabledImage(window))
    {
        if (BitIsSet(instData->getState(), WIN_STATUS_USE_OVERLAY_STATES))
        {
            ...
            DEBUG_CRASH(
                ("Button at %d,%d is attempting to render with W3DGadgetPushButtonImageDrawThree(), but is using overlay "
                 "states! Forcing the code to use W3DGadgetPushButtonImageDrawOne() instead.",
                 start.x,
                 start.y));
