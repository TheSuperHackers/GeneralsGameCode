...
void W3DPowerDraw(GameWindow *window, WinInstanceData *instData)
{
  ...
  Real consumptionForNeedle =
      (consumption == 1) ? 1.5f : INT_TO_REAL(consumption); // Log(1) == 0, but we need to show something for 1 power used.
...
void W3DCommandBarBackgroundDraw(GameWindow *window, WinInstanceData *instData)
{
  ...
  {
    return;
    // win = TheWindowManager->winGetWindowFromId(NULL,TheNameKeyGenerator->nameToKey( AsciiString(
