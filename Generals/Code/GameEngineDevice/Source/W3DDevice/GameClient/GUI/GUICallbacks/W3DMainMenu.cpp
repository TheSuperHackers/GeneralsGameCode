...
void W3DMainMenuDraw(GameWindow *window, WinInstanceData *instData)
{
	...
	//	static IRegion2D	verticle5 ={pos.x + (size.x * .7250), pos.y + (size.y * .12), pos.x + (size.x * .7250), pos.y +
	//(size.y * .86) 	}; 	static IRegion2D	verticle6 ={pos.x + (size.x * .9062), pos.y + (size.y * .12), pos.x + (size.x *
	//.9062), pos.y + (size.y * .86) 	};

	TheDisplay->drawLine(topHorizontal1.lo.x, topHorizontal1.lo.y, topHorizontal1.hi.x, topHorizontal1.hi.y, 2, color);
	TheDisplay->drawLine(
			topHorizontal1drop.lo.x,
			topHorizontal1drop.lo.y,
			topHorizontal1drop.hi.x,
			topHorizontal1drop.hi.y,
			2,
			colorDrop);
	TheDisplay->drawLine(topHorizontal2.lo.x, topHorizontal2.lo.y, topHorizontal2.hi.x, topHorizontal2.hi.y, 1, color);
	TheDisplay->drawLine(
			topHorizontal2drop.lo.x,
			topHorizontal2drop.lo.y,
			topHorizontal2drop.hi.x,
			topHorizontal2drop.hi.y,
			1,
			colorDrop);
	TheDisplay
			->drawLine(bottomHorizontal1.lo.x, bottomHorizontal1.lo.y, bottomHorizontal1.hi.x, bottomHorizontal1.hi.y, 1, color);
	TheDisplay->drawLine(
			bottomHorizontal1drop.lo.x,
			bottomHorizontal1drop.lo.y,
			bottomHorizontal1drop.hi.x,
			bottomHorizontal1drop.hi.y,
			1,
			colorDrop);
	TheDisplay
			->drawLine(bottomHorizontal2.lo.x, bottomHorizontal2.lo.y, bottomHorizontal2.hi.x, bottomHorizontal2.hi.y, 2, color);
	TheDisplay->drawLine(
			bottomHorizontal2drop.lo.x,
			bottomHorizontal2drop.lo.y,
			bottomHorizontal2drop.hi.x,
			bottomHorizontal2drop.hi.y,
			2,
			colorDrop);
...
//
//	TheDisplay->drawImage(image,x, y , x + image->getImageWidth(), y + image->getImageHeight(),
// GameMakeColor(255,255,255,alpha));
//	//TheDisplay->drawImage(image,x, y , x + image->getImageWidth(), y + image->getImageHeight(),
// GameMakeColor(255,200,250,alpha)); 	TheDisplay->enableClipping(FALSE );
//	//TheDisplay->drawLine();
//
...
void W3DMainMenuMapBorder(GameWindow *window, WinInstanceData *instData)
{
	...
	TheDisplay->drawImage(TheMappedImageCollection->findImageByName("FrameCornerUL"), x, y, x + size, y + size);
	x = maxX - BORDER_CORNER_SIZE;
	y = originalY - BORDER_CORNER_SIZE;
	TheDisplay->drawImage(TheMappedImageCollection->findImageByName("FrameCornerUR"), x, y, x + size, y + size);
	x = originalX - BORDER_CORNER_SIZE;
	y = maxY - BORDER_CORNER_SIZE;
	TheDisplay->drawImage(TheMappedImageCollection->findImageByName("FrameCornerLL"), x, y, x + size, y + size);
	x = maxX - BORDER_CORNER_SIZE;
	y = maxY - BORDER_CORNER_SIZE;
	TheDisplay->drawImage(TheMappedImageCollection->findImageByName("FrameCornerLR"), x, y, x + size, y + size);
...
//=============================================================================
void W3DMainMenuButtonDropShadowDraw(GameWindow *window, WinInstanceData *instData)
{
...
void W3DMainMenuInit(WindowLayout *layout, void *userData)
{
	...
	//GameWindow *getUpdate = TheWindowManager->winGetWindowFromId( parent, getUpdateID );
		...
		GameWindow *clipRegionWin = TheWindowManager->winGetWindowFromId( parent, TheNameKeyGenerator->nameToKey(
	AsciiString("MainMenu.wnd:MapBorder") )); Int x,y,width,height; clipRegionWin->winGetScreenPosition(&x, &y);
