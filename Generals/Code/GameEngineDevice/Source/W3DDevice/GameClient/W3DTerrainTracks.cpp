...
void TerrainTracksRenderObjClassSystem::ReAcquireResources(void)
{
    ...
    m_vertexBuffer = NEW_REF(
        ...
        (DX8_FVF_XYZDUV1, numModules * m_maxTankTrackEdges * 2, DX8VertexBufferClass::USAGE_DYNAMIC));
}
            ...
            if (mod->m_activeEdgeCount >= 2 && mod->Is_Really_Visible())
            {
                for (i = 0, index = mod->m_bottomIndex; i < mod->m_activeEdgeCount; i++, index++)
                {
                    ...
                    if ((mod->m_activeEdgeCount - 1 - i)
                        >= m_maxTankTrackOpaqueEdges) // && i < (MAX_PER_TRACK_EDGE_COUNT-FORCE_FADE_AT_EDGE))
                    { // we're getting close to the limit on the number of track pieces allowed
                        // so force it to fade out.
                        distanceFade =
                            1.0f - (float)((mod->m_activeEdgeCount - i) - m_maxTankTrackOpaqueEdges) / numFadedEdges;
                    }
