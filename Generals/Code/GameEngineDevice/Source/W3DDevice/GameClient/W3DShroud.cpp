...
void W3DShroud::init(WorldHeightMap *pMap, Real worldCellSizeX, Real worldCellSizeY)
{
	...
	if (pMap)
	{
		...
		m_numCellsY =
				REAL_TO_INT_CEIL((Real)(pMap->getYExtent() - 1 - pMap->getBorderSize() * 2) * MAP_XY_FACTOR / m_cellHeight);
...
void W3DShroud::render(CameraClass *cam)
{
	...
	Int visStartX =
			REAL_TO_INT_FLOOR((Real)(hm->getDrawOrgX() - hm->getBorderSize()) * MAP_XY_FACTOR / m_cellWidth); // start of rendered
																																																				// heightmap
																																																				// rectangle
	if (visStartX < 0)
