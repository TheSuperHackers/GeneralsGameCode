...
#define SC_DETAIL_BLEND \
    (SHADE_CNST( \
        ...
        ShaderClass::DETAILCOLOR_DETAILBLEND, \
...
#define SC_ZFILL_BLEND \
    (SHADE_CNST( \
        ShaderClass::PASS_LEQUAL, \
        ShaderClass::DEPTH_WRITE_ENABLE, \
        ShaderClass::COLOR_WRITE_DISABLE, \
...
void WaterRenderObjClass::setupJbaWaterShader(void)
{
    ...
    if (m_riverWaterPixelShader)
    {
        DX8Wrapper::_Get_D3D_Device8()->SetPixelShaderConstant(
            0,
            D3DXVECTOR4(REFLECTION_FACTOR, REFLECTION_FACTOR, REFLECTION_FACTOR, 1.0f),
            1);
...
HRESULT WaterRenderObjClass::initBumpMap(LPDIRECT3DTEXTURE8 *pTex, TextureClass *pBumpSource)
{
    ...
    pTex[0] =
        DX8Wrapper::_Create_DX8_Texture(d3dsd.Width, d3dsd.Height, WW3D_FORMAT_U8V8, MIP_LEVELS_ALL, D3DPOOL_MANAGED, false);
    ...
    m_pBumpTexture[i] =
        DX8Wrapper::_Create_DX8_Texture(d3dsd.Width, d3dsd.Height, WW3D_FORMAT_U8V8, MIP_LEVELS_1, D3DPOOL_MANAGED, false);
...
HRESULT WaterRenderObjClass::generateVertexBuffer(Int sizeX, Int sizeY, Int vertexSize, Bool doStatic)
{
    ...
    // different parts of the buffer. 5-15-03: Disabled this since we use DISCARD mode instead to avoid Nvidia Runtime bug.
    // -MW m_numVertices=(65536 / (sizeX*sizeY))*sizeX*sizeY;
    ...
    { // Create vertex buffer
        ...
        if (FAILED(hr = m_pDev->CreateVertexBuffer(m_numVertices * vertexSize, usage, fvf, pool, &m_vertexBufferD3D)))
            return hr;
    }
...
HRESULT WaterRenderObjClass::generateIndexBuffer(Int sizeX, Int sizeY)
{
    ...
    if (FAILED(hr = m_indexBufferD3D->Lock(0, m_numIndices * sizeof(WORD), (BYTE **)&pIndices, 0)))
        return hr;
...
Int WaterRenderObjClass::init(Real waterLevel, Real dx, Real dy, SceneClass *parentScene, WaterType type)
{
    ...
    Set_Force_Visible(
        TRUE); // water is always visible since it's a composite object made of multiple planes all over the map.
    ...
    m_skyBox = ((W3DAssetManager *)W3DAssetManager::Get_Instance())
    ...
    // For some reason setting a NULL texture does not result in 0xffffffff for pixel shaders so using explicit "white"
    // texture.
...
{
    if (m_riverTexture
        && TheWaterTransparency->m_standingWaterTexture.compareNoCase(m_riverTexture->Get_Texture_Name()) != 0)
    {
        REF_PTR_RELEASE(m_riverTexture);
        m_riverTexture = WW3DAssetManager::Get_Instance()->Get_Texture(TheWaterTransparency->m_standingWaterTexture.str());
    }
    ...
    if (lastLogicFrame != currLogicFrame)
    {
        ...
        if (m_doWaterGrid && m_meshInMotion == TRUE)
        {
            ...
            const Real AT_REST_VELOCITY_FUDGE =
                1.0f; ///< when we're close enought to at rest height and velocity we will stop
...
void WaterRenderObjClass::loadSetting(Setting *setting, TimeOfDay timeOfDay)
{
    ...
    setting->vertex00Diffuse = (WaterSettings[timeOfDay].m_vertex00Diffuse.red << 16)
        | (WaterSettings[timeOfDay].m_vertex00Diffuse.green << 8) | WaterSettings[timeOfDay].m_vertex00Diffuse.blue;
    // top left
    setting->vertex01Diffuse = (WaterSettings[timeOfDay].m_vertex01Diffuse.red << 16)
        | (WaterSettings[timeOfDay].m_vertex01Diffuse.green << 8) | WaterSettings[timeOfDay].m_vertex01Diffuse.blue;
    // bottom right
    setting->vertex10Diffuse = (WaterSettings[timeOfDay].m_vertex10Diffuse.red << 16)
        | (WaterSettings[timeOfDay].m_vertex10Diffuse.green << 8) | WaterSettings[timeOfDay].m_vertex10Diffuse.blue;
    // top right
    setting->vertex11Diffuse = (WaterSettings[timeOfDay].m_vertex11Diffuse.red << 16)
        | (WaterSettings[timeOfDay].m_vertex11Diffuse.green << 8) | WaterSettings[timeOfDay].m_vertex11Diffuse.blue;
    ...
    setting->waterDiffuse = (WaterSettings[timeOfDay].m_waterDiffuseColor.alpha << 24)
        ...
        | (WaterSettings[timeOfDay].m_waterDiffuseColor.green << 8) | WaterSettings[timeOfDay].m_waterDiffuseColor.blue;
...
{
    if (m_waterType == WATER_TYPE_2_PVSHADER && getClippedWaterPlane(cam, NULL) && TheTerrainRenderObject
...
void WaterRenderObjClass::renderMirror(CameraClass *cam)
{
    ...
    WW3D::Begin_Render(
        false,
        true,
        Vector3(0.0f, 0.0f, 0.0f)); // clearing only z-buffer since background always filled with clouds
...
void WaterRenderObjClass::drawSea(RenderInfoClass &rinfo)
{
    ...
    for (startY = patchY = (seaBox.Center.Y - seaBox.Extent.Y) / (PATCH_WIDTH * PATCH_SCALE);
         (patchY * PATCH_WIDTH * PATCH_SCALE) < (seaBox.Center.Y + seaBox.Extent.Y);
         patchY++)
    {
        for (startX = patchX = (seaBox.Center.X - seaBox.Extent.X) / (PATCH_WIDTH * PATCH_SCALE);
             (patchX * PATCH_WIDTH * PATCH_SCALE) < (seaBox.Center.X + seaBox.Extent.X);
             patchX++)
        {
...
void WaterRenderObjClass::renderSky(void)
{
    ...
    m_shader2.Set_Depth_Mask(
        ShaderClass::DEPTH_WRITE_DISABLE); // sky is always behind everything so no need to update z-buffer
...
void WaterRenderObjClass::renderWaterMesh(void)
{
    ...
    for (j = 0, pData = m_meshData; j < (my + 2); j++)
    {
        for (i = 0; i < (mx + 2); i++)
        {
            ...
            pData->height = WATER_OFFSET
                + WATER_AMP
                    * (sin((float)i * WATER_FREQ * 0.4 + PhasePerFrameX * 0.5)
    ...
    { // ran out of room in last VB, request a substitute VB.
        if (m_vertexBufferD3D->Lock(0, mx * my * sizeof(MaterMeshVertexFormat), (unsigned char **)&vb, D3DLOCK_DISCARD)
            != D3D_OK)
            return;
    ...
    for (j = 0, pData = m_meshData + mx + 2 + 1; j < my; j++, pData += 2) // skip 2 horizontal border samples after each row
    {
        ...
        Real v1Offset =
            m_riverVOrigin + (float)j * vScale + uvCosScale * WWMath::Fast_Sin(sinOffset + y * PI / (8 * MAP_XY_FACTOR));
...
//			vb->v2=m_initialGridV2+(float)j*vScale2;
    ...
    m_shaderClass.Set_Depth_Mask(
        ShaderClass::DEPTH_WRITE_DISABLE); // disable writing to z-buffer to prevent particle clipping.
...
void WaterRenderObjClass::addVelocity(Real worldX, Real worldY, Real zVelocity, Real preferredHeight)
{
    if (m_doWaterGrid)
    {
        Real gx, gy;
    ...
    { // find extents of influence
        ...
        for (y = minY; y <= maxY; y++)
        {
            for (x = minX; x <= maxX; x++)
            {
                ...
                newData = *oldData
                    + 1.0f / (m_gridChangeAtt0 + m_gridChangeAtt1 * distance + distance * distance * m_gridChangeAtt2)
                        * delta;
    ...
    if (shadeR == 1.0f && shadeG == 1.0f && shadeB == 1.0f)
    {
        ...
        for (Int lightIndex = 0; lightIndex < TheGlobalData->m_numGlobalLights; lightIndex++)
        {
            if (-TheGlobalData->m_terrainLightPos[lightIndex].z > 0)
            {
                ...
                shadeG +=
                    -TheGlobalData->m_terrainLightPos[lightIndex].z * TheGlobalData->m_terrainDiffuse[lightIndex].green;
...
#define HEIGHT_TO_USE (0.5f)
    ...
    DynamicVBAccessClass vb_access(BUFFER_TYPE_DYNAMIC_DX8, dynamic_fvf_type, (rectangleCount + 1) * 2);
    {
        ...
        for (i = 0; i < (pTrig->getNumPoints() / 2); i++)
        {
            ...
            Real wobbleConst =
                -m_riverVOrigin + vScale * (Real)i + WWMath::Fast_Sin(2 * PI * (vScale * (Real)i) - constA) / 22.0f;
            // old slower version
...
void WaterRenderObjClass::drawTrapezoidWater(Vector3 points[4])
{
    ...
    if (DX8Wrapper::getBackBufferFormat() == WW3D_FORMAT_A8R8G8B8 && TheGlobalData->m_showSoftWaterEdge
    ...
    // #endif // FEATHER_WATER
    {
        ...
        DX8Wrapper::Draw_Triangles(
            ...
            rectangleCount * 2,
            0,
            (rectangleCount + 1) * 2); // lorenzen thinks this is where to itereate the soft shoreline effect
    }
