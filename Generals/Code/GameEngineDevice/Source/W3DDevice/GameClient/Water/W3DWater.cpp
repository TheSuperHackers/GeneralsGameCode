...
Int WaterRenderObjClass::init(Real waterLevel, Real dx, Real dy, SceneClass *parentScene, WaterType type)
{
	...
	m_skyBox =
			((W3DAssetManager *)W3DAssetManager::Get_Instance())->Create_Render_Obj("new_skybox", TheGlobalData->m_skyBoxScale, 0);
...
void WaterRenderObjClass::setTimeOfDay(TimeOfDay tod)
{
	...
	if (m_waterType == WATER_TYPE_2_PVSHADER)
		generateVertexBuffer(PATCH_SIZE, PATCH_SIZE, sizeof(SEA_PATCH_VERTEX), true); // update the water mesh with new
																																									// lighting/alpha
}
...
void WaterRenderObjClass::loadSetting(Setting *setting, TimeOfDay timeOfDay)
{
	...
	setting->waterDiffuse = (WaterSettings[timeOfDay].m_waterDiffuseColor.alpha << 24)
			| (WaterSettings[timeOfDay].m_waterDiffuseColor.red << 16) | (WaterSettings[timeOfDay].m_waterDiffuseColor.green << 8)
...
void WaterRenderObjClass::renderMirror(CameraClass *cam)
{
	...
	WW3D::Begin_Render(false, true, Vector3(0.0f, 0.0f, 0.0f)); // clearing only z-buffer since background always filled with
																															// clouds
...
void WaterRenderObjClass::renderSky(void)
{
	...
	m_shader2.Set_Depth_Mask(ShaderClass::DEPTH_WRITE_DISABLE); // sky is always behind everything so no need to update
																															// z-buffer
			...
			pData->height = WATER_OFFSET
					+ WATER_AMP
							* (sin((float)i * WATER_FREQ * 0.4 + PhasePerFrameX * 0.5)
								 + sin((float)i * WATER_FREQ * 0.6 + PhasePerFrameX * 0.2) + sin((float)j * WATER_FREQ + PhasePerFrameX)
...
//			vb->v2=m_initialGridV2+(float)j*vScale2;
	...
	m_shaderClass.Set_Depth_Mask(ShaderClass::DEPTH_WRITE_DISABLE); // disable writing to z-buffer to prevent particle
																																	// clipping.
	...
	{ // find extents of influence
		...
		for (y = minY; y <= maxY; y++)
		{
			for (x = minX; x <= maxX; x++)
			{
				...
				newData = *oldData
						+ 1.0f / (m_gridChangeAtt0 + m_gridChangeAtt1 * distance + distance * distance * m_gridChangeAtt2) * delta;
...
void WaterRenderObjClass::drawTrapezoidWater(Vector3 points[4])
{
	...
	// #endif // FEATHER_WATER
	{
		...
		// #endif // FEATHER_WATER
		// #endif //WAVY_WATER
		DX8Wrapper::Draw_Triangles(0, rectangleCount * 2, 0, (rectangleCount + 1) * 2); // lorenzen thinks this is where to
																																										// itereate the soft shoreline effect
	}
