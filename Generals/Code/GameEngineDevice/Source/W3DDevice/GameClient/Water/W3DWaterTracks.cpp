    ...
    Int waveTimeOffset)
{
    ...
    m_waveInitialWidth =
        length * waveTypeInfo[m_type].m_initialWidthFraction; ///< width of wave segment when it first appears
    m_waveInitialHeight = m_waveInitialWidth
        * waveTypeInfo[m_type].m_initialHeightWidthFraction; ///< height of wave segment when it first appears
    ...
    m_frontSlowDownAcc =
        -(m_initialVelocity * m_initialVelocity) / (2 * m_waveFinalHeight); // deceleration of wave after it hits land
    ...
    m_backSlowDownAcc =
        (2.0f * m_waveInitialHeight
         / (m_timeToStop
            * m_timeToStop)); //((m_waveInitialHeight - m_velocity*m_timeToStop)*2.0f)/(m_timeToStop*m_timeToStop);
    m_timeToCompress =
        waveTypeInfo[m_type]
            .m_timeToCompress; // time for back of wave to continue moving forward after front starts retreating.
...
Int WaterTracksObj::render(DX8VertexBufferClass *vertexBuffer, Int batchStart)
{
    ...
    { // ran out of room in last VB, request a substitute VB.
        if (vertexBuffer->Get_DX8_Vertex_Buffer()
                ->Lock(0, m_x * m_y * vertexBuffer->FVF_Info().Get_FVF_Size(), (unsigned char **)&vb, D3DLOCK_DISCARD)
            != D3D_OK)
            return batchStart;
    ...
    { // stationary wave
        ...
        { // fading out
            waveAlpha = m_elapsedMs
                - (m_timeToReachBeach + m_timeToStop - 1000
                   + m_fadeMs); //(m_totalMs-m_timeToRetreat -m_fadeMs - m_elapsedMs)/m_fadeMs;
        ...
        { // start fading up
            ...
            waveAlpha = m_elapsedMs
                - (m_timeToReachBeach + m_timeToStop - 1000); //(m_totalMs-m_timeToRetreat -m_fadeMs - m_elapsedMs)/m_fadeMs;
    ...
    { // moving wave
        ...
        { // wave has not reached beach yet so position only depends on velocity
            ...
            if (m_elapsedMs < m_totalMs)
            {
                ...
                waveFrontOrigin += (m_initialVelocity * elapsedMs + 0.5f * m_frontSlowDownAcc * elapsedMs * elapsedMs)
                ...
                if (m_elapsedMs > (m_timeToReachBeach + m_timeToStop + m_timeToCompress))
                {
                    ...
                    waveTailOrigin +=
                        (0.5f * m_backSlowDownAcc * (m_timeToStop + m_timeToCompress) * (m_timeToStop + m_timeToCompress))
                        * ooWaveDirLen * m_waveDir;
...
void WaterTracksRenderSystem::ReAcquireResources(void)
{
    ...
    m_vertexBuffer = NEW_REF(
        DX8VertexBufferClass,
        (DX8_FVF_XYZDUV1, m_stripSizeX * m_stripSizeY * WATER_VB_PAGES, DX8VertexBufferClass::USAGE_DYNAMIC));
    m_batchStart = 0;
}
...
WaterTracksObj *WaterTracksRenderSystem::findTrack(Vector2 &start, Vector2 &end, waveType type)
{
    ...
    while (mod)
    {
        if (mod->m_initEndPos == end && mod->m_initStartPos == start && mod->m_type == type)
            return mod;
        mod = mod->m_nextSystem;
    } // while (mod)
    return NULL;
    ...
    if (file)
    {
        ...
        tryagain:
            ...
            { // umod->init(1.5f*MAP_XY_FACTOR,Vector2(0,0),Vector2(1,1),"wave256.tga");
                ...
                umod->init(
                    waveTypeInfo[wtype].m_finalHeight,
                    waveTypeInfo[wtype].m_finalWidth,
                    startPos,
                    endPos,
