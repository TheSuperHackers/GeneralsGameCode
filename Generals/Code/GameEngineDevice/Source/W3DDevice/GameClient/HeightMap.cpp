...
bool HeightMapRenderObjClass::Cast_Ray(RayCollisionTestClass &raytest)
{
	...
	Int StartCellY = 0;
	Int EndCellY = 0;
	const Int overhang = 2 * VERTEX_BUFFER_TILE_LENGTH + m_map->getBorderSize(); // Allow picking past the edge for scrolling &
																																							 // objects.
	...
	for (offset = 1; offset < 5; offset *= 3)
	{
		for (Y = StartCellY - offset; Y <= EndCellY + offset; Y++)
		{
			...
			for (X = StartCellX - offset; X <= EndCellX + offset; X++)
			{
				...
				if (hit)
					raytest.Result->SurfaceType = SURFACE_TYPE_DEFAULT; ///@todo: WW3D uses this to return dirt, grass, etc.  Do we
																															/// need this?
			}
...
void HeightMapRenderObjClass::updateShorelineTiles(Int minX, Int minY, Int maxX, Int maxY, WorldHeightMap *pMap)
{
	...
	for (j = minY; j < maxY; j++)
		...
		{
			waterSide = (waterZ0 = TheWaterRenderObj->getWaterHeight((i - border) * MAP_XY_FACTOR, (j - border) * MAP_XY_FACTOR))
					> ((terrainZ0 = MAP_HEIGHT_SCALE * pMap->getHeight(i, j)));
			waterSide |=
					((waterZ1 = TheWaterRenderObj->getWaterHeight((i - border + 1) * MAP_XY_FACTOR, (j - border) * MAP_XY_FACTOR))
					 > ((terrainZ1 = MAP_HEIGHT_SCALE * pMap->getHeight(i + 1, j))))
					<< 1;
			waterSide |=
					((waterZ2 = TheWaterRenderObj->getWaterHeight((i - border + 1) * MAP_XY_FACTOR, (j - border + 1) * MAP_XY_FACTOR))
					 > ((terrainZ2 = MAP_HEIGHT_SCALE * pMap->getHeight(i + 1, j + 1))))
					<< 2;
			waterSide |=
					((waterZ3 = TheWaterRenderObj->getWaterHeight((i - border) * MAP_XY_FACTOR, (j - border + 1) * MAP_XY_FACTOR))
					 > ((terrainZ3 = MAP_HEIGHT_SCALE * pMap->getHeight(i, j + 1))))
					<< 3;

			if (!waterSide || (waterZ0 * waterZ1 * waterZ2 * waterZ3) <= 0)
				continue; // all verts are on positive (surface) side of water so don't need blending.  Or one of them is outside the
									// water plane bounds (waterHeight <= 0!)
	...
	for (curScorch = m_numScorches - 1; curScorch >= 0; curScorch--)
	{
		...
		for (j = minY; j < maxY; j++)
		{
			for (i = minX; i < maxX; i++)
			{
				...
				Real theZ;
				theZ =
						amtToFloat + ((float)getClipHeight(i + m_map->getBorderSize(), j + m_map->getBorderSize()) * MAP_HEIGHT_SCALE);
				if (m_halfResMesh)
				{
