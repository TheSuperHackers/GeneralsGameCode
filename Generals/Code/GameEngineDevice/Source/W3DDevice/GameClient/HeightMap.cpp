    ...
    Int numLights)
{
    ...
    //	vb->diffuse=REAL_TO_INT(shadeB) | (REAL_TO_INT(shadeG) << 8) | (REAL_TO_INT(shadeR) << 16) | ((int)alpha << 24);
    UnsignedInt light_val = WWMath::Float_To_Int_Chop(shadeB) | (WWMath::Float_To_Int_Chop(shadeG) << 8)
        | (WWMath::Float_To_Int_Chop(shadeR) << 16) | ((int)alpha << 24);
...
#ifdef RTS_DEBUG
        assert(
            x0 >= originX && y0 >= originY && x1 > x0 && y1 > y0 && x1 <= originX + VERTEX_BUFFER_TILE_LENGTH
        ...
        for (j = y0; j < y1; j++)
        {
            ...
            for (i = x0; i < x1; i++)
            {
                ...
                l2r.Set(
                    ...
                    0,
                    MAP_HEIGHT_SCALE
                        * (pMap->getDisplayHeight(getXWithOrigin(i) + cellOffset, getYWithOrigin(j))
                           - pMap->getDisplayHeight(un0, getYWithOrigin(j))));
                n2f.Set(
                    0,
                    2 * MAP_XY_FACTOR,
                    MAP_HEIGHT_SCALE
                        * (pMap->getDisplayHeight(getXWithOrigin(i), (getYWithOrigin(j) + cellOffset))
                ...
                n2f.Set(
                    ...
                    2 * MAP_XY_FACTOR,
                    MAP_HEIGHT_SCALE
                        * (pMap->getDisplayHeight(getXWithOrigin(i) + cellOffset, (getYWithOrigin(j) + cellOffset))
                ...
                vb->y = yCoord;
                vb->z =
                    ((float)pMap->getDisplayHeight(getXWithOrigin(i) + cellOffset, getYWithOrigin(j))) * MAP_HEIGHT_SCALE;
                vb->x = ADJUST_FROM_INDEX_TO_REAL(vb->x);
                vb->y = ADJUST_FROM_INDEX_TO_REAL(vb->y);
                vb->u1 = U[1];
                vb->v1 = V[1];
                ...
                vb->z = ((float)pMap->getDisplayHeight(getXWithOrigin(i) + cellOffset, getYWithOrigin(j) + cellOffset))
                    * MAP_HEIGHT_SCALE;
                vb->x = ADJUST_FROM_INDEX_TO_REAL(vb->x);
                vb->y = ADJUST_FROM_INDEX_TO_REAL(vb->y);
                vb->u1 = U[2];
                ...
                l2r.Set(
                    ...
                    MAP_HEIGHT_SCALE
                        * (pMap->getDisplayHeight(getXWithOrigin(i) + cellOffset, (getYWithOrigin(j) + cellOffset))
                           - pMap->getDisplayHeight(un0, (getYWithOrigin(j) + cellOffset))));
                n2f.Set(
                    0,
                    2 * MAP_XY_FACTOR,
                    MAP_HEIGHT_SCALE
                ...
                vb->z =
                    ((float)pMap->getDisplayHeight(getXWithOrigin(i), getYWithOrigin(j) + cellOffset)) * MAP_HEIGHT_SCALE;
                vb->x = ADJUST_FROM_INDEX_TO_REAL(vb->x);
                vb->y = ADJUST_FROM_INDEX_TO_REAL(vb->y);
                vb->u1 = U[3];
                vb->v1 = V[3];
                ...
                if (m_showImpassableAreas)
                {
                    ...
                    DEBUG_ASSERTCRASH(
                        PATHFIND_CELL_SIZE_F == MAP_XY_FACTOR,
                        ("Pathfind must be terrain cell size, or this code needs reworking.  John A."));
                    ...
                    Bool isCliff =
                        pMap->getCliffState(getXWithOrigin(i) + pMap->getDrawOrgX(), getYWithOrigin(j) + pMap->getDrawOrgY())
                        || showAsVisibleCliff(
                            getXWithOrigin(i) + pMap->getDrawOrgX(),
...
Int HeightMapRenderObjClass::updateVBForLight(
    ...
    Int numLights)
{
#if (OPTIMIZED_HEIGHTMAP_LIGHTING) // (gth) if optimizations are enabled, jump over to the "optimized" version of this
                                   // function.
    ...
    if (m_vertexBufferTiles && m_map)
    {
        ...
        for (j = y0; j < y1; j++)
        {
            ...
            for (k = 0; k < numLights; k++)
            {
                ...
                if (pLights[k]->m_prevMinY <= yCoord + 1 && pLights[k]->m_prevMaxY >= yCoord)
                {
                    intersect = true;
                }
            }
            ...
            for (i = x0; i < x1; i++)
            {
                ...
                for (k = 0; k < numLights; k++)
                {
                    if (pLights[k]->m_minX <= xCoord + 1 && pLights[k]->m_maxX >= xCoord && pLights[k]->m_minY <= yCoord + 1
                        && pLights[k]->m_maxY >= yCoord)
                    {
                        intersect = true;
                    }
                    if (pLights[k]->m_prevMinX <= xCoord + 1 && pLights[k]->m_prevMaxX >= xCoord
                        && pLights[k]->m_prevMinY <= yCoord + 1 && pLights[k]->m_prevMaxY >= yCoord)
                    {
                        intersect = true;
                    }
                }
                ...
                l2r.Set(
                    ...
                    0,
                    MAP_HEIGHT_SCALE
                        * (m_map->getDisplayHeight(getXWithOrigin(i) + 1, getYWithOrigin(j))
                           - m_map->getDisplayHeight(un0, getYWithOrigin(j))));
                n2f.Set(
                    0,
                    2 * MAP_XY_FACTOR,
                    MAP_HEIGHT_SCALE
                        * (m_map->getDisplayHeight(getXWithOrigin(i), (getYWithOrigin(j) + 1))
                ...
                l2r.Set(
                    ...
                    MAP_HEIGHT_SCALE
                        * (m_map->getDisplayHeight(up1, getYWithOrigin(j))
                           - m_map->getDisplayHeight(getXWithOrigin(i), getYWithOrigin(j))));
                n2f.Set(
                    0,
                    2 * MAP_XY_FACTOR,
                    MAP_HEIGHT_SCALE
                        * (m_map->getDisplayHeight(getXWithOrigin(i) + 1, (getYWithOrigin(j) + 1))
            ...
            for (i = x0; i < x1; i++)
            {
                ...
                if (j == y0)
                {
                    ...
                    if (i < x1 - 1)
                    {
                        ...
                        (vBase + offset + quad_right_offset)->diffuse =
                            (light_copy & 0x00FFFFFF) | ((vbaseMirror + offset + quad_right_offset)->diffuse & 0xff000000);
                    }
                }
                vb++;
                vbMirror++;
                ...
                if (i < x1 - 1)
                {
                    ...
                    (vBase + offset + quad_right_offset + 3)->diffuse =
                        (light_copy & 0x00FFFFFF) | ((vbaseMirror + offset + quad_right_offset + 3)->diffuse & 0xff000000);
                }
                if (j < y1 - 1)
                {
                    ...
                    (vBase + offset + quad_right_offset + 1)->diffuse =
                        (light_copy & 0x00FFFFFF) | ((vbaseMirror + offset + quad_right_offset + 1)->diffuse & 0xff000000);
                }
                if ((i < x1 - 1) && (j < y1 - 1))
                {
                ...
                if (i == x0)
                {
                    ...
                    if (j < y1 - 1)
                    {
                        ...
                        (vBase + offset + quad_below_offset)->diffuse =
                            (light_copy & 0x00FFFFFF) | ((vbaseMirror + offset + quad_below_offset)->diffuse & 0xff000000);
                    }
                }
                vb++;
                vbMirror++;
    ...
    RefRenderObjListIterator *pLightsIterator)
{
    ...
    for (j = 0; j < m_numExtraBlendTiles; j++)
    {
        ...
        if (x >= partialRange.lo.x && x < partialRange.hi.x && y >= partialRange.lo.y && y < partialRange.hi.y)
        { // this tile is inside region being updated so remove it by shifting tile array
            memcpy(
                m_extraBlendTilePositions + j,
                m_extraBlendTilePositions + j + 1,
...
void HeightMapRenderObjClass::adjustTerrainLOD(Int adj)
{
    ...
    this->initHeightData(m_map->getDrawWidth(), m_map->getDrawHeight(), m_map, NULL);
...
void HeightMapRenderObjClass::reset(void)
{
    ...
    if (m_shroud)
    {
        ...
        m_shroud->setBorderShroudLevel(
            (W3DShroudLevel)TheGlobalData->m_shroudAlpha); // assume border is always black at start.
    }
}
...
bool HeightMapRenderObjClass::Cast_Ray(RayCollisionTestClass &raytest)
{
    ...
    Int EndCellY = 0;
    const Int overhang =
        2 * VERTEX_BUFFER_TILE_LENGTH + m_map->getBorderSize(); // Allow picking past the edge for scrolling & objects.
    Vector3 minPt(MAP_XY_FACTOR * (-overhang), MAP_XY_FACTOR * (-overhang), -MAP_XY_FACTOR);
    Vector3 maxPt(
        MAP_XY_FACTOR * (m_map->getXExtent() + overhang),
            ...
            for (X = StartCellX - offset; X <= EndCellX + offset; X++)
            {
                ...
                if (hit)
                    raytest.Result->SurfaceType =
                        SURFACE_TYPE_DEFAULT; ///@todo: WW3D uses this to return dirt, grass, etc.  Do we need this?
            }
...
void HeightMapRenderObjClass::Get_Obj_Space_Bounding_Sphere(SphereClass &sphere) const
{
    Vector3 ObjSpaceCenter(
        (float)m_x * 0.5f * MAP_XY_FACTOR,
        (float)m_y * 0.5f * MAP_XY_FACTOR,
...
// ============================================================================
void HeightMapRenderObjClass::worldBuilderUpdateBridgeTowers(W3DAssetManager *assetManager, SimpleSceneClass *scene)
{
    if (m_bridgeBuffer)
...
void HeightMapRenderObjClass::updateShorelineTiles(Int minX, Int minY, Int maxX, Int maxY, WorldHeightMap *pMap)
{
    ...
    for (; j < m_numShoreLineTiles; j++)
    {
        ...
        { // this tile is inside region being updated so remove it by shifting tile array
            memcpy(
                m_shoreLineTilePositions + j,
                m_shoreLineTilePositions + j + 1,
    ...
    for (j = minY; j < maxY; j++)
        ...
        {
            waterSide =
                (waterZ0 = TheWaterRenderObj->getWaterHeight((i - border) * MAP_XY_FACTOR, (j - border) * MAP_XY_FACTOR))
                > ((terrainZ0 = MAP_HEIGHT_SCALE * pMap->getHeight(i, j)));
            waterSide |=
                ((waterZ1 =
                      TheWaterRenderObj->getWaterHeight((i - border + 1) * MAP_XY_FACTOR, (j - border) * MAP_XY_FACTOR))
                 > ((terrainZ1 = MAP_HEIGHT_SCALE * pMap->getHeight(i + 1, j))))
                << 1;
            waterSide |=
                ((waterZ2 =
                      TheWaterRenderObj->getWaterHeight((i - border + 1) * MAP_XY_FACTOR, (j - border + 1) * MAP_XY_FACTOR))
                 > ((terrainZ2 = MAP_HEIGHT_SCALE * pMap->getHeight(i + 1, j + 1))))
                << 2;
            waterSide |=
                ((waterZ3 =
                      TheWaterRenderObj->getWaterHeight((i - border) * MAP_XY_FACTOR, (j - border + 1) * MAP_XY_FACTOR))
                 > ((terrainZ3 = MAP_HEIGHT_SCALE * pMap->getHeight(i, j + 1))))
                << 3;

            if (!waterSide || (waterZ0 * waterZ1 * waterZ2 * waterZ3) <= 0)
                continue; // all verts are on positive (surface) side of water so don't need blending.  Or one of them is
                          // outside the water plane bounds (waterHeight <= 0!)

            // Check if mix of under/over water vertices or some vertices within depth fade region.
            if (waterSide < 0xf || (waterZ0 - terrainZ0) < transparentDepth || (waterZ1 - terrainZ1) < transparentDepth
                || (waterZ2 - terrainZ2) < transparentDepth || (waterZ3 - terrainZ3) < transparentDepth)
...
Int HeightMapRenderObjClass::initHeightData(Int x, Int y, WorldHeightMap *pMap, RefRenderObjListIterator *pLightsIterator)
{
    ...
    { // requested heightmap different from old one.
        ...
        {
#ifdef USE_NORMALS
            m_vertexBufferTiles[i] =
                NEW_REF(DX8VertexBufferClass, (DX8_FVF_XYZNUV2, numVertex, DX8VertexBufferClass::USAGE_DEFAULT));
#else
            m_vertexBufferTiles[i] =
                NEW_REF(DX8VertexBufferClass, (DX8_VERTEX_FORMAT, numVertex, DX8VertexBufferClass::USAGE_DEFAULT));
#endif
...
{
    m_vertexScorch =
        NEW_REF(DX8VertexBufferClass, (DX8_FVF_XYZDUV1, MAX_SCORCH_VERTEX, DX8VertexBufferClass::USAGE_DEFAULT));
    ...
    for (curScorch = m_numScorches - 1; curScorch >= 0; curScorch--)
    {
        ...
        for (j = minY; j < maxY; j++)
        {
            for (i = minX; i < maxX; i++)
            {
                ...
                Real theZ;
                theZ = amtToFloat
                    + ((float)getClipHeight(i + m_map->getBorderSize(), j + m_map->getBorderSize()) * MAP_HEIGHT_SCALE);
                if (m_halfResMesh)
                {
...
void HeightMapRenderObjClass::addScorch(Vector3 location, Real radius, Scorches type)
{
    ...
    {
        if (abs(location.X - m_scorches[i].location.X) < limit && abs(location.Y - m_scorches[i].location.Y) < limit
            && abs(radius - m_scorches[i].radius) < limit && m_scorches[i].scorchType == type)
        {
...
#define not_VERTS_MATCH
    ...
    if (x == VERTEX_BUFFER_TILE_LENGTH)
        x--;
    j = 0;
    ...
    if (y == VERTEX_BUFFER_TILE_LENGTH)
        y--;
...
#pragma MESSAGE("*** WARNING *** DOING DO_UNIT_TIMINGS!!!!")
    ...
    for (pDynamicLightsIterator.First(); !pDynamicLightsIterator.Is_Done(); pDynamicLightsIterator.Next())
    {
        ...
        if (pLight->m_enabled || pLight->m_priorEnable)
        {
            ...
            if (pLight->m_minX < xCoordMax && pLight->m_minY < yCoordMax && pLight->m_maxX > xCoordMin
                && pLight->m_maxY > yCoordMin)
            {
                pLight->m_processMe = TRUE;
            }
            else if (
                pLight->m_prevMinX < xCoordMax && pLight->m_prevMinY < yCoordMax && pLight->m_prevMaxX > xCoordMin
    ...
    if (numDynaLights > 0)
    {
        ...
        for (j = 0; j < m_numVBTilesY; j++)
        {
            ...
            if (yCoordMax > yCoordMin)
            {
                ...
                {
                    if (enabledLights[k]->m_minY < yCoordMax && enabledLights[k]->m_maxY > yCoordMin)
                    {
                        intersect = true;
                        break;
                    }
                    if (enabledLights[k]->m_prevMinY < yCoordMax && enabledLights[k]->m_prevMaxY > yCoordMin)
                    {
                        intersect = true;
                        break;
                    }
            ...
            else
            {
                ...
                for (k = 0; k < numDynaLights; k++)
                {
                    if (enabledLights[k]->m_minY <= yCoordMin || enabledLights[k]->m_maxY >= yCoordMax)
                    {
                        intersect = true;
                        break;
                    }
                    if (enabledLights[k]->m_prevMinY <= yCoordMin || enabledLights[k]->m_prevMaxY >= yCoordMax)
                    {
                        intersect = true;
                        break;
                    }
            ...
            for (i = 0; i < m_numVBTilesX; i++)
            {
                ...
                if (xCoordMax > xCoordMin)
                {
                    ...
                    for (k = 0; k < numDynaLights; k++)
                    {
                        if (enabledLights[k]->m_minX < xCoordMax && enabledLights[k]->m_maxX > xCoordMin)
                        {
                            intersect = true;
                            break;
                        }
                        if (enabledLights[k]->m_prevMinX < xCoordMax && enabledLights[k]->m_prevMaxX > xCoordMin)
                        {
                            intersect = true;
                            break;
                        }
                ...
                else
                {
                    ...
                    for (k = 0; k < numDynaLights; k++)
                    {
                        if (enabledLights[k]->m_minX <= xCoordMin || enabledLights[k]->m_maxX >= xCoordMax)
                        {
                            intersect = true;
                            break;
                        }
                        if (enabledLights[k]->m_prevMinX <= xCoordMin || enabledLights[k]->m_prevMaxX >= xCoordMax)
                        {
                            intersect = true;
                            break;
                        }
...
//=============================================================================
void HeightMapRenderObjClass::addTree(Coord3D location, Real scale, Real angle, AsciiString name, Bool visibleInMirror)
{
...
//=============================================================================
void HeightMapRenderObjClass::addTerrainBib(Vector3 corners[4], ObjectID id, Bool highlight)
{
    if (m_bibBuffer)
...
/** Adds a terrainBib to the bib buffer.*/
//=============================================================================
void HeightMapRenderObjClass::addTerrainBibDrawable(Vector3 corners[4], DrawableID id, Bool highlight)
{
    if (m_bibBuffer)
...
static Bool check(const FrustumClass &frustum, WorldHeightMap *pMap, Int x, Int y)
{
    ...
    Vector3 loc(
        (x - pMap->getBorderSize()) * MAP_XY_FACTOR,
        ...
        for (j = 0; j < m_numVBTilesY; j++)
            for (i = 0; i < m_numVBTilesX; i++)
            {
                ...
                if (m_xformedVertexBuffer && pass == 0)
                {
                    ...
                    DX8Wrapper::Apply_Render_State_Changes();
                    int code = DX8Wrapper::_Get_D3D_Device8()
                                   ->ProcessVertices(0, 0, numVertex, m_xformedVertexBuffer[j * m_numVBTilesX + i], 0);
