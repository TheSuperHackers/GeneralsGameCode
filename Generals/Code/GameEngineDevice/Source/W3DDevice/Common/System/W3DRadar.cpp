                        ...
                        for (i = x - samplesAway; i <= x + samplesAway; i++)
                        {
                            if (i >= 0 && i < m_textureWidth)
                            {
                                ...
                                if (workingBridge)
                                {
                                    ...
                                    //
                                    Real bridgeHeight =
                                        (bridge->peekBridgeInfo()->fromLeft.z + bridge->peekBridgeInfo()->fromRight.z
    ...
    void W3DRadar::renderObjectList( const RadarObject *listHead, TextureClass *texture )
{
    ...
    c = rObj->getColor();
      ...
      minAlpha <<= 2; // decoy
            ...
            if ( ( obj->isLocallyControlled() == (Bool)a ) // another decoy, comparing the return of this non-inline with a
local
