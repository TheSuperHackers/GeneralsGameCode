...
IDirect3DTexture8 *TextureLoader::Load_Thumbnail(const StringClass &filename, WW3DFormat texture_format)
{
	...
	IDirect3DTexture8 *d3d_texture =
			DX8Wrapper::_Create_DX8_Texture(thumb->Get_Width(), thumb->Get_Height(), dest_format, MIP_LEVELS_ALL);
...
void TextureLoader::Load_Mipmap_Levels(TextureLoadTaskClass *task)
{
	...
	if (Load_Uncompressed_Mipmap_Levels_From_TGA(task))
		return;
}
...
{
	WWASSERT_PRINT(
			DX8Wrapper::_Get_Main_Thread_ID() == ThreadClass::_Get_Current_Thread_ID(),
...
IDirect3DTexture8 *TextureLoader::Generate_Bumpmap(TextureBaseClass *texture)
{
	...
	IDirect3DTexture8 *d3d_texture = DX8Wrapper::_Create_DX8_Texture(width, height, bump_format, MIP_LEVELS_1);

	D3DLOCKED_RECT src_locked_rect;
	DX8_ErrorCode(texture->Peek_D3D_Texture()->LockRect(0, &src_locked_rect, NULL, D3DLOCK_READONLY));
...
void TextureLoader::Request_High_Priority_Loading(TextureBaseClass *tc, MipCountType mip_level_count)
{
