		...
		LODBias(1.0f)
{
	...
	for (int lod_index = 0; lod_index < LodCount; lod_index++)
	{
		...
		if ((lod_obj->Class_ID() == RenderObjClass::CLASSID_HMODEL) || (lod_obj->Class_ID() == RenderObjClass::CLASSID_HLOD)
...
const AABoxClass &HLodClass::Get_Bounding_Box(void) const
{
	if (BoundingBoxIndex >= 0)
	{
		...
		//
		Get_Transform()
				.Transform_Center_Extent_AABox(box.Center, box.Extent, &CachedBoundingBox.Center, &CachedBoundingBox.Extent);
	}
	else
	{
