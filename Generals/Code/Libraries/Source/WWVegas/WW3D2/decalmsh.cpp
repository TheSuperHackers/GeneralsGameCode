 ...
 *   1/31/00    NH : Created.                                                                  *
 *=============================================================================================*/
RigidDecalMeshClass::RigidDecalMeshClass(MeshClass *parent, DecalSystemClass *system) : DecalMeshClass(parent, system)
{
}
...
int RigidDecalMeshClass::Process_Material_Run(int start_index)
{
	...
	while ((next_index < Polys.Count()) && (Textures[next_index] == Textures[start_index])
				 && (Shaders[next_index] == Shaders[start_index]) && (VertexMaterials[next_index] == VertexMaterials[start_index]))
	{
		next_index++;
	}
	return next_index;
		...
		for (int vi = 0; vi < 3; vi++)
		{
			...
			*/
			...
			generator->Compute_Texture_Coordinate(
					(*world_vertex_locs)[ParentVertexIndices[ParentVertexIndices.Count() - 1]],
					&stq);
			TexCoords.Add(Vector2(stq.X, stq.Y));
		}
	}
