...
const char *HALO_INTENSITY_POW_STRING = "HaloIntensityPow"; // 1.0 would be linear fadeout, smaller than that will steepen
																														// the curve (smaller hotspot)
const char *HALO_SIZE_POW_STRING = "HaloSizePow"; // 1.0 would be linear fadeout, smaller than that will steepen the curve
																									// (smaller hotspot)
		...
		const Vector4 &transformed_location)
{
	...
	float distance_multiplier =
			sqrt(transformed_location[0] * transformed_location[0] + transformed_location[1] * transformed_location[1]) + 1.0f;
		...
		radius(is.radius)
{
	...
	dazzle_test_color_integer = 0xff000000 | (unsigned(255.0f * ic.dazzle_test_color[2]) << 16)
			| (unsigned(255.0f * ic.dazzle_test_color[1]) << 8) | (unsigned(255.0f * ic.dazzle_test_color[0]));
...
void DazzleRenderObjClass::Init_From_INI(const INIClass *ini)
{
	...
	{
		char section_name[80];
		ini->Get_String(DAZZLE_LIST_STRING, ini->Get_Entry(DAZZLE_LIST_STRING, entry), "", section_name, sizeof(section_name));
...
LensflareTypeClass *DazzleRenderObjClass::Get_Lensflare_Class(unsigned id) // Return lensflare type class pointer, or NULL if
																																					 // not found
{
