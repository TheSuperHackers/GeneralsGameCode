  ...
  for (int pass = 0; pass < MAX_PASSES; pass++)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        {
          WWDEBUG_SAY(
              ("Unimplemented case: mesh has more than one default vertex material but no alternate vertex "
    ...
    if ((DCGSource[pass] != VertexMaterialClass::MATERIAL) && (ColorArray[0] != NULL))
    {
      ...
      for (int vidx = 0; vidx < VertexCount; vidx++)
      {
        ...
        if (!diffuse_used && !ambient_used && emissive_used)
        {
          ...
          mtl->Set_Diffuse_Color_Source(VertexMaterialClass::COLOR1);
          mtl->Set_Emissive_Color_Source(VertexMaterialClass::MATERIAL);
          // MW:  Vegas guys asked me to disable this because it can cause z-fighting if lighting is disabled in
          // multi-pass 					mtl->Set_Lighting(false);
        }
      }
    }
