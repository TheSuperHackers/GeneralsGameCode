...
enum CanEnterType CPP_11( : Int){ CHECK_CAPACITY, DONT_CHECK_CAPACITY, COMBATDROP_INTO };
...
public:
	...
	CanAttackResult getCanAttackObject(
			const Object *obj,
	...
	Bool canDisableVehicleViaHacking(
			const Object *obj,
	...
	Bool canDoSpecialPowerAtLocation(
			const Object *obj,
			const Coord3D *loc,
			CommandSourceType commandSource,
			...
			Bool checkSourceRequirements = true);
	Bool canDoSpecialPowerAtObject(
			const Object *obj,
			const Object *target,
			CommandSourceType commandSource,
			const SpecialPowerTemplate *spTemplate,
	...
	Bool canFireWeaponAtLocation(
			const Object *obj,
			const Coord3D *loc,
			CommandSourceType commandSource,
			const WeaponSlotType slot,
			const Object *objectInWay);
	Bool canFireWeaponAtObject(
			const Object *obj,
			const Object *target,
			CommandSourceType commandSource,
			const WeaponSlotType slot);
	...
	Bool canOverrideSpecialPowerDestination(
			const Object *obj,
			const Coord3D *loc,
