...
struct StaticGameLODInfo
{
	...
	Int m_sampleCount2D; ///< How many 2-D (UI) samples should we allow simultaneously (Note: Hardware may support less than
											 ///< the desired number)
	Int m_sampleCount3D; ///< How many 3-D (World) samples should we allow simultaneously (Note: Hardware may support less than
											 ///< the desired number)
...
public:
	...
	inline ParticlePriorityType getMinDynamicParticlePriority(void); ///< priority at which particles will still render at
																																	 ///< current FPS.
	inline ParticlePriorityType getMinDynamicParticleSkipPriority(void); ///< priority at which particles will never be skipped
																																			 ///< at any FPS.
	Int getRecommendedTextureReduction(void); ///< return the optimal texture reduction for the system.
	Int getLevelTextureReduction(StaticGameLODLevel level); ///< return texture reduction specified in INI for this game
																													///< detail.
