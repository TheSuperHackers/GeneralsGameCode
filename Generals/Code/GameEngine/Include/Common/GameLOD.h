...
enum StaticGameLODLevel CPP_11( : Int){ STATIC_GAME_LOD_UNKNOWN = -1, STATIC_GAME_LOD_LOW,
                                        STATIC_GAME_LOD_MEDIUM,       STATIC_GAME_LOD_HIGH,
                                        STATIC_GAME_LOD_CUSTOM, // make sure this remains last!
                                        STATIC_GAME_LOD_COUNT };

enum DynamicGameLODLevel CPP_11( : Int){ DYNAMIC_GAME_LOD_UNKNOWN = -1, DYNAMIC_GAME_LOD_LOW,       DYNAMIC_GAME_LOD_MEDIUM,
                                         DYNAMIC_GAME_LOD_HIGH,         DYNAMIC_GAME_LOD_VERY_HIGH, DYNAMIC_GAME_LOD_COUNT };
...
enum ChipsetType CPP_11( : Int){ DC_UNKNOWN,
                                 DC_VOODOO2,
                                 DC_VOODOO3,
                                 DC_VOODOO4,
                                 DC_VOODOO5,
...
struct StaticGameLODInfo
{
  ...
  Int m_sampleCount2D; ///< How many 2-D (UI) samples should we allow simultaneously (Note: Hardware may support less than
                       ///< the desired number)
  Int m_sampleCount3D; ///< How many 3-D (World) samples should we allow simultaneously (Note: Hardware may support less
                       ///< than the desired number)
  Int m_streamCount; ///< How many streaming audio things should we allow simultaneously (Note: Hardware may support less
                     ///< than the desired number)
...
class GameLODManager
{
  ...
  inline ParticlePriorityType getMinDynamicParticlePriority(
      void); ///< priority at which particles will still render at current FPS.
  inline ParticlePriorityType getMinDynamicParticleSkipPriority(
      void); ///< priority at which particles will never be skipped at any FPS.
  Int getRecommendedTextureReduction(void); ///< return the optimal texture reduction for the system.
  Int getLevelTextureReduction(
      StaticGameLODLevel level); ///< return texture reduction specified in INI for this game detail.
