...
public:
    ...
    Real m_destroyedThreshold; ///< OnDestroyed threshold - 1.0 = 100% = all destroyed, .5 = 50% = half of the units
                               ///< destroyed, 0 = useless.
    ...
    Bool
        m_executeActions; ///< If this is true, then when the production condition becomes true, we also execute the actions.
...
public:
    ...
    */
    ...
    Bool someInsideSomeOutside(PolygonTrigger *pTrigger, UnsignedInt whichToConsider)
        const; ///< One or more in, one or more out.
    ...
    Object *tryToRecruit(
        ...
        Real maxDist); ///< Try to recruit the closest unit of this thing type.  Return true if successful.
