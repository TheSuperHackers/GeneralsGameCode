...
public: 
	...
	{	
		if (object)
		{
			MemoryPool *pool = object->getObjectMemoryPool(); // save this, since the dtor will nuke our vtbl
			object->~MemoryPoolObject();	// it's virtual, so the right one will be called.
			pool->freeBlock((void *)object); 
		}
	} 
};
