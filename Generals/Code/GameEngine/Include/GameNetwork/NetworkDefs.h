...
enum ConnectionNumbers CPP_11( : Int){ MAX_PLAYER = 7, // The index of the highest possible player number.  This is 0 based,
...
static const Int numCommandsPerCommandPacket =
		(MAX_MESSAGE_LEN - sizeof(UnsignedInt) - sizeof(UnsignedShort)) / sizeof(GameMessage);
#pragma pack(push, 1)
 ...
 * Message types
 */
enum NetMessageFlag CPP_11( : Int){ MSG_ACK = 1, MSG_NEEDACK = 2, MSG_SEQUENCED = 4, MSG_SUPERCEDING = 8 };
typedef UnsignedByte NetMessageFlags;

enum NetCommandType CPP_11( : Int){ NETCOMMANDTYPE_UNKNOWN = -1,
																		...
																		NETCOMMANDTYPE_WRAPPER, // A wrapper command that holds a command thats too big to fit in
																														// a single packet.
																		NETCOMMANDTYPE_FILE,
...
enum NetLocalStatus CPP_11( : Int){ NETLOCALSTATUS_PREGAME = 0,
																		NETLOCALSTATUS_INGAME,
																		NETLOCALSTATUS_LEAVING,
																		NETLOCALSTATUS_LEFT,
																		NETLOCALSTATUS_POSTGAME };

enum PlayerLeaveCode CPP_11( : Int){ PLAYERLEAVECODE_CLIENT = 0,
