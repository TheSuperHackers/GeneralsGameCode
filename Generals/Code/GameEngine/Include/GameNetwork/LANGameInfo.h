...
class LANGameInfo : public GameInfo
{
  ...
  Int getSlotNum(
      UnicodeString userName); ///< Get the slot number corresponding to a specific user, or -1 if he's not present
  ...
  inline UnsignedInt getIP(int who) { return m_LANSlot[who].getIP(); }

  /// Set the IP of the Selected Player
  inline void setIP(int who, UnsignedInt IP) { m_LANSlot[who].setIP(IP); }

  /// set whether or not this is a direct connect game or not.
  inline void setIsDirectConnect(Bool isDirectConnect) { m_isDirectConnect = isDirectConnect; }

  /// returns whether or not this is a direct connect game or not.
  inline Bool getIsDirectConnect() { return m_isDirectConnect; }

  /// Set the Player Name
  inline void setPlayerName(int who, UnicodeString name) { m_LANSlot[who].setName(name); }
  ...
  {
    DEBUG_LOG(
        ("LANGameInfo::setPlayerLastHeard - changing player %d last heard from %d to %d",
         who,
         getPlayerLastHeard(who),
         lastHeard));
    if (m_LANSlot[who].isHuman())
...
void LANDisplayGameList(
    GameWindow *gameListbox,
    LANGameInfo *gameList); ///< Displays the list of games in a listbox, preserving selections
