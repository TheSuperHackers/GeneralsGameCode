...
public:
    ...
    virtual UnicodeString fetch(
        AsciiString label,
        Bool *exists = NULL) = 0; ///< Returns the associated labeled unicode text ; TheSuperHackers @todo Remove
...
#define FETCH_OR_SUBSTITUTE_FORMAT(labelA, substituteFormatW, ...) \
    fetchOrSubstituteFormat(labelA, substituteFormatW, __VA_ARGS__)
#endif
