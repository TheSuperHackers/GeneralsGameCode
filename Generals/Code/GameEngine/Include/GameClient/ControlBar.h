...
enum CommandOption CPP_11( : Int){
	...
	CONTEXTMODE_COMMAND = 0x00000200, // a context sensitive command mode that requires code to determine whether cursor is
																		// valid or not.
	...
	ATTACK_OBJECTS_POSITION = 0x00001000, // for weapons that need an object target but attack the position indirectly (like
																				// burning trees)
...
private:
	...
	AsciiString m_conflictingLabel; ///< Description for the current command if it can't be selected due to multually-exclusive
																	///< choice.
...
protected:
	...
	static ContainEntry m_containData[MAX_COMMANDS_PER_SET]; ///< inventory buttons integrated into the regular command set for
