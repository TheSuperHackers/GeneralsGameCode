...
static const char *TheRadiusCursorNames[] = { "NONE",
																							"ATTACK_DAMAGE_AREA",
																							"ATTACK_SCATTER_AREA",
...
public: // ***************************************************************************************
	...
	virtual void messageNoFormat(const RGBColor *rgbColor, const UnicodeString &message); ///< display a colored message to the
																																												///< user
	...
	virtual void beginAreaSelectHint(const GameMessage *msg); ///< Used by HintSpy. An area selection is occurring, start
																														///< graphical "hint"
	virtual void endAreaSelectHint(const GameMessage *msg); ///< Used by HintSpy. An area selection had occurred, finish
																													///< graphical "hint"
	...
	virtual void createForceAttackHint(const GameMessage *msg); ///< A force attack command has occurred, start graphical
																															///< "hint"

	virtual void createMouseoverHint(const GameMessage *msg); ///< An object is mouse hovered over, start hint if any
	virtual void createCommandHint(const GameMessage *msg); ///< Used by HintSpy. Someone is selected so generate the right
																													///< Cursor for the potential action
	virtual void createGarrisonHint(const GameMessage *msg); ///< A garrison command has occurred, start graphical "hint"

	virtual void addSuperweapon(
			Int playerIndex,
			const AsciiString &powerName,
			ObjectID id,
			const SpecialPowerTemplate *powerTemplate);
	virtual Bool removeSuperweapon(
			Int playerIndex,
			const AsciiString &powerName,
			ObjectID id,
			const SpecialPowerTemplate *powerTemplate);
	virtual void objectChangedTeam(const Object *obj, Int oldPlayerIndex, Int newPlayerIndex); // notification for
																																														 // superweapons, etc
	...
	virtual void setGUICommand(const CommandButton *command); ///< the command has been clicked in the UI and needs additional
																														///< data
	...
	{
		m_preventLeftClickDeselectionInAlternateMouseModeForOneClick = set;
	}
	...
	{
		return m_frameSelectionChanged;
	} ///< Get the max number of selected drawables
	virtual const DrawableList *getAllSelectedDrawables(void) const; ///< Return the list of all the currently selected
																																	 ///< Drawable IDs.
	virtual const DrawableList *getAllSelectedLocalDrawables(void); ///< Return the list of all the currently selected Drawable
																																	///< IDs owned by the current player.
	...
	{
		return m_soloNexusSelectedDrawableID;
	...
	virtual Bool areAllObjectsSelected(const std::vector<Object *> &objectsToCheck) const; ///< Return true if all of the
																																												 ///< selected objects are in the
																																												 ///< drawable list
	...
	virtual void disregardDrawable(Drawable *draw); ///< Drawable is being destroyed, clean up any UI elements associated with
																									///< it
	...
	CanAttackResult getCanSelectedObjectsAttack(
			ActionType action,
			const Object *objectToInteractWith,
			SelectionRules rule,
			Bool additionalChecking = FALSE) const;
	Bool canSelectedObjectsDoAction(
			ActionType action,
			const Object *objectToInteractWith,
			SelectionRules rule,
			Bool additionalChecking = FALSE) const;
	Bool canSelectedObjectsDoSpecialPower(
	...
	Bool canSelectedObjectsOverrideSpecialPowerDestination(
			const Coord3D *loc,
			SelectionRules rule,
	...
	virtual void buildRegion(const ICoord2D *anchor, const ICoord2D *dest, IRegion2D *region); ///< builds a region around the
...
protected:
	...
	struct MilitarySubtitleData
	{
		...
		UnsignedInt currentDisplayString; ///< contains the current display string we're on. (also lets us know the last display
																			///< string allocated
	...
	void addMessageText(const UnicodeString &formattedMessage, const RGBColor *rgbColor = NULL); ///< internal workhorse for
																																															 ///< adding plain text for
																																															 ///< messages
	...
	DrawableID m_soloNexusSelectedDrawableID; ///< The drawable of the nexus, if only one angry mob is selected, otherwise,
																						///< null
