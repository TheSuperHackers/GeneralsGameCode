...
public:
	...
	virtual Bool setDisplayMode(UnsignedInt xres, UnsignedInt yres, UnsignedInt bitdepth, Bool windowed); ///< sets screen
																																																				///< resolution/mode
	virtual Int getDisplayModeCount(void)
	{
		return 0;
	} ///< return number of display modes/resolutions supported by video
		///< card.
	virtual void getDisplayModeDescription(Int modeIndex, Int *xres, Int *yres, Int *bitDepth) {} ///< return description of
																																																///< mode
	virtual void setGamma(Real gamma, Real bright, Real contrast, Bool calibrate) {};
	virtual Bool testMinSpecRequirements(
			Bool *videoPassed,
			Bool *cpuPassed,
			Bool *memPassed,
	...
	virtual void createLightPulse(
			const Coord3D *pos,
			const RGBColor *color,
			Real innerRadius,
			Real attenuationWidth,
			UnsignedInt increaseFrameTime,
			UnsignedInt decayFrameTime //, Bool donut = FALSE
			) = 0;
	...
	virtual void drawImage(
			...
			DrawImageMode mode = DRAW_IMAGE_ALPHA) = 0;
