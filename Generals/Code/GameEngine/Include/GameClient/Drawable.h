...
enum DrawableStatus CPP_11( : Int){
	...
	DRAWABLE_STATUS_NO_SAVE = 0x00000010, ///< do *not* save this drawable (UI fluff only). ignored (error, actually) if
...
public:
	...
	void updateDrawableClipStatus(
			...
			WeaponSlotType slot); ///< This will do the
														///< show/hide work
														///< if
														///< ProjectileBoneFeedbackEnabled
														///< is set.
...
private:
	...
	mutable Bool m_isModelDirty; ///< if true, must call replaceModelConditionState() before drawing or accessing drawmodule
															 ///< info
#endif
