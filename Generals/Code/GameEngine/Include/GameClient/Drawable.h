...
enum DrawableStatus CPP_11( : Int){
    ...
    DRAWABLE_STATUS_NO_SAVE =
        0x00000010, ///< do *not* save this drawable (UI fluff only). ignored (error, actually) if attached to an object
};
...
public:
    ...
    void updateDrawableClipStatus(
        UnsignedInt shotsRemaining,
        UnsignedInt maxShots,
        WeaponSlotType slot); ///< This will do the show/hide work if ProjectileBoneFeedbackEnabled is set.
    ...
    {
        return (ClientUpdateModule const **)getModuleList(MODULETYPE_CLIENT_UPDATE);
    }
    ...
    {
        return BitIsSet(m_status, DRAWABLE_STATUS_DRAWS_IN_MIRROR) || isKindOf(KINDOF_CAN_CAST_REFLECTIONS);
    }
    ...
    void colorFlash(
        ...
        UnsignedInt sustainAtPeak = FALSE); ///< flash a drawable in the color specified for a short time
    ...
    */
    Int getPristineBonePositions(
        const char *boneNamePrefix,
        Int startIndex,
        Coord3D *positions,
        Matrix3D *transforms,
        Int maxBones) const;
    Int getCurrentClientBonePositions(
        const char *boneNamePrefix,
        Int startIndex,
        Coord3D *positions,
        Matrix3D *transforms,
    ...
    Bool getProjectileLaunchOffset(
        WeaponSlotType wslot,
        Int specificBarrelToUse,
        Matrix3D *launchPos,
        WhichTurretType tur,
    ...
    */
    ...
    inline void setDrawableOpacity(Real value)
    {
        m_explicitOpacity = value;
    ...
    inline Real getEffectiveOpacity() const
    {
        return m_explicitOpacity * m_effectiveStealthOpacity;
    ...
    // this is for the heatvision effect which operates completely independently of the stealth opacity effects. Draw() does
    // the fading every frame.
    inline Real getHeatVisionOpacity() const
    {
        return m_heatVisionOpacity;
    } ///< get alpha/opacity value used to render add'l heatvision rendering pass.
    void setHeatVisionOpacity(Real op)
    {
        m_heatVisionOpacity = op;
    }; ///< set alpha/opacity value used to render add'l heatvision rendering pass.

    // both of these assume that you are starting at one extreme 100% or 0% opacity and are trying to go to the other!! --
    // amit
    ...
    void saturateRGB(
        RGBColor &color,
        Real factor); // not strictly for flash color, but it is the only practical use for this
    //---------------------------------------------------------------------------------
...
private:
    ...
    mutable Bool
        m_isModelDirty; ///< if true, must call replaceModelConditionState() before drawing or accessing drawmodule info
#endif
...
{
public:
    StDrawableDirtyStuffLocker() { Drawable::friend_lockDirtyStuffForIteration(); }
    ~StDrawableDirtyStuffLocker() { Drawable::friend_unlockDirtyStuffForIteration(); }
};
#endif
