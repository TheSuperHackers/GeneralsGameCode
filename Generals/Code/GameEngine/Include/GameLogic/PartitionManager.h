...
class CellAndObjectIntersection // not MPO: we allocate these in arrays
{
  ...
  CellAndObjectIntersection *m_prevCoi,
      *m_nextCoi; ///< if in use, next/prev in this cell. if not in use, next/prev free in this module.
...
class PartitionData : public MemoryPoolObject
{
  ...
  GhostObject
      *m_ghostObject; ///< Temporary object used when real object is gone but player thinks it's there because of fog.
  ...
  */
  ...
  inline Int wasSeenByAnyPlayers()
      const ///< check if a player in the game has seen the object but is now looking at fogged version.
  {
    Int i = 0;
  ...
  */
  ...
  void friend_setShroudednessPrevious(
      Int playerIndex,
      ObjectShroudStatus status); ///< only used to restore state after map border resizing and/or xfer!
...
class PartitionManager : public SubsystemInterface, public Snapshot
{
  ...
  std::queue<SightingInfo *> m_pendingUndoShroudReveals; ///< Anything can queue up an Undo to happen later. This is a
                                                         ///< queue, because "later" is a constant
  ...
  */
  Object *getClosestObjects(
      ...
      SimpleObjectIterator
          *iter, // if nonnull, append ALL satisfactory objects to the iterator (not just the single closest)
      Real *closestDistArg,
  ...
  void processPendingUndoShroudRevealQueue(
      Bool considerTimestamp = TRUE); ///< keep popping and processing untill you get to one that is in the future
  ...
  void registerGhostObject(
      GhostObject *object); ///< recreate partition data needed to hold object (only used to restore after PM reset).
