...
public:
	...
	{
		return UpdateModule::getInterfaceMask() | (MODULEINTERFACE_CONTAIN) | (MODULEINTERFACE_COLLIDE) | (MODULEINTERFACE_DIE)
	...
	virtual void orderAllPassengersToExit(CommandSourceType commandSource); ///< All of the smarts of exiting are in the
																																					///< passenger's AIExit. removeAllFrommContain is a
																																					///< last ditch system call, this is the game
																																					///< Evacuate
	virtual void markAllPassengersDetected(); ///< Cool game stuff got added to the system calls since this layer didn't exist,
																						///< so this regains that functionality
	...
	virtual void setOverrideDestination(const Coord3D *) {} ///< Instead of falling peacefully towards a clear spot, I will now
																													///< aim here
	...
	virtual Bool getNaturalRallyPoint(Coord3D &rallyPoint, Bool offset = TRUE) const; ///< get the natural "rally point" for
																																										///< units to move towards
...
protected:
	virtual void monitorConditionChanges(void); ///< check to see if we need to update our occupant postions from a model
	...
	void removeFromContainViaIterator(ContainedItemsList::iterator it, Bool exposeStealthUnits = FALSE); ///< remove item from
																																																			 ///< contain list
