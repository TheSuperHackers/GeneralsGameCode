...
public:
	...
	Real m_selfSpinUpdateDelay; ///< (frames) every this many frames we will update the self spin angle, but we'll keep it
															///< inbetween the min and max self spin
	Real m_selfSpinUpdateAmount; ///< (radian) when we update the self spin every SelfSpinUpdateDelay frames, we change it this
															 ///< much, but keep it between min and max self spin
