...
//-------------------------------------------------------------------------------------------------
enum ObjectEnterExitType CPP_11( : Int){ WANTS_TO_ENTER, WANTS_TO_EXIT, WANTS_NEITHER };
...
class ContainModuleInterface
{
  ...
  */
  ...
  virtual void onCapture(Player *oldOwner, Player *newOwner) = 0; // Very important to handle capture of container, don't
                                                                  // want to differ in teams from passenger to us.
  ...
  virtual Int getContainMax()
      const = 0; ///< The max needs to be virtual, but only two inheritors care.  -1 means "I don't care".

  virtual ExitInterface *getContainExitInterface() = 0;

  virtual void orderAllPassengersToExit(
      CommandSourceType) = 0; ///< All of the smarts of exiting are in the passenger's AIExit. removeAllFrommContain is a
                              ///< last ditch system call, this is the game Evacuate
  virtual void markAllPassengersDetected() = 0; ///< Cool game stuff got added to the system calls since this layer didn't
                                                ///< exist, so this regains that functionality
  ...
  */
  ...
  virtual void addToContainList(Object *obj) = 0; ///< The part of AddToContain that inheritors can override (Can't do
                                                  ///< whole thing because of all the private stuff involved)
  ...
  virtual Bool isEnclosingContainerFor(
      const Object *obj) const = 0; ///< Does this type of Contain Visibly enclose its contents?
  virtual Bool isPassengerAllowedToFire() const = 0; ///< Hey, can I shoot out of this container?
  virtual void setOverrideDestination(
      const Coord3D *) = 0; ///< Instead of falling peacefully towards a clear spot, I will now aim here
  ...
  virtual const Object *friend_getRider() const = 0; ///< Damn.  The draw order dependency bug for riders means that our
                                                     ///< draw module needs to cheat to get around it.
