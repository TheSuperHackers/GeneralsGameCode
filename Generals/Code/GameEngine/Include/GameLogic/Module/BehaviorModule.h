...
class BehaviorModuleData : public ModuleData
{
  ...
  static void buildFieldParse(MultiIniFieldParse &p) { ModuleData::buildFieldParse(p); }
};
...
{
  public:
  virtual Bool tryToEvacuate(Bool exposeStealthedUnits) = 0; ///< Will try to kick everybody out with game checks, and will
                                                             ///< return whether anyone made it
};
...
{
  public:
  virtual void tryToSetCaveIndex(
      Int newIndex) = 0; ///< Called by script as an alternative to instancing separate objects.  'Try', because can fail.
  virtual void setOriginalTeam(
      Team *oldTeam) = 0; ///< This is a distributed Garrison in terms of capturing, so when one node triggers the change,
                          ///< he needs to tell everyone, so anyone can do the un-change.
};
