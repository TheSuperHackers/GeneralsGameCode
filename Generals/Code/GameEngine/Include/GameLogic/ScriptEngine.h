...
public:
    ...
    virtual void runObjectScript(
        const AsciiString &scriptName,
        Object *pThisObject = NULL); ///<  Runs a script attached to this object.
    ...
    virtual void doObjectTypeListMaintenance(
    ...
    virtual void notifyOfCompletedVideo(
        const AsciiString &completedVideo); ///< Notify the script engine that a video has completed
    ...
    virtual void signalUIInteract(const AsciiString &hookName); ///< Notify that a UI button was pressed and some flag should
                                                                ///< go true, for one frame only.

    virtual Bool isVideoComplete(
        const AsciiString &completedVideo,
        Bool removeFromList); ///< Determine whether a video has completed
    virtual Bool isSpeechComplete(
        const AsciiString &completedSpeech,
        Bool removeFromList); ///< Determine whether a speech has completed
    virtual Bool isAudioComplete(
        const AsciiString &completedAudio,
        Bool removeFromList); ///< Determine whether a sound has completed
    virtual Bool isSpecialPowerTriggered(
        Int playerIndex,
        const AsciiString &completedPower,
        Bool removeFromList,
        ObjectID sourceObj);
    virtual Bool isSpecialPowerMidway(
        Int playerIndex,
        const AsciiString &completedPower,
        Bool removeFromList,
        ObjectID sourceObj);
    virtual Bool isSpecialPowerComplete(
        Int playerIndex,
        const AsciiString &completedPower,
        Bool removeFromList,
        ObjectID sourceObj);
