 ...
 */
enum TurretStateType CPP_11( : Int){ TURRETAI_IDLE,			 TURRETAI_IDLESCAN,
...
public:
	TurretAIHoldTurretState(TurretStateMachine *machine) : TurretState(machine, "AIHoldTurretState") { m_timestamp = 0; }
	virtual StateReturnType onEnter();
	virtual void onExit(StateExitType status);
	virtual StateReturnType update();
...
public:
	...
	Real m_minPitch; ///< dependent on allowspitch. defaults to 0 (horizontal). The lowest pitch allowed (negative to allow
									 ///< pointing down of a high turret)
	Real m_groundUnitPitch; ///< dependent on allowspitch. defaults to 0 (horizontal). The lowest pitch allowed when firing at
													///< ground units to give the weapon an arc.  jba
...
//-----------------------------------------------------------------------------------------------------
enum TurretTargetType CPP_11( : Int){ TARGET_NONE, TARGET_OBJECT, TARGET_POSITION };
...
private:
	...
	Team *m_victimInitialTeam; // The team of the victim at the BEGINNING of the attack. If it changes, we may need to stop the
														 // attack.
