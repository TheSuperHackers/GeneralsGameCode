...
class TurretAIData : public MemoryPoolObject
{
  ...
  Real m_groundUnitPitch; ///< dependent on allowspitch. defaults to 0 (horizontal). The lowest pitch allowed when firing
                          ///< at ground units to give the weapon an arc.  jba
...
class TurretAI : public MemoryPoolObject, public Snapshot, public NotifyWeaponFiredInterface
{
  ...
  */
  ...
  Team *m_victimInitialTeam; // The team of the victim at the BEGINNING of the attack. If it changes, we may need to stop
                             // the attack.
