...
protected:
	...
	void processDestroyPlayerCommand(NetDestroyPlayerCommandMsg *msg); ///< Do what needs to be done when we need to destroy a
																																		 ///< player.
	void endOfGameCheck(); ///< Checks to see if its ok to leave this game.  If it is, send the apropriate command to the game
												 ///< logic.
...
void Network::GetCommandsFromCommandList()
{
	...
	while (msg != NULL)
	{
		...
		{ // Is this something we should be sending to the other players?
			...
			TheCommandList->removeMessage(msg); // This does not destroy msg's prev and next pointers, so they should still be
																					// valid.
			deleteInstance(msg);
		}
		else
...
void Network::RelayCommandsToCommandList(UnsignedInt frame)
{
	...
	while (msg != NULL)
	{
		...
		if (cmdType == NETCOMMANDTYPE_GAMECOMMAND)
		{
			...
			// msg->getCommand()->getID(), ((NetGameCommandMsg *)msg->getCommand())->constructGameMessage()->getCommandAsString(),
			// TheGameLogic->getFrame()));
...
Bool Network::timeForNewFrame()
{
	...
	if (m_conMgr != NULL)
	{
		...
		Real runAheadPercentage = m_runAhead * (TheGlobalData->m_networkRunAheadSlack / (Real)100.0); // If we are at least 50%
																																																	// into our slack, we need
																																																	// to slow down.
