...
Bool ConnectionManager::processNetCommand(NetCommandRef *ref)
{
  ...
  {
    DEBUG_LOG(
        ("ConnectionManager::processNetCommand - got a TimeOut GameStart net command from player %d", msg->getPlayerID()));
...
void ConnectionManager::processWrapper(NetCommandRef *ref)
{
  ...
  DEBUG_LOG_LEVEL(
      DEBUG_LEVEL_NET,
      ("ConnectionManager::processWrapper() - origProgress[%d] == %d for command %d", m_localSlot, origProgress, commandID));
  ...
  if (fcIt != s_fileCommandMap.end())
  {
    ...
    DEBUG_LOG_LEVEL(
        DEBUG_LEVEL_NET,
        ("ConnectionManager::processWrapper() - newProgress[%d] == %d for command %d", m_localSlot, newProgress, commandID));
    ...
    if ((m_packetRouterSlot >= 0) && (m_packetRouterSlot < MAX_SLOTS))
    {
      ...
      else if (m_localSlot == m_packetRouterSlot)
      {
        ...
        if ((msg->getPlayerID() >= 0) && (msg->getPlayerID() < MAX_SLOTS))
        {
          if (m_connections[msg->getPlayerID()] != NULL)
          {
            //						DEBUG_LOG(("ConnectionManager::ackCommand - acking command %d from player %d directly
            // to player.", commandID, msg->getPlayerID()));
            m_connections[msg->getPlayerID()]->sendNetCommandMsg(ackmsg, 1 << msg->getPlayerID());
          }
          else
          {
            //					DEBUG_ASSERTCRASH(m_connections[msg->getPlayerID()] != NULL, ("Connection to player is
            // NULL"));
          }
        }
        else
  ...
  if (m_localSlot == m_packetRouterSlot)
  {
    ...
    for (Int i = 0; i < MAX_SLOTS; ++i)
    {
      ...
      if (((m_connections[i] != NULL) && (m_connections[i]->isQuitting() == FALSE)) && (relay & (1 << i)))
      {
        ...
        m_connections[i]->sendNetCommandMsg(msg,
...
//		if ((m_localAddr != addr) || (m_localPort != port)) {
  ...
  if (numUsers < 2 || m_localSlot == -1)
  {
    DEBUG_LOG(("ConnectionManager::parseUserList - FAILED (local user = %d, num players = %d) with list [%s]", m_localSlot,
numUsers, buf)); return;
  }
