    ...
    for (; i < MAX_SPARE_SOCKETS; ++i)
    {
        if (m_spareSockets[i].udp != NULL)
        {
            ...
            if (retval > 0)
            {
                ...
                {
                    DEBUG_LOG(
                        ("FirewallHelperClass::getManglerResponse - Saw message, CRC mismatch.  Expected CRC %u, computed "
                         "CRC %u",
                         message->data.CRC,
                         crc.get()));
                    continue;
                ...
                DEBUG_LOG(
                    ("FirewallHelperClass::getManglerResponse - Message has packet ID %d 0x%08X, looking for packet id %d "
                     "0x%08X",
                ...
                {
                    DEBUG_LOG(
                        ("FirewallHelperClass::getManglerResponse - NETWORK BYTE ORDER packet ID's match, returning "
                         "message"));
                    msg = message;
                    message->length = 0;
                }
...
Bool FirewallHelperClass::detectionTest2Update()
{
    ...
    */
    ...
    {
        DEBUG_LOG(
            ("FirewallHelperClass::detectionTest2Update - port mangling depends on destination IP, setting to "
             "SMART_MANGLING"));
        ...
        for (; i < NUM_TEST_PORTS; i++)
        {
            ...
            if (!openSpareSocket(m_sparePorts[i]))
            {
                ...
                */
                ...
                DEBUG_LOG(
                    ("FirewallHelperClass::detectionTest3Update - Failed to open all the spare sockets, bailing test"));
                m_currentState = DETECTIONSTATE_DONE;
                return TRUE;
            }
        }
...
Bool FirewallHelperClass::detectionTest3WaitForResponsesUpdate()
{
    ...
    */
    if (m_numResponses < 4)
    {
        ...
        DEBUG_LOG(
            ("FirewallHelperClass::detectionTest3WaitForResponsesUpdate - didn't get enough responses, using %d as the "
             "source port allocation delta, finished test",
    ...
    else
    {
        ...
        if (m_lastSourcePortAllocationDelta != 0 && (Int)m_lastBehavior > (Int)FIREWALL_TYPE_SIMPLE)
        {
            ...
            */
            DEBUG_LOG(
                ("FirewallHelperClass::detectionTest3WaitForResponsesUpdate - using the port allocation delta we have from "
                 "before which is %d",
    ...
    */
    if ((m_behavior & FIREWALL_TYPE_SMART_MANGLING) != 0)
    {
        ...
        {
            DEBUG_LOG(
                ("FirewallHelperClass::detectionTest3WaitForRepsonsesUpdate - simple port allocation, Testing to see if the "
                 "NAT mangles differently per destination port at the same IP"));
            ...
            */
            ...
            if (!openSpareSocket(m_sparePorts[0]))
            {
                m_currentState = DETECTIONSTATE_DONE;
                DEBUG_LOG((
                    "FirewallHelperClass::detectionTest3WaitForRepsonsesUpdate - Failed to open first spare port, bailing"));
                return TRUE;
            }
            ...
            if (!openSpareSocket(m_sparePorts[1]))
            {
                ...
                m_currentState = DETECTIONSTATE_DONE;
                DEBUG_LOG(
                    ("FirewallHelperClass::detectionTest3WaitForRepsonsesUpdate - Failed to open second spare port, "
                     "bailing"));
                return TRUE;
            }
        ...
        else
        {
            ...
            */
            DEBUG_LOG(
                ("FirewallHelperClass::detectionTest3WaitForRepsonsesUpdate - relative port allocation, NAT32 right?"));
            UnsignedInt addbehavior = 0;
            addbehavior = (UnsignedInt)FIREWALL_TYPE_DESTINATION_PORT_DELTA;
            DEBUG_LOG(
                ("FirewallHelperClass::detectionTest3WaitForRepsonsesUpdate - adding DESTINATION PORT DELTA to behavior"));
            addbehavior |= (UnsignedInt)m_behavior;
            m_behavior = (FirewallBehaviorType)addbehavior;
        }
    }
...
Bool FirewallHelperClass::detectionTest4Stage1Update()
{
    ...
    if (m_mangledPorts[0] == 0)
    {
        if ((timeGetTime() - m_timeoutStart) > m_timeoutLength)
        {
            ...
            m_currentState = DETECTIONSTATE_DONE;
            DEBUG_LOG(
                ("FirewallHelperClass::detectionTest4Stage1Update - timed out waiting for mangler response, quitting"));
            return TRUE;
        }
        return FALSE;
    }
...
Bool FirewallHelperClass::detectionTest4Stage2Update()
{
    ...
    if (m_mangledPorts[1] == 0)
    {
        if ((timeGetTime() - m_timeoutStart) > m_timeoutLength)
        {
            ...
            m_currentState = DETECTIONSTATE_DONE;
            DEBUG_LOG(
                ("FirewallHelperClass::detectionTest4Stage2Update - timed out waiting for the second mangler response, "
                 "quitting"));
            return TRUE;
        }
        return FALSE;
    ...
    else
    {
        ...
        {
            DEBUG_LOG(
                ("FirewallHelperClass::detectionTest4Stage2Update - Unable to complete destination port mangling test"));
