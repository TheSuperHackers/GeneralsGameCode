...
void LANAPI::handleRequestJoin(LANMessage *msg, UnsignedInt senderIP)
{
    ...
    if (!m_inLobby && m_currentGame && m_currentGame->getIP(0) == m_localIP)
    {
        ...
        else
        {
            ...
            {
#endif
                if (msg->GameToJoin.iniCRC != TheGlobalData->m_iniCRC || msg->GameToJoin.exeCRC != TheGlobalData->m_exeCRC)
                {
                    DEBUG_LOG(
                        ("LANAPI::handleRequestJoin - join denied because of CRC mismatch. CRCs are them/us INI:%X/%X "
                         "exe:%X/%X",
                         msg->GameToJoin.iniCRC,
                         TheGlobalData->m_iniCRC,
                         msg->GameToJoin.exeCRC,
            ...
            if (numPlayers < numStartingSpots)
            {
                for (player = 0; canJoin && player < MAX_SLOTS; ++player)
                {
                    if (m_currentGame->getLANSlot(player)->isOpen())
                    {
                        ...
                        DEBUG_LOG(
                            ("LANAPI::handleRequestJoin - added player %ls at ip 0x%08x to the game", msg->name, senderIP));
        ...
        for (player = 0; player < MAX_SLOTS; ++player)
        {
            if (m_currentGame->getIP(player) == senderIP)
            {
                if (player == 0)
                {
                    ...
                    /// @todo re-add myself to lobby?  Or just keep me there all the time?  If we send a LOBBY_ANNOUNCE
                    /// things'll work out...
        ...
        for (player = 0; player < MAX_SLOTS; ++player)
        {
            if (m_currentGame && m_currentGame->getIP(player) == senderIP)
            {
                OnChat(
                    ...
                    UnicodeString(msg->Chat.message),
                    msg->Chat.chatType);
                break;
            }
        }
