...
void NetPacket::FillBufferWithGameCommand(UnsignedByte *buffer, NetCommandRef *msg)
{
  ...
  //		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket::addGameMessage - added game message, frame %d, player %d, command ID
...
Bool NetPacket::addDisconnectVoteCommand(NetCommandRef *msg)
{
  ...
  if (isRoomForDisconnectVoteMessage(msg))
  {
    ...
    //		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket::addDisconnectVoteCommand - added disconnect vote command, player
    // id %d command id %d, voted slot %d", m_lastPlayerID, m_lastCommandID, slot));
      ...
      case 'D':
      {
        ...
        switch ((NetCommandType)commandType)
        {
          ...
          case NETCOMMANDTYPE_PLAYERLEAVE:
            ...
            //				DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("read player leave message from player %d for execution
          ...
          case NETCOMMANDTYPE_RUNAHEAD:
            ...
            //				DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("read run ahead message from player %d for execution on
            // frame %d", playerID, frame));
            break;
          case NETCOMMANDTYPE_DESTROYPLAYER:
            msg = readDestroyPlayerMessage(m_packet, i);
            //				DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("read CRC info message from player %d for execution on
            // frame %d", playerID, frame));
            break;
          case NETCOMMANDTYPE_KEEPALIVE:
      ...
      case 'Z':
      {
        ...
        switch (commandType)
        {
          ...
          case NETCOMMANDTYPE_FRAMEINFO:
          {
            ...
            DEBUG_LOG_LEVEL(
                DEBUG_LEVEL_NET,
                ("Read a repeated frame command, frame = %d, player = %d, commandID = %d", frame, playerID, commandID));
            break;
          }
          default:
