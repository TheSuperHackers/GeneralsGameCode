...
{
    DEBUG_LOG(
        ("GameSpyInfo::getAdditionalDisconnects() would have returned %d.  Returning 0 instead.", m_additionalDisconnects));
    return 0;
}
    ...
    for (i = 0; i < MAX_SLOTS; ++i)
    {
        ...
        else
        {
            ...
            if (slot->isHuman())
            {
                ...
                req.gameOptions.faction[req.gameOptions.numObservers + req.gameOptions.numPlayers] =
...
void GameSpyInfo::addGroupRoom(GameSpyGroupRoom room)
{
    if (room.m_groupID == 0)
    {
        ...
        if (!names.empty() && names.size() != numRooms)
        {
            ...
            for (iter = TheGameSpyInfo->getGroupRoomList()->begin(); iter != TheGameSpyInfo->getGroupRoomList()->end();
                 ++iter)
            {
                GameSpyGroupRoom room = iter->second;
                if (room.m_groupID != TheGameSpyConfig->getQMChannel())
...
void GameSpyInfo::updateAdditionalGameSpyDisconnections(Int count)
{
    if (TheRecorder->isMultiplayer() && TheGameLogic->isInInternetGame())
    {
        ...
        if (anyAI || stats.id == 0 || myTemplate->isObserver() || player->getPlayerType() != PLAYER_HUMAN
