...
static void gameTooltip(GameWindow *window, WinInstanceData *instData, UnsignedInt mouse)
{
    ...
    if (col == COLUMN_PING)
    {
        ...
        TheMouse->setCursorTooltip(
            ...
            10,
            NULL,
            2.0f); // the text and width are the only params used.  the others are the default values.
#else
        TheMouse->setCursorTooltip(
            TheGameText->fetch("TOOLTIP:PingInfo"),
            10,
            NULL,
            2.0f); // the text and width are the only params used.  the others are the default values.
#endif
    ...
    {
        TheMouse->setCursorTooltip(
            TheGameText->fetch("TOOLTIP:NumberOfPlayers"),
            10,
            NULL,
            2.0f); // the text and width are the only params used.  the others are the default values.
        return;
    ...
    for (Int i = 0; i < MAX_SLOTS; ++i)
    {
        ...
        {
            tmp.format(
                TheGameText->fetch("TOOLTIP:GameInfoPlayer"),
                slot->getName().str(),
                slot->getWins(),
                slot->getLosses());
            tooltip.concat(tmp);
