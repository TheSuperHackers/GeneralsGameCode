...
GameSpyGameSlot::GameSpyGameSlot() : GameSlot()
{
...
BOOL(__stdcall *SnmpExtensionQueryPtr)(
		IN BYTE bPduType,
...
Bool GetLocalChatConnectionAddress(AsciiString serverName, UnsignedShort serverPort, UnsignedInt &localIP)
{
	...
	*/
	...
	SnmpExtensionQueryPtr =
			(int(__stdcall *)(unsigned char, SnmpVarBindList *, long *, long *))GetProcAddress(mib_ii_dll, "SnmpExtensionQuery");
		...
		if (m_GameSpySlot[i].isHuman())
		{
			...
			if (m_isQM)
			{
				if (getLocalSlotNum() == i)
					m_GameSpySlot[i].setProfileID(TheGameSpyInfo->getLocalProfileID()); // hehe - we know our own.  the rest, they'll
																																							// tell us.
			}
			else
			{
...
AsciiString GameSpyStagingRoom::generateGameSpyGameResultsPacket(void)
{
	...
	results.format(
			...
			getSeed(),
			m_GameSpySlot[0].getLoginName().str(),
			mapName.str(),
			numPlayers,
			endFrame,
			localSlotNum);
	...
	for (i = 0; i < MAX_SLOTS; ++i)
	{
		...
		if (p)
		{
			...
			playerStr.format(
					"\\player_%d\\%s\\pid_%d\\%d\\team_%d\\%d\\result_%d\\%s\\side_%d\\%s",
					playerID,
					playerName.str(),
					playerID,
					gsPlayerID,
...
AsciiString GameSpyStagingRoom::generateLadderGameResultsPacket(void)
{
	...
	for (i = 0; i < MAX_SLOTS; ++i)
	{
		...
		if (p[i])
		{
			...
			AsciiString playerStr;
			playerStr.format(
					",player%d=%s,playerID%d=%d,locale%d=%d",
					playerID,
					playerName.str(),
					playerID,
					gsPlayerID,
			...
			results.concat(playerStr);
			playerStr.format(
					",fps%d=%d,cash%d=%d,capturedTech%d=%d,discon%d=%d,side%d=%s,team%d=%d",
					playerID,
					fps,
					playerID,
					earnings,
					...
					disconnected,
					playerID,
					p[i]->getPlayerTemplate()->getSide().str(),
					playerID,
					slot->getTeamNumber());
			results.concat(playerStr);
