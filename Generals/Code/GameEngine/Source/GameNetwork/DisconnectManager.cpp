		...
		if (conMgr->isPlayerConnected(i))
		{
			...
			if (slot != -1)
			{
				...
				if (m_haveNotifiedOtherPlayersOfCurrentFrame == FALSE)
				{
					...
					DEBUG_LOG(
							("DisconnectManager::updateDisconnectStatus - curTime = %d, m_timeOfDisconnectScreenOn = %d, curTime - "
				...
				if ((newTime < 0) || (isPlayerVotedOut(slot, conMgr) == TRUE))
				{
					newTime = 0;
					DEBUG_LOG(
							("DisconnectManager::updateDisconnectStatus - player %d(translated slot %d) has been voted out or timed out",
							 i,
							 slot));
					if (allOnSameFrame(conMgr) == TRUE)
					{
						...
						if (isLocalPlayerNextPacketRouter(conMgr) == TRUE)
						{
							...
							DEBUG_LOG(
									("DisconnectManager::updateDisconnectStatus - about to do the disconnect procedure for player %d", i));
...
void DisconnectManager::processDisconnectPlayer(NetCommandMsg *msg, ConnectionManager *conMgr)
{
	...
	DEBUG_LOG(
			("DisconnectManager::processDisconnectPlayer - Got disconnect player command from player %d.  Disconnecting player %d "
			 "on frame %d",
			 msg->getPlayerID(),
...
void DisconnectManager::processPacketRouterQuery(NetCommandMsg *msg, ConnectionManager *conMgr)
{
	...
	if (conMgr->getPacketRouterSlot() == conMgr->getLocalPlayerID())
	{
		...
		DEBUG_LOG(
				("DisconnectManager::processPacketRouterQuery - We are the new packet router, responding with an packet router ack. "
				 "Local player is %d",
				 ackmsg->getPlayerID()));
	...
	{
		DEBUG_LOG(
				("DisconnectManager::processPacketRouterQuery - We are NOT the new packet router, these are not the droids you're "
				 "looking for."));
	}
}
...
void DisconnectManager::processPacketRouterAck(NetCommandMsg *msg, ConnectionManager *conMgr)
{
	...
	if (conMgr->getPacketRouterSlot() == cmdMsg->getPlayerID())
	{
		...
		m_disconnectState = DISCONNECTSTATETYPE_SCREENON; ///< set it to screen on so that the next call to AllCommandsReady can
																											///< set up everything for the next frame properly.
	}
}
...
void DisconnectManager::processDisconnectFrame(NetCommandMsg *msg, ConnectionManager *conMgr)
{
	...
	{
		DEBUG_LOG(
				("DisconnectManager::processDisconnectFrame - Got two disconnect frames without an intervening disconnect screen "
				 "off command from player %d. Frames are %d and %d",
				 playerID,
		...
		for (Int i = 0; i < MAX_SLOTS; ++i)
		{
			if (i != playerID)
			{
				...
				if (isPlayerInGame(transSlot, conMgr) == TRUE)
				{
					...
					{
						DEBUG_LOG(
								("DisconnectManager::processDisconnectFrame - I have more frames than player %d, my frame = %d, their frame "
								 "= %d",
								 i,
								 m_disconnectFrames[conMgr->getLocalPlayerID()],
...
void DisconnectManager::processDisconnectScreenOff(NetCommandMsg *msg, ConnectionManager *conMgr)
{
	...
	{
		DEBUG_LOG(
				("DisconnectManager::processDisconnectScreenOff - resetting the disconnect screen status for player %d", playerID));
