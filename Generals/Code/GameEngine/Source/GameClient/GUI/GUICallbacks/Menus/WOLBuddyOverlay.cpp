...
void InitBuddyControls(Int type)
{
    ...
    switch (type)
    {
        ...
        case BUDDY_WINDOW_BUDDIES:
            ...
            buddyControls.listboxBuddiesID =
                TheNameKeyGenerator->nameToKey(AsciiString("WOLBuddyOverlay.wnd:ListboxBuddies"));
            buddyControls.listboxChatID =
                TheNameKeyGenerator->nameToKey(AsciiString("WOLBuddyOverlay.wnd:ListboxBuddyChat"));
        ...
        default:
            DEBUG_ASSERTCRASH(
                FALSE,
                ("Well, you really shouldn't have gotten here, if you really care about GUI Bugs, search for this string, "
                 "you you don't care, call chris (who probably doesn't care either"));
    }
}
            ...
            if (selected >= 0)
            {
                    ...
                    && !ThePlayerList->getLocalPlayer()->isPlayerActive())
                {
                    ...
                    for (Int i = 0; i < MAX_SLOTS; ++i)
                    {
                        ...
                        {
                            // can't send to someone in our game if we're dead/observing.  security breach and all that.  no
                            // seances for you.
    ...
    if (buddyControls.listboxChat)
    {
        ...
        else if (!localSender /*&& senderIt != m->end()*/)
        {
            ...
            else
            {
                ...
                Int index =
                    GadgetListBoxAddEntryText(buddyControls.listboxChat, s, GameSpyColor[GSCOLOR_PLAYER_BUDDY], -1, -1);
...
void updateBuddyInfo(void)
{
    ...
    for (bIt = buddies->begin(); bIt != buddies->end(); ++bIt)
    {
        ...
        if (!info.m_statusString.compareNoCase(L"Offline") || !info.m_statusString.compareNoCase(L"Online")
            || !info.m_statusString.compareNoCase(L"Matching"))
        {
            formatStr = TheGameText->fetch(marker);
        }
        else if (
            !info.m_statusString.compareNoCase(L"Staging") || !info.m_statusString.compareNoCase(L"Loading")
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType WOLBuddyOverlayInput(GameWindow *window, UnsignedInt msg, WindowMsgData mData1, WindowMsgData mData2)
{
    switch (msg)
    {
        // --------------------------------------------------------------------------------------------
        case GWM_CHAR:
        {
            ...
            switch (key)
            {
                ...
                case KEY_ESC:
                {
                    ...
                    {
                        TheWindowManager->winSendSystemMsg(window, GBM_SELECTED, (WindowMsgData)buttonHide, buttonHideID);
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType WOLBuddyOverlaySystem(GameWindow *window, UnsignedInt msg, WindowMsgData mData1, WindowMsgData mData2)
{
    UnicodeString txtInput;
    ...
    switch (msg)
    {
        ...
        case GBM_SELECTED:
        {
            ...
            else if (controlID == buttonDeleteBuddyID)
            {
                ...
                if (selected >= 0)
                    selectedName = TheNameKeyGenerator->keyToName((NameKeyType)(int)GadgetListBoxGetItemData(listbox,
                selected));
...
WindowMsgHandledType PopupBuddyNotificationSystem(
    GameWindow *window,
    UnsignedInt msg,
...
WindowMsgHandledType WOLBuddyOverlayRCMenuSystem(
    GameWindow *window,
    UnsignedInt msg,
    WindowMsgData mData1,
    WindowMsgData mData2)
{
    switch (msg)
    {
        ...
        case GBM_SELECTED:
        {
            ...
            DEBUG_ASSERTCRASH(
                rcData,
                ("WOLBuddyOverlayRCMenuSystem GBM_SELECTED:: we're attempting to read the GameSpyRCMenuData from the "
