...
static void restartMissionMenu()
{
	...
	else
	{
		...
		DEBUG_LOG(
				("Restarting game mode %d, Diff=%d, RankPoints=%d",
				 gameMode,
...
void ToggleQuitMenu()
{
	...
	if (TheShell->getOptionsLayout(FALSE) != FALSE)
	{
		...
		GameWindow *optionsBack = TheWindowManager->winGetWindowFromId(
				optionsParent,
				TheNameKeyGenerator->nameToKey(AsciiString("OptionsMenu.wnd:ButtonBack")));
	...
	if ((saveLoadMenuLayout != NULL) && (saveLoadMenuLayout->isHidden() == FALSE))
	{
		...
		GameWindow *saveLoadBack = TheWindowManager->winGetWindowFromId(
				saveLoadParent,
				TheNameKeyGenerator->nameToKey(AsciiString("PopupSaveLoad.wnd:ButtonBack")));
		DEBUG_ASSERTCRASH(saveLoadBack != NULL, ("Not able to get the back button window from the save/load menu"));
		TheWindowManager
				->winSendSystemMsg(saveLoadMenuLayout->getFirstWindow(), GBM_SELECTED, (WindowMsgData)saveLoadBack, NULL);
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType QuitMenuSystem(GameWindow *window, UnsignedInt msg, WindowMsgData mData1, WindowMsgData mData2)
{
	switch (msg)
	{
		// --------------------------------------------------------------------------------------------
		...
		case GBM_SELECTED:
		{
			...
			//			{
			//				quitConfirmationWindow = MessageBoxYesNo(TheGameText->fetch("GUI:QuitPopupTitle"),
			...
			else if (controlID == buttonRestart)
			{
				if (TheGameLogic->isInMultiplayerGame())
				{
					...
					quitConfirmationWindow = MessageBoxYesNo(
							...
							/*quitCallback*/ surrenderQuitMenu,
							noExitQuitMenu);
				}
				else
				{
					...
					quitConfirmationWindow = MessageBoxYesNo(
							...
							/*quitCallback*/ restartMissionMenu,
							noExitQuitMenu);
				}
			} // end else if
