...
static void processLoadButtonPress(GameWindow *window)
{
	...
	if (selectedGameInfo)
	{
		...
		else
		{
			...
			// "PopupSaveLoad.wnd:MenuButtonFrame" ) ); GameWindow *listboxGames = TheWindowManager->winGetWindowFromId( parent,
			// NAMEKEY( "PopupSaveLoad.wnd:ListboxGames" ) ); GameWindow *loadConfirm  = TheWindowManager->winGetWindowFromId(
		...
		case GBM_SELECTED:
		{
			...
			else if (controlID == buttonOverwriteCancel || controlID == buttonOverwriteConfirm)
			{
				...
				if (controlID == buttonOverwriteConfirm)
				{
					...
					DEBUG_ASSERTCRASH(
							selectedGameInfo,
							("SaveLoadMenuSystem: Internal error, listbox entry to overwrite game has no item data set into listbox "
							 "element"));
