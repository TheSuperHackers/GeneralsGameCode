...
static void playerTooltip(GameWindow *window, WinInstanceData *instData, UnsignedInt mouse)
{
  ...
  playerInfo.format(
      TheGameText->fetch("TOOLTIP:PlayerInfo"),
  ...
  if (listboxLobbyPlayers)
  {
    ...
    for (it = players->begin(); it != players->end(); ++it)
    {
      ...
      if (info.m_flags & PEER_FLAG_OP || TheGameSpyConfig->isPlayerVIP(info.m_profileID))
      {
        Int index = insertPlayerInListbox(
            info,
            info.isIgnored() ? GameSpyColor[GSCOLOR_PLAYER_IGNORED] : GameSpyColor[GSCOLOR_PLAYER_OWNER]);
    ...
    for (it = players->begin(); it != players->end(); ++it)
    {
      ...
      if (!(info.m_flags & PEER_FLAG_OP || TheGameSpyConfig->isPlayerVIP(info.m_profileID)) && bIt != buddies->end())
      {
        Int index = insertPlayerInListbox(
            info,
            info.isIgnored() ? GameSpyColor[GSCOLOR_PLAYER_IGNORED] : GameSpyColor[GSCOLOR_PLAYER_BUDDY]);
    ...
    for (it = players->begin(); it != players->end(); ++it)
    {
      ...
      if (!(info.m_flags & PEER_FLAG_OP || TheGameSpyConfig->isPlayerVIP(info.m_profileID)) && bIt == buddies->end())
      {
        Int index = insertPlayerInListbox(
            info,
            info.isIgnored() ? GameSpyColor[GSCOLOR_PLAYER_IGNORED] : GameSpyColor[GSCOLOR_PLAYER_NORMAL]);
...
void WOLLobbyMenuInit(WindowLayout *layout, void *userData)
{
  ...
  GameWindow *win =
      TheWindowManager->winGetWindowFromId(NULL, TheNameKeyGenerator->nameToKey("WOLCustomLobby.wnd:GadgetParent"));
  if (win)
    win->winHide(TRUE);
...
static void refreshGameList(Bool forceRefresh)
{
  ...
  if (forceRefresh || ((gameListRefreshTime == 0) || ((gameListRefreshTime + refreshInterval) <= timeGetTime())))
  {
    if (TheGameSpyInfo->hasStagingRoomListChanged())
    {
      ...
      // DEBUG_LOG(("gameRefreshTime=%d, refreshInterval=%d, now=%d", gameListRefreshTime, refreshInterval,
      // timeGetTime()));
      ...
      switch (resp.peerResponseType)
      {
        ...
        case PeerResponse::PEERRESPONSE_STAGINGROOM:
        {
          ...
          switch (resp.stagingRoom.action)
          {
            case PEER_CLEAR:
              TheGameSpyInfo->clearStagingRoomList();
              // TheGameSpyInfo->addText( UnicodeString(L"gameList: PEER_CLEAR"),
            ...
            case PEER_UPDATE:
            {
              ...
              if (serverOk)
              {
                ...
                // DEBUG_LOG(("Saw room: hasPass=%d, allowsObservers=%d", room.getHasPassword(),
                // room.getAllowObservers()));
                if (resp.stagingRoom.action == PEER_ADD)
                {
                  TheGameSpyInfo->addStagingRoom(room);
                  // TheGameSpyInfo->addText( UnicodeString(L"gameList: PEER_ADD"),
                  // GameSpyColor[GSCOLOR_DEFAULT], listboxLobbyChat );
                }
                else
                {
                  TheGameSpyInfo->updateStagingRoom(room);
                  // TheGameSpyInfo->addText( UnicodeString(L"gameList: PEER_UPDATE"),
                  // GameSpyColor[GSCOLOR_DEFAULT], listboxLobbyChat );
                }
              }
              else
              {
                ...
                // TheGameSpyInfo->addText( UnicodeString(L"gameList: PEER_UPDATE FAILED"),
                // GameSpyColor[GSCOLOR_DEFAULT], listboxLobbyChat );
              }
              break;
            }
            case PEER_REMOVE:
              room.setID(resp.stagingRoom.id);
              TheGameSpyInfo->removeStagingRoom(room);
              // TheGameSpyInfo->addText( UnicodeString(L"gameList: PEER_REMOVE"),
              // GameSpyColor[GSCOLOR_DEFAULT], listboxLobbyChat );
              break;
            default:
              // TheGameSpyInfo->addText( UnicodeString(L"gameList: Unknown"), GameSpyColor[GSCOLOR_DEFAULT],
              // listboxLobbyChat );
              break;
          }
        }
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType WOLLobbyMenuInput(GameWindow *window, UnsignedInt msg, WindowMsgData mData1, WindowMsgData mData2)
{
  switch (msg)
  {
    // --------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType WOLLobbyMenuSystem(GameWindow *window, UnsignedInt msg, WindowMsgData mData1, WindowMsgData mData2)
{
        ...
        if (selected >= 0)
        {
          ...
          if (selectedID > 0)
          {
            ...
            if (srmIt != srm->end())
            {
              ...
              if (!roomToJoin || roomToJoin->getExeCRC() != TheGlobalData->m_exeCRC
                  || roomToJoin->getIniCRC() != TheGlobalData->m_iniCRC)
              {
                // bad crc.  don't go.
                DEBUG_LOG(
                    ("WOLLobbyMenuSystem - CRC mismatch with the game I'm trying to join. My CRC's - "
                     "EXE:0x%08X INI:0x%08X  Their CRC's - EXE:0x%08x INI:0x%08x",
                     TheGlobalData->m_exeCRC,
                     TheGlobalData->m_iniCRC,
                     roomToJoin->getExeCRC(),
                     roomToJoin->getIniCRC()));
#if defined(RTS_DEBUG)
              ...
              Bool unknownLadder =
                  (roomToJoin->getLadderPort()
                   && TheLadderList->findLadder(roomToJoin->getLadderIP(), roomToJoin->getLadderPort()) == NULL);
              if (unknownLadder)
              {
                GSMessageBoxOk(
                    TheGameText->fetch("GUI:JoinFailedDefault"),
    ...
    case GLM_DOUBLE_CLICKED:
    {
      ...
      if (controlID == GetGameListBoxID())
      {
        ...
        if (rowSelected >= 0)
        {
          ...
          TheWindowManager->winSendSystemMsg(window, GBM_SELECTED, (WindowMsgData)button, buttonJoinID);
        }
      }
      break;
    } // case GLM_DOUBLE_CLICKED:
