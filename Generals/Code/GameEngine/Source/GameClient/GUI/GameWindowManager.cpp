...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType PassSelectedButtonsToParentSystem(
    GameWindow *window,
    UnsignedInt msg,
...
/** Generic function to simply propagate only button press messages to parent and let it deal with it */
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType PassMessagesToParentSystem(
    GameWindow *window,
    UnsignedInt msg,
    WindowMsgData mData1,
    WindowMsgData mData2)
...
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType GameWindowManager::winSendSystemMsg(
    GameWindow *window,
    UnsignedInt msg,
    WindowMsgData mData1,
    WindowMsgData mData2)
...
/** Send a system message to the specified window */
//-------------------------------------------------------------------------------------------------
WindowMsgHandledType GameWindowManager::winSendInputMsg(
    GameWindow *window,
    UnsignedInt msg,
    WindowMsgData mData1,
    WindowMsgData mData2)
...
//-------------------------------------------------------------------------------------------------
WinInputReturnCode GameWindowManager::winProcessKey(UnsignedByte key, UnsignedByte state)
{
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    if (BitIsSet(window->m_status, WIN_STATUS_ABOVE) && !BitIsSet(window->m_status, WIN_STATUS_HIDDEN)
                        && mousePos->x >= window->m_region.lo.x && mousePos->x <= window->m_region.hi.x
                        && mousePos->y >= window->m_region.lo.y && mousePos->y <= window->m_region.hi.y)
                    {
                ...
                if (window == NULL)
                {
                    for (window = m_windowList; window; window = window->m_next)
                    {
                        if (!BitIsSet(window->m_status, WIN_STATUS_ABOVE | WIN_STATUS_BELOW | WIN_STATUS_HIDDEN)
                            && mousePos->x >= window->m_region.lo.x && mousePos->x <= window->m_region.hi.x
                            && mousePos->y >= window->m_region.lo.y && mousePos->y <= window->m_region.hi.y)
                        {
                            childWindow = window->winPointInAnyChild(mousePos->x, mousePos->y, TRUE, TRUE);
                ...
                if (window == NULL)
                {
                    for (window = m_windowList; window; window = window->m_next)
                    {
                        if (BitIsSet(window->m_status, WIN_STATUS_BELOW) && !BitIsSet(window->m_status, WIN_STATUS_HIDDEN)
                            && mousePos->x >= window->m_region.lo.x && mousePos->x <= window->m_region.hi.x
                            && mousePos->y >= window->m_region.lo.y && mousePos->y <= window->m_region.hi.y)
                        {
                            childWindow = window->winPointInAnyChild(mousePos->x, mousePos->y, TRUE, TRUE);
            ...
            if (window)
                if (BitIsSet(window->m_status, WIN_STATUS_NO_INPUT))
                {
                    if (window->winGetParent()
                        && BitIsSet(window->winGetParent()->winGetInstanceData()->getStyle(), GWS_COMBO_BOX))
                        window = window->winGetParent();
                    else
                        window = NULL;
                }
                        ...
                        && (msg == GWM_LEFT_UP || msg == GWM_MIDDLE_UP || msg == GWM_RIGHT_UP || tempWin))
                    {
                        ...
                        /*
                                ComboBoxData *cData = (ComboBoxData *)m_comboBoxOpen->winGetUserData();
                                                            // verify that the window that ate the message wasn't one of our
                           own if(cData->dropDownButton != tempWin && cData->editBox != tempWin && cData->listBox != tempWin
                           && cData->listboxData->upButton != tempWin && cData->listboxData->downButton != tempWin &&
            ...
            if (tooltipsOn)
            {
                //				if( toolTipWindow && toolTipWindow->winGetParent() && BitIsSet(
                //toolTipWindow->winGetParent()->winGetInstanceData()->getStyle(), GWS_COMBO_BOX )) 					toolTipWindow =
                //toolTipWindow->winGetParent();
                if (toolTipWindow)
                {
                    // do we have a callback to call for the tooltip
                    if (toolTipWindow->m_tooltip)
                        toolTipWindow->m_tooltip(toolTipWindow, &toolTipWindow->m_instData, packedMouseCoords);
...
Int GameWindowManager::drawWindow(GameWindow *window)
{
    ...
    if (BitIsSet(window->m_status, WIN_STATUS_HIDDEN) == FALSE)
    {
        ...
        if (BitIsSet(window->winGetStyle(), GWS_SCROLL_LISTBOX))
            if (BitIsSet(window->m_status, WIN_STATUS_BORDER) == TRUE && !BitIsSet(window->m_status, WIN_STATUS_SEE_THRU))
...
void GameWindowManager::winRepaint(void)
{
    ...
    for (window = m_windowTail; window; window = next)
    {
        ...
        if (BitIsSet(window->m_status, WIN_STATUS_ABOVE | WIN_STATUS_BELOW) == FALSE)
            drawWindow(window);
    }
...
void GameWindowManager::dumpWindow(GameWindow *window)
{
    ...
    DEBUG_LOG(("ID: %d\tRedraw: 0x%08X\tUser Data: %d", window->winGetWindowId(), window->m_draw, window->m_userData));
...
//-------------------------------------------------------------------------------------------------
GameWindow *GameWindowManager::winCreate(
    GameWindow *parent,
    UnsignedInt status,
    Int x,
    Int y,
...
Int GameWindowManager::winDestroy(GameWindow *window)
{
    ...
    //
    DEBUG_ASSERTCRASH(window->winGetEditData() == NULL, ("winDestroy(): edit data should NOT be present!"));
...
Int GameWindowManager::winUnsetModal(GameWindow *window)
{
    ...
    {
        // return error if not
        DEBUG_LOG(("WinUnsetModal: Invalid window attempting to unset modal (%d)", window->winGetWindowId()));
        return WIN_ERR_GENERAL_FAILURE;
    ...
    Bool defaultVisual)
{
    ...
    button = TheWindowManager->winCreate(parent, status, x, y, width, height, GadgetPushButtonSystem, instData);
    if (button == NULL)
    {
...

{
    ...
    // create the button window
    checkbox = TheWindowManager->winCreate(parent, status, x, y, width, height, GadgetCheckBoxSystem, instData);
    if (checkbox == NULL)
    {
...

{
    ...
    // create the button window
    radioButton = TheWindowManager->winCreate(parent, status, x, y, width, height, GadgetRadioButtonSystem, instData);
    if (radioButton == NULL)
    {
...

{
    ...
    tabControl = TheWindowManager->winCreate(parent, status, x, y, width, height, GadgetTabControlSystem, instData);
    if (tabControl == NULL)
    {
...

{
    ...
    // create the listbox
    listbox = winCreate(parent, status, x, y, width, height, GadgetListBoxSystem, instData);
...

{
    ...
    else if (BitIsSet(instData->getStyle(), GWS_VERT_SLIDER))
    {
        ...
        if (BitIsSet(slider->winGetStatus(), WIN_STATUS_IMAGE)
            && !(parent && BitIsSet(parent->winGetStyle(), GWS_SCROLL_LISTBOX)))
    ...
    if (BitIsSet(instData->getStyle(), GWS_HORZ_SLIDER))
        button = gogoGadgetPushButton(
            ...
            HORIZONTAL_SLIDER_THUMB_WIDTH,
            HORIZONTAL_SLIDER_THUMB_HEIGHT,
            &buttonInstData,
            NULL,
            TRUE);
    else
    ...
    {
        sliderData->numTicks =
            (float)(width - HORIZONTAL_SLIDER_THUMB_WIDTH) / (float)(sliderData->maxVal - sliderData->minVal);
    }
    else
    {
    ...
    Bool defaultVisual)
{
    ...
    // create the listbox
    comboBox = winCreate(parent, status, x, y, width, height, GadgetComboBoxSystem, instData);
    ...
    comboBoxData->listBox = TheWindowManager->gogoGadgetListBox(
        comboBox,
        status | WIN_STATUS_ABOVE | WIN_STATUS_ONE_LINE,
        0,
        height,
        width,
        height,
...

{
    ...
    // create the button window
    progressBar = TheWindowManager->winCreate(parent, status, x, y, width, height, GadgetProgressBarSystem, instData);
    if (progressBar == NULL)
    {
...

{
    ...
    {
        textWin = winCreate(parent, status, x, y, width, height, GadgetStaticTextSystem, instData);
    }
    else
    {
...

{
    ...
    // create the window
    entry = winCreate(parent, status, x, y, width, height, GadgetTextEntrySystem, instData);
    if (entry == NULL)
    {
    ...
    if (OurLanguage == LANGUAGE_ID_KOREAN || OurLanguage == LANGUAGE_ID_JAPANESE)
    {
        ...
        data->constructList = gogoGadgetListBox(
            NULL,
            WIN_STATUS_ABOVE | WIN_STATUS_HIDDEN | WIN_STATUS_NO_FOCUS | WIN_STATUS_ONE_LINE,
            0,
            height,
            110,
            119,
    ...
    else if (BitIsSet(instData->getStyle(), GWS_COMBO_BOX))
    {
        ...
        if (listBox)
        {
            ...
            GadgetListBoxSetEnabledSelectedItemImageCenter(
                listBox,
                winFindImage("ListBoxEnabledSelectedItemRepeatingCenter"));
            GadgetListBoxSetEnabledSelectedItemImageSmallCenter(
                listBox,
                winFindImage("ListBoxEnabledSelectedItemSmallRepeatingCenter"));
            ...
            GadgetListBoxSetDisabledSelectedItemImageCenter(
                listBox,
                winFindImage("ListBoxDisabledSelectedItemRepeatingCenter"));
            GadgetListBoxSetDisabledSelectedItemImageSmallCenter(
                listBox,
                winFindImage("ListBoxDisabledSelectedItemSmallRepeatingCenter"));
...
Bool GameWindowManager::initTestGUI(void)
{
    ...
    window =
        TheWindowManager
            ->gogoGadgetPushButton(NULL, WIN_STATUS_ENABLED | WIN_STATUS_IMAGE, 200, 100, 100, 30, &instData, NULL, TRUE);

    // make a push button
    instData.init();
    BitSet(instData.m_style, GWS_PUSH_BUTTON | GWS_MOUSE_TRACK);
    instData.m_textLabelString = "Enabled";
    window = TheWindowManager->gogoGadgetPushButton(NULL, WIN_STATUS_ENABLED, 330, 100, 100, 30, &instData, NULL, TRUE);

    // make a push button
    instData.init();
    BitSet(instData.m_style, GWS_PUSH_BUTTON | GWS_MOUSE_TRACK);
    instData.m_textLabelString = "Disabled";
    window = TheWindowManager->gogoGadgetPushButton(NULL, 0, 450, 100, 100, 30, &instData, NULL, TRUE);

    // make a check box
    instData.init();
    instData.m_style = GWS_CHECK_BOX | GWS_MOUSE_TRACK;
    instData.m_textLabelString = "Check";
    window = TheWindowManager
                 ->gogoGadgetCheckbox(NULL, WIN_STATUS_ENABLED | WIN_STATUS_IMAGE, 200, 150, 100, 30, &instData, NULL, TRUE);

    // make a check box
    instData.init();
    instData.m_style = GWS_CHECK_BOX | GWS_MOUSE_TRACK;
    instData.m_textLabelString = "Check";
    window = TheWindowManager->gogoGadgetCheckbox(NULL, WIN_STATUS_ENABLED, 330, 150, 100, 30, &instData, NULL, TRUE);

    // make window to hold radio buttons
    window = TheWindowManager->winCreate(NULL, WIN_STATUS_ENABLED | WIN_STATUS_DRAGABLE, 200, 200, 250, 45, NULL);
    window->winSetInputFunc(testGrab);
    ...
    radio =
        TheWindowManager->gogoGadgetRadioButton(window, WIN_STATUS_ENABLED, 130, 10, 100, 30, &instData, &rData, NULL, TRUE);
    ...
    window =
        TheWindowManager->gogoGadgetListBox(NULL, WIN_STATUS_ENABLED, 200, 250, 100, 100, &instData, &listData, NULL, TRUE);
    GadgetListBoxAddEntryText(
        window,
        UnicodeString(L"Listbox text"),
        TheWindowManager->winMakeColor(255, 255, 255, 255),
        -1,
        0);
    GadgetListBoxAddEntryText(
        window,
        UnicodeString(L"More text"),
        TheWindowManager->winMakeColor(105, 105, 255, 255),
        -1,
        0);
    GadgetListBoxAddEntryText(window, UnicodeString(L"Nothing"), TheWindowManager->winMakeColor(105, 105, 255, 255), -1, 0);
    GadgetListBoxAddEntryText(window, UnicodeString(L"Seasons"), TheWindowManager->winMakeColor(105, 205, 255, 255), -1, 0);
    GadgetListBoxAddEntryText(window, UnicodeString(L"Misery"), TheWindowManager->winMakeColor(235, 105, 255, 255), -1, 0);
    GadgetListBoxAddEntryText(window, UnicodeString(L"Natural"), TheWindowManager->winMakeColor(105, 205, 45, 255), -1, 0);
    ...
    GadgetListBoxAddEntryText(
        ...
        -1,
        -1);
    GadgetListBoxAddEntryText(window, UnicodeString(L"Nothing"), TheWindowManager->winMakeColor(105, 105, 255, 255), -1, -1);
    GadgetListBoxAddEntryText(window, UnicodeString(L"Seasons"), TheWindowManager->winMakeColor(105, 205, 255, 255), -1, -1);
    GadgetListBoxAddEntryText(window, UnicodeString(L"Misery"), TheWindowManager->winMakeColor(235, 105, 255, 255), -1, -1);
    GadgetListBoxAddEntryText(window, UnicodeString(L"Natural"), TheWindowManager->winMakeColor(105, 205, 45, 255), -1, -1);
    ...
    window =
        TheWindowManager->gogoGadgetSlider(NULL, WIN_STATUS_ENABLED, 360, 250, 11, 100, &instData, &sliderData, NULL, TRUE);
    ...
    window =
        TheWindowManager->gogoGadgetSlider(NULL, WIN_STATUS_ENABLED, 200, 400, 200, 11, &instData, &sliderData, NULL, TRUE);
    ...
    window = TheWindowManager->gogoGadgetProgressBar(NULL, WIN_STATUS_ENABLED, 200, 450, 250, 15, &instData, NULL, TRUE);
    ...
    window =
        TheWindowManager
            ->gogoGadgetProgressBar(NULL, WIN_STATUS_ENABLED | WIN_STATUS_IMAGE, 200, 470, 250, 15, &instData, NULL, TRUE);
    ...
    window = TheWindowManager
                 ->gogoGadgetStaticText(NULL, WIN_STATUS_ENABLED, 200, 490, 300, 25, &instData, &textData, NULL, TRUE);
    ...
    window = TheWindowManager
                 ->gogoGadgetTextEntry(NULL, WIN_STATUS_ENABLED, 450, 270, 400, 30, &instData, &entryData, NULL, TRUE);
