            ...
            switch (mData1)
            {
                ...
                case KEY_DOWN:
                {
                    if (BitIsSet(mData2, KEY_STATE_DOWN))
                    {
                        ...
                        TheWindowManager
                            ->winSendSystemMsg(window->winGetOwner(), GLM_SELECTED, (WindowMsgData)window, list->selectPos);
                    } // end if
                ...
                case KEY_UP:
                {
                    if (BitIsSet(mData2, KEY_STATE_DOWN))
                    {
                        ...
                        else if (list->selectPos > 0)
                        {
                            ...
                            while (1)
                            {
                                if (list->listData[list->selectPos].listHeight - list->listData[list->selectPos].height
                                    < list->displayPos)
                                {
...
WindowMsgHandledType GadgetListBoxSystem(GameWindow *window, UnsignedInt msg, WindowMsgData mData1, WindowMsgData mData2)
{
    ...
    switch (msg)
    {
        ...
        case GLM_DEL_ENTRY:
        {
            ...
            memcpy(
                &list->listData[mData1],
                &list->listData[(mData1 + 1)],
