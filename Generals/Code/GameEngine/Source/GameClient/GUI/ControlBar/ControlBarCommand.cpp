                    ...
                    if (power && power->getRequiredScience() != SCIENCE_INVALID)
                    {
                        ...
                        else
                        {
                            ...
                            if (bestIndex != -1)
                            {
                                ...
                                // Now we have to search through the command buttons to find a matching purchase science
                                // button.
                                for (const CommandButton *command = m_commandButtons; command; command = command->getNext())
                                {
                                    if (command && command->getCommandType() == GUI_COMMAND_PURCHASE_SCIENCE)
                                    {
                                        ...
                                        {
                                            DEBUG_CRASH(
                                                ("Commandbutton %s is a purchase science button without any science! Please "
                                                 "add it.",
                                                 command->getName().str()));
                                        }
    ...
    Bool forceDisabledEvaluation) const
{
    ...
    if (disabled && !forceDisabledEvaluation)
    {
        ...
        if (commandType != GUI_COMMAND_SELL && commandType != GUI_COMMAND_EVACUATE
            ...
            && commandType != GUI_COMMAND_SET_RALLY_POINT && commandType != GUI_COMMAND_SWITCH_WEAPON)
        {
