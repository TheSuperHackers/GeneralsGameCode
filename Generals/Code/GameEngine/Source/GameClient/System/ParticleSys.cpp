...
void Particle::doWindMotion(void)
{
	...
	if (distFromWind < noForceDistance)
	{
		...
		if (distFromWind > fullForceDistance)
			windForceStrength *= (1.0f - ((distFromWind - fullForceDistance) / (noForceDistance - fullForceDistance)));
...
void Particle::xfer(Xfer *xfer)
{
	...
	ParticleSystemID systemUnderControlID =
			m_systemUnderControl ? m_systemUnderControl->getSystemID() : INVALID_PARTICLE_SYSTEM_ID;
...
// ------------------------------------------------------------------------------------------------
ParticleSystem::ParticleSystem(const ParticleSystemTemplate *sysTemplate, ParticleSystemID id, Bool createSlaves)
{
...
// ------------------------------------------------------------------------------------------------
Particle *ParticleSystem::createParticle(const ParticleInfo *info, ParticlePriorityType priority, Bool forceCreate)
{
	//
	...
	if (forceCreate == FALSE)
	{
		...
		if (priority < TheGameLODManager->getMinDynamicParticlePriority()
				|| (priority < TheGameLODManager->getMinDynamicParticleSkipPriority() && TheGameLODManager->isParticleSkipped()))
			return NULL;

		if (getParticleCount() > 0 && priority == AREA_EFFECT && m_isGroundAligned
		...
		Bool slaveNeedsFullPromotion)
{
	...
	if (slaveNeedsFullPromotion)
	{
		...
		slaveParticleSystem->m_startSize.setRange(
				masterParticleSystem->m_startSize.getMinimumValue() * slaveParticleSystem->m_startSize.getMinimumValue(),
		...
		slaveParticleSystem->m_sizeRate.setRange(
				masterParticleSystem->m_sizeRate.getMinimumValue() * slaveParticleSystem->m_sizeRate.getMinimumValue(),
		...
		slaveParticleSystem->m_sizeRateDamping.setRange(
				masterParticleSystem->m_sizeRateDamping.getMinimumValue() * slaveParticleSystem->m_sizeRateDamping.getMinimumValue(),
...
const FieldParse ParticleSystemTemplate::m_fieldParseTable[] = {
	...
	//	{ "COLOR",									ParticleSystemTemplate::parseRandomRGBColor,	NULL,		offsetof( ParticleSystemTemplate,
	// m_color ) },
	...
	{ "VelSpherical",
		INI::parseGameClientRandomVariable,
		NULL,
...
void ParticleSystemManager::reset(void)
{
	...
	DEBUG_ASSERTCRASH(
			m_particleSystemCount == 0,
			("ParticleSystemManager::reset: m_particleSystemCount is %u, not 0", m_particleSystemCount));
...
{
	DEBUG_ASSERTCRASH(
			particleSystemToAdd != NULL,
			("ParticleSystemManager::friend_addParticleSystem: ParticleSystem is null"));
...
void ParticleSystemManager::friend_removeParticleSystem(ParticleSystem *particleSystemToRemove)
{
	ParticleSystemListIt it =
			std::find(m_allParticleSystemList.begin(), m_allParticleSystemList.end(), particleSystemToRemove);
...
// ------------------------------------------------------------------------------------------------
Int ParticleSystemManager::removeOldestParticles(UnsignedInt count, ParticlePriorityType priorityCap)
{
...
void ParticleSystemManager::preloadAssets(TimeOfDay timeOfDay)
{
	...
	for (; begin != end; ++begin)
	{
		...
		if (tmplate->m_particleType == ParticleSystemInfo::PARTICLE && (!tmplate->m_particleTypeName.isEmpty()))
		{
...
void ParticleSystemManager::xfer(Xfer *xfer)
{
	...
	else
	{
		...
		for (UnsignedInt i = 0; i < systemCount; ++i)
		{
			...
			{
				DEBUG_CRASH(("ParticleSystemManager::xfer - Unknown particle system template '%s'", systemName.str()));
				throw SC_INVALID_DATA;
			...
			if (system == NULL)
			{
				DEBUG_CRASH(("ParticleSystemManager::xfer - Unable to allocate particle system '%s'", systemName.str()));
				throw SC_INVALID_DATA;
