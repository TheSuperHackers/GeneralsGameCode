...
/*static*/ void Anim2DTemplate::parseImageSequence(INI *ini, void *instance, void *store, const void *userData)
{
	...
	{
		DEBUG_CRASH(
				("Anim2DTemplate::parseImageSequence - You must specify the number of animation frames for animation '%s' *BEFORE* "
...
void Anim2DTemplate::storeImage(const Image *image)
{
	...
	DEBUG_CRASH(
			("Anim2DTemplate::storeImage - Unable to store image '%s' into animation '%s' because the animation is setup to only "
			 "support '%d' image frames",
			 image->getName().str(),
			 getName().str(),
			 m_numFrames));
	throw INI_INVALID_DATA;
