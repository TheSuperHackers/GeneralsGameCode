...
const Image *Anim2DTemplate::getFrame(UnsignedShort frameNumber) const
{
    ...
    DEBUG_ASSERTCRASH(
        m_images != NULL,
        ("Anim2DTemplate::getFrame - Image data is NULL for animation '%s'", getName().str()));

    // sanity
    if (frameNumber < 0 || frameNumber >= m_numFrames)
    {
        DEBUG_CRASH(
            ("Anim2DTemplate::getFrame - Illegal frame number '%d' for animation '%s'", frameNumber, getName().str()));
        return NULL;
...
void Anim2D::reset(void)
{
    ...
    switch (m_template->getAnimMode())
    {
        ...
        default:
            DEBUG_CRASH(
                ("Anim2D::reset - Unknown animation mode '%d' for '%s'",
