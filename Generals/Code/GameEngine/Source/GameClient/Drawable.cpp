...
void Drawable::updateDrawable(void)
{
    ...
    if (m_ambientSound && m_ambientSoundEnabled && !m_ambientSound->m_event.getEventName().isEmpty()
...
void Drawable::draw(View *view)
{
    ...
    else if (m_heatVisionOpacity > VERY_TRANSPARENT_HEATVISION) // keep fading any heatvision unless something has set it to
                                                                // zero
...
void Drawable::drawDisabled(const IRegion2D *healthBarRegion)
{
    ...
    //                   7/
    if (obj->isDisabledByType(DISABLED_HACKED) || obj->isDisabledByType(DISABLED_PARALYZED)
        || obj->isDisabledByType(DISABLED_EMP) || obj->isDisabledByType(DISABLED_UNDERPOWERED))
    {
        // create icon if necessary
