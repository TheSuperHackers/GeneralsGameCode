  ...
  switch (t)
  {
          ...
          for (temp; temp != selectedDrawable; temp = temp->getNextDrawable())
          {
            ...
            if (temp == NULL)
            {
              ...
              if (tempObject && temp->getNextDrawable() == selectedDrawable && !temp->isSelected() && tempObject->isMobile()
                  && tempObject->isLocallyControlled() && !tempObject->isContained() && !tempObject->isKindOf(KINDOF_DRONE))
              {
                newDrawable = temp;
                break;
              }
    ...
    */
    ...
    case GameMessage::MSG_MOUSEOVER_DRAWABLE_HINT:
    {
      ...
      if (TheInGameUI->getSelectCount() > 0
          || (command
              && command->getCommandType() == GUI_COMMAND_SPECIAL_POWER_FROM_COMMAND_CENTER)) // If something is selected
      {
