...
//
// The multiplier of 2 was logically chosen because originally the Scroll Factor did practically not affect the RMB scroll
// speed and because the default Scroll Factor is/was 0.5, it needs to be doubled to get to a neutral 1x multiplier.
  ...
  switch (t)
  {
      ...
      if (TheMouse->isCursorCaptured())
      {
        if (m_isScrolling)
        {
          if (m_scrollType == SCROLL_SCREENEDGE
              && (m_currentPos.x >= edgeScrollSize && m_currentPos.y >= edgeScrollSize
                  && m_currentPos.y < height - edgeScrollSize && m_currentPos.x < width - edgeScrollSize))
          {
            stopScrolling();
          }
        }
        ...
        {
          if (m_currentPos.x < edgeScrollSize || m_currentPos.y < edgeScrollSize || m_currentPos.y >= height - edgeScrollSize
    ...
    case GameMessage::MSG_FRAME_TICK:
    {
      ...
      if (m_isScrolling && !TheInGameUI->isScrolling())
      {
        ...
        if (m_isScrolling)
        {
          // TheSuperHackers @bugfix Mauller 07/06/2025 Adjust the viewport scrolling so it is independent of
          // GameClient FPS The scaling is based on the current logic rate, this provides a consistent scroll speed
          // at all GameClient FPS This also fixes scrolling within replays when fast forwarding due to the
          // uncapped FPS When the FPS is in excess of the expected frame rate, the ratio will reduce the offset of
          ...
          switch (m_scrollType)
          {
            case SCROLL_RMB:
            {
              ...
              // TheSuperHackers @fix Mauller 16/06/2025 fix RMB scrolling to allow it to scale with the user
              // adjusted scroll factor
              Coord2D vec;
              vec.x = (m_currentPos.x - m_anchor.x);
              vec.y = (m_currentPos.y - m_anchor.y);
              // TheSuperHackers @info calculate the length of the vector to obtain the movement speed before
              // the vector is normalized
              float vecLength = vec.length();
              vec.normalize();
              offset.x = TheGlobalData->m_horizontalScrollSpeedFactor * logicToFpsRatio * vecLength * vec.x
                  * SCROLL_MULTIPLIER * TheGlobalData->m_keyboardScrollFactor;
              offset.y = TheGlobalData->m_verticalScrollSpeedFactor * logicToFpsRatio * vecLength * vec.y * SCROLL_MULTIPLIER
            ...
            case SCROLL_KEY:
            {
              if (scrollDir[DIR_UP])
              {
                offset.y -= TheGlobalData->m_verticalScrollSpeedFactor * logicToFpsRatio * SCROLL_AMT
                    * TheGlobalData->m_keyboardScrollFactor;
              }
              ...
              {
                offset.x -= TheGlobalData->m_horizontalScrollSpeedFactor * logicToFpsRatio * SCROLL_AMT
                    * TheGlobalData->m_keyboardScrollFactor;
              }
      ...
      // if (TheGlobalData->m_saveCameraInReplay /*&& TheRecorder->getMode() != RECORDERMODETYPE_PLAYBACK *//**/&&
      // (TheGameLogic->isInSinglePlayerGame() || TheGameLogic->isInSkirmishGame())/**/) if
      // (TheGlobalData->m_saveCameraInReplay && (TheGameLogic->isInMultiplayerGame() ||
      // TheGameLogic->isInSinglePlayerGame() || TheGameLogic->isInSkirmishGame()))
