...
void Mouse::updateMouseData()
{
    ...
    if (busy == FALSE)
    {
        ...
        do
        {
            ...
            index++;
        } while ((result != MOUSE_NONE) && (index < sizeof(m_mouseEvents) / sizeof(MouseIO)));
...
void Mouse::processMouseEvent(Int index)
{
    ...
    moveMouse(m_mouseEvents[index].pos.x, m_mouseEvents[index].pos.y, movementType);
    ...
    if (m_mouseEvents[index].leftFrame)
    {
        ...
        m_currMouse.leftState != MBS_Up
        && ((m_prevMouse.leftEvent == GWM_LEFT_DOWN) || (m_prevMouse.leftEvent == GWM_LEFT_DRAG)))
    {
    ...
    if (m_mouseEvents[index].rightFrame)
    {
        ...
        m_currMouse.rightState != MBS_Up
        && ((m_prevMouse.rightEvent == GWM_RIGHT_DOWN) || (m_prevMouse.rightEvent == GWM_RIGHT_DRAG)))
    {
    ...
    if (m_mouseEvents[index].middleFrame)
    {
        ...
        m_currMouse.middleState != MBS_Up
        && ((m_prevMouse.middleEvent == GWM_MIDDLE_DOWN) || (m_prevMouse.middleEvent == GWM_MIDDLE_DRAG)))
    {
    ...
    //	DEBUG_LOG(("Mouse dx %d, dy %d, index %d, frame %d", m_currMouse.deltaPos.x, m_currMouse.deltaPos.y, index,
    //m_inputFrame));
...
{
    if (m_currMouse.leftState && ((m_prevMouse.leftEvent == GWM_LEFT_DOWN) || (m_prevMouse.leftEvent == GWM_LEFT_DRAG)))
    {
        m_currMouse.leftEvent = GWM_LEFT_DRAG;
    }

    if (m_currMouse.rightState && ((m_prevMouse.rightEvent == GWM_RIGHT_DOWN) || (m_prevMouse.rightEvent == GWM_RIGHT_DRAG)))
    {
        m_currMouse.rightEvent = GWM_RIGHT_DRAG;
    }
...
// ------------------------------------------------------------------------------------------------
void Mouse::setMouseText(UnicodeString text, const RGBAColorInt *color, const RGBAColorInt *dropColor)
{
...
{
    CONSTEXPR const CursorCaptureBlockReasonInt noGameBits =
        CursorCaptureBlockReason_NoGame | CursorCaptureBlockReason_Paused;
...
void Mouse::unblockCapture(CursorCaptureBlockReason reason)
{
    ...
    if (canCaptureNow != canCaptureBefore)
    {
        DEBUG_ASSERTCRASH(
            canCaptureNow,
            ("Mouse::unblockCapture(%s): Unexpected logic", CursorCaptureBlockReasonNames[reason]));
        capture();
    }

    DEBUG_LOG(
        ("Mouse::unblockCapture(%s): m_captureBlockReason=%u canCapture=%d",
         CursorCaptureBlockReasonNames[reason],
...
void Mouse::blockCapture(CursorCaptureBlockReason reason)
{
    ...
    if (canCaptureNow != canCaptureBefore)
    {
        DEBUG_ASSERTCRASH(
            !canCaptureNow,
            ("Mouse::blockCapture(%s): Unexpected logic", CursorCaptureBlockReasonNames[reason]));
        releaseCapture();
    }

    DEBUG_LOG(
        ("Mouse::blockCapture(%s): m_captureBlockReason=%u canCapture=%d",
         CursorCaptureBlockReasonNames[reason],
         m_captureBlockReasonBits,
         (Int)canCapture()));
}
...
void Mouse::drawTooltip(void)
{
    ...
    if (m_displayTooltip && TheDisplay && m_tooltipDisplayString && (m_tooltipDisplayString->getTextLength() > 0)
        && !m_isTooltipEmpty)
    {
        Int width, xPos;
        Int height, yPos;
...
#define COLOR(x) GMC(m_tooltipColor##x)
        ...
        m_tooltipDisplayString->draw(
            xPos + 2,
            yPos + 1,
            GMC(m_tooltipTextColor),
            COLOR(Shadow)); // GameMakeColor(220,220,220,255),GameMakeColor(20,20,20,125));
        ...
        m_tooltipDisplayString->draw(
            ...
            yPos + 1,
            COLOR(Highlight),
            COLOR(Shadow)); // GameMakeColor(255,255,0,255),GameMakeColor(20,20,20,125));
...
void Mouse::drawCursorText(void)
{
    ...
    color = GameMakeColor(m_cursorTextColor.red, m_cursorTextColor.green, m_cursorTextColor.blue, m_cursorTextColor.alpha);
    dropColor = GameMakeColor(
        m_cursorTextDropColor.red,
...
void Mouse::setCursor(MouseCursor cursor)
{
    ...
    if (m_cursorTextDisplayString != NULL)
    {
        ...
        if (cursorInfo->cursorText.isEmpty() == FALSE)
            setMouseText(
                TheGameText->fetch(cursorInfo->cursorText.str()),
