...
/*static*/ void ScienceStore::friend_parseScienceDefinition(INI *ini)
{
    ...
    if (TheScienceStore)
    {
        ...
        for (ScienceInfoVec::iterator it = TheScienceStore->m_sciences.begin(); it != TheScienceStore->m_sciences.end();
             ++it)
        {
...
void ScienceStore::getPurchasableSciences(const Player *player, ScienceVec &purchasable, ScienceVec &potentiallyPurchasable)
    const
{
    purchasable.clear();
