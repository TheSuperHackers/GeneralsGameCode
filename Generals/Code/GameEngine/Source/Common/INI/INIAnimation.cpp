...
void INI::parseAnim2DDefinition(INI *ini)
{
	...
	if (animTemplate == NULL)
	{
		...
		DEBUG_ASSERTCRASH(
				animTemplate,
				("INI"
				 "parseAnim2DDefinition -  unable to allocate animation template for '%s'",
				 name.str()));
	...
	else
	{
		...
		DEBUG_CRASH(("INI::parseAnim2DDefinition - Animation template '%s' already exists", animTemplate->getName().str()));
		return;
