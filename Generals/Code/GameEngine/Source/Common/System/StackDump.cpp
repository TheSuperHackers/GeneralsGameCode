...
BOOL InitSymbolInfo()
{
	...
	gsSymGetLineFromAddr = (BOOL(
			__stdcall *)(IN HANDLE, IN DWORD, OUT PDWORD, OUT PIMAGEHLP_LINE))GetProcAddress(hInstDebugHlp, "SymGetLineFromAddr");
...
void DumpExceptionInfo(unsigned int u, EXCEPTION_POINTERS *e_info)
{
	...
	*/
	static char const *_code_txt[] = {
		...
		"Error code: EXCEPTION_ARRAY_BOUNDS_EXCEEDED\nDescription: The thread tried to access an array element that is out of "
		...
		"Error code: EXCEPTION_DATATYPE_MISALIGNMENT\nDescription: The thread tried to read or write data that is misaligned on "
		"hardware that does not provide alignment. For example, 16-bit values must be aligned on 2-byte boundaries; 32-bit "
		...
		"Error code: EXCEPTION_FLT_INEXACT_RESULT\nDescription: The result of a floating-point operation cannot be represented "
		...
		"Error code: EXCEPTION_INVALID_DISPOSITION\nDescription: An exception handler returned an invalid disposition to the "
		...
		"Error code: EXCEPTION_PRIV_INSTRUCTION\nDescription: The thread tried to execute an instruction whose operation is not "
		"allowed in the current machine mode.",
		"Error code: EXCEPTION_SINGLE_STEP\nDescription: A trace trap or other single-instruction mechanism signaled that one "
