...
#define PROFILE_ERROR_LIMIT \
  0.94f // fraction of profiled result needed to get a match.  Allows some room for error/fluctuation.

// Hack to get access to a static method on the W3DDevice side. -MW
extern Bool testMinimumRequirements(
...
static const char *DynamicGameLODNames[] = { "Low", "Medium", "High", "VeryHigh" };
...
// Keep this in sync with enum in GameLOD.h
static const char *VideoNames[] = { "XX",   "V2",  "V3",  "V4",   "V5",   "TNT",  "TNT2", "GF2", "R100",
                                    "PS11", "GF3", "GF4", "PS14", "R200", "PS20", "R300", NULL };
  ...
  if (m_idealDetailLevel == STATIC_GAME_LOD_UNKNOWN || TheGlobalData->m_forceBenchmark)
  {
    if (m_cpuType == XX || TheGlobalData->m_forceBenchmark)
    {
      ...
      m_compositeBenchIndex =
          m_intBenchIndex + m_floatBenchIndex; ///@todo: Need to scale these based on our apps usage of int/float/mem ops.
      ...
      for (Int k = 0; k < m_numBenchProfiles; k++)
      {
            ...
            && m_memBenchIndex / prof->m_memBenchIndex >= PROFILE_ERROR_LIMIT)
        {
          for (Int i = STATIC_GAME_LOD_HIGH; i >= STATIC_GAME_LOD_LOW; i--)
          {
            ...
            if (currentLevel >= i)
              break; // we already found a higher level than the remaining presets so no need to keep
                     // searching.
          }
        }
        prof++;
    ...
    for (Int i = STATIC_GAME_LOD_HIGH; i >= STATIC_GAME_LOD_LOW; i--)
    {
      ...
      for (Int j = 0; j < m_numLevelPresets[i]; j++)
      {
        if (m_cpuType == preset->m_cpuType && ((Real)m_cpuFreq / (Real)preset->m_mhz >= PROFILE_ERROR_LIMIT)
            && // make sure we're within 5% or higher
            m_videoChipType >= preset->m_videoType && ((Real)numMBRam / (Real)preset->m_memory >= PROFILE_ERROR_LIMIT))
        {
...
void GameLODManager::applyStaticLODLevel(StaticGameLODLevel level)
{
  ...
  else if (level >= STATIC_GAME_LOD_LOW)
  ...
  if (TheGlobalData)
  {
    ...
    if (m_currentStaticLOD == STATIC_GAME_LOD_UNKNOWN || lodInfo->m_useShadowVolumes != prevLodInfo->m_useShadowVolumes
