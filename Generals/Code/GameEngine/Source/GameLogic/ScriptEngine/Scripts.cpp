    ...
    if (!pParm->m_string.isEmpty())
    {
      ...
      for (int i = 0; kindofNames[i]; ++i)
      {
        ...
        if (!pParm->m_string.compareNoCase("CRUSHER"))
        {
          ...
          found = true;
          DEBUG_CRASH(
              ("Kindof CRUSHER no longer exists -- in order to get your map to load, it has been switched to "
               "OBSTACLE, please call Kris (x36844).",
               pParm->m_string.str()));
          break;
        }
        else if (!pParm->m_string.compareNoCase("CRUSHABLE"))
        {
          ...
          found = true;
          DEBUG_CRASH(
              ("Kindof CRUSHABLE no longer exists -- in order to get your map to load, it has been switched to "
               "OBSTACLE, please call Kris (x36844).",
               pParm->m_string.str()));
          break;
        }
        else if (!pParm->m_string.compareNoCase("OVERLAPPABLE"))
        {
          ...
          found = true;
          DEBUG_CRASH(
              ("Kindof OVERLAPPABLE no longer exists -- in order to get your map to load, it has been switched to "
               "OBSTACLE, please call Kris (x36844).",
               pParm->m_string.str()));
          break;
        }
