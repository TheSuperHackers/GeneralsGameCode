        ...
        for (; curVictim != NULL; curVictim = iter ? iter->nextWithNumeric(&curVictimDistSqr) : NULL)
        {
            ...
            if (source != NULL)
            {
                ...
                if (curVictim != primaryVictim)
                {
                    ...
                    {
                        // This means we probably are affecting allies, but don't want to kill nearby members that are the
                        // same type as us. A good example are a group of terrorists blowing themselves up. We don't want to
