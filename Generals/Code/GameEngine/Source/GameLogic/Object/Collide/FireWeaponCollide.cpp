...
{
	m_collideWeapon =
			TheWeaponStore->allocateNewWeapon(getFireWeaponCollideModuleData()->m_collideWeaponTemplate, PRIMARY_WEAPON);
	m_everFired = FALSE;
}
...
void FireWeaponCollide::xfer(Xfer *xfer)
{
	...
	{
		DEBUG_ASSERTCRASH(m_collideWeapon != NULL, ("FireWeaponCollide::xfer - m_collideWeapon present mismatch"));
		xfer->xferSnapshot(m_collideWeapon);

	} // end else
	else
	{
		DEBUG_ASSERTCRASH(m_collideWeapon == NULL, ("FireWeaponCollide::Xfer - m_collideWeapon missing mismatch"));
