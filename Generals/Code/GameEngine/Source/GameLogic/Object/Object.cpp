...
Bool Object::isSelectable() const
{
    return getTemplate()->isKindOf(KINDOF_ALWAYS_SELECTABLE)
        || (m_isSelectable && !testStatus(OBJECT_STATUS_UNSELECTABLE) && !isEffectivelyDead()
            && !getTemplate()->isKindOf(
                KINDOF_DRONE) // Most drones are unselectable from being slaved, but the SpyDrone needs help
        );
}
    ...
    else
    {
        ...
        for (UnsignedShort i = 0; i < moduleCount; ++i)
        {
            ...
            if (module == NULL)
            {
                ...
                DEBUG_CRASH(
                    ("Object::xfer - Module '%s' was indicated in file, but not found on object '%s'(%d)",
...
void Object::look()
{
    ...
    if (controller)
    {
            ...
            && (!isEffectivelyDead()) && (getShroudClearingRange() > 0.0f))
        {
            ...
            Coord3D pos = *getPosition();
            ThePartitionManager->doShroudReveal(pos.x, pos.y, getShroudClearingRange(), lookingMask);
        ...
        if (button
            && ((button->getCommandType() == GUI_COMMAND_PLAYER_UPGRADE)
                || (button->getCommandType()
                    == GUI_COMMAND_OBJECT_UPGRADE)) // Or else a button that requires an upgrade will appear the same as a
