...
void Object::setDisabledUntil(DisabledType type, UnsignedInt frame)
{
  ...
  else if (type == DISABLED_UNDERPOWERED || type == DISABLED_EMP || type == DISABLED_HACKED)
  {
    ...
    if (!isDisabledByType(DISABLED_UNDERPOWERED) && !isDisabledByType(DISABLED_EMP) && !isDisabledByType(DISABLED_HACKED))
    {
  ...
  for (Int buttonIndex = 0; buttonIndex < MAX_COMMANDS_PER_SET; buttonIndex++)
  {
    ...
    if (button
        && ((button->getCommandType() == GUI_COMMAND_PLAYER_UPGRADE)
            || (button->getCommandType() == GUI_COMMAND_OBJECT_UPGRADE)) // Or else a button that requires an upgrade will
                                                                         // appear the same as a button that gives an upgrade
