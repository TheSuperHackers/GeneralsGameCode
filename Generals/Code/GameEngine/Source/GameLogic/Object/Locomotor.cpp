...
void LocomotorTemplate::validate()
{
  ...
  // gravity!")); DEBUG_ASSERTCRASH(m_liftDamaged == 0.0f || m_liftDamaged > fabs(TheGlobalData->m_gravity), ("LiftDamaged
  // is too low to counteract gravity!")); DEBUG_ASSERTCRASH(m_preferredHeight == 0.0f || (m_behaviorZ ==
...
const FieldParse *LocomotorTemplate::getFieldParse() const
{
  static const FieldParse TheFieldParse[] = {
    ...
    { "MaxThrustAngle",
      INI::parseAngleReal,
      NULL,
      offsetof(LocomotorTemplate, m_maxThrustAngle) }, // yes, angle, not angular-vel
...
void Locomotor::maintainCurrentPositionWings(Object *obj, PhysicsBehavior *physics)
{
  ...
  physics->setTurning(TURN_NONE);
  if (physics->isMotive()
      && obj->isAboveTerrain()) // no need to stop something that isn't moving (or is just sitting on the ground)
  {
    // aim for the spot on the opposite side of the circle.
    BodyDamageType bdt = obj->getBodyModule()->getDamageState();
