...
static void parseOCL(INI *ini, void *instance, void * /*store*/, const void * /*userData*/)
{
    ...
    for (const char *token = ini->getNextToken(); token != NULL; token = ini->getNextTokenOrNull())
    {
        const ObjectCreationList *ocl =
            TheObjectCreationListStore->findObjectCreationList(token); // could be null! this is OK!
...
/*static*/ void SlowDeathBehaviorModuleData::buildFieldParse(MultiIniFieldParse &p)
{
    ...
    static const FieldParse dataFieldParse[] = {
        ...
        {"ModifierBonusPerOverkillPercent",
         ...
         NULL,
         offsetof(SlowDeathBehaviorModuleData, m_modifierBonusPerOverkillPercent)},
    ...
    if (!isSlowDeathActivated())
    {
        ...
        else
        {
            ...
            else
            {
                ...
                m_destructionFrame =
                    timeScale * (d->m_destructionDelay + GameLogicRandomValue(0, d->m_destructionDelayVariance));
                m_midpointFrame =
                    GameLogicRandomValue(BEGIN_MIDPOINT_RATIO * m_destructionFrame, END_MIDPOINT_RATIO * m_destructionFrame);
        ...
        if (d->m_flingForce > 0)
        {
            ...
            if (physics)
            {
                ...
                calcRandomForce(
                    d->m_flingForce,
                    d->m_flingForce + d->m_flingForceVariance,
                    d->m_flingPitch,
                    d->m_flingPitch + d->m_flingPitchVariance,
                    force);
                ...
                if (tree)
                {
                    if (tree->isKindOf(KINDOF_SHRUBBERY))
                    {
                        ...
                        obj->setModelConditionFlags(
                            MAKE_MODELCONDITION_MASK(MODELCONDITION_PARACHUTING)); // looks like he is snagged in a tree
