...
static void parseFXLocInfo(INI *ini, void *instance, FXLocInfo *locInfo)
{
	...
	if (stricmp(token, "bone") == 0)
	{
		...
		{
			DEBUG_CRASH(
					("parseFXLocInfo: Bone name not followed by RandomBone specifier\nPress IGNORE to see which INI file and line # "
					 "is incorrect."));
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
TransitionDamageFX::TransitionDamageFX(Thing *thing, const ModuleData *moduleData) : DamageModule(thing, moduleData)
{
	Int i, j;
...
//-------------------------------------------------------------------------------------------------
void TransitionDamageFX::onBodyDamageStateChange(
		...
		BodyDamageType newState)
{
	...
	if (IS_CONDITION_WORSE(newState, oldState))
	{
		...
		for (i = 0; i < DAMAGE_MODULE_MAX_FX; i++)
		{
			...
			{
				if (lastDamageInfo == NULL || getDamageTypeFlag(modData->m_damageFXTypes, lastDamageInfo->in.m_damageType))
				{
			...
			{
				if (lastDamageInfo == NULL || getDamageTypeFlag(modData->m_damageOCLTypes, lastDamageInfo->in.m_damageType))
				{
					pos = getLocalEffectPos(&modData->m_OCL[newState][i].locInfo, draw);
					getObject()->convertBonePosToWorldPos(&pos, NULL, &pos, NULL);
					ObjectCreationList::create(modData->m_OCL[newState][i].ocl, getObject(), &pos, damageSource->getPosition());
			...
			if (pSystemT)
			{
				if (lastDamageInfo == NULL || getDamageTypeFlag(modData->m_damageParticleTypes, lastDamageInfo->in.m_damageType))
				{
