...
const Real CLOSE_ENOUGH = 15; // Our moveTo commands and pathfinding can't handle people in the way, so quit trying to hump
															// someone on your spot
	...
	if (data->m_guardMaxRange)
	{
		...
		if (myAI->isIdle() && ThePartitionManager->getDistanceSquared(me, &pinnedPosition, FROM_CENTER_3D) > CLOSE_ENOUGH_SQR)
		{
			...
			doGuardLogic(&pinnedPosition);
				...
				ThePartitionManager->getDistanceSquared(me, master, FROM_CENTER_3D) > sqr(STRAY_MULTIPLIER * data->m_guardMaxRange))
		{
...
void SlavedUpdate::stopSlavedEffects()
{
	...
	/// @todo Just a thought.  Our Status bits on objects really need to be reference counts so you don't clear someone else's
	/// flag
