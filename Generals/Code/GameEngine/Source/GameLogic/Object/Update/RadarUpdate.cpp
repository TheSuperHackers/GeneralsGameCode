...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
RadarUpdate::RadarUpdate(Thing *thing, const ModuleData *moduleData) : UpdateModule(thing, moduleData)
{
	m_radarActive = FALSE;
	m_extendDoneFrame = 0;
...
UpdateSleepTime RadarUpdate::update(void)
{
	...
	if (TheGameLogic->getFrame() > m_extendDoneFrame)
	{
		...
		Drawable *draw = getObject()->getDrawable();
		if (draw)
			draw->clearAndSetModelConditionState(MODELCONDITION_RADAR_EXTENDING, MODELCONDITION_RADAR_UPGRADED);
