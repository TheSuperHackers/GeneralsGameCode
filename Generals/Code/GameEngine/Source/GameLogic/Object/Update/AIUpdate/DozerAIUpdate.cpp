...
StateReturnType DozerActionPickActionPosState::update(void)
{
    ...
    else
    {
        ...
        if (ThePartitionManager->findPositionAround(goalObject->getPosition(), &fpOptions, &goalPos) == FALSE)
        {
...
StateReturnType DozerActionDoActionState::update(void)
{
    ...
    switch (m_task)
    {
        ...
        {
            // GS Moved this inside Build, since you are allowed to Repair things that are not your player (canRepairObject
            // handles it)
            if (dozer->getControllingPlayer()
                != goalObject->getControllingPlayer()) // Yipes, SOmehow I have changed sides in mid build!
                return STATE_FAILURE;
            ...
            else
            {
                ...
                if (goalObject->isKindOf(KINDOF_BRIDGE_TOWER))
                {
                    BridgeTowerBehaviorInterface *btbi =
                        BridgeTowerBehavior::getBridgeTowerBehaviorInterfaceFromObject(goalObject);
                ...
                if (canHeal)
                {
                    ...
                    Real health = body->getMaxHealth() * dozerAI->getRepairHealthPerSecond() / LOGICFRAMES_PER_SECOND;
                    ...
                    if (!goalObject->attemptHealingFromSoleBenefactor(health, dozer, 2)) // this frame and the next
                    {
                        ...
                        // so that several dozers/workers can target the same sick building for repair, but the first one to
                        // begin this is done by attemptHealingFromSoleBenefactor() which lets the beneficiary of the healing
                        // remember who has been healing it, and will return false to everybody else
                        // or if the goalObject is already receiving healing, I must stop, since my healing is getting
                        // rejected here
                        dozerAI->internalTaskComplete(m_task);
                        getMachine()->setGoalObject(NULL);
                        return STATE_FAILURE;
...
static Object *findObjectToRepair(Object *dozer)
{
    ...
    ObjectIterator *iter =
        ThePartitionManager->iterateObjectsInRange(dozer->getPosition(), dozerAI->getBoredRange(), FROM_CENTER_2D, filters);
...
StateReturnType DozerPrimaryIdleState::update(void)
{
    ...
    //
    if (TheGameLogic->getFrame() - m_idleTooLongTimestamp > dozerAI->getBoredTime() && dozerAI->isAnyTaskPending() == FALSE)
    {
        //
        ...
        else
        {
            getMachineOwner()->setWeaponSetFlag(
                WEAPONSET_MINE_CLEARING_DETAIL); // maybe go clear some mines, if I feel like it
...
{
    if (m_actionMachine)
        deleteInstance(m_actionMachine);
}
...
DozerPrimaryStateMachine::DozerPrimaryStateMachine(Object *owner) : StateMachine(owner, "DozerPrimaryStateMachine")
{
    ...
    defineState(
        DOZER_PRIMARY_IDLE,
        newInstance(DozerPrimaryIdleState)(this),
        INVALID_STATE_ID,
        INVALID_STATE_ID,
        idleConditions);
    defineState(
    ...
    defineState(
        ...
        newInstance(DozerPrimaryGoingHomeState)(this),
        DOZER_PRIMARY_IDLE,
        DOZER_PRIMARY_IDLE);
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
DozerAIUpdate::DozerAIUpdate(Thing *thing, const ModuleData *moduleData) : AIUpdateInterface(thing, moduleData)
...
//-------------------------------------------------------------------------------------------------
Object *DozerAIUpdate::construct(
    ...
    Bool isRebuild)
{
    ...
    // sanity
    DEBUG_ASSERTCRASH(
        getObject()->getControllingPlayer() == owningPlayer,
...
Bool DozerAIUpdate::canAcceptNewRepair(Object *obj)
{
    ...
    if (currentRepair)
    {
        ...
        if (currentRepair->isKindOf(KINDOF_BRIDGE_TOWER) && obj->isKindOf(KINDOF_BRIDGE_TOWER))
        {
...
void DozerAIUpdate::internalTaskCompleteOrCancelled(DozerTask task)
{
    switch (task)
    {
        ...
        case DOZER_TASK_REPAIR:
        {
            ...
            //		BridgeTowerBehaviorInterface *btbi = BridgeTowerBehavior::getBridgeTowerBehaviorInterfaceFromObject( obj
            //); 		DEBUG_ASSERTCRASH( btbi, ("Unable to find bridge tower behavior interface") ); 		Object *bridge =
            //TheGameLogic->findObjectByID( btbi->getBridgeID() ); 		DEBUG_ASSERTCRASH( bridge, ("Unable to find bridge
            //object") ); 		bridge->clearStatus( OBJECT_STATUS_UNDERGOING_REPAIR );
            //
            //	}  // end if
            //
            //}  // end if
...
{
    if (getObject()->getControllingPlayer() && getObject()->getControllingPlayer()->getPlayerType() == PLAYER_COMPUTER)
    {
