...
StateReturnType DozerActionMoveToActionPosState::update(void)
{
	...
	if (m_task == DOZER_TASK_REPAIR && currentRepairer != INVALID_ID && currentRepairer != dozer->getID()) // oops I guess
																																																				 // someone beat me
																																																				 // to it!
	{
		if (ai)
...
StateReturnType DozerActionDoActionState::update(void)
{
	...
	switch (m_task)
	{
		...
		{
			// GS Moved this inside Build, since you are allowed to Repair things that are not your player (canRepairObject handles
			// it)
			if (dozer->getControllingPlayer() != goalObject->getControllingPlayer()) // Yipes, SOmehow I have changed sides in mid
																																							 // build!
				return STATE_FAILURE;
