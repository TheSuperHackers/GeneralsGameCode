  ...
  if (production->m_percentComplete >= 100.0f)
  {
    ...
    if (production->m_type == PRODUCTION_UNIT)
    {
      ...
      if (exitInterface)
      {
        ...
        //
        Int numberToTry =
            production->getProductionQuantityRemaining(); // this will change midloop, so it can't be the For clause
        for (int i = 0; i < numberToTry; i++)
        {
          ...
          if (exitDoor != DOOR_NONE_AVAILABLE)
          {
            ...
            if (d->m_numDoorAnimations == 0 || door == NULL || door->m_doorWaitOpenFrame != 0)
            {
              ...
              {
                // Call the voice created for the 1st object -- because it's possible to create multiple
                // objects like redguards!
