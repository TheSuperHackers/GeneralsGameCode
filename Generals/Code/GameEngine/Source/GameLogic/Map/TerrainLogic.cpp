...
{
    return (
        pt->x >= clipRegion->lo.x && pt->y >= clipRegion->lo.y && pt->x <= clipRegion->hi.x && pt->y <= clipRegion->hi.y);
}
...
void TerrainLogic::setWaterHeight(const WaterHandle *water, Real height, Real damageAmount, Bool forcePathfindUpdate)
{
    ...
    if (damageAmount > 0.0f && height > previousHeight)
    {
        ...
        Real maxDist =
            sqrt(affectedRegion.width() * affectedRegion.width() + affectedRegion.height() * affectedRegion.height());
    ...
    if (version >= 2)
    {
        ...
        for (UnsignedInt i = 0; i < m_numWaterToUpdate; ++i)
        {
            ...
            else if (xfer->getXferMode() == XFER_LOAD)
            {
                ...
                {
                    DEBUG_CRASH(
                        ("TerrainLogic::xfer - Unable to find polygon trigger for water table with trigger ID '%d'",
                         triggerID));
