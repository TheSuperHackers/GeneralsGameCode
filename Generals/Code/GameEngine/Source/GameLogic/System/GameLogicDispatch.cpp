...
void GameLogic::logicMessageDispatcher(GameMessage *msg, void *userData)
{
	...
	switch (msgType)
	{
		...
		{
			// DEBUG_ASSERTCRASH(msg->getArgumentCount() == 1 || msg->getArgumentCount() == 2, ("%d arguments to MSG_NEW_GAME",
		...
		case GameMessage::MSG_REMOVE_BEACON:
		{
...
#if RETAIL_COMPATIBLE_AIGROUP
			thisPlayer->getCurrentSelectionAsAIGroup(allSelectedObjects); // need to act on all objects, so we can hide teammates'
																																		// beacons.
#else
			thisPlayer->getCurrentSelectionAsAIGroup(allSelectedObjects.Peek()); // need to act on all objects, so we can hide
																																					 // teammates' beacons.
#endif
			if (allSelectedObjects)
			{
				...
				for (VecObjectID::const_iterator it = selectedObjects.begin(); it != selectedObjects.end(); ++it)
				{
					...
					if (beacon)
					{
						// TheSuperHackers @bugfix Prevent runtime crashing when a beacon is no longer associated with an initialized
						// player.
		...
		case GameMessage::MSG_CREATE_TEAM9:
		{
			...
			// TheSuperHackers @tweak Stubbjax 17/08/2025 The local player processes this message in CommandXlat for immediate
			// assignment.
