...
static void doSetRallyPoint(Object *obj, const Coord3D &pos)
{
    ...
    if (isLocalPlayer)
    {
        ...
        UnicodeString info;
        info.format(TheGameText->fetch("GUI:RallyPointSet"), obj->getTemplate()->getDisplayName().str());
...
void GameLogic::closeWindows(void)
{
    ...
    GameWindow *window =
        TheWindowManager->winGetWindowFromId(NULL, TheNameKeyGenerator->nameToKey("OptionsMenu.wnd:OptionsMenuParent"));
    if (window)
...
void GameLogic::clearGameData(Bool showScoreScreen)
{
    ...
    if ((!isInShellGame() || !isInGame()) && showScoreScreen && !TheGlobalData->m_headless)
    {
        ...
        TheShell->showShell(
            FALSE); // by passing in false, we don't want to run the Init on the shell screen we just pushed on
...
void GameLogic::logicMessageDispatcher(GameMessage *msg, void *userData)
{
    ...
    DEBUG_ASSERTCRASH(
        thisPlayer,
        ("logicMessageDispatcher: Processing message from unknown player (player index '%d')", msg->getPlayerIndex()));
            ...
            && msg->getType() <= GameMessage::MSG_END_NETWORK_MESSAGES)
        {
            if (msg->getType() != GameMessage::MSG_LOGIC_CRC && msg->getType() != GameMessage::MSG_SET_REPLAY_CAMERA)
            {
                currentlySelectedGroup =
                    TheAI->createGroup(); // can't do this outside a game - it'll cause sync errors galore.
                CRCGEN_LOG(
                ...
                // We can't issue commands to groups that contain units that don't belong to the issuing player, so pretend
                // like there's nothing selected. Also, if currentlySelectedGroup is empty, go ahead and delete it, so that
                // we can skip any processing on it.
    ...
    switch (msgType)
    {
        ...
        {
            // DEBUG_ASSERTCRASH(msg->getArgumentCount() == 1 || msg->getArgumentCount() == 2, ("%d arguments to
            // MSG_NEW_GAME", msg->getArgumentCount()));
        ...
        case GameMessage::MSG_COMBATDROP_AT_OBJECT:
        {
            ...
            if (currentlySelectedGroup)
                currentlySelectedGroup->groupCombatDrop(targetObject, *targetObject->getPosition(), CMD_FROM_PLAYER);
        ...
        case GameMessage::MSG_DO_SPECIAL_POWER_AT_LOCATION:
        {
            ...
            else
            {
                ...
                {
                    currentlySelectedGroup
                        ->groupDoSpecialPowerAtLocation(specialPowerID, &targetCoord, INVALID_ANGLE, objectInWay, options);
                }
            }
            break;
        ...
        case GameMessage::MSG_EXIT:
        {
            ...
            // Just like Enter, Exit needs to know the thing to exit.  This can no longer be assumed because of the Tunnel
            // system. If you do not specify the thing to Exit, it will Exit the thing it thinks it is in.  For a tunnel
            // network, that will be the specific Tunnel it entered.  (Scripts can talk directly to the guy to say Get Out
            // Regardless)
        ...
        case GameMessage::MSG_QUEUE_UPGRADE:
        {
            const UpgradeTemplate *upgradeT =
                TheUpgradeCenter->findUpgradeByKey((NameKeyType)(msg->getArgument(1)->integer));
            if (!upgradeT) // sanity
                break;
...
#endif
            const UpgradeTemplate *upgradeT =
                TheUpgradeCenter->findUpgradeByKey((NameKeyType)(msg->getArgument(0)->integer));
        ...
        case GameMessage::MSG_QUEUE_UNIT_CREATE:
        {
            ...
            if (pu == NULL)
            {
                DEBUG_ASSERTCRASH(
                    0,
                    ("MSG_QUEUE_UNIT_CREATE: Producer '%s' doesn't have a unit production interface",
        ...
        case GameMessage::MSG_DOZER_CONSTRUCT_LINE:
        {
            ...
            {
                TheBuildAssistant
                    ->buildObjectNow(constructorObject, place, &loc, angle, constructorObject->getControllingPlayer());
        ...
        case GameMessage::MSG_PLACE_BEACON:
        {
            ...
            if (thing && !TheVictoryConditions->hasSinglePlayerBeenDefeated(thisPlayer))
            {
                ...
                if (thisPlayer->getRelationship(ThePlayerList->getLocalPlayer()->getDefaultTeam()) == ALLIES
        ...
        case GameMessage::MSG_REMOVE_BEACON:
        {
            ...
            thisPlayer->getCurrentSelectionAsAIGroup(
                allSelectedObjects); // need to act on all objects, so we can hide teammates' beacons.
#else
            thisPlayer->getCurrentSelectionAsAIGroup(
                allSelectedObjects.Peek()); // need to act on all objects, so we can hide teammates' beacons.
#endif
            if (allSelectedObjects)
            {
                ...
                for (VecObjectID::const_iterator it = selectedObjects.begin(); it != selectedObjects.end(); ++it)
                {
                    ...
                    if (beacon)
                    {
                        // TheSuperHackers @bugfix Prevent runtime crashing when a beacon is no longer associated with an
                        // initialized player.
        ...
        {
            if (TheRecorder->isPlaybackMode() && TheGlobalData->m_useCameraInReplay
            ...
            if (TheNetwork)
            {
                ...
                for (Int i = 0; i < MAX_SLOTS; ++i)
                {
                    if (thisPlayer->getPlayerType() == PLAYER_HUMAN
                        && TheNetwork->getPlayerName(i) == thisPlayer->getPlayerDisplayName())
                    {
                        slotIndex = i;
                        break;
                    }
    ...
    if (currentlySelectedGroup && TheRecorder->isPlaybackMode() && TheGlobalData->m_useCameraInReplay
        && TheControlBar->getObserverLookAtPlayer() == thisPlayer /*&& !TheRecorder->isMultiplayer()*/)
    {
        const VecObjectID &selectedObjects = currentlySelectedGroup->getAllIDs();
