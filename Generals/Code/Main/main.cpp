...
static void configureSDLWindowFromGlobals(SDL_Window *window)
{
	...
	SDL_SetWindowTitle(window, "Command and Conquer Generals");
...
static SDL_Window *createWindowFromSettings()
{
	...
	SDL_Window *window = SDL_CreateWindow("Command and Conquer Generals", width, height, flags | SDL_WINDOW_HIDDEN);
	if (!window)
	{
...
static SDL_Window *createSplashScreenWindow()
{
	...
	SDL_Window *window = SDL_CreateWindow("Command and Conquer Generals", width, height, flags);
	if (!window)
	{
